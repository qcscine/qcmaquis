...
#define TUPLE_TYPE1 (std::complex<double>,z)
...
#define BOOST_PP_DEF(z, I, _) \
    inline void gemm(const char *transa, const char *transb, const int  *m, const int *n, const int *k, \
                     ...
                     const BOOST_PP_TUPLE_ELEM(2,0,TUPLE_TYPE ## I) *beta, BOOST_PP_TUPLE_ELEM(2,0,TUPLE_TYPE ## I) *c, const int *ldc) \
                { \
                    BOOST_PP_CAT(BOOST_PP_TUPLE_ELEM(2,1,TUPLE_TYPE ## I),gemm_)(transa, transb, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc); \
                } \
    inline void axpy(const int *n, const  BOOST_PP_TUPLE_ELEM(2,0,TUPLE_TYPE ## I) *alpha, const BOOST_PP_TUPLE_ELEM(2,0,TUPLE_TYPE ## I) *x, const int *incx, \
                     BOOST_PP_TUPLE_ELEM(2,0,TUPLE_TYPE ## I) *y, const int *incy) \
                { \
                    BOOST_PP_CAT(BOOST_PP_TUPLE_ELEM(2,1,TUPLE_TYPE ## I),axpy_)(n, alpha, x, incx, y, incy); \
                } \
    inline void gesvd(const char* jobu, const char* jobvt, const int* m, \
                      const int* n, BOOST_PP_TUPLE_ELEM(2,0,TUPLE_TYPE ## I) *a, const int* lda, double * s, \
                      BOOST_PP_TUPLE_ELEM(2,0,TUPLE_TYPE ## I)* u, const int* ldu, BOOST_PP_TUPLE_ELEM(2,0,TUPLE_TYPE ## I)* vt, const int* ldvt, \
                      BOOST_PP_TUPLE_ELEM(2,0,TUPLE_TYPE ## I)* work, const int* lwork, double* rwork, int* info ) \
                { \
                    BOOST_PP_CAT(BOOST_PP_TUPLE_ELEM(2,1,TUPLE_TYPE ## I),gesvd_)(jobu, jobvt, m, n, a, lda, s, u, ldu, vt, ldvt, work, lwork, BOOST_PP_EXPR_IF(I,rwork) COMMA_(I) info); \
                }
    /**/
BOOST_PP_REPEAT(TUPLE_TYPE_CNT, BOOST_PP_DEF, _)
#undef BOOST_PP_DEF
