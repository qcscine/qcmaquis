...
These new paradigms are always introduced by the new products deliver from the computer industry.  If we examined the two last decades, the most powerful
machines were based on shared memory architecture(CITE TOP 500), these machines were powerful but expensive and not  configurable, easily. Since, three revolutions appeared.
...
As the systems are became independents \df the address space is not shared among nodes,  parallel processes have to transmit data over an interconnecting network in order to access
data that other processes have updated. Several technologies were developed to allow communication between nodes (PVM, NX, Express, P4, \etc), one in particularly were developed as 
a standard; the Message Passing Interface (MPI). This interface has been developed by the participation of more 40 organizations, started working on the standard in 1992, delivered in 1994 for the first version and 1997 for 
the second version. Nowadays, it is the standard for  large grain scalability, it is available under several distributions  from the academic world and HPC constructors.
...
used  a main MPI layer, we can cite as popular   in a few scientist field :  {\sc Quantum ESPRESSO} \cite{QE-2009} in quantum chemistry, Abinit \cite{ABINIT-2005, ABINIT-2009} in quantum mechanic,
  ScaLAPACK \cite{SCALAPACK-1997} in linear  algebra. Of course, this short list is not exhaustive, and it can be fill up easily in every fields of science and engineering.
The limitation of this programming model (the scalability) will be the communication overhead and the workload imbalance among process.
...
The technology is available on several processor architecture (Power, X86, \etc). One more time, the programming 
model was modified. It is curious than multicore technology redirect the shared memory model on the sunset. 
To program shared memory model, a proved technology exists, it is the POSIX threads library. Two possibilities exist,
the first one, the programmer utilizes the library directly (pThreads) or he lets the compiler generates a multi-thread executables.  
Generally, the former option places the burden on the programmer, but when well done, it provides good performance.
On the other hand,  the second option  named OpenM, easy to program, it is based on preprocessor directive.
...
is the actual programming model in HPC \cite{EXASCALE-2010}.  Per example the performance of the ScaLAPACK library can be improved by a multithread BLAS library (MK, ESSLSMP or ACML). 
...
And then, the present evolution comes from the introduction of the graphic card into HPC, graphic card exists since the eightiees \cite{iSBX_1983}, the graphic card market 
was disconnected to the HPC market until a few years ago due to the incredible power of calculation of last generation of GPU. The introducing of the CUDA API allows a real
opportunity to exploit graphic card into a cluster.  To summarize the GPU ideology : it is the ability to performed thousand of threads simultaneously, where the thread applied a transformation (kernel)
on a pixel. Translate to a scientist computational problem, the pixel becomes an entry a tensor (mathematic) or a particle  (kinetic fluid dynamic), \etc.
...
As in the CUDA framework, in Ambient developers write kernels which are executed dynamically, and independently if needed, all other the cluster in function of the  resource.
The needed communications  are assured by a nested MPI layer away from the users. In this paper, we are going to describe the difference of workflow between an Ambient  and a CUDA program, 
describe the main functionality  of Ambient and perform several  benchmarks.  Until the end of this paper, we will apply and illustrate our framework on basic linear algebra problems.
...
\item Execution of logistic kernel: argument's work groups decomposition according to workDim [INSERT OPENCL REF HERE].
