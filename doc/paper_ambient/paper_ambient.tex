...
\begin{itemize}
\item Line 1:  the signature of the logistic kernel, showing the input/ouput by the keyword\texttt{const} (???). In this example, there are only two matrices, but additional arguments like float numbers, integers \etc can be added.
...
We have introduced a keyword (as the \texttt{const} keyword (???)) named \texttt{pinned}. This keyword indicates to the framework that the kernel must be executed over all Ambient work groups. Inside a kernel, we employ several Ambient API, analogous to CUDA, to obtain key information about the work groups 
...
The delegate object inside the packet manager class contains a series of functions piled up in a stack, such as forwarding requests, 
layout, and block of data (???). As this delegate is inside the infinite loop, the delegate execution 
will propagate the packets process by process after every iteration (as shown in Fig. \ref{EXECUTION}). When the delegate execution is over, a finite state machine
