...
In the last decade, the distributed memory were confirmed like the technology in HPC architecture machine, 100 \% of the top 500 are now based on this model, as the large scalability applications
...
quantum mechanic,   ScaLAPACK \cite{SCALAPACK-1997} in linear  algebra or SMILE in hypersonic rarefied flow \cite{SMILE-2005}. Of course, this short list is not exhaustive, and it can be fill up easily in every fields of 
...
This technology is now available on all  processor architecture (Power, X86, ARM, \etc). One more time, the programming 
...
The programmers have two possibilities. They utilize the library directly (pThreads) or he lets the compiler generates a multi-thread executables.  
Generally, the former option places the burden on the programmer, but when well done, it provides good performance.
On the other hand,  the second option more knew under the  name OpenMP  \cite{openmp:2002}, easier,  is based on  preprocessor directive. This second option
...
This programming model becomes popular, it is now integrated natively in all compilers and it allows hybrid programs. Hybrid mode MPI/OpenMP
...
In this paper, we proposed an alternative approach of programming.  We developed a powerful oriented object framework named Ambient, that transposed  the CUDA ideology to a cluster. 
As in the CUDA framework, in Ambient, the developer writes kernels which are executed in parallel, dynamically, and independently if needed, all other the resource of the cluster.
The needed communications  are assured by a nested MPI layer away from the users. From the user point of view, the objective is to write  parallel program without interacting the MPI API.

In this paper, we are going to describe the difference of workflow between an Ambient  and a CUDA program,  describe the blue print of Ambient and evaluated on several  benchmarks.  
Until the end of this paper, we will apply and illustrate our framework on basic linear algebra problems.
...
As soon as Ambient encounters the flush signal (contrary to CUDA, the programmer orders the execution   as in queue model) kernel
...
First is the object's lifetime. As we employ the technique of accumulated execution, we have to ensure that the objects that were created on the stack are still alive at the time of the actual execution. For this purpose we introduced \texttt { livelong } class that forces derived data types to duplicate themselves in heap memory upon construction if they were allocated on stack. Using this duplicate we can ensure that the object will stay alive until the moment of execution of the actual kernels using this data. 
