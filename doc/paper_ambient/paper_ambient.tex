...
Before presenting various elements of Ambient in more details, here we describe the
workflow of execution for an Ambient-parallelized application in general terms. 
As the Ambient computational architecture derives from CUDA, we will show a simplified workflow of CUDA
 vs Ambient enabled applications execution side-by-side:
...
\begin{itemize}
\item Execution of logistics kernel: allocation of device (MPI group) according to specified request. 
\item Execution of logistics kernel: argument's work groups decomposure according to workDim.
\item Execution of logistics kernel: selected argument's work groups mapping onto device (MPI processes).
...
Generally, Ambient enabled application operates with abstract types that already have tiled memory structure. Upon a kernel operation 
call, the Ambient framework stacks the operation for later execution and proceeds to the following part of the serial code. This design
enables high degree of original serial code's reusage while providing powerfull instrumentation capabilities to manage resources and tune performance 
of the overall application.

When the framework encounters the signal to flush the stack of kernels - the actual execution starts. Framework's engine goes through the accumulated stack 
and executes all independent operations at once. Executing an operation first the engine invokes the associated logistics kernel and allocates specified
...
simply being mapped onto threads of MPI process as if they were GPU SMP (mainly, the difference is the user's control over processes/work groups mapping).
