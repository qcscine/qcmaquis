...
[REPHRASED THIS SECTION. I THINK WE SHOULD EXPAND IT A LITTLE BIT, EXPLAINING
FOR EXAMPLE WHERE IN THE LATER SECTIONS THE VARIOUS ELEMENTS WILL BE EXPLAINED
(LIKE I DID WITH "PINNED" AND "PLAYOUT", AND GENERALLY EXPLAIN A LITTLE MORE WHAT THE
BROAD FEATURES AND CAPABILITIES OF AMBIENT ARE (E.G. BOOTSTRAP PARALLELIZATION OF EXISTING
SERIAL CODE,]
Before presenting the various elements of Ambient in more detail, we want to describe the
workflow of execution for an Ambient-parallelized application in general terms. 
As the Ambient approach derives from CUDA, we consider a simplified workflow of CUDA
 and Ambient-application's execution side-by-side:
	...
	\begin{center}
	 	\begin{tabular}{l l  l l} % L L  L L 
                        \hline 
                            ...
                            $\diamond$ & Kernel encountered                                  &  $\diamond$ & Kernel-pair (logistics/computa- \\
                               & & & tional) encountered \\
...
As soon as Ambient encounters the flush signal (called \texttt{playout()}, see section [APPROPRIATE CITATION HERE]), kernel
execution is initiated, along the following sequence:

\begin{itemize}
\item Execution of logistics kernel: allocation of device (MPI group) according to specifics of request. 
\item Execution of logistics kernel: argument's work groups (tiles) marking with workDim. [DON'T KNOW WHAT IS MEANT HERE, CANNOT REFORMULATE]
\item Execution of logistics kernel: selected argument's workgroups  (see keyword \texttt{pinned}, section [APPROPRIATE CITATION HERE])
mapping onto device (MPI processes).
...
are available for a kernel's launch, the data becomes the key factor around which we evolve Ambient's design.
In particular, in order to control the data movements and resource usage, we introduce the logistics kernel and make kernel calls non-blocking. 

In general, Ambient applications operate with abstract types that already have tiled memory structure. Upon encountering a kernel operation 
call, the Ambient framework stacks it for later execution, and proceeds in this way throughout the code. This ideology of design
allows a high degree of code re-usage from the original serial code, while at the same time providing both flexible and automatized
tools to manage resources and thus  tune performance.

When the framework encounters the signal to flush the stack of kernels - the actual execution starts, going through the accumulated stack of operations 
and executes the independent ones at once. When executing one operation, the engine invokes first the associated logistics kernel and allocates a specified
amount of resources for this operation. Next, the engine marks argument's tiling using specified parameters and maps resulting work groups onto allocated
processes in a user specified way. Now, the engine can freely transfer the tiled data from previous owners to the allocated processes changing
the tile size if needed. After the transfer is complete the computational kernel kicks in, behaving in almost the same way as in CUDA, with work groups
simply being mapped onto threads of MPI process as if they were GPU SMP (the main difference being that the user is in control as to which process owns which workgroup).

In the following sections, we expand on the role and functionalities of the different elements listed here.
...
sent message  requires a \texttt{mpi\_test} to check whether the packet can be received.  As we avoid synchronization instructions,
...
data. 
 ...
 For the packet manager to be adaptable to any communication pattern, it consists of two parts:
the delegate pattern and a finite state machine (FSM), both are placed inside this loop.
...
The delegate pattern (from C\#) encapsulates  methods, behaving similar to a function pointer in C++,  but allowing for safer execution.
The delegate object inside the packet manager class contains a series of functions pile up in a stack, such as forwarding requests, 
layout and block of data. As this delegate is inside the infinite loop, the delegate execution 
will propagate the packets process by process after every iterations (as shown on Fig. \ref{EXECUTION}). When the delegate execution is over, a finite state machine
checks the states of the packets (arrived or not).  If all packets (data packet, layout packet, etc.) have been received, the FSM breaks the infinite loop.
