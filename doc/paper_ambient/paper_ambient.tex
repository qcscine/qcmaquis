...
\item Execution of logistic kernel: argument's work groups decomposure according to workDim.
...
and executes all independent operations at once. Executing an operation first the engine invokes the associated logistic kernel and allocates specified
...
CPU core has much more additional capabilities comparing to the one of a GPU and therefore we can't simply downcast the CPU core to the GPU one.
Thus, the scope of the data for a single CPU (on which a single Ambient's thread typically runs) in Ambient is rather comparable to the capacity of CUDA's thread block or an OpenCL work group, the typical block size is $ 128 \times 128$. Though, the user has full control of the size and can adapt it for the needs of target problem. Like in CUDA, blocks (i.e. groups of threads) and the grid of blocks can be formed in Ambient. A basic comparison between CUDA and Ambient is shown in the table \ref{ANALOGY}.
...
Thus, every operations will have an associated logistic and computational kernel \eg multiplication, addition, or more sophisticated linear algebra operations
...
\begin{itemize}
\item Line 1:  the signature of the logistic kernel, showing the input/ouput by the keyword\texttt{const}. In this example there are onlz three matrices, but additional arguments like float number, integer \etc can be added.
...
by the pairs of logistic/computational kernels. 
...
Conventionally, these three operations would be executed in parallel one after another. Transposing into Ambients' conception means associating each operation with a logistic and a computational kernel. Considering the first example of example (\ref{3OPERATIONS}),  a user would invoke the following \texttt{push} function:
...
\item The work group is in the memory space of the present MPI-group (previously created by the associated logistic kernel). 
...
algebra operations (associated to logistic and computational kernel).This matrix class derived from an other who assured the connection to Ambient.
