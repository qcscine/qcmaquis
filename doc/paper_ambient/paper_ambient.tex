...
In this section we describe the workflow of Ambient applications execution in general terms.
As the Ambient approach derives from CUDA - first we will look at the simplified workflow of CUDA and Ambient-application's execution:
	 	...
	 	\begin{tabular}{l l  l l} % L L  L L 
                        \hline 
                            ...
                            $\diamond$ & Kernel encountered                                  &  $\diamond$ & Kernel-pair (logistic/computa- \\
                               ...
                               $\diamond$ &  Thread blocks mapping onto de-  &   $\diamond$  &  Serial code   \\
                                & vice (GPU SMPs) &  & \\
                               $\diamond$ &  Execution of kernel: GPU SMP  &  $\diamond$&Stack flush signal encountered :  \\
                               &  executes kernel on every thread& &   execution of kernels  \\
                               &  of corresponding thread blocks  & & \\
                                 $\diamond$  &  Serial code &  & \\
                                  \hline
        		 \end{tabular}  
              \caption{Workflow analogy between CUDA and Ambient-application's execution} \label{WORKFLOW}
	\end{center}
\end{table}

When the Ambient's flush signal indicates the execution of kernels. It is performed by the followings sequences:

\begin{itemize}
\item Execution of logistic kernel: allocation of device (MPI group) according to the request. 
\item Execution of logistic kernel: argument's workgroups (tiles) marking with workDim.
\item Execution of logistic kernel: selected (pinned) argument's workgroups mapping onto device (MPI processes).
\item Transfer of workgroups to the target processes.
\item Execution of computational kernel: MPI process executes kernel thread for work items of assigned work groups.
\end{itemize}

The main reason of the workflow difference comparing to CUDA is the scale of available resources. 
As we can use unlimited amount of resources for a kernel's launch - the data becomes the key factor around which we evolve Ambient's design.
In particular, in order to control the data movements and resource usage we introduce the logistic kernel and making kernel calls non-blocking. 

In general, Ambient application operates with abstract types that already have tiled memory structure. Upon the kernel operation 
call the Ambient framework stacks the operation for later execution and proceeds to the rest of the code.

When the framework encounters the signal to flush the stack of kernels - the actual execution starts. Framework goes through accumulated stack 
and executes independent operations at once. When executing one operation first the engine goes through the logistic kernel and allocates specified
 amount of resources for this operation. Next the engine marks argument's tiling using specified parameters and maps resulting workgroups onto allocated processes in a user specified way. 
Now engine can freely transfer the tiled data from previous owners to the allocated processes changing the tile size if needed. After the transfer is complete the computational kernel kicks in. 
The computational kernel behaves almost the same way as in CUDA and workgroups are simply being mapped onto threads of MPI process as if it were GPU SMP (the main difference here lies in user being in control of particular process workgroup's ownership).

In further sections we are going to review mechanisms that we have used here in more details.
...
In a typical HPC application, the programming model imposes a  parallel execution of every function of the code, with data granularity imposing the details 
...
are stacked up in a list first, and are executed afterwards according to the model of the logistics/computation kernel. This model is inspired by computer architecture design, i.e.
...
by the pairs of logistics/computational kernels. 
...
Conventionally, these three operations would be executed in parallel one after another. Transposing into Ambients' conception means associating each operation with a logistics and a computational kernel. Considering the first example of example (\ref{3OPERATIONS}),  a user would invoke the following \texttt{push} function:

\begin{eqnarray}
     \texttt{push(Addition\_logistics, Addition\_computional, \textit{A}, \textit{B})},
\end{eqnarray} 

with the appropriate logistics and computational kernels \texttt{Addition\_logistics} and \texttt{Addition\_computational}  (c.f. table \ref{LKERNEL} and \ref{CKERNEL}), followed by the arguments \texttt{\textit{A}} and \texttt{\textit{B}}. In this way operations, such as those in (\ref{3OPERATIONS}), will stack up during execution of the application. At any time then, the operations on the stack can be executed using the basic Ambient instruction
...
Overall, the Ambient approach combines two levels of parallelism: that of each computational kernel and that of several kernels executed in parallel by assigning them to different MPI groups.  
...
pushing the required kernels on the top of stack. When the \texttt{playout()} function is called line 12, the engine will execute the stack. By resolving dependency, it will create the directed acyclic graph of the application naturally. 
\begin{table}[t] 
	\begin{center}
	 	\begin{tabular}{l l }
                           \hline 
...
performance. In Ambient, the data layout inside each work group necessitates additional metadata to coordinate the execution of logistics and 
...
To assure proper execution of the framework, several additional type of packets are  available, all derived from a standard packet. (MORE INFO HERE. OTHERWISE LAST
...
(ref. \ref{CURRENT}), where \textit{A} has to be a matrix  compatible with Ambient (REFERENCE HERE TO SECTION WHERE WE EXPLAIN WHAT AMBIENT-COMPATIBLE
MATRICES MUST LOOK LIKE), the indices $i$ and $j$ being the coordinates of the work group (Fig. \ref{MPIGROUP}). 
...
\item The work group is in the memory space of the present MPI-group (previously created by the associated logistics kernel). 
...
the that process, which in turn sends the requested workgroup to the requesting process. 
\item The work group is held inside another MPI-group. As previously, the process which executes the computational kernel sends a query to the master process of its MPI group,
...
identified by a singleton pattern. For the packet manager to be adaptable to any communication pattern, it consists of two parts:
...
sent message  requires a MPI test to check whether the packet can be received.  As we avoid synchronization instructions,
...
data. Both the delegate pattern and the FSM are placed inside this loop.
...
The delegate pattern (from C\#) encapsulates a method, behaving similar to a function pointer in C++,  but allowing for safer execution.
The delegate object inside the packet manager class contains a series of operations pil in a stack, such as forwarding requests, 
data communication and communication of changes in MPI group layout (i.e. block distribution). As this delegate is inside the infinite loop, the packets
will propagate process by process after every iterations (as shown on Fig. \ref{EXECUTION}). After the delegate execution is done, a finite state machine
checks the states of the packets.  If all packets (data packet, layout packet, etc.) have been received, the FSM (Fig. \ref{FSM}) breaks the infinite loop.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%% END PACKET MANAGER 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%% BEGIN ABSTRACT PARALLEL DATATYPES 
\subsection{Abstract parallel datatypes} 

To assure the largest scope to Ambient, we designed the framework on the data structures and not a specific problem. Thus, we enlarge the utilization of the framework
for a various set of applied computing fields. The connection  between Ambient and the \guillemotleft outside world \guillemotright  is done by an intermediate class that assure the
connectivity between a class and ambient.

For linear algebra, we designed a matrix class, we contained usual data members for describing the object (number of raws, number of columns, \dots) and  methods for linear
algebra operations (associated to logistics and computational kernel).This matrix class derived from an other who assured the connection to Ambient.
When an instance of the matrix class is created, the inheritance mechanics will call constructors, methods in the mother class that  will 
structure  the memory (tile  layout).

Therefore, we can extend Ambient for new problems. Per example, in fluid mechanics the  two-dimensional finite difference method is very popular for solving the Navier-Stokes  differential equations.
These equations describes the parameters of the \guillemotleft fluid particle \guillemotright. In this case Ambient will discretize the space and assure, as for a matrix,  the distribution
 of the data inside the memory. Thus, a fluid particle class will contain enough information to assure a good discretization in the cartezian space (as the number of raws and columns inside the matrix class). 
...
The algorithm of our implementation is based on the classical method [CITATION NEEDED] using a column order distribution for the matrices (c.f. table \ref{AMBIENTDGEMM} 
...
Using the C++ language, the operations $\times$ and $=$ have been overloaded appropriately, now containing the "pushing" of the required logistics and computational kernels.
...
where  the precision factor $\epsilon$  is the \texttt{double} precision $10^{-16}$, indices  $S$ and $A$ represent the ScaLAPACK and the Ambient version, and $n$ is equal to 16 (decimal digits), the exponent of numerical precision. 
...
The first benchmark consists of evaluating the performance of our parallel matrix multiplication to the implementation into the ScaLAPACK-MKL. Both solvers are included as computational kernels
