...
{
    // Put a and b on the gpu and call a kernel to do the operation
    ...
    polynomial_vector_type::const_iterator it = a.begin();
    polynomial_vector_type::const_iterator it_b = b.begin();
    while( it != a.end() )
    {
        result += *it * *it_b;
        ++it;
        ...
        polynomial_vector_type apply(polynomial_vector_type const& v)
        {
            ...
            for(polynomial_vector_type::size_type  index = 0; index < v.size(); ++index)
            {
                    ...
                    )
                {
                    ...
                    {
                        result[index] += -1*monomial<large_int>(1,0)*v[index];
                    }
                }
