...
int main(int argc, char** argv) {
  try {
    ...
    model_parser<matrix, symm>(
        parms["lattice_library"], parms["model_library"], model_parms, lattice,
        model
    );
    ...
    /// Initialize MPS
    MPS<matrix, symm> mps(
        lattice->size(), parms["init_bond_dimension"], H.get_phys(),
        model->initc(model_parms), *(model->initializer(parms))
    );
    ...
    for (int sweep = 0; sweep < parms["nsweeps"]; ++sweep) {
      int exit_site =
          optimizer
              .sweep(sweep, Both, -1 /* starting site */, -1 /* runlimit */);
