...
#pragma omp barrier
...
#else
    {
      ...
      if (num_states2[rank] > 0) {
        for (int i = 0; i < L; ++i) assert(vmps2[rank][i].reasonable());
        ...
        vmps1[rank] = join(
            vmps1[rank], vmps2[rank], std::sqrt(num_states1[rank]),
            std::sqrt(num_states2[rank])
        );
        // compress & normalize
        vmps1[rank] = compression::l2r_compress(
            vmps1[rank], parms["init_bond_dimension"],
            parms["truncation_initial"]
        );
    ...
    for (size_t n = 0; n < nthreads; ++n) {
      if (num_states2[n] > 0) {
        mps = join(
            mps, vmps1[n], std::sqrt(num_states1[0]), std::sqrt(num_states1[n])
        );
        mps.normalize_left();
        mps = compression::l2r_compress(
            mps, parms["init_bond_dimension"], parms["truncation_initial"]
        );
 ...
 private:
  ...
  void build_fast() {
    ...
    while (it[0] != alllocal.end()) {
      ...
      for (size_t i = 0; i < L; ++i) N = SymmGroup::fuse(N, it[i]->first);
      ...
      if (N == initc) {
        ...
        std::vector<states_iterator>* tmp =
            new std::vector<states_iterator>(it);
#ifdef MAQUIS_OPENMP
#pragma omp task firstprivate(tmp \
)  // ugly because gcc doesn't support copy of complex objects
#endif
        {
...
#pragma omp taskwait
  ...
  void permutate_states(std::vector<states_iterator> its) {
    std::vector<local_state> state(L);
    do {
      for (size_t i = 0; i < L; ++i) state[i] = *(its[i]);
      add_state(state);
  ...
  MPS<Matrix, SymmGroup> state_mps(std::vector<local_state> const& state) {
    ...
    for (int i = 0; i < state.size(); ++i) {
      ...
      size_t news = (i == state.size() - 1)
                        ? 1
                        : phys[phys.position(state[i].first)].second;
      ...
      size_t b_in = left(state[i].first, curr_i[0].first) +
                    state[i].second * curr_i[0].second + curr_b;
  ...
  void add_state(std::vector<local_state> const& state) {
    ...
    size_t nstates1 = num_states1[rank], nstates2 = num_states2[rank],
           tstates = totstates[rank];

    MPS<Matrix, SymmGroup>& curr =
        (nstates1 < parms["init_bond_dimension"]) ? vmps1[rank] : vmps2[rank];
    size_t& nstates =
        (nstates1 < parms["init_bond_dimension"]) ? nstates1 : nstates2;
    ...
    if (nstates2 > parms["init_bond_dimension"]) {
      if (nstates1 == parms["init_bond_dimension"]) mps1.normalize_left();
      mps2.normalize_left();
      mps1 = join(mps1, mps2, std::sqrt(nstates1), std::sqrt(nstates2));
      // compress & normalize
      mps1 = compression::l2r_compress(
          mps1, parms["init_bond_dimension"], parms["truncation_initial"]
      );
