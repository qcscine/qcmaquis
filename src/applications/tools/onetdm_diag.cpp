  ...
  try {
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      for (std::size_t p = 0; p < L; ++p) {
        ...
        meas_op[type] =
            tag_handler->get_op(model.get_operator_tag(op_name, type));
        // std::cout << "number of blocks for operator at site " << p << ": "<<
        // meas_op[type].n_blocks() << std::endl;

        // generate MPO
        if (meas_op[type].n_blocks() > 0) {
          generate_mpo::MPOMaker<matrix, symm> mpom(
              lattice, identities, fillings
          );
          ...
          // std::cout << "label for site p " << p << ": "<<
          // lattice.get_prop<std::string>("label", p)  << std::endl;
        }
      }
      /// end of TODO
      ...
      for (mpo_map::const_iterator mit = mpos.begin(); mit != mpos.end();
           ++mit) {
        ...
        std::vector<MPS<matrix, symm>::scalar_type> dct =
            multi_expval(mps1, mps2, mit->second);
        match->second = dct[0];
      }
      ...
      for (result_type::const_iterator it = res.begin(); it != res.end();
           ++it) {
        /// debug print
        std::cout << "indices:" << it->first << it->first
      ...
      std::cout << "saving data for measurement " << measurement
                << " amount of data: " << vector_results.size() << std::endl;
