...
std::vector<std::vector<typename SymmGroup::charge> > parse_config(
    std::string file, std::vector<Index<SymmGroup> > const& site_dims
) {
  ...
  for (std::string line; std::getline(config_file, line);) {
    ...
    if (det.size() != site_dims.size())
      throw std::runtime_error(
          "The determinant length doesn't match the mps length\n"
      );
...
template <class Matrix, class SymmGroup>
typename Matrix::value_type extract_coefficient(
    MPS<Matrix, SymmGroup> const& mps,
    std::vector<typename SymmGroup::charge> const& det
) {
  typedef typename SymmGroup::charge charge;

  if (mps.length() != det.size())
    throw std::runtime_error(
        "extract_coefficient: length of mps != length of basis state\n"
    );

  charge total =
      std::accumulate(det.begin(), det.end(), SymmGroup::IdentityCharge);
  ...
  if (total != target) {
    ...
    throw std::runtime_error(
        "Determinant has wrong number of up/down electrons: " + ss.str()
    );
  }
  ...
  for (unsigned p = 1; p < mps.length(); ++p) {
    ...
    if (!mps[p].site_dim().has(site_charge)) return 0.0;
    ...
    if (b == data.left_basis().size()) return 0.0;

    Matrix const& sector_matrix = data[b];

    // determine voffset - row offset for site_charge in sector_matrix
    // if permformance is an issue, do not recalculate the ProductBasis on every
    // call
    ...
    unsigned vlen =
        mps[p - 1].col_dim().size_of_block(SymmGroup::fuse(sector, -site_charge)
        );
    ...
    for (unsigned rowcnt = 0; rowcnt < vlen; ++rowcnt)
      std::copy(
          sector_matrix.row(voffset + rowcnt).first,
          sector_matrix.row(voffset + rowcnt).second,
    ...
    if (coeff.num_cols() != site_block.num_rows()) {
      ...
      maquis::cout << "site " << p << ", selected " << vlen << "x" << hlen
                   << " block, offset " << voffset << ", sector_matrix "
                   << sector_matrix.num_rows() << "x"
                   << sector_matrix.num_cols() << std::endl;
  ...
  if (coeff.num_rows() != 1 || coeff.num_cols() != 1)
    throw std::runtime_error(
        "final coefficient is not a scalar: " +
...
void set_coefficient(
    ...
    std::vector<typename SymmGroup::charge> const& det,
    typename Matrix::value_type coefficient
) {
  typedef typename SymmGroup::charge charge;
  ...
  for (std::size_t p = 1; p < (L + 1) / 2; ++p) {
    ...
    maquis::cout << "coding site " << p << ", site_charge " << site_charge
                 << ", left_sector " << left_sector << std::endl;
    ...
    std::size_t vlen = mps[p - 1].col_dim().size_of_block(
        SymmGroup::fuse(left_sector, -site_charge)
    );
    std::size_t hlen = data.right_basis().size_of_block(left_sector);
    maquis::cout << "  site " << p << ", selected " << vlen << "x" << hlen
                 << " block, offset " << voffset << ", sector_matrix "
                 << sector_matrix.num_rows() << "x" << sector_matrix.num_cols()
                 << std::endl;

    // activate subblock
    left_offset += voffset;
    std::size_t col_continue =
        std::min(left_offset, sector_matrix.num_cols() - 1);
    ...
    if (p < (L + 1) / 2 - 1) {
      maquis::cout << "  activating site " << p << " at " << left_offset << ", "
                   << col_continue << std::endl;
  ...
  charge right_sector =
      SymmGroup::fuse(-det[L - 1], mps[L - 1].col_dim()[0].first);
  std::size_t right_offset = 0;

  mps[L - 1].make_right_paired();
  mps[L - 1].data(
  )[mps[L - 1].data().left_basis().position(right_sector)](0, 0) = 1;
  ...
  for (std::size_t p = L - 2; p >= (L + 1) / 2; --p) {
    ...
    maquis::cout << "coding site " << p << ", site_charge " << site_charge
                 << ", right_sector " << right_sector << std::endl;
    ...
    while (phys[sci].first > site_charge) {
      ...
      // maquis::cout << "adding offset for phys " << phys[sci].first << ", " <<
      // offset_charge << std::endl;
      hoffset += mps[p + 1].row_dim().size_of_block(offset_charge, true);
      sci++;
    }

    maquis::cout << "  site " << p << ", offset " << hoffset
                 << ", sector_matrix " << sector_matrix.num_rows() << "x"
                 << sector_matrix.num_cols() << std::endl;

    // activate subblock
    right_offset += hoffset;
    std::size_t row_continue =
        std::min(right_offset, sector_matrix.num_rows() - 1);
    ...
    maquis::cout << "  activating site " << p << " at " << row_continue << ", "
  ...
  if (left_offset >= code_block.num_rows()) {
    maquis::cout << "code_block " << left_sector << ": "
                 << code_block.num_rows() << "x" << code_block.num_cols()
                 << std::endl;
    throw std::runtime_error(
        "center left_offset too big: " +
        boost::lexical_cast<std::string>(left_offset) + "\n"
    );
  }
  if (right_offset >= code_block.num_cols())
    throw std::runtime_error("center right_offset too big\n");

  // code_block[code_block.left_basis().position(left_sector)](left_offset,
  // right_offset) = coefficient;
