...
void mps_print_ci_(MPS<Matrix, TwoU1PG> const& mps, std::string detfile) {
  ...
  std::vector<Index<TwoU1PG> > per_site,
      phys_dims = chem::detail::make_2u1_site_basis<Matrix, TwoU1PG>(
          L, Nup, Ndown, site_types
      );
  for (pos_t q = 0; q < L; ++q) per_site.push_back(phys_dims[irreps[q]]);

  // load the determinants
  std::vector<std::vector<charge> > determinants =
      parse_config<Matrix, TwoU1PG>(detfile, per_site);
  ...
  for (typename std::vector<std::vector<charge> >::iterator it =
           determinants.begin();
       it != determinants.end(); ++it)
    maquis::cout << "CI coefficient of det " << it - determinants.begin() + 1
                 << ": " << extract_coefficient(mps, *it) << std::endl;
...
template <>
void mps_print_ci(
    MPS<alps::numeric::matrix<double>, TwoU1PG> const& mps_in,
    std::string detfile
) {
...
template <>
void mps_print_ci(
    MPS<alps::numeric::matrix<double>, SU2U1PG> const& mps_in,
    std::string detfile
) {
  ...
  MPS<Matrix, TwoU1PG> mps =
      transform_mps<Matrix, SU2U1PG>()(mps_in, (N + S) / 2, (N - S) / 2);
