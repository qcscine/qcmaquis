...
BOOST_FIXTURE_TEST_CASE(MPSTensorFromEigen, BenzeneFixture) {
  ...
  auto mpsHF = MPS<matrix, SymmetryType>(
  ...
  auto normFromEigen =
      std::sqrt(mpsTensor.getEigenRepresentation().squaredNorm());
...
BOOST_FIXTURE_TEST_CASE(EigenFromMPSTensor, BenzeneFixture) {
  ...
  Eigen::Matrix<double, Eigen::Dynamic, 1> vectorInitialize =
      Eigen::Matrix<double, Eigen::Dynamic, 1>::Zero(numElements);
