...
BOOST_FIXTURE_TEST_CASE(Test_NmodeModels_Energy, NModeFixture) {
  ...
  maquis::DMRGInterface<std::complex<double>> interface_compact_1body(
      parametersWater
  );
  ...
  parametersWater.set("LATTICE", "watson lattice");
  parametersWater.set("L", 3);
  maquis::DMRGInterface<std::complex<double>> interface_paired_1body(
      parametersWater
  );
  parametersWater.set("nmode_num_basis", "6,6,6");
  parametersWater.set("init_basis_state", "0,0,0");
  ...
  maquis::DMRGInterface<std::complex<double>> interface_nu1_1body(
      parametersWater
  );
  interface_compact_1body.optimize();
  interface_paired_1body.optimize();
  interface_nu1_1body.optimize();
  double interface_compact_optimizedEnergy_1body =
      interface_compact_1body.energy().real();
  double interface_paired_optimizedEnergy_1body =
      interface_paired_1body.energy().real();
  double interface_nu1_optimizedEnergy_1body =
      interface_nu1_1body.energy().real();
  BOOST_CHECK_CLOSE(
      interface_compact_optimizedEnergy_1body,
      interface_paired_optimizedEnergy_1body, 1.0E-8
  );
  BOOST_CHECK_CLOSE(
      interface_compact_optimizedEnergy_1body,
      interface_nu1_optimizedEnergy_1body, 1.0E-8
  );
  BOOST_CHECK_CLOSE(
      interface_paired_optimizedEnergy_1body,
      interface_nu1_optimizedEnergy_1body, 1.0E-8
  );
  BOOST_CHECK_CLOSE(
      interface_compact_optimizedEnergy_1body, 4725.63505369, 1.0E-8
  );
  BOOST_CHECK_CLOSE(
      interface_paired_optimizedEnergy_1body, 4725.63505369, 1.0E-8
  );
