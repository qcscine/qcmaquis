...
std::vector<MPO<Matrix, grp> > getU(
    std::vector<Hamiltonian<Matrix, grp> > const& split_H, size_t length
) {
  //    double alpha = -dt;
  double alpha = 1;

  std::vector<MPO<Matrix, grp> > expMPO(
      split_H.size(), MPO<Matrix, grp>(length)
  );
...
int main(int argc, char** argv) {
  ...
  std::vector<Hamiltonian<Matrix, grp> > split_H_signed =
      separate_overlaps(H_signed);
