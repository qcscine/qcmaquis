...
double measure_local_trace(
    MPS<matrix, SymmGroup> const& mps,
    block_matrix<matrix, SymmGroup> const& op, size_t pos
) {
  // asuming mps is canonized at site pos!
  block_matrix<matrix, SymmGroup> dm =
      contraction::density_matrix(mps[pos], mps[pos]);
  block_matrix<matrix, SymmGroup> tmp;
  gemm(op, dm, tmp);
  return trace(tmp);
}

double measure_local_trace_2(
    MPS<matrix, SymmGroup> const& mps,
    block_matrix<matrix, SymmGroup> const& op, size_t pos
) {
  // asuming mps is canonized at site pos!
  block_matrix<matrix, SymmGroup> dm =
      contraction::density_matrix_2(mps[pos], mps[pos]);
  block_matrix<matrix, SymmGroup> tmp;
  gemm(op, dm, tmp);
  return trace(tmp);
}
...
BOOST_AUTO_TEST_CASE(obs_bosons_nmax2) {
  ...
  {  // measure density (at some positions)
    ...
    for (int i = 0; i < Nrep; ++i) {
      ...
      for (int i = 1; i < 4; ++i) BOOST_CHECK_CLOSE(meas[0], meas[i], 1e-8);
    }
  }
}
