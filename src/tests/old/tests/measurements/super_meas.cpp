...
std::vector<double> measure_local_multi(
    MPS<matrix, SymmGroup> const& mps,
    operator_selector<matrix, SymmGroup>::type const& ident,
    operator_selector<matrix, SymmGroup>::type const& op
) {
  ...
  std::vector<std::pair<op_vec, bool> > ops(
      1, std::make_pair(op_vec(1, op), false)
  );

  std::shared_ptr<lattice_impl> lat_ptr(new ChainLattice(mps.length()));
  Lattice lattice(lat_ptr);
  generate_mpo::CorrMaker<matrix, SymmGroup> dcorr(
      lattice, op_vec(1, ident), op_vec(1, ident), ops, -1
  );
...
BOOST_AUTO_TEST_CASE(density_trivial_init) {
  ...
  maquis::cout << std::endl
               << "** TESTING density_trivial_init **" << std::endl;
  int L = 7;
  ...
  MPS<matrix, SymmGroup> mps = state_mps<matrix>(
      state, std::vector<Index<SymmGroup> >(1, phys_rho), std::vector<int>(L, 0)
  );
  ...
  for (int p = 0; p < L; ++p) {
    maquis::cout << "site " << p << ": " << meas_dens[p] / nn << "  "
                 << meas_dens_multi[p] / nn << std::endl;
...
BOOST_AUTO_TEST_CASE(density_join_init) {
  ...
  MPS<matrix, SymmGroup> mps;
  {
    ...
    mps = state_mps<matrix>(
        state, std::vector<Index<SymmGroup> >(1, phys_rho),
        std::vector<int>(L, 0)
    );
  }

  {
    ...
    MPS<matrix, SymmGroup> tmp = state_mps<matrix>(
        state, std::vector<Index<SymmGroup> >(1, phys_rho),
        std::vector<int>(L, 0)
    );
    mps = join(tmp, mps);
...
BOOST_AUTO_TEST_CASE(density_coherent_init) {
  typedef operator_selector<matrix, SymmGroup>::type op_t;
  maquis::cout << std::endl
               << "** TESTING density_coherent_init **" << std::endl;
  ...
  MPS<matrix, SymmGroup> mps =
      coherent_init_dm<matrix>(coeff, phys_psi, phys_rho);
...
BOOST_AUTO_TEST_CASE(density_random_init) {
  ...
  default_mps_init<matrix, SymmGroup> initializer(
      parms, std::vector<Index<SymmGroup> >(1, phys_rho), C,
      std::vector<int>(L, 0)
  );
