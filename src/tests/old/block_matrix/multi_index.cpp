...
// };
// template<class SymmGroup> struct is_mystruct<std::vector<std::pair<typename
...
void test_iterator() {
  ...
  for (index_product_iterator<symm> it(vec);
       it != index_product_iterator<symm>(); it++) {
...
void test_multi_index() {
  ...
  std::vector<std::pair<MultiIndex<symm>::index_id, bool> > set1_left(2),
      set1_right(2);
  set1_left[0] = std::make_pair(s1, true);
  set1_left[1] = std::make_pair(s2, true);
  set1_right[0] = std::make_pair(s3, true);
  set1_right[1] = std::make_pair(s4, true);
  MultiIndex<symm>::set_id set1 = midx.create_set(set1_left, set1_right);

  std::vector<std::pair<MultiIndex<symm>::index_id, bool> > set2_left(2),
      set2_right(2);
  set2_left[0] = std::make_pair(s1, true);
  set2_left[1] = std::make_pair(s3, false);
  set2_right[0] = std::make_pair(s4, true);
  ...
  for (index_product_iterator<symm> it = midx.begin(); it != midx.end(); it++) {
    maquis::cout << *it << " = " << midx.get_coords(set1, *it) << " or "
                 << midx.get_coords(set2, *it) << std::endl;
  }
}
