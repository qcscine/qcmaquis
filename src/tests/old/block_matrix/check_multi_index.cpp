...
std::ostream& operator<<(
    std::ostream& os, std::pair<symm::charge, std::size_t> const& p
) {
  os << "(" << p.first << " : " << p.second << ")";
  return os;
}
...
std::ostream& operator<<(
    std::ostream& os, index_product_iterator<symm>::value_type const& v
) {
  // std::copy(v.begin(), v.end(), std::ostream_iterator<std::pair<symm::charge,
  // std::size_t> >(os, " "));
...
std::ostream& operator<<(
    std::ostream& os,
    std::pair<MultiIndex<symm>::coord_t, MultiIndex<symm>::coord_t> const& p
) {
  os << p.first << ", " << p.second;
  return os;
}
...
typedef std::vector<std::pair<
    std::pair<symm::charge, std::size_t>,
    std::pair<symm::charge, std::size_t> > >
    map_val_t;
typedef std::map<access_t, map_val_t> ret_map;

ret_map test_multi_index(
    Index<symm> const& ix1, Index<symm> const& ix2, Index<symm> const& phys
) {
  typedef std::vector<std::pair<MultiIndex<symm>::index_id, bool> >
      set_descriptor_t;
  ...
  set1_left[0] = std::make_pair(s1, true);
  set1_left[1] = std::make_pair(s2, true);
  set1_left[2] = std::make_pair(alpha, true);
  ...
  set2_right[0] = std::make_pair(s1, false);
  set2_right[1] = std::make_pair(s2, false);
  set2_right[2] = std::make_pair(beta, true);
  MultiIndex<symm>::set_id set2 = midx.create_set(set2_left, set2_right);

  set_descriptor_t set3_left(2), set3_right(2);
  set3_left[0] = std::make_pair(s1, true);
  set3_left[1] = std::make_pair(alpha, true);
  set3_right[0] = std::make_pair(s2, false);
  set3_right[1] = std::make_pair(beta, true);
  MultiIndex<symm>::set_id set3 = midx.create_set(set3_left, set3_right);

  ret_map ret;
  for (index_product_iterator<symm> it = midx.begin(); it != midx.end(); it++) {
...
ret_map test_usual_l(
    Index<symm> const& left_i, Index<symm> const& right_i,
    Index<symm> const& physical_i
) {
  ...
  ProductBasis<symm> right_both(
      physical_i, right_i,
      boost::lambda::bind(
          static_cast<charge (*)(charge, charge)>(symm::fuse),
          -boost::lambda::_1, boost::lambda::_2
      )
  );
  ProductBasis<symm> left_p(phys2_i, left_i);
  ProductBasis<symm> right_p(
      phys2_i, right_i,
      boost::lambda::bind(
          static_cast<charge (*)(charge, charge)>(symm::fuse),
          -boost::lambda::_1, boost::lambda::_2
    ...
    for (size_t r = 0; r < right_i.size(); ++r)
      for (size_t s1 = 0; s1 < physical_i.size(); ++s1)
        for (size_t s2 = 0; s2 < physical_i.size(); ++s2) {
          charge s_charge =
              symm::fuse(physical_i[s1].first, physical_i[s2].first);
          size_t s_out = phys2_i.find(s_charge);

          charge bp_l_charge = symm::fuse(
              physical_i[s1].first, left_i[l].first
          );  // both paired,  left
          charge bp_r_charge = symm::fuse(
              -physical_i[s2].first, right_i[r].first
          );  // both paired,  right
          charge lp_l_charge =
              symm::fuse(s_charge, left_i[l].first);  // left paired,  left
          charge lp_r_charge = right_i[r].first;      // left paired,  right
          charge rp_l_charge = left_i[l].first;       // right paired, left
          charge rp_r_charge =
              symm::fuse(-s_charge, right_i[r].first);  // right paired, right

          size_t bp_left_offset =
              left_both(physical_i[s1].first, left_i[l].first);
          size_t bp_right_offset =
              right_both(physical_i[s2].first, right_i[r].first);
          ...
          size_t out_phys_offset =
              phys_pb(physical_i[s1].first, physical_i[s2].first);

          for (size_t ss1 = 0; ss1 < physical_i[s1].second; ++ss1)
            for (size_t ss2 = 0; ss2 < physical_i[s2].second; ++ss2) {
              size_t ss_out =
                  out_phys_offset + ss1 * physical_i[s2].second + ss2;
              for (size_t rr = 0; rr < right_i[r].second; ++rr)
                for (size_t ll = 0; ll < left_i[l].second; ++ll) {
                  ...
                  vv[0] = std::make_pair(
                      std::make_pair(
                          lp_l_charge,
                          lp_left_offset + ss_out * left_i[l].second + ll
                      ),
                      std::make_pair(lp_r_charge, lp_right_offset + rr)
                  );
                  // right pairing
                  vv[1] = std::make_pair(
                      std::make_pair(rp_l_charge, rp_left_offset + ll),
                      std::make_pair(
                          rp_r_charge,
                          rp_right_offset + ss_out * right_i[r].second + rr
                      )
                  );
                  // both pairing
                  vv[2] = std::make_pair(
                      std::make_pair(
                          bp_l_charge,
                          bp_left_offset + ss1 * left_i[l].second + ll
                      ),
                      std::make_pair(
                          bp_r_charge,
                          bp_right_offset + ss2 * right_i[r].second + rr
                      )
                  );

                  maquis::cout << aa << " = " << vv[0] << " or " << vv[1]
                               << " or " << vv[2] << std::endl;
    ...
    for (size_t r = 0; r < right_i.size(); ++r)
      for (size_t s1 = 0; s1 < physical_i.size(); ++s1)
        for (size_t s2 = 0; s2 < physical_i.size(); ++s2) {
          ...
          charge pyr_charge =
              symm::fuse(-physical_i[s2].first, right_i[r].first);
          ...
          charge lp_l_charge = symm::fuse(
              physical_i[s1].first, pyl_charge
          );                                      // left paired,  left
          charge lp_r_charge = right_i[r].first;  // left paired,  right
          charge rp_l_charge = left_i[l].first;   // right paired, left
          charge rp_r_charge = symm::fuse(
              -physical_i[s1].first, pyr_charge
          );  // right paired, right
              ...
              for (size_t rr = 0; rr < right_i[r].second; ++rr)
                for (size_t ll = 0; ll < left_i[l].second; ++ll) {
                  ...
                  vv[0] = std::make_pair(
                      std::make_pair(
                          lp_l_charge,
                          lp_left_offset + ss1 * pyleft_i[pyl].second + pyll
                      ),
                      std::make_pair(lp_r_charge, lp_right_offset + rr)
                  );
                  // right pairing
                  vv[1] = std::make_pair(
                      ...
                      std::make_pair(
                          rp_r_charge,
                          rp_right_offset + ss1 * pyright_i[pyr].second + pyrr
                      )
                  );
                  // both pairing
                  vv[2] = std::make_pair(
...
int main() {
  ...
  for (ret_map::const_iterator it = multi_res.begin(); it != multi_res.end();
       ++it) {
    if (multi_right)
      multi_right = std::equal(
          it->second.begin(), it->second.end(), right_res[it->first].begin()
      );
    if (multi_left)
      multi_left = std::equal(
          it->second.begin(), it->second.end(), left_res[it->first].begin()
      );
    if (right_left)
      right_left = std::equal(
          right_res[it->first].begin(), right_res[it->first].end(),
          left_res[it->first].begin()
      );
  }

  maquis::cout << "multi_index is equal to right_first? "
               << ((multi_right) ? "yes" : "no") << std::endl;
  maquis::cout << "multi_index is equal to left_first? "
               << ((multi_left) ? "yes" : "no") << std::endl;
  maquis::cout << "right_first is equal to left_first? "
               << ((right_left) ? "yes" : "no") << std::endl;
}
