...
BOOST_AUTO_TEST_CASE(Test1) {
  ...
  //                             \n" "  0.185125251547             2     1     2
  //                             1 \n" "  0.782984788117E-13         2     2 2
  //                             1 \n" "  0.361001163519             1     1 2
  //                             2 \n" "  0.371320200119             2     2 2
  //                             2 \n" " -0.678487901790             1     1 0
  //                             0 \n" " -0.539801158857E-14         2     1 0
  //                             0 \n" " -0.653221638776             2     2 0
  //                             0 \n" "  0.176392403557             0     0 0
  //                             0 \n");
  ...
  for (auto&& s : symmetries) {
    ...
    // we don't have a map for the measurements yet, so we'll do it the stupid
    // way
    for (int i = 0; i < meas1.first.size(); i++)
      if ((meas1.first[i] == std::vector<int>{0, 0}) ||
          (meas1.first[i] == std::vector<int>{1, 1}))
        value += meas1.second[i];
    BOOST_CHECK_CLOSE(value, 2.0, 1e-7);

    // test 2-RDM
    const typename maquis::DMRGInterface<double>::meas_with_results_type&
        meas2 = interface.twordm();
