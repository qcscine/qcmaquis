...
// Allow differences in sign (phase) in the whole matrix if
// allow_phase_difference==true
template <class Meas>
void check_measurement_mat(
    const Meas& meas, const Meas& reference,
    bool allow_phase_difference = false, double threshold = 1.0E-16
) {
  // check sizes
  ...
  for (auto&& it = reference.first.begin(); it != reference.first.end(); it++) {
    auto index = std::distance(
        meas.first.begin(), std::find(meas.first.begin(), meas.first.end(), *it)
    );
...
// Class that inits a unique directory for checkpoints used in excited state
// tests and deletes it upon termination
class TestTmpPath {
 public:
  TestTmpPath()
      : p_(boost::filesystem::unique_path(
            boost::filesystem::temp_directory_path() /
            std::string("/qcmaquis_test_%%%%%%%%%%%%/")
        )) {
    try {
      boost::filesystem::create_directories(p_);
    } catch (...) {
      throw std::runtime_error(
          "Cannot create temporary directory at " + p_.string() + ".\n"
      );
    }
  ...
  ~TestTmpPath() {
    try {
      boost::filesystem::remove_all(p_);
    } catch (...) {
    }
  }
