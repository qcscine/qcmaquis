...
BOOST_FIXTURE_TEST_CASE(Test_Relativistic, RelativisticFixture) {
  ...
  const typename maquis::DMRGInterface<
      std::complex<double>>::meas_with_results_type& meas = interface.onerdm();
  // Calculate the trace of the 1-RDM and check if it adds up to the number of
  // electrons
  std::complex<double> value = 0.0;
  for (int i = 0; i < meas.first.size(); i++)
    if (meas.first[i][0] ==
        meas.first[i][1])  // sum up diagonal elements for the trace
      value += meas.second[i];
