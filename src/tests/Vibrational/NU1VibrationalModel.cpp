...
BOOST_FIXTURE_TEST_CASE(Test_Integral_Parser_OneBody, NModeFixture) {
  ...
  auto integrals = Vibrational::detail::NModeIntegralParser<double>(
      parametersFADOneBody, lattice_factory(parametersFADOneBody)
  );
  // Checks sizes
...
BOOST_FIXTURE_TEST_CASE(Test_Integral_Parser_TwoBody, NModeFixture) {
  auto integrals = Vibrational::detail::NModeIntegralParser<double>(
      parametersFADTwoBody, lattice_factory(parametersFADTwoBody)
  );
  // Checks sizes
...
BOOST_FIXTURE_TEST_CASE(Test_Model_Create_Terms, NModeFixture) {
  ...
  auto nModeModel =
      NMode<tmatrix<double>, 1>(lattice, parametersFADOneBody, false);
...
BOOST_FIXTURE_TEST_CASE(Test_Model_PhysDim_TwoMode, NModeFixture) {
  ...
  auto nModeModel =
      NMode<tmatrix<double>, 2>(lattice, parametersFADTwoBody, false);
  const auto& physicalDimensions0 = nModeModel.phys_dim(0);
  BOOST_CHECK_EQUAL(physicalDimensions0.sum_of_sizes(), 2);
...
BOOST_FIXTURE_TEST_CASE(Test_Model_TotalQN_TwoMode, NModeFixture) {
  ...
  auto nModeModelWrongDim =
      NMode<tmatrix<double>, 3>(lattice, parametersFADTwoBody, false);
  auto totalQNWrongDim =
      nModeModelWrongDim.total_quantum_numbers(parametersFADTwoBody);
...
BOOST_FIXTURE_TEST_CASE(Test_Model_Symbolic_Operator_TwoMode, NModeFixture) {
  auto lattice = Lattice(parametersFADOneBody);
  auto nModeModel =
      NMode<tmatrix<double>, 2>(lattice, parametersFADOneBody, false);
  BOOST_CHECK(
      nModeModel.filling_matrix_tag(0) == nModeModel.get_operator_tag("fill", 0)
  );
  BOOST_CHECK(
...
BOOST_FIXTURE_TEST_CASE(Test_Model_Operator_Table, NModeFixture) {
  ...
  auto countTagFromTable =
      nModeModel.operators_table()->get_product_tag(destroyTag, createTag);
  auto siteOperatorCount = nModeModel.operators_table()->get_op(countTag);
  auto siteOperatorCountFromTable =
      nModeModel.operators_table()->get_op(countTagFromTable.first);
