...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(
    TestConstructorSingleSiteEvolution, S, symmetries, TestTimeEvolverFixture
) {
  using SingleSiteTimeEvolution =
      SingleSiteTimeEvolution<cmatrix, S, storage::disk>;
  ...
  auto mps = MPS<cmatrix, S>(
      lat.size(), *(model.initializer(lat, parametersH2FourOrbitals))
  );
  mps.normalize_right();
  // Prepares the boundaries
  time_stopper stop_callback(10000.);
  auto ssEvolver = SingleSiteTimeEvolution(
      mps, mpo, parametersH2FourOrbitals, stop_callback
  );
  auto initialEnergy = expval(mps, mpo);
  for (int iSweep = 0; iSweep < 10; iSweep++) ssEvolver.evolve_sweep(iSweep);
  auto finalEnergy = expval(mps, mpo);
  BOOST_CHECK_CLOSE(
      maquis::real(initialEnergy), maquis::real(finalEnergy), 1.0E-10
  );
}

/** Checks the constructor of a site shifter object */
BOOST_FIXTURE_TEST_CASE_TEMPLATE(
    TestConstructorTwoSiteEvolution, S, symmetries, TestTimeEvolverFixture
) {
  ...
  time_stopper stop_callback(10000.);
  auto tsEvolver =
      TwoSiteTimeEvolution(mps, mpo, parametersH2FourOrbitals, stop_callback);
  auto initialEnergy = expval(mps, mpo);
  for (int iSweep = 0; iSweep < 10; iSweep++) tsEvolver.evolve_sweep(iSweep);
  auto finalEnergy = expval(mps, mpo);
  BOOST_CHECK_CLOSE(
      maquis::real(initialEnergy), maquis::real(finalEnergy), 1.0E-10
  );
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(
    TestSingleSiteImaginaryTimeVsTI, S, symmetries, TestTimeEvolverFixture
) {
  using SingleSiteTimeEvolution =
      SingleSiteTimeEvolution<cmatrix, S, storage::disk>;
  // We construct everything as is done in [sim.hpp]
  ...
  auto mpo = make_mpo(lat, model);
  auto mps = MPS<cmatrix, S>(
      lat.size(), *(model.initializer(lat, parametersH2FourOrbitalsImaginary))
  );
  mps.normalize_right();
  // Prepares the boundaries
  time_stopper stop_callback(10000.);
  auto ssEvolver = SingleSiteTimeEvolution(
      mps, mpo, parametersH2FourOrbitalsImaginary, stop_callback
  );
  // We extract the number of sweeps from the parameters to ensure consistency
  // with TI-DMRG
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(
    TestTwoSiteImaginaryTimeVsTI, S, symmetries, TestTimeEvolverFixture
) {
  using TwoSiteTimeEvolution = TwoSiteTimeEvolution<cmatrix, S, storage::disk>;
  // We construct everything as is done in [sim.hpp]
  auto lat = Lattice(parametersH2FourOrbitalsImaginary);
  ...
  // Prepares the boundaries
  time_stopper stop_callback(10000.);
  auto tsEvolver = TwoSiteTimeEvolution(
      mps, mpo, parametersH2FourOrbitalsImaginary, stop_callback
  );
  // We extract the number of sweeps from the parameters to ensure consistency
  // with TI-DMRG
...
BOOST_FIXTURE_TEST_CASE(
    TestTwoSiteRealTimeVsTIRelativistic, TestTimeEvolverFixture
) {
  using S = U1DG;
  using TwoSiteTimeEvolution = TwoSiteTimeEvolution<cmatrix, S, storage::disk>;
  // We construct everything as is done in [sim.hpp]
  ...
  auto mpo = make_mpo(lat, model);
  auto mps = MPS<cmatrix, S>(
      lat.size(), *(model.initializer(lat, parametersRelativistic))
  );
  mps.normalize_right();
  ...
  time_stopper stop_callback(10000.);
  auto tsEvolver =
      TwoSiteTimeEvolution(mps, mpo, parametersRelativistic, stop_callback);
  auto energyBefore = expval(mps, mpo) / norm(mps);
  for (int iSweep = 0; iSweep < 20; iSweep++) tsEvolver.evolve_sweep(iSweep);
  auto energyAfter = expval(mps, mpo) / norm(mps);
  // Checks consistency
  BOOST_CHECK_CLOSE(
      maquis::real(energyBefore), maquis::real(energyAfter), 1.0E-8
  );
}
 ...
 */
BOOST_FIXTURE_TEST_CASE(
    TestSingleSiteImaginaryTimeVsTIRelativistic, TestTimeEvolverFixture
) {
  using S = U1DG;
  using TwoSiteTimeEvolution = TwoSiteTimeEvolution<cmatrix, S, storage::disk>;
  // We construct everything as is done in [sim.hpp]
