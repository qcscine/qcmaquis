...
MPS<matrix, SymmGroup> state_mps(
    std::vector<std::pair<SymmGroup::charge, size_t> > const& state,
  ...
  for (int i = 0; i < state.size(); ++i) {
    ...
    size_t in = left(state[i].first, curr_i[0].first) +
...
void run_test_bosons(
    int L, Index<SymmGroup> const& phys,
    operator_selector<matrix, SymmGroup>::type const& ident,
  ...
  for (int p = 0; p < L; ++p) {
    ...
    reshape_left_to_right<matrix>(
        phys, mps3[p].row_dim(), mps3[p].col_dim(), mps3[p].data(), tmp
    );
    assert(weak_equal(mps3[p].row_dim(), tmp.left_basis()));
    assert(mps3[p].reasonable());
    std::cout << "site " << p << " is reasonable." << std::endl;
    if (verbose) std::cout << tmp << std::endl;
  }
  ...
  for (int p = 0; p < L; ++p)
    BOOST_CHECK_CLOSE(
        dens3[p],
        (dens1[p] * n1 + dens2[p] * n2 + 2 * densmix[p]) / (n1 + n2 + 2 * n12),
        1e-8
    );
}

void test_bosons(
    std::vector<std::pair<SymmGroup::charge, size_t> > const& b1,
    std::vector<std::pair<SymmGroup::charge, size_t> > const& b2, int M,
    bool verbose = false
) {
  typedef operator_selector<matrix, SymmGroup>::type op_t;
...
BOOST_AUTO_TEST_CASE(join_orthogonal_mps_cmp_dens) {
  std::cout << std::endl
            << std::endl
            << "*** join_orthogonal_mps_cmp_dens ***" << std::endl;
  int M = 10;
  int L = 4;

  std::vector<std::pair<SymmGroup::charge, size_t> > b1(
      L, std::pair<SymmGroup::charge, size_t>(0, 0)
  );
  b1[0].first = 1;
  b1[2].first = 1;

  std::vector<std::pair<SymmGroup::charge, size_t> > b2(
      L, std::pair<SymmGroup::charge, size_t>(0, 0)
  );
  b2[1].first = 1;
  b2[3].first = 1;
...
BOOST_AUTO_TEST_CASE(join_same_mps_cmp_dens) {
  std::cout << std::endl
            << std::endl
            << "*** join_same_mps_cmp_dens ***" << std::endl;
  int M = 10;
  int L = 4;
...
BOOST_AUTO_TEST_CASE(join_semirnd_mps_cmp_dens) {
  ...
  std::cout << std::endl
            << std::endl
            << "*** join_semirnd_mps_cmp_dens ***" << std::endl;
  int M = 10;
  int L = 4;

  DmrgParameters parms;

  std::vector<std::pair<SymmGroup::charge, size_t> > b(
      L, std::pair<SymmGroup::charge, size_t>(0, 0)
  );
  b[0].first = 1;
  b[2].first = 1;
  ...
  default_mps_init<matrix, SymmGroup> initializer(
      parms, std::vector<Index<SymmGroup> >(1, phys), initc,
      std::vector<int>(L, 0)
  );
...
BOOST_AUTO_TEST_CASE(join_rnd_mps_cmp_dens) {
  ...
  std::cout << std::endl
            << std::endl
            << "*** join_rnd_mps_cmp_dens ***" << std::endl;
  ...
  for (int i = 0; i < Nmps; ++i) {
    ...
    for (int p = 0; p < mps3.length(); ++p) assert(mps3[p].reasonable());
    std::cout << "norm(mps3): " << norm(mps3) << std::endl;
    mps3.normalize_left();
    for (int p = 0; p < mps3.length(); ++p) assert(mps3[p].reasonable());
    double n3 = norm(mps3);
    ...
    for (int p = 0; p < L; ++p)
      BOOST_CHECK_CLOSE(
          dens3[p],
          (dens[p] * n + dens2[p] * n2 + 2 * densmix[p]) / (n + n2 + 2 * n12),
          1e-8
      );
    std::swap(mps, mps3);
    std::swap(dens, dens3);
