...
/**
 * @brief Verifies that the OverlapPropagator works for the Watson vibrational
 * Hamiltonian The check is done by verifying that the overlap returned by
 * [OverlapPropagator] is the same as the one obtained by calling overlap(mps1,
 * mps2)
 */
BOOST_FIXTURE_TEST_CASE(
    Test_OverlapPropagator_Vibrational_Watson, WatsonFixture
) {
#ifdef HAVE_TrivialGroup
  // Data Generation
  using OverlapPropagatorType =
      OverlapPropagator<matrix, TrivialGroup, storage::disk>;
...
BOOST_FIXTURE_TEST_CASE(Test_OverlapPropagator_Vibrational_NU1, NModeFixture) {
  ...
  using OverlapPropagatorType =
      OverlapPropagator<matrix, NU1SymmGroup, storage::disk>;
