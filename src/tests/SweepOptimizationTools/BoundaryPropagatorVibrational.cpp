...
/**
 * @brief Verifies that the BoundaryPropagator works for the Watson vibrational
 * Hamiltonian The check is done by verifying that, once feeded to the
 * SiteProblem constructor, the correct energy is retrieved
 */
BOOST_FIXTURE_TEST_CASE(
    Test_BoundaryPropagator_Vibrational_Watson, WatsonFixture
) {
#ifdef HAVE_TrivialGroup
  using BoundaryPropagatorType =
      BoundaryPropagator<matrix, TrivialGroup, storage::disk>;
  ...
  auto boundaryPropagator =
      BoundaryPropagatorType(mpsDefault, watsonHarmonicMPO, 5);
  auto siteProblem = SiteProblemType(
      boundaryPropagator.getLeftBoundary(5),
      boundaryPropagator.getRightBoundary(6), watsonHarmonicMPO[5]
  );
...
BOOST_FIXTURE_TEST_CASE(Test_BoundaryPropagator_Vibrational_NU1, NModeFixture) {
  ...
  using BoundaryPropagatorType =
      BoundaryPropagator<matrix, NU1SymmGroup, storage::disk>;
  ...
  auto nModeModel =
      Model<matrix, NU1SymmGroup>(nModeLattice, parametersFADTwoBody);
  auto nModeMPO = make_mpo(nModeLattice, nModeModel);
  auto mpsDefault = MPS<matrix, NU1SymmGroup>(
      nModeLattice.size(),
      *(nModeModel.initializer(nModeLattice, parametersFADTwoBody))
  );
  mpsDefault.normalize_right();
  mpsDefault.canonize(5);
  ...
  auto siteProblem = SiteProblemType(
      boundaryPropagator.getLeftBoundary(7),
      boundaryPropagator.getRightBoundary(8), nModeMPO[7]
  );
