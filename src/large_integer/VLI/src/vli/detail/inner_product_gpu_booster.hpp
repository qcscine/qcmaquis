...
namespace detail
{
    ...
    class inner_product_gpu_booster
    {
      ...
      public:
        ...
        : v1_(partsize*element_size), v2_(partsize*element_size), tmp_(partsize*element_size), partsize_(partsize)
        {
        ...
        {
            std::vector<polynomial_type> restmp(partsize_); 
            std::vector<polynomial_type> poly(omp_get_max_threads()); 

            gpu::cu_check_error(cudaMemcpy((void*)&restmp[0](0,0),(void*)tmp_.p(),partsize_*element_size*sizeof(base_int_type),cudaMemcpyDeviceToHost),__LINE__);
