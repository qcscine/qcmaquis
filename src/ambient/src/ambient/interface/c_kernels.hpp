...
template <typename T>
void __a_memcpy(p_dense_matrix<T>& dest, T* dd, p_dense_matrix<T> const& src, T *sd,  dim2 const& dpos, dim2 const& spos, std::size_t w, double alfa){
...
template <typename T>
void __a_memscal(p_dense_matrix<T>& dest, T* dd, p_dense_matrix<T> const& src, T *sd,  dim2 const& dpos, dim2 const& spos, std::size_t w, double alfa){
    std::size_t v = get_mem_t_dim(src).y-spos.x;
...
// V = double, S = size_t
template<typename T>
void __a_memptf(void (*ptf)(p_dense_matrix<T>& dest, T* dd, p_dense_matrix<T> const& src, T *sd , dim2 const& dpos, dim2 const& spos, std::size_t w, double alfa),
                p_dense_matrix<T>& dest, dim2 dest_p, const p_dense_matrix<T>& src, dim2 src_p, dim2 size, double alfa = 0.0)
{
...
// let's exhaust first src block
    T* dd = current(dest)(get_block_id(dest).y, get_block_id(dest).x);
    ...
    for(size_t j = startj; j < limj; j++){
        ...
        for(int k = 0; k < src_blocks_i; k++){
            T* sd = current(src)(si / get_mem_t_dim(src).y + k, sj / get_mem_t_dim(src).x);
...
template<typename T>
void gemm_inplace_c(pinned p_dense_matrix<T>& a, const p_dense_matrix<T>& b)
{
//  --- --- ---       --- --- ---       --- --- ---
// | 0 | 1 | 2 |     | 0 | 1 | 2 |     | 0 | 1 | 2 |
//  --- --- ---       --- --- ---       --- --- ---
...
template<typename T>
void gemm_c(pinned const p_dense_matrix<T>& a, const p_dense_matrix<T>& b, p_dense_matrix<T>& c)
{
//  --- --- ---       --- --- ---       --- --- ---
// | 0 | 1 | 2 |     | 0 | 1 | 2 |     | 0 | 1 | 2 |
//  --- --- ---       --- --- ---       --- --- ---
...
void copy_c(p_dense_matrix<T>& ac, pinned const p_dense_matrix<T>& a)
{
    ...
    memcpy(ac_elements, a_elements, sizeof(T)*get_mem_t_dim(a).y*get_mem_t_dim(a).x);
}
...
template<typename T>
void remove_rows_c(pinned p_dense_matrix<T>& a, const size_t& i_mark, const size_t& k)
{
...
template<typename T>
void remove_cols_c(pinned p_dense_matrix<T>& a, const size_t& j_mark, const size_t& k)
{
...
template<typename T>
void resize_c(p_dense_matrix<T>& a, const size_t& rows, const size_t& cols)
{
    __a_memptf(&__a_memcpy<T>, a, dim2(0,0), a, dim2(0,0), dim2(rows,cols));
}

template<typename T>
void sqrt_diagonal_c(pinned p_dense_matrix<T>& a)
{
...
template<typename T>
void exp_diagonal_c(pinned p_dense_matrix<T>& a)
{
    T* ad = current(a)(get_block_id(a).y, get_block_id(a).x);
    std::size_t size = get_mem_t_dim(a).y;
    for(int i=0; i < size; i++)
...
template<typename T>
void cast_to_dense_c(std::vector<T>*& ac, pinned const p_dense_matrix<T>& a, const size_t& m, const size_t& n)
{ // note as we cast to dense matrix, l_kernel on 1 proc, thus half work done
    ...
    for(int ii=0; ii < size_x; ++ii){
        ...
        memcpy((void*)&(*ac)[offset],(void*)&ad[ii*get_mem_t_dim(a).x], size_y*sizeof(T));  
    }
}

template <typename T>
void cast_to_p_dense_c(const std::vector<T>*& ac, pinned p_dense_matrix<T>& a, const size_t& m, const size_t& n, const size_t& lda)
{
    int offset,size_y(get_mem_t_dim(a).y),size_x(get_mem_t_dim(a).x);
    int i = get_block_id(a).y;
    int j = get_block_id(a).x;
    ...
    for(int ii=0; ii < size_x; ++ii){
        ...
        memcpy((void*)&ad[ii*get_mem_t_dim(a).x],(void*)&(*ac)[offset], size_y*sizeof(T)); // y direction 
    }
}

template <typename T>
void reshape_l2r_c(const p_dense_matrix<T>& left, pinned p_dense_matrix<T>& right,
...
template <typename T>
void reshape_r2l_c(pinned p_dense_matrix<T>& left, const p_dense_matrix<T>& right,
                   const size_t& left_offset, const size_t& right_offset, 
                   const size_t& sdim, const size_t& ldim, const size_t& rdim)
{
    for(size_t ss = 0; ss < sdim; ++ss)
...
template<typename T>
void scalar_norm_c(pinned const p_dense_matrix<T>& a, T*& norm)
{
    T summ = 0;
    int i = get_block_id(a).y;
    int j = get_block_id(a).x;
...
template<typename T>
void scalar_overlap_c(pinned const p_dense_matrix<T>& a, const p_dense_matrix<T>& b, T*& overlap)
{
    T summ = 0;
    int i = get_block_id(a).y;
    int j = get_block_id(a).x;
...
template<typename T>
void gemm_diagonal_lhs_c(const p_dense_matrix<T>& a_diag, pinned const p_dense_matrix<T>& b, p_dense_matrix<T>& c)
{
    int j = get_block_id(b).y*get_mem_t_dim(b).y;
    int size = get_mem_t_dim(b).x;
    int lda  = get_mem_t_dim(b).y;
    int ONE  = 1;
    T* bd = current(b)(get_block_id(b).y, get_block_id(b).x);
    T* cd = current(c)(get_block_id(b).y, get_block_id(b).x);

    memset(cd, 0, get_mem_t_dim(c).x*get_mem_t_dim(c).y*sizeof(T));
    for(int jj = 0 ; jj < get_mem_t_dim(b).y ; jj++){
         T* alpha = current(a_diag)((j+jj)/get_mem_t_dim(a_diag).y,0);
	 axpy(&size, &alpha[(j+jj)%get_mem_t_dim(a_diag).y], &bd[jj], &lda, &cd[jj], &lda);
    }
}

template<typename T>
void gemm_diagonal_rhs_c(pinned const p_dense_matrix<T>& a, const p_dense_matrix<T>& b_diag, p_dense_matrix<T>& c)
{
    ...
    int ONE = 1;
    T* ad = current(a)(get_block_id(a).y, get_block_id(a).x);
    T* cd = current(c)(get_block_id(a).y, get_block_id(a).x);

    memset(cd, 0, get_mem_t_dim(c).x*get_mem_t_dim(c).y*sizeof(T));
    for(int jj = 0 ; jj < get_mem_t_dim(a).x ; jj++){
	 T* alpha = current(b_diag)((j+jj)/get_mem_t_dim(b_diag).y,0);
	 axpy(&size, &alpha[(j+jj)%get_mem_t_dim(b_diag).y], &ad[jj*get_mem_t_dim(a).y], &ONE, &cd[jj*get_mem_t_dim(c).y], &ONE);
    }
}

template<typename T>
void trace_c(pinned const p_dense_matrix<T>& a, T*& trace)
...
template<typename T>
void transpose_c(pinned p_dense_matrix<T>& transposed, const p_dense_matrix<T>& original)
{
    std::size_t i = get_block_id(transposed).y;
    std::size_t j = get_block_id(transposed).x;
    T* td = current(transposed)(i,j);
    T* od = current(original)(j,i);

    for(std::size_t i = 0; i < get_mem_t_dim(original).y; ++i){
...
template<typename T>
void nullcut_c(pinned p_dense_matrix<T>& a, const size_t& num_rows, const size_t& num_cols)
{
    ...
    for(size_t jj = 0; jj < get_mem_t_dim(a).x; jj++){
        if(j+jj < num_cols && (i+get_mem_t_dim(a).y > num_rows)) memset(&ad[jj*get_mem_t_dim(a).y+num_rows%get_mem_t_dim(a).y], 0, (get_mem_t_dim(a).y-num_rows%get_mem_t_dim(a).y)*sizeof(T));
        else if(j+jj >= num_cols) memset(&ad[jj*get_mem_t_dim(a).y], 0, get_mem_t_dim(a).y*sizeof(T));
    }
}

template <typename T> 
void validation_c(pinned const p_dense_matrix<T>& a, const p_dense_matrix<T>& b, int*& bl) // C - See paper for Reference Dongara 
{ 
    int i = get_block_id(a).y;
    int j = get_block_id(a).x;
    T* ad = current(a)(get_block_id(a).y, get_block_id(a).x); 
    ...
    for(int ii=0; ii < get_mem_t_dim(a).x; ++ii){ // C - the std::resize + cast dense to p_dense can add ghost elements between lda + num_rows
        for(int jj=0; jj < get_mem_t_dim(a).y; ++jj){
            ...
            if(position_x < get_dim(a).x && position_y < get_dim(a).y){
                ...
                if(res > 256){ // 16 is recommended by Dongara, 256 because lapack gives != runs after runs
                    std::cout <<   get_block_id(a).y << " " <<  get_block_id(a).x << " " << res << " " << ad[i] << " " << bd[i] << std::endl; // C - cout because double or complex
...
// MKL LAPACK kernels
template<typename T>
void svd_c(const p_dense_matrix<T>& a, int& m, int& n, p_dense_matrix<T>& u, p_dense_matrix<T>& vt, p_dense_matrix<double>& s)
{
/* Locals */
...
/* Query and allocate the optimal workspace */
    ...
    gesvd( "S", "S", &m, &n, (T*)breakdown(a).data, &lda, (double*)breakdown(s).data, (T*)breakdown(u).data, &ldu, (T*)breakdown(vt).data, &ldvt, &wkopt, &lwork, rwork,  &info );
    lwork = (int)wkopt;
    work = (T*)malloc( lwork*sizeof(T) );
/* Compute SVD */
    gesvd( "S", "S", &m, &n, (T*)breakdown(a).data, &lda, (double*)breakdown(s).data, (T*)breakdown(u).data, &ldu, (T*)breakdown(vt).data, &ldvt, work, &lwork, rwork,  &info );
...
// C - For devs
template<typename T>
void initv_c(pinned p_dense_matrix<T>&a, T const&v)
{
    T* ad = current(a)(get_block_id(a).y, get_block_id(a).x);
