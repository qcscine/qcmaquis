...
void info(void_pt* p){
    ...
    printf("Matrix %d size of the task is %d x %d groups sized %d x %d items of %d x %d elements\n", p->id, p->grid_dim().y, p->grid_dim().x, p->group_dim().y, p->group_dim().x, p->item_dim().x, p->item_dim().y);
}
