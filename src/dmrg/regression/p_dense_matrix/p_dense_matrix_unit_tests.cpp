...
{
    p_dense_matrix<T> a(10,10,1);
    p_dense_matrix<T> b(1,1,0);
    p_dense_matrix<T> c(a);
    p_dense_matrix<T> d(b);
    std::swap(a,b);
...
BOOST_AUTO_TEST_CASE_TEMPLATE( row_iterator_test, T, test_types )
{
    ...
    for(unsigned int i=0; i<num_rows(a); ++i)
    {
        std::pair<typename p_dense_matrix<T>::row_element_iterator, typename p_dense_matrix<T>::row_element_iterator> range(row(a,i));
        unsigned int j=0;
        for(typename p_dense_matrix<T>::const_row_element_iterator it(range.first); it != range.second; ++it)
        {
...
BOOST_AUTO_TEST_CASE_TEMPLATE( column_iterator_test, T, test_types )
{
    ...
    for(unsigned int j=0; j<num_columns(a); ++j)
    {
        std::pair<typename p_dense_matrix<T>::column_element_iterator, typename p_dense_matrix<T>::column_element_iterator> range(column(a,j));
        unsigned int i=0;
        for(typename p_dense_matrix<T>::const_column_element_iterator it(range.first); it != range.second; ++it)
        {
            BOOST_CHECK_EQUAL(a(i,j), *it);
            ++i;
        }
...
{
    p_dense_matrix<T> a(10,20);
    p_dense_matrix<T> b(10,20);
    std::pair<typename p_dense_matrix<T>::element_iterator,typename p_dense_matrix<T>::element_iterator> range(elements(a));
...
BOOST_AUTO_TEST_CASE_TEMPLATE( append_rows_test, T, test_types)
{
    ...
    p_dense_matrix<T> a(initsize,initsize);
    fill_matrix_with_numbers(a);
    p_dense_matrix<T> b(a);
...
{
    p_dense_matrix<T> a(20,30);
    p_dense_matrix<T> zero(20,30,T(0));
    fill_matrix_with_numbers(a);
    p_dense_matrix<T> b(a);
    a += b;
    a -= b;
...
BOOST_AUTO_TEST_CASE_TEMPLATE( multiplies_test, T, test_types)
{
    ...
    fill_matrix_with_numbers(a);
    p_dense_matrix<T> b(a);
    p_dense_matrix<T> ref_b(b);
    a*= T(2);
    p_dense_matrix<T> c = T(2) * b;
    //TODO Do we really want to assume commutative types?
    p_dense_matrix<T> d = b * T(2);
    BOOST_CHECK_EQUAL(c,a);
    ...
    p_dense_matrix<T> e(b);
    b*= 5;
    for(unsigned int i=0; i<num_rows(c); ++i)
        for(unsigned int j=0; j<num_columns(c); ++j)
        {
            typename p_dense_matrix<T>::value_type tmp (e(i,j));
            tmp *= 5;
            BOOST_CHECK_EQUAL(b(i,j),tmp);
        }
    p_dense_matrix<T> ref_e(e);
    p_dense_matrix<T> f ( e * 5 );
    p_dense_matrix<T> g ( 5 * e );
...
{
    // -p_dense_matrix<T> * vector<int>
    
    p_dense_matrix<typename TPair::first_type> a(20,30);
    vector<typename TPair::second_type> v(30);
    fill_matrix_with_numbers(a);
    fill_range_with_numbers(v.begin(),v.end(),0);
    p_dense_matrix<typename TPair::first_type> a_(a);
