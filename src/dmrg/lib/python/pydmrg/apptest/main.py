...
def main():
    if sys.version_info < (2, 7):
        parser = optparse.OptionParser(description='Testing tool for MAQUIS DMRG')
        parser.add_option('-c', '--create', dest='action', action='store_const',
                          const='create', default='run', help='create test reference results')
        parser.add_option('-p', '--params', dest='action', action='store_const',
        ...
        for test in find_subclasses():
            ...
            if options.action == 'create':
                t.create(dmrg_app=args[0], meas_app=args[1])
            elif options.action == 'params':
                t.write_parameters()
            else:
                t.run(dmrg_app=args[0], meas_app=args[1])
                t.check()
    else:
