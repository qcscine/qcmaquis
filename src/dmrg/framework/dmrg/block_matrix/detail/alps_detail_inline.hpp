...
namespace maquis { namespace dmrg { namespace detail {
                              ...
                              double * out1, double val)
    {
        ...
        /*
        if ((std::size_t)in1%32 != 0) {
            std::cout << "in1 not aligned\n";
            exit(1);
        }
        if ((std::size_t)out1%32 != 0) {
            std::cout << "out1 not aligned\n";
            exit(1);
        }
        fortran_int_t ndiv4 = diff/4;
        __m256d alfa = _mm256_broadcast_sd(&val);
        for(fortran_int_t n=0; n < ndiv4; ++n)
        {
            __m256d x = _mm256_load_pd(in1 + 4*n);
                    x = _mm256_mul_pd(alfa, x);

            __m256d y = _mm256_load_pd(out1 + 4*n);

            _mm256_store_pd(out1 + 4*n, _mm256_add_pd(y,x));
        }
        for(fortran_int_t n = ndiv4 * 4; n < diff; ++n)
        {
            out1[n] += in1[n] * val;
        }
        */
        ...
        for(size_t rr = 0; rr < rdim; ++rr) {
            for(size_t ss1 = 0; ss1 < sdim1; ++ss1) {
                for(size_t ss2 = 0; ss2 < sdim2; ++ss2) {
                    ...
                    if (alps::numeric::is_zero(alfa_t))
                        continue;
                        /*
                    switch (ldim){
                        case 8: out(out_offset + ss2*ldim+7, rr) += in(in_offset + ss1*ldim+7, rr) * alfa_t;
                        case 7: out(out_offset + ss2*ldim+6, rr) += in(in_offset + ss1*ldim+6, rr) * alfa_t;
                        case 6: out(out_offset + ss2*ldim+5, rr) += in(in_offset + ss1*ldim+5, rr) * alfa_t;
                        case 5: out(out_offset + ss2*ldim+4, rr) += in(in_offset + ss1*ldim+4, rr) * alfa_t;
                        case 4: out(out_offset + ss2*ldim+3, rr) += in(in_offset + ss1*ldim+3, rr) * alfa_t;
                        case 3: out(out_offset + ss2*ldim+2, rr) += in(in_offset + ss1*ldim+2, rr) * alfa_t;
                        case 2: out(out_offset + ss2*ldim+1, rr) += in(in_offset + ss1*ldim+1, rr) * alfa_t;
                        case 1: out(out_offset + ss2*ldim  , rr) += in(in_offset + ss1*ldim  , rr) * alfa_t;
                        break;
                        default:
                        */
