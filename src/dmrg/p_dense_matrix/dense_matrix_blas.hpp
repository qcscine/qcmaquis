...
#include "p_dense_matrix/detail/blasmacros.h"
...
#define MATRIX_MATRIX_MULTIPLY(T) \
    ...
    { \
        ...
        boost::numeric::bindings::blas::gemm \
            ( \
               typename p_dense_matrix<T,MemoryBlock>::value_type(1), \
               lhs, \
               rhs, \
               typename p_dense_matrix<T,MemoryBlock>::value_type(0), \
               result \
            ); \
        timer.end(); \
        return result; \
...
#define MATRIX_VECTOR_MULTIPLY(T) \
    ...
    const vector<T,MemoryBlock2> matrix_vector_multiply(p_dense_matrix<T,MemoryBlock> const& m, vector<T,MemoryBlock2> const& v) \
    { \
...
#define PLUS_MINUS_ASSIGN(T) \
    template <typename MemoryBlock> \
    void plus_and_minus_assign_impl(p_dense_matrix<T,MemoryBlock>& m, p_dense_matrix<T,MemoryBlock> const& rhs, typename p_dense_matrix<T,MemoryBlock>::value_type const& sign) \
    { \
    ...
    void plus_assign(p_dense_matrix<T,MemoryBlock>& m, p_dense_matrix<T,MemoryBlock> const& rhs) \
        { plus_and_minus_assign_impl(m, rhs, typename p_dense_matrix<T,MemoryBlock>::value_type(1)); } \
    template <typename MemoryBlock> \
...
#define MULTIPLIES_ASSIGN(T) \
    template <typename MemoryBlock> \
    void multiplies_assign(p_dense_matrix<T,MemoryBlock>& m, T const& t) \
    { \
        if( !(m.is_shrinkable()) ) \
        { \
