...
class my_sim_type : public alps::mcbase {
 public:
  ...
  my_sim_type(parameters_type const &params, Arg comm)
      : alps::mcbase(params, comm), total_count(params["COUNT"]) {
    measurements << alps::accumulator::RealObservable("SValue")
                 << alps::accumulator::RealVectorObservable("VValue");
  }
  ...
  double fraction_completed() const { return count / double(total_count); }
...
int main(int argc, char *argv[]) {
  try {
    ...
    alps::mcmpiadapter<my_sim_type> my_sim(
        params, c, alps::check_schedule(options.tmin, options.tmax)
    );  // creat a simulation

    my_sim.run(alps::stop_callback(c, options.timelimit)
    );  // run the simulation

    using alps::collect_results;

    if (c.rank() == 0) {  // print the results and save it to hdf5
      alps::results_type<alps::mcmpiadapter<my_sim_type> >::type results =
      ...
      // std::cout << results["SValue"] << " " << results["SValue"] *
      // results["SValue"] << sin(results["SValue"]) << std::endl; std::cout <<
      // results["SValue"] * results["SValue"] << std::endl; std::cout << << 2.
      // * results["SValue"] / 2. << std::endl; std::cout <<
      // sin(results["SValue"]) << std::endl; std::cout << results["VValue"] <<
      // " " << 2. * results["VValue"] / 2. << sin(results["SValue"]) <<
      // std::endl;
