...
inline void gemv(
    const Order order, const Trans trans, const int m, const int n,
    const float alpha, const float* a, const int lda, const float* x,
    const int incx, const float beta, float* y, const int incy
) {
  cblas_sgemv(
      cblas_option<Order>::value, cblas_option<Trans>::value, m, n, alpha, a,
...
template <typename Order, typename Trans>
inline void gemv(
    const Order order, const Trans trans, const fortran_int_t m,
  ...
  BOOST_STATIC_ASSERT((is_same<Order, tag::column_major>::value));
  BLAS_SGEMV(
      &blas_option<Trans>::value, &m, &n, &alpha, a, &lda, x, &incx, &beta, y,
      &incy
  );
}
...
struct gemv_impl {
  ...
  static result_type transform(
      MatrixA& A, VectorX& x, VectorY& y, const value_type alpha,
  ...
  static result_type invoke(
      const value_type alpha, const MatrixA& a, const VectorX& x,
      const value_type beta, VectorY& y
  ) {
    ...
    detail::gemv(
        order(), trans(), bindings::size_row_op(a, trans()),
        bindings::size_column_op(a, trans()), alpha, bindings::begin_value(a),
        bindings::stride_major(a), bindings::begin_value(x),
        bindings::stride(x), beta, bindings::begin_value(y), bindings::stride(y)
    );
  }
};
...
gemv(
    ...
    const VectorX& x, const typename bindings::value_type<MatrixA>::type beta,
    VectorY& y
) {
  gemv_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      alpha, a, x, beta, y
  );
}
