...
inline void gerc(
    ...
    const fortran_int_t incy, std::complex<float>* a, const fortran_int_t lda
) {
  BOOST_STATIC_ASSERT((is_same<Order, tag::column_major>::value));
...
inline void gerc(
    ...
    const fortran_int_t incy, std::complex<double>* a, const fortran_int_t lda
) {
  BOOST_STATIC_ASSERT((is_same<Order, tag::column_major>::value));
...
struct gerc_impl {
  ...
  static result_type invoke(
      const value_type alpha, const VectorX& x, const VectorY& y, MatrixA& a
  ) {
...
gerc(
  ...
  gerc_impl<typename bindings::value_type<VectorX>::type>::invoke(
      alpha, x, y, a
  );
}
