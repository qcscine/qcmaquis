...
inline void gemm(
    const Order order, const TransA transa, const TransB transb, const int m,
    const int n, const int k, const float alpha, const float* a, const int lda,
    const float* b, const int ldb, const float beta, float* c, const int ldc
) {
  cblas_sgemm(
      cblas_option<Order>::value, cblas_option<TransA>::value,
      cblas_option<TransB>::value, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc
  );
}
...
inline void gemm(
  ...
  cblas_cgemm(
      cblas_option<Order>::value, cblas_option<TransA>::value,
      cblas_option<TransB>::value, m, n, k, &alpha, a, lda, b, ldb, &beta, c,
      ldc
  );
}
...
inline void gemm(
  ...
  cublasSgemm(
      blas_option<TransA>::value, blas_option<TransB>::value, m, n, k, alpha, a,
...
template <typename Order, typename TransA, typename TransB>
inline void gemm(
    const Order order, const TransA transa, const TransB transb,
    const fortran_int_t m, const fortran_int_t n, const fortran_int_t k,
    const float alpha, const float* a, const fortran_int_t lda, const float* b,
    const fortran_int_t ldb, const float beta, float* c, const fortran_int_t ldc
) {
  BOOST_STATIC_ASSERT((is_same<Order, tag::column_major>::value));
  BLAS_SGEMM(
      &blas_option<TransA>::value, &blas_option<TransB>::value, &m, &n, &k,
...
inline void gemm(
    ...
    const double* b, const fortran_int_t ldb, const double beta, double* c,
...
struct gemm_impl {
  ...
  static result_type invoke(
    ...
    detail::gemm(
        order(), transa(), transb(), bindings::size_row(c),
...
gemm(
    ...
    const MatrixB& b, const typename bindings::value_type<MatrixA>::type beta,
    MatrixC& c
) {
  gemm_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      alpha, a, b, beta, c
  );
}
