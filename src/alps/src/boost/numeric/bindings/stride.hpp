...
struct stride_impl<
    T, AddressingIndex,
    typename boost::enable_if<mpl::equal_to<rank<T>, tag::scalar> >::type> {
...
struct fold_stride_size {
  ...
  typedef typename mpl::if_<
      mpl::or_<
          is_same<State, std::ptrdiff_t>, is_same<size_type, std::ptrdiff_t>,
          is_same<stride_type, std::ptrdiff_t> >,
      std::ptrdiff_t,
      mpl::plus<State, mpl::times<size_type, stride_type> > >::type type;
};
...
struct apply_fold {
  static Result invoke(const T& t) { return Result(); }
};
...
struct apply_fold<T, std::ptrdiff_t, 0> {
  static std::ptrdiff_t invoke(const T&) { return 0; }
};
...
struct stride_impl<
    ...
    typename boost::enable_if<mpl::and_<
        ...
        mpl::greater<AddressingIndex, rank<T> > > >::type> {
  typedef mpl::range_c<int, 1, rank<T>::value + 1> index_range;
  typedef typename mpl::fold<
      index_range, mpl::int_<0>, fold_stride_size<T, mpl::_1, mpl::_2> >::type
      result_type;
...
inline typename result_of::stride<const T, Tag>::type stride(const T& t, Tag) {
...
inline typename boost::enable_if<
    ...
    typename result_of::stride<const T, tag::addressing_index<1> >::type>::type
stride(const T& t) {
