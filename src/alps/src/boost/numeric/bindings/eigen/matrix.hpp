...
struct eigen_size_type {
  typedef typename mpl::if_<
      mpl::bool_<Value == Eigen::Dynamic>, std::ptrdiff_t,
      mpl::int_<Value> >::type type;
};

template <int Value>
struct eigen_data_order {
  typedef typename mpl::if_<
      mpl::bool_<Value & Eigen::RowMajorBit>, tag::row_major,
      tag::column_major>::type type;
};

template <
    typename T, int Rows, int Cols, int Options, typename Id, typename Enable>
  ...
  typedef mpl::map<
      ...
      mpl::pair<
          ...
          typename if_row_major<
              data_order, tag::contiguous, size_type1>::type> >
      property_map;

  static std::ptrdiff_t size1(const Id& id) { return id.rows(); }

  static std::ptrdiff_t size2(const Id& id) { return id.cols(); }

  static value_type* begin_value(Id& id) { return id.data(); }

  static value_type* end_value(Id& id) { return id.data() + id.size(); }

  static std::ptrdiff_t stride1(const Id& id) { return id.cols(); }

  static std::ptrdiff_t stride2(const Id& id) { return id.rows(); }
};
