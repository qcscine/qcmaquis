...
#define GENERATE_GET(z, which, unused)                                     \
  ...
  struct get_dispatch<tag::size_type<which> > {                            \
    ...
    static std::ptrdiff_t invoke(const T& t) {                             \
      return detail::adaptor_access<T>::BOOST_PP_CAT(size, which)(t);      \
    }                                                                      \
  };                                                                       \
                                                                           \
  template <>                                                              \
  struct get_dispatch<tag::stride_type<which> > {                          \
    template <typename T>                                                  \
    static std::ptrdiff_t invoke(const T& t) {                             \
      return detail::adaptor_access<T>::BOOST_PP_CAT(stride, which)(t);    \
    }                                                                      \
  };                                                                       \
                                                                           \
  template <>                                                              \
  struct get_dispatch<tag::bandwidth_type<which> > {                       \
    template <typename T>                                                  \
    static std::ptrdiff_t invoke(const T& t) {                             \
      return detail::adaptor_access<T>::BOOST_PP_CAT(bandwidth, which)(t); \
    }                                                                      \
  };
...
struct get_impl<
    T, Key,
    typename boost::enable_if<is_same_at<T, Key, std::ptrdiff_t> >::type> {
...
struct get_impl<
  ...
  static result_type invoke(const T&) { return result_type(); }
};
