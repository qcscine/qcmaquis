...
inline std::ptrdiff_t ggbak(
    const char job, const Side, const fortran_int_t n, const fortran_int_t ilo,
    const fortran_int_t ihi, const float* lscale, const float* rscale,
    const fortran_int_t m, float* v, const fortran_int_t ldv
) {
  fortran_int_t info(0);
  LAPACK_SGGBAK(
      &job, &lapack_option<Side>::value, &n, &ilo, &ihi, lscale, rscale, &m, v,
      &ldv, &info
  );
  return info;
}
...
inline std::ptrdiff_t ggbak(
    const char job, const Side, const fortran_int_t n, const fortran_int_t ilo,
    const fortran_int_t ihi, const float* lscale, const float* rscale,
    const fortran_int_t m, std::complex<float>* v, const fortran_int_t ldv
) {
  fortran_int_t info(0);
  LAPACK_CGGBAK(
      &job, &lapack_option<Side>::value, &n, &ilo, &ihi, lscale, rscale, &m, v,
...
inline std::ptrdiff_t ggbak(
    const char job, const Side, const fortran_int_t n, const fortran_int_t ilo,
    const fortran_int_t ihi, const double* lscale, const double* rscale,
    const fortran_int_t m, std::complex<double>* v, const fortran_int_t ldv
) {
  fortran_int_t info(0);
  LAPACK_ZGGBAK(
      &job, &lapack_option<Side>::value, &n, &ilo, &ihi, lscale, rscale, &m, v,
...
struct ggbak_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  //
  template <
      typename Side, typename VectorLSCALE, typename VectorRSCALE,
      typename MatrixV>
  static std::ptrdiff_t invoke(
      const char job, const Side side, const fortran_int_t ilo,
    ...
    BOOST_ASSERT(
        ...
        std::max<std::ptrdiff_t>(1, bindings::size_row(v))
    );
    BOOST_ASSERT(job == 'N' || job == 'P' || job == 'S' || job == 'B');
    return detail::ggbak(
        job, side, bindings::size_row(v), ilo, ihi,
...
inline std::ptrdiff_t ggbak(
  ...
  return ggbak_impl<typename bindings::value_type<MatrixV>::type>::invoke(
      job, side, ilo, ihi, lscale, rscale, v
  );
}
