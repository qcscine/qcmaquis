...
inline std::ptrdiff_t gbcon(
    const char norm, const fortran_int_t n, const fortran_int_t kl,
    const fortran_int_t ku, const std::complex<float>* ab,
    const fortran_int_t ldab, const fortran_int_t* ipiv, const float anorm,
...
inline std::ptrdiff_t gbcon(
    const char norm, const fortran_int_t n, const fortran_int_t kl,
    const fortran_int_t ku, const std::complex<double>* ab,
    const fortran_int_t ldab, const fortran_int_t* ipiv, const double anorm,
...
struct gbcon_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  //
  template <
      typename MatrixAB, typename VectorIPIV, typename WORK, typename IWORK>
  static std::ptrdiff_t invoke(
      const char norm, const MatrixAB& ab, const VectorIPIV& ipiv,
    ...
    BOOST_ASSERT(
        ...
        2 * bindings::bandwidth_lower(ab) +
            (bindings::bandwidth_upper(ab) - bindings::bandwidth_lower(ab)) + 1
    );
    BOOST_ASSERT(
        (bindings::bandwidth_upper(ab) - bindings::bandwidth_lower(ab)) >= 0
    );
    BOOST_ASSERT(norm == '1' || norm == 'O' || norm == 'I');
    return detail::gbcon(
        norm, bindings::size_column(ab), bindings::bandwidth_lower(ab),
  ...
  static std::ptrdiff_t invoke(
    ...
    return invoke(norm, ab, ipiv, anorm, rcond, workspace(tmp_work, tmp_iwork));
  }
...
struct gbcon_impl<Value, typename boost::enable_if<is_complex<Value> >::type> {
  ...
  //
  template <
      typename MatrixAB, typename VectorIPIV, typename WORK, typename RWORK>
  static std::ptrdiff_t invoke(
      const char norm, const MatrixAB& ab, const VectorIPIV& ipiv,
      const real_type anorm, real_type& rcond,
  ...
  static std::ptrdiff_t invoke(
    ...
    return invoke(norm, ab, ipiv, anorm, rcond, workspace(tmp_work, tmp_rwork));
  }
...
gbcon(
  ...
  return gbcon_impl<typename bindings::value_type<MatrixAB>::type>::invoke(
      norm, ab, ipiv, anorm, rcond, work
  );
}
...
gbcon(
  ...
  return gbcon_impl<typename bindings::value_type<MatrixAB>::type>::invoke(
      norm, ab, ipiv, anorm, rcond, optimal_workspace()
  );
}
