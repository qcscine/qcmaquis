...
// * float value-type.
//
inline std::ptrdiff_t pttrs(
    const fortran_int_t n, const fortran_int_t nrhs, const float* d,
    const float* e, float* b, const fortran_int_t ldb
) {
...
inline std::ptrdiff_t pttrs(
    ...
    const double* e, double* b, const fortran_int_t ldb
) {
  fortran_int_t info(0);
...
inline std::ptrdiff_t pttrs(
    const char uplo, const fortran_int_t n, const fortran_int_t nrhs,
    const double* d, const std::complex<double>* e, std::complex<double>* b,
    const fortran_int_t ldb
) {
  fortran_int_t info(0);
...
struct pttrs_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(const VectorD& d, const VectorE& e, MatrixB& b) {
...
struct pttrs_impl<Value, typename boost::enable_if<is_complex<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
      const char uplo, const VectorD& d, const VectorE& e, MatrixB& b
  ) {
    namespace bindings = ::boost::numeric::bindings;
    BOOST_STATIC_ASSERT((bindings::is_column_major<MatrixB>::value));
    BOOST_STATIC_ASSERT(
...
//
template <typename VectorD, typename VectorE, typename MatrixB>
inline std::ptrdiff_t pttrs(const VectorD& d, const VectorE& e, MatrixB& b) {
  return pttrs_impl<typename bindings::value_type<VectorE>::type>::invoke(
      d, e, b
  );
}
//
