...
inline std::ptrdiff_t gebak(
  ...
  LAPACK_SGEBAK(
      &job, &lapack_option<Side>::value, &n, &ilo, &ihi, scale, &m, v, &ldv,
      &info
  );
  return info;
}
...
struct gebak_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
      ...
      const fortran_int_t ihi, const VectorSCALE& scale, MatrixV& v
  ) {
...
inline std::ptrdiff_t gebak(
  ...
  return gebak_impl<typename bindings::value_type<MatrixV>::type>::invoke(
      job, side, ilo, ihi, scale, v
  );
}
