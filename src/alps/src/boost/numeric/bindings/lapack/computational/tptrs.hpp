...
inline std::ptrdiff_t tptrs(
    const UpLo, const Trans, const Diag, const fortran_int_t n,
    const fortran_int_t nrhs, const float* ap, float* b, const fortran_int_t ldb
) {
  fortran_int_t info(0);
  LAPACK_STPTRS(
      &lapack_option<UpLo>::value, &lapack_option<Trans>::value,
      &lapack_option<Diag>::value, &n, &nrhs, ap, b, &ldb, &info
  );
  return info;
}
...
inline std::ptrdiff_t tptrs(const MatrixAP& ap, MatrixB& b) {
  return tptrs_impl<typename bindings::value_type<MatrixAP>::type>::invoke(
      ap, b
  );
}
