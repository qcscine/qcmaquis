...
inline std::ptrdiff_t unmbr(
    const char vect, const Side, const Trans, const fortran_int_t m,
  ...
  LAPACK_SORMBR(
      &vect, &lapack_option<Side>::value, &lapack_option<Trans>::value, &m, &n,
      &k, a, &lda, tau, c, &ldc, work, &lwork, &info
  );
  return info;
}
...
inline std::ptrdiff_t unmbr(
    const char vect, const Side, const Trans, const fortran_int_t m,
    const fortran_int_t n, const fortran_int_t k, const std::complex<float>* a,
...
inline std::ptrdiff_t unmbr(
    const char vect, const Side, const Trans, const fortran_int_t m,
    const fortran_int_t n, const fortran_int_t k, const std::complex<double>* a,
...
struct unmbr_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
      const char vect, const Side side, const MatrixA& a, const VectorTAU& tau,
    ...
    return detail::unmbr(
        vect, side, trans(), bindings::size_row(c), bindings::size_column(c),
...
struct unmbr_impl<Value, typename boost::enable_if<is_complex<Value> >::type> {
  ...
  template <
      ...
      typename WORK>
  static std::ptrdiff_t invoke(
      const char vect, const Side side, const fortran_int_t k, const MatrixA& a,
    ...
    BOOST_ASSERT(vect == 'Q' || vect == 'P');
    return detail::unmbr(
        vect, side, trans(), bindings::size_row(c), bindings::size_column(c), k,
...
unmbr(
  ...
  return unmbr_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      vect, side, a, tau, c, work
  );
}
...
unmbr(
  ...
  return unmbr_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      vect, side, a, tau, c, optimal_workspace()
  );
}
//
// Overloaded function for unmbr. Its overload differs for
...
unmbr(
  ...
  return unmbr_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      vect, side, k, a, tau, c, optimal_workspace()
  );
}
