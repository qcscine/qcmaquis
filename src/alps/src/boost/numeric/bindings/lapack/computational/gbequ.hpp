...
// * float value-type.
//
inline std::ptrdiff_t gbequ(
    const fortran_int_t m, const fortran_int_t n, const fortran_int_t kl,
    const fortran_int_t ku, const float* ab, const fortran_int_t ldab, float* r,
...
inline std::ptrdiff_t gbequ(
    ...
    const fortran_int_t ldab, float* r, float* c, float& rowcnd, float& colcnd,
    float& amax
) {
  fortran_int_t info(0);
  LAPACK_CGBEQU(
...
struct gbequ_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
      const MatrixAB& ab, VectorR& r, VectorC& c, real_type& rowcnd,
      real_type& colcnd, real_type& amax
  ) {
    ...
    return detail::gbequ(
        ...
        bindings::begin_value(r), bindings::begin_value(c), rowcnd, colcnd, amax
    );
  }
};
...
template <typename MatrixAB, typename VectorR, typename VectorC>
inline std::ptrdiff_t gbequ(
    const MatrixAB& ab, VectorR& r, VectorC& c,
  ...
  return gbequ_impl<typename bindings::value_type<MatrixAB>::type>::invoke(
      ab, r, c, rowcnd, colcnd, amax
  );
}
