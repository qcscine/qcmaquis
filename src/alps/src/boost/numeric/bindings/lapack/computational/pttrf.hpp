...
//
inline std::ptrdiff_t pttrf(
    const fortran_int_t n, float* d, std::complex<float>* e
) {
  fortran_int_t info(0);
...
//
inline std::ptrdiff_t pttrf(
    const fortran_int_t n, double* d, std::complex<double>* e
) {
  fortran_int_t info(0);
...
inline std::ptrdiff_t pttrf(VectorD& d, VectorE& e) {
  return pttrf_impl<typename bindings::value_type<VectorE>::type>::invoke(d, e);
}
