...
inline std::ptrdiff_t pbequ(
  ...
  LAPACK_SPBEQU(
      &lapack_option<UpLo>::value, &n, &kd, ab, &ldab, s, &scond, &amax, &info
  );
  return info;
}
...
inline std::ptrdiff_t pbequ(
    ...
    const std::complex<float>* ab, const fortran_int_t ldab, float* s,
...
inline std::ptrdiff_t pbequ(
    const UpLo, const fortran_int_t n, const fortran_int_t kd,
    const std::complex<double>* ab, const fortran_int_t ldab, double* s,
...
template <typename MatrixAB, typename VectorS>
inline std::ptrdiff_t pbequ(
    const MatrixAB& ab, VectorS& s,
    typename remove_imaginary<
        typename bindings::value_type<MatrixAB>::type>::type& scond,
    typename remove_imaginary<
        typename bindings::value_type<MatrixAB>::type>::type& amax
) {
  return pbequ_impl<typename bindings::value_type<MatrixAB>::type>::invoke(
      ab, s, scond, amax
  );
}
