...
inline std::ptrdiff_t hegst(
    const fortran_int_t itype, const UpLo, const fortran_int_t n, float* a,
    const fortran_int_t lda, const float* b, const fortran_int_t ldb
) {
  fortran_int_t info(0);
  LAPACK_SSYGST(
      &itype, &lapack_option<UpLo>::value, &n, a, &lda, b, &ldb, &info
  );
  return info;
}
...
template <typename UpLo>
inline std::ptrdiff_t hegst(
    const fortran_int_t itype, const UpLo, const fortran_int_t n, double* a,
...
template <typename UpLo>
inline std::ptrdiff_t hegst(
    const fortran_int_t itype, const UpLo, const fortran_int_t n,
...
struct hegst_impl {
  ...
  static std::ptrdiff_t invoke(
      const fortran_int_t itype, MatrixA& a, const MatrixB& b
  ) {
    ...
    return detail::hegst(
        itype, uplo(), bindings::size_column(a), bindings::begin_value(a),
...
inline std::ptrdiff_t hegst(
  ...
  return hegst_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      itype, a, b
  );
}
