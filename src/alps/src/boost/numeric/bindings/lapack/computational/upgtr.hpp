...
//
inline std::ptrdiff_t upgtr(
    const char uplo, const fortran_int_t n, const double* ap, const double* tau,
...
//
inline std::ptrdiff_t upgtr(
    const char uplo, const fortran_int_t n, const std::complex<float>* ap,
...
//
inline std::ptrdiff_t upgtr(
    const char uplo, const fortran_int_t n, const std::complex<double>* ap,
...
struct upgtr_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  //
  template <
      typename VectorAP, typename VectorTAU, typename MatrixQ, typename WORK>
  static std::ptrdiff_t invoke(
      const char uplo, const VectorAP& ap, const VectorTAU& tau, MatrixQ& q,
    ...
    BOOST_ASSERT(
        ...
        std::max<std::ptrdiff_t>(1, bindings::size_column(q))
    );
    return detail::upgtr(
        uplo, bindings::size_column(q), bindings::begin_value(ap),
        bindings::begin_value(tau), bindings::begin_value(q),
        bindings::stride_major(q),
...
//
template <
    typename VectorAP, typename VectorTAU, typename MatrixQ, typename Workspace>
...
upgtr(
  ...
  return upgtr_impl<typename bindings::value_type<VectorAP>::type>::invoke(
      uplo, ap, tau, q, work
  );
}
...
upgtr(const char uplo, const VectorAP& ap, const VectorTAU& tau, MatrixQ& q) {
  return upgtr_impl<typename bindings::value_type<VectorAP>::type>::invoke(
      uplo, ap, tau, q, optimal_workspace()
  );
}
