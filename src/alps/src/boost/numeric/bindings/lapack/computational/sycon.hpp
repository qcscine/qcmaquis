...
inline std::ptrdiff_t sycon(
    const UpLo, const fortran_int_t n, const float* a, const fortran_int_t lda,
    const fortran_int_t* ipiv, const float anorm, float& rcond, float* work,
    fortran_int_t* iwork
) {
  fortran_int_t info(0);
  LAPACK_SSYCON(
      &lapack_option<UpLo>::value, &n, a, &lda, ipiv, &anorm, &rcond, work,
      iwork, &info
  );
  return info;
}
...
template <typename UpLo>
inline std::ptrdiff_t sycon(
    const UpLo, const fortran_int_t n, const double* a, const fortran_int_t lda,
...
template <typename UpLo>
inline std::ptrdiff_t sycon(
    const UpLo, const fortran_int_t n, const std::complex<float>* a,
...
template <typename UpLo>
inline std::ptrdiff_t sycon(
    const UpLo, const fortran_int_t n, const std::complex<double>* a,
...
struct sycon_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  //
  template <
      typename MatrixA, typename VectorIPIV, typename WORK, typename IWORK>
  static std::ptrdiff_t invoke(
      const MatrixA& a, const VectorIPIV& ipiv, const real_type anorm,
    ...
    return detail::sycon(
        ...
        bindings::stride_major(a), bindings::begin_value(ipiv), anorm, rcond,
  ...
  static std::ptrdiff_t invoke(
    ...
    return invoke(a, ipiv, anorm, rcond, workspace(tmp_work, tmp_iwork));
  }
...
sycon(
    const MatrixA& a, const VectorIPIV& ipiv,
  ...
  return sycon_impl<typename bindings::value_type<MatrixA>::type>::invoke(
