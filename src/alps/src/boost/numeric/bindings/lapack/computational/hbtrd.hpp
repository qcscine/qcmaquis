...
inline std::ptrdiff_t hbtrd(
    const char vect, const UpLo, const fortran_int_t n, const fortran_int_t kd,
    float* ab, const fortran_int_t ldab, float* d, float* e, float* q,
    const fortran_int_t ldq, float* work
) {
  fortran_int_t info(0);
  LAPACK_SSBTRD(
      &vect, &lapack_option<UpLo>::value, &n, &kd, ab, &ldab, d, e, q, &ldq,
      work, &info
  );
  return info;
}
...
inline std::ptrdiff_t hbtrd(
    ...
    std::complex<double>* q, const fortran_int_t ldq, std::complex<double>* work
) {
  fortran_int_t info(0);
  LAPACK_ZHBTRD(
      &vect, &lapack_option<UpLo>::value, &n, &kd, ab, &ldab, d, e, q, &ldq,
...
hbtrd(
  ...
  return hbtrd_impl<typename bindings::value_type<MatrixAB>::type>::invoke(
      vect, ab, d, e, q, work
  );
}
...
hbtrd(const char vect, MatrixAB& ab, VectorD& d, VectorE& e, MatrixQ& q) {
  return hbtrd_impl<typename bindings::value_type<MatrixAB>::type>::invoke(
      vect, ab, d, e, q, optimal_workspace()
  );
}
