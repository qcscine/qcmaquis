...
inline std::ptrdiff_t hetrd(
    const UpLo, const fortran_int_t n, float* a, const fortran_int_t lda,
    float* d, float* e, float* tau, float* work, const fortran_int_t lwork
) {
  fortran_int_t info(0);
  LAPACK_SSYTRD(
      &lapack_option<UpLo>::value, &n, a, &lda, d, e, tau, work, &lwork, &info
  );
  return info;
}
...
inline std::ptrdiff_t hetrd(
    ...
    double* d, double* e, double* tau, double* work, const fortran_int_t lwork
) {
  fortran_int_t info(0);
  LAPACK_DSYTRD(
      &lapack_option<UpLo>::value, &n, a, &lda, d, e, tau, work, &lwork, &info
...
inline std::ptrdiff_t hetrd(
    ...
    const fortran_int_t lda, float* d, float* e, std::complex<float>* tau,
...
inline std::ptrdiff_t hetrd(
    ...
    const fortran_int_t lda, double* d, double* e, std::complex<double>* tau,
...
struct hetrd_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  //
  template <
      typename MatrixA, typename VectorD, typename VectorE, typename VectorTAU,
      typename WORK>
  static std::ptrdiff_t invoke(
      MatrixA& a, VectorD& d, VectorE& e, VectorTAU& tau,
  ...
  template <
      typename MatrixA, typename VectorD, typename VectorE, typename VectorTAU>
  static std::ptrdiff_t invoke(
      MatrixA& a, VectorD& d, VectorE& e, VectorTAU& tau, minimal_workspace
  ) {
    namespace bindings = ::boost::numeric::bindings;
    typedef typename result_of::uplo_tag<MatrixA>::type uplo;
  ...
  template <
      typename MatrixA, typename VectorD, typename VectorE, typename VectorTAU>
  static std::ptrdiff_t invoke(
      MatrixA& a, VectorD& d, VectorE& e, VectorTAU& tau, optimal_workspace
  ) {
    namespace bindings = ::boost::numeric::bindings;
    typedef typename result_of::uplo_tag<MatrixA>::type uplo;
    ...
    detail::hetrd(
        uplo(), bindings::size_column(a), bindings::begin_value(a),
        bindings::stride_major(a), bindings::begin_value(d),
        bindings::begin_value(e), bindings::begin_value(tau), &opt_size_work, -1
    );
...
hetrd(MatrixA& a, VectorD& d, VectorE& e, VectorTAU& tau, Workspace work) {
  return hetrd_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      a, d, e, tau, work
  );
}
