...
inline std::ptrdiff_t trsen(
    ...
    std::complex<float>* q, const fortran_int_t ldq, std::complex<float>* w,
    fortran_int_t& m, float& s, float& sep, std::complex<float>* work,
    const fortran_int_t lwork
) {
  fortran_int_t info(0);
  LAPACK_CTRSEN(
      &job, &compq, select, &n, t, &ldt, q, &ldq, w, &m, &s, &sep, work, &lwork,
      &info
  );
  return info;
}
...
inline std::ptrdiff_t trsen(
    ...
    std::complex<double>* q, const fortran_int_t ldq, std::complex<double>* w,
...
struct trsen_impl {
  ...
  //
  template <
      typename VectorSELECT, typename MatrixT, typename MatrixQ,
      typename VectorW, typename WORK>
  static std::ptrdiff_t invoke(
      const char job, const char compq, const VectorSELECT& select, MatrixT& t,
      MatrixQ& q, VectorW& w, fortran_int_t& m, real_type& s, real_type& sep,
  ...
  static std::ptrdiff_t invoke(
    ...
    return invoke(job, compq, select, t, q, w, m, s, sep, workspace(tmp_work));
  }
  ...
  static std::ptrdiff_t invoke(
    ...
    detail::trsen(
        ...
        bindings::begin_value(t), bindings::stride_major(t),
        bindings::begin_value(q), bindings::stride_major(q),
        bindings::begin_value(w), m, s, sep, &opt_size_work, -1
    );
    bindings::detail::array<value_type> tmp_work(
  ...
  static std::ptrdiff_t min_size_work(
      const char job, const std::ptrdiff_t n, fortran_int_t& m
  ) {
    if (job == 'N')
      return 1;
    else if (job == 'E')
...
//
template <
    typename VectorSELECT, typename MatrixT, typename MatrixQ, typename VectorW,
    typename Workspace>
inline typename boost::enable_if<
    detail::is_workspace<Workspace>, std::ptrdiff_t>::type
trsen(
    const char job, const char compq, const VectorSELECT& select, MatrixT& t,
    MatrixQ& q, VectorW& w, fortran_int_t& m,
  ...
  return trsen_impl<typename bindings::value_type<VectorSELECT>::type>::invoke(
      job, compq, select, t, q, w, m, s, sep, work
  );
}
...
trsen(
  ...
  return trsen_impl<typename bindings::value_type<VectorSELECT>::type>::invoke(
      job, compq, select, t, q, w, m, s, sep, optimal_workspace()
  );
}
