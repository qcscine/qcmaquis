...
inline std::ptrdiff_t pbcon(
    const UpLo, const fortran_int_t n, const fortran_int_t kd, const float* ab,
    const fortran_int_t ldab, const float anorm, float& rcond, float* work,
    fortran_int_t* iwork
) {
  fortran_int_t info(0);
  LAPACK_SPBCON(
      &lapack_option<UpLo>::value, &n, &kd, ab, &ldab, &anorm, &rcond, work,
      iwork, &info
  );
  return info;
}
...
template <typename UpLo>
inline std::ptrdiff_t pbcon(
    const UpLo, const fortran_int_t n, const fortran_int_t kd, const double* ab,
    const fortran_int_t ldab, const double anorm, double& rcond, double* work,
    fortran_int_t* iwork
) {
  fortran_int_t info(0);
  LAPACK_DPBCON(
...
inline std::ptrdiff_t pbcon(
    ...
    const std::complex<float>* ab, const fortran_int_t ldab, const float anorm,
...
struct pbcon_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
    ...
    return detail::pbcon(
        ...
        bindings::begin_value(ab), bindings::stride_major(ab), anorm, rcond,
...
struct pbcon_impl<Value, typename boost::enable_if<is_complex<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
    ...
    BOOST_ASSERT(
        bindings::stride_major(ab) >= bindings::bandwidth_upper(ab) + 1
    );
    return detail::pbcon(
        uplo(), bindings::size_column(ab), bindings::bandwidth_upper(ab),
        bindings::begin_value(ab), bindings::stride_major(ab), anorm, rcond,
...
pbcon(
  ...
  return pbcon_impl<typename bindings::value_type<MatrixAB>::type>::invoke(
      ab, anorm, rcond, work
  );
}
