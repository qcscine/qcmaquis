...
// * float value-type.
//
inline std::ptrdiff_t geqlf(
    const fortran_int_t m, const fortran_int_t n, float* a,
    const fortran_int_t lda, float* tau, float* work, const fortran_int_t lwork
) {
...
geqlf(MatrixA& a, VectorTAU& tau, Workspace work) {
  return geqlf_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      a, tau, work
  );
}
