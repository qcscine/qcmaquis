...
unmlq(
  ...
  return unmlq_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      side, a, tau, c, work
  );
}
...
unmlq(const Side side, const MatrixA& a, const VectorTAU& tau, MatrixC& c) {
  return unmlq_impl<typename bindings::value_type<MatrixA>::type>::invoke(
