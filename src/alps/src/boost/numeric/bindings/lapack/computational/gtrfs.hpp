...
inline std::ptrdiff_t gtrfs(
    const Trans, const fortran_int_t n, const fortran_int_t nrhs,
  ...
  LAPACK_SGTRFS(
      &lapack_option<Trans>::value, &n, &nrhs, dl, d, du, dlf, df, duf, du2,
...
inline std::ptrdiff_t gtrfs(
    ...
    std::complex<float>* x, const fortran_int_t ldx, float* ferr, float* berr,
...
struct gtrfs_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
    ...
    BOOST_ASSERT(bindings::size(df) >= bindings::size_column_op(d, trans()));
    BOOST_ASSERT(
        bindings::size(dl) >= bindings::size_column_op(d, trans()) - 1
    );
    BOOST_ASSERT(
        bindings::size(dlf) >= bindings::size_column_op(d, trans()) - 1
    );
    BOOST_ASSERT(
        bindings::size(du) >= bindings::size_column_op(d, trans()) - 1
    );
    BOOST_ASSERT(
        bindings::size(du2) >= bindings::size_column_op(d, trans()) - 2
    );
    BOOST_ASSERT(
        bindings::size(duf) >= bindings::size_column_op(d, trans()) - 1
    );
    BOOST_ASSERT(bindings::size(ipiv) >= bindings::size_column_op(d, trans()));
    BOOST_ASSERT(
    ...
    return detail::gtrfs(
        trans(), bindings::size_column_op(d, trans()), bindings::size_column(b),
  ...
  static std::ptrdiff_t invoke(
      ...
      const VectorDLF& dlf, const VectorDF& df, const VectorDUF& duf,
      const VectorDU2& du2, const VectorIPIV& ipiv, const MatrixB& b,
      MatrixX& x, VectorFERR& ferr, VectorBERR& berr, minimal_workspace
  ) {
    namespace bindings = ::boost::numeric::bindings;
    typedef tag::column_major order;
    typedef typename result_of::trans_tag<VectorD, order>::type trans;
    ...
    return invoke(
        dl, d, du, dlf, df, duf, du2, ipiv, b, x, ferr, berr,
  ...
  static std::ptrdiff_t invoke(
      ...
      const VectorDLF& dlf, const VectorDF& df, const VectorDUF& duf,
      const VectorDU2& du2, const VectorIPIV& ipiv, const MatrixB& b,
      MatrixX& x, VectorFERR& ferr, VectorBERR& berr, optimal_workspace
  ) {
    namespace bindings = ::boost::numeric::bindings;
    typedef tag::column_major order;
    typedef typename result_of::trans_tag<VectorD, order>::type trans;
...
gtrfs(
    const VectorDL& dl, const VectorD& d, const VectorDU& du,
    const VectorDLF& dlf, const VectorDF& df, const VectorDUF& duf,
    const VectorDU2& du2, const VectorIPIV& ipiv, const MatrixB& b, MatrixX& x,
    VectorFERR& ferr, VectorBERR& berr, Workspace work
) {
  return gtrfs_impl<typename bindings::value_type<VectorDL>::type>::invoke(
      dl, d, du, dlf, df, duf, du2, ipiv, b, x, ferr, berr, work
  );
}
...
gtrfs(
  ...
  return gtrfs_impl<typename bindings::value_type<VectorDL>::type>::invoke(
      dl, d, du, dlf, df, duf, du2, ipiv, b, x, ferr, berr, optimal_workspace()
  );
}
