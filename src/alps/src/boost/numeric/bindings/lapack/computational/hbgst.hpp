...
inline std::ptrdiff_t hbgst(
    const char vect, const UpLo, const fortran_int_t n, const fortran_int_t ka,
    const fortran_int_t kb, float* ab, const fortran_int_t ldab,
    const float* bb, const fortran_int_t ldbb, float* x,
    const fortran_int_t ldx, float* work
) {
  fortran_int_t info(0);
  LAPACK_SSBGST(
      &vect, &lapack_option<UpLo>::value, &n, &ka, &kb, ab, &ldab, bb, &ldbb, x,
      &ldx, work, &info
  );
  return info;
}
...
inline std::ptrdiff_t hbgst(
    ...
    const fortran_int_t ldx, double* work
) {
  fortran_int_t info(0);
  LAPACK_DSBGST(
      &vect, &lapack_option<UpLo>::value, &n, &ka, &kb, ab, &ldab, bb, &ldbb, x,
...
inline std::ptrdiff_t hbgst(
    ...
    std::complex<float>* x, const fortran_int_t ldx, std::complex<float>* work,
    float* rwork
) {
  fortran_int_t info(0);
  LAPACK_CHBGST(
...
struct hbgst_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  //
  template <
      typename MatrixAB, typename MatrixBB, typename MatrixX, typename WORK>
  static std::ptrdiff_t invoke(
      const char vect, MatrixAB& ab, const MatrixBB& bb, MatrixX& x,
    ...
    return detail::hbgst(
        vect, uplo(), bindings::size_column(ab),
...
//
template <
    typename MatrixAB, typename MatrixBB, typename MatrixX, typename Workspace>
...
hbgst(
  ...
  return hbgst_impl<typename bindings::value_type<MatrixAB>::type>::invoke(
      vect, ab, bb, x, work
  );
}
...
hbgst(const char vect, MatrixAB& ab, const MatrixBB& bb, MatrixX& x) {
  return hbgst_impl<typename bindings::value_type<MatrixAB>::type>::invoke(
      vect, ab, bb, x, optimal_workspace()
  );
}
