...
inline std::ptrdiff_t gebrd(
    ...
    const fortran_int_t lda, float* d, float* e, std::complex<float>* tauq,
...
inline std::ptrdiff_t gebrd(
    ...
    const fortran_int_t lda, double* d, double* e, std::complex<double>* tauq,
...
struct gebrd_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  template <
      ...
      typename VectorTAUP, typename WORK>
  static std::ptrdiff_t invoke(
    ...
    BOOST_ASSERT(
        bindings::size(d) >= std::min<std::ptrdiff_t>(
    ...
    BOOST_ASSERT(
        ...
        min_size_work(bindings::size_row(a), bindings::size_column(a))
    );
...
template <
    typename MatrixA, typename VectorD, typename VectorE, typename VectorTAUQ,
    typename VectorTAUP, typename Workspace>
...
gebrd(
  ...
  return gebrd_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      a, d, e, tauq, taup, work
  );
}
...
inline typename boost::disable_if<
    detail::is_workspace<VectorTAUP>, std::ptrdiff_t>::type
gebrd(MatrixA& a, VectorD& d, VectorE& e, VectorTAUQ& tauq, VectorTAUP& taup) {
  return gebrd_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      a, d, e, tauq, taup, optimal_workspace()
  );
}
