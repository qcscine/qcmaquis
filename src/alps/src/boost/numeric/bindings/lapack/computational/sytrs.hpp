...
inline std::ptrdiff_t sytrs(
    const UpLo, const fortran_int_t n, const fortran_int_t nrhs, const float* a,
    const fortran_int_t lda, const fortran_int_t* ipiv, float* b,
    const fortran_int_t ldb
) {
  fortran_int_t info(0);
  LAPACK_SSYTRS(
      &lapack_option<UpLo>::value, &n, &nrhs, a, &lda, ipiv, b, &ldb, &info
  );
  return info;
}
...
inline std::ptrdiff_t sytrs(
  ...
  return sytrs_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      a, ipiv, b
  );
}
