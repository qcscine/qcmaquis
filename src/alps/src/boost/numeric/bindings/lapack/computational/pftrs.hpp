...
inline std::ptrdiff_t pftrs(
    const TransR, const UpLo, const fortran_int_t n, const fortran_int_t nrhs,
    const float* a, float* b, const fortran_int_t ldb
) {
  fortran_int_t info(0);
  LAPACK_SPFTRS(
      &lapack_option<TransR>::value, &lapack_option<UpLo>::value, &n, &nrhs, a,
      b, &ldb, &info
  );
  return info;
}
...
inline std::ptrdiff_t pftrs(
    const TransR, const UpLo, const fortran_int_t n, const fortran_int_t nrhs,
    const double* a, double* b, const fortran_int_t ldb
) {
  fortran_int_t info(0);
  LAPACK_DPFTRS(
      &lapack_option<TransR>::value, &lapack_option<UpLo>::value, &n, &nrhs, a,
...
struct pftrs_impl {
  ...
  static std::ptrdiff_t invoke(const MatrixA& a, MatrixB& b) {
    ...
    BOOST_ASSERT(
        ...
        std::max<std::ptrdiff_t>(1, bindings::size_column_op(a, transr()))
    );
    return detail::pftrs(
        transr(), uplo(), bindings::size_column_op(a, transr()),
...
inline std::ptrdiff_t pftrs(const MatrixA& a, MatrixB& b) {
  return pftrs_impl<typename bindings::value_type<MatrixA>::type>::invoke(a, b);
}
