...
inline std::ptrdiff_t unmql(
    const Side, const Trans, const fortran_int_t m, const fortran_int_t n,
    const fortran_int_t k, float* a, const fortran_int_t lda, const float* tau,
    float* c, const fortran_int_t ldc, float* work, const fortran_int_t lwork
) {
...
struct unmql_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
      const Side side, MatrixA& a, const VectorTAU& tau, MatrixC& c,
...
unmql(
  ...
  return unmql_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      side, a, tau, c, work
  );
}
