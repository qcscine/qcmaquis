...
inline std::ptrdiff_t trsyl(
    ...
    double* c, const fortran_int_t ldc, double& scale
) {
  fortran_int_t info(0);
  LAPACK_DTRSYL(
      &trana, &tranb, &isgn, &m, &n, a, &lda, b, &ldb, c, &ldc, &scale, &info
...
inline std::ptrdiff_t trsyl(
    const char trana, const char tranb, const fortran_int_t isgn,
    const fortran_int_t m, const fortran_int_t n, const std::complex<float>* a,
...
inline std::ptrdiff_t trsyl(
    const char trana, const char tranb, const fortran_int_t isgn,
    const fortran_int_t m, const fortran_int_t n, const std::complex<double>* a,
...
struct trsyl_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
      const char trana, const char tranb, const fortran_int_t isgn,
      const fortran_int_t m, const fortran_int_t n, const MatrixA& a,
      const MatrixB& b, MatrixC& c, real_type& scale
  ) {
...
inline std::ptrdiff_t trsyl(
  ...
  return trsyl_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      trana, tranb, isgn, m, n, a, b, c, scale
  );
}
