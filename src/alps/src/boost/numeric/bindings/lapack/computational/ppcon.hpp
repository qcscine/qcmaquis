...
inline std::ptrdiff_t ppcon(
    const UpLo, const fortran_int_t n, const float* ap, const float anorm,
    float& rcond, float* work, fortran_int_t* iwork
) {
  fortran_int_t info(0);
  LAPACK_SPPCON(
      &lapack_option<UpLo>::value, &n, ap, &anorm, &rcond, work, iwork, &info
  );
  return info;
}
...
template <typename UpLo>
inline std::ptrdiff_t ppcon(
    const UpLo, const fortran_int_t n, const double* ap, const double anorm,
...
template <typename UpLo>
inline std::ptrdiff_t ppcon(
    const UpLo, const fortran_int_t n, const std::complex<float>* ap,
    const float anorm, float& rcond, std::complex<float>* work, float* rwork
) {
  fortran_int_t info(0);
  LAPACK_CPPCON(
      &lapack_option<UpLo>::value, &n, ap, &anorm, &rcond, work, rwork, &info
  );
  return info;
}
...
inline std::ptrdiff_t ppcon(
    ...
    const double anorm, double& rcond, std::complex<double>* work, double* rwork
) {
  fortran_int_t info(0);
  LAPACK_ZPPCON(
      &lapack_option<UpLo>::value, &n, ap, &anorm, &rcond, work, rwork, &info
...
struct ppcon_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
      const MatrixAP& ap, const real_type anorm, real_type& rcond,
    ...
    return detail::ppcon(
        uplo(), bindings::size_column(ap), bindings::begin_value(ap), anorm,
...
ppcon(
  ...
  return ppcon_impl<typename bindings::value_type<MatrixAP>::type>::invoke(
      ap, anorm, rcond, work
  );
}
...
ppcon(
  ...
  return ppcon_impl<typename bindings::value_type<MatrixAP>::type>::invoke(
      ap, anorm, rcond, optimal_workspace()
  );
}
