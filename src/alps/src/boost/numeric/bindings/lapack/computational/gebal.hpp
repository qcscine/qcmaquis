...
struct gebal_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
      const char job, MatrixA& a, fortran_int_t& ilo, fortran_int_t& ihi,
      VectorSCALE& scale
  ) {
    ...
    return detail::gebal(
        ...
        bindings::stride_major(a), ilo, ihi, bindings::begin_value(scale)
    );
  }
};
...
inline std::ptrdiff_t gebal(
  ...
  return gebal_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      job, a, ilo, ihi, scale
  );
}
