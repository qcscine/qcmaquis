...
inline std::ptrdiff_t ormtr(
    const Side, const UpLo, const Trans, const fortran_int_t m,
  ...
  LAPACK_SORMTR(
      &lapack_option<Side>::value, &lapack_option<UpLo>::value,
      &lapack_option<Trans>::value, &m, &n, a, &lda, tau, c, &ldc, work, &lwork,
      &info
  );
  return info;
}
...
ormtr(
  ...
  return ormtr_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      side, a, tau, c, work
  );
}
...
ormtr(const Side side, const MatrixA& a, const VectorTAU& tau, MatrixC& c) {
  return ormtr_impl<typename bindings::value_type<MatrixA>::type>::invoke(
