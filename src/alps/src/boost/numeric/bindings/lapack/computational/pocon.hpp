...
inline std::ptrdiff_t pocon(
  ...
  LAPACK_SPOCON(
      &lapack_option<UpLo>::value, &n, a, &lda, &anorm, &rcond, work, iwork,
      &info
  );
  return info;
}
...
inline std::ptrdiff_t pocon(
  ...
  fortran_int_t info(0);
  LAPACK_CPOCON(
      &lapack_option<UpLo>::value, &n, a, &lda, &anorm, &rcond, work, rwork,
      &info
  );
  return info;
}
...
struct pocon_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
      const MatrixA& a, const real_type anorm, real_type& rcond,
    ...
    return detail::pocon(
        ...
        bindings::stride_major(a), anorm, rcond,
...
pocon(
  ...
  return pocon_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      a, anorm, rcond, work
  );
}
