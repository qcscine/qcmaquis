...
inline std::ptrdiff_t hptri(
    const UpLo, const fortran_int_t n, float* ap, const fortran_int_t* ipiv,
    float* work
) {
...
template <typename UpLo>
inline std::ptrdiff_t hptri(
    const UpLo, const fortran_int_t n, double* ap, const fortran_int_t* ipiv,
    double* work
) {
  fortran_int_t info(0);
...
struct hptri_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
      MatrixAP& ap, const VectorIPIV& ipiv, detail::workspace1<WORK> work
  ) {
  ...
  static std::ptrdiff_t invoke(
      MatrixAP& ap, const VectorIPIV& ipiv, minimal_workspace
  ) {
    namespace bindings = ::boost::numeric::bindings;
    typedef typename result_of::uplo_tag<MatrixAP>::type uplo;
  ...
  template <typename MatrixAP, typename VectorIPIV>
  static std::ptrdiff_t invoke(
      MatrixAP& ap, const VectorIPIV& ipiv, optimal_workspace
  ) {
    namespace bindings = ::boost::numeric::bindings;
    typedef typename result_of::uplo_tag<MatrixAP>::type uplo;
...
hptri(MatrixAP& ap, const VectorIPIV& ipiv, Workspace work) {
  return hptri_impl<typename bindings::value_type<MatrixAP>::type>::invoke(
      ap, ipiv, work
  );
}
