...
inline std::ptrdiff_t pptrs(
  ...
  fortran_int_t info(0);
  LAPACK_DPPTRS(&lapack_option<UpLo>::value, &n, &nrhs, ap, b, &ldb, &info);
  return info;
}
...
inline std::ptrdiff_t pptrs(
  ...
  fortran_int_t info(0);
  LAPACK_CPPTRS(&lapack_option<UpLo>::value, &n, &nrhs, ap, b, &ldb, &info);
  return info;
}
...
inline std::ptrdiff_t pptrs(
  ...
  fortran_int_t info(0);
  LAPACK_ZPPTRS(&lapack_option<UpLo>::value, &n, &nrhs, ap, b, &ldb, &info);
  return info;
}
...
inline std::ptrdiff_t pptrs(const MatrixAP& ap, MatrixB& b) {
  return pptrs_impl<typename bindings::value_type<MatrixAP>::type>::invoke(
      ap, b
  );
}
