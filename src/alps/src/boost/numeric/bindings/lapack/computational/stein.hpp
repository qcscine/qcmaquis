...
struct stein_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
      ...
      const fortran_int_t m, const VectorW& w, const VectorIBLOCK& iblock,
    ...
    BOOST_STATIC_ASSERT(
        (std::is_same<
            typename remove_const<
                typename bindings::value_type<VectorIBLOCK>::type>::type,
            typename remove_const<
                typename bindings::value_type<VectorIFAIL>::type>::type>::value)
    );
    ...
    BOOST_ASSERT(bindings::stride_major(z) >= std::max<std::ptrdiff_t>(1, n));
    BOOST_ASSERT(n >= 0);
    return detail::stein(
        n, bindings::begin_value(d), bindings::begin_value(e), m,
        bindings::begin_value(w), bindings::begin_value(iblock),
        bindings::begin_value(isplit), bindings::begin_value(z),
  ...
  static std::ptrdiff_t invoke(
    ...
    return invoke(
        n, d, e, m, w, iblock, isplit, z, ifail, workspace(tmp_work, tmp_iwork)
    );
  }
  ...
  static std::ptrdiff_t invoke(
    ...
    namespace bindings = ::boost::numeric::bindings;
    return invoke(n, d, e, m, w, iblock, isplit, z, ifail, minimal_workspace());
  }
...
template <
    ...
    typename VectorISPLIT, typename MatrixZ, typename VectorIFAIL,
    typename Workspace>
...
stein(
    const fortran_int_t n, const VectorD& d, const VectorE& e,
    const fortran_int_t m, const VectorW& w, const VectorIBLOCK& iblock,
    const VectorISPLIT& isplit, MatrixZ& z, VectorIFAIL& ifail, Workspace work
) {
  return stein_impl<typename bindings::value_type<MatrixZ>::type>::invoke(
      n, d, e, m, w, iblock, isplit, z, ifail, work
  );
}
...
stein(
  ...
  return stein_impl<typename bindings::value_type<MatrixZ>::type>::invoke(
      n, d, e, m, w, iblock, isplit, z, ifail, optimal_workspace()
  );
}
