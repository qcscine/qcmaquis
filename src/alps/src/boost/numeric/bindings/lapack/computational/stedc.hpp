...
inline std::ptrdiff_t stedc(
  ...
  LAPACK_SSTEDC(&compz, &n, d, e, z, &ldz, work, &lwork, iwork, &liwork, &info);
  return info;
}
...
inline std::ptrdiff_t stedc(
  ...
  fortran_int_t info(0);
  LAPACK_DSTEDC(&compz, &n, d, e, z, &ldz, work, &lwork, iwork, &liwork, &info);
  return info;
}
...
struct stedc_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  static std::ptrdiff_t min_size_work(
      const char compz, const std::ptrdiff_t n
  ) {
    if (compz == 'N' || n <= 1) {
      return 1;
    } else if (compz == 'V') {
...
struct stedc_impl<Value, typename boost::enable_if<is_complex<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
    ...
    return invoke(compz, d, e, z, workspace(tmp_work, tmp_rwork, tmp_iwork));
  }
...
stedc(const char compz, VectorD& d, VectorE& e, MatrixZ& z, Workspace work) {
  return stedc_impl<typename bindings::value_type<MatrixZ>::type>::invoke(
      compz, d, e, z, work
  );
}
