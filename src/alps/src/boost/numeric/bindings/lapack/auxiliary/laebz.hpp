...
// * float value-type.
//
inline std::ptrdiff_t laebz(
    const fortran_int_t ijob, const fortran_int_t nitmax, const fortran_int_t n,
  ...
  LAPACK_SLAEBZ(
      &ijob, &nitmax, &n, &mmax, &minp, &nbmin, &abstol, &reltol, &pivmin, d, e,
...
inline std::ptrdiff_t laebz(
    ...
    const double pivmin, const double* d, const double* e, const double* e2,
...
struct laebz_impl {
  ...
  static std::ptrdiff_t invoke(
      ...
      const fortran_int_t nbmin, const real_type abstol, const real_type reltol,
      const real_type pivmin, const VectorD& d, const VectorE& e,
      const VectorE2& e2, VectorNVAL& nval, MatrixAB& ab, VectorC& c,
      fortran_int_t& mout, MatrixNAB& nab, detail::workspace2<WORK, IWORK> work
  ) {
    ...
    BOOST_STATIC_ASSERT(
        (boost::is_same<
            typename remove_const<
                typename bindings::value_type<VectorD>::type>::type,
            typename remove_const<
                typename bindings::value_type<MatrixAB>::type>::type>::value)
    );
    BOOST_STATIC_ASSERT(
        (boost::is_same<
            typename remove_const<
    ...
    BOOST_STATIC_ASSERT(
        (boost::is_same<
            ...
            typename remove_const<
                typename bindings::value_type<MatrixNAB>::type>::type>::value)
    );
    ...
    return detail::laebz(
        ijob, nitmax, n, bindings::stride_major(ab), minp, nbmin, abstol,
        reltol, pivmin, bindings::begin_value(d), bindings::begin_value(e),
        bindings::begin_value(e2), bindings::begin_value(nval),
        bindings::begin_value(ab), bindings::begin_value(c), mout,
        bindings::begin_value(nab),
  ...
  static std::ptrdiff_t invoke(
    ...
    return invoke(
        ijob, nitmax, n, minp, nbmin, abstol, reltol, pivmin, d, e, e2, nval,
        ab, c, mout, nab, workspace(tmp_work, tmp_iwork)
    );
  }
  ...
  static std::ptrdiff_t invoke(
    ...
    return invoke(
        ijob, nitmax, n, minp, nbmin, abstol, reltol, pivmin, d, e, e2, nval,
        ab, c, mout, nab, minimal_workspace()
    );
  }
...
//
template <
    typename VectorD, typename VectorE, typename VectorE2, typename VectorNVAL,
    typename MatrixAB, typename VectorC, typename MatrixNAB, typename Workspace>
...
laebz(
    ...
    const typename remove_imaginary<
        typename bindings::value_type<VectorD>::type>::type reltol,
    const typename remove_imaginary<
        typename bindings::value_type<VectorD>::type>::type pivmin,
    const VectorD& d, const VectorE& e, const VectorE2& e2, VectorNVAL& nval,
    MatrixAB& ab, VectorC& c, fortran_int_t& mout, MatrixNAB& nab,
    Workspace work
) {
  return laebz_impl<typename bindings::value_type<VectorD>::type>::invoke(
      ijob, nitmax, n, minp, nbmin, abstol, reltol, pivmin, d, e, e2, nval, ab,
      c, mout, nab, work
  );
}
...
template <
    typename VectorD, typename VectorE, typename VectorE2, typename VectorNVAL,
    typename MatrixAB, typename VectorC, typename MatrixNAB>
inline typename boost::disable_if<
    detail::is_workspace<MatrixNAB>, std::ptrdiff_t>::type
laebz(
    const fortran_int_t ijob, const fortran_int_t nitmax, const fortran_int_t n,
    const fortran_int_t minp, const fortran_int_t nbmin,
    const typename remove_imaginary<
