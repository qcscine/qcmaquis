...
struct latrz_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
      MatrixA& a, VectorTAU& tau, detail::workspace1<WORK> work
  ) {
  ...
  static std::ptrdiff_t invoke(MatrixA& a, VectorTAU& tau, minimal_workspace) {
    namespace bindings = ::boost::numeric::bindings;
  ...
  //
  static std::ptrdiff_t min_size_work(const std::ptrdiff_t m) { return m; }
};
...
latrz(MatrixA& a, VectorTAU& tau, Workspace work) {
  return latrz_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      a, tau, work
  );
}
