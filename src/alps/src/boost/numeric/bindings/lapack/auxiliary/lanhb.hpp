...
inline std::ptrdiff_t lanhb(
  ...
  LAPACK_CLANHB(&norm, &lapack_option<UpLo>::value, &n, &k, ab, &ldab, work);
  return info;
}
...
inline std::ptrdiff_t lanhb(
  ...
  fortran_int_t info(0);
  LAPACK_ZLANHB(&norm, &lapack_option<UpLo>::value, &n, &k, ab, &ldab, work);
  return info;
}
...
lanhb(const char norm, const MatrixAB& ab, Workspace work) {
  return lanhb_impl<typename bindings::value_type<MatrixAB>::type>::invoke(
      norm, ab, work
  );
}
