...
struct larnv_impl {
  ...
  static std::ptrdiff_t invoke(
      const fortran_int_t idist, VectorISEED& iseed, const fortran_int_t n,
      VectorX& x
  ) {
    ...
    return detail::larnv(
        idist, bindings::begin_value(iseed), n, bindings::begin_value(x)
    );
  }
};
...
inline std::ptrdiff_t larnv(
  ...
  return larnv_impl<typename bindings::value_type<VectorX>::type>::invoke(
      idist, iseed, n, x
  );
}
