...
inline std::ptrdiff_t lansb(
    const char norm, const UpLo, const fortran_int_t n, const fortran_int_t k,
    const float* ab, const fortran_int_t ldab, float* work
) {
  fortran_int_t info(0);
  LAPACK_SLANSB(&norm, &lapack_option<UpLo>::value, &n, &k, ab, &ldab, work);
  return info;
}
...
inline std::ptrdiff_t lansb(
    const char norm, const UpLo, const fortran_int_t n, const fortran_int_t k,
    const double* ab, const fortran_int_t ldab, double* work
) {
  fortran_int_t info(0);
  LAPACK_DLANSB(&norm, &lapack_option<UpLo>::value, &n, &k, ab, &ldab, work);
  return info;
}
...
inline std::ptrdiff_t lansb(
    const char norm, const UpLo, const fortran_int_t n, const fortran_int_t k,
    const std::complex<float>* ab, const fortran_int_t ldab, float* work
) {
  fortran_int_t info(0);
  LAPACK_CLANSB(&norm, &lapack_option<UpLo>::value, &n, &k, ab, &ldab, work);
  return info;
}
...
inline std::ptrdiff_t lansb(
    const char norm, const UpLo, const fortran_int_t n, const fortran_int_t k,
    const std::complex<double>* ab, const fortran_int_t ldab, double* work
) {
  fortran_int_t info(0);
  LAPACK_ZLANSB(&norm, &lapack_option<UpLo>::value, &n, &k, ab, &ldab, work);
  return info;
}
...
struct lansb_impl {
  ...
  static std::ptrdiff_t invoke(
      const char norm, const MatrixAB& ab, detail::workspace1<WORK> work
  ) {
    ...
    BOOST_ASSERT(
        bindings::stride_major(ab) >= bindings::bandwidth_lower(ab) + 1
    );
    return detail::lansb(
        norm, uplo(), bindings::size_column(ab), bindings::bandwidth_lower(ab),
  ...
  static std::ptrdiff_t invoke(
      const char norm, const MatrixAB& ab, minimal_workspace
  ) {
    namespace bindings = ::boost::numeric::bindings;
    typedef typename result_of::uplo_tag<MatrixAB>::type uplo;
  ...
  template <typename MatrixAB>
  static std::ptrdiff_t invoke(
      const char norm, const MatrixAB& ab, optimal_workspace
  ) {
    namespace bindings = ::boost::numeric::bindings;
    typedef typename result_of::uplo_tag<MatrixAB>::type uplo;
...
lansb(const char norm, const MatrixAB& ab, Workspace work) {
  return lansb_impl<typename bindings::value_type<MatrixAB>::type>::invoke(
      norm, ab, work
  );
}
...
inline typename boost::disable_if<
    detail::is_workspace<MatrixAB>, std::ptrdiff_t>::type
lansb(const char norm, const MatrixAB& ab) {
  return lansb_impl<typename bindings::value_type<MatrixAB>::type>::invoke(
      norm, ab, optimal_workspace()
  );
}
