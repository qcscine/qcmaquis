...
inline std::ptrdiff_t latrs(
    const char uplo, const Trans, const Diag, const char normin,
    const fortran_int_t n, const float* a, const fortran_int_t lda, float* x,
    float& scale, float* cnorm
) {
  fortran_int_t info(0);
  LAPACK_SLATRS(
      &uplo, &lapack_option<Trans>::value, &lapack_option<Diag>::value, &normin,
...
inline std::ptrdiff_t latrs(
    ...
    const fortran_int_t lda, std::complex<float>* x, float& scale, float* cnorm
) {
  fortran_int_t info(0);
  LAPACK_CLATRS(
      &uplo, &lapack_option<Trans>::value, &lapack_option<Diag>::value, &normin,
...
struct latrs_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
      const char uplo, const char normin, const MatrixA& a, VectorX& x,
      real_type& scale, VectorCNORM& cnorm
  ) {
    ...
    BOOST_STATIC_ASSERT(
        (std::is_same<
            typename remove_const<
                typename bindings::value_type<MatrixA>::type>::type,
            typename remove_const<
                typename bindings::value_type<VectorCNORM>::type>::type>::value)
    );
    BOOST_STATIC_ASSERT((bindings::is_mutable<VectorX>::value));
    BOOST_STATIC_ASSERT((bindings::is_mutable<VectorCNORM>::value));
    BOOST_ASSERT(bindings::size(x) >= bindings::size_column_op(a, trans()));
    ...
    return detail::latrs(
        uplo, trans(), diag(), normin, bindings::size_column_op(a, trans()),
        bindings::begin_value(a), bindings::stride_major(a),
        bindings::begin_value(x), scale, bindings::begin_value(cnorm)
    );
  }
};
...
inline std::ptrdiff_t latrs(
  ...
  return latrs_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      uplo, normin, a, x, scale, cnorm
  );
}
