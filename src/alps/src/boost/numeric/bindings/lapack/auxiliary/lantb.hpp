...
inline std::ptrdiff_t lantb(
  ...
  LAPACK_SLANTB(
      &norm, &uplo, &lapack_option<Diag>::value, &n, &k, ab, &ldab, work
  );
  return info;
}
...
struct lantb_impl {
  ...
  static std::ptrdiff_t invoke(
      const char norm, const char uplo, const fortran_int_t k,
    ...
    BOOST_ASSERT(k >= 0);
    return detail::lantb(
        norm, uplo, diag(), bindings::size_column(ab), k,
  ...
  static std::ptrdiff_t invoke(
      const char norm, const char uplo, const fortran_int_t k,
      const MatrixAB& ab, minimal_workspace
  ) {
    namespace bindings = ::boost::numeric::bindings;
    typedef typename result_of::diag_tag<MatrixAB>::type diag;
  ...
  static std::ptrdiff_t invoke(
      const char norm, const char uplo, const fortran_int_t k,
      const MatrixAB& ab, optimal_workspace
  ) {
    namespace bindings = ::boost::numeric::bindings;
    typedef typename result_of::diag_tag<MatrixAB>::type diag;
...
lantb(
    const char norm, const char uplo, const fortran_int_t k, const MatrixAB& ab,
    Workspace work
) {
  return lantb_impl<typename bindings::value_type<MatrixAB>::type>::invoke(
      norm, uplo, k, ab, work
  );
}
...
lantb(
  ...
  return lantb_impl<typename bindings::value_type<MatrixAB>::type>::invoke(
      norm, uplo, k, ab, optimal_workspace()
  );
}
