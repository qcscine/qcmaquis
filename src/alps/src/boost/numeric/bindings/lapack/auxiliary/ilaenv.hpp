...
inline std::ptrdiff_t ilaenv(
    const fortran_int_t ispec, const char* name, const char* opts,
    const fortran_int_t n1 = -1, const fortran_int_t n2 = -1,
    const fortran_int_t n3 = -1, const fortran_int_t n4 = -1
) {
  return LAPACK_ILAENV(
      &ispec, name, opts, &n1, &n2, &n3, &n4, std::strlen(name),
      std::strlen(opts)
  );
}
