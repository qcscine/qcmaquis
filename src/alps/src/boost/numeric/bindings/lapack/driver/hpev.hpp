...
inline std::ptrdiff_t hpev(
  ...
  fortran_int_t info(0);
  LAPACK_CHPEV(
      &jobz, &lapack_option<UpLo>::value, &n, ap, w, z, &ldz, work, rwork, &info
  );
  return info;
}
...
hpev(const char jobz, MatrixAP& ap, VectorW& w, MatrixZ& z, Workspace work) {
  return hpev_impl<typename bindings::value_type<MatrixAP>::type>::invoke(
      jobz, ap, w, z, work
  );
}
