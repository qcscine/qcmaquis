...
inline std::ptrdiff_t gesvx(
    const char fact, const Trans, const fortran_int_t n,
  ...
  LAPACK_SGESVX(
      &fact, &lapack_option<Trans>::value, &n, &nrhs, a, &lda, af, &ldaf, ipiv,
...
inline std::ptrdiff_t gesvx(
    const char fact, const Trans, const fortran_int_t n,
    const fortran_int_t nrhs, std::complex<float>* a, const fortran_int_t lda,
    std::complex<float>* af, const fortran_int_t ldaf, fortran_int_t* ipiv,
    char& equed, float* r, float* c, std::complex<float>* b,
    const fortran_int_t ldb, std::complex<float>* x, const fortran_int_t ldx,
...
inline std::ptrdiff_t gesvx(
    const char fact, const Trans, const fortran_int_t n,
    const fortran_int_t nrhs, std::complex<double>* a, const fortran_int_t lda,
    std::complex<double>* af, const fortran_int_t ldaf, fortran_int_t* ipiv,
    char& equed, double* r, double* c, std::complex<double>* b,
    const fortran_int_t ldb, std::complex<double>* x, const fortran_int_t ldx,
...
struct gesvx_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  //
  template <
      typename MatrixA, typename MatrixAF, typename VectorIPIV,
      typename VectorR, typename VectorC, typename MatrixB, typename MatrixX,
      typename VectorFERR, typename VectorBERR, typename WORK, typename IWORK>
  static std::ptrdiff_t invoke(
      const char fact, MatrixA& a, MatrixAF& af, VectorIPIV& ipiv, char& equed,
      VectorR& r, VectorC& c, MatrixB& b, MatrixX& x, real_type& rcond,
      VectorFERR& ferr, VectorBERR& berr, detail::workspace2<WORK, IWORK> work
  ) {
    ...
    BOOST_STATIC_ASSERT(
        (boost::is_same<
            ...
            typename remove_const<
                typename bindings::value_type<MatrixAF>::type>::type>::value)
    );
    BOOST_STATIC_ASSERT(
        (boost::is_same<
            typename remove_const<
                typename bindings::value_type<MatrixA>::type>::type,
            typename remove_const<
                typename bindings::value_type<VectorR>::type>::type>::value)
    );
    BOOST_STATIC_ASSERT(
        (boost::is_same<
            typename remove_const<
    ...
    BOOST_ASSERT(
        bindings::size_minor(af) == 1 || bindings::stride_minor(af) == 1
    );
    BOOST_ASSERT(
        bindings::size_minor(b) == 1 || bindings::stride_minor(b) == 1
    );
    BOOST_ASSERT(
        bindings::size_minor(x) == 1 || bindings::stride_minor(x) == 1
    );
    BOOST_ASSERT(
        bindings::stride_major(a) >=
        std::max<std::ptrdiff_t>(1, bindings::size_column_op(a, trans()))
    );
    BOOST_ASSERT(
        bindings::stride_major(af) >=
        std::max<std::ptrdiff_t>(1, bindings::size_column_op(a, trans()))
    );
    BOOST_ASSERT(
    ...
    BOOST_ASSERT(equed == 'N' || equed == 'R' || equed == 'C' || equed == 'B');
    BOOST_ASSERT(fact == 'F' || fact == 'N' || fact == 'E');
    return detail::gesvx(
        fact, trans(), bindings::size_column_op(a, trans()),
        bindings::size_column(b), bindings::begin_value(a),
        bindings::stride_major(a), bindings::begin_value(af),
        bindings::stride_major(af), bindings::begin_value(ipiv), equed,
        bindings::begin_value(r), bindings::begin_value(c),
  ...
  static std::ptrdiff_t invoke(
    ...
    return invoke(
        fact, a, af, ipiv, equed, r, c, b, x, rcond, ferr, berr,
...
struct gesvx_impl<Value, typename boost::enable_if<is_complex<Value> >::type> {
  ...
  template <
      typename MatrixA, typename MatrixAF, typename VectorIPIV,
      typename VectorR, typename VectorC, typename MatrixB, typename MatrixX,
      typename VectorFERR, typename VectorBERR, typename WORK, typename RWORK>
  static std::ptrdiff_t invoke(
      const char fact, MatrixA& a, MatrixAF& af, VectorIPIV& ipiv, char& equed,
      VectorR& r, VectorC& c, MatrixB& b, MatrixX& x, real_type& rcond,
      VectorFERR& ferr, VectorBERR& berr, detail::workspace2<WORK, RWORK> work
  ) {
    namespace bindings = ::boost::numeric::bindings;
    typedef typename result_of::data_order<MatrixAF>::type order;
    typedef typename result_of::trans_tag<MatrixA, order>::type trans;
...
//
template <
    typename MatrixA, typename MatrixAF, typename VectorIPIV, typename VectorR,
    typename VectorC, typename MatrixB, typename MatrixX, typename VectorFERR,
...
gesvx(
  ...
  return gesvx_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      fact, a, af, ipiv, equed, r, c, b, x, rcond, ferr, berr, work
  );
}
...
inline typename boost::disable_if<
    detail::is_workspace<VectorBERR>, std::ptrdiff_t>::type
gesvx(
    const char fact, MatrixA& a, MatrixAF& af, VectorIPIV& ipiv, char& equed,
    VectorR& r, VectorC& c, MatrixB& b, MatrixX& x,
    typename remove_imaginary<
