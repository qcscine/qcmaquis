...
inline std::ptrdiff_t pbsv(
  ...
  LAPACK_SPBSV(
      &lapack_option<UpLo>::value, &n, &kd, &nrhs, ab, &ldab, b, &ldb, &info
  );
  return info;
}
...
inline std::ptrdiff_t pbsv(
    ...
    const fortran_int_t ldab, std::complex<double>* b, const fortran_int_t ldb
) {
  fortran_int_t info(0);
  LAPACK_ZPBSV(
      &lapack_option<UpLo>::value, &n, &kd, &nrhs, ab, &ldab, b, &ldb, &info
...
inline std::ptrdiff_t pbsv(MatrixAB& ab, MatrixB& b) {
  return pbsv_impl<typename bindings::value_type<MatrixAB>::type>::invoke(
      ab, b
  );
}
