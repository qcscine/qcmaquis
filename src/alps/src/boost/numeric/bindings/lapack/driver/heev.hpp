...
inline std::ptrdiff_t heev(
  ...
  LAPACK_SSYEV(
      &jobz, &lapack_option<UpLo>::value, &n, a, &lda, w, work, &lwork, &info
  );
  return info;
}
...
struct heev_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
      const char jobz, MatrixA& a, VectorW& w, detail::workspace1<WORK> work
  ) {
...
struct heev_impl<Value, typename boost::enable_if<is_complex<Value> >::type> {
  ...
  // * Asserts that most arguments make sense.
  //
  template <typename MatrixA, typename VectorW, typename WORK, typename RWORK>
  static std::ptrdiff_t invoke(
...
heev(const char jobz, MatrixA& a, VectorW& w, Workspace work) {
  return heev_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      jobz, a, w, work
  );
}
