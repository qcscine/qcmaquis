...
// * float value-type.
//
inline std::ptrdiff_t gtsv(
    const fortran_int_t n, const fortran_int_t nrhs, float* dl, float* d,
    float* du, float* b, const fortran_int_t ldb
) {
...
inline std::ptrdiff_t gtsv(
    ...
    double* du, double* b, const fortran_int_t ldb
) {
  fortran_int_t info(0);
...
//
inline std::ptrdiff_t gtsv(
    const fortran_int_t n, const fortran_int_t nrhs, std::complex<float>* dl,
    std::complex<float>* d, std::complex<float>* du, std::complex<float>* b,
...
//
inline std::ptrdiff_t gtsv(
    const fortran_int_t n, const fortran_int_t nrhs, std::complex<double>* dl,
    std::complex<double>* d, std::complex<double>* du, std::complex<double>* b,
    const fortran_int_t ldb
) {
  fortran_int_t info(0);
...
struct gtsv_impl {
  ...
  //
  template <
      typename VectorDL, typename VectorD, typename VectorDU, typename MatrixB>
  static std::ptrdiff_t invoke(
      const fortran_int_t n, VectorDL& dl, VectorD& d, VectorDU& du, MatrixB& b
  ) {
    ...
    BOOST_ASSERT(n >= 0);
    return detail::gtsv(
        n, bindings::size_column(b), bindings::begin_value(dl),
...
inline std::ptrdiff_t gtsv(
  ...
  return gtsv_impl<typename bindings::value_type<VectorDL>::type>::invoke(
      n, dl, d, du, b
  );
}
