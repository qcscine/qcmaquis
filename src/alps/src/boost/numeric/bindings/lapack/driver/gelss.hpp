...
inline std::ptrdiff_t gelss(
    ...
    const fortran_int_t ldb, double* s, const double rcond, fortran_int_t& rank,
...
struct gelss_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  //
  template <typename MatrixA, typename MatrixB, typename VectorS, typename WORK>
  static std::ptrdiff_t invoke(
      MatrixA& a, MatrixB& b, VectorS& s, const real_type rcond,
      fortran_int_t& rank, detail::workspace1<WORK> work
  ) {
  ...
  static std::ptrdiff_t min_size_work(
      const std::ptrdiff_t m, const std::ptrdiff_t n, const std::ptrdiff_t nrhs
  ) {
    std::ptrdiff_t minmn = std::min<std::ptrdiff_t>(m, n);
    return std::max<std::ptrdiff_t>(
        1, 3 * minmn + std::max<std::ptrdiff_t>(
                           std::max<std::ptrdiff_t>(
                               2 * minmn, std::max<std::ptrdiff_t>(m, n)
                           ),
                           nrhs
                       )
    );
...
struct gelss_impl<Value, typename boost::enable_if<is_complex<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
      MatrixA& a, MatrixB& b, VectorS& s, const real_type rcond,
      fortran_int_t& rank, detail::workspace2<WORK, RWORK> work
  ) {
    namespace bindings = ::boost::numeric::bindings;
    BOOST_STATIC_ASSERT((bindings::is_column_major<MatrixA>::value));
    BOOST_STATIC_ASSERT((bindings::is_column_major<MatrixB>::value));
  ...
  static std::ptrdiff_t min_size_work(
    ...
    return std::max<std::ptrdiff_t>(
        1, 2 * minmn +
               std::max<std::ptrdiff_t>(std::max<std::ptrdiff_t>(m, n), nrhs)
    );
  }
...
gelss(
  ...
  return gelss_impl<typename bindings::value_type<MatrixA>::type>::invoke(
