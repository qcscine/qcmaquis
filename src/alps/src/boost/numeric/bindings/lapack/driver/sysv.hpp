...
inline std::ptrdiff_t sysv(
    const UpLo, const fortran_int_t n, const fortran_int_t nrhs, float* a,
    const fortran_int_t lda, fortran_int_t* ipiv, float* b,
    const fortran_int_t ldb, float* work, const fortran_int_t lwork
) {
  fortran_int_t info(0);
  LAPACK_SSYSV(
      &lapack_option<UpLo>::value, &n, &nrhs, a, &lda, ipiv, b, &ldb, work,
      &lwork, &info
  );
  return info;
}
...
template <typename UpLo>
inline std::ptrdiff_t sysv(
    const UpLo, const fortran_int_t n, const fortran_int_t nrhs, double* a,
...
inline std::ptrdiff_t sysv(
    ...
    std::complex<float>* b, const fortran_int_t ldb, std::complex<float>* work,
...
struct sysv_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  //
  template <
      typename MatrixA, typename VectorIPIV, typename MatrixB, typename WORK>
  static std::ptrdiff_t invoke(
      MatrixA& a, VectorIPIV& ipiv, MatrixB& b, detail::workspace1<WORK> work
  ) {
    ...
    BOOST_ASSERT(bindings::size(work.select(real_type())) >= min_size_work());
  ...
  static std::ptrdiff_t invoke(
      MatrixA& a, VectorIPIV& ipiv, MatrixB& b, minimal_workspace
  ) {
    namespace bindings = ::boost::numeric::bindings;
    typedef typename result_of::uplo_tag<MatrixA>::type uplo;
  ...
  template <typename MatrixA, typename VectorIPIV, typename MatrixB>
  static std::ptrdiff_t invoke(
      MatrixA& a, VectorIPIV& ipiv, MatrixB& b, optimal_workspace
  ) {
    namespace bindings = ::boost::numeric::bindings;
    typedef typename result_of::uplo_tag<MatrixA>::type uplo;
  ...
  //
  static std::ptrdiff_t min_size_work() { return 1; }
};
...
struct sysv_impl<Value, typename boost::enable_if<is_complex<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
    ...
    BOOST_STATIC_ASSERT((bindings::is_mutable<VectorIPIV>::value));
    BOOST_STATIC_ASSERT((bindings::is_mutable<MatrixB>::value));
    BOOST_ASSERT(bindings::size(work.select(value_type())) >= min_size_work());
    BOOST_ASSERT(bindings::size_column(a) >= 0);
    BOOST_ASSERT(bindings::size_column(b) >= 0);
    BOOST_ASSERT(
        bindings::size_minor(a) == 1 || bindings::stride_minor(a) == 1
...
//
template <
    typename MatrixA, typename VectorIPIV, typename MatrixB, typename Workspace>
inline typename boost::enable_if<
    detail::is_workspace<Workspace>, std::ptrdiff_t>::type
sysv(MatrixA& a, VectorIPIV& ipiv, MatrixB& b, Workspace work) {
  return sysv_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      a, ipiv, b, work
  );
}
