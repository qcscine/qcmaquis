...
inline std::ptrdiff_t gees(
  ...
  LAPACK_SGEES(
      &jobvs, &sort, select, &n, a, &lda, &sdim, wr, wi, vs, &ldvs, work,
      &lwork, bwork, &info
  );
  return info;
}
...
inline std::ptrdiff_t gees(
    ...
    const fortran_int_t ldvs, std::complex<float>* work,
    const fortran_int_t lwork, float* rwork, fortran_bool_t* bwork
) {
  fortran_int_t info(0);
  LAPACK_CGEES(
      &jobvs, &sort, select, &n, a, &lda, &sdim, w, vs, &ldvs, work, &lwork,
      rwork, bwork, &info
  );
  return info;
}
...
inline std::ptrdiff_t gees(
    ...
    fortran_int_t& sdim, std::complex<double>* w, std::complex<double>* vs,
    const fortran_int_t ldvs, std::complex<double>* work,
...
struct gees_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  //
  template <
      typename MatrixA, typename VectorWR, typename VectorWI, typename MatrixVS,
      typename WORK, typename BWORK>
  static std::ptrdiff_t invoke(
      const char jobvs, const char sort, external_fp select, MatrixA& a,
    ...
    BOOST_STATIC_ASSERT(
        (boost::is_same<
            typename remove_const<
                typename bindings::value_type<MatrixA>::type>::type,
            typename remove_const<
                typename bindings::value_type<MatrixVS>::type>::type>::value)
    );
    ...
    BOOST_ASSERT(
        bindings::size_minor(vs) == 1 || bindings::stride_minor(vs) == 1
    );
    BOOST_ASSERT(
    ...
    return detail::gees(
        jobvs, sort, select, bindings::size_column(a), bindings::begin_value(a),
  ...
  static std::ptrdiff_t invoke(
    ...
    return invoke(
        jobvs, sort, select, a, sdim, wr, wi, vs, workspace(tmp_work, tmp_bwork)
    );
  }
...
struct gees_impl<Value, typename boost::enable_if<is_complex<Value> >::type> {
  ...
  //
  template <
      typename MatrixA, typename VectorW, typename MatrixVS, typename WORK,
      typename RWORK, typename BWORK>
  static std::ptrdiff_t invoke(
      const char jobvs, const char sort, external_fp select, MatrixA& a,
  ...
  static std::ptrdiff_t invoke(
      const char jobvs, const char sort, external_fp select, MatrixA& a,
      fortran_int_t& sdim, VectorW& w, MatrixVS& vs, minimal_workspace
  ) {
    namespace bindings = ::boost::numeric::bindings;
  ...
  static std::ptrdiff_t invoke(
      const char jobvs, const char sort, external_fp select, MatrixA& a,
      fortran_int_t& sdim, VectorW& w, MatrixVS& vs, optimal_workspace
  ) {
    namespace bindings = ::boost::numeric::bindings;
    ...
    detail::gees(
        jobvs, sort, select, bindings::size_column(a), bindings::begin_value(a),
        bindings::stride_major(a), sdim, bindings::begin_value(w),
        bindings::begin_value(vs), bindings::stride_major(vs), &opt_size_work,
...
gees(
  ...
  return gees_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      jobvs, sort, select, a, sdim, wr, wi, vs, work
  );
}
...
inline typename boost::disable_if<
    detail::is_workspace<MatrixVS>, std::ptrdiff_t>::type
gees(
    const char jobvs, const char sort, external_fp select, MatrixA& a,
    fortran_int_t& sdim, VectorWR& wr, VectorWI& wi, MatrixVS& vs
) {
  return gees_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      jobvs, sort, select, a, sdim, wr, wi, vs, optimal_workspace()
  );
}
//
// Overloaded function for gees. Its overload differs for
...
//
template <
    typename MatrixA, typename VectorW, typename MatrixVS, typename Workspace>
inline typename boost::enable_if<
    mpl::and_<
        is_complex<typename bindings::value_type<MatrixA>::type>,
...
gees(
  ...
  return gees_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      jobvs, sort, select, a, sdim, w, vs, work
  );
}
...
inline typename boost::disable_if<
    mpl::or_<
        is_real<typename bindings::value_type<MatrixA>::type>,
...
gees(
  ...
  return gees_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      jobvs, sort, select, a, sdim, w, vs, optimal_workspace()
  );
}
//
// Overloaded function for gees. Its overload differs for
...
gees(
  ...
  std::ptrdiff_t info =
      gees_impl<typename bindings::value_type<MatrixA>::type>::invoke(
          jobvs, sort, select, a, sdim, bindings::detail::real_part_view(w),
          bindings::detail::imag_part_view(w), vs, work
      );
...
gees(
  ...
  std::ptrdiff_t info =
      gees_impl<typename bindings::value_type<MatrixA>::type>::invoke(
          jobvs, sort, select, a, sdim, bindings::detail::real_part_view(w),
          bindings::detail::imag_part_view(w), vs, optimal_workspace()
      );
  bindings::detail::interlace(w);
  return info;
}
