...
inline std::ptrdiff_t gtsvx(
    ...
    const float* b, const fortran_int_t ldb, float* x, const fortran_int_t ldx,
    float& rcond, float* ferr, float* berr, float* work, fortran_int_t* iwork
) {
  fortran_int_t info(0);
  LAPACK_SGTSVX(
      &fact, &lapack_option<Trans>::value, &n, &nrhs, dl, d, du, dlf, df, duf,
...
inline std::ptrdiff_t gtsvx(
    ...
    std::complex<float>* dlf, std::complex<float>* df, std::complex<float>* duf,
    std::complex<float>* du2, fortran_int_t* ipiv, const std::complex<float>* b,
  ...
  LAPACK_CGTSVX(
      &fact, &lapack_option<Trans>::value, &n, &nrhs, dl, d, du, dlf, df, duf,
      du2, ipiv, b, &ldb, x, &ldx, &rcond, ferr, berr, work, rwork, &info
  );
  return info;
}
...
inline std::ptrdiff_t gtsvx(
    ...
    std::complex<double>* duf, std::complex<double>* du2, fortran_int_t* ipiv,
...
struct gtsvx_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  //
  template <
      typename VectorDL, typename VectorD, typename VectorDU,
  ...
  static std::ptrdiff_t invoke(
      const char fact, const VectorDL& dl, const VectorD& d, const VectorDU& du,
    ...
    BOOST_ASSERT(bindings::size(d) >= bindings::size_column_op(d, trans()));
    BOOST_ASSERT(
        bindings::size(dl) >= bindings::size_column_op(d, trans()) - 1
    );
    BOOST_ASSERT(
        bindings::size(du) >= bindings::size_column_op(d, trans()) - 1
    );
    BOOST_ASSERT(
    ...
    BOOST_ASSERT(
        ...
        std::max<std::ptrdiff_t>(1, bindings::size_column_op(d, trans()))
    );
    BOOST_ASSERT(
    ...
    return detail::gtsvx(
        fact, trans(), bindings::size_column_op(d, trans()),
  ...
  static std::ptrdiff_t invoke(
    ...
    return invoke(
        fact, dl, d, du, dlf, df, duf, du2, ipiv, b, x, rcond, ferr, berr,
...
//
template <
    typename VectorDL, typename VectorD, typename VectorDU, typename VectorDLF,
    typename VectorDF, typename VectorDUF, typename VectorDU2,
    typename VectorIPIV, typename MatrixB, typename MatrixX,
...
gtsvx(
  ...
  return gtsvx_impl<typename bindings::value_type<VectorDL>::type>::invoke(
      fact, dl, d, du, dlf, df, duf, du2, ipiv, b, x, rcond, ferr, berr, work
  );
}
