...
inline std::ptrdiff_t heevd(
    const char jobz, const UpLo, const fortran_int_t n, float* a,
    const fortran_int_t lda, float* w, float* work, const fortran_int_t lwork,
    fortran_int_t* iwork, const fortran_int_t liwork
) {
  fortran_int_t info(0);
  LAPACK_SSYEVD(
      &jobz, &lapack_option<UpLo>::value, &n, a, &lda, w, work, &lwork, iwork,
      &liwork, &info
  );
  return info;
}
...
template <typename UpLo>
inline std::ptrdiff_t heevd(
    const char jobz, const UpLo, const fortran_int_t n, double* a,
    const fortran_int_t lda, double* w, double* work, const fortran_int_t lwork,
    fortran_int_t* iwork, const fortran_int_t liwork
) {
  fortran_int_t info(0);
  LAPACK_DSYEVD(
...
template <typename UpLo>
inline std::ptrdiff_t heevd(
    const char jobz, const UpLo, const fortran_int_t n, std::complex<float>* a,
  ...
  fortran_int_t info(0);
  LAPACK_CHEEVD(
      &jobz, &lapack_option<UpLo>::value, &n, a, &lda, w, work, &lwork, rwork,
...
template <typename UpLo>
inline std::ptrdiff_t heevd(
    const char jobz, const UpLo, const fortran_int_t n, std::complex<double>* a,
...
struct heevd_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  //
  template <typename MatrixA, typename VectorW, typename WORK, typename IWORK>
  static std::ptrdiff_t invoke(
      const char jobz, MatrixA& a, VectorW& w,
    ...
    return detail::heevd(
        jobz, uplo(), bindings::size_column(a), bindings::begin_value(a),
  ...
  static std::ptrdiff_t invoke(
      const char jobz, MatrixA& a, VectorW& w, minimal_workspace
  ) {
    namespace bindings = ::boost::numeric::bindings;
    typedef typename result_of::uplo_tag<MatrixA>::type uplo;
  ...
  template <typename MatrixA, typename VectorW>
  static std::ptrdiff_t invoke(
      const char jobz, MatrixA& a, VectorW& w, optimal_workspace
  ) {
    namespace bindings = ::boost::numeric::bindings;
    typedef typename result_of::uplo_tag<MatrixA>::type uplo;
    real_type opt_size_work = 0;
    fortran_int_t opt_size_iwork = 0;
    std::ptrdiff_t iTmp = detail::heevd(
        jobz, uplo(), bindings::size_column(a), bindings::begin_value(a),
        bindings::stride_major(a), bindings::begin_value(w), &opt_size_work, -1,
        &opt_size_iwork, -1
    );
    // Workaround for MKL which reports too small work arrays on query sometimes
    // (as implemented by Leon Freitag in the SVD)
...
struct heevd_impl<Value, typename boost::enable_if<is_complex<Value> >::type> {
  ...
  //
  template <
      typename MatrixA, typename VectorW, typename WORK, typename RWORK,
      typename IWORK>
  static std::ptrdiff_t invoke(
      const char jobz, MatrixA& a, VectorW& w,
  ...
  static std::ptrdiff_t invoke(
    ...
    return invoke(jobz, a, w, workspace(tmp_work, tmp_rwork, tmp_iwork));
  }
...
heevd(const char jobz, MatrixA& a, VectorW& w, Workspace work) {
  return heevd_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      jobz, a, w, work
  );
}
