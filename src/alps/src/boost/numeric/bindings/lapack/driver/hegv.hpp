...
struct hegv_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  //
  template <typename MatrixA, typename MatrixB, typename VectorW, typename WORK>
  static std::ptrdiff_t invoke(
      const fortran_int_t itype, const char jobz, MatrixA& a, MatrixB& b,
      VectorW& w, detail::workspace1<WORK> work
  ) {
...
struct hegv_impl<Value, typename boost::enable_if<is_complex<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
      const fortran_int_t itype, const char jobz, MatrixA& a, MatrixB& b,
      VectorW& w, detail::workspace2<WORK, RWORK> work
  ) {
    namespace bindings = ::boost::numeric::bindings;
    typedef typename result_of::uplo_tag<MatrixA>::type uplo;
    BOOST_STATIC_ASSERT((bindings::is_column_major<MatrixA>::value));
...
hegv(
  ...
  return hegv_impl<typename bindings::value_type<MatrixA>::type>::invoke(
