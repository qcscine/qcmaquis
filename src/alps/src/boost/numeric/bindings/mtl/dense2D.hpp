...
struct adaptor<mtl::dense2D<T, Parameters>, Id, Enable> {
  ...
  typedef typename convert_to<
      tag::data_order, typename Parameters::orientation>::type data_order;
  typedef
      typename mtl_matrix_size_type<typename Parameters::dimensions, 1>::type
          size_type1;
  typedef
      typename mtl_matrix_size_type<typename Parameters::dimensions, 2>::type
          size_type2;
  ...
  static std::ptrdiff_t size2(const Id& id) { return id.num_cols(); }

  static value_type* begin_value(Id& id) { return id.elements(); }

  static value_type* end_value(Id& id) {
    return id.elements() + id.used_memory();
  }

  static std::ptrdiff_t stride1(const Id& id) { return id.num_cols(); }
