...
struct adaptor<mtl::compressed2D<T, Parameters>, Id, Enable> {
  ...
  typedef typename copy_const<
      ...
      std::ptrdiff_t  // (Seems to be an actual usable type for bindings
                      // purposes)
      >::type index_type;
  ...
  static value_type* begin_value(Id& id) { return id.address_data(); }

  static value_type* end_value(Id& id) { return id.address_data() + id.nnz(); }
