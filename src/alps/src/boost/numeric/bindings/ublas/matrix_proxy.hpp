...
struct adaptor<ublas::matrix_range<T>, Id, Enable> {
  ...
  static typename result_of::end_value<adapted_type>::type end_value(Id& id) {
    return bindings::begin_value(id.data()) +
           offset(
               id.data(), id.start1() + id.size1(), id.start2() + id.size2()
           );
  }
...
struct adaptor<ublas::matrix_slice<T>, Id, Enable> {
  ...
  static std::ptrdiff_t stride1(const Id& id) { return id.stride1(); }

  static std::ptrdiff_t stride2(const Id& id) { return id.stride2(); }
...
struct adaptor<ublas::matrix_column<T>, Id, Enable> {
  typedef typename copy_const<Id, typename bindings::value_type<T>::type>::type
      value_type;
  typedef mpl::map<
  ...
  static value_type* begin_value(Id& id) {
    return bindings::begin_value(id.data()) + offset(id.data(), 0, id.index());
  }
...
struct adaptor<ublas::matrix_row<T>, Id, Enable> {
  ...
  static value_type* begin_value(Id& id) {
    return bindings::begin_value(id.data()) + offset(id.data(), id.index(), 0);
  }
...
struct adaptor<ublas::matrix_vector_range<T>, Id, Enable> {
  ...
  typedef typename property_insert<
      adapted_type, mpl::pair<tag::entity, tag::vector>,
      mpl::pair<tag::stride_type<1>, std::ptrdiff_t> >::type property_map;
