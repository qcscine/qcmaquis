...
class vectorspace {
 public:
  ...
  inline size_type vec_dimension() const { return n_; }
...
class vector_wrapper : public boost::shared_ptr<V> {
 ...
 public:
  ...
  template <class T>
  const vector_wrapper& operator*=(T x) {
    *super_type::get() *= x;
    return *this;
  }
  template <class T>
  ...
  const vector_wrapper& operator+=(const scaled_vector_wrapper<V, S>& x) {
    *super_type::get() += x.scalar() * x.vector();
    return *this;
  }
  template <class S>
  const vector_wrapper& operator-=(const scaled_vector_wrapper<V, S>& x) {
...
template <class VS>
void project(
    typename ietl::vectorspace_traits<VS>::vector_type& v, const VS& vs
) {
  vs.project(v);
}
...
class scaled_vector_wrapper {
 public:
  scaled_vector_wrapper(const ietl::vector_wrapper<V>& v, S s) : v_(v), s_(s) {}
...
template <class V>
typename number_traits<typename V::value_type>::magnitude_type two_norm(
    const ietl::vector_wrapper<V>& src
) {
  return ietl::two_norm(*src.get());
}

template <class V>
typename V::value_type dot(
    const ietl::vector_wrapper<V>& src1, const ietl::vector_wrapper<V>& src2
) {
...
void mult(
    A a, const ietl::vector_wrapper<V>& src, ietl::vector_wrapper<V>& dst
) {
...
template <class V, class S>
ietl::scaled_vector_wrapper<V, S> operator*(
    const ietl::vector_wrapper<V>& v, S s
) {
...
template <class V, class S>
ietl::scaled_vector_wrapper<V, S> operator*(
    S s, const ietl::vector_wrapper<V>& v
) {
  return ietl::scaled_vector_wrapper<V, S>(v, s);
}
