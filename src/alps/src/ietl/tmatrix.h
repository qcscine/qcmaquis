...
class Tmatrix {
 public:
  ...
  Tmatrix()
      : error_tol(pow(std::numeric_limits<magnitude_type>::epsilon(), 2. / 3.)
        ) {}
  ...
  const std::vector<magnitude_type>& eigenvalues(bool discard_ghosts = true)
      const {
    if (!computed) compute();
    if (discard_ghosts)
 ...
 private:
  ...
  mutable std::vector<magnitude_type>
      eigval_distinct;  // distinct eigen values.
  mutable std::vector<magnitude_type>
      eigval_distinct_noghost;  // distinct eigen values.
...
void Tmatrix<VS>::compute(bool do_err) const {
  ...
  if (info > 0) throw std::runtime_error("LAPACK error, stev function failed.");

  // tolerance values:
  multol = std::max(alpha_max, beta_max) * 2 *
           std::numeric_limits<magnitude_type>::epsilon() * (10000 + n);
  thold = std::max(eval[0], eval[n - 1]);
  thold = std::max(error_tol * thold, 5 * multol);

  // error estimates of eigen values starts:
  // the unique eigen values selection, their multiplicities and corresponding
  // errors calculation follows:
    ...
    if ((eval[i] - temp) > thold) {
      ...
      if (do_err) {
        if (multiple > 1)
          ...
          err.push_back(fabs(*beta.rbegin() * (*z2)(n - 1, i - 1))
          );  // *beta.rbegin() = betaMplusOne.
      }
      multiple = 1;
    } else
      multiple++;
  ...
  if (do_err) {
    if (multiple > 1)
      ...
      err.push_back(fabs(*beta.rbegin() * (*z2)(n - 1, n - 1))
      );  // *beta.rbegin() = betaMplusOne.
  }
  ...
  for (k = eigval_distinct.begin(); k != eigval_distinct.end(); k++, i++) {
    if (multiplicty[i] == 1) {  // test of spuriousness for the eigenvalues
                                // whose multiplicity is one.
      for (int j = t2; j < n - 2;
           j++, t2++) {  // since size of reduced matrix is n-1
        if ((eval_g[j] - *k) >= multol) break;

        if (fabs(*k - eval_g[j]) < multol) {
          multiplicty[i] = 0;
          if (do_err)
            err[i] = 0;  // if eigen value is a ghost => error calculation not
                         // required, 0=> ignore error.
          t2++;
          break;
        }
      }
  ...
  for (k = eigval_distinct.begin(); k != eigval_distinct.end(); k++, i++) {
    if (multiplicty[i] != 0) {
      ...
      if (do_err) err_noghost.push_back(err[i]);
    }
  }
}  // end of compute.
}  // namespace ietl
