...
class bandlanczos {
 public:
  ...
  typedef
      typename ietl::number_traits<scalar_type>::magnitude_type magnitude_type;
 ...
 private:
  bool convergence_test(
      const int dim, int pc, int evs, magnitude_type dep_tol,
      magnitude_type ghost_tol, bool ghost_discarding, bool low
  );
  int calc_approx_eigenvalues(
      const int& dim, int evs, magnitude_type ghost_tol, bool ghost_discarding,
      bool low
  );  // store evs eigenvalues, if evs==-1, store all of them
  void getev_(
      const fortran_int_t dim, double w[], fortran_int_t info, double type
  );
  void getev_(
      const fortran_int_t dim, float w[], fortran_int_t info, float type
  );
  void getev_(
      const fortran_int_t dim, double w[], fortran_int_t info,
      std::complex<double> type
  );
  void getev_(
      const fortran_int_t dim, float w[], fortran_int_t info,
...
void indexer::next() {
  ...
  do {
    lastloc_++;
    if (lastloc_ == location_.size()) lastloc_ = 0;
...
void indexer::deflate(int old) {
  pc_--;
  int place =
      std::find(location_.begin(), location_.end(), old) - location_.begin();
...
bandlanczos<MATRIX, VS>::bandlanczos(
    const MATRIX& matrix, const VS& vec, const int& p
)
    : matrix_(matrix),
      p_(p),
      vecspace_(vec),
...
template <class MATRIX, class VS>
const std::vector<typename bandlanczos<MATRIX, VS>::magnitude_type>&
bandlanczos<MATRIX, VS>::eigenvalues() {
  return eigenvals;
}

template <class MATRIX, class VS>
const std::vector<typename bandlanczos<MATRIX, VS>::vector_type>&
bandlanczos<MATRIX, VS>::eigenvectors() {
  return reigenvecs;
}

template <class MATRIX, class VS>
template <class GEN, class ITER>
void bandlanczos<MATRIX, VS>::calculate_eigenvectors(
    ITER& iter, const GEN& gen
) {
  magnitude_type rnorm;
  scalar_type cnorm;
  int j, k, k0, i;
  reigenvecs = std::vector<vector_type>(eigenvecs.size());
  for (i = 0; i < reigenvecs.size(); i++) reigenvecs[i] = new_vector(vecspace_);

  // set v_k starting vectors for k=1,...,p
  indexer index(p_);
  int pc = p_;
  v = std::vector<vector_type>(2 * p_ + 1);
  for (i = 0; i < (2 * p_ + 1); i++) v[i] = new_vector(vecspace_);
  for (int x = 0; x < p_; x++) ietl::generate(v[x], gen);
  ...
  for (j = 0; (j <= iter.iterations() && pc > 0); j++) {
    ...
    // v_j now won't be modified anymore. It's a Krylov basis vector, thus can
    // be used to transform the eigenvectors in the Krylov basis back to the
    // original basis
...
template <class MATRIX, class VS>
template <class GEN, class ITER>
void bandlanczos<MATRIX, VS>::calculate_eigenvalues(
    ITER& iter, const GEN& gen
) {
  magnitude_type rnorm;
  scalar_type cnorm;
  ...
  for (j = 0; ((j <= iter.max_iter()) && (pc > 0)); j++) {
    ...
    std::sort(I.begin(), I.end());
    for (i = 0; i < I.size() + 1; i++) {
      // set t_{k,j} = v_k^\star v_{j+p_c}  and  v_{j+p_c} = v_{j+p_c} - v_k
      // t_{k,j}
      if (i == I.size())
        k = j;
      else
    ...
    if (convergence_test(
            j + 1, pc, iter.evs(), iter.dep_tol(), iter.ghost_tol(),
            iter.ghost_discarding(), iter.low()
        ) == true)
      break;
    index.next();
  }  // END OF for (j=0;....)
  calc_approx_eigenvalues(
      iter.iterations() + 1, iter.evs(), iter.ghost_tol(),
      iter.ghost_discarding(), iter.low()
  );
  // can be used for debugging, this returns ALL computed eigenvalues, even when
  // not converged... calc_approx_eigenvalues(iter.iterations()+1, -1,
  // iter.ghost_tol(), iter.ghost_discarding(), iter.low());
}

template <class MATRIX, class VS>
bool bandlanczos<MATRIX, VS>::convergence_test(
    const int dim, int pc, int evs, magnitude_type dep_tol,
    magnitude_type ghost_tol, bool ghost_discarding, bool low
) {
  if (dim < evs)  // cannot have converged, because dim of tridiagonal matrix is
                  // smaller than #wanted ev
    return false;
  int num_evs =
      calc_approx_eigenvalues(dim, -1, ghost_tol, ghost_discarding, low);
  if (num_evs <
      evs)  // cannot have converged, we have less eigenvalues than wanted
    return false;
  ...
  for (int i = 1; i <= evs; i++) {
    vnorm = 0;
    for (int j = 1; j <= pc; j++)
      vnorm += std::abs(eigenvecs[evs - i][dim - j]
      );  // * std::abs(eigenvecs[evs-i][dim-j]);
...
template <class MATRIX, class VS>
int bandlanczos<MATRIX, VS>::calc_approx_eigenvalues(
    const int& dim, int evs, magnitude_type ghost_tol, bool ghost_discarding,
    bool low
) {
  ...
  Tjpr.resize(dim, dim);
  for (int i = 0; i < dim; i++)
    for (int j = 0; j <= i; j++) Tjpr(j, i) = tmat(j, i);
  magnitude_type* w = new magnitude_type[dim];
  int info = 0;
  scalar_type type = 0;
  getev_(dim, w, info, type);
  int* mult = new int[dim];
  int* evpl = new int[dim];  // Stores the index of w, where the eigenvals are
                             // (second ev is at place 7 -> evpl[7]=2)
  // bool ghost[dim];
  bool* ghost = new bool[dim];
  int num_evs;
  int discarded = 0;  // number of ghosts, is substracted from #eigenvalues of T
                      // to get #ev of the original matrix
  evpl[0] = 0;
  mult[0] = 1;
  ghost[0] = false;
  num_evs = 1;
  ...
  if (((num_evs > evs) && (ghost_discarding)) && (dim > 1)) {
    ...
    for (int i = 1; i < dim; i++)
      for (int j = 1; j <= i; j++) Tjpr(j - 1, i - 1) = tmat(j, i);
      ...
      if (mult[i] == 1) {
        for (int j = 0; j < dim - 1; j++) {
          if (std::abs((w2[j] - w[evpl[i]])) < ghost_tol) {
            ...
            if (low)
              eigenvecs.erase(
                  eigenvecs.begin() + i - discarded,
                  eigenvecs.begin() + i + 1 - discarded
              );
            else
              eigenvecs.erase(
                  eigenvecs.begin() + num_evs - i - 1,
                  eigenvecs.begin() + num_evs - i
              );  // CHANGE !!!
            discarded++;
          }
        }
  ...
  if (evs == -1)
    ...
    discarded =
        0;  // now used to check if we already stored the wanted eigenvalues
...
template <class MATRIX, class VS>
void bandlanczos<MATRIX, VS>::getev_(
    const fortran_int_t dim, double w[], fortran_int_t info, double type
) {
  char jobz = 'V';
  char uplo = 'U';
