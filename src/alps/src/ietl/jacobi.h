...
jacobi_davidson<MATRIX, VS>::calculate_eigenvalue(
  ...
  do {
    ...
    // u = V s
    u = V[0] * s[0];
    for (j = 1; j <= iter.iterations(); j++) u += V[j] * s[j];

    // u^A = V^A s
    // ietl::mult(matrix_,u,uA);
    uA = VA[0] * s[0];
    for (j = 1; j <= iter.iterations(); ++j) uA += VA[j] * s[j];

    ietl::project(uA, vecspace_);

    // r = u^A - \theta u
    r = uA - theta * u;
    //            std::cout << "Iteration " << iter.iterations() << ", resid = "
    //            << ietl::two_norm(r) << std::endl;

    // if (|r|_2 < \epsilon) stop
    ++iter;
    if (iter.finished(ietl::two_norm(r), theta)) break;
