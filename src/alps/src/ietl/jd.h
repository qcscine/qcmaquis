...
class jd_iteration : public basic_iteration<T> {
 public:
  jd_iteration(
      size_t max_iter, size_t m_min = 10, size_t m_max = 20,
      T reltol = sqrt(std::numeric_limits<double>::epsilon()),
      T abstol = sqrt(std::numeric_limits<double>::epsilon())
  )
      : basic_iteration<T>(max_iter, reltol, abstol),
        m_min_(m_min),
        m_max_(m_max) {}

  inline size_t m_min() const { return m_min_; }
  inline size_t m_max() const { return m_max_; }
  const std::string& error_msg() { return basic_iteration<T>::err_msg; }
...
class deflated_matrix {
 public:
  ...
  deflated_matrix(
      const MATRIX& A, double theta, const std::vector<vector_type>& Q
  )
      : A_(A), theta_(theta), Q_(Q) {}
  ~deflated_matrix() {}

  void set_theta(real_type theta) { theta_ = theta; }

  friend void mult<>(
      const deflated_matrix& A_def, const vector_type& v, vector_type& r
  );
...
template <class MATRIX, class VS, class VECTOR>
void mult(
    const deflated_matrix<MATRIX, VS>& Adef, const VECTOR& v,
    VECTOR& r
...
void mult(
    std::vector<VECTOR>& vecset, const MATRIX& mat
...
void mult(std::vector<ublas::vector<T> >& vecset, const MATRIX& mat) {
  ...
  for (std::size_t i = 0; i < vecset[0].size(); ++i) {
    ...
    for (std::size_t j = 0; j < mat.size2(); ++j) vecset[j](i) = tmp(j);
  }
...
class left_prec_solver {
 public:
  ...
  left_prec_solver(
      SOLV& s, const MATRIX& A, const std::vector<vector_type>& Q, const VS& vs,
      PREC& K
  )
  ...
  friend void mult<>(
      const left_prec_solver& A_def, const vector_type& v, vector_type& z
  );
...
template <class SOLV, class MATRIX, class VS, class PREC, class VECTOR>
void mult(
    const left_prec_solver<SOLV, MATRIX, VS, PREC>& LPS, const VECTOR& v,
    VECTOR& z
) {
  VECTOR temp;
...
template <class SOLV, class MATRIX, class VS, class PREC>
template <class IT>
void left_prec_solver<SOLV, MATRIX, VS, PREC>::operator()(
    real_type theta, vector_type& r, vector_type& t, IT& iter
) {
  theta_ = theta;
  ...
  for (size_t i = m_old; i < m; ++i) ietl::mult(K_, Q_[i], Q_hat_[i]);
  ...
  for (size_t i = 0; i < m; ++i) gamma_(i) = ietl::dot(Q_[i], t);
  ...
  for (size_t i = 0; i < m; ++i) r += gamma_(i) * Q_hat_[i];

  t = solv_(*this, r, x0_, abs_tol);

}  // left_prec_solver::void()

// non-preconditioned simple
// solver//////////////////////////////////////////////////////
...
class jd_solver {
 public:
  ...
  jd_solver(
      SOLV& s, const MATRIX& A, const std::vector<vector_type>& Q,
      const VS& vspace
  )
      : Adef_(A, 0, Q),
        vspace_(vspace),
...
template <class SOLV, class MATRIX, class VS>
template <class IT>
void jd_solver<SOLV, MATRIX, VS>::operator()(
    real_type theta, vector_type& r, vector_type& t, IT& iter
) {
  ...
  // starting with x0_ = 0
  t = solv_(
      Adef_, r, x0_, iter.absolute_tolerance() * iter.absolute_tolerance()
  );
}
}  // namespace solver
...
// without preconditioning
  ...
  void eigensystem(
      IT& iter, GEN& gen, size_type k_max, SOLV f, bool search_highest = false
  )
#else
  template <class SOLV = ietl_gmres, class IT, class GEN>
  void eigensystem(
      IT& iter, GEN& gen, size_type k_max, SOLV f = ietl_gmres(),
  ...
  void eigensystem(
      IT& iter, GEN& gen, size_type k_max, PREC& K, SOLV f,
      bool search_highest = false
  ) {
  ...
  template <class SOLV, class IT, class GEN>
  void eigensystem_harmonic(
      IT& iter, GEN& gen, size_type k_max, real_type tau, SOLV& f
  ) {
    typedef solver::jd_solver<SOLV, MATRIX, VS> solver_type;
    solver_type solver(f, A_, X_, vspace_);
  ...
  real_set_type eigenvalues() { return Lambda_; }
 ...
 protected:
  ...
  void jdqr(
      SOLVER& solver, IT& iter, GEN& gen, size_type k_max, bool search_highest
  );

  template <class SOLVER, class IT, class GEN>
  void jdqr_harmonic(
      SOLVER& solver, IT& iter, GEN& gen, size_type k_max, real_type tau
  );
};  // end of class jd
...
void jd<MATRIX, VS>::jdqr(
  ...
  for (size_t i = 0; i < X_.size() - 1; ++i) t -= ietl::dot(t, X_[i]) * X_[i];

  while (true) {
    // modified Gram-Schmidt
    norm_t = ietl::two_norm(t);

    for (size_type i = 0; i < m; ++i) t -= ietl::dot(V[i], t) * V[i];
    ...
    if (norm_t == 0 /*< iter.absolute_tolerance()*/) {
      ...
      if (iter.first())
        ...
        throw std::runtime_error(
            "try to solve the correction equation more accurately."
        );
    }
    ...
    for (size_type i = 0; i < m; ++i) M(i, m - 1) = ietl::dot(V[i], VA[m - 1]);
    ...
    info = lapack::heev(
        'V', boost::numeric::bindings::upper(S), Theta
    );  // eigenvectors in columns of S
    ...
    // u = V s_1
    X_.back() = V[0] * S(0, 0);
    for (size_type j = 1; j < m; ++j) X_.back() += V[j] * S(j, 0);

    // uA = VA s_1
    uA = VA[0] * S(0, 0);
    for (size_type j = 1; j < m; ++j) uA += VA[j] * S(j, 0);
    ...
    while (iter.finished(norm_r, Theta[0])) {
      if (iter.error_code() == 1) throw std::runtime_error(iter.error_msg());

      if (verbose_ > 0)
        std::cout << "Accepted eigenpair #" << k + 1 << "\t" << Theta[0]
                  << "\tResidual: " << norm_r << "\n";
      ...
      if (m < 1)
        throw std::runtime_error(
            "search space is depleted, try a different generator."
        );

      --m;
      M = ublas::zero_matrix<scalar_type>(m, m);

      ublas::matrix_range<matrix_type> Sproxy(
          S, ublas::range(0, S.size1()), ublas::range(1, S.size2())
      );
    ...
    // restart
    if (m >= m_max) {
      if (verbose_ > 1) std::cout << "restart...\n";
      M = ublas::zero_matrix<scalar_type>(m_min, m_min);

      ublas::matrix_range<matrix_type> Sproxy(
          S, ublas::range(0, S.size1()), ublas::range(1, m_min)
      );
    ...
    do {
      norm_t = ietl::two_norm(t);
      for (size_t i = 0; i < X_.size(); ++i) t -= ietl::dot(t, X_[i]) * X_[i];
    ...
    if (verbose_ > 1)
      std::cout << "JD iteration " << iter.iterations()
                << "\t resid = " << norm_r << "\n";
  }  // main loop

}  // jdqr for exterior evs

// jdqr for interior eigenvalues with harmonic ritz
// values////////////////////////////////////////
template <class MATRIX, class VS>
template <class SOLVER, class IT, class GEN>
void jd<MATRIX, VS>::jdqr_harmonic(
    SOLVER& solver, IT& iter, GEN& gen, size_type k_max, real_type tau
) {
  typedef std::vector<vector_type> vector_set_type;
  typedef std::vector<real_type> real_set_type;
  ...
  while (true)  // k<k_max
  {
    ...
    if (norm_w == 0) throw std::runtime_error("New search vector is zero.");

    W.back() /= norm_w;
    V.push_back(t / norm_w);

    M.resize(m, m);
    for (size_t i = 0; i < m; ++i) M(i, m - 1) = ietl::dot(W[i], V.back());

    // eigendecomposition M = S THETA S*
    Theta.resize(m);
    S = M;  // copy M

    int info = lapack::heev(
        'V', boost::numeric::bindings::upper(S), Theta
    );  // eigenvectors in columns of S
    if (info < 0) throw std::runtime_error("lapack::heev - illegal value.");
    if (info > 0)
    ...
    for (size_t i = 1; i < m; ++i) u_tilde += V[i] * S(i, m - 1);
    ...
    for (size_t i = 1; i < m; ++i) w_tilde += W[i] * S(i, m - 1);
    ...
    while (iter.finished(norm_r, Theta[0])) {
      ...
      if (verbose_ > 0)
        std::cout << "Accepted eigenpair #" << k + 1 << "\t" << theta + tau
                  << "\t Residual: " << norm_r << "\n";
      ...
      V.pop_back();
      for (size_t i = 0; i < m; ++i) std::swap(V[i], tempvs[i]);
      for (size_t i = 0; i < m; ++i) {
      ...
      W.pop_back();
      for (size_t i = 0; i < m; ++i) std::swap(W[i], tempvs[i]);
      Theta.pop_back();
      S.resize(m, m);
      for (size_t i = 0; i < m; ++i) {
        M(i, i) = Theta[m - 1 - i];
        ublas::column(S, m - 1 - i) =
            ublas::unit_vector<scalar_type>(S.size1(), i);
      }
    ...
    if (m >= iter.m_max()) {
      ...
      for (size_t i = 1; i < iter.m_min(); ++i) {
        ...
        for (size_t j = 1; j < m; ++j) tempvs[i] += V[j] * S(j, m - 1 - i);
      }
      V.resize(iter.m_min());

      for (size_t i = 1; i < iter.m_min(); ++i) std::swap(V[i], tempvs[i]);

      for (size_t i = 1; i < iter.m_min(); ++i) {
        tempvs[i] = W[0] * S(0, m - 1 - i);
        for (size_t j = 1; j < m; ++j) tempvs[i] += W[j] * S(j, m - 1 - i);
      }
      W.resize(iter.m_min());
      for (size_t i = 1; i < iter.m_min(); ++i) std::swap(W[i], tempvs[i]);

      Theta.resize(iter.m_min());
      for (size_t i = 0; i < iter.m_min(); ++i) M(i, i) = Theta[m - 1 - i];
