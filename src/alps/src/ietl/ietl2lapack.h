...
inline void stev(
    const char& jobz, fortran_int_t n, double dd[], double de[], double dz[],
    fortran_int_t ldz, fortran_int_t info
) {
  double* dwork = new double[2 * n - 2];
  LAPACK_DSTEV(&jobz, &n, dd, de, dz, &ldz, dwork, &info);
  delete[] dwork;
  if (info) throw std::runtime_error("Error return from dstev");
}

inline void stev(
    const char& jobz, fortran_int_t n, double dd[], double de[],
    std::complex<double> dz[], fortran_int_t ldz, fortran_int_t info
) {
  double* dwork = new double[2 * n - 2];
  LAPACK_ZSTEQR(&jobz, &n, dd, de, dz, &ldz, dwork, &info);
  delete[] dwork;
  if (info) throw std::runtime_error("Error return from zsteqr");
}
...
fortran_int_t stev(
    const Vector& alpha, const Vector& beta, Vector& eval, fortran_int_t n
) {
  if (n == 0) n = alpha.size();
  ...
  ietl_lapack_dispatch::stev(
      _jobz, n, ietl::get_data(eval), ietl::get_data(beta_tmp),
      ietl::get_data(z), _ldz, _info
  );
  return _info;
}
...
int stev(
    const Vector& alpha, const Vector& beta, Vector& eval, FortranMatrix& z,
    fortran_int_t n
) {
  if (n == 0) n = alpha.size();
  std::copy(alpha.begin(), alpha.begin() + n, eval.begin());
  ...
  ietl_lapack_dispatch::stev(
      _jobz, n, ietl::get_data(eval), ietl::get_data(beta_tmp), z.data(), _ldz,
      _info
  );
  return _info;
}
