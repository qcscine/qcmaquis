...
/* $Id: evaluatable.h 3365 2009-11-20 14:11:32Z gamperl $ */
...
class Evaluatable {
 public:
  ...
  Evaluatable() {}
  virtual ~Evaluatable() {}
  virtual value_type value(const Evaluator<T>& = Evaluator<T>(), bool = false)
      const = 0;
  virtual bool can_evaluate(const Evaluator<T>& = Evaluator<T>(), bool = false)
      const = 0;
  ...
  virtual Evaluatable* partial_evaluate_replace(
...
template <class T>
inline Term<T> Evaluatable<T>::term() const {
  return Term<T>();
}
...
inline std::ostream& operator<<(
    std::ostream& os, const alps::expression::Evaluatable<T>& e
) {
  e.output(os);
  return os;
}
