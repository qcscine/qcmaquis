...
/* $Id: function.h 1646 2005-05-13 17:46:35Z troyer $ */
...
class Function : public Evaluatable<T> {
 public:
  ...
  Function(const std::string& n, const Expression<T>& e)
      : name_(n), args_(1, e) {}
  Function(const std::string& n, const std::vector<Expression<T> >& e)
      : name_(n), args_(e) {}
  ...
  Evaluatable<T>* partial_evaluate_replace(
      const Evaluator<T>& = Evaluator<T>(), bool = false
  );
  ...
  if (c != ')') {
    ...
    if (c != ')')
      boost::throw_exception(std::runtime_error(
          std::string("received ") + c +
          " instead of ) at end of function argument list"
      ));
  }
}
...
bool Function<T>::depends_on(const std::string& s) const {
  ...
  for (typename std::vector<Expression<T> >::const_iterator it = args_.begin();
       it != args_.end(); ++it)
...
boost::shared_ptr<Evaluatable<T> > Function<T>::flatten_one() {
  for (typename std::vector<Expression<T> >::iterator it = args_.begin();
       it != args_.end(); ++it)
    it->flatten();
...
template <class T>
Evaluatable<T>* Function<T>::partial_evaluate_replace(
...
template <class T>
typename Function<T>::value_type Function<T>::value(
    const Evaluator<T>& p, bool isarg
) const {
