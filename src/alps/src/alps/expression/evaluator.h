...
/* $Id: evaluator.h 3120 2009-02-25 15:57:16Z wistaria $ */
...
class ALPS_DECL Disorder {
 ...
 public:
  ...
  static boost::variate_generator<random_type&, boost::normal_distribution<> >
      gaussian_random;
...
class Evaluator {
 public:
  ...
  virtual bool can_evaluate_function(
      const std::string&, const Expression<T>&, bool = false
  ) const;
  virtual bool can_evaluate_function(
      const std::string&, const std::vector<Expression<T> >&, bool = false
  ) const;
  virtual value_type evaluate(const std::string&, bool = false) const;
  virtual value_type evaluate_function(
      const std::string&, const Expression<T>&, bool = false
  ) const;
  virtual value_type evaluate_function(
      const std::string&, const std::vector<Expression<T> >&, bool = false
  ) const;
  virtual Expression<T> partial_evaluate(const std::string& name, bool = false)
      const;
  virtual Expression<T> partial_evaluate_function(
      const std::string& name, const Expression<T>&, bool = false
  ) const;
  virtual Expression<T> partial_evaluate_function(
      const std::string& name, const std::vector<Expression<T> >&, bool = false
  ) const;
  virtual Direction direction() const;

  bool can_evaluate_expressions(
      const std::vector<Expression<T> >&, bool = false
  ) const;
  void partial_evaluate_expressions(std::vector<Expression<T> >&, bool = false)
      const;
...
class ParameterEvaluator : public Evaluator<T> {
 public:
  ...
  ParameterEvaluator(const Parameters& p, bool rand = true)
      : Evaluator<T>(rand), parms_(p) {
    Disorder::seed_if_unseeded(p);
  }
...
template <class T>
bool Evaluator<T>::can_evaluate_function(
    const std::string& name, const Expression<T>& arg, bool
) const {
  return arg.can_evaluate(*this, true) &&
         (name == "sqrt" || name == "abs" || name == "sin" || name == "cos" ||
          name == "tan" || name == "asin" || name == "acos" || name == "atan" ||
          name == "log" || name == "exp" ||
          (evaluate_random_ && name == "integer_random"));
}

template <class T>
bool Evaluator<T>::can_evaluate_expressions(
    const std::vector<Expression<T> >& arg, bool f
) const {
  bool can = true;
  for (typename std::vector<Expression<T> >::const_iterator it = arg.begin();
       it != arg.end(); ++it)
...
template <class T>
void Evaluator<T>::partial_evaluate_expressions(
    std::vector<Expression<T> >& arg, bool f
) const {
  for (typename std::vector<Expression<T> >::iterator it = arg.begin();
       it != arg.end(); ++it) {
...
template <class T>
bool Evaluator<T>::can_evaluate_function(
    const std::string& name, const std::vector<Expression<T> >& arg, bool f
) const {
  bool can =
      can_evaluate_expressions(arg, true) &&
      ((arg.size() == 0 && evaluate_random_ &&
        (name == "random" || name == "gaussian_random" ||
         name == "normal_random")) ||
       (arg.size() == 1 && can_evaluate_function(name, arg[0], f)) ||
       (arg.size() == 2 &&
        (evaluate_random_ && (name == "gaussian_random" || name == "atan2"))));
  return can;
}
...
template <class T>
typename Evaluator<T>::value_type Evaluator<T>::evaluate(
    const std::string& name, bool isarg
) const {
  return partial_evaluate(name, isarg).value();
}

template <class T>
typename Evaluator<T>::value_type Evaluator<T>::evaluate_function(
    const std::string& name, const Expression<T>& arg, bool isarg
) const {
...
template <class T>
typename Evaluator<T>::value_type Evaluator<T>::evaluate_function(
    const std::string& name, const std::vector<Expression<T> >& arg, bool isarg
) const {
  return partial_evaluate_function(name, arg, isarg).value();
}

template <class T>
Expression<T> Evaluator<T>::partial_evaluate(const std::string& name, bool)
    const {
  return Expression<T>(name);
}

template <class T>
Expression<T> Evaluator<T>::partial_evaluate_function(
    const std::string& name, const Expression<T>& arg, bool
) const {
...
template <class T>
Expression<T> Evaluator<T>::partial_evaluate_function(
    const std::string& name, const std::vector<Expression<T> >& args, bool isarg
) const {
  if (args.size() == 1) return partial_evaluate_function(name, args[0], isarg);
