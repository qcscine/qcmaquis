...
/* $Id: evaluator.C 7042 2013-09-06 12:28:36Z troyer $ */
...
ALPS_DECL boost::variate_generator<
    Disorder::random_type&, boost::normal_distribution<> >
...
void Disorder::seed_if_unseeded(const alps::Parameters& p) {
  ...
  if (s && s != last_seed_) seed(s);
}

StringValue simplify_value(
    StringValue const& val, Parameters const& parms, bool eval_random
) {
  try {
...
bool same_values(StringValue const& x, StringValue const& y, double eps) {
  try {
    ...
    if (exprx.can_evaluate(eval) && expry.can_evaluate(eval)) {
      return std::abs(exprx.value() - expry.value()) <=
             eps * std::max(std::abs(exprx.value()), std::abs(expry.value()));
    } else {
      return x == y;
    }
  } catch (...) {
