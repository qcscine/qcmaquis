...
/* $Id: expression.h 6901 2013-05-28 12:16:36Z tamama $ */
...
class Expression : public Evaluatable<T> {
 public:
  ...
  template <class U>
  Expression(
      U val, typename boost::enable_if<boost::is_arithmetic<U> >::type* = 0
  )
      : terms_(1, Term<T>(value_type(val))) {}
#else
  ...
  const Expression& operator-=(Expression const& e) { return operator+=(-e); }
...
bool Expression<T>::depends_on(const std::string& s) const {
  for (term_iterator it = terms().first; it != terms().second; ++it)
    if (it->depends_on(s)) return true;
  return false;
}
...
Term<T> Expression<T>::term() const {
  if (!is_single_term())
    boost::throw_exception(
        std::logic_error("Called term() for multi-term expression")
    );
  return terms_[0];
}

template <class T>
Expression<T> Expression<T>::expression_dependent_on(const std::string& str
) const {
  Expression<T> e;
  for (typename std::vector<Term<T> >::const_iterator it = terms_.begin();
       it != terms_.end(); ++it)
    if (it->depends_on(str)) e += (*it);
  return e;
}

template <class T>
Expression<T> Expression<T>::expression_dependent_only_on(const std::string& str
) const {
  Expression<T> e;
  for (typename std::vector<Term<T> >::const_iterator it = terms_.begin();
       it != terms_.end(); ++it)
    if (it->depends_only_on(str)) e += (*it);
  return e;
}
...
void Expression<T>::parse(const std::string& str) {
  ...
  if (!parse(in))
    boost::throw_exception(
        std::runtime_error("Did not parse to end of string '" + str + "'")
    );
}
...
void Expression<T>::sort() {
  ...
  if (added && numeric::is_zero(prev_term.first)) terms_.erase(prev);
}

template <class T>
typename Expression<T>::value_type Expression<T>::value(
    const Evaluator<T>& p, bool isarg
) const {
  if (terms_.size() == 0) return value_type(0.);
...
template <class T>
inline alps::expression::Expression<T> operator+(
    const alps::expression::Expression<T>& ex1,
    const alps::expression::Expression<T>& ex2
) {
...
inline std::istream& operator>>(
    std::istream& is, alps::expression::Expression<T>& e
) {
  std::string s;
  is >> s;
  e.parse(s);
...
inline bool operator==(
  ...
  return (
      boost::lexical_cast<std::string>(ex1) ==
...
template <class T>
inline bool operator==(
    const alps::expression::Expression<T>& ex, const std::string& s
) {
...
template <class T>
inline bool operator==(
    const std::string& s, const alps::expression::Expression<T>& ex
) {
  return ex == s;
}
