...
/* $Id: factor.h 5850 2011-11-30 01:39:54Z wistaria $ */
...
class SimpleFactor : public Evaluatable<T> {
 public:
  ...
  SimpleFactor(const SimpleFactor& v) : Evaluatable<T>(v), term_() {
...
class Factor : public SimpleFactor<T> {
 public:
  ...
  Factor(const std::string& s)
      : super_type(s), is_inverse_(false), power_(1.) {}
  Factor(const Evaluatable<T>& v)
      : super_type(v), is_inverse_(false), power_(1.) {}
  ...
  void partial_evaluate(const Evaluator<T>& = Evaluator<T>(), bool = false);
  Term<T> term() const {
    return unit_power() ? super_type::term()
                        : (super_type::term_ ? Term<T>(*this) : Term<T>());
  }
  bool depends_on(const std::string& s) const {
    return super_type::depends_on(s) || power_.depends_on(s);
  }
  bool unit_power() const {
    return power_.can_evaluate() && power_.value() == 1.;
  }
  bool is_single_term() const {
    return super_type::is_single_term() && unit_power();
  }
  ...
  if (std::isdigit(c) || c == '.' || c == '+' || c == '-') {
    ...
    if (!in)
      boost::throw_exception(
          std::runtime_error("Failed to parse number in factor")
      );
...
Factor<T>::Factor(std::istream& in, bool inv)
    : super_type(in), is_inverse_(inv), power_(1.) {
  char c;
  in >> c;
  if (in) {
    if (c == '^') {
...
bool Factor<T>::can_evaluate(const Evaluator<T>& p, bool isarg) const {
  return super_type::can_evaluate(p, unit_power() ? isarg : true) &&
         power_.can_evaluate(p, true);
}

template <class T>
typename SimpleFactor<T>::value_type SimpleFactor<T>::value(
...
template <class T>
typename Factor<T>::value_type Factor<T>::value(
    const Evaluator<T>& p, bool isarg
) const {
  value_type val = super_type::value(p, unit_power() ? isarg : true);
  if (is_inverse()) val = 1. / val;
  if (!unit_power())
    val = std::pow(
        evaluate_helper<T>::real(val),
        evaluate_helper<T>::real(power_.value(p, true))
    );
  return val;
}
...
inline bool operator==(
    const alps::expression::Factor<T>& ex1,
    const alps::expression::Factor<T>& ex2
) {
  return (
...
template <class T>
inline bool operator==(
    const alps::expression::Factor<T>& ex, const std::string& s
) {
...
template <class T>
inline bool operator==(
    const std::string& s, const alps::expression::Factor<T>& ex
) {
  return ex == s;
}
