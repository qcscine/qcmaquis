...
// include ( + (add) , - (subtract, negation) , * (multiply) , / (divide) )
// operators for vectors from boost accumulator library (developed by Eric
// Niebier)
using namespace boost::numeric::operators;

// include ( + (positivity) )
template <class T>
inline std::vector<T>& operator+(std::vector<T>& vec) {
  return vec;
}
...
#define IMPLEMENT_ALPS_VECTOR_SCALAR_OPERATION(OPERATOR_NAME, OPERATOR) \
  template <class T>                                                    \
  inline std::vector<T> OPERATOR_NAME(                                  \
      std::vector<T> vector, T const& scalar                            \
  ) {                                                                   \
    std::vector<T> res;                                                 \
  ...
  template <class T>                                                    \
  inline std::vector<T> OPERATOR_NAME(                                  \
      T const& scalar, std::vector<T> vector                            \
  ) {                                                                   \
    std::vector<T> res;                                                 \
    res.reserve(vector.size());                                         \
    for (typename std::vector<T>::iterator it = vector.begin();         \
...
#define IMPLEMENT_ALPS_VECTOR_FUNCTION(LIB_HEADER, FUNCTION_NAME) \
  namespace detail {                                              \
  template <typename T>                                           \
  struct FUNCTION_NAME##_VECTOR_OP_HELPER {                       \
    T operator()(T arg) {                                         \
  ...
  std::vector<T> FUNCTION_NAME(std::vector<T> vec) {              \
    std::transform(                                               \
        ...
        detail::FUNCTION_NAME##_VECTOR_OP_HELPER<T>()             \
    );                                                            \
    return vec;                                                   \
  }
...
#define IMPLEMENT_ALPS_VECTOR_FUNCTION2(LIB_HEADER, FUNCTION_NAME)             \
  ...
  static std::vector<T> FUNCTION_NAME(std::vector<T> vec, U index) {           \
    ...
    std::transform(                                                            \
        vec.begin(), vec.end(), vec.begin(),                                   \
        boost::lambda::bind<T>(                                                \
            static_cast<T (*)(T, U)>(&FUNCTION_NAME), boost::lambda::_1, index \
        )                                                                      \
    );                                                                         \
    return vec;                                                                \
  }
