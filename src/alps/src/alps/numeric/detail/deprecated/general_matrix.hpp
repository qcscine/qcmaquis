...
#ifdef USE_MATRIX_DISPATCH  // use dispatch to tiny matrix functions for small
                            // matrices
#undef __APPLE_CC__
...
#define MLOOP(unused, product)                                                \
  BOOST_PP_CAT(                                                               \
      BOOST_PP_CAT(                                                           \
          BOOST_PP_CAT(                                                       \
              BOOST_PP_CAT(                                                   \
                  BOOST_PP_CAT(void matmult_, BOOST_PP_SEQ_ELEM(0, product)), \
                  _                                                           \
              ),                                                              \
...
#define CASE_MACRO_R(ignored, nr, np_and_nq)                                  \
  BOOST_PP_CAT(                                                               \
      BOOST_PP_CAT(                                                           \
          BOOST_PP_CAT(                                                       \
              BOOST_PP_CAT(                                                   \
                  BOOST_PP_CAT(case BOOST_PP_SEQ_ELEM(nr, MSIZE_R)            \
                               : matmult_,                                    \
                                 BOOST_PP_SEQ_ELEM(                           \
                                     BOOST_PP_SEQ_ELEM(0, np_and_nq), MSIZE_P \
                                 )),                                          \
                  _                                                           \
              ),                                                              \
      ...
      BOOST_PP_SEQ_ELEM(nr, MSIZE_R)(                                         \
          &values_[0], &M2.values_[0], &Mres.values_[0]                       \
      );                                                                      \
      break;                                                                  \
  )
...
// std::ostream &operator<<(std::ostream &os, const general_matrix &M);
// //forward declaration
template <typename T>
 ...
 public:
  ...
  general_matrix(int size1, int size2)
      : size1_(size1),
        ...
        values_(size1 * size2, T(0)) {}
  general_matrix(int size1 = 0)
      : size1_(size1),
        size2_(size1),
        total_memory_size_(size1 * size1),
        values_(size1 * size1, T(0)) {}
  ...
  inline std::vector<T> &values() { return values_; }
  ...
  inline const fortran_int_t &size1() const { return size1_; }

  inline const fortran_int_t &size2() const { return size2_; }
  ...
  inline void matrix_right_multiply(
      const general_matrix<T> &M2, general_matrix<T> &Mres, T beta = 0
  ) const {
  ...
  bool operator==(const general_matrix &other) const {
    ...
    typename std::vector<T>::iterator it =
        find_if(diff.begin(), diff.end(), deviates<T>);
    return (it == diff.end());
  }
  bool operator!=(const general_matrix &other) const {
    return !(*this == other);
  ...
  void left_multiply_diagonal_matrix(const blas::vector &diagonal_matrix) {
    ...
    fortran_int_t inc = 1;
    for (int i = 0; i < size1_;
         ++i) {  // for each col: multiply col. with constant
  ...
  void right_multiply_diagonal_matrix(const blas::vector &diagonal_matrix) {
    assert(
        size2_ == diagonal_matrix.size()
    );  // must have as many rows as diag has size.
    for (int i = 0; i < size1_; ++i) {
      dscal_(&size2_, &diagonal_matrix(i), &values_[i], &size2_);
    }
  }

  inline void right_multiply(const vector &v1, vector &v2)
      const {  // perform v2[i]=M[ij]v1[j]
    assert(size1_ == size2_);
    ...
    fortran_int_t inc = 1;
    dgemv_(
        &trans, &size1_, &size1_, &alpha, &values_[0], &size1_, &(v1(0)), &inc,
        &beta, &(v2(0)), &inc
    );
  }
  ...
  general_matrix &invert() {
    throw(
        std::logic_error(std::string("you linked the general case for invert. "
                                     "Please use the specializations."))
    );
  }
  ...
  void save(alps::hdf5::archive &ar) const {
    ...
    ar << make_pvp(
        "", &values_.front(), std::vector<std::size_t>(2, size1_, size2_)
    );
  }
...
template <>
inline void general_matrix<double>::matrix_right_multiply(
    const general_matrix<double> &M2, general_matrix<double> &Mres, double beta
) const {
  // call the BLAS routine for matrix matrix multiplication:
  // std::cerr<<"sizes are: "<<size1_<<" "<<Mres.size1()<<"
  // "<<Mres.size2()<<std::endl;
  assert(Mres.size1() == size1_);
  assert(Mres.size2() == M2.size2());
  assert(size2_ == M2.size1());
  if (size1_ < 4 || size2_ < 4 ||
      M2.size2_ < 4) {  // do it by hand! a test showed this to be the optimal
                        // size on my intel.
    int s2 = Mres.size2();
    ...
    dgemm_(
        &notrans, &notrans, &M2.size2_, &size1_, &size2_, &one_double,
        &M2.values_[0], &M2.size2_, &values_[0], &size2_, &beta,
        &Mres.values_[0], &M2.size2_
    );
  }
}
#else
...
inline void general_matrix<double>::matrix_right_multiply(
  ...
  bool use_blas = false;
  switch (Mres.size1()
  ) {  // boost pp dispatcher for matrix multiplication - see also dispatcher.
...
template <>
inline void general_matrix<std::complex<double> >::matrix_right_multiply(
    const general_matrix<std::complex<double> > &M2,
    general_matrix<std::complex<double> > &Mres, std::complex<double> beta
) const {
  // call the BLAS routine for matrix matrix multiplication:
  assert(Mres.size1() == size1_);
  assert(Mres.size2() == M2.size2());
  ...
  char notrans = 'N';
  if (size1_ < 4 && size2_ < 4 &&
      M2.size2() < 4) {  // do it by hand! a test showed this to be the optimal
                         // size on my intel.
    general_matrix M2t(M2);
    M2t.transpose();
    int s2 = Mres.size2();
    for (int i = 0; i < size1_ * s2; ++i) {
      Mres.values_[i] *= beta;
    }
    for (int i = 0; i < size1_; ++i) {
      for (int k = 0; k < size2_; ++k) {
...
template <>
inline double general_matrix<std::complex<double> >::trace() const {
  assert(size1_ == size2_);
  std::complex<double> trace = 0;
  for (int i = 0; i < size1_; ++i) {
    trace += operator()(i, i);
  }
  if (trace.imag() > 1.e-10)
    std::cerr << "careful, trace is complex: " << trace << std::endl;
  return trace.real();
}

template <>
inline double general_matrix<std::complex<double> >::max() const {
  int max_index;
  fortran_int_t total_size = size1_ * size2_;
  fortran_int_t inc = 1;
  if (total_size == 0) return 0;
  if (total_size == 1)
    return std::abs(values_[0]);
  else
...
template <typename T>
inline std::ostream &operator<<(std::ostream &os, const general_matrix<T> &M) {
  os << "[ ";
...
inline std::ostream &operator<<(
    std::ostream &os, const general_matrix<std::complex<double> > &M
) {
  for (int i = 0; i < M.size1(); ++i) {
    for (int j = 0; j < M.size2(); ++j) {
...
template <>
inline std::ostream &operator<<(
    std::ostream &os, const general_matrix<double> &M
) {
  for (int i = 0; i < M.size1(); ++i) {
    for (int j = 0; j < M.size2(); ++j) {
      os << M(i, j) << " ";
...
general_matrix<std::complex<double> >::invert() {
  ...
  B.set_to_identity();
  FORTRAN_ID(zgesv)
  (&size1_, &size1_, &values_[0], &size1_, ipiv, &(B(0, 0)), &size1_, &info);
  delete[] ipiv;
  if (info) {
    throw(std::logic_error("in dgesv: info was not zero."));
  }
  swap(B, *this);
  return *this;
}
...
template <>
inline general_matrix<double> &general_matrix<double>::invert() {
...
template <typename T>
inline general_matrix<T> operator*(
    const T &lambda, const general_matrix<T> &M
) {
...
template <typename T>
inline general_matrix<T> operator*(
    const general_matrix<T> &M, const T &lambda
) {
  general_matrix<T> M2(M);
  M2 *= lambda;
  return M2;
...
template <typename T>
inline general_matrix<T> operator*(
    const blas::vector &v, const general_matrix<T> &M
) {
  general_matrix<T> M2(M);
