...
typedef boost::numeric::ublas::matrix<
    ...
    dense_matrix;
typedef boost::numeric::ublas::matrix<
    std::complex<double>, boost::numeric::ublas::column_major>
    complex_dense_matrix;
#endif

namespace blas {
// simple BLAS matrix that uses BLAS calls for rank one and matrix vector.
// std::ostream &operator<<(std::ostream &os, const matrix &M); //forward
// declaration
...
class matrix {
 public:
  ...
  inline const double &operator()(const unsigned int i, const unsigned int j)
      const {
    assert((i < size_) && (j < size_));
    return values_[i * size_ + j];
  }
  ...
  inline const int size1() const { return size_; }

  inline const int size2() const { return size_; }
  inline const std::vector<double> values() const { return values_; }
  void resize(int new_size) {
    if (new_size == size_)
      ...
      for (int i = size_ - 1; i > 0; --i) {
        std::copy(
            values_.rbegin() + new_size * new_size - (i + 1) * size_,
            values_.rbegin() + new_size * new_size - i * size_,
            values_.rbegin() + new_size * (new_size - i) - size_
        );
      }
    } else {
      for (int i = 1; i < size_; ++i) {
        std::copy(
            values_.begin() + i * size_, values_.begin() + i * size_ + new_size,
            values_.begin() + i * new_size
        );
      }
      values_.resize(new_size * new_size);
    }
  ...
  inline void add_outer_product(
      const blas::vector &v1, const blas::vector &v2, double alpha = 1.
  ) {
    fortran_int_t inc = 1;
    if (size_ > 1) {
      FORTRAN_ID(dger)
      (&size_, &size_, &alpha, &v2.values()[0], &inc, &v1.values()[0], &inc,
       &values_[0], &size_);
    } else if (size_ == 1) {
  ...
  inline void insert_row_column_last(
      blas::vector &row, blas::vector &col, double Mkk
  ) {
    fortran_int_t oldsize = size_;
    resize(size_ + 1);
    fortran_int_t one = 1;
    FORTRAN_ID(dcopy)
    (&oldsize, &(col.values()[0]), &one, &(values_[oldsize]),
     &size_);  // copy in row (careful: col. major)
    FORTRAN_ID(dcopy)
    (&oldsize, &(row.values()[0]), &one, &(values_[oldsize * size_]),
     &one);  // copy in column
  ...
  blas::vector getrow(int k) const {
    ...
    fortran_int_t one = 1;
    FORTRAN_ID(dcopy)
    (&size_, &(values_[k * size_]), &one, &row.values()[0], &one);
    return row;
  }
  ...
  inline void setrow(int k, const blas::vector row) {
    ...
    fortran_int_t one = 1;
    FORTRAN_ID(dcopy)
    (&size_, &row.values()[0], &one, &(values_[k * size_]), &one);
  }
  ...
  inline void insert_row_column(
      blas::vector &row, blas::vector &col, double Mkk, int k
  ) {
    assert((k <= size_));
    resize(size_ + 1);
  ...
  inline void remove_row_column_last() { size_--; }
  ...
  inline void swap_rows(int c1, int c2) {
    ...
    fortran_int_t one = 1;
    FORTRAN_ID(dswap)
    (&size_, &(values_[c1 * size_]), &one, &(values_[c2 * size_]), &one);
  }
  ...
  ) {  // convert dense or sparse matrices (or any other with  (i,j) and size1()
       // to blas matrix.
  ...
  inline void right_multiply(const vector &v1, vector &v2) const {
    ...
    char trans = 'T';
    double alpha = 1.,
           beta = 0.;  // no need to multiply a constant or add a vector
    fortran_int_t inc = 1;
    FORTRAN_ID(dgemv)
    (&trans, &size_, &size_, &alpha, &values_[0], &size_, &v1.values()[0], &inc,
     &beta, &v2.values()[0], &inc);
  }
  ...
  inline void matrix_right_multiply(const matrix &M2, matrix &Mres) const {
    ...
    FORTRAN_ID(dgemm)
    (&notrans, &notrans, &size_, &size_, &size_, &one_double, &M2.values_[0],
     &size_, &values_[0], &size_, &zero_double, &Mres.values_[0], &size_);
  }

  inline void matrix_right_multiply(
      const blas::general_matrix<double> &M2, blas::general_matrix<double> &Mres
  ) const {
    if (size_ == 0) return;
    // call the BLAS routine for matrix matrix multiplication:
    double one_double = 1.;
    ...
    FORTRAN_ID(dgemm)
    (&notrans, &notrans, &M2_size2, &size_, &size_, &one_double, &(M2(0, 0)),
     &(M2.size2()), &values_[0], &size_, &zero_double, &(Mres(0, 0)),
     &(Mres.size2()));
  }
  ...
  inline double determinant() const {
    ...
    // LU factorization
    FORTRAN_ID(dgesv)
    (&size_, &size_, &det_matrix.values_[0], &size_, ipiv, &identity.values_[0],
     &size_, &info);
    if (info < 0) {
      std::cout << "LAPACK ERROR IN APPLY!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
                << std::endl;
  ...
  bool operator==(const matrix &other) const {
    ...
    transform(
        values_.begin(), values_.end(), other.values_.begin(), diff.begin(),
        std::minus<double>()
    );
    std::vector<double>::iterator it =
        std::find_if(diff.begin(), diff.end(), deviates<double>);
    return (it == diff.end());
  }
  ...
  matrix &operator+=(const matrix &other) {
    ...
    transform(
        values_.begin(), values_.end(), temp.begin(), values_.begin(),
        std::plus<double>()
    );
    return *this;
  }
  ...
  void eigenvalues_eigenvectors_symmetric(
    ...
    FORTRAN_ID(dsyev)
    (&jobs, &uplo, &size_, &eigenvectors.values_[0], &size_,
     &(eigenvalues.values()[0]), &work_size, &lwork, &info);
    lwork = (int)work_size;
    double *work = new double[lwork];
    FORTRAN_ID(dsyev)
    (&jobs, &uplo, &size_, &eigenvectors.values_[0], &size_,
     &(eigenvalues.values()[0]), work, &lwork, &info);
    delete[] work;
  }
...
#ifdef UBLAS
inline complex_dense_matrix mult(
    const complex_dense_matrix &A, const blas::matrix &B
) {  // slow matrix multiplication real & complex.
  if ((int)A.size1() != (int)B.size() || (int)A.size2() != (int)B.size()) {
    std::cerr << "wrong matrix sizes in mult." << std::endl;
    exit(1);
  }
  int size = A.size1();
  ...
  char notrans = 'N';
  FORTRAN_ID(zgemm)
  (&notrans, &notrans, &size, &size, &size, &one_complex, (void *)&(A(0, 0)),
   &size, (void *)&(B_complex(0, 0)), &size, (void *)&zero_complex,
   (void *)&(D(0, 0)), &size);
  return D;
}
inline complex_dense_matrix mult(
    const blas::matrix &A, const complex_dense_matrix &B
) {  // slow matrix multiplication real & complex.
  if ((int)B.size1() != A.size() || (int)B.size2() != A.size()) {
    std::cerr << "wrong matrix sizes in mult." << std::endl;
    exit(1);
  }
  int size = B.size1();
  ...
  char notrans = 'N';
  FORTRAN_ID(zgemm)
  (&notrans, &notrans, &size, &size, &size, &one_complex,
   (void *)&(A_complex(0, 0)), &size, (void *)&(B(0, 0)), &size,
   (void *)&zero_complex, (void *)&(D(0, 0)), &size);
  return D;
}
inline complex_dense_matrix mult(
    const complex_dense_matrix &A, const complex_dense_matrix &B
) {  // slow matrix multiplication real & complex.
  if (B.size1() != A.size1() || B.size2() != A.size1() ||
      A.size1() != A.size2()) {
    std::cerr << "wrong matrix sizes in mult." << std::endl;
    exit(1);
  }
  ...
  FORTRAN_ID(zgemm)
  (&notrans, &notrans, &size, &size, &size, &one_complex, (void *)&(A(0, 0)),
   &size, (void *)&(B(0, 0)), &size, (void *)&zero_complex, (void *)&(D(0, 0)),
   &size);
  return D;
}
#endif  // UBLAS
