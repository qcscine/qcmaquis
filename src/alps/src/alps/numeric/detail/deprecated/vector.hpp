...
inline double expfunc(double entry) { return std::exp(entry); }
...
class vector {
 public:
  ...
  vector(const vector &rhs) : values_(rhs.values_), size_(rhs.size_) {}
  ...
  inline vector operator*=(double lambda) {
    fortran_int_t inc = 1;
    if (size()) FORTRAN_ID(dscal)(&size_, &lambda, &values_[0], &inc);
    return *this;
  }
  ...
  inline std::vector<double> &values() { return values_; }
