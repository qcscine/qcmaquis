...
namespace detail {
//        struct functor_##name { template<class T> return_type operator()
//        (arg_type t) { return (static_cast<return_type (*)
//        (arg_type)>(name))(t); } };

#define DEFINE_FUNCTION_OBJECT(name, return_type, arg_type) \
  struct functor_##name {                                   \
    template <class T>                                      \
    return_type operator()(arg_type t) {                    \
      using std::name;                                      \
      return name(t);                                       \
    }                                                       \
  };
...
class diagonal_matrix {
 public:
  ...
  void remove_cols(size_type k, size_type n = 1) { remove_rows(k, n); }
  ...
  void save(alps::hdf5::archive& ar) const { ar << alps::make_pvp("", data_); }

  void load(alps::hdf5::archive& ar) { ar >> alps::make_pvp("", data_); }
#endif
...
template <typename T>
void gemm(
    diagonal_matrix<T> const& m1, diagonal_matrix<T> const& m2,
    diagonal_matrix<T>& m3
) {
  assert(num_cols(m1) == num_rows(m2));
  resize(m3, num_rows(m1), num_cols(m2));
  for (std::size_t j = 0; j < num_cols(m2); ++j) m3(j, j) = m1(j, j) * m2(j, j);
}

template <typename T>
const diagonal_matrix<T> operator+(
    diagonal_matrix<T> a, diagonal_matrix<T> const& b
) {
  a += b;
  return a;
}
...
void conj_inplace(diagonal_matrix<T>& m) {
  using std::transform;
  transform(
      m.diagonal().first, m.diagonal().second, m.diagonal().first,
      detail::functor_conj()
  );
}
...
std::ostream& operator<<(std::ostream& os, diagonal_matrix<T> const& m) {
  std::copy(
      m.diagonal().first, m.diagonal().second, std::ostream_iterator<T>(os, " ")
  );
  return os;
}
...
class transpose_view<diagonal_matrix<T> > {
 public:
  ...
  typedef typename diagonal_matrix<T>::value_type
      value_type;  // The type T of the elements of the matrix
  typedef typename diagonal_matrix<T>::reference
      reference;  // Reference to value_type
  typedef typename diagonal_matrix<T>::const_reference
      const_reference;  // Const reference to value_type
  typedef typename diagonal_matrix<T>::size_type
      size_type;  // Unsigned integer type that represents the dimensions of the
                  // matrix
  typedef typename diagonal_matrix<T>::difference_type
