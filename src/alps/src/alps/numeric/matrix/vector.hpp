...
class vector : public MemoryBlock {
 public:
  ...
  vector(vector const& v) : MemoryBlock(v) {}

  template <class InputIterator>
  vector(InputIterator first, InputIterator last) : MemoryBlock(first, last) {}
  ...
  vector& operator+=(T2 const& rhs) {
    plus_assign(
        *this, rhs, typename get_entity<vector>::type(),
  ...
  vector& operator*=(T2 const& x) {
    multiplies_assign(
        *this, x, typename get_entity<vector>::type(),
        typename get_entity<T2>::type()
    );
    return *this;
  }
...
inline vector<T, MemoryBlock> exp(T c, vector<T, MemoryBlock> v) {
  ...
  v *= c;
  std::transform(
      v.begin(), v.end(), result.begin(), static_cast<T (*)(T)>(&std::exp)
  );
  return result;
}

template <typename MemoryBlock>
inline vector<double, MemoryBlock> exp(
    double c, vector<double, MemoryBlock> v
) {
...
inline std::ostream& operator<<(
    std::ostream& os, const vector<T, MemoryBlock>& v
) {
...
struct plus_minus_return_type<
    vector<T1, MemoryBlock1>, vector<T2, MemoryBlock2>, tag::vector,
    tag::vector> {
 private:
  typedef
