...
void conj_inplace(Matrix& m, tag::matrix) {
  ...
  for (typename Matrix::size_type j = 0; j < num_cols(m); ++j)
    for (std::pair<col_element_iterator, col_element_iterator> range =
             col(m, j);
...
/**
 * Returns a matrix or a vector of type T containing a the complex conjugates of
 * the original object. It does an element-wise conjugation.
 */
template <typename T>
