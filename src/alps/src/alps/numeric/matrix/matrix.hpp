...
class matrix {
 public:
  ...
  typedef std::size_t size_type;  ///< Unsigned integer type that represents the
                                  ///< dimensions of the matrix
  typedef std::ptrdiff_t
      difference_type;  ///< Signed integer type to represent the distance of
  ...
  typedef strided_iterator<matrix, value_type>
      row_element_iterator;  ///< Iterator to iterate through the elements of a
                             ///< row of the matrix
  typedef strided_iterator<const matrix, const value_type>
      const_row_element_iterator;  ///< Const version of row_element_iterator
  typedef value_type*
      col_element_iterator;  ///< Iterator to iterate through the elements of a
                             ///< columns of the matrix
  typedef value_type const*
      const_col_element_iterator;  ///< Const version of col_element_iterator
  typedef strided_iterator<matrix, value_type>
      diagonal_iterator;  ///< Iterator to iterate through the elements on the
                          ///< diagonal of the matrix
  typedef strided_iterator<const matrix, const value_type>
      const_diagonal_iterator;  ///< Const version of the diagonal_iterator
  typedef matrix_element_iterator<matrix, value_type>
      element_iterator;  ///< Iterator to iterate through all elements of the
                         ///< matrix (REALLY SLOW! USE row_-/column_iterators
                         ///< INSTEAD!)
  typedef matrix_element_iterator<const matrix, const value_type>
      const_element_iterator;  ///< Const version of element_iterator (REALLY
                               ///< SLOW! USE row_-/column_iterators INSTEAD!)
   ...
   * @param colums a vector containing the column ranges (ForwardIterator pairs
   * marking the begin and end of the data to be stored in a column)
   */
  template <typename ForwardIterator>
  explicit matrix(
      std::vector<std::pair<ForwardIterator, ForwardIterator> > const& columns
  );
  ...
  matrix<T, MemoryBlock>& operator+=(T2 const& rhs) {
    plus_assign(
        *this, rhs, typename get_entity<matrix>::type(),
  ...
  matrix<T, MemoryBlock>& operator*=(T2 const& t) {
    multiplies_assign(
        *this, t, typename get_entity<matrix>::type(),
        typename get_entity<T2>::type()
    );
    return *this;
  }
  ...
  matrix<T, MemoryBlock>& operator/=(T2 const& t) {
    ...
    multiplies_assign(
        *this, value_type(1) / t, typename get_entity<matrix>::type(),
        typename get_entity<T2>::type()
    );
    return *this;
  }
   ...
   * (increasing 1st index by 1). Since the matrix is stored in column major
   * order, it returns 1.
   */
  inline difference_type stride1() const;

  /**
   * @return The stride for moving to the next element along a row (increasing
   * 2nd index by 1)
   */
   ...
   *
   * Exception behaviour:
   * As long as the assignment and copy operation of the T values don't throw an
   * exception, any exception will leave the matrix unchanged. (Assuming the
   * same behaviour of the underlying MemoryBlock. This is true for
   * std::vector.)
   ...
   * @param rows For how many rows should memory be reserved, value is ignored
   * if it's smaller than the current number of rows
   * @param cols For how many columns should memory be reserved, value is
   * ignored if it's smaller than the current number of columns
   * @param init_value i
   */
   ...
   * Any resize to a size smaller than this value won't move any of the matrix
   * elements in memory.
   * @return A pair p of size_type, where p.first == reserved rows, p.second ==
   * reserved_columns
   */
   ...
   * @return true iff (capacity().first == num_rows() && capacity().second ==
   * num_cols()), false otherwise
   */
   ...
   * Since the matrix is column-major these iterators are the best choice for a
   * fast traversal though the matrix.
   * @param col Index of the column to be iterated through (starting from
   * col=0).
   * @return a pair of random access iterators marking the begin and end of the
   * column.
   */
  std::pair<col_element_iterator, col_element_iterator> col(size_type col = 0) {
    assert(col < size2_);
    return std::make_pair(
        col_element_iterator(&values_[0] + (col * reserved_size1_)),
        col_element_iterator(&values_[0] + (col * reserved_size1_ + size1_))
    );
  }
  std::pair<const_col_element_iterator, const_col_element_iterator> col(
      size_type col = 0
  ) const {
    assert(col < size2_);
    return std::make_pair(
        const_col_element_iterator(&values_[0] + col * reserved_size1_),
        const_col_element_iterator(&values_[0] + col * reserved_size1_ + size1_)
    );
  }
  ...
  std::pair<row_element_iterator, row_element_iterator> row(size_type row = 0) {
    assert(row < size1_);
    return std::make_pair(
        row_element_iterator(&values_[0] + row, reserved_size1_),
        row_element_iterator(
            &values_[0] + (row + reserved_size1_ * size2_), reserved_size1_
        )
    );
  }

  std::pair<const_row_element_iterator, const_row_element_iterator> row(
      size_type row = 0
  ) const {
    assert(row < size1_);
    return std::make_pair(
        const_row_element_iterator(&values_[0] + row, reserved_size1_),
   ...
   *
   * In case of a non-squared matrix, it will return the diagonal of the upper
   * left square part.
   * @return a pair of random access iterators marking the begin and end of the
   * diagonal.
   */
  std::pair<diagonal_iterator, diagonal_iterator> diagonal() {
    ...
    return std::make_pair(
        diagonal_iterator(&values_[0], reserved_size1_ + 1),
        diagonal_iterator(
            &values_[0] + (square_part * reserved_size1_ + square_part),
            reserved_size1_ + 1
        )
    );
  }
  ...
  std::pair<const_diagonal_iterator, const_diagonal_iterator> diagonal() const {
    size_type const square_part = (std::min)(size1_, size2_);
    return std::make_pair(
        const_diagonal_iterator(&values_[0], reserved_size1_ + 1),
   ...
   * You should consider using col_element_iterators to iterate through a column
   * and loop over each column of the matrix.
   * @return a pair of random access iterators marking the begin and the end of
   * the matrix. The iterators iterate column first and advance to the next row
   * if the end of a column is reached.
   */
  std::pair<element_iterator, element_iterator> elements() {
    return std::make_pair(
        element_iterator(this, 0, 0), element_iterator(this, 0, num_cols())
    );
  }
  std::pair<const_element_iterator, const_element_iterator> elements() const {
    return std::make_pair(
        const_element_iterator(this, 0, 0),
        const_element_iterator(this, 0, num_cols())
    );
  }

  /**
   * Append k columns using the data given by the iterator pair range, where
   * distance(range.first,range.second) == k*num_rows(m).
   * @param range a pair of InputIterators containing the data for the new
   * columns
   * @param k the number of columns to append
   */
  template <typename InputIterator>
  void append_cols(
      std::pair<InputIterator, InputIterator> const& range,
      difference_type k = 1
  );

  /**
   * Append k rows using the data given by the iterator pair range, where
   * distance(range.first,range.second) == k*num_cols(m).
  ...
  /**
   * Inserts new cols before column `j` using the data given by range moving all
   * columns further to the right (j -> j+k).
   * @param j index of the column before which the new rows will be inserted
   * (i.e. the first new column will have index `j`)
   * @param range a InputIterator pair containing the data for the new rows,
   * where distance(range.first,range.second == k*num_cols(m).
   * @param k the number of rows to insert.
   */
  template <typename InputIterator>
  void insert_cols(
      size_type j, std::pair<InputIterator, InputIterator> const& range,
      difference_type k = 1
  );

  /**
   * Inserts new rows before row `i` using the data given by range moving all
   * rows further down (i -> i+k).
  ...
  template <typename InputIterator>
  void insert_rows(
      size_type i, std::pair<InputIterator, InputIterator> const& range,
      difference_type k = 1
  );
...
template <typename T, typename MemoryBlock>
alps::oxstream& operator<<(
    alps::oxstream& xml, matrix<T, MemoryBlock> const& m
);
...
template <
    typename T1, typename MemoryBlock1, typename T2, typename MemoryBlock2>
struct multiply_return_type<
    matrix<T1, MemoryBlock1>, matrix<T2, MemoryBlock2>, tag::matrix,
    tag::matrix> {
 private:
  typedef typename detail::auto_deduce_multiply_return_type<T1, T2>::type
      value_type;
  typedef typename boost::mpl::if_<
      typename detail::auto_deduce_multiply_return_type<T1, T2>::select_first,
      MemoryBlock1, MemoryBlock2>::type memory_block_type;
...
struct plus_minus_return_type<
 ...
 private:
  typedef
      typename detail::auto_deduce_plus_return_type<T1, T2>::type value_type;
  typedef typename boost::mpl::if_<
      typename detail::auto_deduce_plus_return_type<T1, T2>::select_first,
      MemoryBlock1, MemoryBlock2>::type memory_block_type;
...
struct get_sign;
template <>
struct get_sign<plus_assign_operator> {
  static int const value = 1;
};
template <>
struct get_sign<minus_assign_operator> {
  static int const value = -1;
};
...
template <
    typename T1, typename MemoryBlock1, typename T2, typename MemoryBlock2,
    typename Operation>
void plus_minus_assign_impl(
    matrix<T1, MemoryBlock1>& lhs, matrix<T2, MemoryBlock2> const& rhs,
    Operation op, tag::matrix, tag::matrix
) {
  // One could do also a dispatch on row vs. column major, but since we don't
  // have row major right now, let's leave it like that.
  typedef typename matrix<T1, MemoryBlock1>::size_type size_type;
  typedef typename matrix<T1, MemoryBlock1>::col_element_iterator
  ...
  for (size_type j = 0; j < num_cols(lhs); ++j) {
    for (size_type i = 0; i < num_rows(lhs); ++i) op(lhs(i, j), rhs(i, j));
  }
#else  // defined(__clang_major__) && __clang_major__ < 3 || (__clang_major__ ==
       // 3 && __clang_minor__ == 0)
  if (!(lhs.is_shrinkable() || rhs.is_shrinkable())) {
    ...
    typename matrix<T2, MemoryBlock2>::const_col_element_iterator rhs_it(
        rhs.col(0).first
    );
    for (; lhs_it != lhs_end; ++lhs_it, ++rhs_it) op(*lhs_it, *rhs_it);
  } else {
...
#endif  // defined(__clang_major__) && __clang_major__ < 3 || (__clang_major__
        // == 3 && __clang_minor__ == 0)
}
#define ALPS_MATRIX_PLUS_MINUS_ASSIGN(T)                                       \
  template <typename MemoryBlock, typename MemoryBlock2, typename Operation>   \
  void plus_minus_assign_impl(                                                 \
      matrix<T, MemoryBlock>& m, matrix<T, MemoryBlock2> const& rhs,           \
      Operation op, tag::matrix, tag::matrix                                   \
  ) {                                                                          \
    ALPS_NUMERIC_MATRIX_DEBUG_OUTPUT(                                          \
        "using blas axpy for " << typeid(m).name() << " "                      \
                               << typeid(rhs).name()                           \
    );                                                                         \
    typename matrix<T, MemoryBlock>::value_type const sign(                    \
        detail::get_sign<Operation>::value                                     \
    );                                                                         \
    assert(m.num_cols() == rhs.num_cols() && m.num_rows() == rhs.num_rows());  \
    if (!(m.is_shrinkable() || rhs.is_shrinkable())) {                         \
      boost::numeric::bindings::blas::detail::axpy(                            \
          m.num_rows() * m.num_cols(), sign, &(*rhs.col(0).first), 1,          \
          &(*m.col(0).first), 1                                                \
      );                                                                       \
    } else {                                                                   \
      for (std::size_t j = 0; j < m.num_cols(); ++j)                           \
        boost::numeric::bindings::blas::detail::axpy(                          \
            m.num_rows(), sign, &(*rhs.col(j).first), 1, &(*m.col(j).first), 1 \
        );                                                                     \
    }                                                                          \
  }
...
void multiplies_assign_impl(
    matrix<T, MemoryBlock>& lhs, T2 const& t, tag::matrix, tag::scalar
) {
  typedef typename matrix<T, MemoryBlock>::size_type size_type;
  typedef typename matrix<T, MemoryBlock>::col_element_iterator
      col_element_iterator;
  if (!(lhs.is_shrinkable())) {
    std::for_each(
        lhs.col(0).first, lhs.col(lhs.num_cols() - 1).second,
        boost::lambda::_1 *= t
    );
  } else {
    // Do the operation column by column
...
#define ALPS_MATRIX_MULTIPLIES_ASSIGN(T)                                      \
  template <typename MemoryBlock>                                             \
  void multiplies_assign_impl(                                                \
      matrix<T, MemoryBlock>& m, T const& t, tag::matrix, tag::scalar         \
  ) {                                                                         \
    ALPS_NUMERIC_MATRIX_DEBUG_OUTPUT(                                         \
        "using blas scal for " << typeid(m).name() << " " << typeid(t).name() \
    );                                                                        \
    if (!(m.is_shrinkable())) {                                               \
      boost::numeric::bindings::blas::detail::scal(                           \
          m.num_rows() * m.num_cols(), t, &(*m.col(0).first), 1               \
      );                                                                      \
    } else {                                                                  \
      for (std::size_t j = 0; j < m.num_cols(); ++j)                          \
        boost::numeric::bindings::blas::detail::scal(                         \
            m.num_rows(), t, &(*m.col(j).first), 1                            \
        );                                                                    \
    }                                                                         \
  }
...
namespace alps {
namespace numeric {
// If someone has a better idea how to handle the comma, please improve these
// lines
