...
class matrix_element_iterator : public boost::iterator_facade<
                                    matrix_element_iterator<Matrix, T>, T,
 ...
 public:
  ...
  matrix_element_iterator(
      Matrix* m, typename Matrix::size_type row_p,
      typename Matrix::size_type col_p
  )
      : m(m), i(row_p), j(col_p) {
    ...
    std::cerr << "WARNING: matrix_element_iterators are very slow!"
              << std::endl;
    std::cerr << "You should use strided_iterators (eg. row_iterator) instead, "
                 "unless you really don't care."
              << std::endl;
    std::cerr << "To disable this warning compile with "
 ...
 private:
  ...
  friend class matrix_element_iterator;

  value_type& dereference() const { return m->operator()(i, j); }
  ...
  template <typename U>
  typename Matrix::difference_type distance_to(
      matrix_element_iterator<Matrix, U> const& z
  ) const {
