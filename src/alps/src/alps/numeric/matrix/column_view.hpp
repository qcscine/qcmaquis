...
class column_view {
 public:
  ...
  typedef typename boost::mpl::if_<
      boost::is_const<Matrix>, const_iterator,
      typename Matrix::col_element_iterator>::type iterator;

  explicit column_view(Matrix& m, size_type j) : col_(col(m, j)) {}

  iterator begin() { return col_.first; }
  iterator end() { return col_.second; }
  const_iterator begin() const { return col_.first; }
  const_iterator end() const { return col_.second; }
  ...
  inline reference operator()(std::size_t i) { return this->operator[](i); }
  ...
  column_view& operator+=(T2 const& rhs) {
    plus_assign(
        *this, rhs, typename get_entity<column_view>::type(),
  ...
  column_view& operator*=(T2 const& x) {
    multiplies_assign(
        *this, x, typename get_entity<column_view>::type(),
        typename get_entity<T2>::type()
    );
    return *this;
  }
...
struct plus_minus_return_type<
    column_view<Matrix>, Vector, tag::vector, tag::vector> {
  typedef vector<typename detail::auto_deduce_plus_return_type<
      typename Matrix::value_type, typename Vector::value_type>::type>
      type;
};

template <typename Matrix, typename Vector>
struct plus_minus_return_type<
    Vector, column_view<Matrix>, tag::vector, tag::vector> {
  typedef vector<typename detail::auto_deduce_plus_return_type<
      typename Matrix::value_type, typename Vector::value_type>::type>
      type;
};
...
struct plus_minus_return_type<
    column_view<Matrix1>, column_view<Matrix2>, tag::vector, tag::vector> {
  typedef vector<typename detail::auto_deduce_plus_return_type<
      typename Matrix1::value_type, typename Matrix2::value_type>::type>
      type;
};

template <typename Matrix1, typename Matrix2>
struct multiply_return_type<
    Matrix1, column_view<Matrix2>, tag::matrix, tag::vector> {
  typedef vector<typename detail::auto_deduce_multiply_return_type<
      typename Matrix1::value_type, typename Matrix2::value_type>::type>
      type;
};
