...
struct scalar_type<boost::multi_array<T, N, A> > {
  typedef typename scalar_type<typename boost::remove_reference<
      typename boost::remove_cv<T>::type>::type>::type type;
};
template <typename T, std::size_t N, typename A>
struct scalar_type<alps::multi_array<T, N, A> >
    : public scalar_type<boost::multi_array<T, N, A> > {};

template <typename T, std::size_t N, typename A>
struct is_content_continuous<alps::multi_array<T, N, A> >
...
template <typename T, std::size_t N, typename A>
struct has_complex_elements<alps::multi_array<T, N, A> >
...
struct get_extent<boost::multi_array<T, N, A> > {
  static std::vector<std::size_t> apply(boost::multi_array<T, N, A> const &value
    ...
    std::vector<std::size_t> result(
        value.shape(),
        value.shape() + boost::multi_array<T, N, A>::dimensionality
    );
...
template <typename T, std::size_t N, typename A>
struct get_extent<alps::multi_array<T, N, A> >
    : public get_extent<boost::multi_array<T, N, A> > {};

template <typename T, std::size_t N, typename A>
struct set_extent<boost::multi_array<T, N, A> > {
  static void apply(
      boost::multi_array<T, N, A> &value, std::vector<std::size_t> const &size
  ) {
    ...
    if (!is_continuous<T>::value &&
        boost::multi_array<T, N, A>::dimensionality < size.size())
      for (std::size_t i = 0; i < value.num_elements(); ++i)
        set_extent(
            value.data()[i],
            std::vector<std::size_t>(
                size.begin() + boost::multi_array<T, N, A>::dimensionality,
                size.end()
            )
        );
  }
 ...
 private:
  ...
  static void gen_extent(
      boost::multi_array<T, N, A> &value,
      boost::detail::multi_array::extent_gen<M> extents,
      std::vector<std::size_t> const &size
  ) {
    gen_extent(
        value, extents[size.front()],
        std::vector<std::size_t>(size.begin() + 1, size.end())
    );
  }
  static void gen_extent(
      boost::multi_array<T, N, A> &value,
      typename boost::detail::multi_array::extent_gen<N> extents,
      std::vector<std::size_t> const &size
  ) {
...
template <typename T, std::size_t N, typename A>
struct set_extent<alps::multi_array<T, N, A> >
...
struct is_vectorizable<boost::multi_array<T, N, A> > {
  static bool apply(boost::multi_array<T, N, A> const &value) {
    ...
    for (std::size_t i = 1; i < value.num_elements(); ++i)
      if (!is_vectorizable(value.data()[i]) ||
          !std::equal(
              size.begin(), size.end(), get_extent(value.data()[i]).begin()
          ))
        return false;
    return true;
  }
...
template <typename T, std::size_t N, typename A>
struct is_vectorizable<alps::multi_array<T, N, A> >
    : public is_vectorizable<boost::multi_array<T, N, A> > {};

template <typename T, std::size_t N, typename A>
struct get_pointer<boost::multi_array<T, N, A> > {
  static typename alps::hdf5::scalar_type<boost::multi_array<T, N, A> >::type *
  apply(boost::multi_array<T, N, A> &value) {
    using alps::hdf5::get_pointer;
    return get_pointer(*value.data());
  }
};
template <typename T, std::size_t N, typename A>
struct get_pointer<alps::multi_array<T, N, A> >
...
template <typename T, std::size_t N, typename A>
struct get_pointer<alps::multi_array<T, N, A> const>
...
void save(
  ...
  if (is_continuous<T>::value) {
    ...
    for (std::size_t i = 1; i < value.num_elements(); ++i) {
      ...
      for (typename boost::multi_array<T, N, A>::size_type const *it =
               value.shape() + 1;
...
void save(
  ...
  save(
      ar, path, static_cast<boost::multi_array<T, N, A> const &>(value), size,
      chunk, offset
  );
}
...
template <typename T, std::size_t N, typename A>
void load(
    archive &ar, std::string const &path, boost::multi_array<T, N, A> &value,
    std::vector<std::size_t> chunk = std::vector<std::size_t>(),
    std::vector<std::size_t> offset = std::vector<std::size_t>()
) {
  if (ar.is_group(path))
...
template <typename T, std::size_t N, typename A>
void load(
    archive &ar, std::string const &path, alps::multi_array<T, N, A> &value,
    std::vector<std::size_t> chunk = std::vector<std::size_t>(),
    std::vector<std::size_t> offset = std::vector<std::size_t>()
) {
  load(
      ar, path, static_cast<boost::multi_array<T, N, A> &>(value), chunk, offset
  );
}
}  // namespace hdf5
}  // namespace alps
