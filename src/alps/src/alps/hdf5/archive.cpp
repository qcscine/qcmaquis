 ...
 * Library License; you can use, redistribute it and/or modify it under * the
 * terms of the license, either version 1 or (at your option) any later        *
 * version. *
 *                                                                                 *
 * You should have received a copy of the ALPS Library License along with * the
 * ALPS Libraries; see the file LICENSE.txt. If not, the license is also       *
...
herr_t noop(hid_t) { return 0; }

template <typename T>
struct native_ptr_converter {
  native_ptr_converter(std::size_t) {}
  inline T const *apply(T const *v) { return v; }
};
...
struct native_ptr_converter<std::string> {
  ...
  inline char const *const *apply(std::string const *v) {
    for (std::vector<char const *>::iterator it = data.begin();
         it != data.end(); ++it)
 ...
 private:
  static herr_t callback(unsigned n, H5E_error2_t const *desc, void *buffer) {
    *reinterpret_cast<std::ostringstream *>(buffer)
        << "    #" << cast<std::string>(n) << " " << desc->file_name << " line "
        << cast<std::string>(desc->line) << " in " << desc->func_name
        << "(): " << desc->desc << std::endl;
    return 0;
  }
};

template <herr_t (*F)(hid_t)>
class resource {
 public:
  resource() : _id(-1) {}
  resource(hid_t id) : _id(id) {
    if (_id < 0) throw archive_error(error().invoke(_id) + ALPS_STACKTRACE);
  }

  ~resource() {
    if (_id < 0 || (_id = F(_id)) < 0) {
      std::cerr
          << "Error in " << __FILE__ << " on " << ALPS_NGS_STRINGIFY(__LINE__)
          << " in "
          << __FUNCTION__  // TODO: check for gcc and use __PRETTY_FUNCTION__
          << ":" << std::endl
          << error().invoke(_id) << std::endl;
      std::abort();
    }
  }
...
hid_t check_group(hid_t id) {
  group_type unused(id);
  return unused;
}
hid_t check_data(hid_t id) {
  data_type unused(id);
  return unused;
}
hid_t check_attribute(hid_t id) {
  attribute_type unused(id);
  return unused;
}
hid_t check_space(hid_t id) {
  space_type unused(id);
  return unused;
}
hid_t check_type(hid_t id) {
  type_type unused(id);
  return unused;
}
hid_t check_property(hid_t id) {
  property_type unused(id);
  return unused;
}
hid_t check_error(hid_t id) {
  error_type unused(id);
  return unused;
}
...
hid_t open_attribute(archive const &ar, hid_t file_id, std::string path) {
  ...
  return H5Aopen_by_name(
      file_id, path.substr(0, path.find_last_of('@') - 1).c_str(),
      path.substr(path.find_last_of('@') + 1).c_str(), H5P_DEFAULT, H5P_DEFAULT
  );
}

herr_t list_children_visitor(
...
herr_t list_attributes_visitor(
...
struct ALPS_DECL archivecontext : boost::noncopyable {
  archivecontext(
      std::string const &filename, bool write, bool replace, bool compress,
      bool large, bool memory
  )
      : compress_(compress),
 ...
 private:
  void construct() {
    ...
    if (memory_ && large_)
      throw archive_error(
          "either memory or large file system can be used!" + ALPS_STACKTRACE
      );
    else if (memory_) {
      ...
      if (write_) {
        if ((file_id_ = H5Fopen(
          ...
          detail::check_error(
              file_id_ = H5Fcreate(
                  (filename_ + suffix_).c_str(), H5F_ACC_TRUNC, fcrt_id, prop_id
              )
          );
        }
      ...
      if (replace_ && large_)
        throw archive_error(
            "the combination 'wl' is not allowd!" + ALPS_STACKTRACE
        );
      if (replace_)
      ...
      if (large_) {
        {
          ...
          if (!strcmp(filename0, filename1))
            throw archive_error(
                "Large hdf5 archives need to have a '%d' part in the filename" +
                ALPS_STACKTRACE
            );
        }
        detail::property_type prop_id(H5Pcreate(H5P_FILE_ACCESS));
        ...
        if (write_) {
          if ((file_id_ = H5Fopen(
            ...
            detail::check_error(
                file_id_ = H5Fcreate(
                    (filename_ + suffix_).c_str(), H5F_ACC_TRUNC, fcrt_id,
                    prop_id
                )
            );
          }
        ...
        if (!write_) {
          if (!boost::filesystem::exists(filename_ + suffix_))
            throw archive_not_found(
                "file does not exist: " + filename_ + suffix_ + ALPS_STACKTRACE
            );
          if (detail::check_error(H5Fis_hdf5((filename_ + suffix_).c_str())) ==
              0)
            throw archive_error(
                "no valid hdf5 file: " + filename_ + suffix_ + ALPS_STACKTRACE
            );
        }
#ifndef ALPS_HDF5_CLOSE_GREEDY
        detail::property_type ALPS_HDF5_FILE_ACCESS(H5Pcreate(H5P_FILE_ACCESS));
        detail::check_error(
            H5Pset_fclose_degree(ALPS_HDF5_FILE_ACCESS, H5F_CLOSE_SEMI)
        );
#else
#define ALPS_HDF5_FILE_ACCESS H5P_DEFAULT
        ...
        if (write_) {
          if ((file_id_ = H5Fopen(
                   (filename_ + suffix_).c_str(), H5F_ACC_RDWR,
                   ALPS_HDF5_FILE_ACCESS
               )) < 0) {
            detail::property_type fcrt_id(H5Pcreate(H5P_FILE_CREATE));
            detail::check_error(H5Pset_link_creation_order(
                fcrt_id, (H5P_CRT_ORDER_TRACKED | H5P_CRT_ORDER_INDEXED)
          ...
          detail::check_error(
              file_id_ = H5Fopen(
                  (filename_ + suffix_).c_str(), H5F_ACC_RDONLY,
                  ALPS_HDF5_FILE_ACCESS
              )
          );
    ...
    try {
...
#ifndef ALPS_HDF5_CLOSE_GREEDY
      if (H5Fget_obj_count(file_id_, H5F_OBJ_DATATYPE) > 0 ||
          H5Fget_obj_count(file_id_, H5F_OBJ_ALL) -
                  H5Fget_obj_count(file_id_, H5F_OBJ_FILE) >
              0) {
        std::cerr << "Not all resources closed in file '" << filename_
                  << suffix_ << "'" << std::endl;
        std::abort();
      }
#endif
      if (H5Fclose(file_id_) < 0)
        std::cerr
            ...
            << __FUNCTION__  // TODO: check for gcc and use __PRETTY_FUNCTION__
            << ":" << std::endl
            << error().invoke(file_id_) << std::endl;
      if (replace_) {
      ...
      if (abort) {
        std::cerr << "Error destroying HDF5 context of file '" << filename_
                  << suffix_ << "'\n"
                  << ex.what() << std::endl;
        std::abort();
      } else
...
archive::archive(
  ...
  construct(
      filename, ('w' == prop ? WRITE | REPLACE : 0) |
                    ('a' == prop ? WRITE : 0) | ('c' == prop ? COMPRESS : 0) |
                    ('l' == prop ? LARGE : 0) | ('m' == prop ? MEMORY : 0)
  );
}
...
archive::archive(archive const &arg)
    : current_(arg.current_), context_(arg.context_) {
  if (context_ != NULL) {
    ALPS_HDF5_LOCK_MUTEX
    ++ref_cnt_[file_key(
                   context_->filename_, context_->large_, context_->memory_
               )]
          .second;
  }
}
...
archive::~archive() {
  if (context_ != NULL) try {
      close();
    } catch (std::exception &ex) {
      std::cerr << "Error destructing archive of file '" << ex.what()
                << std::endl;
      std::abort();
    }
}
...
void archive::abort() {
  ...
  for (std::map<
           std::string,
           std::pair<detail::archivecontext *, std::size_t> >::iterator it =
           ref_cnt_.begin();
       it != ref_cnt_.end(); ++it) {
...
void archive::close() {
  ...
  if (!--ref_cnt_[file_key(
                      context_->filename_, context_->large_, context_->memory_
                  )]
             .second) {
    ref_cnt_.erase(
        file_key(context_->filename_, context_->large_, context_->memory_)
    );
    delete context_;
  }
  context_ = NULL;
...
bool archive::is_open() { return context_ != NULL; }
...
std::string archive::encode_segment(std::string segment) const {
  ...
  for (std::size_t i = 0; i < sizeof(chars); ++i)
    for (std::size_t pos = segment.find_first_of(chars[i]);
         pos < std::string::npos;
         pos = segment.find_first_of(chars[i], pos + 1))
      segment = segment.substr(0, pos) + "&#" +
                cast<std::string>(static_cast<int>(chars[i])) + ";" +
                segment.substr(pos + 1);
  return segment;
}

std::string archive::decode_segment(std::string segment) const {
  for (std::size_t pos = segment.find_first_of('&'); pos < std::string::npos;
       pos = segment.find_first_of('&', pos + 1))
    segment = segment.substr(0, pos) +
              static_cast<char>(cast<int>(segment.substr(
                  pos + 2, segment.find_first_of(';', pos) - pos - 2
              ))) +
...
bool archive::is_attribute(std::string path) const {
  ...
  if ((path = complete_path(path)).find_last_of('@') == std::string::npos)
    return false;
  ALPS_HDF5_FAKE_THREADSAFETY
  return detail::check_error(H5Aexists_by_name(
      context_->file_id_, path.substr(0, path.find_last_of('@') - 1).c_str(),
      path.substr(path.find_last_of('@') + 1).c_str(), H5P_DEFAULT
  ));
}
...
bool archive::is_scalar(std::string path) const {
  ...
  if ((path = complete_path(path)).find_last_of('@') != std::string::npos &&
      is_attribute(path)) {
    detail::attribute_type attr_id(
        detail::open_attribute(*this, context_->file_id_, path)
    );
...
bool archive::is_complex(std::string path) const {
  ...
  if ((path = complete_path(path)).find_last_of('@') != std::string::npos)
    return is_attribute(
               ...
               "@__complex__:" + path.substr(path.find_last_of('@') + 1)
           ) &&
           is_scalar(
               path.substr(0, path.find_last_of('@')) +
               "@__complex__:" + path.substr(path.find_last_of('@') + 1)
    ...
    for (std::size_t i = 0; i < children.size(); ++i)
      if (is_complex(path + "/" + children[i])) return true;
    return false;
  } else
    return is_attribute(path + "/@__complex__") &&
           is_scalar(path + "/@__complex__");
}
...
std::vector<std::string> archive::list_children(std::string path) const {
  ...
  if (!is_group(path))
    throw path_not_found(
        "The group '" + path + "' does not exist." + ALPS_STACKTRACE
    );
  detail::group_type group_id(
      H5Gopen2(context_->file_id_, path.c_str(), H5P_DEFAULT)
  );
  detail::check_error(H5Literate(
      group_id, H5_INDEX_NAME, H5_ITER_NATIVE, NULL,
      detail::list_children_visitor, &list
  ));
  return list;
}
...
std::vector<std::string> archive::list_attributes(std::string path) const {
  ...
  if (is_group(path)) {
    ...
    throw path_not_found(
        "The path '" + path + "' does not exist." + ALPS_STACKTRACE
    );
  return list;
}
...
std::size_t archive::dimensions(std::string path) const {
  ...
  if ((path = complete_path(path)).find_last_of('@') != std::string::npos) {
    ...
    return detail::check_error(H5Sget_simple_extent_dims(
        detail::space_type(H5Aget_space(attr_id)), NULL, NULL
    ));
  } else {
    detail::data_type data_id(
        H5Dopen2(context_->file_id_, path.c_str(), H5P_DEFAULT)
    ...
    return detail::check_error(H5Sget_simple_extent_dims(
        detail::space_type(H5Dget_space(data_id)), NULL, NULL
    ));
  }
}
...
void archive::create_group(std::string path) const {
  ...
  if ((path = complete_path(path)).find_last_of('@') != std::string::npos)
    throw invalid_path("no group path: " + path + ALPS_STACKTRACE);
  ALPS_HDF5_FAKE_THREADSAFETY
  if (is_data(path)) delete_data(path);
  if (!is_group(path)) {
    std::size_t pos;
    hid_t group_id = -1;
    for (pos = path.find_last_of('/');
         group_id < 0 && pos > 0 && pos < std::string::npos;
         pos = path.find_last_of('/', pos - 1))
      group_id = H5Gopen2(
          context_->file_id_, path.substr(0, pos).c_str(), H5P_DEFAULT
      );
    if (group_id < 0) {
    ...
    while (pos != std::string::npos &&
           (pos = path.find_first_of('/', pos + 1)) != std::string::npos &&
           pos > 0) {
      detail::property_type prop_id(H5Pcreate(H5P_GROUP_CREATE));
      detail::check_error(H5Pset_link_creation_order(
          prop_id, (H5P_CRT_ORDER_TRACKED | H5P_CRT_ORDER_INDEXED)
      ...
      detail::check_group(H5Gcreate2(
          context_->file_id_, path.substr(0, pos).c_str(), H5P_DEFAULT, prop_id,
          H5P_DEFAULT
      ));
    }
    detail::property_type prop_id(H5Pcreate(H5P_GROUP_CREATE));
    ...
    detail::check_group(H5Gcreate2(
        context_->file_id_, path.c_str(), H5P_DEFAULT, prop_id, H5P_DEFAULT
    ));
  }
}
...
void archive::delete_group(std::string path) const {
  ...
  if (is_group(path))
    ...
    throw invalid_path(
        "the path contains a dataset: " + path + ALPS_STACKTRACE
    );
}
...
#define ALPS_NGS_HDF5_READ_SCALAR(T)                                           \
  void archive::read(std::string path, T &value) const {                       \
    ...
    if ((path = complete_path(path)).find_last_of('@') == std::string::npos) { \
      if (!is_data(path))                                                      \
        throw path_not_found(                                                  \
            "the path does not exist: " + path + ALPS_STACKTRACE               \
        );                                                                     \
      else if (!is_scalar(path))                                               \
        throw wrong_type(                                                      \
            "scalar - vector conflict in path: " + path + ALPS_STACKTRACE      \
        );                                                                     \
      detail::data_type data_id(                                               \
          H5Dopen2(context_->file_id_, path.c_str(), H5P_DEFAULT)              \
      );                                                                       \
      ...
      if (H5Tget_class(native_id) == H5T_STRING &&                             \
        ...
        detail::check_error(H5Dread(                                           \
            data_id, native_id, H5S_ALL, H5S_ALL, H5P_DEFAULT, &raw[0]         \
        ));                                                                    \
        value = cast<T>(raw);                                                  \
      } else if (H5Tget_class(native_id) == H5T_STRING) {                      \
        char *raw;                                                             \
        detail::check_error(                                                   \
            H5Dread(data_id, native_id, H5S_ALL, H5S_ALL, H5P_DEFAULT, &raw)   \
        );                                                                     \
        value = cast<T>(std::string(raw));                                     \
        detail::check_error(H5Dvlen_reclaim(                                   \
            type_id, detail::space_type(H5Dget_space(data_id)), H5P_DEFAULT,   \
            &raw                                                               \
        ));                                                                    \
        ALPS_NGS_HDF5_FOREACH_NATIVE_TYPE_INTEGRAL(                            \
            ALPS_NGS_HDF5_READ_SCALAR_DATA_HELPER, T                           \
        )                                                                      \
      } else                                                                   \
      ...
      if (H5Tget_class(native_id) == H5T_STRING &&                             \
        ...
        value = cast<T>(std::string(raw));                                     \
        ALPS_NGS_HDF5_FOREACH_NATIVE_TYPE_INTEGRAL(                            \
            ALPS_NGS_HDF5_READ_SCALAR_ATTRIBUTE_HELPER, T                      \
        )                                                                      \
      } else                                                                   \
        throw wrong_type("invalid type" + ALPS_STACKTRACE);                    \
    }                                                                          \
...
#define ALPS_NGS_HDF5_READ_VECTOR(T)                                           \
  void archive::read(                                                          \
      std::string path, T *value, std::vector<std::size_t> chunk,              \
    ...
    if (data_size.size() != chunk.size() || data_size.size() != offset.size()) \
      throw archive_error(                                                     \
          "wrong size or offset passed for path: " + path + ALPS_STACKTRACE    \
      );                                                                       \
    ...
    if (is_null(path))                                                         \
      ...
      if ((path = complete_path(path)).find_last_of('@') ==                    \
          std::string::npos) {                                                 \
        if (!is_data(path))                                                    \
          throw path_not_found(                                                \
              "the path does not exist: " + path + ALPS_STACKTRACE             \
        ...
        if (H5Tget_class(native_id) == H5T_STRING &&                           \
            !detail::check_error(H5Tis_variable_str(type_id)))                 \
          throw std::logic_error(                                              \
              "multidimensional dataset of fixed string datas is not "         \
              "implemented (" +                                                \
              path + ")" + ALPS_STACKTRACE                                     \
          );                                                                   \
        else if (H5Tget_class(native_id) == H5T_STRING) {                      \
          std::size_t len = std::accumulate(                                   \
              chunk.begin(), chunk.end(), std::size_t(1),                      \
          ...
          boost::scoped_array<char *> raw(new char *[len]);                    \
          if (std::equal(chunk.begin(), chunk.end(), data_size.begin())) {     \
            detail::check_error(H5Dread(                                       \
                data_id, native_id, H5S_ALL, H5S_ALL, H5P_DEFAULT, raw.get()   \
            ));                                                                \
            cast(raw.get(), raw.get() + len, value);                           \
            detail::check_error(H5Dvlen_reclaim(                               \
                type_id, detail::space_type(H5Dget_space(data_id)),            \
                H5P_DEFAULT, raw.get()                                         \
            ));                                                                \
          } else {                                                             \
            std::vector<hsize_t> offset_hid(offset.begin(), offset.end()),     \
            ...
            cast(raw.get(), raw.get() + len, value);                           \
            detail::check_error(                                               \
                H5Dvlen_reclaim(type_id, mem_id, H5P_DEFAULT, raw.get())       \
            );                                                                 \
          }                                                                    \
          ALPS_NGS_HDF5_FOREACH_NATIVE_TYPE_INTEGRAL(                          \
              ALPS_NGS_HDF5_READ_VECTOR_DATA_HELPER, T                         \
          )                                                                    \
        } else                                                                 \
          throw wrong_type("invalid type" + ALPS_STACKTRACE);                  \
      } else {                                                                 \
        ...
        if (is_group(path.substr(0, path.find_last_of('@') - 1)))              \
          parent_id = detail::check_error(H5Gopen2(                            \
              context_->file_id_,                                              \
              path.substr(0, path.find_last_of('@') - 1).c_str(), H5P_DEFAULT  \
          ));                                                                  \
        else if (is_data(path.substr(0, path.find_last_of('@') - 1)))          \
          parent_id = detail::check_error(H5Dopen2(                            \
              context_->file_id_,                                              \
              path.substr(0, path.find_last_of('@') - 1).c_str(), H5P_DEFAULT  \
          ));                                                                  \
        else                                                                   \
          throw path_not_found(                                                \
              "unknown path: " + path.substr(0, path.find_last_of('@') - 1) +  \
              ALPS_STACKTRACE                                                  \
          );                                                                   \
        detail::attribute_type attribute_id(H5Aopen(                           \
            parent_id, path.substr(path.find_last_of('@') + 1).c_str(),        \
            H5P_DEFAULT                                                        \
        ));                                                                    \
        detail::type_type type_id(H5Aget_type(attribute_id));                  \
        detail::type_type native_id(                                           \
        ...
        if (H5Tget_class(native_id) == H5T_STRING &&                           \
          ...
          if (std::equal(chunk.begin(), chunk.end(), data_size.begin())) {     \
            ...
            throw std::logic_error(                                            \
                "non continous multidimensional dataset as attributes are "    \
                "not implemented (" +                                          \
                path + ")" + ALPS_STACKTRACE                                   \
            );                                                                 \
          detail::check_error(H5Dvlen_reclaim(                                 \
              type_id, detail::space_type(H5Aget_space(attribute_id)),         \
              H5P_DEFAULT, raw.get()                                           \
          ));                                                                  \
        } else if (H5Tget_class(native_id) == H5T_STRING) {                    \
          char **raw = NULL;                                                   \
          detail::check_error(H5Aread(attribute_id, native_id, raw));          \
          throw std::logic_error(                                              \
              "multidimensional dataset of variable len string datas is not "  \
              "implemented (" +                                                \
              path + ")" + ALPS_STACKTRACE                                     \
          );                                                                   \
          ALPS_NGS_HDF5_FOREACH_NATIVE_TYPE_INTEGRAL(                          \
              ALPS_NGS_HDF5_READ_VECTOR_ATTRIBUTE_HELPER, T                    \
          )                                                                    \
        } else                                                                 \
          throw wrong_type("invalid type" + ALPS_STACKTRACE);                  \
        if (is_group(path.substr(0, path.find_last_of('@') - 1)))              \
...
#define ALPS_NGS_HDF5_WRITE_SCALAR(T)                                          \
  void archive::write(std::string path, T value) const {                       \
    ...
    hid_t data_id;                                                             \
    if ((path = complete_path(path)).find_last_of('@') == std::string::npos) { \
      if (is_group(path)) delete_group(path);                                  \
      ...
      if (data_id < 0) {                                                       \
        ...
        if (class_type != H5S_SCALAR || !is_datatype<T>(path)) {               \
          ...
          if (path.find_last_of('/') < std::string::npos &&                    \
              path.find_last_of('/') > 0) {                                    \
            detail::group_type group_id(H5Gopen2(                              \
                context_->file_id_,                                            \
                path.substr(0, path.find_last_of('/')).c_str(), H5P_DEFAULT    \
            ));                                                                \
            detail::check_error(H5Ldelete(                                     \
                group_id, path.substr(path.find_last_of('/') + 1).c_str(),     \
                H5P_DEFAULT                                                    \
            ));                                                                \
          } else                                                               \
            detail::check_error(                                               \
      ...
      detail::type_type type_id(                                               \
          detail::get_native_type(alps::detail::type_wrapper<T>::type())       \
      );                                                                       \
      if (data_id < 0) {                                                       \
        ...
        data_id = H5Dcreate2(                                                  \
            ...
            detail::space_type(H5Screate(H5S_SCALAR)), H5P_DEFAULT, prop_id,   \
            H5P_DEFAULT                                                        \
        );                                                                     \
      }                                                                        \
      ...
      detail::check_error(H5Dwrite(                                            \
          data_id, type_id, H5S_ALL, H5S_ALL, H5P_DEFAULT,                     \
      ...
      if (data_id < 0)                                                         \
        data_id = H5Acreate2(                                                  \
            parent_id, path.substr(path.find_last_of('@') + 1).c_str(),        \
            type_id, detail::space_type(H5Screate(H5S_SCALAR)), H5P_DEFAULT,   \
            H5P_DEFAULT                                                        \
        );                                                                     \
      detail::native_ptr_converter<                                            \
          boost::remove_cv<boost::remove_reference<T>::type>::type>            \
...
#define ALPS_NGS_HDF5_WRITE_VECTOR(T)                                          \
  void archive::write(                                                         \
      std::string path, T const *value, std::vector<std::size_t> size,         \
    ...
    if (offset.size() == 0) offset = std::vector<std::size_t>(size.size(), 0); \
    if (size.size() != offset.size())                                          \
      throw archive_error(                                                     \
          "wrong chunk or offset passed for path: " + path + ALPS_STACKTRACE   \
      );                                                                       \
    hid_t data_id;                                                             \
    if ((path = complete_path(path)).find_last_of('@') == std::string::npos) { \
      if (is_group(path)) delete_group(path);                                  \
      ...
      if (std::accumulate(size.begin(), size.end(), std::size_t(0)) == 0) {    \
        if (data_id < 0) {                                                     \
          ...
          detail::check_data(H5Dcreate2(                                       \
              context_->file_id_, path.c_str(), type_id,                       \
              detail::space_type(H5Screate(H5S_NULL)), H5P_DEFAULT, prop_id,   \
              H5P_DEFAULT                                                      \
          ));                                                                  \
        } else                                                                 \
          detail::check_data(data_id);                                         \
        ...
        if (data_id < 0) {                                                     \
          ...
          if (std::is_same<T, std::string>::value)                             \
            detail::check_error(                                               \
                data_id = H5Dcreate2(                                          \
                    ...
                    detail::space_type(H5Screate_simple(                       \
                        static_cast<int>(size_hid.size()), &size_hid.front(),  \
                        NULL                                                   \
                    )),                                                        \
            ...
            if (dataset_size < ALPS_HDF5_SZIP_BLOCK_SIZE * sizeof(T))          \
              ...
              detail::check_error(H5Pset_chunk(                                \
                  prop_id, static_cast<int>(max_chunk.size()),                 \
                  &max_chunk.front()                                           \
              ));                                                              \
            }                                                                  \
            if (context_->compress_ &&                                         \
                dataset_size > ALPS_HDF5_SZIP_BLOCK_SIZE * sizeof(T))          \
              detail::check_error(H5Pset_szip(                                 \
                  prop_id, H5_SZIP_NN_OPTION_MASK, ALPS_HDF5_SZIP_BLOCK_SIZE   \
              ));                                                              \
            detail::check_error(H5Pset_attr_creation_order(                    \
                prop_id, (H5P_CRT_ORDER_TRACKED | H5P_CRT_ORDER_INDEXED)       \
            ));                                                                \
      ...
      if (std::accumulate(size.begin(), size.end(), std::size_t(0)) == 0) {    \
        if (data_id < 0)                                                       \
          detail::check_attribute(H5Acreate2(                                  \
              parent_id, path.substr(path.find_last_of('@') + 1).c_str(),      \
              type_id, detail::space_type(H5Screate(H5S_NULL)), H5P_DEFAULT,   \
              H5P_DEFAULT                                                      \
          ));                                                                  \
        else                                                                   \
          detail::check_attribute(data_id);                                    \
        ...
        if (data_id < 0)                                                       \
          data_id = detail::check_error(H5Acreate2(                            \
              ...
              type_id,                                                         \
              detail::space_type(H5Screate_simple(                             \
                  static_cast<int>(size_hid.size()), &size_hid.front(), NULL   \
              )),                                                              \
...
#define ALPS_NGS_HDF5_IMPLEMENT_FREE_FUNCTIONS(T)                      \
  ...
  alps::hdf5::scalar_type<T>::type *get_pointer<T>::apply(T &value) {  \
    return &value;                                                     \
                                                                       ...
                                                                       \
  alps::hdf5::scalar_type<T>::type const *get_pointer<T const>::apply( \
      T const &value                                                   \
  ) {                                                                  \
    return &value;                                                     \
  }                                                                    \
                                                                       \
  bool is_vectorizable<T>::apply(T const &value) { return true; }      \
  bool is_vectorizable<T const>::apply(T &value) { return true; }      \
  }                                                                    \
                                                                       \
  void save(                                                           \
      archive &ar, std::string const &path, T const &value,            \
    ...
    if (!size.size())                                                  \
      ...
      ar.write(path, get_pointer(value), size, chunk, offset);         \
                                                                       ...
                                                                       \
  void load(                                                           \
      archive &ar, std::string const &path, T &value,                  \
      std::vector<std::size_t> chunk, std::vector<std::size_t> offset  \
  ) {                                                                  \
    if (!chunk.size())                                                 \
      ar.read(path, value);                                            \
...
struct is_datatype_impl_compare {
  static bool apply(type_type const &native_id) {
    return check_error(H5Tequal(
               type_type(H5Tcopy(native_id)),
               type_type(get_native_type(
...
void archive::construct(std::string const &filename, std::size_t props) {
  ...
  if (ref_cnt_.find(file_key(filename, props & LARGE, props & MEMORY)) ==
    ...
    ++ref_cnt_.find(file_key(filename, props & LARGE, props & MEMORY))
          ->second.second;
  }
}
...
#endif
std::map<std::string, std::pair<detail::archivecontext *, std::size_t> >
    archive::ref_cnt_;
}  // namespace hdf5
}  // namespace alps
