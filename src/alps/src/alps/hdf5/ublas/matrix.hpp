...
struct scalar_type<boost::numeric::ublas::matrix<T, F, A> > {
  typedef typename scalar_type<typename boost::remove_reference<
      typename boost::remove_cv<T>::type>::type>::type type;
};

template <typename T, typename F, typename A>
struct has_complex_elements<boost::numeric::ublas::matrix<T, F, A> >
...
template <typename T, typename F, typename A>
struct get_extent<boost::numeric::ublas::matrix<T, F, A> > {
...
template <typename T, typename F, typename A>
struct set_extent<boost::numeric::ublas::matrix<T, F, A> > {
  static void apply(
    ...
    if (!is_continuous<T>::value && size.size() != 2)
      for (std::size_t i = 0; i < value.size1(); ++i)
        for (std::size_t j = 0; j < value.size2(); ++j)
          set_extent(
              value(i, j),
              std::vector<std::size_t>(size.begin() + 2, size.end())
          );
  }
};

template <typename T, typename F, typename A>
struct is_vectorizable<boost::numeric::ublas::matrix<T, F, A> > {
  static bool apply(boost::numeric::ublas::matrix<T, F, A> const& value) {
    using alps::hdf5::get_extent;
    using alps::hdf5::is_vectorizable;
    if (!boost::is_scalar<typename boost::numeric::ublas::matrix<
            T, F, A>::value_type>::value) {
      std::vector<std::size_t> size(get_extent(value(0, 0)));
      for (std::size_t i = 0; i < value.size1(); ++i)
        for (std::size_t j = 1; j < value.size2(); ++j)
          if (!is_vectorizable(value(i, j)) ||
              !std::equal(
                  size.begin(), size.end(), get_extent(value(i, j)).begin()
              ))
            return false;
    }
    return true;
...
template <typename T, typename F, typename A>
struct get_pointer<boost::numeric::ublas::matrix<T, F, A> > {
  static typename alps::hdf5::scalar_type<
      boost::numeric::ublas::matrix<T, F, A> >::type*
  apply(boost::numeric::ublas::matrix<T, F, A>& value) {
...
template <typename T, typename F, typename A>
struct get_pointer<boost::numeric::ublas::matrix<T, F, A> const> {
  static typename alps::hdf5::scalar_type<
      boost::numeric::ublas::matrix<T, F, A> >::type const*
  apply(boost::numeric::ublas::matrix<T, F, A> const& value) {
    using alps::hdf5::get_pointer;
    return get_pointer(value(0, 0));
  }
};
