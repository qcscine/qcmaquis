...
template <typename T>
struct is_content_continuous<std::valarray<T> > : public is_continuous<T> {};
  ...
  static std::vector<std::size_t> apply(std::valarray<T> const &value) {
    ...
    if (value.size()) {
      std::vector<std::size_t> extent(
          get_extent(const_cast<std::valarray<T> &>(value)[0])
      );
      if (!boost::is_scalar<T>::value)
        for (std::size_t i = 1; i < value.size(); ++i)
          if (!std::equal(
                  extent.begin(), extent.end(),
                  get_extent(const_cast<std::valarray<T> &>(value)[i]).begin()
              ))
...
struct set_extent<std::valarray<T> > {
  static void apply(
      std::valarray<T> &value, std::vector<std::size_t> const &extent
  ) {
...
template <typename T>
struct is_vectorizable<std::valarray<T> > {
        ...
        for (std::size_t i = 0; i < value.size(); ++i)
          if (!is_vectorizable(const_cast<std::valarray<T> &>(value)[i])) {
            ...
            std::vector<std::size_t> size(
                get_extent(const_cast<std::valarray<T> &>(value)[i])
            );
...
struct get_pointer<std::valarray<T> > {
  static typename alps::hdf5::scalar_type<std::valarray<T> >::type *apply(
...
struct get_pointer<std::valarray<T> const> {
  static typename alps::hdf5::scalar_type<std::valarray<T> >::type const *apply(
...
template <typename T>
void save(
    archive &ar, std::string const &path, std::valarray<T> const &value,
  ...
  if (is_continuous<T>::value && value.size() == 0)
    ...
    for (std::size_t i = 0; i < value.size(); ++i) {
      offset.back() = i;
      save(
          ar, path, const_cast<std::valarray<T> &>(value)[i], size, chunk,
          offset
      );
    }
  } else {
    ...
    for (std::size_t i = 0; i < value.size(); ++i)
      save(
          ar, ar.complete_path(path) + "/" + cast<std::string>(i),
          const_cast<std::valarray<T> &>(value)[i]
      );
  }
}
...
template <typename T>
void load(
    archive &ar, std::string const &path, std::valarray<T> &value,
    std::vector<std::size_t> chunk = std::vector<std::size_t>(),
    std::vector<std::size_t> offset = std::vector<std::size_t>()
) {
  if (ar.is_group(path)) {
