...
void save(
    archive &ar, std::string const &path, std::pair<T, U> const &value,
...
template <typename T, typename U>
void load(
    archive &ar, std::string const &path, std::pair<T, U> &value,
    std::vector<std::size_t> chunk = std::vector<std::size_t>(),
    std::vector<std::size_t> offset = std::vector<std::size_t>()
) {
  try {
...
struct scalar_type<std::pair<T *, std::vector<std::size_t> > > {
  typedef typename scalar_type<typename alps::detail::remove_cvr<T>::type>::type
      type;
};
...
struct get_extent<std::pair<T *, std::vector<std::size_t> > > {
  static std::vector<std::size_t> apply(
    ...
    std::vector<std::size_t> size(
        value.second.size() ? get_extent(*value.first)
                            : std::vector<std::size_t>()
    );
    if (!is_continuous<T>::value && value.second.size()) {
      for (std::size_t i = 1;
           i < std::accumulate(
                   value.second.begin(), value.second.end(), std::size_t(1),
                   std::multiplies<std::size_t>()
               );
           ++i)
        if (!std::equal(
                size.begin(), size.end(), get_extent(value.first[i]).begin()
            ))
...
struct set_extent<std::pair<T *, std::vector<std::size_t> > > {
  static void apply(
    ...
    if (value.second.size() > size.size() ||
        !std::equal(value.second.begin(), value.second.end(), size.begin()))
      throw archive_error("invalid data size" + ALPS_STACKTRACE);
    if (!is_continuous<T>::value && value.second.size() &&
        value.second.size() < size.size())
      for (std::size_t i = 0;
           i < std::accumulate(
                   value.second.begin(), value.second.end(), std::size_t(1),
                   std::multiplies<std::size_t>()
           ...
           ++i)
        set_extent(
            value.first[i], std::vector<std::size_t>(
                                size.begin() + value.second.size(), size.end()
                            )
        );
  }
};
...
struct is_vectorizable<std::pair<T *, std::vector<std::size_t> > > {
  static bool apply(std::pair<T *, std::vector<std::size_t> > const &value) {
    ...
    for (std::size_t i = 1;
         ...
         ++i)
      if (!is_vectorizable(value.first[i]) ||
          !std::equal(
              size.begin(), size.end(), get_extent(value.first[i]).begin()
          ))
        return false;
...
struct get_pointer<std::pair<T *, std::vector<std::size_t> > > {
  static typename alps::hdf5::scalar_type<
      std::pair<T *, std::vector<std::size_t> > >::type *
  apply(std::pair<T *, std::vector<std::size_t> > &value) {
...
struct get_pointer<std::pair<T *, std::vector<std::size_t> > const> {
  static typename alps::hdf5::scalar_type<
      std::pair<T *, std::vector<std::size_t> > >::type const *
...
void save(
  ...
  if (is_continuous<T>::value) {
    ...
    std::copy(
        value.second.begin(), value.second.end(), std::back_inserter(size)
    );
    ...
    for (std::size_t i = 0;
      ...
      local_offset.push_back(
          i / std::accumulate(
                  value.second.begin() + 1, value.second.end(), std::size_t(1),
                  std::multiplies<std::size_t>()
              )
      );
      for (std::vector<std::size_t>::const_iterator it =
               value.second.begin() + 1;
           it != value.second.end(); ++it)
        local_offset.push_back(
            (i % std::accumulate(
                     it, value.second.end(), std::size_t(1),
                     std::multiplies<std::size_t>()
                 )) /
            std::accumulate(
                it + 1, value.second.end(), std::size_t(1),
                std::multiplies<std::size_t>()
            )
        );
    ...
    if (path.find_last_of('@') != std::string::npos)
      throw archive_error(
          "attributes needs to be vectorizable: " + path + ALPS_STACKTRACE
      );
    ...
    do {
      ...
      std::string location = "";
      for (std::vector<std::size_t>::const_iterator it = offset.begin();
           it != offset.end(); ++it) {
        location += "/" + cast<std::string>(*it);
        pos += *it * std::accumulate(
                         value.second.begin() + (it - offset.begin()) + 1,
                         value.second.end(), std::size_t(1),
                         std::multiplies<std::size_t>()
                     );
      }
...
void load(
  ...
  if (ar.is_group(path)) {
    ...
    set_extent(
        value, std::vector<std::size_t>(size.begin() + chunk.size(), size.end())
    );
    if (is_continuous<T>::value) {
      ...
      for (std::size_t i = 0;
        ...
        local_offset.push_back(
            i / std::accumulate(
                    value.second.begin() + 1, value.second.end(),
                    std::size_t(1), std::multiplies<std::size_t>()
                )
        );
        for (std::vector<std::size_t>::iterator it = value.second.begin() + 1;
             it != value.second.end(); ++it)
          local_offset.push_back(
              (i % std::accumulate(
                       it, value.second.end(), std::size_t(1),
