...
typename lazy_enable_if<
    ...
    detail::make_valarray<functional::divides<Left, Right> > >::type
...
typename lazy_enable_if<
    mpl::and_<is_scalar<Right>, mpl::not_<is_same<Left, Right> > >,
    detail::make_valarray<functional::multiplies<Left, Right> > >::type
operator*(std::valarray<Left> const &left, Right const &right) {
...
typename lazy_disable_if<
    is_same<Left, Right>,
    detail::make_valarray<functional::plus<Left, Right> > >::type
...
#define BOOST_NUMERIC_FUNCTIONAL_DEFINE_VALARRAY_BIN_OP(Name, Op)              \
  ...
  struct Name<Left, Right, std_valarray_tag, std_valarray_tag>                 \
      : std::binary_function<                                                  \
            Left, Right,                                                       \
            std::valarray<typename Name<                                       \
                typename Left::value_type,                                     \
                typename Right::value_type>::result_type> > {                  \
  ...
  struct Name<Left, Right, std_valarray_tag, void>                             \
      : std::binary_function<                                                  \
            Left, Right,                                                       \
            std::valarray<typename Name<                                       \
                typename Left::value_type, Right>::result_type> > {            \
    typedef typename Left::value_type left_value_type;                         \
    typedef std::valarray<typename Name<left_value_type, Right>::result_type>  \
        result_type;                                                           \
    result_type operator()(Left &left, Right &right) const {                   \
  ...
  struct Name<Left, Right, void, std_valarray_tag>                             \
      : std::binary_function<                                                  \
            Left, Right,                                                       \
            std::valarray<typename Name<                                       \
                Left, typename Right::value_type>::result_type> > {            \
    typedef typename Right::value_type right_value_type;                       \
    typedef std::valarray<typename Name<Left, right_value_type>::result_type>  \
        result_type;                                                           \
    result_type operator()(Left &left, Right &right) const {                   \
...
struct as_min<T, std_valarray_tag>
  ...
  typename remove_const<T>::type operator()(T &arr) const {
    return 0 == arr.size() ? T() : T(numeric::as_min(arr[0]), arr.size());
  }
};
...
struct as_max<T, std_valarray_tag>
    : std::unary_function<T, typename remove_const<T>::type> {
  typename remove_const<T>::type operator()(T &arr) const {
    return 0 == arr.size() ? T() : T(numeric::as_max(arr[0]), arr.size());
  }
};
...
struct as_zero<T, std_valarray_tag>
    : std::unary_function<T, typename remove_const<T>::type> {
  typename remove_const<T>::type operator()(T &arr) const {
    return 0 == arr.size() ? T() : T(numeric::as_zero(arr[0]), arr.size());
  }
};
...
struct as_one<T, std_valarray_tag>
    : std::unary_function<T, typename remove_const<T>::type> {
  typename remove_const<T>::type operator()(T &arr) const {
    return 0 == arr.size() ? T() : T(numeric::as_one(arr[0]), arr.size());
  }
};
