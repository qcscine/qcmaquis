...
#define ALPS_IMPLEMENT_FUNCTION(FUN)                                   \
  ...
  multi_array<T, D, Allocator> FUN(multi_array<T, D, Allocator> arg) { \
    using std::FUN;                                                    \
    std::transform(                                                    \
        arg.data(), arg.data() + arg.num_elements(), arg.data(),       \
        [](const auto& e) { return FUN(e); }                           \
    );                                                                 \
    return arg;                                                        \
  }
...
multi_array<T1, D, Allocator> pow(multi_array<T1, D, Allocator> a, T2 s) {
  std::transform(
      a.data(), a.data() + a.num_elements(), a.data(),
      [&s](const auto& e) { return std::pow(e, s); }
  );
  return a;
}
...
T sum(multi_array<T, D, Allocator>& a) {
  return std::accumulate(
      a.data(), a.data() + a.num_elements(), 0., std::plus<T>()
  );
}
