...
/* $Id: parameters.h 6523 2012-11-06 07:51:23Z gamperl $ */
...
///
/// the class acts like an associative array but at the same time remembers the
/// order in which elements were added
class ALPS_DECL Parameters {
 public:
  /// the key (parameter name) is a string
  typedef std::string key_type;
  /// the parameter value is a String Value, able to store any type in a text
  /// representation
  ...
  ///
  /// iteration goes in the order of insertion into the class, not
  /// alphabetically like in a std::map
  ...
  /// the type of container used internally to implment the associative array
  /// access
  ...
  /// replace '${FOO}' in each parameter with the the content of environment
  /// variable FOO
  void replace_envvar();
  ...
  const value_type& operator[](const key_type& k) const {
    if (!defined(k))
      boost::throw_exception(
          std::runtime_error("parameter " + k + " not defined")
      );
    return map_.find(k)->second->value();
  }
  ...
  /// \return if a parameter with the given name \a k exists, its value is
  /// returned, otherwise the default v
  ...
  /// \param allow_overwrite indicates whether existing parameters may be
  /// overwritten \throw a std::runtime_error if the parameter key is empty or
  /// if it exists already and \a allow_overwrite is false
  ...
  /// \param allow_overwrite indicates whether existing parameters may be
  /// overwritten \throw a std::runtime_error if the parameter key \a k is empty
  /// or if it exists already and \a allow_overwrite is false
  void push_back(
      const key_type& k, const value_type& v, bool allow_overwrite = false
  ) {
...
inline alps::ODump& operator<<(
    alps::ODump& od, const alps::Parameters& params
) {
...
inline alps::oxstream& operator<<(
    alps::oxstream& oxs, const alps::Parameters& parameters
) {
  oxs << alps::start_tag("PARAMETERS");
  alps::Parameters::const_iterator p_end = parameters.end();
  for (alps::Parameters::const_iterator p = parameters.begin(); p != p_end; ++p)
    oxs << *p;
