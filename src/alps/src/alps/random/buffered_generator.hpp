...
///
/// This class template is an abstract base class for runtime-polymorphic
/// generators. In order to mask the abstraction penalty of a virtual @c
/// operator(), the buffered_generator does not produce single numbers at each
/// call, but instead a large buffer is filled in a virtual function call, and
/// then numbers from this buffer used when @c operator() is called.
...
class buffered_generator {
 public:
  ...
  ///
  /// Constructs a @c buffered_generator with a buffer of the size given as
  /// argument.

  buffered_generator(
      std::size_t buffer_size = ALPS_BUFFERED_GENERATOR_BUFFER_SIZE
  )
      : buffer_(buffer_size), ptr_(buffer_.end()) {
 ...
 private:
  ...
  ///
  /// A pure virtual function to fill the buffer. It needs to be implemented by
  /// the concrete derived classes
...
/// @tparam ResultType the type of values generated
///
/// This class template is a concrete derived class template for
/// runtime-polymorphic generators. It uses the generator provided as template
/// argument to fill the buffer of the @ref buffered_generator base class. If
/// the @c Generator is a reference type, a reference to the generator passed to
/// the constructor is used. Otherwise a copy of the generator is used.
...
class basic_buffered_generator : public buffered_generator<ResultType> {
 ...
 public:
  ...
  /// constructs a buffer of the size given as argument, and a default-generated
  /// Generator.
  /// @param buffer_size the size of the buffer
  ...
  /// @param buffer_size the size of the buffer
  ///
  /// If a reference type is specifed as \c Generator type, a reference to the
  /// generator
  ...
  basic_buffered_generator(
      ...
      : base_type(buffer_size), generator_(gen) {}
 ...
 private:
  ...
  void fill_buffer(buffer_type& buffer) {
    for (typename buffer_type::iterator it = buffer.begin(); it != buffer.end();
         ++it)
      *it = generator_();
  }
