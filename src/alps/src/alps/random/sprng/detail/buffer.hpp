...
///
/// The SPRNG libraries can pack and unpack random number generators into and
/// from an array of char. This class is wrapper around these packing and
/// unpacking functions of the SPRNG libraries and has convenience functions to
/// write and read the buffer contents as a sequence of integers
...
class buffer {
 public:
  ...
  buffer(int* genptr, int (*pack_fun)(int*, char**)) { pack(genptr, pack_fun); }
  ...
  void pack(int* genptr, int (*pack_fun)(int*, char**)) {
    ...
    if (buf == 0) boost::throw_exception(std::bad_alloc());
  ...
  /// @returns a newly created SPRNG generator. The return value is 0 if the
  /// creation of the generator failed
  int* unpack(int* (*unpack_fun)(const char*)) {
    BOOST_ASSERT(!buffer_.empty());
    int* genptr = (*unpack_fun)(&buffer_[0]);
    if (genptr == 0) boost::throw_exception(std::bad_alloc());
    return genptr;
  }
  ...
  void write(OutputStream& os) const {
    os << buffer_.size();
    for (buffer_type::const_iterator it = buffer_.begin(); it != buffer_.end();
         ++it)
  ...
  void read(InputStream& is) {
    ...
    is >> len;
    buffer_.resize(len);
    for (buffer_type::iterator it = buffer_.begin(); it != buffer_.end();
         ++it) {
      int x;
      is >> x;
      *it = x;
  ...
  bool operator==(const buffer& b) { return b.buffer_ == buffer_; }
