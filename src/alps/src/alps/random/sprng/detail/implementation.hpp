...
#define ALPS_SPRNG_CALL(FUN) \
  BOOST_PP_CAT(ALPS_SPRNG_GENERATOR, BOOST_PP_CAT(_, FUN))
...
class ALPS_SPRNG_GENERATOR {
 public:
  ...
  ALPS_SPRNG_GENERATOR(ALPS_SPRNG_GENERATOR const& rhs) {
    random::sprng::detail::buffer buf(
        rhs.sprng_ptr, &ALPS_SPRNG_CALL(pack_rng)
    );
    sprng_ptr = buf.unpack(&ALPS_SPRNG_CALL(unpack_rng));
  }

  ~ALPS_SPRNG_GENERATOR() { free(); }
  ...
  /// @param stream The number of this streamuence. Needs to be less than the
  /// num_stream parameter.
  /// @param num_stream The total number of random number streams to be created.
  /// Needs to be less than max_streams
  /// @param s the common seed for all random number streams
  /// @param param optional parametrization of the generator. Needs to be less
  /// than max_param.
  ///
  /// The SPRNG library guarantees that if the global_seed and param arguments
  /// are the same, the random number streams specified by stream (0 <= stream <
  /// num_stream) are independent and non-overlapping. Changing eiher
  /// global_seed or the param will lead to different random number streams

  ALPS_RANDOM_PARALLEL_SEED_PARAMS(ALPS_SPRNG_GENERATOR, sprng_seed_params,
                                   : sprng_ptr(0)) {
    ...
    sprng_ptr =
        ALPS_SPRNG_CALL(init_rng)(sprng_type, stream, num_stream, s, param);
    if (sprng_ptr == 0)
      boost::throw_exception(
          std::runtime_error("Failed initializing SPRNG generator")
      );
  }
  ...
  result_type operator()() { return ALPS_SPRNG_CALL(get_rn_dbl)(sprng_ptr); }
  ...
  friend bool operator==(
      const ALPS_SPRNG_GENERATOR& x, const ALPS_SPRNG_GENERATOR& y
  ) {
  ...
  friend std::basic_ostream<CharT, Traits>& operator<<(
      std::basic_ostream<CharT, Traits>& os, const ALPS_SPRNG_GENERATOR& lcg
  ) {
  ...
  friend std::basic_istream<CharT, Traits>& operator>>(
      std::basic_istream<CharT, Traits>& is, ALPS_SPRNG_GENERATOR& lcg
  ) {
    detail::buffer buf;
    buf.read(is);
 ...
 private:
  ...
  void free() {
    if (sprng_ptr) ALPS_SPRNG_CALL(free_rng(sprng_ptr));
    sprng_ptr = 0;
  }
