...
class lcg64 {
 public:
  ...
  static const result_type max_value =
      boost::integer_traits<uint64_t>::const_max;
  /// @brief The maximum number of streams is 146138719, the number of primes
  /// among all 64-bit unsigned integer values
  ...
  */
  ...
  ALPS_RANDOM_PARALLEL_SEED(lcg64) {
    ...
    _x = (uint64_t(0x2bc6ffffU) << 32 | 0x8cfe166dU) ^
         ((uint64_t(s) << 33) | stream);
    // and advance it a bit
    for (uint64_t i = 0; i < 127 * stream; i++) operator()();
  }
  ...
  /// integer generated from a default-seeded generator
  static bool validation(uint64_t x) { return x == val; }

#ifdef BOOST_NO_OPERATORS_IN_NAMESPACE

  // Use a member function; Streamable concept not supported.
  bool operator==(const lcg64& rhs) const { return _x == rhs._x && c == rhs.c; }
  bool operator!=(const lcg64& rhs) const { return !(*this == rhs); }

#else
  friend bool operator==(const lcg64& x, const lcg64& y) {
  ...
  friend std::basic_ostream<CharT, Traits>& operator<<(
      std::basic_ostream<CharT, Traits>& os, const lcg64& lcg
  ) {
  ...
  friend std::basic_istream<CharT, Traits>& operator>>(
      std::basic_istream<CharT, Traits>& is, lcg64& lcg
  ) {
...
std::basic_ostream<CharT, Traits>& operator<<(
    std::basic_ostream<CharT, Traits>& os, const lcg64<a, val>& lcg
) {
  return os << lcg._x << " " << lcg.c;
}
...
std::basic_istream<CharT, Traits>& operator>>(
    std::basic_istream<CharT, Traits>& is, lcg64<a, val>& lcg
) {
  return is >> lcg._x >> lcg.c;
}
#endif
...
typedef random::parallel::lcg64<
    uint64_t(0x87b0b0fdU) | uint64_t(0x27bb2ee6U) << 32,
    uint64_t(481823773Ul) + (uint64_t(3380683238Ul) << 32)>
    lcg64a;

/// Instantiation of lcg64 with a  good choice of multiplier
typedef random::parallel::lcg64<
    uint64_t(0xe78b6955U) | uint64_t(0x2c6fe96eU) << 32,
    uint64_t(3274024413Ul) + (uint64_t(3475904802Ul) << 32)>
    lcg64b;

/// Instantiation of lcg64 with a  good choice of multiplier
typedef random::parallel::lcg64<
    uint64_t(0x31a53f85U) | uint64_t(0x369dea0fU) << 32,
    uint64_t(950651229Ul) + (uint64_t(3996309981Ul) << 32)>
    lcg64c;
