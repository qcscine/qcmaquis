...
///
/// This header provides default implementations of the parallel seeding
/// functions.
...
/// seed by asssuming a named parameter seeding interface. It is implemented as
...
/// provides a default implementation of the parallel seeding from a pair
/// iterators by asssuming a named parameter seeding interface. It is
/// implemented as
/// @c prng.seed(first, last, global_seed=s, stream_number=num,
/// total_streams=total);
///
/// Requirements: 0 <= @c num < @c total
///
...
void seed(
    PRNG& prng, unsigned int num, unsigned int total, Iterator& first,
    Iterator const& last
) {
  if (first == last)
