...
template <
    class UIntType, int w, int n, int m, int r, UIntType a, int u, int s,
    UIntType b, int t, UIntType c, int l, UIntType val>
void seed(
    boost::random::mersenne_twister<
        UIntType, w, n, m, r, a, u, s, b, t, c, l, val>& prng,
...
template <
    class UIntType, int w, int n, int m, int r, UIntType a, int u, int s,
    UIntType b, int t, UIntType c, int l, UIntType val, class SeedType>
void seed(
    boost::random::mersenne_twister<
        UIntType, w, n, m, r, a, u, s, b, t, c, l, val>& prng,
    unsigned int num, unsigned int total, SeedType const& the_seed
) {
...
void seed(
  ...
  // seeds the seeder, which in turn gives the seedvalue for the
  // mersenne_twister-rng
  typedef boost::uniform_int<unsigned int> dist_t;
  boost::variate_generator<lcg64a&, dist_t> rng(
      engine, dist_t(0u, std::numeric_limits<unsigned int>::max())
  );
  ...
  for (int i = 0; i < 2 * n; i++) buffer.push_back(rng());
...
template <
    class UIntType, int w, int n, int m, int r, UIntType a, int u, int s,
    UIntType b, int t, UIntType c, int l, UIntType val, class Iterator>
void seed(
    boost::random::mersenne_twister<
        UIntType, w, n, m, r, a, u, s, b, t, c, l, val>& prng,
    unsigned int num, unsigned int total, Iterator& first, Iterator const& last
) {
  first += 2 * n * num;
  if (first > last + 2 * n)
