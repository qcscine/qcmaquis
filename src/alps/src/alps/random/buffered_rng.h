...
/* $Id: buffered_rng.h 5705 2011-08-15 05:05:03Z troyer $ */

/// \file buffered_rng.h
/// \brief contains an efficient buffered implementation of a
/// runtime-polymorphic random number generator
...
class buffered_rng_base {
 public:
  ...
  buffered_rng_base(std::size_t b = 10240) : buf_(b), ptr_(buf_.end()) {}

  buffered_rng_base(const buffered_rng_base& gen) {
    buf_ = gen.buf_;
    ptr_ =
        buf_.begin() + (gen.ptr_ - const_cast<buffer_type&>(gen.buf_).begin());
  }
...
template <class RNG>
class buffered_rng : public buffered_rng_base {
 private:
  BOOST_STATIC_ASSERT(
      (::boost::is_same<typename RNG::result_type, uint32_t>::value)
  );
 ...
 public:
  ...
  result_type min BOOST_PREVENT_MACRO_SUBSTITUTION() const {
    return rng_.min BOOST_PREVENT_MACRO_SUBSTITUTION();
  }
  result_type max BOOST_PREVENT_MACRO_SUBSTITUTION() const {
    return rng_.max BOOST_PREVENT_MACRO_SUBSTITUTION();
  }
