...
/* $Id: xmlstream.h 5001 2010-10-30 20:55:57Z troyer $ */
...
struct header_t {
  header_t(const std::string& enc = "") : version("1.0"), encoding(enc) {}
  std::string version;
  std::string encoding;
};
...
class ALPS_DECL oxstream {
 public:
  ...
  oxstream& operator<<(const std::string& t) { return text_str(t); }
  oxstream& operator<<(const char t) { return text_str(std::string(1, t)); }
  oxstream& operator<<(const char* t) { return text_str(t); }
...
inline detail::attribute_t xml_namespace(
