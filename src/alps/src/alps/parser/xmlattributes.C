...
/* $Id: xmlattributes.C 3125 2009-02-27 08:30:36Z wistaria $ */
...
struct append_attr {
  append_attr(
      const XMLAttributes& attr, const std::string& name,
...
struct attr_parser : public bs::grammar<attr_parser> {
  attr_parser(XMLAttributes& a) : attr(a) {}

  template <typename ScannerT>
  struct definition {
    bs::rule<ScannerT> name_p, value_p, attribute_p, attributes_p;
    definition(const attr_parser& self) {
      ...
      name_p =
          ...
          )[assign_string(self.name)];
      value_p = ch_p('\"') >>
                (*(anychar_p - ch_p('\"')))[assign_string(self.value)] >>
                ch_p('\"');
      attributes_p =
          *(name_p >> ch_p('=') >> value_p
          )[append_attr(self.attr, self.name, self.value)];
    }
    const bs::rule<ScannerT>& start() const { return attributes_p; }
  };
...
XMLAttributes::XMLAttributes(const std::string& str) {
  if (!bs::parse(str.c_str(), attr_parser(*this), bs::space_p).full)
...
void XMLAttributes::push_back(const XMLAttribute& attr) {
  if (defined(attr.key()))
    boost::throw_exception(
        std::runtime_error("duplicated attribute " + attr.key())
    );
