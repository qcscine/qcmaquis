...
/* $Id: xmlstream.C 4843 2010-09-10 15:29:07Z gamperl $ */
...
oxstream& oxstream::operator<<(const detail::header_t& c) {
  if (context_ == Comment || context_ == Cdata)
    boost::throw_exception(
        std::runtime_error("header not allowed in comment or CDATA section")
    );

  *this << detail::pi_t("xml") << detail::attribute_t("version", c.version);
  if (!c.encoding.empty()) *this << detail::attribute_t("encoding", c.encoding);
  return *this;
}
...
oxstream& oxstream::operator<<(const detail::start_tag_t& c) {
  if (context_ == Comment || context_ == Cdata)
    boost::throw_exception(
        std::runtime_error("start tag not allowed in comment or CDATA section")
    );
...
oxstream& oxstream::operator<<(const detail::end_tag_t& c) {
  if (!c.name.empty() && c.name != stack_.top().first)
    boost::throw_exception(std::runtime_error(
        "inconsistent end tag name: " + c.name + " does not agree with " +
        stack_.top().first
    ));
  output(true);
  return *this;
...
oxstream& oxstream::no_linebreak() {
  if (context_ != StartTag)
    boost::throw_exception(
        std::runtime_error("no_linebreak is allowed only in starttag")
    );
  linebreak_ = false;
  return *this;
}
...
void oxstream::output(bool close) {
  if (context_ == StartTag || context_ == PI) {
    ...
    for (XMLAttributes::const_iterator a = attr_.begin(); a != attr_.end(); ++a)
...
std::string convert(const std::string& str) {
  ...
  std::regex_replace(
      oi, str.begin(), str.end(), std::regex("(&)|(')|(>)|(<)|(\")"),
      "(?1&amp;)(?2&apos;)(?3&gt;)(?4&lt;)(?5&quot;)",
