...
typename boost::enable_if<
    ...
    slices(T const& x) {
  return std::pair<
      typename slice_index<T>::type, typename slice_index<T>::type>(
      0, x.size()
  );
}
...
typename boost::disable_if<
    ...
    slices(T const&) {
  return std::pair<
      typename slice_index<T>::type, typename slice_index<T>::type>(0, 1);
}

template <class ValueType>
typename boost::enable_if<is_sequence<ValueType>, std::string>::type slice_name(
    ValueType const&, unsigned i
) {
...
template <class ValueType>
typename boost::enable_if<
    is_sequence<ValueType>, typename element_type<ValueType>::type>::type
