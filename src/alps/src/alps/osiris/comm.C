...
/* $Id: comm.C 3867 2010-02-05 14:44:04Z gamperl $ */
...
bool alps::is_master() {
  ...
  int num = 0;
  if (mpi_initialized) MPI_Comm_rank(MPI_COMM_WORLD, &num);
  return (num == 0);
...
alps::ProcessList alps::all_processes() {
  ...
  int num = 1;
  if (mpi_initialized) MPI_Comm_size(MPI_COMM_WORLD, &num);
  for (int i = 0; i < num; i++) p.push_back(Process(i));
...
alps::Process alps::master_process() { return Process(0); }
