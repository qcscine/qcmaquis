...
/* $Id: dump.C 4205 2010-04-20 19:25:49Z parcollet $ */
...
#define ALPS_DUMP_DO_TYPE(A, B) \
  void ODump::write_simple(A x) { write_simple(static_cast<B>(x)); }
...
void ODump::write_string(const std::string& s) {
  ...
  if (s.size()) write_string(s.size() + 1, s.c_str());
}
...
#define ALPS_DUMP_DO_TYPE(A, B) \
  void IDump::read_simple(A& x) { x = get<B>(); }
