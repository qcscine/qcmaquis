...
/* $Id: dump.h 5760 2011-09-18 11:34:15Z troyer $ */
...
/// INTERNAL ONLY
#define ALPS_DUMP_DO_TYPE(T) virtual void write_simple(T x);
...
#undef ALPS_DUMP_DO_TYPE
  ...
  //
  // template<class T>
  // ODump& operator<<(const std::complex<T>& x) { write_complex(x); return
  // *this; }

  template <class T>
  ODump& store(const T& x) {
    x.save(*this);
    return *this;
  }

  template <class T>
  ODump& store(const std::complex<T>& x) {
    write_complex(x);
    return *this;
  }
...
/// INTERNAL ONLY
#define ALPS_DUMP_DO_TYPE(T) virtual void read_simple(T& x);
