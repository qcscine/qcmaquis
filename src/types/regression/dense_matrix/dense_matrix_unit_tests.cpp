...
// {
//     // -maquis::types::dense_matrix<T> * std::vector<int>
//     
...
//     
//     std::vector<typename maquis::types::MultiplyReturnType<typename TPair::first_type,std::vector<typename TPair::first_type>,typename TPair::second_type, std::vector<typename TPair::second_type> >::value_type> result(a*v);
//     BOOST_CHECK_EQUAL(result.size(),num_rows(a));
//     BOOST_CHECK_EQUAL(a,a_);
//     BOOST_CHECK_EQUAL(v,v_);
//     for(unsigned int i=0; i<num_rows(a); ++i)
