...
class local : public measurement<Matrix, SymmGroup> {
 ...
 public:
  local(
      ...
      op_vec const& fillings_, std::vector<bond_element> const& terms
  )
      : base(name_),
        ...
        is_bond(true),
        mpo_terms(terms) {
    this->cast_to_real = all_true(
        mpo_terms.begin(), mpo_terms.end(),
  ...
  local(
      std::string const& name_, Lattice lat, op_vec const& identities_,
      op_vec const& fillings_, op_vec const& op
  )
      : base(name_),
        lattice(std::move(lat)),
        identities(identities_),
        ...
        is_bond(false),
        site_term(op),
        mpo_terms(std::vector<bond_element>(
            1, bond_element(1, std::make_pair(op, false))
        )) {
        ...
        if (site_term[type].n_blocks() > 0) {
          ...
          MPSTensor<Matrix, SymmGroup> vec2 =
              contraction::Engine<Matrix, Matrix, SymmGroup>::site_hamil2(
                  mps[p], rmps.get().left(p), rmps.get().right(p), temp
              );

          typename MPS<Matrix, SymmGroup>::scalar_type res =
              mps[p].scalar_overlap(vec2);
 ...
 protected:
  measurement<Matrix, SymmGroup>* do_clone() const { return new local(*this); }
  ...
  void evaluate_with_mpo(MPS<Matrix, SymmGroup> const& mps) {
    ...
    /// collect results from all mpo terms, i.e. all requested combinations of
    /// operators.
    for (const auto& it : mpo_terms) {
      using mpo_map = std::map<std::string, MPO<Matrix, SymmGroup>>;
      mpo_map mpos = meas_prepare::local<Matrix, SymmGroup>(
          lattice, identities, fillings, it
      );
      ...
      for (const auto& mit : mpos) {
        ...
        if (match == res.end()) {
          std::tie(match, std::ignore) =
              res.insert(std::make_pair(mit.first, 0.));
        }

        if (!this->is_super_meas) {
          match->second += (this->cast_to_real)
                               ? maquis::real(expval(mps, mit.second))
                               : expval(mps, mit.second);
        } else {
          MPS<Matrix, SymmGroup> super_mpo =
              mpo_to_smps(mit.second, this->phys_psi);
          // static_cast needed for icpc 12.x
          using overlap_func = typename MPS<Matrix, SymmGroup>::
              scalar_type (*)(const MPS<Matrix, SymmGroup>&, const MPS<Matrix, SymmGroup>&);
          typename MPS<Matrix, SymmGroup>::scalar_type val =
              ::overlap(super_mpo, mps);
          match->second += val / nn;
        }
      }
    }
