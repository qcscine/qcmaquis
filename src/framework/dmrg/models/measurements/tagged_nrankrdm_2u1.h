      ...
      if (std::filesystem::exists(bra_ckp)) {
...
#if (defined(HAVE_SU2U1) || defined(HAVE_SU2U1PG))
        {
          ...
          int N = SU2Symm::particleNumber(
              su2_mps[su2_mps.size() - 1].col_dim()[0].first
          );
          int TwoS =
              SU2Symm::spin(su2_mps[su2_mps.size() - 1].col_dim()[0].first);
          int Nup = (N + TwoS) / 2;
          int Ndown = (N - TwoS) / 2;
          BaseParameters parms_tmp =
              chem::detail::set_2u1_parameters(su2_mps.size(), Nup, Ndown);
...
#else
          throw std::runtime_error(
              "SU2U1 support has not been compiled, cannot transform the MPS "
              "from SU2U1 group."
          );
#endif
        else
      ...
      for (pos_t p2 = bra_neq_ket ? 0 : p1; p2 < lattice.size(); ++p2) {
        ...
        for (std::size_t synop = 0; synop < operator_terms.size(); ++synop) {
          ...
          if (measurements_details::checkpg<SymmGroup>()(
            ...
            value += operator_terms[synop].second *
...
#pragma omp critical
  ...
  template <int N>
  void measure_nrdm(
      const MPS<Matrix, SymmGroup>& dummy_bra_mps,
      const MPS<Matrix, SymmGroup>& ket_mps
  ) {
    // Test if a separate bra state has been specified bool bra_neq_ket =
    // (dummy_bra_mps.length() > 0);
    bool bra_neq_ket = (dummy_bra_mps.length() > 0);
    // MPS<Matrix, SymmGroup> const & bra_mps = (bra_neq_ket) ? dummy_bra_mps :
    // ket_mps;
    MPS<Matrix, SymmGroup> bra_mps = (bra_neq_ket) ? dummy_bra_mps : ket_mps;
    MPS<Matrix, SymmGroup> ket_mps_local = ket_mps;
    // Obtain the total number of RDM elements and the list of all indices
    // (eventually for a given slice)
    auto indices = measurements_details::iterate_nrdm<N>(
        lattice.size(), bra_neq_ket, positions_first
    );
    maquis::cout << "Number of total " << N
                 << "-RDM elements measured: " << indices.size() << std::endl;
...
#pragma omp parallel for schedule(dynamic) firstprivate(bra_mps, ket_mps_local)
    ...
    for (int i = 0; i < indices.size(); i++) {
      ...
      // MPS<Matrix, SymmGroup> ket_mps_local = ket_mps; // enable if you get
      // pairing issues Make a local copy of tag_handler since it can be
      // modified by the MPO creator
      std::shared_ptr<TagHandler<Matrix, SymmGroup> > tag_handler_local(
          new TagHandler<Matrix, SymmGroup>(*tag_handler)
      );
      // Setup MPO and calculate the expectation value for a given indices set
      this->vector_results[i] =
          nrdm_expval(N, bra_mps, ket_mps_local, positions, tag_handler_local);
    }  // iterator loop
  }
