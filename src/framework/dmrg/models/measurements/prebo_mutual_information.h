...
class PreBOMutualInformation : public measurement<Matrix, NU1_template<N>> {
 public:
  ...
  using tag_type =
      typename generate_mpo::OperatorTagTerm<Matrix, NU1_template<N>>::tag_type;
 ...
 public:
   ...
   * @param identities vector with the tags associated with the identity
   * operators
   * @param fillings vector with the tags associated with the filling operators
   * @param ptr_term_generator_ tag generator object (required by the MPO
   * management in the NU1 class)
   */
  PreBOMutualInformation(
      BaseParameters& parms, Lattice const& lat_, const tag_vec& identities,
      const tag_vec& fillings,
      ...
      : base("mutinf"),
        ...
        fillings(fillings),
        ptr_term_generator(ptr_term_generator_),
        verbose_(verbose) {}
   ...
   */
  void evaluate(
      MPS<Matrix, NU1> const& ket_mps,
      boost::optional<reduced_mps<Matrix, NU1> const&> rmps = boost::none
  ) {
    ...
    auto inv_order =
        lat.template get_prop<std::vector<Lattice::pos_t>>("inv_order");
      ...
      for (size_t j = 0; j < vec_orbitals.size(); j++) {
        for (size_t mu = 0; mu < vec_orbitals.at(i); mu++) {
          for (size_t nu = 0; nu < vec_orbitals.at(j); nu++) {
            ...
            double trace = 0;
            auto site1 =
                NBodyTerm::retrieve_abs_index(mu, i, vec_orbitals, inv_order);
            auto site2 =
                NBodyTerm::retrieve_abs_index(nu, j, vec_orbitals, inv_order);
            if (site2 > site1) {
              site1 =
                  NBodyTerm::retrieve_abs_index(nu, j, vec_orbitals, inv_order);
              site2 =
            ...
            if (i == j && isFermion[i] && mu == nu) {
              ...
              for (size_t iter = 0; iter < 4; iter++) {
                auto terms = ptr_term_generator->generate_termsTransitionOp(
                    i, site1, vals.at(iter)
                );
                generate_mpo::TaggedMPOMaker<Matrix, NU1> mpom(
                    lat, identities, fillings,
                    ptr_term_generator->getTagHandler(), terms
                );
              ...
              for (auto const w : OneOrbRDM) {
                if (!std::isnan(w)) trace += w;
                if (w > 1e-15) entropy += w * std::log(w);
              }
              // Finalize:
            ...
            // The orbital entropy is of course symmetric with permuting mu and
            // nu.
              ...
              //     6/1
              {
                ...
                for (size_t it = 0; it < evals.size(); it++) {
                  if (!std::isnan(evals(it))) trace += evals(it);
                  if (evals(it) > 1e-15)
              ...
              for (size_t iter1 = 0; iter1 < 4; iter1++)
                for (size_t iter2 = 0; iter2 < 4; iter2++) {
                  auto terms = ptr_term_generator->generate_termsTransitionOp(
                      i, j, site1, site2, vals.at(iter1), vals.at(iter2)
                  );
                  generate_mpo::TaggedMPOMaker<Matrix, NU1> mpom(
                      lat, identities, fillings,
                      ptr_term_generator->getTagHandler(), terms
            ...
            if (verbose_) {
              std::cout << std::setw(10) << i << "-" << mu << std::setw(10) << j
                        << "-" << nu << std::setw(10) << trace << std::setw(20)
                        << entropyTable(site1, site2) << "\n";
            }
          }
        }
      }
    ...
    for (int row = 0; row < L; row++) {
      for (int col = 0; col <= row; col++) {
        // Single-orbital entropies on diagonal amd mutual information on
        // off-diagonal
        if (row == col) {
          dct.push_back(entropyTable(row, row));
        } else {
          dct.push_back(
              0.5 * (entropyTable(row, row) + entropyTable(col, col) -
                     entropyTable(row, col))
          );
        }
    ...
    std::vector<std::string> lbt = label_strings(num_labels);
    {
      ...
      std::copy(
          dct.begin(), dct.end(), std::back_inserter(this->vector_results)
      );
      ...
      std::copy(
          num_labels.begin(), num_labels.end(),
