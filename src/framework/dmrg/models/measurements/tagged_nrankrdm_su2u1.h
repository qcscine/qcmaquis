      ...
      for (pos_t p2 = (bra_neq_ket ? 0 : p1); p2 < lattice.size(); ++p2) {
        ...
        if (p1 != p2) {
          ...
          terms.push_back(TermMakerSU2<Matrix, SymmGroup>::positional_two_term(
              true, op_collection.ident.no_couple, std::sqrt(2.), p1, p2,
              op_collection.create.couple_down,
              op_collection.create.fill_couple_up,
              op_collection.destroy.couple_down,
              op_collection.destroy.fill_couple_up, lattice
          ));
        } else {
          term_descriptor term;
          term.coeff = 1.;
          term.push_back(std::make_pair(
              p1, op_collection.count.no_couple[lattice.get_prop<
        ...
        generate_mpo::TaggedMPOMaker<Matrix, SymmGroup> mpo_m(
            lattice, op_collection.ident.no_couple,
            op_collection.ident_full.no_couple, op_collection.fill.no_couple,
            tag_handler_local, terms
        );
...
#pragma omp critical
  ...
  void measure_2rdm(
    ...
    // get all the labels ahead of the measurement and initialise the result
    // arrays with the correct size
    auto indices =
        measurements_details::iterate_nrdm<2>(lattice.size(), bra_neq_ket);
...
#pragma omp parallel for schedule(dynamic) firstprivate(ket_mps_local, bra_mps)
    ...
    for (int i = 0; i < indices.size(); i++) {
      ...
      std::vector<term_descriptor> terms =
          SpinSumSU2<Matrix, SymmGroup>::V_term(
              1., positions[0], positions[1], positions[2], positions[3],
              op_collection, lattice
          );
      // save labels
