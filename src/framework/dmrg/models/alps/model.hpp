...
inline alps::graph_helper<> const& get_graph(Lattice const& lat_) {
  alps_lattice const* alattice =
      static_cast<alps_lattice const*>(lat_.impl().get());
  return alattice->alps_graph();
}
}  // namespace detail

template <class I>
bool safe_is_fermionic(
    alps::SiteBasisDescriptor<I> const& b, alps::SiteOperator const& op
) {
  std::set<std::string> operator_names = op.operator_names();
  return std::count_if(
      operator_names.begin(), operator_names.end(),
      [&](const string& name) {
...
class ALPSModel : public model_impl<Matrix, SymmGroup> {
 ...
 public:
  ...
  ALPSModel(Lattice const& lattice_, const alps::Parameters& parms_)
    ...
    for (int type = 0; type <= alps::maximum_vertex_type(lattice.graph());
         ++type) {
    ...
    if (parms.defined("CONSERVED_QUANTUMNUMBERS")) {
      ...
      int n = 0;
      for (tokenizer::iterator it = qn_tokens.begin(); it != qn_tokens.end();
           it++) {
        if (parms.defined(*it + "_total")) {
          if (all_qn.find(*it) != all_qn.end())
            ...
            throw std::runtime_error(
                "quantumnumber " + (*it) + " not defined in the model."
            );
        }
      }
    }
    ...
    for (int type = 0; type <= alps::maximum_vertex_type(lattice.graph());
      ...
      symm_basis.push_back(symmetric_basis_descriptor<SymmGroup>(
          basis_descriptors[type], all_conserved_qn
      ));
      ...
      for (int i = 0; i < symm_basis[type].size(); ++i) {
        ...
        // maquis::cout << "Inserting " << c << " for " << site_bases[type][i]
        // << std::endl;
        ...
        int sign =
            (alps::is_fermionic(basis_descriptors[type], site_bases[type][i]))
                ? -1
                : 1;
      ...
      operators[opkey_type("ident", type)] =
          tag_handler->register_op(ident, tag_detail::bosonic);
      operators[opkey_type("fill", type)] =
          tag_handler->register_op(fill, tag_detail::bosonic);
    }

    /// site_term loop with cache to avoid recomputing matrices
    std::vector<std::vector<std::pair<value_type, tag_type> > > site_terms(
        num_vertex_types
    );
    for (graph_type::site_iterator it = lattice.sites().first;
         it != lattice.sites().second; ++it) {
      int p = lattice.vertex_index(*it);
      int type = lattice.site_type(*it);

      if (lattice.inhomogeneous_sites())
        alps::throw_if_xyz_defined(
            parms, *it
        );  // check whether x, y, or z is set
      alps::expression::ParameterEvaluator<value_type> coords(
          coordinate_as_parameter(lattice.graph(), *it)
      );

      if (site_terms[type].size() == 0) {
        typedef std::vector<
            std::tuple<alps::expression::Term<value_type>, alps::SiteOperator> >
            V;
        ...
        for (int n = 0; n < ops.size(); ++n) {
          ...
          opmap_const_iterator match =
              operators.find(opkey_type(simplify_name(op), type));
          if (match == operators.end())
            match = register_operator(op, type, parms);
          // site_terms[type].push_back(
          // std::make_pair(std::get<0>(ops[n]).value(), match->second)  );
      ...
      //            if (site_terms[type].size() > 0) {
      //                opmap_const_iterator match =
      //                operators.find(opkey_type("site_terms", type)); if
      //                (match == operators.end()) {
      //                    op_t op_matrix;
      //                    for (int n=0; n<site_terms[type].size(); ++n)
      //                        op_matrix += site_terms[type][n].first *
      //                        tag_handler->get_op(site_terms[type][n].second);
      //                    tag_type mytag = tag_handler->register_op(op_matrix,
      //                    tag_detail::bosonic); std::tie(match, std::ignore) =
      //                    operators.insert(
      //                    std::make_pair(opkey_type("site_terms", type),
      //                    mytag) );
      //                }
      //
    ...
    /// bond terms loop
    for (graph_type::bond_iterator it = lattice.bonds().first;
         it != lattice.bonds().second; ++it) {
      ...
      bool wrap_pbc =
          std::get(alps::boundary_crossing_t(), lattice.graph(), *it);

      BondOperator bondop = model.bond_term(type);

      typedef std::vector<std::tuple<
          alps::expression::Term<value_type>, alps::SiteOperator,
          alps::SiteOperator> >
          V;
      ...
      for (typename V::iterator tit = ops.begin(); tit != ops.end(); ++tit) {
        ...
        opmap_const_iterator match1 =
            operators.find(opkey_type(simplify_name(op1), type_s));
        if (match1 == operators.end())
          match1 = register_operator(op1, type_s, parms);
        opmap_const_iterator match2 =
            operators.find(opkey_type(simplify_name(op2), type_t));
...

        {
          ...
          if (with_sign && !wrap_pbc) {
            ...
            std::pair<tag_type, value_type> ptag = tag_handler->get_product_tag(
                operators[opkey_type("fill", type_s)], mytag
            );
            mytag = ptag.first;
            term.coeff *= ptag.second;
          }
          if (with_sign && wrap_pbc) term.coeff *= value_type(-1.);
        ...
        }
        {
          ...
          if (with_sign && wrap_pbc) {
            // Note inverse notation because of notation in operator.
            std::pair<tag_type, value_type> ptag = tag_handler->get_product_tag(
                operators[opkey_type("fill", type_t)], mytag
            );
            mytag = ptag.first;
            term.coeff *= ptag.second;
          }
 ...
 private:
  ...
  bool fermionic(
      alps::SiteBasisDescriptor<I> const& b1, SiteOperator const& op1,
      alps::SiteBasisDescriptor<I> const& b2, SiteOperator const& op2
  ) const {
    ...
    for (int i = 0; i < m.shape()[0]; ++i) {
      for (int j = 0; j < m.shape()[1]; ++j) {
        if (m[i][j] != 0.) {
          ...
          newm(symm_basis[type].coords(i), symm_basis[type].coords(j)) =
              m[i][j];
        }
      }
    }
  ...
  alps::SiteOperator make_site_term(
      std::string x, alps::Parameters const& parms
  ) const {
    if (x[x.size() - 1] != ')') x += "(i)";
  ...
  opmap_const_iterator register_operator(
      SiteOperator const& op, int type, alps::Parameters const& p
  ) const {
    ...
    tag_detail::operator_kind kind =
        safe_is_fermionic(b, op) ? tag_detail::fermionic : tag_detail::bosonic;
    tag_type mytag = tag_handler->register_op(convert_matrix(m, type), kind);

    opmap_const_iterator match;
    std::tie(match, std::ignore) = operators.insert(
        std::make_pair(opkey_type(simplify_name(op), type), mytag)
    );
    return match;
  }

  std::pair<meas_operators_type, short> operators_for_meas(
      std::string const& ops, bool repeat_one = false
  ) const {
    ...
    for (tokenizer::iterator it = corr_tokens.begin(); it != corr_tokens.end();
         it++) {
      if (model.has_bond_operator(*it)) {
        ...
        if (std::distance(bond_terms.begin(), bond_terms.end()) != 1)
          throw std::runtime_error(
    ...
    for (std::vector<std::string>::const_iterator it2 = opnames.begin();
         it2 != opnames.end(); ++it2) {
  ...
  void generate_terms() {
    ...
    // typedef typename boost::container::flat_map<expression_type,
    // value_type>::iterator coeff_iterator; for(coeff_iterator it =
    // expression_coeff.begin(); it != expression_coeff.end(); ++it)
    //     it->second = alps::partial_evaluate<value_type>(it->first,
    //     parms_with_defaults);

    typedef
        typename std::vector<expression_term>::const_iterator terms_iterator;
    for (terms_iterator it = expression_terms.begin();
         it != expression_terms.end(); ++it) {
      value_type val = 0.;
      if (lattice.inhomogeneous_sites() && it->size() == 1) {
        ...
        p << coordinate_as_parameter(
            lattice.graph(), lattice.site(it->position(0))
        );
...
// Initial states
template <class Matrix, class SymmGroup>
typename ALPSModel<Matrix, SymmGroup>::initializer_ptr
ALPSModel<Matrix, SymmGroup>::initializer(
    Lattice const& lat, BaseParameters& p_
) const {
  if (p_["init_type"] == "local_quantumnumbers") {
    ...
    std::copy(
        site_types.begin(), site_types.end(),
        maquis::ostream_iterator<int>(maquis::cout, " ")
    );
    ...
    for (std::set<std::string>::const_iterator it = all_qn.begin();
         it != all_qn.end(); ++it) {
      const std::string pname = "initial_local_" + *it;
      if (!p_.defined(pname))
        throw std::runtime_error(
            pname + " required for local_quantumnumbers initial state."
        );
    ...
    return initializer_ptr(new basis_mps_init_generic<Matrix, SymmGroup>(
        state, phys_bases, this->total_quantum_numbers(p_), site_types
    ));
  } else {
...
ALPSModel<Matrix, SymmGroup>::measurements() const {
      ...
      if (meas_found != not_found) {
          ...
          for (int type1 = 0; type1 < ntypes; ++type1)
            for (int type2 = 0; type2 < ntypes; ++type2) {
              if (used1.find(type1) != used1.end() &&
                  used2.find(type2) != used2.end())
                continue;
              ...
              V ops = bondop.template templated_split<value_type>(b1, b2);
              if (operators.size() < ops.size())
                operators.resize(
                    ops.size(),
                    bond_element(2, std::make_pair(op_vec(ntypes), false))
                );
              int num_done = 0;
              for (typename V::iterator tit = ops.begin(); tit != ops.end();
                   ++tit) {
                SiteOperator op1 = std::get<1>(*tit);
                SiteOperator op2 = std::get<2>(*tit);

                if (!b1.has_operator(simplify_name(op1)) ||
                    !b2.has_operator(simplify_name(op2)))
                  continue;
                ...
                unsigned ii = std::distance(ops.begin(), tit);
                {
                  ...
                  if (operators[ii][0].second)
                    gemm(
                        fillings[type1],
                        this->get_operator(simplify_name(op1), type1), m
                    );  // Note inverse notation because of notation in
                        // operator.
                  else
          ...
          if (meas_found == is_average)
            meas.push_back(new measurements::average<Matrix, SymmGroup>(
                obsname, raw_lattice, identitities, fillings, operators
            ));
          else
            meas.push_back(new measurements::local<Matrix, SymmGroup>(
                obsname, raw_lattice, identitities, fillings, operators
            ));
                } else {
          std::vector<op_t> tops(ntypes);
          ...
          if (meas_found == is_average)
            meas.push_back(new measurements::average<Matrix, SymmGroup>(
                obsname, raw_lattice, identitities, fillings, tops
            ));
          else
            meas.push_back(new measurements::local<Matrix, SymmGroup>(
                obsname, raw_lattice, identitities, fillings, tops
  ...
  {  // Example: MEASURE_LOCAL_AT[Custom correlation] =
     // "bdag:b|(1,2),(3,4),(5,6)"
    ...
    for (auto&& it : parms.get_range()) {
      ...
      if (std::regex_match(lhs, what, expression)) {
        ...
        std::copy(
            part_tokens.begin(), part_tokens.end(), std::back_inserter(parts)
        );
        ...
        std::regex pos_re("\\(([^(^)]*)\\)");
        boost::sregex_token_iterator it_pos(
            parts[1].begin(), parts[1].end(), pos_re, 1
        );
        ...
        meas.push_back(new measurements::local_at<Matrix, SymmGroup>(
            name, raw_lattice, positions, identitities, fillings, operators
        ));
      }
    }
  }
...

  {
    ...
    for (auto&& it : parms.get_range()) {
      ...
      if (!name.empty()) {
        ...
        meas_operators_type operators;
        short ops_type;
        std::tie(operators, ops_type) =
            operators_for_meas(value_split[0], true);
        ...
        meas.push_back(new measurements::correlations<Matrix, SymmGroup>(
            name, raw_lattice, identitities, fillings, operators, half_only,
            nearest_neighbors_only, positions
        ));
      }
    }
  }
