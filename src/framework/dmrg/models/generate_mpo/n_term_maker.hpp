...
class NTermsMPO {
 ...
 public:
  NTermsMPO(
      ...
      const std::vector<op_t>& fill_, std::vector<pos_op_t> const& ops_,
      const int& phase
  )
      : lat(lat_), identities(ident_), fillings(fill_), ops(ops_) {
    // Assuming all operators are fermionic!!
    bool trivial_fill = true;
    std::sort(ops.begin(), ops.end(), [](const pos_op_t& a, const pos_op_t& b) {
      return a.first < b.first;
    });

    for (pos_t p = 0; p < lat.size(); ++p) {
      for (typename std::vector<pos_op_t>::iterator it = ops.begin();
           it != ops.end(); ++it) {
        if (p == it->first) {
          if (prempo.count(p) > 0) {
      ...
      if (prempo.count(p) == 0) {
        if (trivial_fill) {
          prempo.insert(
              std::make_pair(p, identities[lat.get_prop<int>("type", p)])
          );
        } else {
          prempo.insert(
              std::make_pair(p, fillings[lat.get_prop<int>("type", p)])
          );
        }
      }
    }
