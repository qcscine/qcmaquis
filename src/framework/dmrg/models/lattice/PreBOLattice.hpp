...
class PreBOLattice : public lattice_impl {
 public:
  ...
  explicit PreBOLattice(BaseParameters& parms) {
    // Populate variables:
    auto vec_particles_str =
        parms["PreBO_ParticleTypeVector"].as<std::string>();
    ...
    auto vec_ini_state_str =
        parms["PreBO_InitialStateVector"].as<std::string>();
    ...
    // the length of the lattice is determined by the number of different
    // particle types and the number of the basis functions of each particle
    // type.
    ...
    if (parms.is_set("orbital_order")) {
      ...
      for (int p = 0; p < m_order.size(); ++p) {
        m_inv_order[p] = std::distance(
            m_order.begin(), std::find(m_order.begin(), m_order.end(), p)
        );
      }
      for (pos_t i = 0; i < L; i++) {
  ...
  // -- GETTERS --
  /**
   * Takes the relative position of a given particle type and returns its
   * absolute position on the lattice
   * @param pt
   * @param rel_pos
   * @return
   */
  pos_t get_abs_position(part_type const& pt, pos_t const& rel_pos)
      const override {
    ...
    return std::accumulate(
        vec_orbitals.begin(), (vec_orbitals.begin() + pt), rel_pos
    );
  }
  // The following methods are the same as for the Orbital and the Open Chain
  // Lattice. The only difference is the way in which the lattice site type is
  // extracted, which is taken from the orbital lattice
 ...
 private:
  ...
  std::vector<part_type>
      vec_lattice_type;  // Stores the index of the particle type for each site.
  ...
  std::vector<pos_t>
      m_order;  // ordering of the sites -> E.g. canonical or Fiedler
  std::vector<pos_t> m_inv_order;  // inverted ordering of the sites
  std::vector<std::size_t>
      vec_max_m;  // Stores max bond dim for all particle types
