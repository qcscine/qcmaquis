...
class lattice_impl {
 public:
  ...
  // non-virtual!
  template <class T>
  T get_prop(const std::string& property) const
  ...
  template <class T>
  T get_prop(const std::string& property, pos_t site) const {
    return std::any_cast<T>(get_prop_(property, std::vector<pos_t>(1, site)));
  }

  template <class T>
  T get_prop(const std::string& property, pos_t bond1, pos_t bond2) const {
  ...
  template <class T>
  T get_prop(const std::string& property, std::vector<pos_t> const& positions)
      const {
    return std::any_cast<T>(get_prop_(property, positions));
  }

  // virtual!
  virtual std::any get_prop_(std::string const&, std::vector<pos_t> const&)
      const = 0;

  virtual pos_t get_abs_position(part_type const& pt, pos_t const& rel_pos)
      const {
    return 0;
  };
...
class Lattice {
 ...
 public:
  ...
  Lattice(BaseParameters& parms) : impl_(lattice_factory(parms)) {}
  ...
  template <class T>
  T get_prop(std::string property) const {
