...
std::map<std::string, MPO<Matrix, SymmGroup> > local(
    const Lattice& lat,
  ...
  for (std::size_t p = 0; p < lat.size(); ++p) {
    if (ops.size() == 1) {
      ...
      for (int neigh : neighs) {
        ...
        if (ops[0].first[type1].n_blocks() > 0 &&
            ops[1].first[type2].n_blocks() > 0) {
          generate_mpo::MPOMaker<Matrix, SymmGroup> mpom(
              lat, identities, fillings
          );
          generate_mpo::OperatorTerm<Matrix, SymmGroup> term;
          ...
          mpos[lat.get_prop<std::string>("label", p, neigh)] =
              mpom.create_mpo();
        }
      }
    }
...
MPO<Matrix, SymmGroup> average(
  ...
  for (std::size_t p = 0; p < lat.size(); ++p) {
    ...
    term.operators.push_back(
        std::make_pair(p, ops[0].first[lat.get_prop<int>("type", p)])
    );
    if (ops.size() == 1) {
      mpom.add_term(term);
    } else {
      ...
      for (int neigh : neighs) {
        ...
        term2.operators.push_back(
            std::make_pair(neigh, ops[1].first[lat.get_prop<int>("type", p)])
        );
        mpom.add_term(term2);
      }
    }
