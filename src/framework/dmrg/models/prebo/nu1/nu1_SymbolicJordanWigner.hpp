 ...
 * This is needed to generate the proper terms based on the Jordan--Wigner
 * transformation.
 */
class SymbolicOperator {
 public:
 ...
 public:
  /** @brief Constructor */
  SymbolicOperator(
      pos_t site, OpType opType, part_type pt, Spin spin = Spin::None
  )
      : m_site(site), m_opType(opType), m_part_type(pt), m_spin(spin) {}

  /**
   * @brief Copy constructor + op type
   * Note that in this case the operator type is given externally, and is not
   * read from the SymOp object.
   */
  ...
  void print() const {
    ...
    if (m_opType == OpType::Filling)
      ...
      op_str.append("b");
    if (m_opType == OpType::Create) op_str.append("+");
    op_str.append("^");
    std::stringstream ss;
    ss << m_part_type;
 ...
 * @brief This class handels the Jordan-Wigner transformation, based on the
 * `SymbolicOperator`.
 * @note This is the most important and delicate part of the pre-BO model.
 *
 * For a description of the algorithm, see:
 * Correlation effects in Multicomponent Quantum Chemistry (2020), Robin
 * Feldmann, Markus Reiher group ETH Zurich, p 27
 */
class SymbolicJordanWigner {
 ...
 public:
  ...
  explicit SymbolicJordanWigner(std::vector<SymbolicOperator> SymOpStr) {
    ...
    if (!(std::equal(
            vec_pt_check.begin() + 1, vec_pt_check.end(), vec_pt_check.begin()
        )))
      throw std::runtime_error(
          "JW must only be applied to operator strings containing equal "
          "particle types."
      );
  ...
  const std::vector<SymbolicOperator> &getSymOpStr() const {
    return m_SymOpStr;
  }
 ...
 private:
   ...
   * and right side.
   */
  static void JW_generate_lr_index(
      const std::vector<std::pair<part_type, pos_t>> &nb_term,
    ...
    //
    // VERY IMPORTANT:  This std::vector of maps takes care of the
    // Jordan--Wigner transformation!!
    //                  --> the procedure is very delicate and error-prone,
    //                  --> any modifications must be debugged very carefully
    //
    // This std::vector of maps stores the filling string for the relevant sites
    // for each operator. key: index, value: true if operator at position in
    // std::vector provides unassigned filling for site ´´índex´´.
  ...
  void insertFillings(
      const std::vector<pos_t> &lindex, const std::vector<pos_t> &rindex
  ) {
    std::vector<SymbolicOperator> new_SymOpStr;
    size_t count = 0;
    for (auto const &symOp : m_SymOpStr) {
      // fermion
      if (symOp.getSpin() == Spin::Down || symOp.getSpin() == Spin::Up) {
        // c-(d) := F a(d)
        if (symOp.getSpin() == Spin::Down &&
            symOp.getOpType() == OpType::Annihilate)
          new_SymOpStr.push_back(SymbolicOperator(symOp, OpType::Filling));
