 ...
 * General info: the ordering of the lattice for Hamiltonian and particle RDM is
 * handled by the nu1_nBodyTerm class. For the orbital-RDMs, it is handled here.
 *
...
class TermGenerator {
  ...
  using symbolic_terms = typename std::vector<
      std::pair<value_type, std::vector<SymbolicOperator>>>;
 ...
 private:
  ...
  std::vector<tag_type> fer_ident_tag, fer_filling_tag, fer_create_up_tag,
      fer_create_down_tag, fer_dest_up_tag, fer_dest_down_tag, bos_ident_tag,
      bos_create_tag, bos_dest_tag;
 ...
 public:
  ...
  /**
   * Constructor. Automatically updates the tag_handler, but the updated
   * phys_indexes have to be retrieved after the constructor was called.
   * @param lat
   * @param ptr_tag_handler_
   */
  TermGenerator(
      std::shared_ptr<Lattice>& lat,
      std::shared_ptr<TagHandler<Matrix, NU1>>& ptr_tag_handler_, bool verbose
  )
      : ptr_lat(lat), ptr_tag_handler(ptr_tag_handler_), verbose_(verbose) {
    ptr_tag_container =
        std::make_shared<prebo::TagGenerator<Matrix, N>>(lat, ptr_tag_handler);

    // Fermion
    fer_create_up_tag = ptr_tag_container->get_tag_vector(
        Type::Fermion, OpType::Create, Spin::Up
    );
    fer_dest_up_tag = ptr_tag_container->get_tag_vector(
        Type::Fermion, OpType::Annihilate, Spin::Up
    );
    fer_create_down_tag = ptr_tag_container->get_tag_vector(
        Type::Fermion, OpType::Create, Spin::Down
    );
    fer_dest_down_tag = ptr_tag_container->get_tag_vector(
        Type::Fermion, OpType::Annihilate, Spin::Down
    );
    fer_filling_tag = ptr_tag_container->get_tag_vector(
        Type::Fermion, OpType::Filling, Spin::None
    );
    fer_ident_tag = ptr_tag_container->get_tag_vector(
        Type::Fermion, OpType::Ident, Spin::None
    );
    // Bosons
    bos_create_tag = ptr_tag_container->get_tag_vector(
        Type::Boson, OpType::Create, Spin::Zero
    );
    bos_dest_tag = ptr_tag_container->get_tag_vector(
        Type::Boson, OpType::Annihilate, Spin::Zero
    );
    bos_ident_tag = ptr_tag_container->get_tag_vector(
        Type::Boson, OpType::Ident, Spin::None
    );
    // tag handler
  ...
  /**
   * This method generates the terms of the pre-BO Hamiltonian. It is called
   * inside the model.
   * @param integrals
   * @return
   */
  auto generate_Hamiltonian(
      const std::pair<
          std::vector<chem::index_type<chem::Hamiltonian::PreBO>>,
          const std::vector<value_type>>& integrals
  ) -> terms_type {
    ...
    auto getTermOrder =
        [](const chem::index_type<chem::Hamiltonian::PreBO>& indices) {
    ...
    for (auto line = 0; line < integrals.first.size(); ++line) {
      ...
      if (termOrder == 0) {
        ...
        if (isFermion[ptr_lat->template get_prop<int>("type", pos)])
          ops.push_back(
              fer_ident_tag[ptr_lat->template get_prop<int>("type", pos)]
          );
        else
          ops.push_back(
              bos_ident_tag[ptr_lat->template get_prop<int>("type", pos)]
          );
        modelHelper<Matrix, NU1>::add_term(
            pos, ops, integral, ptr_tag_handler, hamiltonian
        );
        continue;
      }
      ...
      // followed.
      assert(
          (termOrder == 1 && nbody_term[0].first == nbody_term[1].first) ||
      ...
      NBodyTerm nBodyTerm =
          NBodyTerm(nbody_term, isFermion, vec_orbitals, inv_order);

      std::vector<std::vector<SymbolicOperator>> vec_SymOpStr =
          nBodyTerm.getVecSymOpStr();
  ...
  auto clean_hamiltonian(const terms_type& terms_temp) -> terms_type {
    ...
    std::unordered_map<term_descriptor, value_type, term_descriptor_hasher>
        count_map;
    ...
    if (verbose_) {
      std::cout << "======================================================"
                << std::endl;
      std::cout << "The final size of the Hamiltonian is:" << std::endl
                << std::endl;
  ...
  /**
   * @brief This method takes the symbolic operator string and populates
   * operators and positions vector.
   * --> This method is the link between the symbolic algebra routine and the
   * terms used for the MPO construction.
   * @param pos
   * @param ops
   * @param SymOpStr
   */
  void Symbols2Tag(
      positions_type& pos, operators_type& ops,
      const std::vector<SymbolicOperator>& SymOpStr
  ) {
  ...
  /**
   * @brief Returns the `terms` element that contains the RDM terms for a given
   * particle type and two sites This function relies on the symbolic algebra
   * routine but does not expose it.
   * @param i --> particle type
   * @param mu --> site index
   * @param nu --> site index
   * @return terms
   */
  auto generate_terms1RDM(const int& i, const int& mu, const int& nu)
      -> terms_type {
    //
    // < Psi | a+^i_mu a^j_nu | Psi> => Generate operator string for particle
    // type i and sites mu and nu
    //
    terms_type res;
...
// the given type
    ...
    for (auto const& OpStr : vec_SymOpStr) {
      ...
      std::pair<term_descriptor, bool> ret =
          modelHelper<Matrix, NU1>::arrange_operators(
              pos, ops, scaling, ptr_tag_handler
          );
      if (!ret.second) {
        auto term = ret.first;
        term.coeff = scaling;
        res.push_back(term);
        if (this->termVerbose_) std::cout << term << std::endl;
      }
    }
    return res;
  }
   ...
   * @return product
   */
  auto multiply(const symbolic_terms& lhs, const symbolic_terms& rhs)
      -> symbolic_terms {
    symbolic_terms res;
    res.reserve(lhs.size() * rhs.size());
    // Multiply the two strings of symbolic operators.
    // Loop over terms on the left hand side and multiply for each term with
    // every other term on the right hand side
  ...
  /**
   * @brief Generates the `symbolic_terms` of the transition operator for the
   * evaluation of the orbital-RDMs. Note 1: that there are 16 transition
   * operators for fermions. Note 2: Bosons are not implemented, yet.
   * @param i --> particle type
   * @param mu --> site index
   * @param which  --> which transition operator {1, 2, ..., 16}
   * @return symbolic_terms
   */
  auto generate_transitionOps(
      const part_type& i, const pos_t& mu, const int& which
  ) -> symbolic_terms {
    ...
    if (isFermion[i]) {
      switch (which) {
        case 1:
          ...
          SymOp_temp.push_back(
              SymbolicOperator(mu, OpType::Annihilate, i, Spin::Up)
          );
          scaling = -1.;
          transition_op.push_back(std::make_pair(scaling, SymOp_temp));
          // - n_down
          SymOp_temp.clear();
          SymOp_temp.push_back(
              SymbolicOperator(mu, OpType::Create, i, Spin::Down)
          );
          SymOp_temp.push_back(
              SymbolicOperator(mu, OpType::Annihilate, i, Spin::Down)
          );
          scaling = -1.;
          transition_op.push_back(std::make_pair(scaling, SymOp_temp));
          // + n_up n_down
      ...
      std::cout << "Bosonic Transition Operators not implemented, yet."
                << std::endl;
    }
  ...
  /**
   * Generates the symbolic transition operators for the two-orbital RDM and
   * handles the JW transformation, and the two cases for the same and different
   * particle types.
   * @param i
   * @param j
   * @param mu
   ...
   */
  auto generate_transitionOps(
      const part_type& i, const part_type& j, const pos_t& mu, const pos_t& nu,
      const int& which1, const int& which2
  ) -> symbolic_terms {
    SymbolicJordanWigner JW;

    symbolic_terms transition_ops1 = generate_transitionOps(i, mu, which1);
    symbolic_terms transition_ops2 = generate_transitionOps(j, nu, which2);
    ...
    symbolic_terms transition_ops_res =
        multiply(transition_ops1, transition_ops2);
    if (i == j) {
  ...
  /**
   * @brief Generates the actual `terms` of the one-orbital RDM. Takes care of
   * the orbital ordering. This function relies on the symbolic algebra routine
   * but does not expose it. ATTENTION: Insert already ordered site index!!
   * @param i --> particle type
   * @param mu --> !unordered! site index belonging to i
   * @param which
   * @return terms
   */
  auto generate_termsTransitionOp(const int& i, const int& mu, const int& which)
      -> terms_type {
    terms_type res;

    symbolic_terms transition_ops = generate_transitionOps(i, mu, which);
      ...
      if (!ret.second) {
        ...
        if (termVerbose_) {
          ...
          for (int i = 0; i < term.size(); ++i)
            std::cout << ptr_tag_handler->get_op(term.operator_tag(i))
                      << std::endl;
        }
      }
    }
  ...
  /**
   * @brief Generates the actual `terms` of the two-orbital RDM. Takes care of
   * the orbital ordering. This function relies on the symbolic algebra routine
   * but does not expose it. ATTENTION: Insert already ordered site index!!
   * @param i --> particle type
   ...
   * @return terms
   */
  auto generate_termsTransitionOp(
      const int& i, const int& j, const int& mu, const int& nu,
      const int& which1, const int& which2
  ) -> terms_type {
    terms_type res;
 ...
 public:
  ...
  const auto& getTagContainer() const { return ptr_tag_container; }

  /** @brief Getter for the physical indices */
  const std::vector<Index<NU1>>& getPhysIndexes() const { return phys_indexes; }
