...
/** @brief Function that transforms the mps tensor at a given site from su2 to
 * 2u1 */
      ...
      for (size_t s = 0; s < physical_i_in.size(); ++s) {
        ...
        // transfer the blocks
          ...
          std::size_t in_left_offset =
              in_left_pb(physical_i_in[s].first, left_i_in[l].first);
          ...
          for (typename std::vector<
            ...
            if (!m_out.has_block(rightc, rightc)) continue;
            ...
            int m_tot = rightc[0] - rightc[1];  // spin proj resulting from left
                                                // and site --> total spin proj
            double prefac =
                pow(-1.0, (l_left - l_site + m_tot) / 2) * sqrt(l_tot + 1.0);
            double clebsch_gordan =
                prefac * WignerWrapper::gsl_sf_coupling_3j(
                             l_left, l_site, l_tot, m_left, m_site, -m_tot
                         );
...
struct transform_mps {
  typedef typename boost::mpl::if_<
      symm_traits::HasPG<SymmGroup>, TwoU1PG, TwoU1>::type SymmOut;
