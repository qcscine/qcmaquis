...
// function objects to set point group symmetry in Indices depending on the
// occupation (do not set irrep if empty or doubly occupied)
...
class PGCharge {
 public:
  typename SymmGroup::charge operator()(
      typename SymmGroup::charge rhs, int irr
  ) {
    return rhs;
  }
};
...
class PGCharge<
 ...
 public:
  using subcharge = typename SymmGroup::subcharge;
  typename SymmGroup::charge operator()(
      typename SymmGroup::charge rhs, subcharge irr
  ) {
    SymmGroup::irrep(rhs) = irr;
    return rhs;
  }
