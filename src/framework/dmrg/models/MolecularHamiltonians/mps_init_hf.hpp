...
struct hf_mps_init : public mps_initializer<Matrix, SymmGroup> {
  ...
  void operator()(MPS<Matrix, SymmGroup>& mps) {
    ...
    if (!parms.is_set("orbital_order"))
      for (pos_t p = 0; p < mps.length(); ++p) order[p] = p + 1;
    else
    ...
    for (pos_t i = 0; i < mps.length(); ++i) {
...
#ifndef NDEBUG
      maquis::cout << "site " << i << " activating sector " << max_charge
                   << std::endl;
#endif
      ...
      if (max_pos >= mps[i].data().n_blocks()) {
        maquis::cout << "ERROR: Symmetry block " << max_charge
                     << " not found\n";
        maquis::cout << "site " << i << ", site_charge " << site_charge
                     << ", cumulated_charge " << max_charge
                     << ", block_pos: " << max_pos
                     << ", number of blocks: " << mps[i].data().n_blocks()
                     << std::endl;

        maquis::cout << "This error occurs if the specified HF determinant is "
                        "not in the same symmetry sector as the target state\n";
        exit(1);
      }
...
struct hf_mps_init<Matrix, SymmGroup, symm_traits::enable_if_su2_t<SymmGroup> >
  ...
  hf_mps_init(
      const BaseParameters& parms_,
      std::vector<Index<SymmGroup> > const& phys_dims_, charge right_end,
  ...
  void operator()(MPS<Matrix, SymmGroup>& mps) {
    ...
    // count total number of electrons in the given HF guess to make sure it
    // matches
    ...
    for (pos_t i = 0; i < mps.length(); ++i) {
      ...
      if (sc_input > 4)
        throw std::runtime_error(
            "The hf_occ format has been changed to: 1=empty, 2=down, 3=up, "
            "4=updown\n (not cumulative anymore)\n"
        );
      if (phys_dims[site_types[i]].size() != 4)
        throw std::runtime_error("HF init expects 4 states per orbital\n");
      ...
      for (typename container_type::const_iterator it = bond_charges.begin();
           it != bond_charges.end(); ++it)
        for (typename container_type::const_iterator it2 = site_charges.begin();
             it2 != site_charges.end(); ++it2) {
          charge sector = SymmGroup::fuse(*it, *it2);
          if (mps[i].col_dim().has(sector)) next_bond_charges.insert(sector);
        }

#ifndef NDEBUG
      for (typename container_type::const_iterator it =
               next_bond_charges.begin();
           it != next_bond_charges.end(); ++it)
        maquis::cout << "site " << i << " activating sector " << *it
                     << std::endl;
      ...
      for (typename container_type::const_iterator it =
        ...
        if (max_pos >= mps[i].data().n_blocks()) {
          maquis::cout << "ERROR: Symmetry block " << max_charge
                       << " not found\n";
          maquis::cout << "site " << i << ", cumulated_charge " << max_charge
                       << ", block_pos: " << max_pos
                       << ", number of blocks: " << mps[i].data().n_blocks()
                       << std::endl;

          maquis::cout
              << "This error occurs if the specified HF determinant is not in "
                 "the same symmetry sector as the target state\n";
          exit(1);
        }
        Matrix& mfirst = mps[i].data()[max_pos];
        size_t nrow = mfirst.num_rows();
    ...
    if (nelec_tmp != nelec)
      throw std::runtime_error(
          "Number of electrons in the HF guess is incorrect: expected " +
          std::to_string(nelec) + " but got " + std::to_string(nelec_tmp)
      );
  }
