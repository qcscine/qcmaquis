...
#include "dmrg/models/MolecularHamiltonians/rel/chem_helper.h"
...
#undef GENERATE_SITE_SPECIFIC
        ...
        if (std::regex_match(lhs, what, expression)) {
          ...
          if (it.second == "N")
            ...
            throw std::runtime_error(
                "Invalid observable\nLocal measurement supported so far is "
                "\"N\"\n"
            );
      ...
      for (auto&& it : parms.get_range()) {
        ...
        if (std::regex_match(lhs, what, expression_oneptdm) ||
          ...
          using scaled_bond_element =
              std::pair<std::vector<tag_vec>, value_type>;
 ...
 private:
  ...
  std::vector<op_t> generate_site_specific_ops(op_t const& op) const {
    ...
    std::vector<op_t> ret;
    for (typename SymmGroup::subcharge sc = 0;
         sc < SymmGroup::get_max_irrep() + 1; ++sc) {
      op_t mod(set_symm(op.basis(), sc));
      for (std::size_t b = 0; b < op.n_blocks(); ++b) mod[b] = op[b];
...
#include "dmrg/models/MolecularHamiltonians/rel/model.hpp"
