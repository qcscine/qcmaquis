 ...
 * In this case, all modals of a unique mode are mapped to a single site of the
 * dmrg lattice, involving multiple creation/annihilation operator pairs per
 * site
 */
...
class NModeModelPaired : public model_impl<Matrix, TrivialGroup> {
 ...
 public:
   ...
   */
  NModeModelPaired(
      const Lattice& lattice_, BaseParameters& parameters_, bool verbose
  )
      : lattice(lattice_),
        parameters(parameters_),
        tag_handler(new table_type()),
        phys_indexes(0)  // check whether everything needed is here
  {
    ...
    if (nModalsVec.size() == 1) {
      ...
      throw std::runtime_error(
          "nmode_num_basis needs to be either a single integer or a list with "
          "lenght L"
      );
    }
    ...
    std::vector<int> nModalsUniqueVec(
        nModalsUnique.begin(), nModalsUnique.end()
    );
    ...
    // we define the identity, the creation, the annihilation, and the count
    // operator. define the number of operators we need
    ...
    if (nModalsUnique.size() == 1) {
      ...
      ident = modelHelper<Matrix, TrivialGroup>::register_all_types(
          ident_op, tag_detail::bosonic, tag_handler
      );
      count = modelHelper<Matrix, TrivialGroup>::register_all_types(
          count_op, tag_detail::bosonic, tag_handler
      );
      paired = modelHelper<Matrix, TrivialGroup>::register_all_types(
  ...
  create_terms() override {  // this is taken from nu1/model.hpp. i dont think i
    ...
    for (int iTerm = 0; iTerm < hamiltonianSize; iTerm++) {
      ...
      convertLineToOperators(
          HamiltonianTerms.first[iTerm], positions, operators
      );
      if (positions.size() / 2 <= maxCouplingDegree) {
        auto matrixElement =
            static_cast<value_type>(HamiltonianTerms.second[iTerm]);
 ...
 private:
  ...
  void convertLineToOperators(
    ...
    std::set<int> nModalsUnique(nModalsVec.begin(), nModalsVec.end());
    if (nModalsUnique.size() ==
        1) {  // if all modes have the same size physical basis
      do {
      ...
      do {
        ...
        int collector = 0;
        int element;
        for (auto it = nModalsUnique.begin();
             it != nModalsUnique.end() && currentIndex < indexInSet;
             ++it, ++currentIndex) {
          element = *it;
          element *= element;
          collector += element;
        ...
        if (pair) {
          int index =
              (collector - 1) + modalToCreate * (basisSize) + modalToDestroy;
          ops.push_back(paired[index]);
          pos.push_back(mode);
        }
        pair = false;
    ...
    if (parameters.is_set("MEASURE[ModeExcitationDegree]")) {
      for (int iSite = 0; iSite < lattice_size; iSite++) {
        ...
        meas.push_back(new measurements::local_at<Matrix, TrivialGroup>(
            name, lattice, pos_local, identities_local, fillings_local, ops
        ));
      }
    }
    return meas;
