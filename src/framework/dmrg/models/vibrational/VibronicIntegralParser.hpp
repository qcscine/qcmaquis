  ...
  if (parms.is_set("integral_file")) {
    ...
    while (getline(orb_file, line_string)) {
      ...
      for (const auto& sq_op_str : line_splitted) {
        ...
        boost::split(
            temp, sq_op_str, boost::is_any_of("-\t ")
        );  // splits string whenever a hyphen, tab or space is encountered
        indices_str.insert(
      ...
      assert(
          indices_str.size() == 4 + 2 || indices_str.size() == 8 + 2 ||
          indices_str.size() == 12 + 2
      );  //+2 as there si a number denoting excited state and whether mode
