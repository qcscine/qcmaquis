...
std::pair<
    typename OPTable<Matrix, SymmGroup>::tag_type,
    typename OPTable<Matrix, SymmGroup>::mvalue_type>
OPTable<Matrix, SymmGroup>::checked_register(op_t const& sample) {
  ...
  if (auto op_it = find_if(this->begin(), this->end(), get_scale);
      op_it == this->end()) {
    // if operator not found, register new operator
    return std::make_pair(this->register_op(sample), 1.0);
  } else {
    // if operator found, register the existing operator withh the new scale
    // factor
