...
enum class EigenvalueSelection {
  Accepted,
  NotInInterval,
  HighVariance,
  TruncatedEnergyLower
};
...
class FEASTPostProcessor {
 public:
  ...
  FEASTPostProcessor(
      int numberOfStates, int numberOfQuadrature,
      const std::vector<ComplexNumber>& w, const ModelType& inputModel,
      const LatticeType& inputLattice, BaseParameters& parms_, double eMin_,
      double eMax_, bool verbose = false
  )
      : nStates(numberOfStates),
        nQuad(numberOfQuadrature),
        weights(w),
        model(inputModel),
        lattice(inputLattice),
        calculateStandardDeviation(false),
        eMin(eMin_),
        eMax(eMax_),
        verbose_(verbose),
        parms(parms_),
    ...
    accepted = std::vector<EigenvalueSelection>(
        nStates, EigenvalueSelection::Accepted
    );
    totalQN = model.total_quantum_numbers(parms);
    calculateStandardDeviation =
        (parms["feast_calculate_standard_deviation"] == "yes");
    screenStandardDeviation =
        parms.is_set("feast_standard_deviation_threshold");
    if (screenStandardDeviation)
      standardDeviationScreening =
          parms["feast_standard_deviation_threshold"].template as<double>();
  }
  ...
  void solveEigenvalueProblem(const MPOType& mpo) {
    ...
    // Note that here we do not first sum the MPS and then calculate the
    // expectation value, because this would lead to a very large MPS. We
    // instead sum the expectation values directly.
      ...
      for (int iQ = 0; iQ < nQuad; iQ++) {
        ...
        for (int j = 0; j < nStates; j++) {
          for (int jQ = 0; jQ < nQuad; jQ++) {
            ...
            H(i, j) += expval(mpsCopyI, mpsCopyJ, mpo) *
                       std::conj(weights[iQ]) * weights[jQ];
            B(i, j) += overlap(mpsCopyI, mpsCopyJ) * std::conj(weights[iQ]) *
                       weights[jQ];
          }
        }
      }
    ...
    // if (verbose_) {
    //   maquis::cout << " Hamiltonian matrix in the FEAST subspace" <<
    //   std::endl; maquis::cout << H << std::endl; maquis::cout << " Overlap
    //   matrix of the FEAST subspace" << std::endl; maquis::cout << B <<
    //   std::endl;
    // }
    // for (int i = 0; i < nStates; i++)
    ...
    ComplexMatrixType leftEigenVectors(nStates, nStates, zeroComplex),
        rightEigenVectors(nStates, nStates, zeroComplex);
    ComplexVectorType alphaVec(nStates, 0.), betaVec(nStates, zeroComplex);
    alps::numeric::ggev(
        H, B, alphaVec, betaVec, leftEigenVectors, rightEigenVectors,
        thresholdForRank_
    );
    // // Calculates the overlap in the right eigenvectors basis
    // ComplexMatrixType overlapEigenvectorBasis(nStates, nStates);
    // gemm(adjoint(rightEigenVectors), rightEigenVectors,
    // overlapEigenvectorBasis); RealVectorType overlapEigenValues(nStates);
    ...
    for (int iState = 0; iState < nStates; iState++) {
      if (std::imag(alphaVec[iState]) > 1.0E-10 ||
          std::imag(betaVec[iState]) > 1.0E-10)
        maquis::cout << " WARNING: Energy of the " << iState
                     << "-th state has a non-negligible imaginary part"
                     << std::endl;
      if (std::abs(betaVec[iState]) > thresholdForRank_) {
        energies[iState] = maquis::real(alphaVec[iState] / betaVec[iState]);
        accepted[iState] = (energies[iState] > eMin && energies[iState] < eMax)
                               ? EigenvalueSelection::Accepted
   ...
   *
   * The eigenvectors of the FEAST Hamiltonian are used to back-transform the
   * MPS and provide new guess for FEAST iteration.
   *
  ...
  auto performBackTransformation(const MPOType& mpo, int mMax, bool truncEach) {
    ...
    for (int iOutput = 0; iOutput < nStates; iOutput++) {
      if (accepted[iOutput] == EigenvalueSelection::Accepted) {
        for (int iInput = 0; iInput < nStates; iInput++) {
          // maquis::cout << "(" << iInput << "," << iOutput << ") = " <<
          // feastEigenVectors(iInput, iOutput) << std::endl;
          if (std::abs(feastEigenVectors(iInput, iOutput)) >
              thresholdForRank_) {
            for (int iQuad = 0; iQuad < nQuad; iQuad++) {
              // maquis::cout << " - iQuad = " << weights[iQuad] << std::endl;
              MPSType mpsToAdd =
                  mpsContainer->operator[](std::make_pair(iInput, iQuad));
              auto scalingFactor = feastEigenVectors(iInput, iOutput) *
                                   weights[iQuad];  // /normVector[iInput];
              ...
              if (iQuad == 0) {
                ...
                if (std::abs(scalingFactor) > thresholdForRank_) {
                  if (truncEach)
                    mpsTransformed[std::make_pair(iOutput, iInput)] =
                        joinAndTruncate(
                            mpsTransformed[std::make_pair(iOutput, iInput)],
                            mpsToAdd, mMax
                        );
                  else
                    mpsTransformed[std::make_pair(iOutput, iInput)] = join(
                        mpsTransformed[std::make_pair(iOutput, iInput)],
                        mpsToAdd
                    );
                }
            ...
            if (!truncEach)
              mpsTransformed[std::make_pair(iOutput, iInput)] =
                  compression::l2r_compress(
                      mpsTransformed[std::make_pair(iOutput, iInput)], mMax,
                      thresholdForRank_, false
                  );
          }
        }
      }
    ...
    for (int iOutput = 0; iOutput < nStates; iOutput++) {
      ...
      if (accepted[iOutput] == EigenvalueSelection::Accepted) {
        ...
        while (!found) {
          auto position =
              mpsTransformed.find(std::make_pair(iOutput, iFirstInput));
          if (position != mpsTransformed.end())
            found = true;
          else
            iFirstInput += 1;
        }
        currentFEASTMPSs->operator[](iOutput) =
            mpsTransformed[std::make_pair(iOutput, iFirstInput)];
        for (int iInput = iFirstInput + 1; iInput < nStates; iInput++) {
          ...
          if (mpsTransformed.find(key) != mpsTransformed.end()) {
            if (truncEach)
              currentFEASTMPSs->operator[](iOutput) = joinAndTruncate(
                  currentFEASTMPSs->operator[](iOutput), mpsTransformed[key],
                  mMax
              );
            else
              currentFEASTMPSs->operator[](iOutput) = join(
                  currentFEASTMPSs->operator[](iOutput), mpsTransformed[key]
              );
          }
        }
        ...
        if (!truncEach)
          currentFEASTMPSs->operator[](iOutput) = compression::l2r_compress(
              currentFEASTMPSs->operator[](iOutput), mMax, 1.0E-16
          );
        truncatedEnergy[iOutput] = maquis::real(
            expval(currentFEASTMPSs->operator[](iOutput), mpo) /
            norm(currentFEASTMPSs->operator[](iOutput))
        );
        // If requested, calculates the standard deviations
        if (calculateStandardDeviation)
          standardDeviations[iOutput] = this->getStandardDeviation(
              mpo, currentFEASTMPSs->operator[](iOutput), mMax
          );
      }
    ...
    // If a variance-based screening has been requested, overrides again with a
    // random value
    if (screenStandardDeviation && calculateStandardDeviation) {
      for (int iState = 0; iState < nStates; iState++) {
        if (std::abs(standardDeviations[iState]) > standardDeviationScreening &&
            accepted[iState] == EigenvalueSelection::Accepted) {
    ...
    for (int iState = 0; iState < energies.size(); iState++) {
      if (accepted[iState] == EigenvalueSelection::Accepted) {
        ...
        // Check whether the truncated energy is actually higher than the full
        // FEAST one
  ...
  void printResults() const {
    maquis::cout << " +--------------------------------------------------------"
                    "--+----------------------+"
                 << std::endl;
    maquis::cout << " |   State    |      Old energy      |      New energy    "
                    "  |   Truncated energy   |"
                 << std::endl;
    maquis::cout << " +--------------------------------------------------------"
                    "--+----------------------+"
      ...
      switch (accepted[iState]) {
        case EigenvalueSelection::Accepted:
          maquis::cout << std::setw(13) << std::internal << iState
                       << std::setw(23) << std::right << std::fixed
                       << std::setprecision(8) << energiesPrev[iState]
                       << std::setw(23) << std::right << std::fixed
                       << std::setprecision(8) << energies[iState]
                       << std::setw(23) << std::right << std::fixed
                       << std::setprecision(8) << truncatedEnergy[iState]
                       << "  --> ROOT ACCEPTED";
          break;
        case EigenvalueSelection::TruncatedEnergyLower:
          maquis::cout << std::setw(13) << std::internal << iState
                       ...
                       << std::setw(23) << std::right << std::fixed
                       << std::setprecision(8) << truncatedEnergy[iState]
                       << "  --> ROOT ACCEPTED, BUT BE AWARE: truncated energy "
                          "is lower than the full FEAST MPS energy!";
          break;
    ...
    if (calculateStandardDeviation) {
      maquis::cout << " +---------------------------------------------+"
                   << std::endl;
      maquis::cout << " |   State    |    Energy standard deviation   |"
                   << std::endl;
      maquis::cout << " +---------------------------------------------+"
                   << std::endl;
      for (int iState = 0; iState < energies.size(); iState++) {
        if (accepted[iState] == EigenvalueSelection::Accepted ||
          ...
          maquis::cout << std::setw(13) << std::internal << iState
                       << std::setw(23) << std::right << std::fixed
                       << std::setprecision(8) << standardDeviations[iState]
                       << std::endl;
        } else {
          maquis::cout << std::setw(13) << std::internal << iState
                       << "             #########             --> ROOT NOT "
                          "ACCEPTED: variance not calculated"
                       << std::endl;
        }
      }
      maquis::cout << " +---------------------------------------------+"
  ...
  auto getEnergies() const { return energies; }

  /** @brief Getter for the vibrational energy */
  auto getScreenedEnergies() const { return screenedEnergies; }
  ...
  auto getOverallEnergyVariation() const {
    auto overallSum =
        std::accumulate(screenedEnergies.begin(), screenedEnergies.end(), 0.);
    auto oldOverallSum = std::accumulate(
        screenedEnergiesPrev.begin(), screenedEnergiesPrev.end(), 0.
    );
 ...
 private:
  ...
  static ComplexNumber calculateDeterminant(ComplexMatrixType inputMatrix) {
    ...
    if (info != 0) throw std::runtime_error("Error in LU decomposition");
  ...
  /** @brief Standard deviation calculator -- needed to screen the FEAST
   * eigenfunctions */
  auto getStandardDeviation(
      const MPOType& mpo, const MPSType& inputMPS, int mMax
  ) {
    auto traitClass = MPOTimesMPSTraitClass<Matrix, SymmGroup>(
        inputMPS, model, lattice, totalQN, mMax
    );
    auto outputMPS = traitClass.applyMPO(mpo);
    auto squaredEnergy =
        (overlap(outputMPS, outputMPS) +
         2. * maquis::real(overlap(outputMPS, inputMPS) * mpo.getCoreEnergy()) +
         overlap(inputMPS, inputMPS) * std::norm(mpo.getCoreEnergy())) /
        norm(inputMPS);  // this is <H^2>
  ...
  std::shared_ptr<ResultContainerType>
      mpsContainer;    // Data structure storing the result of the FEAST linear
                       // systems.
  int nStates, nQuad;  // FEAST-specific integer parameters.
  std::vector<double> energies, energiesPrev,
      screenedEnergies,  // Energies (possibly screened) at the current and
                         // previous iteration.
      screenedEnergiesPrev;
  std::vector<EigenvalueSelection> accepted;  // Eigenpairs that are accepted.
  std::vector<double> truncatedEnergy,
      standardDeviations;  // FEAST-specific double parameters for checks.
  ComplexMatrixType
      feastEigenVectors;  // FEAST --> eigenvalues transformation matrix.
  ...
  bool calculateStandardDeviation,
      screenStandardDeviation;  // If true, calculates the standard deviation
                                // for each FEAST state.
  double eMin, eMax;            // FEAST integration boundaries.
  double standardDeviationScreening;  // Screening parameter for the standard
                                      // deviation
  std::shared_ptr<VectorOfMPSs>
      currentFEASTMPSs;  // Current approximation to the FEAST eigenvalues;
