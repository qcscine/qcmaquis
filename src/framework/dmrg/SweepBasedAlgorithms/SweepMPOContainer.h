...
/**
 * @brief Class wrapping around an MPO, to be used in conjunction with
 * sweep-based algorithms.
 *
 * The scope of this function is to hide the details of how an MPOTensor is
 * constructed in a sweep-based algorithm. For the single-site case, nothing
 * needs to be done but, for the two-site case, the two-site MPO must be
 * constructed locally. This class enables not to care about this intermediate
 * step
 *
...
class SweepMPOContainer<Matrix, SymmGroup, SweepOptimizationType::SingleSite> {
 public:
  ...
  const auto& getMPO() const { return mpo_; }
...
class SweepMPOContainer<Matrix, SymmGroup, SweepOptimizationType::TwoSite> {
 public:
  ...
  /** @brief Getter for the MPOTensor */
  const auto& getMPOTensor(int site) const { return twoSiteMPOCache_[site]; }
