...
void SRCAS<ScalarType>::printSRCASSettings() {
  ...
  maquis::cout << "Fraction of proposed updates to accept is: "
               << parms_["srcas_samplingFraction"] << std::endl;
}
    ...
    for (int i = 0; i < detTmp_.size(); i++) {
      ...
      do {
        if (uniformRandomNumber_() <
            parms_["srcas_samplingFraction"])  // Only accept a fraction of the
                                               // proposed updates to stay
                                               // closer to reference det
          detTmp_[i] = poissonRandomNumber();
      } while (!(detTmp_[i] < detSpace_[i]));  // Only accept valid occupations
    }
  } else {
    maquis::cout << "SRCAS determinant generation NYI for non-vibrational "
                    "calculations! Abort!"
                 << std::endl;
    exit(1);
  }
