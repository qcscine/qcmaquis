 ...
 *
 * Class representing an algorithm that performs the time-evolution of an MPS in
 * a sweep=based fashion. From this base virtual class, two classes are derived,
 * namely the single-site and the two-sites evolution.
 */
...
class TimeEvolutionSweep {
  ...
  using SiteShifterType =
      SiteShifter<Matrix, SymmGroup, TimeEvolverType, PerturberType>;
 ...
 public:
  ...
  TimeEvolutionSweep(
    ...
    if (parms_["TD_backpropagation"] == "no") do_backpropagation_ = false;
    mps_.canonize(0);
    ...
    site_shifter_ = std::make_shared<SiteShifterType>(
        mps_, time_evolver_, perturber_, site, do_backpropagation_
    );
  }
  ...
  /**
   * @brief Virtual method that must be overloaded by different implementations
   * of the [TimeEvolutionSweep] algorithm.
   * @param sweep: index of the sweep that is currently done.
   * @param d: direction of the sweep (enum, can be LeftOnly, RightOnly or
   * Both).
   */
   ...
   */
  void performFinalOperations() { initial_site = -1; }
   ...
   */
  magnitude_type get_energy() { return energy; }

 protected:
  /**
   * @brief Method that builds the boundaries for the initial MPS.
   * Calls the same methods that are called when the boundaries are moved one
  ...
  /**
   * @brief Given the MPS and the left boundaries at site i, calculates the new
   * boundaries at site i+1.
   * @param mpo: MPO representation of the operator associated to the
   * boundaries.
  ...
  inline void boundary_left_step(MPO<Matrix, SymmGroup> const& mpo, int site) {
    left_[site + 1] = contr::overlap_mpo_left_step(
        mps_[site], mps_[site], left_[site], mpo_[site]
    );
  }

  /**
   * @brief Given the MPS and the tight boundaries at site i+1, calculates the
   * new boundaries at site i.
   * @param mpo: MPO representation of the operator associated to the
   * boundaries.
   * @param site: site for which the boundaries are shifted.
   */
  inline void boundary_right_step(MPO<Matrix, SymmGroup> const& mpo, int site) {
    right_[site] = contr::overlap_mpo_right_step(
        mps_[site], mps_[site], right_[site + 1], mpo_[site]
    );
  }
   ...
   *
   * This method retrieves, for a given sweep, the value of the threshold to be
   * used in the truncation of the MPS. Note that, for the first ngrowsweeps,
   * the threshold is calculated according to a logarithmic decay between the
   * parameters truncation_initial and truncation_main. After that, the
   * threshold is set to truncation_main and kept constant.
   *
  ...
  /**
   * @brief Retrieves the maximum value of the bond dimension that is employed
   * in the MPS truncation.
  ...
  std::shared_ptr<TimeEvolver<Matrix, SymmGroup, BaseParameters> >
      time_evolver_;
  std::shared_ptr<SiteShifter<
      Matrix, SymmGroup, TimeEvolver<Matrix, SymmGroup, BaseParameters>,
      PerturberType> >
      site_shifter_;
