...
/** @brief Functor class for the StoreToFile method, specialized for the
 * boundaries */
template <class Matrix, class SymmGroup>
class StoreToFile_request<Boundary<Matrix, SymmGroup>> {
 public:
  /** @brief Class constructor */
  StoreToFile_request(std::string fp, Boundary<Matrix, SymmGroup>* ptr)
      : fp(std::move(fp)), ptr(ptr) {}
    ...
    for (int b = 0; b < loop_max; ++b) {
      ...
      for (int k = 0; k < o[b].n_blocks(); ++k) {
        ...
        for (int c = 0; c < num_cols(m); ++c)
          ofs.write(
              (char*)(&m(0, c)),
              num_rows(m) * sizeof(typename Matrix::value_type) / sizeof(char)
          );
        m = Matrix();
      }
    }
 ...
 public:
  ...
  void operator()() {
    ...
    for (int b = 0; b < loop_max; ++b) {
      for (int k = 0; k < o[b].n_blocks(); ++k) {
        o[b][k] =
            Matrix(o[b].left_basis()[k].second, o[b].right_basis()[k].second);
        Matrix& m = o[b][k];
        ifs.read(
            (char*)(&m(0, 0)), num_cols(m) * num_rows(m) *
 ...
 public:
  ...
  void operator()() {
    ...
    for (int b = 0; b < o.aux_dim(); ++b)
      for (int k = 0; k < o[b].n_blocks(); ++k) o[b][k] = Matrix();
  }
 ...
 *    a given status (describing whether it's currently being fetched, stored
 * etc) and a given std::thread that manages it.
 *
 */
 ...
 public:
  ...
  class descriptor {
   public:
    /** @brief Class constructor */
    descriptor()
        : state(core), dumped(false), sid(disk::index()), worker(nullptr) {}

    /**
     * @brief Class destructor.
     * Note that the descructor calls the join method on the std::thread --
     * i.e., completes the operation associated with the std::thread.
     */
    ~descriptor() { this->join(); }

    /**
     * @brief Associated the object to a given thread.
     * Note that this method calls the disk::track method, which ensures that
     * the corresponding thread is being "followed" by the memory manager.
     */
     ...
     * Similarly to the [thread] method, also here the disk::untrack method is
     * called under the hood in order to not follow anymore this thread.
     */
    void join() {
      if (this->worker) {
        this->worker->join();
    ...
    /** Class members */
    bool dumped;  // Bool keeping track of whether the object has been written
                  // to disk.
   ...
   *
   * The class is inherited by [descriptor] such that the object is "equipped"
   * with the std::thread and with the flags describing its storing status.
   *
  ...
  class serializable : public descriptor {
   public:
     ...
     *
     * Note that sid is a member of the mother class and is the unique
     * identified associated with the object to be serialized
     */
    ~serializable() {
    ...
    void prefetch() {
      ...
      // stored, finalized the storing such that afterwards one can call
      // "fetch".
    ...
    void StoreToFile() {
      if (state == core) {
        ...
        parallel::sync();
        this->thread(
            new std::thread(StoreToFile_request<T>(disk::fp(sid), (T*)this))
        );
      }
  ...
  static size_t index() { return instance().sid++; }
  ...
  static void untrack(descriptor* d) { instance().queue[d->record] = nullptr; }
  ...
  /** @brief Constructor for a disk (but should be made private in agreement
   * with the singleton)*/
 ...
 * This method chooses -- depending on the input parameters -- whether to
 * activate in-disk storage or not. Note that this method has to be called EVERY
 * TIME a new calculation is started, otherwise the singleton is not reset. For
...
inline static void setup(BaseParameters& parms) {
  ...
  if (!parms["storagedir"].empty() && has_environment) {
    ...
    try {
      ...
      maquis::cerr << "Error creating dir/file at " << dp
