...
inline std::string get_symmetry(const std::string& chkpfile) {
...
// check whether the checkpoint chkpfile has the same symmetry as is declared in
// parms removed guess_alps_symmetry as it is irrelevant for QCMaquis Why is
// parms not const&, this could be potentially dangerous
...
inline void symmetry_check(BaseParameters& parms, const std::string& chkpfile) {
  ...
  if (parms["symmetry"].str() != chkp_sym) {
    throw std::runtime_error(
        "The existing checkpoint file " + chkpfile +
        " has the wrong symmetry group " + chkp_sym + " instead of " +
        parms["symmetry"].str() + "\n"
    );
  }
}
...
void right_end_check(
    const std::string& filename, MPS<Matrix, SymmGroup> const& mps,
    typename SymmGroup::charge right_end
) {
  if (right_end != mps[mps.size() - 1].col_dim()[0].first) {
    std::stringstream parm_sector;
    parm_sector << right_end;
    std::stringstream mps_sector;
    mps_sector << mps[mps.size() - 1].col_dim()[0].first;
    throw std::runtime_error(
        "The existing checkpoint file " + filename +
...
template <class Matrix, class SymmGroup>
void right_end_check(
    MPS<Matrix, SymmGroup> const& mps, typename SymmGroup::charge right_end
) {
  if (right_end != mps[mps.size() - 1].col_dim()[0].first) {
    std::stringstream parm_sector;
    parm_sector << right_end;
    ...
    mps_sector << mps[mps.size() - 1].col_dim()[0].first;
    throw std::runtime_error(
        "The given MPS has a different target symmetry sector (" +
        mps_sector.str() + ") compared to the input (" + parm_sector.str() +
        ")\n"
    );
  }
}

inline void orbital_order_check(
    BaseParameters& parms, const std::string& chkpfile
) {
  storage::archive ar_in(chkpfile + "/props.h5");
  BaseParameters chkp_parms;
  ar_in["/parameters"] >> chkp_parms;
  ...
  if (chkp_parms.defined("orbital_order")) {
    ...
    if (parms.is_set("orbital_order")) {
      ...
      // throw std::runtime_error("The existing checkpoint file " + chkpfile +
      // " has the wrong orbital order " + chkp_order + "\n");
    } else {
      // make sure we apply the orbital order previously used for this MPS
      parms.set("orbital_order", chkp_order);
    }
