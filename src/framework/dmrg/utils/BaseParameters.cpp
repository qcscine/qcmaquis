...
using parameters::proxy;

class BaseParameters::Impl : public alps::Parameters {
  using base = alps::Parameters;
  using base::base;
};

BaseParameters::BaseParameters() : impl_(new Impl()) {}
BaseParameters::BaseParameters(const BaseParameters& p)
    : defaults(p.defaults),
      descriptions(p.descriptions),
      impl_(new Impl(*(p.impl_))) {}

BaseParameters::BaseParameters(std::istream& param_file) {
  try {
    impl_ = std::make_unique<Impl>(param_file);
...
BaseParameters& BaseParameters::operator=(BaseParameters&& rhs) noexcept {
  swap(*this, rhs);
  return *this;
}
...
void BaseParameters::print_description(Stream& os) const {
  using map_iterator = std::map<std::string, std::string>::const_iterator;

  std::size_t column_length = 0;
  for (const auto& description : descriptions) {
    column_length = std::max(column_length, description.first.size());
  }
  column_length += 2;

  for (const auto& description : descriptions) {
    os << std::setw(column_length) << std::left << description.first
       << description.second << std::endl;
    map_iterator matched = defaults.find(description.first);
    if (matched != defaults.end()) {
      os << std::setw(column_length) << ""
         << "(default: " << matched->second << ")" << std::endl;
    }
  }
}
...
bool BaseParameters::empty() const { return impl_->empty(); }
...
BaseParameters BaseParameters::iteration_params(
    std::string const& var, std::size_t val
) {
  BaseParameters p;

  std::regex expression("^(.*)\\[" + var + "\\]$");
  std::smatch what;
  for (alps::Parameters::const_iterator it = impl_->begin(); it != impl_->end();
       ++it) {
    std::string key = it->key();
    if (std::regex_match(key, what, expression)) {
      std::vector<std::string> v =
          (*this)[key];  // use std::strign instead of value type, because value
                         // type is some alps internal type that can anyway be
...
BaseParameters& BaseParameters::operator<<(BaseParameters const& p) {
  for (alps::Parameters::const_iterator it = p.impl_->begin();
       it != p.impl_->end(); ++it) {
...
void BaseParameters::add_option(
...
// for now, an implementation that copies the whole range. consider using
// references
std::list<typename BaseParameters::value_type> BaseParameters::get_range(
) const {
...
template void BaseParameters::save<alps::hdf5::archive>(alps::hdf5::archive& ar
) const;

#define INSTANTIATE_TEMPLATE_FUNCTIONS(r, d, T)              \
  template T BaseParameters::get<T>(std::string const& key); \
  template void BaseParameters::set<T>(                      \
      std::string const& key, T const& value                 \
  );                                                         \
  template BaseParameters& BaseParameters::add<T>(           \
      std::string const& key, T const& value                 \
  );
...
template std::vector<int> BaseParameters::get<std::vector<int> >(
    std::string const& key
);
template std::vector<unsigned long>
BaseParameters::get<std::vector<unsigned long> >(std::string const& key);
