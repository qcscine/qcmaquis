...
void track(block_matrix<Matrix, SymmGroup>& t, const std::string& label) {
  for (int i = 0; i < t.n_blocks(); ++i) ambient::track(t[i], label);
  t.label = label;
}
