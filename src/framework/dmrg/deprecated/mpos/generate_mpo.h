...
size_t next_free(set<size_t> const& s) {
  for (size_t k = 2; true; ++k)
    if (s.count(k) == 0) return k;
}

template <class Matrix, class SymmGroup>
void compress_on_bond(
    std::vector<std::tuple<size_t, size_t, block_matrix<Matrix, SymmGroup> > >&
        pm1,
    std::vector<std::tuple<size_t, size_t, block_matrix<Matrix, SymmGroup> > >&
        pm2
) {
  ...
  //        maquis::cout << "Compression: " <<
  //        *max_element(bond_used_dims.begin(),
  //                                                bond_used_dims.end()) << "
  //                                                -> " <<
  //                                                bond_used_dims.size() <<
  //                                                std::endl;
...
template <class Matrix, class SymmGroup>
std::pair<size_t, size_t> rcdim(
    std::vector<
        std::tuple<size_t, size_t, block_matrix<Matrix, SymmGroup> > > const& pm
) {
  typedef block_matrix<Matrix, SymmGroup> op_t;
  typedef std::tuple<size_t, size_t, op_t> block;
  ...
  for (typename vector<block>::const_iterator it = pm.begin(); it != pm.end();
       ++it) {
  ...
  return make_pair(
      *max_element(l.begin(), l.end()) + 1, *max_element(r.begin(), r.end()) + 1
  );
}

template <class Matrix, class SymmGroup>
bool compare(
    pair<size_t, block_matrix<Matrix, SymmGroup> > const& p1,
...
class CorrMaker {
  ...
  typedef std::tuple<size_t, size_t, op_t> block;
  typedef vector<
      pair<block_matrix<Matrix, SymmGroup>, block_matrix<Matrix, SymmGroup> > >
      op_pairs;
  typedef std::tuple<size_t, size_t, string> tag;

 public:
  CorrMaker(
      std::size_t L, op_t const& identity_, op_t const& fill_,
      std::vector<op_t> const& ops_, int ref = -1
  )
      : prempo(L),
        tags(L),
        used(L),
  ...
  MPO<Matrix, SymmGroup> create_mpo() {
    //            for (typename vector<vector<block> >::iterator it =
    //            prempo.begin();
    //                 it + 1 != prempo.end();
    //                 ++it)
    ...
    //                std::ostream_iterator<size_t>(maquis::cout, " "));
    ...
    //                for (typename vector<tag>::const_iterator it =
    //                tags[p].begin(); it != tags[p].end(); ++it)
    //                    maquis::cout << "    " << get<0>(*it) << " " <<
    //                    get<1>(*it) << " " << get<2>(*it) << std::endl;
    //            }

    MPO<Matrix, SymmGroup> r(prempo.size());
    for (size_t p = 1; p < prempo.size() - 1; ++p) r[p] = as_bulk(prempo[p]);
  ...
  vector<string> label_strings() {
    ...
    for (vector<vector<size_t> >::iterator it = labels.begin();
      ...
      ostringstream oss;
      oss << "( ";
      for (vector<size_t>::iterator it2 = it->begin(); it2 != it->end();
           ++it2) {
        oss << *it2;
        if (it2 + 1 != it->end()) oss << " ) -- ( ";
      }
 ...
 private:
  ...
  size_t term(size_t p, size_t u1, op_t const& op, bool trivial) {
    ...
    used[p].insert(u2);
    //            maquis::cout << "Adding a " << (trivial ? "trivial " : "") <<
    //            "term at " << p << ", " << u1 << " -> " << u2 << std::endl;
    if (trivial)
  ...
  void recurse(
      size_t p0, size_t which, size_t use, vector<size_t> label, int ref
  ) {
    if (p0 + ops.size() - which < prempo.size()) {
      size_t use_next = term(p0, use, which == 0 ? identity : fill, true);
...
class MPOMaker {
 ...
 public:
  MPOMaker(adj::Adjacency const& adj_, Hamiltonian<Matrix, SymmGroup>& H_)
      : adj(adj_),
        H(H_),
        used_dims(adj.size()),
        prempo(adj.size()),
  ...
  void add_bond_ops() {
    for (int p = 0; p < adj.size(); ++p) {
      ...
      for (vector<int>::iterator neigh = neighs.begin(); neigh != neighs.end();
           ++neigh)
  ...
  void add_term(vector<pair<int, op_t> >& ops) {
    ...
    for (typename vector<pair<int, op_t> >::const_iterator it = ops.begin();
      ...
      prempo[it->first].push_back(
          std::make_tuple(first_use_b, second_use_b, it->second)
      );
  ...
  MPO<Matrix, SymmGroup> create_mpo() {
    ...
    for (int p = 1; p < adj.size() - 1; ++p) r[p] = as_bulk(prempo[p]);
    r[0] = as_left(prempo[0]);
