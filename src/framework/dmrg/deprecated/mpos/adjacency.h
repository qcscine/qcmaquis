...
class ChainAdj : public Adjacency {
 public:
  ...
  std::vector<int> all(int p) const {
    std::vector<int> ret;
    if (p < L_ - 1) ret.push_back(p + 1);
    if (p > 0) ret.push_back(p - 1);
    return ret;
  }
...
class SquareAdj : public Adjacency {
 public:
  SquareAdj(int L, int W) : L_(L), W_(W) {}
  ...
  std::vector<int> forward(int p) const {
    std::vector<int> ret;
    if (p + 1 < L_ * W_ && (p + 1) % W_ != 0) ret.push_back(p + 1);
    if (p + W_ < L_ * W_) ret.push_back(p + W_);

    //        maquis::cout << p << " -> ";
    //        std::copy(ret.begin(), ret.end(),
    //        std::ostream_iterator<int>(maquis::cout, " ")); maquis::cout <<
    //        std::endl;
  ...
  std::vector<int> all(int p) const {
    ...
    if (p >= 1 && p % W_ != 0) ret.push_back(p - 1);
    if (p >= W_) ret.push_back(p - W_);
...
#define wrap(pp)                           \
  {                                        \
    ...
    if ((p + 1) % W_ != 0) {
      ...
      if (p_in_bump / W_ == 0)
        wrap(p + 2 * W_ - 1 - 2 * p_in_bump) else wrap(
            p + 1 + 2 * (2 * W_ - p_in_bump - 1)
        )
    }
#undef wrap
...
class CylinderAdj : public Adjacency {
 public:
  CylinderAdj(int L, int W) : L_(L), W_(W) {}

  std::vector<int> forward(int p) const {
    std::vector<int> ret;

    // down
    if ((p + 1) % W_ != 0) ret.push_back(p + 1);

    // right
    if (p + W_ < L_ * W_) ret.push_back(p + W_);

    // around bad direction
    if ((p + 1) % W_ == 0) ret.push_back(p - W_ + 1);
...
class PeriodicSquareLatticeAdj : public Adjacency {
 public:
  PeriodicSquareLatticeAdj(int L, int W) : L_(L), W_(W) {}
