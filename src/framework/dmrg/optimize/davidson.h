...
// Davidson algorithm adapted from the IETL Jacobi-Davidson implementation in
// ALPS
...
class davidson {
 public:
  ...
  std::pair<magnitude_type, vector_type> calculate_eigenvalue(
      const GEN& gen, SOLVER& solver, PRECOND& mdiag, ITER& iter
  );
...
davidson<MATRIX, VS>::davidson(
    ...
    : matrix_(matrix), vecspace_(vec), desired_(desired) {}

template <class MATRIX, class VS>
template <class GEN, class SOLVER, class PRECOND, class ITER>
std::pair<
    typename davidson<MATRIX, VS>::magnitude_type,
    typename davidson<MATRIX, VS>::vector_type>
davidson<MATRIX, VS>::calculate_eigenvalue(
    const GEN& gen, SOLVER& solver, PRECOND& mdiag, ITER& iter
) {
  ...
  do {
    ...
    tau = ietl::two_norm(t);
    for (i = 0; i < V.size(); i++) t -= ietl::dot(V[i], t) * V[i];
    ...
    ++iter;
    if (iter.finished(ietl::two_norm(r), Mevals[0])) break;
