...
class TrivialGroup {
 public:
  ...
  template <int R>
  static charge fuse(std::array<charge, R>) {
    return Plus;
  }
};
...
class hash<std::pair<TrivialGroup::charge,TrivialGroup::charge > >{
    public :
        size_t operator()(std::pair<TrivialGroup::charge, TrivialGroup::charge>
const &Pair_of_charge ) const { return boost::hash_value(Pair_of_charge);
        }
};
*/
};  // namespace boost
