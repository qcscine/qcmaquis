...
class ZqCharge {
  ...
  friend bool operator<(ZqCharge<Q> a, ZqCharge<Q> b) { return a.c_ < b.c_; }
...
class Zq {
 public:
  ...
  static charge fuse(const std::array<charge, R>& v) {
    charge ret = v[0];
    for (int i = 1; i < R; i++) ret = fuse(ret, v[i]);
    return ret;
  }
};

template <int Q>
const typename Zq<Q>::charge Zq<Q>::IdentityCharge = ZqCharge<Q>(0);
