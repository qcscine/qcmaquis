...
class Index : protected std::vector<
  ...
  using base_t =
      std::vector<std::pair<typename SymmGroup::charge, std::size_t>>;
  using pos_t =
      boost::container::flat_map<typename SymmGroup::charge, std::size_t>;
 ...
 private:
  ...
  std::size_t destination(charge c) const {
    return std::find_if(
               this->begin(), this->end(),
               boost::lambda::bind(
                   index_detail::lt<SymmGroup>, boost::lambda::_1,
                   std::make_pair(c, 0)
               )
           ) -
           this->begin();
