...
std::string identify_op(
    typename operator_selector<Matrix, SymmGroup>::type const& m
) {
  if (m.n_blocks() == 2) {
    return "I";
  } else {
    typename SymmGroup::charge c1 = m.basis().left_charge(0);
    typename SymmGroup::charge c2 = m.basis().right_charge(0);
    if (c1 == 1 && c2 == 0) {
      return "c";
    } else if (c1 == 0 && c2 == 1) {
      return "d";
...
template <class Matrix, class SymmGroup>
void follow_mpo(
    MPO<Matrix, SymmGroup> const& mpo, const std::string& s = std::string(),
    int p = 0, int start = 0
) {
...
template <class Matrix, class SymmGroup>
void follow_and_print_terms(
    MPO<Matrix, SymmGroup> const& mpo, int p, int b1, int b2,
    const std::string& s = "",
  ...
  for (typename row_proxy::const_iterator row_it = myrow.begin();
       row_it != myrow.end(); ++row_it) {
...
template <class Matrix, class SymmGroup>
void cleanup_mpo_(
    MPO<Matrix, SymmGroup> const& in_mpo, MPO<Matrix, SymmGroup>& out_mpo,
    std::vector<std::tuple<
        int, int, typename operator_selector<Matrix, SymmGroup>::type> >& ops,
    int p, int start
) {
  for (std::size_t k = 0; k < in_mpo[p].col_dim(); ++k) {
    ...
    ops[p] = std::make_tuple(
        start, k, in_mpo[p].at(start, k).op() * in_mpo[p].at(start, k).scale()
    );
    ...
    if (p + 1 < in_mpo.length()) {
      ...
      for (std::size_t t = 0; t < in_mpo.length(); ++t) {
        MPOTensor_detail::term_descriptor<Matrix, SymmGroup, false> o =
            out_mpo[t].at(get<0>(ops[t]), get<1>(ops[t]));
        if (o.op().n_blocks() == 0) {
          o.op() = get<2>(ops[t]);
          o.scale() = 1.;
        }
...
MPO<Matrix, SymmGroup> cleanup_mpo(MPO<Matrix, SymmGroup> const& mpo) {
  ...
  std::vector<std::tuple<
      int, int, typename operator_selector<Matrix, SymmGroup>::type> >
      prempo(mpo.length());
...
MPO<Matrix, SymmGroup> square_mpo(MPO<Matrix, SymmGroup> const& mpo) {
  ...
  for (size_t p = 0; p < L; ++p) {
    ...
    maquis::cout << "MPOTensor " << inp.row_dim() * inp.row_dim() << " "
                 << inp.col_dim() * inp.col_dim() << std::endl;
    MPOTensor<Matrix, SymmGroup> ret(
        inp.row_dim() * inp.row_dim(), inp.col_dim() * inp.col_dim()
    );
    ...
    for (index_type r1 = 0; r1 < inp.row_dim(); ++r1) {
      ...
      for (index_type r2 = 0; r2 < inp.row_dim(); ++r2) {
        ...
        for (typename row_proxy::const_iterator it1 = row1.begin();
             it1 != row1.end(); ++it1) {
          index_type c1 = it1.index();
          for (typename row_proxy::const_iterator it2 = row2.begin();
               it2 != row2.end(); ++it2) {
            ...
            if (t.n_blocks() > 0) {
              ret.set(
                  r1 * inp.row_dim() + r2, c1 * inp.col_dim() + c2,
