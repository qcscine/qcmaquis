...
// see eq. 44
template <class Matrix, class SymmGroup>
void scale_MPSTensor(
    MPSTensor<Matrix, SymmGroup>& mps, typename Matrix::value_type tjj
) {
  ...
  // loop over blocks in MPS tensor (remember MPS tensor has a block matrix
  // structure)
  for (size_t block = 0; block < m1.n_blocks(); ++block) {
    // loop over physical charges (aka local states); nothing needs to be done
    // for charge <0,0>
    for (size_t i = 0; i < physical_i.size(); i++) {
      ...
      size_t l = left_i.position(
          SymmGroup::fuse(m1.basis().left_charge(block), -phys_charge)
      );
      // if left charge does not exist (meaning that there is no matching input
      // charge in this block matrix)
      ...
      for (size_t k = 0; k < m1[block].num_cols(); k++)
        ...
        // boost::numeric::bindings::blas::detail::scal(lsize, tjj_local,
        // &(*m1[block].col(k).first), 1);
...
struct setupMPO {
  std::vector<MPO<Matrix, SymmGroup> > operator()(
      const Matrix& t, int j, const Lattice& lat,
      const Model<Matrix, SymmGroup>& model
  ) {
    ...
    for (int i = 0; i < t.num_rows(); ++i)
      if (i != j) {
        ...
        operators_up.push_back(
            model.get_operator_tag("create_up", lat.get_prop<sc_t>("type", i))
        );
        operators_up.push_back(
            model.get_operator_tag("destroy_up", lat.get_prop<sc_t>("type", j))
        );
        operators_down.push_back(model.get_operator_tag(
            "create_down_for_meas", lat.get_prop<sc_t>("type", i)
        ));
        operators_down.push_back(model.get_operator_tag(
...
{
    std::vector<MPO<Matrix, SymmGroup> > operator()(const Matrix & t,
std::size_t j, const Lattice& lat, const Model<Matrix, SymmGroup> & model)
    {
        ...
        for (Lattice::pos_t p=0; p < lat.size(); ++p)
            max_irrep = (lat.get_prop<typename SymmGroup::subcharge>("type", p)
> max_irrep) ? lat.get_prop<typename SymmGroup::subcharge>("type", p) :
max_irrep;

        typename TermMakerSU2<Matrix, SymmGroup>::Operators ops =
TermMakerSU2<Matrix, SymmGroup>::construct_operators(max_irrep,
model.operators_table()); typename TermMakerSU2<Matrix,
SymmGroup>::OperatorCollection op_collection = TermMakerSU2<Matrix,
SymmGroup>::construct_operator_collection(ops, max_irrep);

        for (std::size_t i = 0; i < t.num_rows(); ++i)
            if (i != j)
            {
                std::vector<typename Model<Matrix,SymmGroup>::term_descriptor>
terms;
                // copy-paste from 1-RDM expval from tagged_nrankrdm.h
                // The sqrt(2.) balances the magnitudes of Clebsch coeffs C^{1/2
1/2 0}_{mrm'} which apply at the second spin-1/2 operator
                terms.push_back(TermMakerSU2<Matrix,
SymmGroup>::positional_two_term( true, op_collection.ident.no_couple,
t(i,j)/t(j,j)*std::sqrt(2.), i, j, op_collection.create.couple_down,
op_collection.create.fill_couple_up, op_collection.destroy.couple_down,
op_collection.destroy.fill_couple_up, lat
                ));


                // check if term is allowed by symmetry
                // if(not measurements_details::checkpg<SymmGroup>()(terms[0],
model.operators_table(), lat))
                //         continue;

                generate_mpo::TaggedMPOMaker<Matrix, SymmGroup> mpo_m(lat,
op_collection.ident.no_couple, op_collection.ident_full.no_couple,
                                                                        op_collection.fill.no_couple,
model.operators_table(), terms); ret.push_back(mpo_m.create_mpo());
            }
...
// function to calculate MPS' = MPO|MPS> aka (in CI terminology) calculating the
// sigma vector: sigma = H*C as used e.g. in Eqs. 46 and 48
template <class Matrix, class SymmGroup>
MPS<Matrix, SymmGroup> MPS_sigma_vector_product(
    MPS<Matrix, SymmGroup> const& mps,
    std::vector<MPO<Matrix, SymmGroup> > const& mpo_vec
) {
  // mpo_times_mps_contractor_ss<Matrix, SymmGroup, storage::nop>
  // sigma_vector_product(mps, mpo, parms); sigma_vector_product.sweep();
  assert(sensible(mps));
  MPS<Matrix, SymmGroup> ret;
  for (size_t i = 0; i < mpo_vec.size(); ++i) {
    // debug::mps_print(mps, "before round " +
    ...
    for (int p = 0; p < mps.size(); ++p)
      product[p] =
          MPOTimesMPSTraitClass<Matrix, SymmGroup>::mpo_times_mps_singleop(
              mpo_vec[i][p], mps[p], delta
          );
...
void compress_mps(MPS<Matrix, SymmGroup>& mps, const std::string& text = "") {
  ...
  maquis::cout << "- (Relative) norm reduction : " << compression_trace
               << std::endl;
...
template <class Matrix, class SymmGroup>
void rotate_mps(
    MPS<Matrix, SymmGroup>& mps, const Matrix& t,
    typename Matrix::value_type inactive_scaling
) {
  using pos_t = Lattice::pos_t;
  ...
  for (pos_t j = 0; j < L; ++j) {
    maquis::cout << "ROTATION of site " << j << std::endl
                 << "---------------- " << std::endl;
    ...
    std::vector<MPO<Matrix, SymmGroup> > MPO_vec =
        setupMPO<Matrix, SymmGroup>()(t, j, lat, model);
    ...
    mps_prime_prime =
        MPS_sigma_vector_product<Matrix, SymmGroup>(mps_prime, MPO_vec);
    ...
    //
    maquis::cout << "-  Final (for the current site to be rotated) MPS with no "
                    "compression   - "
                 << std::endl;
    // debug::mps_print_ci(mps, "dets.txt");
    ...
    maquis::cout << "-  Final (for the current site to be rotated) MPS with "
                    "full compression - "
                 << std::endl;
    // debug::mps_print_ci(mps, "dets.txt");
    // debug::mps_print(mps, "Final (for the current site to be rotated) MPS at
    // site ");
  }
}
