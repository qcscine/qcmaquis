...
block_matrix<Matrix, SymmGroup> make_mpo_identity_block(Index<SymmGroup> phys_i
  ...
  for (std::size_t k = 0; k < phys_i.size(); ++k)
    ret.insert_block(
        Matrix(phys_i[k].second, phys_i[k].second), phys_i[k].first,
        phys_i[k].first
    );
  ret *= 0;
  return ret;
...
MPOTensor<Matrix, SymmGroup> identity_mpo(Index<SymmGroup> phys_i) {
  ...
  for (bit up = phys_i.basis_begin(); !up.end(); ++up) mpo(0, 0, *up, *up) = 1;
...
MPO<Matrix, TrivialGroup> s12_ising(std::size_t L, double J, double h) {
  ...
  Index<TrivialGroup> phys_i;
  phys_i.insert(std::make_pair(TrivialGroup::Plus, 2));
  Index<TrivialGroup> triv;
  triv.insert(std::make_pair(TrivialGroup::Plus, 1));
  Index<TrivialGroup> aux;
  aux.insert(std::make_pair(TrivialGroup::Plus, 4));
