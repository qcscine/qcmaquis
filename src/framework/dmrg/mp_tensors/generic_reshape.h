...
void reshape(
    MultiIndex<SymmGroup> const& midx,
    ...
    for (size_t block = 0; block < m1.n_blocks(); ++block) {
      ...
      for (size_t i = 0; i < num_rows(in_block); ++i)
        for (size_t j = 0; j < num_cols(in_block); ++j) {
          ...
          std::tie(out_left, out_right) =
              midx.convert_coords(in_set, in_left, in_right, out_set);

          if (in_block(i, j) != 0.) {
            if (pretend)
              m2.reserve(
                  out_left.first, out_right.first,
                  midx.left_size(out_set, out_left.first),
                  midx.right_size(out_set, out_right.first)
              );
            else
...
void reshape2(
  ...
  for (int run = 0; run < 2; ++run) {
    ...
    for (index_product_iterator<SymmGroup> it = midx.begin(); it != midx.end();
         it++) {
      coord_t in_left, in_right;
      std::tie(in_left, in_right) = midx.get_coords(in_set, *it);

      if (!m1.has_block(in_left.first, in_right.first)) continue;
