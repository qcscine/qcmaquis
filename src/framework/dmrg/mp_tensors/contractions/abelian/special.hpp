...
MPSTensor<Matrix, SymmGroup> site_ortho_boundaries(
  ...
  reshape_right_to_left_new(
      mps.site_dim(), ortho_left.left_basis(), ortho_mps.col_dim(), t, t2
  );
  gemm(t2, transpose(ortho_right), t3);

  mps.make_left_paired();
  t = mps.data();
  reshape_and_pad_left(
      mps.site_dim(), ortho_left.left_basis(), ortho_right.left_basis(),
      mps.row_dim(), mps.col_dim(), t3, t
  );

  MPSTensor<Matrix, SymmGroup> t4(
      mps.site_dim(), mps.row_dim(), mps.col_dim(), t, LeftPaired
  );
  return t4;
}

template <class Matrix, class SymmGroup>
block_matrix<Matrix, SymmGroup> multiply_with_twosite(
    block_matrix<Matrix, SymmGroup> const& vec,
    typename operator_selector<Matrix, SymmGroup>::type const& op,
    Index<SymmGroup> const& left_i, Index<SymmGroup> const& right_i,
    Index<SymmGroup> const& phys_i
) {
  using charge = typename SymmGroup::charge;

  Index<SymmGroup> l_index = phys_i * left_i,
                   r_index = adjoin(phys_i) * right_i;
        ...
        for (size_t rps = 0; rps < phys_i.size(); ++rps)
          for (size_t ilps = 0; ilps < phys_i.size(); ++ilps)
            for (size_t irps = 0; irps < phys_i.size(); ++irps) {
              ...
              if (left_out_charge != right_out_charge) continue;
              if (left_vec_charge != right_vec_charge) continue;
              if (!vec.has_block(left_vec_charge, right_vec_charge)) continue;
              ...
              if (!ret.has_block(left_out_charge, right_out_charge))
                ret.insert_block(
                    Matrix(
                        l_index.size_of_block(left_out_charge),
                        r_index.size_of_block(right_out_charge), 0
                    ),
              ...
              maquis::dmrg::detail::mwt(
                  ret(left_out_charge, right_out_charge),
                  vec(left_vec_charge, right_vec_charge),
                  op(both_charge, both_charge), left_pb(lpc, lc),
                  right_pb(rpc, rc), left_pb(ilpc, lc), right_pb(irpc, rc),
                  phys_pb(ilpc, irpc), phys_pb(lpc, rpc), left_i[ls].second,
                  right_i[rs].second, phys_i[lps].second, phys_i[rps].second,
...
template <class Matrix, class SymmGroup>
MPSTensor<Matrix, SymmGroup> multiply_with_op(
    MPSTensor<Matrix, SymmGroup> const& mps,
    typename operator_selector<Matrix, SymmGroup>::type const& op
) {
  using charge = typename SymmGroup::charge;
    ...
    for (size_t rs = 0; rs < right_i.size(); ++rs)
      for (size_t s1 = 0; s1 < phys_i.size(); ++s1)
        for (size_t s2 = 0; s2 < phys_i.size(); ++s2) {
          ...
          if (!op.has_block(s1c, s2c)) continue;
          ...
          if (!out_left_i.has(lc)) out_left_i.insert(left_i[ls]);
          ...
          if (!ret_vec.has_block(left_out_charge, right_out_charge))
            ret_vec.insert_block(
                new Matrix(left_pb.size(s2c, lc), right_i[rs].second, 0),
                left_out_charge, right_out_charge
            );
          ...
          for (size_t ss1 = 0; ss1 < phys_i[s1].second; ++ss1) {
            for (size_t ss2 = 0; ss2 < phys_i[s2].second; ++ss2) {
              ...
              typename Matrix::value_type const& op_val =
                  op_block(i_op_offset + ss1, op_offset + ss2);

              // TODO: replace by kernel
              for (size_t rr = 0; rr < right_i[rs].second; ++rr) {
                for (size_t ll = 0; ll < left_i[ls].second; ++ll) {
                  oblock(o_l_start + ll, o_r_start + rr) +=
                      iblock(i_l_start + ll, i_r_start + rr) * op_val;
                }
              }
            }
          }
  ...
  MPSTensor<Matrix, SymmGroup> ret(
      phys_i, out_left_i, ret_vec.right_basis(), ret_vec, LeftPaired
  );
...
template <class Matrix, class SymmGroup>
MPSTensor<Matrix, SymmGroup> local_op(
    MPSTensor<Matrix, SymmGroup> const& mps,
    typename operator_selector<Matrix, SymmGroup>::type const& op
) {
  using charge = typename SymmGroup::charge;
...
#ifndef NDEBUG
                  oblock(
                      l_offset + ss2 * left_i[ls].second + ll, r_offset + rr
                  );
                  iblock(
                      i_l_offset + ss1 * left_i[ls].second + ll, i_r_offset + rr
                  );
                  op_block(i_op_offset + ss1, op_offset + ss2);
#endif
                  oblock(
                      ...
                      iblock(
                          i_l_offset + ss1 * left_i[ls].second + ll,
                          i_r_offset + rr
                      ) *
                      op_block(i_op_offset + ss1, op_offset + ss2);
                }
...
template <class Matrix, class SymmGroup>
block_matrix<Matrix, SymmGroup> density_matrix(
    ...
    for (size_t rs = 0; rs < right_i.size(); ++rs)
      for (size_t s1 = 0; s1 < phys_i.size(); ++s1)
        for (size_t s2 = 0; s2 < phys_i.size(); ++s2) {
          ...
          if (!ket_vec.has_block(left_ket_charge, right_charge)) continue;
          if (!bra_vec.has_block(left_bra_charge, right_charge)) continue;
          ...
          for (size_t ll = 0; ll < left_i[ls].second; ++ll)
            for (size_t rr = 0; rr < right_i[rs].second; ++rr)
              for (size_t ss1 = 0; ss1 < phys_i[s1].second; ++ss1)
                for (size_t ss2 = 0; ss2 < phys_i[s2].second; ++ss2) {
                  oblock(ss1, ss2) +=
                      utils::conj(bra_block(
                          l_bra_offset + ss2 * left_i[ls].second + ll, rr
                      )) *
                      ket_block(
                          l_ket_offset + ss1 * left_i[ls].second + ll, rr
                      );
                }
        }
  return ret;
...
template <class Matrix, class SymmGroup>
block_matrix<Matrix, SymmGroup> density_matrix_2(
    MPSTensor<Matrix, SymmGroup> const& bra_tensor,
    MPSTensor<Matrix, SymmGroup> const& ket_tensor
) {
  using charge = typename SymmGroup::charge;
