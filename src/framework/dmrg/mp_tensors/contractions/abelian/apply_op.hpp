...
void lbtm_kernel_allocate(
  ...
  for (typename col_proxy::const_iterator col_it = col_b2.begin();
    ...
    DualIndex<SymmGroup> T_basis = detail::T_basis_left(
        left, left_mult_mps, mpo, ket_basis, bra_basis, b1, isHermitian
    );
    if (T_basis.size() == 0) {
      continue;
    }
    ...
    for (size_t oi = 0; oi < access.size(); ++oi) {
      ...
      charge operator_delta =
          SymmGroup::fuse(W.basis().right_charge(0), -W.basis().left_charge(0));
      charge T_delta =
          SymmGroup::fuse(T_basis.right_charge(0), -T_basis.left_charge(0));
      ...
      for (size_t r = 0; r < right_i.size(); ++r) {
        ...
        if (ret.find_block(out_l_charge, out_r_charge) == ret.n_blocks()) {
          ...
          ret.resize_block(
              ret.insert_block(Matrix(1, 1), out_l_charge, out_r_charge),
  ...
  for (typename col_proxy::const_iterator col_it = col_b2.begin();
    ...
    for (size_t oi = 0; oi < access.size(); ++oi) {
      ...
      charge operator_delta =
          SymmGroup::fuse(W.basis().right_charge(0), -W.basis().left_charge(0));
      charge T_delta =
          SymmGroup::fuse(T.basis().right_charge(0), -T.basis().left_charge(0));
      charge total_delta = SymmGroup::fuse(operator_delta, -T_delta);
      block_matrix<Matrix, SymmGroup>& ret = contr_grid(b1, b2);
      ...
      for (size_t r = 0; r < right_i.size(); ++r) {
        ...
        for (size_t w_block = 0; w_block < W.n_blocks(); ++w_block) {
          ...
          maquis::dmrg::detail::lb_tensor_mpo(
              oblock, iblock, wblock, out_left_offset, in_right_offset, phys_s1,
              phys_s2, T.basis().left_size(t_block), r_size, access.scale(oi)
          );
        }
      }  // right index block
    }  // oi
...
template <class Matrix, class OtherMatrix, class SymmGroup>
void rbtm_kernel_allocate(
    size_t b1, block_matrix<Matrix, SymmGroup>& ret,
    Boundary<OtherMatrix, SymmGroup> const& right,
    MPSBoundaryProduct<Matrix, OtherMatrix, SymmGroup, Gemms> const&
        right_mult_mps,
    MPOTensor<Matrix, SymmGroup> const& mpo,
    DualIndex<SymmGroup> const& ket_basis,
    DualIndex<SymmGroup> const& bra_basis, Index<SymmGroup> const& left_i,
    Index<SymmGroup> const& out_right_i, bool isHermitian
) {
  using index_type = typename MPOTensor<Matrix, SymmGroup>::index_type;
  ...
  for (typename row_proxy::const_iterator row_it = row_b1.begin();
    ...
    DualIndex<SymmGroup> T_basis = detail::T_basis_right(
        right, right_mult_mps, mpo, ket_basis, bra_basis, b2, isHermitian
    );
    if (T_basis.size() == 0) {
      continue;
    }
...
#else
          ret.insert_block(
              Matrix(l_size, out_right_i.size_of_block(out_r_charge)),
              out_l_charge, out_r_charge
          );
#endif
        }
    ...
    for (size_t oi = 0; oi < access.size(); ++oi) {
      ...
      for (size_t l = 0; l < left_i.size(); ++l) {
        ...
        for (size_t w_block = 0; w_block < W.n_blocks(); ++w_block) {
          ...
          Matrix& oblock = ret[o];
          parallel::guard proc(scheduler(o));
          maquis::dmrg::detail::rb_tensor_mpo(
              oblock, iblock, wblock, out_right_offset, in_left_offset, phys_s1,
              phys_s2, l_size, T.basis().right_size(t_block), access.scale(oi)
          );
        }
      }
    }  // oi
...
void lbtm_kernel(
  ...
  lbtm_kernel_allocate(
      b2, contr_grid, left, left_mult_mps, mpo, ket_basis, bra_basis, right_i,
      out_left_i, isHermitian
  );
  lbtm_kernel_execute(
      b2, contr_grid, left, left_mult_mps, mpo, ket_basis, right_i, out_left_i,
