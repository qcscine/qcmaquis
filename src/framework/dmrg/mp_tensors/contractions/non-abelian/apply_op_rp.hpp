      ...
      for (size_t t_block = 0; t_block < T.n_blocks(); ++t_block) {
        ...
        for (size_t w_block = 0; w_block < W.basis().size(); ++w_block) {
          ...
          if (!::SU2::triangle(
            ...
            continue;
          if (!right_i.has(out_l_charge))
            continue;  // can also probe out_left_i, but right_i has the same
                       // charges
          ...
          if (o == ret.n_blocks()) {
            o = ret.insert_block(
                Matrix(
                    T.basis().left_size(t_block),
                    in_right_pb.size(-phys_out, out_r_charge)
                ),
                lc, out_r_charge_rp
            );
          }
          ...
          detail::rbtm_blocked<Matrix, SymmGroup>(
              T[t_block], ret[o], W, in_right_offset, out_right_offset, l_size,
