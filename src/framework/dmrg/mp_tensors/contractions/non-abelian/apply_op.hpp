...
void lbtm_kernel(
    size_t b2, ContractionGrid<Matrix, SymmGroup>& contr_grid,
    Boundary<OtherMatrix, SymmGroup> const& left,
    BoundaryMPSProduct<Matrix, OtherMatrix, SymmGroup, ::SU2::SU2Gemms> const&
        left_mult_mps,
    MPOTensor<Matrix, SymmGroup> const& mpo,
    DualIndex<SymmGroup> const& ket_basis, Index<SymmGroup> const& right_i,
  ...
  for (typename col_proxy::const_iterator col_it = col_b2.begin();
    ...
    for (std::size_t op_index = 0; op_index < access.size(); ++op_index) {
      ...
      int a = mpo.left_spin(b1).get(), k = W.spin().get(),
          ap = mpo.right_spin(b2).get();
      ...
      for (size_t t_block = 0; t_block < T.n_blocks(); ++t_block) {
        ...
        const_iterator it =
            ket_basis.left_lower_bound(rc);  // ket_basis comes transposed!
        charge mc = it->rc;
        ...
        for (size_t w_block = 0; w_block < W.basis().size(); ++w_block) {
          ...
          if (!::SU2::triangle(
                  SymmGroup::spin(out_r_charge), ap,
                  SymmGroup::spin(out_l_charge)
              )) {
            continue;
          }
          ...
          if (o == ret.n_blocks()) {
            o = ret.insert_block(
                Matrix(out_left_i.size_of_block(out_l_charge), r_size),
          ...
          ::SU2::set_coupling(
              j, two_s, jp, a, k, ap, i, two_sp, ip, access.scale(op_index),
              couplings
          );
          ...
          detail::lbtm<Matrix, SymmGroup>(
              T[t_block], ret[o], W, in_right_offset, out_left_offset, l_size,
...
struct task_capsule {
  ...
  using map_t = std::map<
      std::pair<charge, charge>, std::vector<micro_task>,
      compare_pair<std::pair<charge, charge>>>;
...
void rbtm_tasks(
    size_t b1,
    MPSBoundaryProduct<Matrix, OtherMatrix, SymmGroup, ::SU2::SU2Gemms> const&
        right_mult_mps,
    MPOTensor<Matrix, SymmGroup> const& mpo,
    DualIndex<SymmGroup> const& ket_basis, Index<SymmGroup> const& left_i,
  ...
  // Note: can't be replaced by range-based for loop since dereferencing
  // row_proxy iterator disabled
  row_proxy row_b1 = mpo.row(b1);
  for (typename row_proxy::const_iterator row_it = row_b1.begin();
       row_it != row_b1.end(); ++row_it) {
    index_type b2 = row_it.index();

    DualIndex<SymmGroup> T = right_mult_mps.basis_at(b2);

    // Get MPO operator corresponding to (b1, b2)
    MPOTensor_detail::term_descriptor<Matrix, SymmGroup, true> access =
        mpo.at(b1, b2);
    ...
    for (size_t op_index = 0; op_index < access.size(); ++op_index) {
      ...
      for (size_t t_block = 0; t_block < T.size(); ++t_block) {
        ...
        for (size_t w_block = 0; w_block < W.basis().size(); ++w_block) {
          ...
          if (!::SU2::triangle(
                  SymmGroup::spin(out_l_charge), a,
                  SymmGroup::spin(out_r_charge)
              )) {
            continue;
          }
          ...
          std::vector<micro_task>& otasks =
              tasks_cap.tasks[std::make_pair(out_l_charge, out_r_charge)];

          int i = SymmGroup::spin(out_r_charge);
          int ip = SymmGroup::spin(rc);
          int j = SymmGroup::spin(out_l_charge);
          int jp = SymmGroup::spin(mc);
          ...
          micro_task tpl;
          tpl.l_size = l_size;
          tpl.stripe = T.left_size(t_block);
          tpl.b2 = b2;
          tpl.k = t_block;

          unsigned short r_size_cache =
              16384 / (l_size * W.basis().right_size(w_block));  // 128 KB
          for (unsigned short slice = 0; slice < r_size / r_size_cache;
               ++slice) {
            size_t right_offset_cache = slice * r_size_cache;
            detail::op_iterate<Matrix, SymmGroup>(
                W, w_block, couplings, otasks, tpl,
                //&T[t_block](in_left_offset, right_offset_cache),
                in_left_offset + tpl.stripe * right_offset_cache, r_size_cache,
                r_size, out_right_offset + right_offset_cache
            );
          }
          ...
          detail::op_iterate<Matrix, SymmGroup>(
              ...
              in_left_offset + tpl.stripe * right_offset_remain, r_size_remain,
...
void rbtm_axpy(
    task_capsule<Matrix, SymmGroup>& tasks_cap,
    block_matrix<Matrix, SymmGroup>& ret, Index<SymmGroup> const& out_right_i,
  ...
  for (typename map_t::iterator it = tasks.begin(); it != tasks.end(); ++it) {
    ...
    for (typename std::vector<micro_task>::const_iterator it2 = otasks.begin();
         it2 != otasks.end(); ++it2) {
      detail::task_axpy(
          *it2, &buf(0, 0), &t.at(it2->b2)[it2->k](0, 0) + it2->in_offset
      );
    }
...
template <class Matrix, class OtherMatrix, class SymmGroup>
void rbtm_kernel(
    size_t b1, block_matrix<Matrix, SymmGroup>& ret,
    Boundary<OtherMatrix, SymmGroup> const& left,
    MPSBoundaryProduct<Matrix, OtherMatrix, SymmGroup, ::SU2::SU2Gemms> const&
        right_mult_mps,
    MPOTensor<Matrix, SymmGroup> const& mpo,
  ...
  rbtm_tasks(
      b1, right_mult_mps, mpo, ket_basis, left_i, out_right_i, in_left_pb,
      out_right_pb, tasks_cap
  );
...
template <class Matrix, class OtherMatrix, class SymmGroup>
void charge_gemm(
    Matrix const& A, OtherMatrix const& B,
    block_matrix<OtherMatrix, SymmGroup>& C, typename SymmGroup::charge rc,
  ...
  boost::numeric::bindings::blas::gemm(
      scale, A, B, typename Matrix::value_type(1), C[c_block]
  );
}

template <class Matrix, class OtherMatrix, class TVMatrix, class SymmGroup>
void rbtm_axpy_gemm(
    size_t b1, task_capsule<Matrix, SymmGroup>& tasks_cap,
    block_matrix<Matrix, SymmGroup>& prod, Index<SymmGroup> const& out_right_i,
    Boundary<OtherMatrix, SymmGroup> const& left,
    MPOTensor<Matrix, SymmGroup> const& mpo,
  ...
  std::vector<value_type> phases =
      (mpo.herm_info.left_skip(b1))
          ? ::contraction::common::conjugate_phases(
                left_b1, mpo, b1, true, false
            )
          : std::vector<value_type>(left_b1.n_blocks(), 1.);

  for (auto& task : tasks_cap.tasks) {
    std::vector<micro_task>& otasks = task.second;
    if (otasks.size() == 0) {
      continue;
    }
    ...
    Matrix buf(otasks[0].l_size, out_right_i.size_of_block(task.first.second));

    size_t k = left_b1.basis().position(task.first.second, task.first.first);
    if (k == left_b1.basis().size()) {
      continue;
    }

    for (const auto& otask : otasks) {
      // 1) &t.at(otask.b2): converts MPSBoundaryProduct to block_matrix
      // (through a GEMM??) 2) ...[otask.k]: selects k-th block_matrix which
      // returns a matrix
      detail::task_axpy(
          otask, &buf(0, 0), &t.at(otask.b2)[otask.k](0, 0) + otask.in_offset
      );
    }
