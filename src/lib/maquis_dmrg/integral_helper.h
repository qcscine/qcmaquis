...
enum class Hamiltonian {
  Electronic,
  RelativisticElectronic,
  VibrationalCanonical,
  VibrationalNMode,
  PreBO,
  Vibronic,
  Excitonic
...
class TranscorrelatedTraitClass<HamiltonianTransformation::Transcorrelated> {
 public:
  static std::string getIntegralFileName() {
    return "transcorrelated_integral_file";
  };
};
 ...
 * - the canonical quantization-based vibration Hamiltonian has a number of
 * indices equal to the max. order of the Taylor expansion of the PES. Here we
 * include up to sixth-order force constants.
 */
constexpr int getIndexDim(
    const Hamiltonian& type, const HamiltonianTransformation& transformation
) {
  int indexDim = 0;
  switch (type) {
    ...
    case Hamiltonian::VibrationalCanonical:
      indexDim = ORDER_NONE;  // This value is defined in the top level
                              // CMakeLists.txt and can be set as a compile flag
      break;
#endif
...
template <
    Hamiltonian HamiltonianType = Hamiltonian::Electronic,
    HamiltonianTransformation Transcorrelation =
        HamiltonianTransformation::Conventional>
using index_type =
    std::array<int, getIndexDim(HamiltonianType, Transcorrelation)>;
...
template <
    ...
    HamiltonianTransformation Transcorrelation =
        HamiltonianTransformation::Conventional>
using integral_tuple =
    std::pair<index_type<HamiltonianType, Transcorrelation>, V>;
...
template <
    ...
    HamiltonianTransformation Transcorrelation =
        HamiltonianTransformation::Conventional>
using integrals = std::vector<integral_tuple<
    V, HamiltonianType, Transcorrelation> >;  // TODO: use a map later
...
struct integral_hash {
 public:
  std::size_t operator()(const index_type<HamiltonianType, Transcorrelation>& id
  ) const {
