...
#error SU2 symmetry (SU2U1 or SU2U1PG) must be enabled to compile DMRG interface
#endif
  Impl(
      const DmrgParameters& parms, const std::string& pname, int nstates,
      bool do_fiedler, bool do_cideas,
      const std::vector<std::vector<int> >& hf_occupations
  )
      : parms_(parms),
        pname_(pname),
        pname_guess_(pname + "_guess"),
        nstates_(nstates),
        do_fiedler_(do_fiedler),
        do_cideas_(do_cideas) {
  ...
  std::string getFiedlerOrder() {
    ...
    // prepare ordering. first create a vector with indices 0..L-1 in ascending
    // order
  ...
  void cideas() {
    ...
    for (int i = 0; i < nstates_; i++) {
      MPS<Matrix, SymmGroup> mps =
          maquis::cideas<Matrix, SymmGroup>(parms_, s1_[i]);
 ...
 private:
  ...
  // Mutual information is state-average while single-orbital entropy is
  // state-specific
  Matrix SA_mutI_;
  ...
  // Obtain measurements
  const std::vector<results_map_type<V> >& get_measurements() const {
    return measurements;
  };
};

template <class V>
StartingGuess<V>::StartingGuess(
    const DmrgParameters& parms, int nstates, const std::string& pname,
    bool do_fiedler, bool do_cideas,
    const std::vector<std::vector<int> >& hf_occupations
)
    : impl_(
          new Impl(parms, pname, nstates, do_fiedler, do_cideas, hf_occupations)
      ) {}
...
template <class V>
std::string StartingGuess<V>::getFiedlerOrder() {
  return impl_->getFiedlerOrder();
}

template <class V>
void StartingGuess<V>::cideas() {
  impl_->cideas();
}
