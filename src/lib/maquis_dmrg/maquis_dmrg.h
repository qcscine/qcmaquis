...
// meas_with_results_type: one measurement = pair of vectors with labels and
// results
...
template <
    typename ScalarType,
    Hamiltonian HamiltonianType = Hamiltonian::Electronic>  // real or complex
class DMRGInterface {
 public:
  ...
  // using OutputType = std::unordered_map<CICoefficientsType, V,
  // boost::hash<CICoefficientsType>>;
  ...
  // Run dmrg_meas (measure all measurements and save them to a HDF5 file
  // specified in parameters) Do not store the measurements in a map.
  void run_measure();

  // Run all measurements and save them as a map
  // Currently, all measurements must be added via parameters first, and then
  // measure() may be executed In the future, we should support single
  // measurements directly from the interface
  void measure();

  /** @brief Getter for the measurements */
  const results_map_type& measurements();
  // #ifdef TRANSCORR_INTEGRALSC
  using tc_integral_map = integral_map<
      ScalarType, Hamiltonian::Electronic,
  ...
  // measurement is not recognised by the model! Fix the model to recognise it!
  ...
  // Measure 3 and 4-RDM (for now in 2U1), and save it into the corresponding
  // (SU2U1) result file (which should be set with parms["rfile"])
  ...
  // The same for transition 3-RDM. bra_name provides the name of the bra
  // checkpoint (doesn't matter if it is in SU2U1 or 2U1, as in the former case
  // it will be transformed)
  void measure_and_save_trans3rdm(const std::string& bra_name);

  // Load an MPS from a given checkpoint and measure the overlap with the
  // current MPS
