...
void qcmaquis_interface_preinit(
    ...
    const char* project_name, bool meas_2rdm
) {
...
void qcmaquis_interface_run_starting_guess(
    int nstates, const char* project_name, bool do_fiedler, bool do_cideas,
    char* fiedler_order_string, int* hf_occupations
) {
  // TODO: Make sure that qcmaquis_interface_preinit and _update_integrals has
  ...
  maquis::StartingGuess<V> starting_guess(
      parms, nstates, project_name_, do_fiedler, do_cideas, hf_occupations_vec
  );

  if (do_fiedler) {
    // TODO: check length of fiedler_order_string. it must be pre-set to the
    // correct length
...
void qcmaquis_interface_remove_param(const char* key) { parms.erase(key); }
...
double qcmaquis_interface_get_energy() { return interface_ptr->energy(); }
...
void qcmaquis_interface_set_state(int state) {
  ...
  parms.set(
      "resultfile", maquis::interface_detail::su2u1_result_name(pname, state)
  );
...
void qcmaquis_interface_get_generic1rdm(
    const typename maquis::meas_with_results_type<V>& meas, int* indices,
    V* values, int size
) {
  // the size attribute is pretty much useless if we allocate the output arrays
  // outside of the interface we'll just use it to check if the size matches the
  // size of the measurement
  //
  // When we have point group symmetry, OpenMOLCAS will allocate an array larger
  // than our number of measurements So as long as our measurements will fit
  // into the allocated array, we're good I.e. the sizes of an array do not have
  // to match exactly
...
void qcmaquis_interface_get_1rdm(int* indices, V* values, int size) {
  qcmaquis_interface_get_generic1rdm(
      interface_ptr->onerdm(), indices, values, size
  );
}

void qcmaquis_interface_get_spdm(int* indices, V* values, int size) {
  qcmaquis_interface_get_generic1rdm(
      interface_ptr->onespdm(), indices, values, size
  );
}
...
#define measure_and_save_rdm(N, state)                           \
  std::string res_name =                                         \
      maquis::interface_detail::su2u1_result_name(pname, state); \
...
void qcmaquis_interface_measure_and_save_trans3rdm(int state, int bra_state) {
  ...
  // because in an ordinary HDF5 result file it is only possible to have one
  // transition 3-RDM but we need one for each bra_state
...
void qcmaquis_interface_get_iteration_results(
    int* nsweeps, std::size_t* m, V* truncated_weight, V* truncated_fraction,
    V* smallest_ev
) {
  ...
  *smallest_ev = 0;
  ...
  if (!iter.empty()) {
    ...
    *m = 0;
    for (auto&& m_ : m_vec) *m += std::any_cast<std::size_t>(m_);
    *truncated_weight = 0;
...
void qcmaquis_interface_restore_stdout() { stdout_redirect.restore(); }

int qcmaquis_interface_get_4rdm_elements(int L, const int* slice) {
  std::vector<int> slice_ = slice != nullptr
                                ? std::vector<int>(slice, slice + 3)
                                : std::vector<int>();
  return measurements_details::get_nrdm_permutations<4>(L, false, slice_);
}

int qcmaquis_interface_get_3rdm_elements(
    int L, bool bra_neq_ket, const int* slice
) {
  std::vector<int> slice_ = slice != nullptr
                                ? std::vector<int>(slice, slice + 1)
                                : std::vector<int>();
...
// set info for 2U1 group
  ...
  std::tie(twou1_checkpoint_name, Nup, Ndown) =
      maquis::interface_detail::twou1_name_Nup_Ndown(
          pname, state, nel, multiplicity
      );

  // generate result file name. this should be analogous to SU2U1 file name to
  // be consistent with on-the-fly evaluation which just writes the rdm into the
  // SU2U1 result
  std::string twou1_result_name = std::regex_replace(
      maquis::interface_detail::su2u1_name(
          pname, state
      ),  // generate SU2U1 checkpoint file
      std::regex("checkpoint"), "results"
...
void qcmaquis_interface_get_fock_contracted_4rdm(
  ...
  parms_caspt2.set(
      "resultfile",
      maquis::interface_detail::pname2workdir(pname) + "results.h5"
  );
