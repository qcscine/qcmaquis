...
class MPSSIInterface {
 public:
  ...
  // // 1-TDM
  // meas_with_results_type onetdm(const std::string& bra_pname, int bra_state,
  // const std::string& ket_pname, int ket_state);

  // 1-TDM, split in four spin components
  std::vector<maquis::meas_with_results_type<V> > onetdm_spin(
      const std::string& bra_pname, int bra_state, const std::string& ket_pname,
      int ket_state
  );
 ...
 private:
  // Generate 2U1 checkpoint name. rotated == true will append .rotated. to
  // prefix
  std::string twou1_name(
      const std::string& pname, int state, int Ms, bool rotated = false
  );
