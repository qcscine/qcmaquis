...
// site_types -- array with irrep # for each site (sized L), NULL if all sites
// should be of irrep 1 conv_thresh -- convergence threshold m -- max. bond
// dimension nsweeps -- number of sweeps sweep_m -- if different bond dimensions
// should be used for different sweeps, provide them here if NULL, it is
// ignored, otherwise if it's not NULL, m is ignored nsweepm -- size of sweep_m
...
// if Fiedler ordering is present, return the ordering as a string (starting
// with 1) in fiedler_order_string (note that its length must be correct!)
// hf_occupation: Array of HF occupations (as 4,3,2,1) for all states, as
// flattened row-major array of (L*nstates) For CI-DEAS mandatory, for Fiedler
// ordering optional
...
// truncated_weight -- sum of all truncated weights for the last sweep, 0 for
// single-site optimisation truncated_fraction -- sum of all truncated fractions
// for the last sweep, 0 for single-site optimisation smallest_ev -- smallest
// cutoff eigenvalue of the last sweep
...
// Measure 3/4-RDM and save it into HDF5 result file (using project name passed
// to qcmaquis_interface_preinit) state: state index (starting from 0) Warning:
// this reinitialises the interface and loads a checkpoint corresponding to the
// state index so this should be called after wavefunction optimisation
...
// bra_neq_ket: true for transition RDM measurement, otherwise false (only for
// 3-RDM) slice: integer array of 4 indices corresponding to a 4-RDM slice with
// four first indices fixed for 3-RDM this has 2 or 3 indices if is not nullptr,
// number of all 3/4-RDM elements will be evaluated, otherwise only the number
...
// Prepare QCMaquis input template file for distributed calculation of
// higher-order RDMs filename: name under which the template should be saved
