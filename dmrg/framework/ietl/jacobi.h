...
class jcd_left_preconditioner {
 public:
  ...
  using magnitude_type =
      typename ietl::number_traits<scalar_type>::magnitude_type;

  jcd_left_preconditioner(
      const MATRIX& matrix, const VS& vec, const int& max_iter
  );
  void operator()(
      const vector_type& u, const magnitude_type& theta, const vector_type& r,
      vector_type& t, const magnitude_type& rel_tol
  );

 private:
  void sysv(
      const char& uplo, fortran_int_t n, fortran_int_t nrhs, double a[],
  ...
  void sysv(
      const char& uplo, fortran_int_t n, fortran_int_t nrhs,
      std::complex<double> a[], fortran_int_t lda, fortran_int_t ipiv[],
      std::complex<double> b[], fortran_int_t ldb, std::complex<double> work[],
...
template <class Matrix, class VS, class Vector>
void mult(
    jcd_solver_operator<Matrix, VS, Vector> const& m,
...
class jcd_gmres_solver {
 public:
  ...
  jcd_gmres_solver(
      Matrix const& matrix, VS const& vec, std::size_t max_iter = 5,
      bool verbose = false
  )
      : matrix_(matrix),
        vecspace_(vec),
  ...
  void operator()(
    ...
    t = -r + dru / duu * u;
    if (max_iter_ > 0) t = gmres(op, inh, t, rel_tol);
  }
...
class jcd_solver {
 public:
  ...
  jcd_solver(Matrix const& matrix, VS const& vec, Solver solv)
      : matrix_(matrix), vecspace_(vec), solv_(solv), n_(vec_dimension(vec)) {}

  template <class Solver>
  void replace_solver(Solver solv) {
    solv_ = solv;
  }
  ...
  void operator()(
    ...
    scalar_type duu = ietl::dot(u, u);
    t = -r + dru / duu * u;
    if (max_iter_ > 0) t = solv_(op, inh, t, rel_tol);
  }

 private:
  Matrix const& matrix_;
  VS vecspace_;
  std::function<vector_type(
      jcd_solver_operator<Matrix, VS, vector_type> const&, vector_type const&,
...
class jacobi_davidson {
 public:
  ...
  jacobi_davidson(
      const MATRIX& matrix, const VS& vec, DesiredEigenvalue desired = Largest
  );
  ~jacobi_davidson();

  template <class GEN, class SOLVER, class ITER>
  std::pair<magnitude_type, vector_type> calculate_eigenvalue(
      const GEN& gen, SOLVER& solver, ITER& iter
  );

 private:
  void get_extremal_eigenvalue(
      magnitude_type& theta, std::vector<double>& s, fortran_int_t dim
  );
  void get_extremal_eigenvalue(
      magnitude_type& theta, std::vector<float>& s, fortran_int_t dim
  );
  void get_extremal_eigenvalue(
      magnitude_type& theta, std::vector<std::complex<double> >& s,
      fortran_int_t dim
  );
  void get_extremal_eigenvalue(
      magnitude_type& theta, std::vector<std::complex<float> >& s,
      fortran_int_t dim
  );
...
template <class MATRIX, class VS>
jcd_simple_solver<MATRIX, VS>::jcd_simple_solver(
    const MATRIX& matrix, const VS& vec
)
    : matrix_(matrix), vecspace_(vec) {
  n_ = vec_dimension(vecspace_);
}

template <class MATRIX, class VS>
void jcd_simple_solver<MATRIX, VS>::operator()(
    const vector_type& u, const magnitude_type& theta, const vector_type& r,
    vector_type& t, const magnitude_type& rel_tol
) {
  // for (int i=0;i<n_;i++)
...
template <class Matrix, class VS, class Vector>
void jcd_solver_operator<Matrix, VS, Vector>::operator()(
    vector_type const& x, vector_type& y
) const {
...
template <class MATRIX, class VS>
jcd_left_preconditioner<MATRIX, VS>::jcd_left_preconditioner(
    const MATRIX& matrix, const VS& vec, const int& max_iter
)
    : K(matrix), vecspace_(vec), max_iter_(max_iter) {
  n_ = vecspace_.vec_dimension();
}

template <class MATRIX, class VS>
void jcd_left_preconditioner<MATRIX, VS>::operator()(
    const vector_type& u, const magnitude_type& theta, const vector_type& r,
    vector_type& t, const magnitude_type& rel_tol
) {
  // define variables
  ...
  for (int i = 0; i < n_; i++)
    for (int j = 0; j < n_; j++) Ktmp(i, j) = K(i, j);
  for (int i = 0; i < n_; i++) Ktmp(i, i) -= theta;
  ...
  ietl::copy(u, u_hat);
  sysv(
      uplo, n, nrhs, Ktmp.data(), lda, ipiv, u_hat.data(), ldb, work, lwork,
      info
  );
  ...
  ietl::copy(r, vec1);
  for (int i = 0; i < n_; i++) Ktmp(i, i) = K(i, i);
  uplo = 'L';
  sysv(
      uplo, n, nrhs, Ktmp.data(), lda, ipiv, vec1.data(), ldb, work, lwork, info
  );
  vec1 -= ietl::dot(u, vec1) / mu * u_hat;
  vec1 *= -1.;

  // aplly a Krylov subspace method with t_0=0; operator K_tilde ^{-1} A_tilde
  // and right hand side -r_tilde; given v, z=K_tilde ^{-1} A_tilde v is
  // computed as
  //   y = (A-\theta I)v
  ...
  for (int i = 0; i < max_iter_; i++) {
    ...
    ietl::mult(K, t, vec1);
    for (int j = 0; j < n_; j++)
      for (int k = 0; k < n_; k++) Ktmp(j, k) = K(j, k);
    for (int j = 0; j < n_; j++) Ktmp(j, j) -= theta;
    sysv(
        uplo, n, nrhs, Ktmp.data(), lda, ipiv, vec1.data(), ldb, work, lwork,
        info
    );
    vec1 -= -ietl::dot(u, vec1) / mu * u_hat;
    norm = ietl::dot(t, vec1);
    vec2 = vec1 - norm * t;
    if (ietl::two_norm(vec2) < rel_tol * std::abs(norm)) break;
  }
...
jacobi_davidson<MATRIX, VS>::jacobi_davidson(
    const MATRIX& matrix, const VS& vec, DesiredEigenvalue desired
)
    : matrix_(matrix), vecspace_(vec), M(1, 1), desired_(desired) {
  //      n_ = vecspace_.vec_dimension();
  n_ = vec_dimension(vecspace_);
}
...
template <class GEN, class SOLVER, class ITER>
std::pair<
    typename jacobi_davidson<MATRIX, VS>::magnitude_type,
    typename jacobi_davidson<MATRIX, VS>::vector_type>
jacobi_davidson<MATRIX, VS>::calculate_eigenvalue(
    const GEN& gen, SOLVER& solver, ITER& iter
) {
...
// u = V s
#ifdef USE_AMBIENT
    std::vector<vector_type> u_parts;
    u_parts.reserve(iter.iterations() + 1);
    for (int j = 0; j <= iter.iterations(); ++j) u_parts.push_back(V[j] * s[j]);
    vector_type u =
        ambient::reduce_sync(u_parts, [](vector_type& dst, vector_type& src) {
          dst += src;
          src.clear();
        });
...
// ietl::mult(matrix_,u,uA);
#ifdef USE_AMBIENT
    std::vector<vector_type> uA_parts;
    uA_parts.reserve(iter.iterations() + 1);
    for (int j = 0; j <= iter.iterations(); ++j)
      uA_parts.push_back(VA[j] * s[j]);
    vector_type uA =
        ambient::reduce_sync(uA_parts, [](vector_type& dst, vector_type& src) {
          dst += src;
          src.clear();
        });
    ...
    V[iter.iterations()].data().iter_index =
        VA[iter.iterations() - 1].data().iter_index;
    storage::migrate(
        V[iter.iterations()],
        parallel::scheduler_balanced_iterative(V[iter.iterations()].data())
    );
  } while (true);
}

template <class MATRIX, class VS>
void jacobi_davidson<MATRIX, VS>::get_extremal_eigenvalue(
    magnitude_type& theta, std::vector<double>& s, fortran_int_t dim
) {
  FortranMatrix<scalar_type> M_(dim, dim);
  for (int i = 0; i < dim; i++)
    for (int j = 0; j <= i; j++) M_(j, i) = M(j, i);
  double abstol = atol_;
  char jobz = 'V';
  char range = 'I';
  char uplo = 'U';
  ...
  LAPACK_DSYEVX(
      &jobz, &range, &uplo, &n, M_.data(), &lda, &vl, &vu, &il, &iu, &abstol,
