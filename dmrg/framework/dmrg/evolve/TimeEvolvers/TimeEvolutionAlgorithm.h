...
class TimeEvolutionAlgorithm {
 public:
   ...
   */
  explicit TimeEvolutionAlgorithm(
      time_type time_step, bool has_td_part, bool is_imag, bool verbose = false
  )
  ...
  /** Wrapper around the method to propagate a MatrixType (SiteProblem or
   * ZeroSiteProblem) objects */
  template <class MatrixType, class SiteProblem>
  MatrixType apply_hamiltonian(
      MatrixType const& matrix, SiteProblem const& site_problem
  ) const {
