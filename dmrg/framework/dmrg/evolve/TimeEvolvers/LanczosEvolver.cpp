...
template <
    class Matrix, class SymmGroup, TimeStepDistributor TimeStepDistributorClass>
template <class SiteProblem, class MatrixType>
void LanczosEvolver<Matrix, SymmGroup, TimeStepDistributorClass>::evolve_kernel(
    const SiteProblem& site_problem, MatrixType& matrix, bool isForward,
    time_type time_current, time_type time_step
) const {
  // Types definition
  // using matrix_type = Eigen::Matrix< typename MatrixType::scalar_type,
  // Eigen::Dynamic, Eigen::Dynamic >; using vector_type = Eigen::Matrix<
  // typename MatrixType::scalar_type, Eigen::Dynamic, 1 >;
  ...
  for (int iExp = 0; iExp < numberOfExponentials; iExp++) {
    ...
    buffer_vector =
        applyOperator(lanczos_space[0], site_problem, iExp, time_current);
    ...
    */
    ...
    for (std::size_t idx = 1; idx < max_iter_; idx++) {
      ...
      if (norm_local < 1.0E-20 || error < threshold_) {
        ...
        buffer_vector =
            applyOperator(lanczos_space[idx], site_problem, iExp, time_current);
        alpha = ietl::dot(buffer_vector, lanczos_space[idx]);
        buffer_vector -=
            alpha * lanczos_space[idx] + norm_local * lanczos_space[idx - 1];
...
template <class SiteProblem, class MatrixType>
MatrixType
LanczosEvolver<Matrix, SymmGroup, TimeStepDistributorClass>::applyOperator(
    const MatrixType& inputVec, const SiteProblem& site_problem, int idExp,
    time_type time_current
) const {
  MatrixType outputVec;
  if (has_td_part_) {
    for (int iExp = 0; iExp < numberOfFactorsPerExponential; iExp++) {
      if (iExp == 0) {
        outputVec = factorsAndSteps[idExp][iExp].first *
                    apply_hamiltonian(inputVec, site_problem);
      } else {
        outputVec += factorsAndSteps[idExp][iExp].first *
                     apply_hamiltonian(inputVec, site_problem);
      }
      // Adds the TD part
      // outputVec +=
      // factorsAndSteps[idExp][iExp].first*site_problem.apply_td_part(inputVec,
      // time_current+time_step_*factorsAndSteps[idExp][iExp].second);
    }
  } else {
...
void LanczosEvolver<Matrix, SymmGroup, TimeStepDistributorClass>::
    apply_exponential(
        MatrixType& hamiltonian_matrix, VectorType& ret, std::size_t local_dim_,
        bool isForward
    ) const {
  // Types definition
  ...
  matrix_complex expM;
  auto coeff = (is_imag_) ? -std::complex<double>(time_step_, 0.)
                          : -std::complex<double>(0., time_step_);
  if (!isForward) coeff *= std::complex<double>(-1., 0.);
  expM = alps::numeric::exp_hermitian(H_hess, coeff, is_imag_);
  /*
  if (is_imag_) {
    // For imaginary-time propagations, we might encounter large numbers, shifts
  the diagonal. Eigen::ComplexEigenSolver<matrix_complex> es;
    es.compute(H_hess);
    matrix_complex exponentialOfEigenValues =
  matrix_complex(es.eigenvalues().asDiagonal())
      - matrix_complex::Identity(local_dim_,
  local_dim_)*es.eigenvalues().real().maxCoeff(); expM =
  es.eigenvectors()*exponentialOfEigenValues.exp()*es.eigenvectors().transpose();
  }
  else {
    expM = H_hess.exp();
  }
