...
class RKEvolver : public TimeEvolutionAlgorithm<Matrix, SymmGroup> {
 public:
  ...
  void evolve(
      ...
      TimeType time_current, TimeType time_step
  ) const override final {
    this->evolve_kernel(
        site_problem, matrix, is_forward, time_current, time_step
    );
  }
 ...
 private:
  ...
  /* Routine for the rescaling of the matrix to obtain, in the end, a complex
   * value */
  template <
      class ArgType, class MatrixType,
      typename std::enable_if<std::is_same<double, ArgType>::value>::type* =
          nullptr>
