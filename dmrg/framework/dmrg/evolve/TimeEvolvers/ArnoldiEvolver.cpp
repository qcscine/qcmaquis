...
void ArnoldiEvolver<Matrix, SymmGroup>::evolve_kernel(
    const SiteProblem& site_problem, MatrixType& matrix, bool is_forward,
  ...
  for (int idx = 0; idx < max_iter_; idx++) {
    ...
    for (int idx2 = 0; idx2 < idx + 1; idx2++) {
      matrix_representation(idx2, idx) =
          ietl::dot(lanczos_space[idx2], buffer_vector);
...
template <class Matrix, class SymmGroup>
template <class SiteProblem, class MatrixType>
MatrixType ArnoldiEvolver<Matrix, SymmGroup>::applyOperator(
...
void ArnoldiEvolver<Matrix, SymmGroup>::apply_exponential(
