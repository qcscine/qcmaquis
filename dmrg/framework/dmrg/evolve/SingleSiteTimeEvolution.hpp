...
class SingleSiteTimeEvolution
 ...
 public:
   ...
   */
  SingleSiteTimeEvolution(
      MPS<Matrix, SymmGroup>& mps, MPO<Matrix, SymmGroup> const& mpo,
      BaseParameters& parms, std::function<bool()> stop_callback,
      int initial_site_ = 0
  )
      : base(
  ...
  void evolve_sweep(int sweep) override {
    ...
    int _site = 0, site;
    if (initial_site != -1) _site = initial_site;
    ...
    for (; _site < 2 * L_; ++_site) {
      ...
      std::pair<
          typename maquis::traits::real_type<value_type>::type,
          MPSTensor<Matrix, SymmGroup> >
          res;
      SiteProblem<Matrix, SymmGroup> sp(
          left_[site], right_[site + 1], mpo_[site]
      );
      // +-----------------+
      //  MAIN PART: sweep.
      // +-----------------+
      std::cout << std::endl;
      std::cout << "Forward propagating the " << site << "-th MPSTensor "
                << std::endl;
      ...
      if (site == 0) {
        ...
        maquis::cout << " Energy with core = "
                     << res.first + mpo_.getCoreEnergy() << std::endl;
      }
      ...
      if (lr == +1) {
        if (site < L - 1) {
          ...
          trunc = site_shifter_->shiftSiteTD(
              mpo_, left_, right_, alpha, cutoff, Mmax, Mmax,
              perturber_->is_perturb_dm()
          );
      ...
      maquis::cout << " Sweep has been running for " << elapsed
                   << " seconds. \n"
                   << std::endl;
      parallel::meminfo();
 ...
 private:
  ...
  void print_header(int sweep, int site, int lr) {
    ...
    std::cout << " +-------------------------------------------+" << std::endl;
    std::cout << "   Sweep number " << a << " - site number " << site
              << std::endl;
    std::cout << " +-------------------------------------------+" << std::endl;
  }
};
