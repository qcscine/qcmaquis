...
Perturber<Matrix, SymmGroup>::Perturber(
    Perturber::boundary_type const& left, Perturber::boundary_type const& right,
...
void Perturber<Matrix, SymmGroup>::update_alpha(Perturber::alpha_type alpha_new
  ...
  alpha_ = alpha_new;
  if (alpha_ < 1.0E-30) perturb_MPS_ = false;
}

template <class Matrix, class SymmGroup>
void Perturber<Matrix, SymmGroup>::perturb_mps(
    MPSTensor_type& MPS, site_type site, bool is_left
) const {
  ...
  if (perturb_MPS_) {
    if (is_left) {
      ...
      perturber = contraction::Engine<Matrix, Matrix, SymmGroup>::
          generate_left_mpo_basis(MPS, MPS, left_bound_[site], MPO_[site]);
      perturber *= alpha_;
      ...
      for (std::size_t b = 0; b < perturber.aux_dim(); ++b) {
        for (std::size_t idx = 0; idx < perturber[b].n_blocks(); idx++)
          if (perturber[b].get_left_charge(idx) ==
                  perturber[b].get_right_charge(idx) &&
              perturber[b].norm() > 1.0E-10)
            ToAddBM.add_block_to_column(
                perturber[b], perturber[b].get_left_charge(idx),
                perturber[b].get_right_charge(idx)
            );
      }
      if (ToAddBM.n_blocks() != 0) MPS.add_block_to_column(ToAddBM);
    } else {
      MPS.make_right_paired();
      perturber = contraction::Engine<Matrix, Matrix, SymmGroup>::
          generate_right_mpo_basis(
              MPS, MPS, right_bound_[site + 1], MPO_[site]
          );
      perturber *= alpha_;
      auto ToAddBM = block_matrix<Matrix, SymmGroup>();
      for (int b = 0; b < perturber.aux_dim(); ++b)
        for (int idx = 0; idx < perturber[b].n_blocks(); idx++)
          if (perturber[b].get_left_charge(idx) ==
                  perturber[b].get_right_charge(idx) &&
              perturber[b].norm() > 1.0E-10)
            ToAddBM.add_block_to_row(
                perturber[b], perturber[b].get_left_charge(idx),
                perturber[b].get_right_charge(idx)
            );
      if (ToAddBM.n_blocks() != 0) MPS.add_block_to_row(ToAddBM);
    }
  }
}
