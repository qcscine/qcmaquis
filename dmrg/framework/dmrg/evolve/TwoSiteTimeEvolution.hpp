...
/**
 * @brief Class implementing the two-site time-evolution algorithm with a
 * sweep-based Trotter decomposition.
...
class TwoSiteTimeEvolution
    : public TimeEvolutionSweep<Matrix, SymmGroup, Storage> {
 public:
  // Types definition
   ...
   * @param tdmpo: (optional) MPO representation of the time-dependent part of
   * the MPO.
   ...
   * [L+1,2L] to [0,L] according to a backward sweep.
   *
  ...
  /**
   * @brief Routine to perform a sweep (that can be forward, backward or both of
   * them.
   *
   * @param sweep: index of the current sweep.
   * @param d: enum indicating if the sweep is forward, backward or both of
   * them.
   */
  void evolve_sweep(int sweep) {
    ...
    for (; _site < 2 * L_ - 2; ++_site) {
      ...
      twin_mps.clear();
      {
        ...
        if (site1 == 0 && verbose) {
          maquis::cout << " Energy = " << res.first + mpo_.getCoreEnergy()
                       << std::endl;
        }
      ...
      if (lr == +1) {
        if (parms_["twosite_truncation"] == "svd") {
          std::tie(mps_[site1], mps_[site2], trunc) =
              two_vec.split_mps_l2r(Mmax, cutoff);
        } else {
          std::tie(mps_[site1], mps_[site2], trunc) = two_vec.predict_split_l2r(
              Mmax, cutoff, alpha, left_[site1], mpo_[site1], true
          );
        }
        ...
        if (site2 != L_ - 1) {
          if (do_backpropagation_) {
            SiteProblem<Matrix, SymmGroup> sp2(
                left_[site2], right_[site2 + 1], mpo_[site2]
            );
        ...
        if (parms_["twosite_truncation"] == "svd") {
          std::tie(mps_[site1], mps_[site2], trunc) =
              two_vec.split_mps_r2l(Mmax, cutoff);
        } else {
          std::tie(mps_[site1], mps_[site2], trunc) = two_vec.predict_split_r2l(
              Mmax, cutoff, alpha, right_[site2 + 1], mpo_[site2], true
          );
        }
        two_vec.clear();
        ...
        if (site1 != 0) {
          if (do_backpropagation_) {
            SiteProblem<Matrix, SymmGroup> sp2(
                left_[site1], right_[site1 + 1], mpo_[site1]
            );
 ...
 private:
  ...
  void print_header(int& sweep, int& site1, int& site2, int& lr) {
    ...
    std::cout << " +--------------------------------------------+" << std::endl;
    std::cout << "  Sweep number " << a << " - site numbers " << site1
              << " and " << site2 << std::endl;
    std::cout << " +--------------------------------------------+" << std::endl;
  }
};
