...
class SiteShifter {
 public:
  ...
  SiteShifter(
      MPS_type& MPS_input, std::shared_ptr<TimeEvolver> time_evolver,
      std::shared_ptr<Perturber> perturber, int init_site, bool back_propagate
  );
   ...
   */
  void print_MPS(int site_index) {
    std::cout << this->MPS_[site_index] << std::endl;
  };

  /** @brief Standard site shifting */
  truncation_results shiftSiteTI(
      const MPO_type& mpo, boundaries_type& left, boundaries_type& right,
      double alpha, double cutoff, std::size_t Mmax, std::size_t Mval,
      bool perturb_dm
  );

  /** @brief TD site shifting (inclused the back-propagation of the zero-site
   * tensor) */
  truncation_results shiftSiteTD(
      const MPO_type& mpo, boundaries_type& left, boundaries_type& right,
      double alpha, double cutoff, std::size_t Mmax, std::size_t Mval,
      bool perturb_dm
  );
