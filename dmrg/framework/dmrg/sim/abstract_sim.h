...
namespace chem {
template <class T>
using TranscorrMap = chem::integral_map<
    T, chem::Hamiltonian::Electronic,
...
class abstract_interface_sim {
 public:
  ...
  virtual void
  update_integrals(const chem::integral_map<typename Matrix::value_type>&) = 0;
  virtual void update_integrals(std::string fileName) = 0;
  virtual void update_tc_integrals(const chem::TranscorrMap<
                                   typename Matrix::value_type>&) = 0;
