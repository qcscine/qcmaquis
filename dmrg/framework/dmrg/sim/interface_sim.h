...
static std::vector<int> sort_vector(std::vector<T>& vec_to_sort) {
  ...
  std::sort(order.begin(), order.end(), [&vec_to_sort](size_t i1, size_t i2) {
...
parms["nsweeps"]) checkpoint_simulation(mps, sweep, -1); if (stopped) break;
  ...
  */
   ...
   **/
  void update_tc_integrals(
      const chem::TranscorrMap<typename Matrix::value_type>& integrals
  ) {
    // integrals are set later anyways
    // deleting old ones should be okay
    if (parms.is_set("integrals")) {
   ...
   * chkp file */
  typename Matrix::value_type get_overlap(const std::string& aux_filename
  ) override {
