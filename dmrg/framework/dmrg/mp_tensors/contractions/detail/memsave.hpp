...
class ContractionGrid {
 public:
  ...
  void index_sizes(size_t) { data_.index_sizes(); }
  static void iterate_reduction_layout(int, int) {}
  int where(size_t b1, size_t b2) { return 0; }
  ...
  void multiply_column(size_t b2, const block_matrix<Matrix, SymmGroup>& rhs) {
    throw std::runtime_error(
        "ContractionGrid::multiply_column not implemented\n"
    );
  }
  void multiply_column_trans(
    ...
    throw std::runtime_error(
        "ContractionGrid::multiply_column_trans not implemented\n"
    );
  }
