...
#define GRID(b1, b2) grid[b1 + b2 * size1]
  static void check_exceptions(
      MPOTensor<Matrix, SymmGroup> const& mpo, int s1, int s2
  ) {
    std::vector<int> ex;
    for (int b2 = 0; b2 < s2; b2++) {
      bool present = false;
      for (int b1 = 0; b1 < s1; b1++)
        if (mpo.has(b1, b2)) {
          if (present) {
            ex.push_back(b2);
            break;
  ...
  ContractionGrid(MPOTensor<Matrix, SymmGroup> const& mpo, size_t s1, size_t s2)
      : mpo(mpo), size1_(s1), size2(s2), e2(s2, false) {
    ...
    for (int b2 = 0; b2 < size2; b2++)
      if (!e2[b2]) GRID(0, b2) = new block_matrix<Matrix, SymmGroup>();
  }
  ...
  parallel::traits::resource_iterator where(size_t b1, size_t b2) {
    if (e2[b2])
      return parallel::traits::permute(b1, mpo.placement_l, granularity);
    return parallel::traits::permute(b2, mpo.placement_r, granularity);
  }
  block_matrix<Matrix, SymmGroup>& operator()(size_t b1, size_t b2) {
    if (e2[b2]) {
      block_matrix<Matrix, SymmGroup>*& el =
          GRID(parallel::traits::distance(where(b1, b2)) / granularity, b2);
  ...
  void multiply_column(size_t b2, const block_matrix<Matrix, SymmGroup>& rhs) {
    for (int b1 = 0; b1 < size1; b1++) {
      if (GRID(b1, b2) == NULL) continue;
      auto it = e2[b2] ? parallel::traits::to_iterator(b1 * granularity)
                       : where(b1, b2);
      parallel::guard group(it, granularity);
      block_matrix<Matrix, SymmGroup> res;
      gemm(
          *GRID(b1, b2), rhs, res,
          parallel::scheduler_size_indexed(*GRID(b1, b2))
      );
      swap(*GRID(b1, b2), res);
      if (!e2[b2]) return;
    }
  }
  void multiply_column_trans(
      size_t b2, const block_matrix<Matrix, SymmGroup>& rhs
  ) {
    block_matrix<Matrix, SymmGroup> tmp;
    auto red = transpose(reduce_column(b2));
    parallel::guard group(
        parallel::traits::permute(b2, mpo.placement_r, granularity), granularity
    );
  ...
  block_matrix<Matrix, SymmGroup>& reduce_column(size_t b2) {
    ...
    parallel::traits::resource_iterator center =
        parallel::traits::permute(b2, mpo.placement_r, granularity);
    int r = parallel::traits::distance(center) / granularity;

    std::vector<std::pair<
        block_matrix<Matrix, SymmGroup>*, parallel::traits::resource_iterator> >
        rvector;
    for (int b1 = r; b1 < size1; b1++)
      if (GRID(b1, b2) != NULL)
        rvector.push_back(std::make_pair(
            GRID(b1, b2), parallel::traits::to_iterator(b1 * granularity)
        ));
        ...
        ambient::reduce(
            ...
            [this](
                std::pair<
                    ...
                    parallel::traits::resource_iterator>& dst_pair,
                std::pair<
                    block_matrix<Matrix, SymmGroup>*,
                    parallel::traits::resource_iterator>& src_pair
            ) {
              ...
              for (size_t k = 0; k < src.n_blocks(); ++k) {
                size_t index = dst.find_block(
                    src.basis().left_charge(k), src.basis().right_charge(k)
                );
                // alternative: index = src.size_index(k);
                parallel::guard proc(scheduler(index));
                dst.match_and_add_block(
                    src[k], src.basis().left_charge(k),
    ...
    storage::migrate(
        *rvector[0].first, parallel::scheduler_size_indexed(*rvector[0].first)
    );
  ...
  block_matrix<Matrix, SymmGroup>& reduce() {
    ...
    for (int b2 = 0; b2 < size2; b2++)
      for (int b1 = 0; b1 < size1; b1++) {
        if (GRID(b1, b2) == NULL) continue;
        parallel::traits::resource_iterator owner =
            e2[b2]
                ? parallel::traits::to_iterator(b1 * granularity)
                : parallel::traits::permute(b2, mpo.placement_r, granularity);
    ...
    std::sort(
        ...
        [](const std::pair<
               block_matrix<Matrix, SymmGroup>*,
               parallel::traits::resource_iterator>& a,
           const std::pair<
               block_matrix<Matrix, SymmGroup>*,
               parallel::traits::resource_iterator>& b) {
    ...
    block_matrix<Matrix, SymmGroup>* skeleton =
        new block_matrix<Matrix, SymmGroup>();
    block_matrix<Matrix, SymmGroup>* res =
        new block_matrix<Matrix, SymmGroup>();
    ...
    for (size_t n = 0; n < rvector.size(); ++n) {
      ...
      for (size_t k = 0; k < src.n_blocks(); ++k) {
        if (!skeleton->has_block(
                src.basis().left_charge(k), src.basis().right_charge(k)
            )) {
          skeleton->insert_block(
              new Matrix(), src.basis().left_charge(k),
              src.basis().right_charge(k)
          );
        }
    ...
    std::vector<
        std::vector<std::pair<Matrix*, parallel::traits::resource_iterator> > >
        rblocks;
    size_t max_stride = 0;
    for (size_t k = 0; k < skeleton->n_blocks(); ++k) {
      ...
      for (size_t n = 0; n < blocks.size(); n++) {
        if (tc1 == c1[n] && tc2 == c2[n])
          rblocks_part.push_back(std::make_pair(blocks[n], owners[n]));
      }
      parallel::traits::resource_iterator root = scheduler(k);
      std::sort(
          rblocks_part.begin(), rblocks_part.end(),
          [root](
              const std::pair<Matrix*, parallel::traits::resource_iterator>& a,
              const std::pair<Matrix*, parallel::traits::resource_iterator>& b
          ) {
            return (ambient::num_procs() + *a.second - *root) %
                       ambient::num_procs() <
                   (ambient::num_procs() + *b.second - *root) %
                       ambient::num_procs();
          }
      );
      rblocks_part[0].second = root;
      rblocks.push_back(rblocks_part);
      if (rblocks_part.size() > max_stride) max_stride = rblocks_part.size();
    }
      ...
      for (size_t n = 0; n < rblocks.size(); ++n) {
        ...
        if (stride < rblocks_part.size())
          for (int k = stride; k < rblocks_part.size(); k += stride * 2) {
            std::pair<Matrix*, parallel::traits::resource_iterator>& dst_pair =
                rblocks_part[k - stride];
            std::pair<Matrix*, parallel::traits::resource_iterator>& src_pair =
                rblocks_part[k];
  ...
  void print_distribution() {
    ...
    for (int b2 = 0; b2 < size2; b2++)
      for (int b1 = 0; b1 < size1; b1++)
        if (GRID(b1, b2) != NULL) total += GRID(b1, b2)->num_elements();
    printf(
        "%.2f GB:",
        total * sizeof(typename Matrix::value_type) / 1024 / 1024 / 1024
    );
        ...
        for (int b1 = 0; b1 < size1; b1++)
          if (GRID(b1, b2) != NULL)
            for (int i = 0; i < (*GRID(b1, b2)).n_blocks(); ++i) {
              if (!ambient::weak((*GRID(b1, b2))[i][0]) &&
                  parallel::traits::placement((*GRID(b1, b2))[i][0]) ==
                      *parallel::traits::to_iterator(p))
                part +=
                    num_rows((*GRID(b1, b2))[i]) * num_cols((*GRID(b1, b2))[i]);
            }
...
template <class Matrix, class SymmGroup>
int ContractionGrid<Matrix, SymmGroup>::reduction_iteration = 0;
template <class Matrix, class SymmGroup>
int ContractionGrid<Matrix, SymmGroup>::reduction_max_iteration = 1;
}  // namespace contraction
