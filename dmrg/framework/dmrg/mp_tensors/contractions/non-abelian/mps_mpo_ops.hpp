...
namespace SU2 {
    ...
    typename MPS<Matrix, SymmGroup>::scalar_type norm(MPS<Matrix, SymmGroup> const & mps)
    {
        ...
        for(size_t i = 0; i < L; ++i) {
            ...
            left = contraction::common::overlap_left_step<Matrix, Matrix, SymmGroup, SU2Gemms>(mps[i], cpy, left); // serial
        }
    ...
    typename MPS<Matrix, SymmGroup>::scalar_type norm_r(MPS<Matrix, SymmGroup> const & mps)
    {
        ...
        for(int i = L-1; i >= 0 ; --i) {
            select_proc(ambient::scope::balance(i,L));
            MPSTensor<Matrix, SymmGroup> cpy = mps[i];
            right = contraction::common::overlap_right_step<Matrix, Matrix, SymmGroup, SU2Gemms>(mps[i], cpy, right); // serial
        }
                  ...
                  int p1, int p2, std::vector<int> config)
    {
        ...
        for(size_t i = 0; i < L; ++i) {
            ...
            left = contraction::common::overlap_mpo_left_step(cpy, mps[i], left, mpo[i]);
        }
