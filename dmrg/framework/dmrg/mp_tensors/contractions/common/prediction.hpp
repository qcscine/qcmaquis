...
predict_new_state_l2r_sweep(
    ...
    Boundary<OtherMatrix, SymmGroup> const& right, double alpha, double cutoff,
    std::size_t Mmax, bool doPerturbDM, bool verbose = false
) {
  ...
  if (doPerturbDM) {
    ...
    Boundary<Matrix, SymmGroup> half_dm =
        left_boundary_tensor_mpo<Matrix, OtherMatrix, SymmGroup, Gemm, Kernel>(
            mps, left, mpo
        );
    for (int b = 0; b < half_dm.aux_dim(); ++b) {
      ...
      for (int k = 0; k < tdm.n_blocks(); ++k) {
        if (mps.data().basis().has(
                tdm.basis().left_charge(k), tdm.basis().right_charge(k)
            ))
          dm.match_and_add_block(
              tdm[k], tdm.basis().left_charge(k), tdm.basis().right_charge(k)
          );
      }
    }
...
static block_matrix<Matrix, SymmGroup> getZeroSiteTensorL2R(
    MPSTensor<Matrix, SymmGroup> mpsNextSite,
...
predict_new_state_r2l_sweep(
  ...
  if (doPerturbDM) {
    ...
    Boundary<Matrix, SymmGroup> half_dm =
        right_boundary_tensor_mpo<Matrix, OtherMatrix, SymmGroup, Gemm, Kernel>(
            mps, right, mpo
        );
...
static block_matrix<Matrix, SymmGroup> getZeroSiteTensorR2L(
    MPSTensor<Matrix, SymmGroup> mpsPreviousSite,
