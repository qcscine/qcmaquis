...
//
// Function template for the calculation of the T matrix by assuming a diagonal
// MPO (see JCP 2015 for additional details). The quantity which is computed is:
//
//  -----  -----   -----          o_l o_l'      |    b_{l-1}          |\/| o_l'
//  \      \       \       \    /               |                     |  |
//  /      /       /        \/\/  b_{l-1} b_l   |___ a_{l-1} a_{l-1}' |  |
//  a_{l-1}' a_l'
//  -----  -----   -----
//   o_l' a_{l-1}' b_{l-1}  |                                        |
//                          +----------------------------------------+
//                                   |
//                                   +--------------->    This is T (summed over
//                                   b). In principle has 5
//                                                        indexes, but only 3 in
//                                                        this case due to
//                                                        symmetry. Note that we
//                                                        sum over the left
//                                                        boundary.
//
// where W is the matrix involved in the definition of the MPO, L is the left
// boundary and M is the matrix involved in the definition of the MPS. We have <
// a_{l-1} a_l o_l | H | o_l' a_{l-1}' a_l' > . Taking the diagonal part means
// that we are taking o_l = o_l' , a_l = a_l' and a_{l-1} = a_{l-1}'. The only
// sum surviving is the one over b_{l-1} NOTE: the index b_l (column) is given
// as input and is fixed throughout the routine
//
//
template <class Matrix, class OtherMatrix, class SymmGroup>
block_matrix<Matrix, SymmGroup> lbtm_diag_kernel(
    size_t b2, Boundary<OtherMatrix, SymmGroup> const &left,
    MPOTensor<Matrix, SymmGroup> const &mpo, Index<SymmGroup> const &out_left_i,
    Index<SymmGroup> const &left_i, Index<SymmGroup> const &right_i,
    Index<SymmGroup> const &phys_i, ProductBasis<SymmGroup> const &left_pb
) {
  //
  // Initialization
  // --------------
  ...
  for (typename col_proxy::const_iterator col_it = col_b2.begin();
    ...
    for (size_t block = 0; block < right_i.size(); ++block) {
      ...
      if (o == ret.n_blocks()) {
        o = ret.insert_block(
            Matrix(out_left_i[block].second, right_i[block].second), in_charge,
            in_charge
        );
      }
      // Loop over the physical indexes
      for (size_t s = 0; s < phys_i.size(); ++s) {
        // Symmetry requirement : o_l*a_{l-1} == a_l'. In this way we determine
        // a_{l-1}, which is also the index of the left boundary to be
        // extracted.
        ...
        if (l_block == left[b1].n_blocks()) {
          continue;
        }
        // copy the diagonal elements of the boundary into a vector
        std::vector<value_type> left_diagonal(left_i[l].second);
        // Diagonal is a method of the alps matrix class that gives two
        // iterators, for the beginning and the end of the diagonal.
        std::copy(
            left[b1][l_block].diagonal().first,
            left[b1][l_block].diagonal().second, left_diagonal.begin()
        );
        ...
        for (size_t w_block = 0; w_block < W.basis().size(); ++w_block) {
          ...
          if (phys_charge != phys_in || phys_in != phys_out) {
            continue;
          }
          using block_iterator =
              typename SparseOperator<Matrix, SymmGroup>::const_iterator;
          std::pair<block_iterator, block_iterator> blocks =
              W.get_sparse().block(w_block);
...
//
//  -----     -----  o_l o_l      +--+  b_l       -+
//  \           |                 +--+             |-> this is has three index
//  (one of which is sigma) /           |    b_l a_{l-1}  |  \ a_l a_l    -+ as
//  any other MPSTensor.
//  -----
//   a_l'
//
...
//
template <class Matrix, class OtherMatrix, class SymmGroup>
block_matrix<Matrix, SymmGroup> diagonal_hamiltonian(
  ...
  for (size_t b2 = 0; b2 < right.aux_dim(); ++b2) {
    block_matrix<Matrix, SymmGroup> lb2 = lbtm_diag_kernel(
        b2, left, mpo, out_left_i, x.row_dim(), x.col_dim(), physical_i,
        out_left_pb
    );
    for (size_t block = 0; block < lb2.n_blocks(); ++block) {
      ...
      if (rblock != right[b2].n_blocks()) {
        ...
        for (size_t c = 0; c < num_cols(lb2[block]); ++c) {
          std::transform(
              lb2[block].col(c).first, lb2[block].col(c).second,
              lb2[block].col(c).first,
              [&](const auto &e) { return e * right[b2][rblock](c, c); }
          );
        }
