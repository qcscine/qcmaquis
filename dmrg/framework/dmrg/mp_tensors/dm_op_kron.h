...
void dm_kron(
    Index<SymmGroup> const& phys, block_matrix<Matrix, SymmGroup> const& A,
    block_matrix<Matrix, SymmGroup> const& B, block_matrix<Matrix, SymmGroup>& C
) {
  ...
  for (int i = 0; i < A.n_blocks(); ++i) {
    for (int j = 0; j < B.n_blocks(); ++j) {
      charge new_left =
          phys_fuse(A.basis().left_charge(i), B.basis().left_charge(j));
      charge new_right =
          phys_fuse(A.basis().right_charge(i), B.basis().right_charge(j));
...
template <class Matrix, class SymmGroup>
void dm_group_kron(
    Index<SymmGroup> const& phys_psi, block_matrix<Matrix, SymmGroup> const& A,
  ...
  for (int i = 0; i < A.n_blocks(); ++i) {
    for (int j = 0; j < B.n_blocks(); ++j) {
      charge new_left =
          group(A.basis().left_charge(i), -B.basis().left_charge(j));
      charge new_right =
          group(A.basis().right_charge(i), -B.basis().right_charge(j));

      Matrix tmp(
          phys_rho.size_of_block(new_left), phys_rho.size_of_block(new_right), 0
      );

      maquis::dmrg::detail::op_kron(
          tmp, B[j], A[i], 0, 0, A.basis().left_size(i), B.basis().left_size(j),
          A.basis().right_size(i), B.basis().right_size(j)
      );
