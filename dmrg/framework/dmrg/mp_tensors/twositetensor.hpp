...
TwoSiteTensor<Matrix, SymmGroup>::TwoSiteTensor(
  ...
  gemm(
      mps1.data(), mps2.data(), data_, parallel::scheduler_balanced(mps1.data())
  );
}
...
template <class Matrix, class SymmGroup>
Index<SymmGroup> const& TwoSiteTensor<Matrix, SymmGroup>::local_site_dim(short d
) const {
...
void TwoSiteTensor<Matrix, SymmGroup>::make_left_paired() const {
  if (cur_storage == TSLeftPaired) return;

  block_matrix<Matrix, SymmGroup> tmp;
  if (cur_storage == TSBothPaired) {
    ts_reshape::reshape_both_to_left<Matrix, SymmGroup>(
        phys_i_left, phys_i_right, left_i, right_i, data_, tmp
    );
  } else {
...
void TwoSiteTensor<Matrix, SymmGroup>::make_both_paired() const {
  if (cur_storage == TSBothPaired) return;

  block_matrix<Matrix, SymmGroup> tmp;
  if (cur_storage == TSRightPaired) {
    ts_reshape::reshape_right_to_both<Matrix, SymmGroup>(
        phys_i_left, phys_i_right, left_i, right_i, data_, tmp
    );
  } else {
    ts_reshape::reshape_left_to_both<Matrix, SymmGroup>(
        phys_i_left, phys_i_right, left_i, right_i, data_, tmp
    );
  }
...
void TwoSiteTensor<Matrix, SymmGroup>::make_right_paired() const {
  if (cur_storage == TSRightPaired) return;

  block_matrix<Matrix, SymmGroup> tmp;
  if (cur_storage == TSBothPaired)
    ts_reshape::reshape_both_to_right<Matrix, SymmGroup>(
        phys_i_left, phys_i_right, left_i, right_i, data_, tmp
    );
  else {
    // direct left to right reshape should not be needed
...
template <class Matrix, class SymmGroup>
MPSTensor<Matrix, SymmGroup> TwoSiteTensor<Matrix, SymmGroup>::make_mps(
) const {
...
TwoSiteTensor<Matrix, SymmGroup>::make_mps_(type_helper<SU2>) const {
  make_right_paired();
  return MPSTensor<Matrix, SymmGroup>(
      phys_i, left_i, right_i, data_, RightPaired
  );
}

template <class Matrix, class SymmGroup>
MPSTensor<Matrix, SymmGroup>
TwoSiteTensor<Matrix, SymmGroup>::make_mps_(type_helper<true>) const {
  make_right_paired();
  block_matrix<Matrix, SymmGroup> tmp;
  Index<SymmGroup> phys_out = ts_reduction::reduce_right(
      phys_i_left, phys_i_right, left_i, right_i, data_, tmp
  );
  return MPSTensor<Matrix, SymmGroup>(
      phys_out, left_i, right_i, tmp, RightPaired
  );
}
...
std::tuple<
    ...
    truncation_results>
TwoSiteTensor<Matrix, SymmGroup>::split_mps_l2r(
  ...
  MPSTensor<Matrix, SymmGroup> mps_tensor1(
      phys_i_left, left_i, u.right_basis(), u, LeftPaired
  );
  assert(mps_tensor1.reasonable());
  gemm(s, v, u);
  MPSTensor<Matrix, SymmGroup> mps_tensor2(
      phys_i_right, u.left_basis(), right_i, u, RightPaired
  );
...
std::tuple<
    MPSTensor<Matrix, SymmGroup>, MPSTensor<Matrix, SymmGroup>,
    truncation_results>
TwoSiteTensor<Matrix, SymmGroup>::predict_split_l2r(
    std::size_t Mmax, double cutoff, double alpha,
    const Boundary<Matrix, SymmGroup>& left,
  ...
  gemm(
      data_, transpose(conjugate(data_)), dm,
      parallel::scheduler_balanced(data_)
  );
  /// state prediction
  if (activatePerturbation) {
    maquis::cout << "Perturbing two-site tensor, noise parameter alpha = "
                 << alpha << std::endl;
    Index<SymmGroup> right_phys_i = adjoin(phys_i_right) * right_i;
    MPSTensor<Matrix, SymmGroup> tmp(
        phys_i_left, left_i, right_phys_i, data_, LeftPaired
    );
    Boundary<Matrix, SymmGroup> half_dm = contraction::Engine<
        Matrix, Matrix, SymmGroup>::left_boundary_tensor_mpo(tmp, left, mpo);
    ...
    for (std::size_t b = 0; b < half_dm.aux_dim(); ++b) {
      ...
      for (std::size_t k = 0; k < tdm.n_blocks(); ++k) {
        if (data_.basis().has(
                tdm.basis().left_charge(k), tdm.basis().right_charge(k)
            ))
          dm.reserve(
              tdm.basis().left_charge(k), tdm.basis().right_charge(k),
    ...
    omp_for(std::size_t k, parallel::range<std::size_t>(0, dm.n_blocks()), {
      for (std::size_t b = 0; b < half_dm.aux_dim(); ++b) {
        std::size_t match = half_dm[b].find_block(
            dm.basis().left_charge(k), dm.basis().right_charge(k)
        );
        if (match < half_dm[b].n_blocks()) dm[k] += (half_dm[b][match]);
      }
    });
  }
...
std::tuple<
    MPSTensor<Matrix, SymmGroup>, MPSTensor<Matrix, SymmGroup>,
    truncation_results>
TwoSiteTensor<Matrix, SymmGroup>::predict_split_r2l(
    std::size_t Mmax, double cutoff, double alpha,
    const Boundary<Matrix, SymmGroup>& right,
    const MPOTensor<Matrix, SymmGroup>& mpo, bool activatePerturbation
) {
  make_both_paired();
  ...
  block_matrix<Matrix, SymmGroup> dm;
  gemm(
      transpose(conjugate(data_)), data_, dm,
      parallel::scheduler_balanced(data_)
  );
  /// state prediction
  if (activatePerturbation) {
    ...
    MPSTensor<Matrix, SymmGroup> tmp(
        phys_i_right, left_phys_i, right_i, data_, RightPaired
    );
    Boundary<Matrix, SymmGroup> half_dm = contraction::Engine<
        Matrix, Matrix, SymmGroup>::right_boundary_tensor_mpo(tmp, right, mpo);
    tmp = MPSTensor<Matrix, SymmGroup>();
    omp_for(std::size_t b, parallel::range<std::size_t>(0, half_dm.aux_dim()), {
      block_matrix<Matrix, SymmGroup> tdm;
  ...
  MPSTensor<Matrix, SymmGroup> mps_tensor2(
      phys_i_right, U.left_basis(), right_i, transpose(conjugate(U)),
      RightPaired
  );
  assert(mps_tensor2.reasonable());
  ...
  MPSTensor<Matrix, SymmGroup> mps_tensor1(
      phys_i_left, left_i, V.right_basis(), V, LeftPaired
  );
  assert(mps_tensor1.reasonable());
...
std::ostream& operator<<(
    std::ostream& os, TwoSiteTensor<Matrix, SymmGroup> const& mps
) {
...
template <class Matrix, class SymmGroup>
block_matrix<Matrix, SymmGroup> const& TwoSiteTensor<Matrix, SymmGroup>::data(
) const {
  return data_;
}
...
template <class Matrix, class SymmGroup>
void TwoSiteTensor<Matrix, SymmGroup>::swap_with(
...
template <class Matrix, class SymmGroup>
TwoSiteTensor<Matrix, SymmGroup>& TwoSiteTensor<Matrix, SymmGroup>::operator<<(
    MPSTensor<Matrix, SymmGroup> const& rhs
) {
  return operator_shift(
      rhs, type_helper<symm_traits::HasSU2<SymmGroup>::value>()
  );
}

template <class Matrix, class SymmGroup>
template <bool SU2>
TwoSiteTensor<Matrix, SymmGroup>& TwoSiteTensor<Matrix, SymmGroup>::
    operator_shift(MPSTensor<Matrix, SymmGroup> const& rhs, type_helper<SU2>) {
  cur_storage = TSLeftPaired;
  rhs.make_left_paired();

  // Precondition: rhs.data() and this->data() have same shape if both are
  // left_paired
...
TwoSiteTensor<Matrix, SymmGroup>& TwoSiteTensor<Matrix, SymmGroup>::
  ...
  block_matrix<Matrix, SymmGroup> tmp;
  Index<SymmGroup> phys_out = ts_reduction::unreduce_left(
      phys_i_left, phys_i_right, left_i, right_i, rhs.data(), tmp
  );
  left_i = rhs.row_dim();
  right_i = rhs.col_dim();
  phys_i = phys_out;
