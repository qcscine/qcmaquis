...
class MPS {
 ...
 public:
  ...
  friend std::ostream& operator<<(
      std::ostream& os, MPS<Matrix_, SymmGroup_> const& mps
  );
  ...
  void move_normalization_l2r(
      size_t p1, size_t p2, DecompMethod method = DefaultSolver()
  );
  void move_normalization_r2l(
      size_t p1, size_t p2, DecompMethod method = DefaultSolver()
  );
  ...
  truncation_results grow_l2r_sweep(
      MPOTensor<Matrix, SymmGroup> const& mpo,
      Boundary<OtherMatrix, SymmGroup> const& left,
      Boundary<OtherMatrix, SymmGroup> const& right, std::size_t l,
      double alpha, double cutoff, std::size_t Mmax, bool perturbDM,
      bool verbose
  );
  template <class OtherMatrix>
  truncation_results grow_r2l_sweep(
      MPOTensor<Matrix, SymmGroup> const& mpo,
      Boundary<OtherMatrix, SymmGroup> const& left,
      Boundary<OtherMatrix, SymmGroup> const& right, std::size_t l,
      double alpha, double cutoff, std::size_t Mmax, bool perturbDM,
  ...
  void apply(
      typename operator_selector<Matrix, SymmGroup>::type const&,
      typename operator_selector<Matrix, SymmGroup>::type const&, size_type
  );
...
template <class Matrix, class SymmGroup>
MPS<Matrix, SymmGroup> join(
    MPS<Matrix, SymmGroup> const& a, MPS<Matrix, SymmGroup> const& b,
...
template <class Matrix, class SymmGroup>
MPS<Matrix, SymmGroup> join_general(
    MPS<Matrix, SymmGroup> const& a, MPS<Matrix, SymmGroup> const& b,
...
template <class Matrix, class SymmGroup>
MPS<Matrix, SymmGroup> joinAndTruncate(
    MPS<Matrix, SymmGroup>& a, MPS<Matrix, SymmGroup>& b, int mMax,
    typename MPSTensor<Matrix, SymmGroup>::scalar_type alpha = 1.,
    typename MPSTensor<Matrix, SymmGroup>::scalar_type beta = 1.
)
...
template <class Matrix, class SymmGroup>
Boundary<Matrix, SymmGroup> make_left_boundary(
  ...
  for (typename Index<SymmGroup>::basis_iterator it1 = i.basis_begin();
       !it1.end(); ++it1) {
    for (typename Index<SymmGroup>::basis_iterator it2 = j.basis_begin();
         !it2.end(); ++it2) {
      ret[0](*it1, *it2) = 1;
    }
  }
...
template <class Matrix, class SymmGroup>
Boundary<Matrix, SymmGroup> make_right_boundary(
    MPS<Matrix, SymmGroup> const& bra, MPS<Matrix, SymmGroup> const& ket
) {
  assert(ket.length() == bra.length());
