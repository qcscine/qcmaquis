 ...
 *     has a numbero of particles that is <= than the second one. Such a
 * comparison is implemented (and makes sense) only for U1-like groups.
 */
...
class ChargeDetailClass {
 public:
  ...
  static bool hasLessParticleThan(
      const ChargeType& inputCharge, const ChargeType& lowerCharge
  ) {
    return true;
  };
};
...
class ChargeDetailClass<U1> {
 public:
  ...
  static bool hasLessParticleThan(
      const ChargeType& inputCharge, const ChargeType& refCharge
  ) {
    return inputCharge <= refCharge;
  };
};
...
class ChargeDetailClass<NU1_template<N>> {
 public:
  ...
  static bool hasLessParticleThan(
    ...
    return std::mismatch(
               inputCharge.begin(), inputCharge.end(), lowerCharge.begin(),
               lowerCharge.end(), std::less_equal<>{}
           ) == std::make_pair(inputCharge.end(), lowerCharge.end());
  }
};
