...
class MPO : public std::vector<MPOTensor<Matrix, SymmGroup> > {
 public:
  ...
  MPO(std::size_t L, elem_type elem = elem_type())
      : std::vector<elem_type>(L, elem), core_energy(0) {}
 ...
 private:
  std::vector<std::map<std::size_t, typename SymmGroup::charge> >
      bond_index_charges;
      ...
      for (size_t c = 0; c < (*this)[p - 1].col_dim(); ++c) {
        ...
        for (size_t r = 0; r < (*this)[p - 1].row_dim(); ++r) {
          ...
          for (size_t b = 0; b < (*this)[p - 1].at(r, c).op().n_blocks(); ++b) {
            charge_diffs.insert(SymmGroup::fuse(
                bond_index_charges[p - 1][r],
                SymmGroup::fuse(
                    (*this)[p - 1].at(r, c).op().basis().left_charge(b),
                    -(*this)[p - 1].at(r, c).op().basis().right_charge(b)
                )
            ));
            //                        maquis::cout << r << " " << c <<
            //                        std::endl; maquis::cout <<
            //                        bond_index_charges[p-1][r] << std::endl;
            //                        maquis::cout <<
            //                        (*this)[p-1](r,c).basis().left_charge(b)
            //                        << std::endl; maquis::cout <<
            //                        (*this)[p-1](r,c).basis().right_charge(b)
            //                        << std::endl;
            //                        std::copy(charge_diffs.begin(),
            //                        charge_diffs.end(),
            //                                  std::ostream_iterator<typename
            //                                  SymmGroup::charge>(maquis::cout,
            //                                  " "));
        ...
        if (charge_diffs.size() == 1) {
          ...
          bond_index_charges[p][c] =
              SymmGroup::IdentityCharge;  // bond_index_charges[p-1][c];
        }
      }
    }
    ...
    for (size_t p = 0; p <= L; ++p) {
      ...
      for (const auto& it : bond_index_charges[p]) {
        if (index.has(it.second)) {
          index[index.position(it.second)] =
              std::make_pair(it.second, index.size_of_block(it.second) + 1);
        } else {
  ...
  block_matrix<Matrix, SymmGroup> make_left_matrix(std::size_t p) {
    ...
    for (size_t r = 0; r < (*this)[p].row_dim(); ++r) {
      for (size_t c = 0; c < (*this)[p].col_dim(); ++c) {
        ...
        for (size_t cs = 0; cs < (*this)[p].at(r, c).op().basis().size();
             ++cs) {
          // std::pair<charge, size_t> sector = (*this)[p].at(r,
          // c).op.left_basis()[cs];
          typename DualIndex<SymmGroup>::value_type sector =
              (*this)[p].at(r, c).op().basis()[cs];
      ...
      for (size_t r = 0; r < (*this)[p].row_dim(); ++r) {
        for (size_t ls = 0; ls < phys_i.size(); ++ls) {
          for (size_t rs = 0; rs < phys_i.size(); ++rs) {
            charge lc = SymmGroup::fuse(
                bond_index_charges[p][r],
                SymmGroup::fuse(phys_i[ls].first, -phys_i[rs].first)
            );
            ...
            if (!(*this)[p].at(r, c).op().has_block(
                    phys_i[ls].first, phys_i[rs].first
                )) {
              continue;
            }

            // std::size_t cs = (*this)[p].at(r,
            // c).op().left_basis().position(phys_i[ls].first);
            std::size_t cs = (*this)[p].at(r, c).op().basis().position(
                phys_i[ls].first, phys_i[rs].first
            );
            ...
            //                        << " | "; maquis::cout << r << " " << c <<
            //                        " " << phys_i[ls].first << " " <<
            //                        phys_i[rs].first; maquis::cout << " -> ";
            //                        maquis::cout << "(" << lc << "," << outr
            //                        << ") (" << rc << "," <<
            //                        visited_c_basis[rc] << ")" << std::endl;
          }
        }
      }
      ...
      for (size_t c = 0; c < (*this)[p].col_dim(); ++c) {
        for (size_t ls = 0; ls < phys_i.size(); ++ls) {
          for (size_t rs = 0; rs < phys_i.size(); ++rs) {
            ...
            charge rc = SymmGroup::fuse(
                bond_index_charges[p + 1][c],
                SymmGroup::fuse(-phys_i[ls].first, phys_i[rs].first)
            );
            ...
            ret(std::make_pair(lc, visited_r_basis[lc]),
                std::make_pair(rc, outc)) = (*this)[p].at(r, c).op()[cs](0, 0);
  ...
  void replace_pair(
      ...
      block_matrix<Matrix, SymmGroup>& right, std::size_t p
  ) {
    using charge = typename SymmGroup::charge;
    ...
    bond_indices[p + 1] = left.right_basis();
    {
      ...
      for (typename Index<SymmGroup>::basis_iterator it =
               left_right_basis_cp.basis_begin();
           !it.end(); ++it) {
        bond_index_charges[p + 1][count++] = (*it).first;
      }
    }

    (*this)[p] = MPOTensor<Matrix, SymmGroup>(
        (*this)[p].row_dim(), bond_indices[p + 1].sum_of_sizes()
    );
      ...
      for (size_t r = 0; r < (*this)[p].row_dim(); ++r) {
        for (size_t ls = 0; ls < phys_i.size(); ++ls) {
          for (size_t rs = 0; rs < phys_i.size(); ++rs) {
            ...
            typename Matrix::value_type val = left(
                std::make_pair(lc, outr),
                std::make_pair(rc, visited_c_basis[rc])
            );
    ...
    (*this)[p + 1] = MPOTensor<Matrix, SymmGroup>(
        bond_indices[p + 1].sum_of_sizes(), (*this)[p + 1].col_dim()
    );
      ...
      for (size_t c = 0; c < (*this)[p + 1].col_dim(); ++c) {
        for (size_t ls = 0; ls < phys_i.size(); ++ls) {
          for (size_t rs = 0; rs < phys_i.size(); ++rs) {
            ...
            typename Matrix::value_type val = right(
                std::make_pair(lc, visited_r_basis[lc]),
