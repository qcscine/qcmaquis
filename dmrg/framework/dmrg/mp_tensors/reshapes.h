...
void reshape_left_to_right(
    Index<SymmGroup> physical_i, Index<SymmGroup> left_i,
    Index<SymmGroup> right_i, block_matrix<Matrix, SymmGroup> const& m1,
  ...
  for (int run = 0; run < 2; ++run) {
    ...
    for (size_t block = 0; block < m1.n_blocks(); ++block) {
      for (size_t s = 0; s < physical_i.size(); ++s) {
        ...
        size_t l = left_i.position(
            SymmGroup::fuse(m1.basis().left_charge(block), -physical_i[s].first)
        );
        if (l == left_i.size()) {
          continue;
        }

        {
          ...
          size_t out_right_offset =
              out_right(physical_i[s].first, right_i[r].first);

          if (!pretend) {
            Matrix const& in_block = m1(in_l_charge, in_r_charge);
            Matrix& out_block = m2(out_l_charge, out_r_charge);

            maquis::dmrg::detail::reshape_l2r(
                in_block, out_block, in_left_offset, out_right_offset,
          ...
          if (pretend) {
            m2.reserve(
                out_l_charge, out_r_charge, left_i[l].second,
                out_right_offset + physical_i[s].second * right_i[r].second
            );
          }
        }
      }
...
/*
 ...
 for (s = 0; s < physical_i.size(); ++s)
         for (l = 0; ...)
                 for (r = 0; ...) {
             charge in_l_charge = SymmGroup::fuse(physical_i[s].first,
 left_i[l].first); charge in_r_charge = right_i[r].first;

                         charge out_l_charge = left_i[l].first;
                         charge out_r_charge =
 SymmGroup::fuse(-physical_i[s].first, right_i[r].first);

                         block_requirements[make_pair(out_l_charge,
 out_r_charge)].push_back(m1.position(in_l_charge, in_r_charge);
 }
  ...
  for (int run = 0; run < 2; ++run) {
    ...
    for (size_t block = 0; block < m1.n_blocks(); ++block) {
      for (size_t s = 0; s < physical_i.size(); ++s) {
        ...
        size_t r = right_i.position(
            SymmGroup::fuse(m1.basis().right_charge(block), physical_i[s].first)
        );
        if (r == right_i.size()) {
          continue;
        }

        {
          ...
          size_t in_right_offset =
              in_right(physical_i[s].first, right_i[r].first);
          size_t out_left_offset =
              out_left(physical_i[s].first, left_i[l].first);

          if (!pretend) {
            Matrix const& in_block = m1(in_l_charge, in_r_charge);
            Matrix& out_block = m2(out_l_charge, out_r_charge);
            maquis::dmrg::detail::reshape_r2l(
                out_block, in_block, out_left_offset, in_right_offset,
                physical_i[s].second, left_i[l].second, right_i[r].second
            );
          }
          ...
          if (pretend) {
            m2.reserve(
                ...
                out_left_offset + physical_i[s].second * left_i[l].second,
                right_i[r].second
            );
          }
        }
...
void reshape_left_to_right_new(
    Index<SymmGroup> const& physical_i, Index<SymmGroup> const& left_i,
  ...
  for (size_t block = 0; block < m1.n_blocks(); ++block) {
    ...
    for (size_t s = 0; s < physical_i.size(); ++s) {
      ...
      if (!m2.has_block(out_l_charge, out_r_charge)) {
        m2.insert_block(
            Matrix(left_i[l].second, out_right.size(out_r_charge), 0),
            out_l_charge, out_r_charge
        );
      }
...
template <class Matrix, class SymmGroup>
void reshape_and_pad_left(
    Index<SymmGroup> physical_i, Index<SymmGroup> in_left_i,
    Index<SymmGroup> in_right_i, Index<SymmGroup> out_left_i,
    Index<SymmGroup> out_right_i, block_matrix<Matrix, SymmGroup> const& m1,
    block_matrix<Matrix, SymmGroup>& m2
) {
  m2 *= 0;
  ...
  for (size_t block = 0; block < m1.n_blocks(); ++block) {
    for (size_t s = 0; s < physical_i.size(); ++s) {
      ...
      if (r == in_right_i.size()) {
        continue;
      }

      size_t l = in_left_i.position(
          SymmGroup::fuse(m1.basis().left_charge(block), -physical_i[s].first)
      );
      if (l == in_left_i.size()) {
        continue;
...

      {
        ...
        if (!out_left_i.has(in_left_i[l].first)) {
          continue;
        }
        if (!m1.has_block(l_charge, r_charge)) {
          continue;
        }
        if (!m2.has_block(l_charge, r_charge)) {
          continue;
        }

        size_t in_left_offset =
            in_left(physical_i[s].first, in_left_i[l].first);
        size_t out_left_offset =
            out_left(physical_i[s].first, in_left_i[l].first);
...
// MD: here we reshape without the `pretend` loop. This way i) we are faster,
// ii) we avoid the bug that the size is not correct. example for ii): the
// output is going to be multiplied with a left_i containing more elements, than
// the size of some blocks might be wrong. Such a situation originates in
// overlap() if right_i and left_i are very different: the block_matrix will
// contain only diagonal blocks. Moving physical index from right to left
// [left_i, (-phys_i, right_i)]  -->  [(phys_i, left_i), right_i]
template <class Matrix, class OtherMatrix, class SymmGroup>
void reshape_right_to_left_new(
    Index<SymmGroup> const& physical_i, Index<SymmGroup> const& left_i,
    Index<SymmGroup> const& right_i,
    block_matrix<OtherMatrix, SymmGroup> const& m1,
    block_matrix<Matrix, SymmGroup>& m2
  ...
  for (size_t block = 0; block < m1.n_blocks(); ++block) {
    ...
    for (size_t s = 0; s < physical_i.size(); ++s) {
      ...
      if (!m2.has_block(out_l_charge, out_r_charge)) {
        m2.insert_block(
            Matrix(
                out_left.size(physical_i[s].first, in_l_charge),
                right_i[r].second, 0
            ),
            out_l_charge, out_r_charge
        );
...
template <class Matrix, class SymmGroup>
void reshape_left_to_physleft(
    Index<SymmGroup> const& physical_i, Index<SymmGroup> const& left_i,
    Index<SymmGroup> const& right_i, block_matrix<Matrix, SymmGroup> const& m1,
    block_matrix<Matrix, SymmGroup>& m2
) {
  m2 = block_matrix<Matrix, SymmGroup>();
  ...
  for (size_t block = 0; block < m1.n_blocks(); ++block) {
    for (size_t s = 0; s < physical_i.size(); ++s) {
...

      {
        ...
        if (!m2.has_block(out_l_charge, out_r_charge)) {
          m2.insert_block(
              Matrix(physical_i[s].second, out_right.size(out_r_charge), 0),
              out_l_charge, out_r_charge
          );
        }
        ...
        for (size_t ss = 0; ss < physical_i[s].second; ++ss) {
          for (size_t rr = 0; rr < right_i[r].second; ++rr) {
            for (size_t ll = 0; ll < left_i[l].second; ++ll) {
              out_block(ss, out_right_offset + ll * right_i[r].second + rr) =
                  in_block(in_left_offset + ss * left_i[l].second + ll, rr);
            }
          }
        }
      }
...
void reshape_physleft_to_left(
  ...
  for (size_t block = 0; block < m1.n_blocks(); ++block) {
    ...
    for (size_t l = 0; l < left_i.size(); ++l) {
      ...
      size_t r = right_i.position(
          SymmGroup::fuse(m1.basis().right_charge(block), l_charge)
      );
      if (r == right_i.size()) {
        continue;
      }
...

      {
        ...
        if (!m2.has_block(out_l_charge, out_r_charge)) {
          m2.insert_block(
              Matrix(out_left.size(out_l_charge), right_i[r].second, 0),
              out_l_charge, out_r_charge
          );
        }
          ...
          for (size_t ss = 0; ss < physical_i[s].second; ++ss) {
            for (size_t ll = 0; ll < left_i[l].second; ++ll) {
              out_block(out_left_offset + ss * left_i[l].second + ll, rr) =
                  in_block(ss, in_right_offset + ll * right_i[r].second + rr);
            }
          }
        }
      }
...
template <class Matrix, class SymmGroup>
void reshape_physright_to_left(
    Index<SymmGroup> const& physical_i, Index<SymmGroup> const& left_i,
    Index<SymmGroup> const& right_i, block_matrix<Matrix, SymmGroup> const& m1,
    block_matrix<Matrix, SymmGroup>& m2
) {
  ...
  ProductBasis<SymmGroup> in_left(
      left_i, right_i,
      [&](const charge& a, const charge& b) { return SymmGroup::fuse(a, -b); }
  );
  ProductBasis<SymmGroup> out_left(physical_i, left_i);
  ...
  for (size_t block = 0; block < m1.n_blocks(); ++block) {
    for (size_t r = 0; r < right_i.size(); ++r) {
      ...
      size_t l = left_i.position(
          SymmGroup::fuse(m1.basis().left_charge(block), right_i[r].first)
      );
      if (l == left_i.size()) {
        continue;
      }

      {
        ...
        if (outb == m2.n_blocks()) {
          outb = m2.insert_block(
              new Matrix(out_left.size(out_l_charge), right_i[r].second, 0),
              out_l_charge, out_r_charge
          );
        }
        ...
        for (size_t ss = 0; ss < physical_i[s].second; ++ss) {
          for (size_t rr = 0; rr < right_i[r].second; ++rr) {
            for (size_t ll = 0; ll < left_i[l].second; ++ll) {
              out_block(out_left_offset + ss * left_i[l].second + ll, rr) =
                  in_block(in_left_offset + rr * left_i[l].second + ll, ss);
            }
          }
        }
      }
...
template <class Matrix, class SymmGroup>
block_matrix<Matrix, SymmGroup> reshape_2site_op(
    Index<SymmGroup> const& phys1, Index<SymmGroup> const& phys2,
    block_matrix<Matrix, SymmGroup> const& A
) {  // only for the dense matrices in MPO (30.04.2012 / scalar / value types
     // discussion)
      ...
      for (bi_t s3 = phys1.basis_begin(); !s3.end(); ++s3) {
        for (bi_t s4 = phys2.basis_begin(); !s4.end(); ++s4) {
          ...
          if (!ret.has_block(out_left_c, out_right_c)) {
            ret.insert_block(
                new Matrix(
                    pb_out_left.size(s1->first, -s3->first),
                    pb_out_right.size(s4->first, -s2->first), 0
                ),
          ...
          ret(std::make_pair(
                  out_left_c, out_left_offset +
                                  s1->second * phys1.size_of_block(s3->first) +
                                  s3->second
              ),
              std::make_pair(
                  out_right_c, out_right_offset +
                                   s2->second * phys2.size_of_block(s4->first) +
                                   s4->second
              )) =
              A(std::make_pair(
                    in_left_c, in_left_offset +
                                   s1->second * phys2.size_of_block(s2->first) +
                                   s2->second
                ),
                std::make_pair(
                    in_right_c,
                    in_right_offset +
                        s3->second * phys2.size_of_block(s4->first) + s4->second
                ));
        }
      }
    }
  ...
  for (typename Index<SymmGroup>::const_iterator it1 = out_left.begin();
       it1 != out_left.end(); it1++) {
    for (typename Index<SymmGroup>::const_iterator it2 = out_right.begin();
         it2 != out_right.end(); it2++) {
      bool empty = true;

      if (!ret.has_block(it1->first, it2->first)) {
        continue;
      }
...
// std::vector<Op> reshape_right_to_list (Index<SymmGroup> const & phys,
// block_matrix<typename Op::matrix_type, SymmGroup> const & A)
std::vector<Op> reshape_right_to_list(
    Index<SymmGroup> const& phys, Op2 const& A
) {
    ...
    for (bi_t s1 = phys.basis_begin(); !s1.end(); ++s1) {
      for (bi_t s2 = phys.basis_begin(); !s2.end(); ++s2) {
        ...
        if (!Ai.has_block(out_left_c, out_right_c)) {
          Ai.insert_block(
              new Matrix(
                  phys.size_of_block(s1->first), phys.size_of_block(s2->first),
                  0
              ),
              out_left_c, out_right_c
          );
        ...
        Ai(*s1, *s2) =
            A(std::make_pair(
                  in_left_c, in_left_offset +
                                 s1->second * phys.size_of_block(s2->first) +
                                 s2->second
              ),
              *b);
      }
...
template <class Op, class Op2, class SymmGroup>
// std::vector<Op> reshape_left_to_list (Index<SymmGroup> const & phys,
// block_matrix<typename Op::matrix_type, SymmGroup> const & A)
std::vector<Op> reshape_left_to_list(
    Index<SymmGroup> const& phys, Op2 const& A
) {
  using Matrix = typename Op::matrix_type;
  using charge = typename SymmGroup::charge;
