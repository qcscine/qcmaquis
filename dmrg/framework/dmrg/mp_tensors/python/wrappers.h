...
template <class sgrp>
struct charge_wrapped_as {};
template <>
struct charge_wrapped_as<TrivialGroup> {
  typedef EnumToClass<TrivialGroup::charge> type;
};
template <>
struct charge_wrapped_as<Ztwo> {
  typedef EnumToClass<Ztwo::charge> type;
};
...
struct wrapped_pair {
  ...
  wrapped_pair(raw_pair const& p) : data_(p) {}
  ...
  wrapped_charge get_first() const { return wrapped_charge(data_.first); }

  std::size_t get_second() const { return data_.second; }
