...
#define EXPORT_SYMM_GROUP(sgrp, name) \
  class_<sgrp>(name).def("fuse", &wrapped_fuse<sgrp>).staticmethod("fuse");
...
#define EXPORT_BLOCK_PAIR(sgrp, name)                               \
  class_<wrapped_pair<sgrp> >(name)                                 \
      ...
      .add_property(                                                \
          "first", &wrapped_pair<sgrp>::get_first,                  \
          &wrapped_pair<sgrp>::set_first                            \
      )                                                             \
      .add_property(                                                \
          "second", &wrapped_pair<sgrp>::get_second,                \
          &wrapped_pair<sgrp>::set_second                           \
      )                                                             \
...
#define EXPORT_BLOCK_MATRIX(matrix, sgrp, name)                   \
  ...
  EXPORT_BLOCK_MATRIX(
      alps::numeric::matrix<double>, TrivialGroup, "NG_d_matrix"
  );
