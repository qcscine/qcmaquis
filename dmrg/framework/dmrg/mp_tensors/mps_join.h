...
MPSTensor<Matrix, SymmGroup> join(
  ...
  if (m1.site_dim() != m2.site_dim())
    for (typename Index<SymmGroup>::const_iterator it = m2.site_dim().begin();
         it != m2.site_dim().end(); ++it)
      if (!phys_i.has(it->first)) phys_i.insert(*it);
  ...
  for (size_t t = 0; t < 2; ++t)  // t=0 --> mps1, t=1 --> mps2
  {
    ...
    for (size_t b = 0; b < m.data().n_blocks(); ++b) {
      typename SymmGroup::charge const& sl_charge =
          m.data().basis().left_charge(b);  // phys + left
      typename SymmGroup::charge const& r_charge =
          m.data().basis().right_charge(b);  // right
      size_t out_r_charge_i = out_right.position(r_charge);

      if (!ret.data().has_block(sl_charge, r_charge))
        ret.data().insert_block(
            Matrix(
                out_left_pb.size(sl_charge), out_right[out_r_charge_i].second
            ),
            sl_charge, r_charge
        );
      ...
      for (size_t s = 0; s < phys_i_m.size(); ++s) {
        ...
        typename SymmGroup::charge l_charge =
            SymmGroup::fuse(sl_charge, -s_charge);  // left

        if (!m.row_dim().has(l_charge)) continue;
        ...
        for (size_t ss = 0; ss < phys_i[s].second; ++ss) {
          copy_block(
              m.data()[b], in_l_offset + ss * in_l_size, 0, nb,
              out_l_offset + ss * out_l_size, out_r_offset, in_l_size, in_r_size
          );
        }
      }
    }
