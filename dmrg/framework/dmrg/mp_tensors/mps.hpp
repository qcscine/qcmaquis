...
template <class Matrix, class SymmGroup>
MPS<Matrix, SymmGroup>::MPS(
    std::initializer_list<MPSTensor<Matrix, SymmGroup> > l
)
    : data_{l}, canonized_i(std::numeric_limits<size_t>::max()) {}
...
template <class Matrix, class SymmGroup>
typename MPS<Matrix, SymmGroup>::value_type const&
MPS<Matrix, SymmGroup>::operator[](size_t i) const {
  return data_[i];
}

template <class Matrix, class SymmGroup>
typename MPS<Matrix, SymmGroup>::value_type& MPS<Matrix, SymmGroup>::operator[](
    size_t i
) {
  if (i != canonized_i) {
    canonized_i = std::numeric_limits<size_t>::max();
...
void MPS<Matrix, SymmGroup>::normalize_left() {
  ...
  auto normalizationFactor =
      (*this)[length() - 1].leftNormalizeAndReturn(DefaultSolver());
...
void MPS<Matrix, SymmGroup>::normalize_right() {
  ...
  auto normalizationFactor =
      (*this)[0].rightNormalizeAndReturn(DefaultSolver());
  if (maquis::real(normalizationFactor.trace()) < 0) {
    this->operator[](0) *= -1.;
  }
...
template <class Matrix, class SymmGroup>
void MPS<Matrix, SymmGroup>::move_normalization_l2r(
    size_t p1, size_t p2, DecompMethod method
) {
  parallel::scheduler_balanced scheduler(length());
...
template <class Matrix, class SymmGroup>
void MPS<Matrix, SymmGroup>::move_normalization_r2l(
    size_t p1, size_t p2, DecompMethod method
) {
  parallel::scheduler_balanced scheduler(length());
...
truncation_results MPS<Matrix, SymmGroup>::grow_l2r_sweep(
  ...
  using Contractor =
      typename contraction::Engine<Matrix, OtherMatrix, SymmGroup>;
  MPSTensor<Matrix, SymmGroup> new_mps;
  truncation_results trunc;
  std::tie(new_mps, trunc) = Contractor::predict_new_state_l2r_sweep(
      (*this)[l], mpo, left, right, alpha, cutoff, Mmax, perturbDM, verbose
  );
  (*this)[l + 1] = contraction::Engine<Matrix, OtherMatrix, SymmGroup>::
      predict_lanczos_l2r_sweep((*this)[l + 1], (*this)[l], new_mps);
  (*this)[l] = new_mps;
  return trunc;
}
...
template <class Matrix, class SymmGroup>
template <class OtherMatrix>
truncation_results MPS<Matrix, SymmGroup>::grow_r2l_sweep(
    MPOTensor<Matrix, SymmGroup> const& mpo,
    Boundary<OtherMatrix, SymmGroup> const& left,
    Boundary<OtherMatrix, SymmGroup> const& right, std::size_t l, double alpha,
    double cutoff, std::size_t Mmax, bool perturbDM, bool verbose
  ...
  MPSTensor<Matrix, SymmGroup> new_mps;
  truncation_results trunc;
  std::tie(new_mps, trunc) = Contractor::predict_new_state_r2l_sweep(
      (*this)[l], mpo, left, right, alpha, cutoff, Mmax, perturbDM, verbose
  );
  (*this)[l - 1] = contraction::Engine<Matrix, OtherMatrix, SymmGroup>::
      predict_lanczos_r2l_sweep((*this)[l - 1], (*this)[l], new_mps);
  (*this)[l] = new_mps;
  return trunc;
}
...
template <class Matrix, class SymmGroup>
void MPS<Matrix, SymmGroup>::apply(
    typename operator_selector<Matrix, SymmGroup>::type const& op,
    typename MPS<Matrix, SymmGroup>::size_type p
) {
  ...
  if (op.n_blocks() > 0) {
    diff =
        SymmGroup::fuse(op.basis().right_charge(0), -op.basis().left_charge(0));
  }
  for (size_t n = 0; n < op.n_blocks(); ++n) {
    if (SymmGroup::fuse(
            op.basis().right_charge(n), -op.basis().left_charge(n)
        ) != diff) {
      throw std::runtime_error(
          "Operator not allowed. All non-zero blocks have to provide same "
          "`diff`."
      );
    }
  }
...
void load(std::string const& dirname, MPS<Matrix, SymmGroup>& mps) {
  ...
  for (size_t k = 0; k < loop_max; ++k) {
    ...
    std::string fname =
        dirname + "/mps" + boost::lexical_cast<std::string>((size_t)k) + ".h5";
...
void save(std::string const& dirname, MPS<Matrix, SymmGroup> const& mps) {
  ...
  for (size_t k = 0; k < loop_max; ++k) {
    ...
    const std::string fname = dirname + "/mps" +
                              boost::lexical_cast<std::string>((size_t)k) +
                              ".h5.new";
  ...
  parallel::sync(
  );  // be sure that chkp is in valid state before overwriting the old one.
...
void check_equal_mps(
    MPS<Matrix, SymmGroup> const& mps1, MPS<Matrix, SymmGroup> const& mps2
) {
  // Length
...
/** @brief Function to ensure consistent indices across bonds by removing blocks
 * without connection across bonds */
template <class Matrix, class SymmGroup>
void clean_mps(MPS<Matrix, SymmGroup>& mps) {
  ...
  do {
    ...
    for (size_t p = 0; p < mps.length() - 1; ++p) {
      ...
      block_matrix<Matrix, SymmGroup> bm1 = mps[p].data();
      block_matrix<Matrix, SymmGroup> bm2 = mps[p + 1].data();
