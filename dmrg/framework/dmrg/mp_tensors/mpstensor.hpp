...
MPSTensor<Matrix, SymmGroup>::MPSTensor(
    Index<SymmGroup> const& sd, Index<SymmGroup> const& ld,
  ...
  // remove blocks from the right index that may not be allowed by the left
  // index
  right_i = rb;
  // remove blocks from the left index that may not be allowed by the right
  // index
  ...
  // this is safe, since sd \otimes ld = rd forces the block_matrix to be
  // diagonal
  lb.sort();
  rb.sort();
  left_i.sort();
  ...
  if (fillrand) {
    data().generate(
        static_cast<dmrg_random::value_type (*)()>(&dmrg_random::uniform)
    );
...
MPSTensor<Matrix, SymmGroup>::MPSTensor(
    Index<SymmGroup> const& sd, Index<SymmGroup> const& ld,
    Index<SymmGroup> const& rd, block_matrix<Matrix, SymmGroup> const& block,
    MPSStorageLayout layout, Indicator normalization
)
    : phys_i(sd),
      left_i(ld),
      right_i(rd),
...
template <class Matrix, class SymmGroup>
void MPSTensor<Matrix, SymmGroup>::replace_left_paired(
    block_matrix<Matrix, SymmGroup> const& rhs, Indicator normalization
) {
  make_left_paired();
...
template <class Matrix, class SymmGroup>
void MPSTensor<Matrix, SymmGroup>::replace_right_paired(
    block_matrix<Matrix, SymmGroup> const& rhs, Indicator normalization
) {
  make_right_paired();
...
void MPSTensor<Matrix, SymmGroup>::make_left_paired() const {
  ...
  reshape_right_to_left_new<Matrix>(phys_i, left_i, right_i, data(), tmp);
  cur_storage = LeftPaired;
  swap(data_, tmp);
...
void MPSTensor<Matrix, SymmGroup>::make_right_paired() const {
  ...
  block_matrix<Matrix, SymmGroup> tmp;
  reshape_left_to_right_new<Matrix>(phys_i, left_i, right_i, data(), tmp);
...
template <class Matrix, class SymmGroup>
void MPSTensor<Matrix, SymmGroup>::performQR(
    BlockMatrixType& Q, BlockMatrixType& R
) {
  make_left_paired();
  qr(data(), Q, R);
  swap(data(), Q);
...
template <class Matrix, class SymmGroup>
void MPSTensor<Matrix, SymmGroup>::performLQ(
    BlockMatrixType& L, BlockMatrixType& Q
) {
  make_right_paired();
  lq(data(), L, Q);
  swap(data(), Q);
...
template <class Matrix, class SymmGroup>
void MPSTensor<Matrix, SymmGroup>::performSVD(
    BlockMatrixType& U, BlockMatrixType& V, BlockMatrixDiagonalType& S
) {
  make_right_paired();
  svd(data(), U, V, S);
  left_i = V.left_basis();
...
template <class Matrix, class SymmGroup>
void MPSTensor<Matrix, SymmGroup>::shift_aux_charges(
    typename SymmGroup::charge diff
) {
  left_i.shift(diff);
  right_i.shift(diff);
...
void MPSTensor<Matrix, SymmGroup>::multiply_from_right(
    block_matrix<OtherMatrix, SymmGroup> const& N
) {
  cur_normalization = Unorm;
  block_matrix<Matrix, SymmGroup> tmp;
  make_left_paired();
...
template <class Matrix, class SymmGroup>
template <class OtherMatrix>
void MPSTensor<Matrix, SymmGroup>::multiply_from_left(
    block_matrix<OtherMatrix, SymmGroup> const& N
) {
  cur_normalization = Unorm;
  block_matrix<Matrix, SymmGroup> tmp;
...
MPSTensor<Matrix, SymmGroup>::scalar_overlap(
  ...
  // Bela says: this is a workaround for the very rare condition that
  // site_hamil2 removes blocks This shouldn't be necessary, but as of Rev.
  // 1702, is necessary in some cases If I haven't fixed this by the end of Feb
  // 2012, remind me
  ...
  }  // should be reformulated in terms of reduction (todo: Matthias, 30.04.12 /
     // scalar-value types)
...
std::ostream& operator<<(
    std::ostream& os, MPSTensor<Matrix, SymmGroup> const& mps
) {
...
template <class Matrix, class SymmGroup>
block_matrix<Matrix, SymmGroup> const& MPSTensor<Matrix, SymmGroup>::data(
) const {
  return const_data();
}

template <class Matrix, class SymmGroup>
block_matrix<Matrix, SymmGroup> const& MPSTensor<Matrix, SymmGroup>::const_data(
) const {
  return data_;
}

template <class Matrix, class SymmGroup>
MPSTensor<Matrix, SymmGroup> const& MPSTensor<Matrix, SymmGroup>::operator*=(
    const scalar_type& t
) {
  data() *= t;
  return *this;
...
bool MPSTensor<Matrix, SymmGroup>::reasonable() const {
  {
    ...
    //        maquis::cout << "reasonable::data_:" << std::endl << data() <<
    //        std::endl;
    make_right_paired();
...
template <class Matrix, class SymmGroup>
void MPSTensor<Matrix, SymmGroup>::check_equal(
    MPSTensor<Matrix, SymmGroup> const& rhs
) const {
  std::string error;
  // Indexes
  ...
  if (data().n_blocks() != rhs.data().n_blocks()) {
    ...
    for (int b = 0; b < data().n_blocks(); ++b) {
      if (data()[b].num_cols() != rhs.data()[b].num_cols() ||
          data()[b].num_rows() != rhs.data()[b].num_rows()) {
...
template <class Matrix, class SymmGroup>
void MPSTensor<Matrix, SymmGroup>::add_block_to_row(
    block_matrix<Matrix, SymmGroup>& bm
) {
  // The modified index is the row one, so the merged index is put into the
  // column
  make_right_paired();
  ...
  for (std::size_t idx = 0; idx < bm.n_blocks(); idx++) {
    ...
    this->data_.add_block_to_row(
        bm, bm.get_left_charge(idx), bm.get_right_charge(idx)
    );
  }
}

template <class Matrix, class SymmGroup>
void MPSTensor<Matrix, SymmGroup>::add_block_to_column(
    block_matrix<Matrix, SymmGroup>& bm
) {
  // The modified index is the column one, so the merged index is put into the
  // row
  make_left_paired();
  // Loop over all the symmetry blocks of the block_matrix object
  for (std::size_t idx = 0; idx < bm.n_blocks(); idx++) {
    ...
    this->data_.add_block_to_column(
        bm, bm.get_left_charge(idx), bm.get_right_charge(idx)
    );
  }
}
...
template <class Matrix, class SymmGroup>
void MPSTensor<Matrix, SymmGroup>::scaleByExponentialProductOfCharges(
    ...
    for (int iPhys = 0; iPhys < this->phys_i.size(); iPhys++) {
      if (phys_i[iPhys].first[0] == 1 && phys_i[iPhys].first[1] == 1) {
        ...
        if (left_i.has(trueLeftCharge)) {
          ...
          for (int iRow = 0; iRow < left_i.size_of_block(trueLeftCharge);
               iRow++)
            for (int iCol = 0; iCol < right_i.size_of_block(rightCharge);
                 iCol++)
              this->data_[iBlock](iRow + offset, iCol) *=
                  std::exp(scalingFactor);
        }
      }
    }
  }
...
template <class Matrix, class SymmGroup>
void MPSTensor<Matrix, SymmGroup>::fillWithEigenVector(
    const EigenVectorType& inputVector
) {
