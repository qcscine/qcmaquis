...
void reshape_both_to_left(
    Index<SymmGroup> const& physical_i_left,
    Index<SymmGroup> const& physical_i_right, Index<SymmGroup> const& left_i,
  ...
  for (size_t block = 0; block < m1.n_blocks(); ++block) {
    for (size_t s1 = 0; s1 < physical_i_left.size(); ++s1) {
      size_t l = left_i.position(SymmGroup::fuse(
          m1.basis().left_charge(block), -physical_i_left[s1].first
      ));
      if (l == left_i.size()) continue;
      for (size_t s2 = 0; s2 < physical_i_right.size(); ++s2) {
        size_t r = right_i.position(SymmGroup::fuse(
            m1.basis().right_charge(block), physical_i_right[s2].first
        ));
        if (r == right_i.size()) continue;

        {
          charge s_charge = SymmGroup::fuse(
              physical_i_left[s1].first, physical_i_right[s2].first
          );
          ...
          if (!m2.has_block(out_l_charge, out_r_charge))
            m2.insert_block(
                new Matrix(
                    out_left.size(s_charge, left_i[l].first), right_i[r].second,
                    0
                ),
                out_l_charge, out_r_charge
            );

          // maquis::dmrg::detail::reshape_b2l( m2(out_l_charge, out_r_charge),
          // m1(in_l_charge, in_r_charge),
          maquis::dmrg::detail::reshape_b2l(
              m2(out_l_charge, out_r_charge), m1[block],
              in_left(physical_i_left[s1].first, left_i[l].first),
              in_right(physical_i_right[s2].first, right_i[r].first),
              out_left(s_charge, left_i[l].first),
              phys_pb(physical_i_left[s1].first, physical_i_right[s2].first),
...
void reshape_left_to_both(
  ...
  for (size_t block = 0; block < m1.n_blocks(); ++block) {
    ...
    if (r == right_i.size())
      throw std::runtime_error("m1 matrix inconsistent with right_i.");
    ...
    for (size_t s1 = 0; s1 < physical_i_left.size(); ++s1)
      for (size_t s2 = 0; s2 < physical_i_right.size(); ++s2) {
        ...
        size_t l = left_i.position(
            SymmGroup::fuse(m1.basis().left_charge(block), -s_charge)
        );
        if (l == left_i.size()) continue;

        {
          ...
          if (o == m2.n_blocks())
            o = m2.insert_block(
                new Matrix(
                    out_left.size(physical_i_left[s1].first, left_i[l].first),
                    out_right.size(
                        -physical_i_right[s2].first, right_i[r].first
                    ),
                    0
                ),
                out_l_charge, out_r_charge
          ...
          maquis::dmrg::detail::reshape_l2b(
              m2[o], m1[block], in_left(s_charge, left_i[l].first),
              phys_pb(physical_i_left[s1].first, physical_i_right[s2].first),
              out_left(physical_i_left[s1].first, left_i[l].first),
              out_right(physical_i_right[s2].first, right_i[r].first),
              physical_i_left[s1].second, physical_i_right[s2].second,
              left_i[l].second, right_i[r].second
          );
        }
    ...
    for (size_t s1 = 0; s1 < physical_i_left.size(); ++s1)
      for (size_t s2 = 0; s2 < physical_i_right.size(); ++s2) {
        ...
        size_t r = right_i.position(
            SymmGroup::fuse(m1.basis().right_charge(block), s_charge)
        );
        if (r == right_i.size()) continue;

        {
          ...
          for (size_t ss1 = 0; ss1 < physical_i_left[s1].second; ++ss1)
            for (size_t ss2 = 0; ss2 < physical_i_right[s2].second; ++ss2) {
              size_t ss_out =
                  in_phys_offset + ss1 * physical_i_right[s2].second + ss2;
              for (size_t rr = 0; rr < right_i[r].second; ++rr)
                for (size_t ll = 0; ll < left_i[l].second; ++ll)
                  out_block(
                      out_left_offset + ss1 * left_i[l].second + ll,
                      out_right_offset + ss2 * right_i[r].second + rr
                  ) =
                      in_block(
                          ll, in_right_offset + ss_out * right_i[r].second + rr
                      );
            }
        }
      }
...

        {
          ...
          if (!m2.has_block(out_l_charge, out_r_charge))
            m2.insert_block(
                new Matrix(
                    left_i[l].second,
                    out_right.size(-s_charge, right_i[r].first), 0
                ),
                out_l_charge, out_r_charge
            );

          maquis::dmrg::detail::reshape_b2r(
              m2(out_l_charge, out_r_charge), m1[block],
              in_left(physical_i_left[s1].first, left_i[l].first),
              in_right(physical_i_right[s2].first, right_i[r].first),
              out_right(s_charge, right_i[r].first),
              phys_pb(physical_i_left[s1].first, physical_i_right[s2].first),
              physical_i_left[s1].second, physical_i_right[s2].second,
              left_i[l].second, right_i[r].second
          );
...
template <class Matrix, class SymmGroup>
void reshape_both_to_physright(
    Index<SymmGroup> const& physical_i_left,
    Index<SymmGroup> const& physical_i_right, Index<SymmGroup> const& left_i,
    Index<SymmGroup> const& right_i, block_matrix<Matrix, SymmGroup> const& m1,
    block_matrix<Matrix, SymmGroup>& m2
  ...
  ProductBasis<SymmGroup> out_left(
      left_i, right_i,
      [&](const charge& a, const charge& b) { return SymmGroup::fuse(-a, b); }
  );
  ProductBasis<SymmGroup> out_right(
      physical_i_left, physical_i_right,
      [&](const charge& a, const charge& b) { return SymmGroup::fuse(-a, b); }
  );
...

        {
          ...
          if (outb == m2.n_blocks())
            outb = m2.insert_block(
                new Matrix(
                    out_left.size(out_l_charge), out_right.size(out_r_charge), 0
                ),
                out_l_charge, out_r_charge
            );
          ...
          size_t out_right_offset =
              out_right(physical_i_left[s1].first, physical_i_right[s2].first);
          ...
          for (size_t ss1 = 0; ss1 < physical_i_left[s1].second; ++ss1)
            for (size_t ss2 = 0; ss2 < physical_i_right[s2].second; ++ss2)
              for (size_t rr = 0; rr < right_i[r].second; ++rr)
                for (size_t ll = 0; ll < left_i[l].second; ++ll)
                  out_block(
                      out_left_offset + rr * left_i[l].second + ll,
                      out_right_offset + ss2 * physical_i_left[s1].second + ss1
                  ) =
                      in_block(
                          in_left_offset + ss1 * left_i[l].second + ll,
