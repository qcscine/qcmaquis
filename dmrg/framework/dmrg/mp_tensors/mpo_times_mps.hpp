...
class MPOTimesMPSTraitClass {
 public:
   ...
   */
  MPOTimesMPSTraitClass(
      const MPSType& mps, const ModelType& model, const Lattice& lattice,
   ...
   * @param upOrDown Enum class indicating whether to ionize an alpha or a beta
   * electron.
  ...
  MPSType ionizeMPS(int siteToIonize, generate_mpo::IonizedOrbital upOrDown) {
    ...
    ionizedQN[0] -= 1;
    auto indexAllowed =
        allowed_sectors(siteTypes, siteBases, ionizedQN, bondDimension);
    auto destructorOperator = generate_mpo::make_destroy_mpo(
        latticeRef, modelRef, siteToIonize, upOrDown
    );
    //
    resetCharges();
    MPS<Matrix, SymmGroup> ionizedMPS(latticeRef.size());
    for (int iMPS = 0; iMPS < ionizedMPS.length(); iMPS++)
      ionizedMPS[iMPS] = mpo_times_mps(
          destructorOperator, mpsRef, iMPS, charges, indexAllowed
      );
    return ionizedMPS;
  }
  ...
  MPSType applyMPO(const MPOType& mpo) {
    auto indexAllowed =
        allowed_sectors(siteTypes, siteBases, totalQN, bondDimension);
    resetCharges();
   ...
   * The result of the contraction is an MPS with the bond dimension that is
   * larger than the original one by a factor of b_i-1 for the left index and
   * b_i for the right index. Mathematically, the contraction to perform is the
   * following:
   *
   * +---
   *  \    |\/| (\sigma_i)    \    / (\sigma_i,\sigma_i')  = |\/| (\sigma_i)
   *  /    |  | (a_{i-1},a_i)  \/\/  (b_{i-1},b_i)         = |  | (b_{i-1}
   * a_{i-1} , a_i b_i)
   * +---
   *
   ...
   *
   * 1) all matrices (the input MPSTensor and the output MPSTensor) are first
   * made right paired. 2) each pair of values (b_{i-1},b_i) will contribute to
   * a block of the final MPSTensor. All pairs must be "stacked" together to
   * give the final, overall tensor. We must pay attention to the fact that the
   * MPOTensor is stored in a sparse format, therefore only the pairs
   * (b_{i-1},b_i) giving non-zero elements are actually stored. 3) we first
   * loop over the row (b_{i-1}) and, for each element of the row, we add all
   * blocks of the MPSTensor obtained for all allowed values of b_i. Blocks are
   * put together with the [mps_join] method, by exploiting the fact that the
   * row can be recycled. This means that the overall structure of the MPS will
   * be:
   *
   *    +- -+ |                       |                     |     | | |   M(a_i,
   * a_{i+1} b_1) | M(a_i, a_{i+1} b_2) | ... | M(a_i, a_{i+1} b_)  | | | | | |
   *    +- -+
   *
   *    Clearly, the blocks will then be partitioned based on the symmetry
   * properties. 4) all the resulting blocks are then "stacked" one over the
   * other columnwise. Again, we can recycle the column index now.
   *
   * @tparam Matrix Class of the matrix in which the elements of the MPOTensor
   * are stored.
   * @tparam Matrix Class of the matrix in which the elements of the MPSTensor
   * are stored.
   ...
   * @param in_delta Charge difference that is "brought" by the left index of
   * the MPO.
   * @param allowed_sectors Index with the physically allowed symmetry sectors
   * per site (see mps_sectors.h for more details).
   * @return MPSTensor<Matrix, SymmGroup> result of the MPOTensor x MPSTensor
   * operation.
   */
  MPSTensor<Matrix, SymmGroup> mpo_times_mps(
      MPO<Matrix, SymmGroup> const& mpo, MPS<Matrix, SymmGroup> const& mps,
    ...
    // We transform the MPS to the right-paired representation and construct the
    // respective basis.
    ...
    ProductBasis<SymmGroup> right_pb(
        mps[site].site_dim(), mps[site].col_dim(),
        boost::lambda::bind(
            static_cast<charge (*)(charge, charge)>(SymmGroup::fuse),
    ...
    // We have to separate this step from what happens below because we check
    // that the input indexes are coherent.
    for (int iRow = 0; iRow < mpo[site].row_dim(); iRow++) {
      row_proxy row_b2 = mpo[site].row(iRow);
      for (typename row_proxy::const_iterator row_it = row_b2.begin();
           row_it != row_b2.end(); ++row_it) {
        // Access to the specific MPO element
        int iCol = row_it.index();
        term_descriptor<Matrix, SymmGroup, true> access =
            mpo[site].at(iRow, iCol);
        typename operator_selector<Matrix, SymmGroup>::type const& W =
            access.op();
        // Calculates the difference in symmetry that is "generated" by the
        // operator. Here we must remember that, in an MPS, the rows and the
        // columns will have the same symmetry. This is not true for the MPO,
        // since the local operator can "induce" a change in the symmetry. This
        // also means that, after the application of the MPO, the charges will
        // undergo a "shift".
        charge W_delta = SymmGroup::fuse(
            W.basis().right_charge(0), -W.basis().left_charge(0)
        );
        out_delta[iCol] = SymmGroup::fuse(in_delta[iRow], W_delta);
        // Loop over the symmetry blocks of the input MPS (that is now right
        // paired). Finds out which block of the final MPS would be populated by
        // the application of that MPO --> note that the final block might be
        // unphysical, in which case we neglect the term.
        for (size_t b = 0; b < data.n_blocks(); ++b) {
          charge lc = data.basis().left_charge(b);
          charge rc = data.basis().right_charge(b);
          // Loop over the symmetry block of the MPO (so, over the physical
          // dimensions).
          for (size_t w_block = 0; w_block < W.basis().size(); ++w_block) {
            ...
            // We extract the right index by difference of the ProductBasis and
            // the input physical index (times -1.).
            ...
            if (!ChargeDetailClass<SymmGroup>::physical(out_l_charge) ||
              ...
              continue;
            if (!mps[site].site_dim().has(phys_in)) continue;
            charge in_r_charge = SymmGroup::fuse(rc, phys_in);
            if (!right_i.has(in_r_charge)) continue;
            charge out_r_charge = SymmGroup::fuse(out_l_charge, phys_out);
            if (!allowed_sectors[site + 1].has(out_r_charge)) continue;
    ...
    for (int iLeftCharge = 0; iLeftCharge < finalLeft.size(); iLeftCharge++)
      thresholdLeft[finalLeft[iLeftCharge].first] =
          std::vector<int>(mpo[site].row_dim(), 0);
    for (int iRightCharge = 0; iRightCharge < finalRight.size(); iRightCharge++)
      for (int iPhysCharge = 0; iPhysCharge < finalPhys.size(); iPhysCharge++)
        thresholdRight[std::make_pair(
            finalPhys[iPhysCharge].first, finalRight[iRightCharge].first
        )] = std::vector<int>(mpo[site].col_dim(), 0);
      ...
      for (auto& iCharge : thresholdLeft) {
        ...
        if (iRow > 0)
          if (mapTrackingBlocks[iRow - 1].has(iCharge.first))
            iCharge.second[iRow] +=
                mapTrackingBlocks[iRow - 1].size_of_block(iCharge.first);
      }
    }
    ...
    for (int iCol = 0; iCol < mpo[site].col_dim(); iCol++) {
      for (auto& iCharge : thresholdRight) {
        ...
        if (iCol > 0 && finalPhys.has(iCharge.first.first) &&
            new_right_i_map[iCol - 1].has(iCharge.first.second))
          iCharge.second[iCol] +=
              /*finalPhys.size_of_block(iCharge.first.first) *  */
              new_right_i_map[iCol - 1].size_of_block(iCharge.first.second);
      }
    }

    // Load the data inside the finalMPS MPSTensor
    ProductBasis<SymmGroup> out_right_pb(
        finalPhys, finalRight,
        boost::lambda::bind(
            static_cast<charge (*)(charge, charge)>(SymmGroup::fuse),
            -boost::lambda::_1, boost::lambda::_2
        )
    ...
    for (int iCol = 0; iCol < mpo[site].col_dim(); iCol++) {
      ...
      for (typename col_proxy::const_iterator col_it = col_b2.begin();
           col_it != col_b2.end(); ++col_it)
      ...
      for (int iRow = 0; iRow < mpo[site].row_dim(); iRow++) {
        ...
        if (std::find(availableRows.begin(), availableRows.end(), iRow) !=
            availableRows.end()) {
          term_descriptor<Matrix, SymmGroup, true> access =
              mpo[site].at(iRow, iCol);
            ...
            for (size_t b = 0; b < data.n_blocks(); ++b) {
              ...
              for (size_t w_block = 0; w_block < W.basis().size(); ++w_block) {
                ...
                for(size_t rr = 0; rr < r_size; ++rr) {
                    maquis::dmrg::detail::iterator_axpy(&iblock(0,
                in_right_offset + rr), &iblock(0, in_right_offset + rr) +
                l_size, &oblock(thresholdLeft[out_l_charge][iRow],
                thresholdRight[std::make_pair(phys_out, out_r_charge)][iCol] +
                out_right_offset + rr), alfa);
                }
                */
                for (int iRowPhys = 0; iRowPhys < W.basis().left_size(w_block);
                     iRowPhys++) {
                  for (int iColPhys = 0;
                       iColPhys < W.basis().right_size(w_block); iColPhys++) {
                    value_type alfa =
                        access.scale(oi) * W[w_block](iRowPhys, iColPhys);
                    auto thresholdRightElement =
                        thresholdRight[std::make_pair(phys_out, out_r_charge)]
                                      [iCol];
                    for (int rr = 0; rr < r_size; ++rr) {
                      maquis::dmrg::detail::iterator_axpy(
                          &iblock(0, in_right_offset + iRowPhys * r_size + rr),
                          &iblock(0, in_right_offset + iRowPhys * r_size + rr) +
                              l_size,
                          &oblock(
                              ...
                              out_right_offset  // Offset given by the product
                                  ...
                                  + thresholdRightElement  // Threshold induced
                                                           // by b
                                  + rr
                          ),  // Different m values for the columns of the
                              // tensor
                          alfa
                      );
                    }
   ...
   *
   * This version of [mpo_times_mps] works only for elementary operators - in
   * other words, for MPOs with bond dimension b=1.
   *
   ...
   */
  static MPSTensor<Matrix, SymmGroup> mpo_times_mps_singleop(
    ...
    charge W_delta =
        SymmGroup::fuse(W.basis().right_charge(0), -W.basis().left_charge(0));
    ...
    for (size_t b = 0; b < data.n_blocks(); ++b) {
      ...
      for (size_t w_block = 0; w_block < W.basis().size(); ++w_block) {
        ...
        if (!right_i.has(in_r_charge))
          continue;  // do we have phys_in in block b?

        charge out_r_charge =
            SymmGroup::fuse(out_l_charge, phys_out);  // unpaired

        if (!new_left_i.has(out_l_charge))
          new_left_i.insert(
              std::make_pair(out_l_charge, data.basis().left_size(b))
          );
        if (!new_right_i.has(out_r_charge))
          new_right_i.insert(
              std::make_pair(out_r_charge, right_i.size_of_block(in_r_charge))
          );
      }
    }
    ...
    for (size_t b = 0; b < data.n_blocks(); ++b) {
      ...
      for (size_t w_block = 0; w_block < W.basis().size(); ++w_block) {
        ...
        size_t o = prod.find_block(
            out_l_charge, out_l_charge
        );  // out_l_charge = out_r_charge right-paired
        if (o == prod.n_blocks())
          o = prod.insert_block(
              Matrix(l_size, out_right_pb.size(-phys_out, out_r_charge)),
              out_l_charge, out_l_charge
          );
        ...
        // maquis::cout << " W[w_block](0,0) ... " << W[w_block](0,0) << "for
        // block " << w_block << std::endl; maquis::cout << " alfa ... " <<
        // access.scale() << std::endl;
        for (size_t rr = 0; rr < r_size; ++rr)
          maquis::dmrg::detail::iterator_axpy(
              &iblock(0, in_right_offset + rr),
              &iblock(0, in_right_offset + rr) + l_size,
              &oblock(0, out_right_offset + rr), alfa
          );
      }
    }
    std::swap(in_delta, out_delta);
 ...
 private:
  ...
  std::vector<ChargeType>
      charges;  // Charges of the MPS that is currently constructed
...
class MPOTimesMPSTraitClass<
    Matrix, SymmGroup, symm_traits::enable_if_su2_t<SymmGroup>> {
 public:
  // Types definition
  using MPSType = MPS<Matrix, SymmGroup>;
  using MPOType = MPO<Matrix, SymmGroup>;
  ...
  MPOTimesMPSTraitClass(
    ...
    // throw std::runtime_error("MPOTimesMPSTraitClass not available for
    // spin-adapted Hamiltonians");
  }
  ...
  MPSTensor<Matrix, SymmGroup> mpo_times_mps(
    ...
    throw MPOTimesMPSException();
    // throw std::runtime_error("[mpo_times_mps] not yet implemented for SU2U1
    // symmetry");
  }
  ...
  static MPSTensor<Matrix, SymmGroup> mpo_times_mps_singleop(
    ...
    throw MPOTimesMPSException();
    // throw std::runtime_error("[mpo_times_mps_singleop] not yet implemented
    // for SU2U1 symmetry");
  }

  /** @brief AppplyOp method */
  MPSType applyMPO(const MPOType& mpo) { throw MPOTimesMPSException(); }
};
