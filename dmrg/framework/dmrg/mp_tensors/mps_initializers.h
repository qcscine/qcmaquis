...
// == DEFAULT_MPS_INIT ==
// Note that this method supports both random and constant initialization of the
// MPS
...
struct default_mps_init : public mps_initializer<Matrix, SymmGroup> {
 public:
   ...
   */
  default_mps_init(
      BaseParameters& parms, std::vector<Index<SymmGroup> > const& phys_dims_,
   ...
   * @brief Functor operator called to generate the MPS
   *
   * Note that fillrand variable is set here to true - so random initialization
   * is done by default.
   *
   * @param mps output MPS
   */
  ...
  // Main routine
  void init_sectors(
      MPS<Matrix, SymmGroup>& mps, size_t Mmax, bool fillrand = true,
    ...
    for (int i = 0; i < L; i++) {
      ...
      mps[i] = MPSTensor<Matrix, SymmGroup>(
          phys_dims[site_type[i]], allowed[i], allowed[i + 1], fillrand, val
      );
...
struct const_mps_init : public mps_initializer<Matrix, SymmGroup> {
 public:
  ...
  const_mps_init(
      BaseParameters& parms, std::vector<Index<SymmGroup> > const& phys_dims,
...
class basis_mps_init_generic : public mps_initializer<Matrix, SymmGroup> {
 public:
   ...
   * @param site_type_ Vector with size == the lattice size, with the type of
   * each site.
   */
  basis_mps_init_generic(
      BaseParameters& params_, const std::vector<Index<SymmGroup> >& phys_dims_,
      typename SymmGroup::charge right_end_, std::vector<int> const& site_type_
  )
      : sym(params_["symmetry"].as<std::string>()),
        phys_dims(phys_dims_),
        right_end(right_end_),
        site_type(site_type_),
        params(params_) {
    ...
    if (params["init_state_type"] == "csf" &&
        (sym == "su2u1" || sym == "su2u1pg")) {
      char jchar;
      while (ss.get(jchar)) {
        if (jchar == '2')
          ichar = 4;  // doubly occ to explicitly construct csf
        else if (jchar == 'u')
          ichar = 6;  // up to explicitly construct csf
        else if (jchar == 'd')
          ichar = 7;  // down to explicitly construct csf
        else if (jchar == '0')
          ichar = 1;  // not occ to explicitly construct csf
        else
          throw std::runtime_error(
              "The specificed state contains symbols which are not recognized. "
              "Abort."
          );
  ...
  void operator()(MPS<Matrix, SymmGroup>& mps) {
    ...
    auto state =
        HelperClassBasisVectorConverter<SymmGroup>::GenerateIndexFromString(
            params, basis_index, phys_dims, site_type, mps.length()
        );
    if (sym == "su2u1" ||
        sym == "su2u1pg") {  // SU2 electronic case --> special because of spin
                             // symmetries etc --> directly use state_mps_cd
      mps = state_mps_cd<Matrix>(
          state, phys_dims, site_type, right_end, 1, false
      );
    } else {
 ...
 public:
  ...
  basis_mps_init_generic_const(
    ...
    if (params["init_space"].str().empty())
      throw std::runtime_error(
          "Init_space needs to be provided to populate "
          "basis_state_generic_const. Abort."
      );
  ...
  void operator()(MPS<Matrix, SymmGroup>& mps) {
    ...
    if (sym == "2u1" || sym == "2u1pg" || sym == "su2u1" ||
        sym == "su2u1pg") {  // electronic case --> special because of spin
                             // symmetries etc --> directly use state_mps
      mps = state_mps_cd<Matrix>(
          state, phys_dims, site_type, right_end, init_bond_dimension, false
      );
    } else {
      mps = state_mps_const<Matrix>(
          state, phys_dims, site_type, right_end, false, init_bond_dimension
      );
    }
    if (mps[mps.length() - 1].col_dim()[0].first != right_end)
      throw std::runtime_error(
 ...
 */
template <class Matrix, class SymmGroup>
class basis_mps_init_generic_default
    : public mps_initializer<Matrix, SymmGroup> {
 public:
  // -- Constructors --
  basis_mps_init_generic_default(
      BaseParameters& params_, std::vector<Index<SymmGroup> > const& phys_dims_,
      typename SymmGroup::charge right_end_, std::vector<int> const& site_type_
  )
      : sym(params_["symmetry"].as<std::string>()),
        init_bond_dimension(params_["init_bond_dimension"]),
    ...
    if (params["init_space"].str().empty())
      throw std::runtime_error(
          "Init_space needs to be provided to populate "
          "basis_state_generic_default. Abort."
      );
    basis_index = params["init_space"].template as<std::vector<int> >();
    if (params.is_set("seed")) dmrg_random::engine.seed(params["seed"]);
  }
  // Operator called when initialization occurs
  void operator()(MPS<Matrix, SymmGroup>& mps) {
    auto state =
    ...
    if (sym == "2u1" || sym == "2u1pg" || sym == "su2u1" ||
        sym == "su2u1pg") {  // electronic case --> special because of spin
                             // symmetries etc --> directly use state_mps
      mps = state_mps_cd<Matrix>(
          state, phys_dims, site_type, right_end, init_bond_dimension, true
      );
    } else {
      mps = state_mps_const<Matrix>(
          state, phys_dims, site_type, right_end, true, init_bond_dimension
      );
    }
    if (mps[mps.length() - 1].col_dim()[0].first != right_end)
      throw std::runtime_error(
...
class coherent_mps_init : public mps_initializer<Matrix, SymmGroup> {
 public:
  /** @brief Class constructor
   *
   * Note that generally determinants are utilized except for SU2, where CSF are
   * constructed.
   *
   */
  coherent_mps_init(
      ...
      : phys_dims(phys_dims_),
        ...
        right_end(right_end_),
        params(params_) {
    if (params["init_file"].str().empty() &&
        params["init_basis_state"].str().empty())
      throw std::runtime_error(
          "Either init_file or init_basis_state has to be provided for the "
          "coherent initializer"
      );

    initialBondDim = (params["init_bond_dimension"] > 5)
                         ? params["init_bond_dimension"]
                         : params["max_bond_dimension"];
    ...
    if (fromFile) {
      ...
      fileName = specifiedFiles[0];  // This is a safeguard for the interface
                                     // sim initialization in case several
                                     // filenames are provided
      if (!boost::filesystem::exists(fileName))
        throw std::runtime_error(
            "Initializer file " + fileName + " does not exist\n"
        );
      ...
      while (std::getline(stateFile, line)) {
        ...
        boost::split(
            line_splitted, line, boost::is_any_of(" "), boost::token_compress_on
        );
      ...
      boost::split(
          list_dets, params["init_basis_state"].str(), boost::is_any_of("|")
      );
      ...
      if (params["init_state_type"] == "csf") {
        ...
        while (ss.get(jchar)) {
          if (jchar == '2')
            ...
            throw std::runtime_error(
                "The specificed state contains symbols which are not "
                "recognized. Abort."
            );
  ...
  void operator()(MPS<Matrix, SymmGroup>& mps) {
    ...
    for (int i = 0; i < basis_index.size(); i++) {
      auto state =
          HelperClassBasisVectorConverter<SymmGroup>::GenerateIndexFromString(
              params, basis_index[i], phys_dims, site_type, mps.length()
          );
      auto mps_tmp =
          (sym == "su2u1" || sym == "su2u1pg")
              ? state_mps_cd<Matrix>(
                    state, phys_dims, site_type, right_end, initialBondDim,
                    false
                )
