...
struct default_mps_init<Matrix, TwoU1PG> : public mps_initializer<Matrix, TwoU1PG>
{
    ...
    void init_sectors(MPS<Matrix, TwoU1PG> & mps, size_t Mmax, bool fillrand = true, typename Matrix::value_type val = 0)
    {
...
/*
template<class Matrix>
struct default_mps_init<Matrix, TwoU1LPG> : public mps_initializer<Matrix, TwoU1LPG>
{
    typedef TwoU1LPG::subcharge subcharge;

    default_mps_init(BaseParameters & parms,
                     std::vector<Index<TwoU1LPG> > const& phys_dims_,
                     TwoU1LPG::charge right_end_,
                     std::vector<int> const& site_type_)
    : irreps(parse_symm<TwoU1LPG>(site_type_.size(), parms))
    , init_bond_dimension(parms["init_bond_dimension"])
    , phys_dims(phys_dims_)
    , right_end(right_end_)
    , site_type(site_type_)
    ...
    void operator()(MPS<Matrix, TwoU1LPG> & mps)
    {
        init_sectors(mps, this->init_bond_dimension, true);
    }
    
    void init_sectors(MPS<Matrix, TwoU1LPG> & mps, size_t Mmax, bool fillrand = true, typename Matrix::value_type val = 0)
    {
        ...
        std::vector<Index<TwoU1LPG> > allowed = allowed_sectors(site_type, phys_dims, right_end, Mmax, irreps);
        
        omp_for(size_t i, range<size_t>(0,L), {
            select_proc(ambient::scope::balance(i,L));
            mps[i] = MPSTensor<Matrix, TwoU1LPG>(PGDecorator<TwoU1LPG>()(phys_dims[site_type[i]], irreps[i]) , allowed[i], allowed[i+1], fillrand, val);
            mps[i].divide_by_scalar(mps[i].scalar_norm());
            #ifdef AMBIENT_TRACKING
            ambient_track_array(mps, i);
            #endif
    ...
    std::vector<subcharge> irreps;
    std::size_t init_bond_dimension;
    std::vector<Index<TwoU1LPG> > phys_dims;
