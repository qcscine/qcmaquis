...
#define BEGIN_TIMING(name) now = std::chrono::high_resolution_clock::now();
#define END_TIMING(name)                                            \
  then = std::chrono::high_resolution_clock::now();                 \
  maquis::cout << "Time elapsed in " << name << ": "                \
               << std::chrono::duration<double>(then - now).count() \
               << std::endl;

/// TODO: 1) implement two-site time evolution. (single-site is stuck in initial
/// MPS structure)
///       2) implement zip-up compression. E. M. Stoudenmire and S. R. White,
///       New Journal of Physics 12, 055026 (2010).
...
class mpo_contractor_ss {
 ...
 public:
  mpo_contractor_ss(
      MPS<Matrix, SymmGroup> const& mps_, MPO<Matrix, SymmGroup> const& mpo_,
      BaseParameters& parms_
  )
      : mps(mps_), mpsp(mps_), mpo(mpo_), parms(parms_) {
    mps.canonize(0);
    init_left_right(mpo);
  ...
  std::pair<double, double> sweep(int sweep) {
    ...
    for (int _site = 0; _site < 2 * L; ++_site) {
      ...
      SiteProblem<Matrix, SymmGroup> sp(
          mps[site], left_[site], right_[site + 1], mpo[site]
      );
      ...
      if (lr == +1) {
        ...
        left_[site + 1] = contr::overlap_mpo_left_step(
            mpsp[site], mps[site], left_[site], mpo[site]
        );
        norm_boudary = contr::overlap_left_step(
            mpsp[site], MPSTensor<Matrix, SymmGroup>(mpsp[site]), norm_boudary
        );
      } else if (lr == -1) {
        if (site > 0) {
          block_matrix<Matrix, SymmGroup> t;
        ...
        right_[site] = contr::overlap_mpo_right_step(
            mpsp[site], mps[site], right_[site + 1], mpo[site]
        );
        norm_boudary = contr::overlap_right_step(
            mpsp[site], MPSTensor<Matrix, SymmGroup>(mpsp[site]), norm_boudary
        );
      }
      ...
      if (_site == L - 1) {
        ...
        norm_boudary.insert_block(
            Matrix(1, 1, 1), mps[L - 1].col_dim()[0].first,
            mps[L - 1].col_dim()[0].first
        );
      }
    ...
    return eps;  /// note: the actual eps contain a constant, which is not
                 /// important here.
  }

  void finalize() { mpsp[0].normalize_right(DefaultSolver()); }
 ...
 private:
  void init_left_right(MPO<Matrix, SymmGroup> const& mpo) {
    ...
    for (int i = 0; i < L; ++i) {
      ...
      left_[i + 1] =
          contr::overlap_mpo_left_step(mpsp[i], mps[i], left_[i], mpo[i]);
    ...
    for (int i = L - 1; i >= 0; --i) {
      ...
      right_[i] =
          contr::overlap_mpo_right_step(mpsp[i], mps[i], right_[i + 1], mpo[i]);
