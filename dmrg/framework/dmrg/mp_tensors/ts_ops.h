...
std::vector<Integer> allowed_spins(
  ...
  for (typename std::vector<Integer>::iterator it = operator_spins.begin();
       it != operator_spins.end(); ++it) {
...
std::map<
    typename SymmGroup::subcharge, typename OPTable<Matrix, SymmGroup>::op_t>
mpo_couple(
    std::set<Integer> const& summands, Integer b1, Integer b3,
    Index<SymmGroup> const& phys_i1, Index<SymmGroup> const& phys_i2,
    MPOTensor<Matrix, SymmGroup> const& mpo1,
    MPOTensor<Matrix, SymmGroup> const& mpo2
) {
  ...
  for (typename std::set<Integer>::const_iterator it1 = summands.begin();
       it1 != summands.end(); ++it1) {
    auto b2 = *it1;
    ...
    for (int ip1 = 0; ip1 < p1.size(); ++ip1) {
      for (int ip2 = 0; ip2 < p2.size(); ++ip2) {
        std::vector<spin_t> op_spins = allowed_spins(
            mpo1.left_spin(b1).get(), mpo2.right_spin(b3).get(),
        ...
        for (auto it2 = op_spins.begin(); it2 != op_spins.end(); ++it2) {
          SpinDescriptor<typename symm_traits::SymmType<SymmGroup>::type>
              prod_spin(
                  *it2, mpo1.left_spin(b1).get(), mpo2.right_spin(b3).get()
              );
          op_t product;
          op_kron(
              phys_i1, phys_i2, p1.op(ip1), p2.op(ip2), product,
...
MPOTensor<MPSMatrix, SymmGroup> make_twosite_mpo(
    MPOTensor<MPOMatrix, SymmGroup> const& mpo1,
  ...
  using prempo_t =
      std::vector<std::tuple<index_type, index_type, tag_type, value_type> >;
  //
  using MPOTensor_detail::term_descriptor;
  ...
  for (b1 = 0; b1 < mpo1.row_dim(); ++b1) {
    for (b3 = 0; b3 < mpo2.col_dim(); ++b3) {
      ...
      for (typename row_proxy::const_iterator it = row1.begin();
           it != row1.end(); ++it) {
        if (mpo2.has(it.index(), b3)) {
          summands.insert(it.index());
        }
      }
      // Evaluates the sum over b2 and returns the (b1, b3) element of the
      // MPOTensor
      auto coupled_ops = ts_ops_detail::mpo_couple(
          summands, b1, b3, phys_i1, phys_i2, mpo1, mpo2
      );
      for (auto it = coupled_ops.begin(); it != coupled_ops.end(); ++it) {
        tag_type new_tag =
            kron_handler.get_kronecker_table()->register_op(it->second);
        prempo.push_back(std::make_tuple(b1, b3, new_tag, 1.0));
      }
    }
  }
  // Creates the final MPO
  MPOTensor<MPSMatrix, SymmGroup> mpo_big_tag(
      mpo1.row_dim(), mpo2.col_dim(), prempo,
      kron_handler.get_kronecker_table(), mpo1.herm_info * mpo2.herm_info,
      mpo1.row_spin_dim(), mpo2.col_spin_dim()
  );
  // #ifdef MAQUIS_OPENMP
  // #pragma omp critical
  // #endif
  // maquis::cout << "TSMPOTensor: " << mpo1.row_dim() << "x" << mpo2.col_dim()
  // << ",  " << prempo.size()
  //              << " operators, " <<
  //              kron_handler.get_kronecker_table()->size() << " tags\n";
  return mpo_big_tag;
}
 ...
 *
 * Note that the (iSite)-th element of the output MPO is the two-site MPO for
 * sites (iSite, iSite+1)
 *
 ...
 */
template <class MPOMatrix, class MPSMatrix, class SymmGroup>
void make_ts_cache_mpo(
    MPO<MPOMatrix, SymmGroup> const& mpo_orig,
  ...
  omp_for(size_t p, parallel::range<size_t>(0, L_ts), {
    mpo_out[p] = make_twosite_mpo<MPOMatrix, MPSMatrix>(
        mpo_orig[p], mpo_orig[p + 1], mps[p].site_dim(), mps[p + 1].site_dim()
    );
  });
