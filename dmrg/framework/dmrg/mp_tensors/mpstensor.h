...
class MPSTensor {
 public:
  ...
  using magnitude_type = double;  // should become future (todo:
                                  // Matthias, 30.04.12 / scalar-value types)
  using size_type = std::size_t;
  using BlockMatrixType = block_matrix<Matrix, SymmGroup>;
  using BlockMatrixDiagonalType = block_matrix<
      typename alps::numeric::associated_real_diagonal_matrix<Matrix>::type,
      SymmGroup>;

  MPSTensor(
      Index<SymmGroup> const& sd = Index<SymmGroup>(),
      Index<SymmGroup> const& ld = Index<SymmGroup>(),
      Index<SymmGroup> const& rd = Index<SymmGroup>(), bool fillrand = true,
      value_type val = 0
  );
  ...
  MPSTensor(
      ...
      MPSStorageLayout layout = LeftPaired, Indicator = Unorm
  );
  ...
  void replace_right_paired(
      block_matrix<Matrix, SymmGroup> const&, Indicator = Unorm
  );
  void replace_left_paired(
      block_matrix<Matrix, SymmGroup> const&, Indicator = Unorm
  );
  ...
  block_matrix<Matrix, SymmGroup> leftNormalizeAndReturn(
      DecompMethod method = DefaultSolver()
  );
  block_matrix<Matrix, SymmGroup> rightNormalizeAndReturn(
      DecompMethod method = DefaultSolver()
  );
  ...
  /** @brief Scales each block of an MPS by the exponential of the product of
  ...
  friend std::ostream&
  operator<<(std::ostream&, MPSTensor<Matrix_, SymmGroup_> const&);
  ...
  friend void swap(MPSTensor& a, MPSTensor& b) { a.swap_with(b); }

  template <class Matrix_, class SymmGroup_>
  friend MPSTensor<Matrix_, SymmGroup_> join(
      MPSTensor<Matrix_, SymmGroup_> const&,
      MPSTensor<Matrix_, SymmGroup_> const&, boundary_flag_t
  );
...
template <class Matrix, class SymmGroup>
MPSTensor<Matrix, SymmGroup> operator*(
