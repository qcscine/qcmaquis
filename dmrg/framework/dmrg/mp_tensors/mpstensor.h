...
public:
    ...
    // another version of replace_right_paired for construction of the B tensors for linear response
    // only replaces the blocks that are found in RHS, setting all other blocks to zero rather than removing them
    void replace_right_paired_Bmatrix(block_matrix<Matrix, SymmGroup> const &, Indicator =Unorm);
