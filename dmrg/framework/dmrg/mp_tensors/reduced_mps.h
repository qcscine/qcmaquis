...
class reduced_mps {
 ...
 public:
  reduced_mps(const MPS<Matrix, SymmGroup>& mps_)
      : mps(mps_), L(mps.length()), left_(L), right_(L), initialized(false) {}
      ...
      for (int i = 1; i < L; ++i) {
        {
...

          {
            ...
            right_[L - 1 - i] = contraction::Engine<Matrix, Matrix, SymmGroup>::
                overlap_mpo_right_step(
                    mps[L - i], mps[L - i], right_[L - i], ident
                );
          }
          {
            parallel::guard proc(scheduler(L - i - 1));
            storage::migrate(right_[L - 1 - i][0]);
          }
        }
        {
          MPOTensor<Matrix, SymmGroup> ident;
          ident.set(0, 0, identity_matrix<op_t>(mps[i - 1].site_dim()));
          {
            ...
            left_[i] = contraction::Engine<Matrix, Matrix, SymmGroup>::
                overlap_mpo_left_step(
                    mps[i - 1], mps[i - 1], left_[i - 1], ident
                );
          }
          {
            parallel::guard proc(scheduler(i));
            storage::migrate(left_[i][0]);
          }
        }
      }
      initialized = true;
