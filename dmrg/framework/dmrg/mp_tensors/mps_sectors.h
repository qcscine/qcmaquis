...
inline std::vector<Index<SymmGroup> > allowed_sectors(
    std::vector<int> const& site_type,
    std::vector<Index<SymmGroup> > const& phys_dims,
    typename SymmGroup::charge right_end, std::size_t Mmax
) {
  bool finitegroup = SymmGroup::finite;

  std::size_t L = site_type.size();

  std::vector<typename SymmGroup::charge> maximum_charges(phys_dims.size());
  std::vector<typename SymmGroup::charge> minimum_charges(phys_dims.size());
  for (int type = 0; type < phys_dims.size(); ++type) {
    ...
    if (minimum_charges[type] > maximum_charges[type]) {
      std::swap(maximum_charges[type], minimum_charges[type]);
    }
  }

  typename SymmGroup::charge maximum_total_charge = SymmGroup::IdentityCharge;
  typename SymmGroup::charge minimum_total_charge = SymmGroup::IdentityCharge;
  for (int i = 0; i < L; ++i) {
    maximum_total_charge =
        SymmGroup::fuse(maximum_total_charge, maximum_charges[site_type[i]]);
    minimum_total_charge =
        SymmGroup::fuse(minimum_total_charge, minimum_charges[site_type[i]]);
  }

  Index<SymmGroup> l_triv;
  Index<SymmGroup> r_triv;
  l_triv.insert(std::make_pair(SymmGroup::IdentityCharge, 1));
  r_triv.insert(std::make_pair(right_end, 1));

  std::vector<Index<SymmGroup> > left_allowed(L + 1);
  std::vector<Index<SymmGroup> > right_allowed(L + 1);
  std::vector<Index<SymmGroup> > allowed(L + 1);
  left_allowed[0] = l_triv;
  right_allowed[L] = r_triv;

  typename SymmGroup::charge cmaxi = maximum_total_charge;
  typename SymmGroup::charge cmini = minimum_total_charge;
  for (int i = 1; i < L + 1; ++i) {
    ...
    while (it != left_allowed[i].end()) {
      if (!finitegroup &&
          (SymmGroup::fuse(it->first, cmaxi) < right_end ||
           SymmGroup::fuse(it->first, cmini) > right_end ||
           (!ChargeDetailClass<SymmGroup>::physical(it->first) &&
            ChargeDetailClass<SymmGroup>::hasLessParticleThan(
                it->first, right_end
            )))) {
  ...
  cmaxi = maximum_total_charge;
  cmini = minimum_total_charge;
  for (int i = L - 1; i >= 0; --i) {
  ...
  for (int i = 0; i < L + 1; ++i) {
    ...
    for (typename Index<SymmGroup>::iterator it = allowed[i].begin();
         it != allowed[i].end(); ++it) {
      it->second = tri_min(
          Mmax, left_allowed[i].size_of_block(it->first),
