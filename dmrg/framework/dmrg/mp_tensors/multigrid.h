...
struct multigrid {
  ...
  void static normalize_restriction_isometry(block_matrix<Matrix, SymmGroup>& T
  ) {
  ...
  template <class Matrix, class SymmGroup>
  block_matrix<Matrix, SymmGroup> static default_restriction_isometry(
      Index<SymmGroup> const& phys_large, Index<SymmGroup> const& phys_small
  ) {
    ...
    for (size_t s1 = 0; s1 < phys_large.size(); ++s1)
      for (size_t s2 = 0; s2 < phys_large.size(); ++s2) {
        ...
        if (s == phys_small.size()) continue;

        if (!T.has_block(s_charge, s_charge))
          T.insert_block(
              new Matrix(
                  phys_small[s].second, pb.size(s1_charge, s2_charge), 0.
              ),
              s_charge, s_charge
          );
        ...
        for (size_t ss1 = 0; ss1 < phys_large[s1].second; ++ss1)
          for (size_t ss2 = 0; ss2 < phys_large[s2].second; ++ss2)
            oblock(0, out_offset + ss1 * phys_large[s2].second + ss2) =
                typename Matrix::value_type(1.);
      }
  ...
  template <class Matrix, class SymmGroup>
  MPSTensor<Matrix, SymmGroup> static restriction(
      MPSTensor<Matrix, SymmGroup> const& M1,
    ...
    ProductBasis<SymmGroup> in_right(
        phys_large, right_i,
    ...
    for (size_t Mblock = 0; Mblock < MM.n_blocks(); ++Mblock)
      for (size_t Tb = 0; Tb < T.n_blocks(); ++Tb)
        for (size_t s1 = 0; s1 < phys_large.size(); ++s1) {
          size_t s2 = phys_large.position(
              SymmGroup::fuse(T.basis().right_charge(Tb), -phys_large[s1].first)
          );
          if (s2 == phys_large.size()) continue;
          size_t l = left_i.position(SymmGroup::fuse(
              MM.basis().left_charge(Mblock), -phys_large[s1].first
          ));
          if (l == left_i.size()) continue;
          size_t r = right_i.position(SymmGroup::fuse(
              MM.basis().right_charge(Mblock), phys_large[s2].first
          ));
          ...
          {
            charge in_l_charge =
                SymmGroup::fuse(phys_large[s1].first, left_i[l].first);
            charge in_r_charge =
                SymmGroup::fuse(-phys_large[s2].first, right_i[r].first);

            charge T_l_charge = T.basis().left_charge(Tb);
            charge T_r_charge =
                SymmGroup::fuse(phys_large[s1].first, phys_large[s2].first);

            charge out_l_charge =
                SymmGroup::fuse(phys_small[s].first, left_i[l].first);
            charge out_r_charge = right_i[r].first;

            if (!MM.has_block(in_l_charge, in_r_charge)) continue;
            if (!T.has_block(T_l_charge, T_r_charge)) continue;

            if (!data.has_block(out_l_charge, out_r_charge))
              data.insert_block(
                  new Matrix(
                      out_left.size(phys_small[s].first, left_i[l].first),
                      right_i[r].second, 0
                  ),
            ...
            size_t in_r_offset =
                in_right(phys_large[s2].first, right_i[r].first);
            size_t out_l_offset =
                out_left(phys_small[s].first, left_i[l].first);
            size_t T_r_offset =
                phys_pb(phys_large[s1].first, phys_large[s2].first);
            ...
            for (size_t ss = 0; ss < phys_small[s].second; ++ss) {
              for (size_t ss1 = 0; ss1 < phys_large[s1].second; ++ss1)
                for (size_t ss2 = 0; ss2 < phys_large[s2].second; ++ss2) {
                  typename Matrix::value_type const& Tval = T_block(
                      ss, T_r_offset + ss1 * phys_large[s2].second + ss2
                  );
                  for (size_t rr = 0; rr < right_i[r].second; ++rr)
                    /* blas implementation */
                    maquis::dmrg::detail::iterator_axpy(
                        &in_block(
                            in_l_offset + ss1 * left_i[l].second,
                            in_r_offset + ss2 * right_i[r].second + rr
                        ),
                        &in_block(
                            in_l_offset + ss1 * left_i[l].second,
                            in_r_offset + ss2 * right_i[r].second + rr
                  ...
                  /* loop implementation:
                   ...
                   += in_block(in_l_offset + ss1*left_i[l].second+ll,
                   in_r_offset + ss2*right_i[r].second+rr) * Tval;
                   */
                }
            }
          }
    ...
    MPSTensor<Matrix, SymmGroup> M(
        phys_small, left_i, right_i, data, LeftPaired
    );
    return M;
  }

  template <class Matrix, class SymmGroup>
  MPS<Matrix, SymmGroup> static restrict_mps(
      MPS<Matrix, SymmGroup> const& mps_large,
      boost::optional<block_matrix<Matrix, SymmGroup> const&> T_ =
          (boost::optional<block_matrix<Matrix, SymmGroup> const&>())
  ) {
    ...
    if (!T_)
      ...
      T = default_restriction_isometry<Matrix>(
          mps_large.site_dim(0), mps_large.site_dim(0)
      );
    else
      T = T_.get();
  ...
  template <class Matrix, class SymmGroup>
  MPO<Matrix, SymmGroup> static restrict_mpo(
    ...
    if (!T_) {
      ...
      for (size_t b1 = 0; b1 < mpo_large[0].col_dim() && phys_i.size() == 0;
           ++b1)
        for (size_t b2 = 0; b2 < mpo_large[0].row_dim() && phys_i.size() == 0;
             ++b2)
    ...
    for (std::size_t p = 0; p < L; ++p) {
      Index<SymmGroup> phys_i;
      for (size_t b1 = 0; b1 < mpo_large[2 * p].col_dim() && phys_i.size() == 0;
           ++b1)
        for (size_t b2 = 0;
             b2 < mpo_large[2 * p].row_dim() && phys_i.size() == 0; ++b2)
          if (mpo_large[2 * p].has(b1, b2))
            phys_i = mpo_large[2 * p](b1, b2).left_basis();
      MPOTensor<Matrix, SymmGroup> mpot =
          make_twosite_mpo(mpo_large[2 * p], mpo_large[2 * p + 1], phys_i);
      for (size_t b1 = 0; b1 < mpot.col_dim(); ++b1)
        for (size_t b2 = 0; b2 < mpot.row_dim(); ++b2) {
          if (!mpot.has(b1, b2)) continue;
  ...
  template <class Matrix, class SymmGroup>
  void static restriction_old(
      MPS<Matrix, SymmGroup> const& mps_large, MPS<Matrix, SymmGroup>& mps_small
  ) {
    ...
    for (std::size_t p = 0; p < L; ++p) {
      ...
      //            maquis::cout << "alpha_basis:" <<std::endl << alpha_basis <<
      //            std::endl; maquis::cout << "beta_basis:" <<std::endl <<
      //            beta_basis << std::endl; maquis::cout << "s_basis:"
      //            <<std::endl << s_basis << std::endl; maquis::cout <<
      //            "s1_basis:" <<std::endl << s1_basis << std::endl;
      //            maquis::cout << "s2_basis:" <<std::endl << s2_basis <<
      //            std::endl;
      ...
      ProductBasis<SymmGroup> in2_right(
          s2_basis, beta_basis,
          ...
          for (bi_t b = b_basis.basis_begin(); !b.end(); ++b)
            for (bi_t s1 = s1_basis.basis_begin(); !s1.end(); ++s1)
              for (bi_t s2 = s2_basis.basis_begin(); !s2.end(); ++s2) {
                std::pair<charge, std::size_t> s(
                    SymmGroup::fuse(s1->first, s2->first),
                    s1->second * s2_basis.size_of_block(s2->first) + s2->second
                );

                if (!s_basis.has(s.first) ||
                    s_basis.size_of_block(s.first) <= s.second)
                  continue;
                ...
                if (!mps_large[2 * p].data().has_block(in1_left_c, in1_right_c))
                  continue;
                if (!mps_large[2 * p + 1].data().has_block(
                        in2_left_c, in2_right_c
                    ))
                  continue;
                ...
                //                            maquis::cout << "alpha: " <<
                //                            alpha->first << ", " <<
                //                            alpha->second << std::endl;
                //                            maquis::cout << "beta: " <<
                //                            beta->first << ", " <<
                //                            beta->second << std::endl;
                //                            maquis::cout << "s1: " <<
                //                            s1->first << ", " << s1->second <<
                //                            std::endl; maquis::cout << "s2: "
                //                            << s2->first << ", " << s2->second
                //                            << std::endl; maquis::cout << "s:
                //                            " << s.first << ", " << s.second
                //                            << std::endl;
                ...
                if (!M.has_block(out_left_c, out_right_c))
                  M.insert_block(
                      new Matrix(
                          out_left.size(s.first, alpha->first),
                          beta_basis.size_of_block(beta->first), 0
                      ),
                      out_left_c, out_right_c
                  );

                //                            maquis::cout << "block has size "
                //                            << out_left.size(s1->first,
                //                            alpha->first)
                //                            << "x"
                //                            << out_right.size(s2->first,
                //                            beta->first,
                //                                              boost::lambda::bind(static_cast<charge(*)(charge,
                //                                              charge)>(SymmGroup::fuse),
                ...
                std::size_t in2_right_offset =
                    in2_right(s2->first, beta->first);

                std::size_t out_left_offset = out_left(s.first, alpha->first);
                std::size_t out_right_offset = 0;

                //                            maquis::cout << "M[" << out_left_c
                //                            << ", " << out_right_c << "]"
                //                            << "(" << out_left_offset +
                //                            s1->second*alpha_basis.size_of_block(alpha->first)+alpha->second
                //                            << ", " << out_right_offset +
                //                            s2->second*beta_basis.size_of_block(beta->first)+beta->second
                //                            << ")" << std::endl; maquis::cout
                //                            << " = " <<
                //                            M(std::make_pair(out_left_c,
                //                            out_left_offset +
                //                            s1->second*alpha_basis.size_of_block(alpha->first)+alpha->second),
                //                                                    std::make_pair(out_right_c,
                //                                                    out_right_offset
                //                                                    +
                //                                                    s2->second*beta_basis.size_of_block(beta->first)+beta->second))
                //                                                    <<
                ...
                //
                //                            maquis::cout << "mps[" <<
                //                            in_left_c << ", " << in_right_c <<
                //                            "]"
                //                            << "(" << in_left_offset +
                //                            s.second*alpha_basis.size_of_block(alpha->first)
                //                            + alpha->second << ", " <<
                //                            in_right_offset + beta->second <<
                //                            ")" << std::endl; maquis::cout <<
                //                            " = " <<
                //                            mps_small[p].data()(std::make_pair(in_left_c,
                //                            in_left_offset +
                //                            s.second*alpha_basis.size_of_block(alpha->first)
                //                            + alpha->second),
      ...
      mps_small[p] = MPSTensor<Matrix, SymmGroup>(
          s_basis, alpha_basis, beta_basis, M, LeftPaired
      );
    }
  ...
  template <class Matrix, class SymmGroup>
  void static site_extension(
      MPSTensor<Matrix, SymmGroup> const& mps_small,
      MPSTensor<Matrix, SymmGroup>& M1, MPSTensor<Matrix, SymmGroup>& M2
  ) {
    using charge = typename SymmGroup::charge;
    using bi_t = typename Index<SymmGroup>::basis_iterator;
    ...
    ProductBasis<SymmGroup> out_right(
        s2_basis, beta_basis,
        [&](const charge& a, const charge& b) { return SymmGroup::fuse(-a, b); }
    );
      ...
      for (bi_t beta = beta_basis.basis_begin(); !beta.end(); ++beta)
        for (bi_t s1 = s1_basis.basis_begin(); !s1.end(); ++s1)
          for (bi_t s2 = s2_basis.basis_begin(); !s2.end(); ++s2) {
            ...
            if (!mps_small.data().has_block(in_left_c, in_right_c)) continue;
            ...
            if (!M.has_block(out_left_c, out_right_c)) {
              M.insert_block(
                  new Matrix(
                      out_left.size(s1->first, alpha->first),
                      out_right.size(
                          s2->first, beta->first,
                          [&](const charge& a, const charge& b) {
                            return SymmGroup::fuse(-a, b);
            ...
            M(std::make_pair(
                  out_left_c,
                  out_left_offset +
                      s1->second * alpha_basis.size_of_block(alpha->first) +
                      alpha->second
              ),
              std::make_pair(
                  out_right_c,
    ...
    block_matrix<
        typename alps::numeric::associated_diagonal_matrix<Matrix>::type,
        SymmGroup>
        S;
    ...
    M1 = MPSTensor<Matrix, SymmGroup>(
        s1_basis, alpha_basis, left.right_basis(), left, LeftPaired
    );
    ...
    M2 = MPSTensor<Matrix, SymmGroup>(
        s2_basis, right.left_basis(), beta_basis, right, RightPaired
    );
  }

  template <class Matrix, class SymmGroup>
  void static extension(
      MPS<Matrix, SymmGroup> const& mps_small, MPS<Matrix, SymmGroup>& mps_large
  ) {
    ...
    for (std::size_t p = 0; p < L1; ++p) {
      ...
      // Normalization
      if (p != 0) mps_large[2 * p].multiply_from_left(t_norm);
  ...
  template <class Matrix, class SymmGroup>
  static results_collector extension_optim(
      BaseParameters& parms, MPS<Matrix, SymmGroup> mps_small,
    ...
    if (sweep >= parms["ngrowsweeps"])
      ...
      cutoff = log_interpolate(
          parms["truncation_initial"], parms["truncation_main"],
          parms["ngrowsweeps"], sweep
      );

    std::size_t Mmax;
    if (parms.is_set("sweep_bond_dimensions")) {
      std::vector<std::size_t> ssizes =
          parms.get<std::vector<std::size_t> >("sweep_bond_dimensions");
    ...
    if (mpos_mix.size() > 0) {
      ...
      for (int i = L - 1; i >= 0; --i) {
        right_[i] = contraction::Engine<Matrix, Matrix, SymmGroup>::
            overlap_mpo_right_step(
                mps_small[i], mps_small[i], right_[i + 1], mpos_mix[0][i]
            );
      }
    }
    ...
    for (std::size_t p = 0; p < L; ++p) {
      ...
      // Normalizing the MPS
      mps_large[2 * p + 1].divide_by_scalar(mps_large[2 * p + 1].scalar_norm()
      );  // that may playout
      assert(mps_large[2 * p].num_check());
      assert(mps_large[2 * p + 1].num_check());
      ...
      if (mpos_mix.size() == 0) {
        ...
        if (p < L - 1) {
          ...
          right_mixed = contraction::Engine<Matrix, Matrix, SymmGroup>::
              overlap_mpo_right_step(
                  mps_small[p + 1], mps_small[p + 1], right_mixed,
                  mpos_mix[p + 1][2 * (p + 1)]
              );
        }
        ...
        if (false) {
          ...
          std::copy(
              mps_large.const_begin(), mps_large.const_begin() + 2 * (p + 1),
              mps_mixed.begin()
          );
          std::copy(
              mps_small.const_begin() + (p + 1), mps_small.const_end(),
              mps_mixed.begin() + 2 * (p + 1)
          );

          {
            maquis::cout << "Norm " << norm(mps_mixed) << std::endl;
            maquis::cout << "Energy "
                         << "finegraining_fullmpomix "
                         << maquis::real(expval(mps_mixed, mpos_mix[p + 1]))
                         << std::endl;
          }
        }
         ...
         */
        {
          ...
          right = contraction::Engine<Matrix, Matrix, SymmGroup>::
              overlap_mpo_right_step(
                  mps_large[2 * p + 1], mps_large[2 * p + 1], right,
                  mpos_mix[p + 1][2 * p + 1]
              );
          if (p == 0) left_[0] = mps_large.left_boundary();

          SiteProblem<Matrix, SymmGroup> sp(
              left_[2 * p], right, mpos_mix[p + 1][2 * p]
          );
          ...
          if (parms["finegrain_optim"]) {
            ...
            maquis::cout << "Energy "
                         << "finegraining_1 " << res.first << std::endl;
            graining_results["Energy"] << res.first;

          } else if (true) {
            // Compute Energy
            MPSTensor<Matrix, SymmGroup> vec2 =
                contraction::Engine<Matrix, Matrix, SymmGroup>::site_hamil2(
                    mps_large[2 * p], sp.left, sp.right, sp.mpo
                );
            double energy = mps_large[2 * p].scalar_overlap(vec2);
            maquis::cout << "Energy "
                         << "finegraining_00 " << energy << std::endl;
        ...
        {
          right = (p < L - 1) ? right_mixed : right_[p + 1];
          left_[2 * p + 1] = contraction::Engine<Matrix, Matrix, SymmGroup>::
              overlap_mpo_left_step(
                  mps_large[2 * p], mps_large[2 * p], left_[2 * p],
                  mpos_mix[L][2 * p]
              );

          SiteProblem<Matrix, SymmGroup> sp(
              left_[2 * p + 1], right, mpos_mix[p + 1][2 * p + 1]
          );
          ...
          if (parms["finegrain_optim"]) {
            ...
            maquis::cout << "Energy "
                         << "finegraining_2 " << res.first << std::endl;
            graining_results["Energy"] << res.first;
            ...
            MPSTensor<Matrix, SymmGroup> vec2 =
                contraction::Engine<Matrix, Matrix, SymmGroup>::site_hamil2(
                    mps_large[2 * p + 1], sp.left, sp.right, sp.mpo
                );
            double energy = mps_large[2 * p + 1].scalar_overlap(vec2);
            maquis::cout << "Energy "
                         << "finegraining_01 " << energy << std::endl;
            graining_results["Energy"] << energy;
          }
          ...
          /*
           ...
           contraction::Engine<Matrix, Matrix,
           SymmGroup>::predict_new_state_l2r_sweep(mps_large[2*p+1],
           mpos_mix[L][2*p+1], left_[2*p+1], right, alpha, cutoff, Mmax);
           // New tensor for next iteration
           Msmall = contraction::Engine<Matrix, Matrix,
           SymmGroup>::predict_lanczos_l2r_sweep(mps_small[p+1],
           mps_large[2*p+1], new_mps);
           mps_large[2*p+1] = new_mps;
           } else {
           block_matrix<Matrix, SymmGroup> t =
           mps_large[2*p+1].normalize_left(DefaultSolver());
           }
        ...
        if (p < L - 1) {
          left_[2 * p + 2] = contraction::Engine<Matrix, Matrix, SymmGroup>::
              overlap_mpo_left_step(
                  mps_large[2 * p + 1], mps_large[2 * p + 1], left_[2 * p + 1],
                  mpos_mix[L][2 * p + 1]
              );
        }
      }
    ...
    if (mpos_mix.size() > 0) {
      ...
      maquis::cout << "Energy "
                   << "finegraining_final " << energy << std::endl;
    }
#endif
