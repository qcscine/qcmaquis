...
/**
 * @brief Boundary corresponds to the contraction between a bra, ket MPS and
 * MPO.
 *
 */
template <class Matrix, class SymmGroup>
class Boundary
    : public storage::disk::serializable<Boundary<Matrix, SymmGroup> > {
 public:
  ...
  Boundary(
      Index<SymmGroup> const& ud = Index<SymmGroup>(),
      Index<SymmGroup> const& ld = Index<SymmGroup>(), std::size_t ad = 1
  )
  ...
  // value_type & operator()(std::size_t i, access_type j, access_type k) {
  // return data_[i](j, k); } // I hope this is never used (30.04.2012 /
  // scalar/value discussion) value_type const & operator()(std::size_t i,
  // access_type j, access_type k) const { return data_[i](j, k); }
 private:
