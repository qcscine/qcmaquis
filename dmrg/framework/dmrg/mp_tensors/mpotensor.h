...
class MPOTensor {
 ...
 private:
  using CSCMatrix = boost::numeric::ublas::compressed_matrix<
      internal_value_type, boost::numeric::ublas::column_major, 0>;

  using RowIndex = std::vector<std::set<index_type>>;

 public:
  using row_proxy = MPOTensor_detail::row_proxy<
      typename RowIndex::value_type::const_iterator>;
  using col_proxy = boost::numeric::ublas::matrix_column<const CSCMatrix>;

  using prempo_t =
      std::vector<std::tuple<std::size_t, std::size_t, tag_type, value_type>>;
  using spin_desc_t =
 ...
 public:
  MPOTensor(
      index_type = 1, index_type = 1, prempo_t = prempo_t(),
      op_table_ptr = op_table_ptr(),
      MPOTensor_detail::Hermitian = MPOTensor_detail::Hermitian(1, 1),
      spin_index const& lspins = spin_index(),
      spin_index const& rspins = spin_index()
  );
  ...
  void set(
      index_type li, index_type ri, op_t const& op, value_type scale_ = 1.0
  );

  // tagged operator() const
  MPOTensor_detail::term_descriptor<Matrix, SymmGroup, true> at(
  ...
  //          get a modified matrix!
  //          better design needed
  MPOTensor_detail::term_descriptor<Matrix, SymmGroup, false> at(
      index_type left_index, index_type right_index
  );
  ...
  tag_type tag_number(
      index_type left_index, index_type right_index, size_t index = 0
  ) const;
