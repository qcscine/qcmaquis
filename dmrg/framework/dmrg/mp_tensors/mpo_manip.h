...
MPO<Matrix, SymmGroup> block_to_mpo(
    Index<SymmGroup> const& phys_i, block_matrix<Matrix, SymmGroup> block,
    std::size_t length
) {
  ...
  for (size_t p = 0; p < length; ++p) {
    ...
    mpo[p] = MPOTensor<Matrix, SymmGroup>(
        aux_left_i.sum_of_sizes(), aux_right_i.sum_of_sizes()
    );
    for (short run = 0; run < 2; ++run) {
      if (run == 1)
        for (size_t r = 0; r < mpo[p].row_dim(); ++r)
          for (size_t c = 0; c < mpo[p].col_dim(); ++c)
            if (mpo[p].has(r, c)) mpo[p](r, c).allocate_blocks();
      ...
      for (index_product_iterator<SymmGroup> it = midx_mpo.begin();
        ...
        if (!btmp.has_block(lc.first, rc.first)) continue;
        ...
        if (btmp(lc, rc) != 0.) {
          if (run == 0)
            mpo[p](li, ri).reserve(
                (*it)[phys_mpo1].first, (*it)[phys_mpo2].first,
                phys_i.size_of_block((*it)[phys_mpo1].first),
                phys_i.size_of_block((*it)[phys_mpo2].first)
            );
          else
