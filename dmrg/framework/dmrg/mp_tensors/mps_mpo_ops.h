...
typename Matrix::value_type expval(
    MPS<Matrix, SymmGroup> const& bra, MPS<Matrix, SymmGroup> const& ket,
    MPO<Matrix, SymmGroup> const& mpo
);

template <class Matrix, class SymmGroup>
typename Matrix::value_type expvalFromRight(
    MPS<Matrix, SymmGroup> const& bra, MPS<Matrix, SymmGroup> const& ket,
    MPO<Matrix, SymmGroup> const& mpo
);
 ...
 * @param mps Input MPS
 * @param mpo Input MPO
 * @param d If == 0, starts from the first site, otherwise, starts from the last
 * site.
 * @return Matrix::value_type Expectation value < mps | H | mps >
 */
template <class Matrix, class SymmGroup>
auto expval(
    MPS<Matrix, SymmGroup> const& mps, MPO<Matrix, SymmGroup> const& mpo, int d
) {
  return (d == 0) ? expval(mps, mps, mpo) : expvalFromRight(mps, mps, mpo);
}

/**
 * @brief Method to calculate the matrix element of an operator between two
 * different MPSs
 *
...
typename Matrix::value_type expval(
  ...
  Boundary<Matrix, SymmGroup> left =
      mps_mpo_detail::mixed_left_boundary(bra, ket);
  for (int i = 0; i < L; ++i) {
    parallel::guard proc(scheduler(i));
    left =
        contraction::Engine<Matrix, Matrix, SymmGroup>::overlap_mpo_left_step(
            bra[i], ket[i], left, mpo[i], false
        );
  }
 ...
 * different MPSs
 *
 * Unlike [expVal], this method contracts the network starting from the last
 * site.
 *
 * @param bra Input MPS representing the bra
 * @param ket Input MPS representing the ket
...
typename Matrix::value_type expvalFromRight(
  ...
  Boundary<Matrix, SymmGroup> right =
      mps_mpo_detail::mixed_right_boundary(bra, ket);
  for (int i = L - 1; i >= 0; --i) {
    parallel::guard proc(scheduler(i));
    right =
        contraction::Engine<Matrix, Matrix, SymmGroup>::overlap_mpo_right_step(
            bra[i], ket[i], right, mpo[i], false
        );
  }
...
template <class Matrix, class SymmGroup>
typename Matrix::value_type expval(
    MPS<Matrix, SymmGroup> const& mps, MPO<Matrix, SymmGroup> const& mpo
) {
  return expval(mps, mps, mpo);
}

template <class Matrix, class SymmGroup>
std::vector<typename MPS<Matrix, SymmGroup>::scalar_type> multi_expval(
    MPS<Matrix, SymmGroup> const& bra, MPS<Matrix, SymmGroup> const& ket,
    MPO<Matrix, SymmGroup> const& mpo
) {
...
double norm(MPS<Matrix, SymmGroup> const& mps) {
  ...
  left.insert_block(
      Matrix(1, 1, 1), SymmGroup::IdentityCharge, SymmGroup::IdentityCharge
  );

  for (size_t i = 0; i < L; ++i) {
    parallel::guard proc(scheduler(i));
    MPSTensor<Matrix, SymmGroup> cpy = mps[i];
    left = contraction::Engine<Matrix, Matrix, SymmGroup>::overlap_left_step(
        mps[i], cpy, left
    );  // serial
  }
...
template <class Matrix, class SymmGroup>
typename MPS<Matrix, SymmGroup>::scalar_type overlap(
    MPS<Matrix, SymmGroup> const& braMps, MPS<Matrix, SymmGroup> const& ketMps
) {
...
template <class Matrix, class SymmGroup>
std::vector<typename MPS<Matrix, SymmGroup>::scalar_type> multi_overlap(
    MPS<Matrix, SymmGroup> const& braMps, MPS<Matrix, SymmGroup> const& ketMps
) {
  assert(braMps.length() == ketMps.length());
  block_matrix<Matrix, SymmGroup> left;
...
using entanglement_spectrum_type =
    std::vector<std::pair<std::vector<std::string>, std::vector<double>>>;
template <class Matrix, class SymmGroup>
std::vector<double> calculate_bond_renyi_entropies(
    MPS<Matrix, SymmGroup> mps, double n,
...
)  // to be optimized later
{
  ...
  if (spectra != nullptr) spectra->clear();
  ...
  for (std::size_t p = 1; p < L; ++p) {
    ...
    if (spectra != nullptr && measure_es_where != nullptr &&
        std::find(measure_es_where->begin(), measure_es_where->end(), p) !=
            measure_es_where->end()) {
...
template <class Matrix, class SymmGroup>
typename MPS<Matrix, SymmGroup>::scalar_type dm_trace(
    MPS<Matrix, SymmGroup> const& mps, Index<SymmGroup> const& phys_psi
) {
  ...
  for (int s = 0; s < phys_psi.size(); ++s)
    for (int ss = 0; ss < phys_psi[s].second; ++ss) {
      identblock(
          pb(phys_psi[s].first, phys_psi[s].first) + ss * phys_psi[s].second +
              ss,
          0
      ) = 1.;
    }
  ...
  for (int p = 0; p < L; ++p) mps_ident[p] = mident;
...
void fix_density(
    ...
    std::vector<typename operator_selector<Matrix, SymmGroup>::type> const&
        dens_ops,
  ...
  for (int p = 0; p < L; ++p) {
    ...
    block_matrix<Matrix, SymmGroup> rho =
        contraction::density_matrix(mps[p], mps[p]);

    for (size_t j = 0; j < dens.size(); ++j) {
      MPSTensor<Matrix, SymmGroup> tmp =
          contraction::local_op(mps[p], dens_ops[j]);
      double cur_dens = mps[p].scalar_overlap(tmp);
      maquis::cout << "Density[" << j << "] (before) = " << cur_dens
                   << std::endl;
    }

    double a = trace(rho(down, down)) * trace(rho(updown, updown));
    double b = trace(rho(up, up)) * trace(rho(down, down)) +
               dens[0][p] * trace(rho(updown, updown)) -
               dens[1][p] * trace(rho(updown, updown));
    double c = -dens[1][p] * trace(rho(up, up));
    double k2 = (-b + sqrt(b * b - 4 * a * c)) / (2 * a);

    double k1 =
        dens[0][p] / (trace(rho(up, up)) + k2 * trace(rho(updown, updown)));
    ...
    op_t rescale =
        identity_matrix<typename operator_selector<Matrix, SymmGroup>::type>(
            phys
        );
...

    {
      for (size_t j = 0; j < dens.size(); ++j) {
        ...
        maquis::cout << "Density[" << j << "] (after) = " << meas_dens
                     << ", should be " << dens[j][p] << std::endl;
      }
    }

    block_matrix<Matrix, SymmGroup> t_norm =
        mps[p].normalize_left(DefaultSolver());
    if (p < L - 1) mps[p + 1].multiply_from_left(t_norm);
  }
}
