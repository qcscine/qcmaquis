...
SRCAS<ScalarType>::
    ...
    : interface_(interface),
      ...
      parms_(parameters),
      verboseForPlotting_(false) {
  if (parms_["srcas_veryVerbose"] == "yes") verboseForPlotting_ = true;
  generator_.seed(parms_["seed"]);
  // Get the sampling space
  if (parms_["MODEL"] == "nmode") {  // Get the number of modes and the maximum
...
void SRCAS<ScalarType>::printSRCASSettings() {
  ...
  maquis::cout << "SRCAS veryVerbose setting is:               "
               << verboseForPlotting_ << std::endl;
}

template <typename ScalarType>
void SRCAS<ScalarType>::quicksort(
...
std::vector<int> SRCAS<ScalarType>::generateNewDet() {
  ...
  if (parms_["MODEL"] == "nmode" || parms_["MODEL"] == "watson") {
    ...
    for (int i = 0; i < detTmp_.size(); i++) {
      boost::poisson_distribution<> poissonDist(
          detTmp_[i] + 0.5
      ...
      do {
        if (uniformRandomNumber_() <
            parms_["srcas_samplingSpeed"])  // Only accept a fraction of the
                                            // proposed updates to stay closer
                                            // to reference det
  ...
  do {
    ...
    for (int isample = 0; isample < parms_["srcas_numSamples"]; isample++) {
      ...
      if (verboseForPlotting_) {
        maquis::cout << "Macroiteration = " << nMacroIter
                     << ", sample = " << isample
                     << ", completeness = " << sum_ci2 << std::endl;
      }
    }
...
template <typename ScalarType>  // real or complex
double SRCAS<ScalarType>::addToCompleteness(
    ScalarType coeff, std::vector<int> det
) {
  double factor = 1.0;
  if (parms_["symmetry"] == "su2u1" || parms_["symmetry"] == "su2u1pg") {
    int nUnpaired = 0;
    for (int i = 0; i < det.size(); i++) {
      if (det[i] == 3 || det[i] == 2) nUnpaired++;
    }
    if (nUnpaired > 0) factor /= pow(2.0, nUnpaired);
  }
