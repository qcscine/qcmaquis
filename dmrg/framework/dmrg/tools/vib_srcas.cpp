...
VibSRCAS<ScalarType>::
  ...
  if (this->parms_["MODEL"] == "nmode") {
    ...
    if (onvSpace_.size() != nParticles) {
      ...
      onvSpace_ =
          std::vector<int>(nParticles, std::stoi(this->parms_["Nmax"].str()));
    }
  }

  if (this->parms_.is_set("init_basis_state")) {
    this->queen_ =
        VibONV(this->parms_["init_basis_state"].template as<std::vector<int>>()
        );
  } else {
  ...
  if (!this->validONV_(this->queen_.vector())) {
    std::string tmp =
        "Initial basis state <" + this->queen_.string() + "> is not valid";
    throw std::runtime_error(tmp);
  }
}

template <typename ScalarType>
VibONV VibSRCAS<ScalarType>::generateSymmetricDeterminant_(const VibONV& onv
) const {
  ...
  while (tmpVec == this->queen_.vector()) {
    for (int i = 0; i < tmpVec.size(); i++) {
      ...
      boost::variate_generator<boost::mt19937&, boost::poisson_distribution<>>
          poissonRandomNumber(this->generator_, poissonDist);

      while (true) {
        if (this->uniformRandomNumber_() <
            this->parms_["srcas_samplingSpeed"]) {
