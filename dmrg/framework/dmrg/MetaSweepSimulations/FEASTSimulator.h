...
class FEASTSimulator {
  ...
  using LinearSystemSSSimulationType = SweepBasedLinearSystem<
      cmatrix, SymmGroup, StorageType, SweepOptimizationType::SingleSite>;
  using LinearSystemTSSimulationType = SweepBasedLinearSystem<
      cmatrix, SymmGroup, StorageType, SweepOptimizationType::TwoSite>;
  using PointerToSSSimulatorType =
      std::unique_ptr<LinearSystemSSSimulationType>;
  using PointerToTSSimulatorType =
      std::unique_ptr<LinearSystemTSSimulationType>;
 ...
 public:
  ...
  FEASTSimulator(
      BaseParameters& parms, const ModelType& model,
      const LatticeType& inputLattice, const MPOType& mpo
  )
      : parameters(parms),
        currentIter(0),
        isSingleSite(true),
        calculateExactError(false),
    ...
    feastThresholdEnergy =
        parameters["feast_energy_convergence_threshold"].template as<double>();
    feastThresholdOverlap =
        parameters["feast_overlap_convergence_threshold"].template as<double>();
    ...
    truncModality =
        parameters["feast_truncation_type"].template as<std::string>();
    ...
    calculateVariance =
        (parameters["feast_calculate_standard_deviation"] == "yes");
    ...
    if (truncModality != "each" && truncModality != "end")
      throw std::runtime_error(
          "Parameter [feast_truncation_type] not recognized"
      );
    ...
    postProcessor = std::make_unique<PostProcessorType>(
        numStates, numQuadraturePoint, complexWeights, model_, lattice,
  ...
  void runFEAST() {
    ...
    while (!converged) {
      ...
      if (currentIter > 1) {
        ...
        double overlapDifference = std::accumulate(
            overlapVector.begin(), overlapVector.end(), 0.,
            [](int sum, double overlap) {
        ...
        maquis::cout << "  - Overall MPS variation: " << overlapDifference
                     << std::endl;
        maquis::cout << "  - Overall energy variation: " << energyDifference
                     << std::endl;
        bool hasReachedConvergence =
            ((overlapDifference < feastThresholdOverlap) &&
        ...
        if (converged) {
          if (currentIter == maxFeastIter && !hasReachedConvergence) {
            maquis::cout << " --> MAXIMUM NUMBER OF FEAST ITERATION REACHED, "
                            "STOPPING DMRG[FEAST]"
                         << std::endl;
          } else {
          ...
          maquis::cout
              << " --> CONVERGENCE NOT REACHED, WILL START NEW ITERATION"
              << std::endl;
        }
        maquis::cout << std::endl;
      } else {
  ...
  auto getCurrentEigenvalues() { return screenedMPSs; }
  ...
  auto getQuadraturePoints() const { return quadPoints; }

  /** @brief Getter for the FEAST energy */
  auto getEnergy(int iState) const { return energies[iState]; }

 private:
  /** @brief Runs a single iteration of DMRG[FEAST] */
  void runFeastSimulation() {
    // Variable initialization
    // Note that, if this is not the first FEAST iteration, then the feastMPSs
    // vector is defined and we can generate the initial guesses from the
    // results of the previous FEAST iteration.
    if (feastMPSs)
      for (int iState = 0; iState < feastMPSs->size(); iState++)
        mpsGuess[iState] = feastMPSs->operator[](iState);
    // For each FEAST iteration, we have a loop over the number of quadrature
    // points *and* of the number of target states.
    auto initialTime = std::chrono::high_resolution_clock::now();
    //
#pragma omp parallel for collapse(2 \
)  // All independent states are collapsed into one parallelization layer
    for (int quadPoint = 0; quadPoint < numQuadraturePoint; quadPoint++) {
      for (int iGuess = 0; iGuess < numStates; iGuess++) {
        ...
        // Here there is a bit of code repetition because the pointer type is
        // different for SS and TS.
        if (isSingleSite) {
          auto ssSimulator = std::make_unique<LinearSystemSSSimulationType>(
              mpsTmp, mpo_, localParameters, model_, lattice, linSysVerbose_
          );
          ...
          auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(
                              std::chrono::duration<double>(
                                  finalInnerTime - initialInnerTime
                              )
          )
                              .count();
#pragma omp critical(PrintResults)
          {
            if (verbose_) {
              printLinearSystemHeader(
                  complexNodes[quadPoint], complexWeights[quadPoint], iGuess,
                  duration
              );
          ...
          auto tsSimulator = std::make_unique<LinearSystemTSSimulationType>(
              mpsTmp, mpo_, localParameters, model_, lattice, linSysVerbose_
          );
...
#pragma omp critical(UpdateOfResults)
        {
          ...
          resultContainer->insert(
              std::make_pair(std::make_pair(iGuess, quadPoint), mpsTmp)
          );
        }
      }
    }
    ...
    if (printTimings_) {
      double elapsed =
          std::chrono::duration<double>(finalTime - initialTime).count();
      maquis::cout << " --> Time elapsed to solve the FEAST problem " << elapsed
                   << "." << std::endl;
    }
    ...
    feastMPSs =
        postProcessor->performBackTransformation(mpo_, mMax, truncateEach);
    if (screenedMPSs) screenedMPSsPrevious = screenedMPSs;
    ...
    for (const auto& iCurrent : *screenedMPSs) {
      ...
      for (const auto& iPrevious : *screenedMPSsPrevious) {
        auto localOverlap = std::abs(overlap(iCurrent, iPrevious)) /
                            std::sqrt(norm(iCurrent) * norm(iPrevious));
      ...
      maquis::cout << " State " << iMPS
                   << " has the largest overlap with state " << iRef
                   << " of the previous iteration, overlap = " << overlaps[iMPS]
                   << std::endl;
      iMPS += 1;
    }
    maquis::cout << std::endl;
  ...
  void initializeGuess(BaseParameters& parms, const ModelType& model) {
    bool needToWriteStates =
        (initType == "basis_state_generic" || initType == "hf" ||
         initType == "coherent");
    std::vector<std::string> specifiedStates;
    int numSpecifiedStates = 0;
    if (needToWriteStates) {
      std::string states;
      states = (initType == "coherent")
                   ? parms["init_file"].as<std::string>()
                   : parms["init_basis_state"].as<std::string>();
      ...
      if (numSpecifiedStates < 1 || numSpecifiedStates > numStates) {
        throw std::runtime_error(
            "You should specify at least one and at most num_states "
            "init_states's if init_type is set to basis_state_generic or "
            "coherent"
        );
      }
      if (numSpecifiedStates != numStates) {
        maquis::cout << "WARNING! Not all feast states have been provided an "
                        "specified state for initialization, so the remaining "
                        "ones will be initialized with generic_default"
                     << std::endl;
      }
    }
      ...
      if (needToWriteStates) {
        if (iState < numSpecifiedStates) {
          if (initType == "coherent") {
            parametersTmp.set(
                "init_file", specifiedStates[iState]
            );  // initialize the specified states with the initfiles containing
                // the ONVs
          } else {
            parametersTmp.set(
                "init_basis_state", specifiedStates[iState]
            );  // initialize the specified states with the provided ONVs
          }
        } else {  // the rest of the states are not specified
          parametersTmp.set(
              "init_type", "basis_state_generic_default"
          );  // initialize the remaining states with generic_default
        }
      }
      mpsGuess.push_back(
          MPSType(lattice.size(), *(model.initializer(lattice, parametersTmp)))
      );
    }
  }
  ...
  void printHeader() const {
    ...
    maquis::cout << " - Maximum number of FEAST iterations: " << maxFeastIter
                 << std::endl;
    maquis::cout << " - Number of targeted states: " << numStates << std::endl;
    maquis::cout << " - Lower bound for the complex contour integration: "
                 << eMin << std::endl;
    maquis::cout << " - Upper bound for the complex contour integration: "
                 << eMax << std::endl;
    maquis::cout << " - Number of quadrature points: " << numQuadraturePoint
                 << std::endl;
    maquis::cout << " - Energy convergence threshold for FEAST: "
                 << feastThresholdEnergy << std::endl;
    maquis::cout << " - Overlap convergence threshold for FEAST: "
                 << feastThresholdOverlap << std::endl;
    ...
    maquis::cout << " - DMRG solver: "
                 << ((isSingleSite) ? "single site" : "two site") << std::endl;
    if (calculateVariance)
      maquis::cout << " - Calculating variance for each eigenpair."
                   << std::endl;
  }

  /** @brief Prints the header for the solution of a given linear system */
  static void printLinearSystemHeader(
      ComplexType node, ComplexType weight, int iGuess, int time
  ) {
    maquis::cout << std::endl;
    ...
    maquis::cout << " - Time elapsed in linear system solver: " << time
  ...
  double feastThresholdEnergy,
      feastThresholdOverlap;  // Threshold to assess the convergence of
                              // DMRG[FEAST] (both energy and overlap).
  int numQuadraturePoint;     // Number of quadrature point.
  std::string intModality;    // "Full" for the full circle integration, "half"
                              // for the half-circle one.
  std::string
      truncModality;  // "Each" if the MPS must be truncated after each sum,
                      // "end" if the truncation must be done only at the end.
  std::string initType;  // Initialization strategy for each guess.
  std::vector<MPSType>
      mpsGuess;  // Stores the current guess for hte FEAST procedure.
  std::shared_ptr<std::vector<MPSType>> feastMPSs, screenedMPSs,
      screenedMPSsPrevious;  // Final, back-transformed FEAST MPSs, and their
                             // screened counterpart.
  std::vector<int> seedForInit;  // Seed for random initialization.
  std::vector<typename FeastHelper::QuadraturePoint>
      quadPoints;     // Vector with the quadrature points and weight.
  bool isSingleSite;  // If true, runs a single-site calculation, otherwise runs
                      // a two-sites one.
  bool truncateEach;  // If true, truncates the MPS after each sum.
  bool calculateExactError;  // If true, calculates the exact error associated
                             // with the linear system.
  bool calculateVariance;    // If true, calculates the variance at the end of
                             // each FEAST iteration.
  std::shared_ptr<ResultContainerType>
      resultContainer;  // Member that stores the result of each linear system.
  std::vector<double> energies;  // FEAST energies at the current iteration.
  std::unique_ptr<PostProcessorType>
      postProcessor;  // Class managing FEAST postprocessing.
  std::vector<ComplexType> complexNodes,
      complexWeights;          // Quadrature rule for the complex circle.
...
template <class SymmGroup>
constexpr typename FEASTSimulator<SymmGroup>::ComplexType
    FEASTSimulator<SymmGroup>::imagUnity;
