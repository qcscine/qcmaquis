...
double mem_index(Index<SymmGroup> const& li, Index<SymmGroup> const& ri) {
  ...
  for (int s = 0; s < lefts.size(); s++) {
    ...
    maquis::cout  //<< ((lefts[s] > 10000) ?
        ...
        << "x"
        //<< ((rights[s] > 10000) ?
        //(boost::lexical_cast<std::string>(rights[s]/1000) + std::string("K"))
        //   : boost::lexical_cast<std::string>(rights[s]))
        << rights[s] << "("
        << ((lefts[s] * rights[s] > 10000)
                ? (boost::lexical_cast<std::string>(
                       lefts[s] * rights[s] / 1000
                   ) +
                   std::string("K"))
...
template <class SymmGroup>
void mem_footprint(
    std::vector<Index<SymmGroup> > const& bi, Index<SymmGroup> const& phys
) {
...
struct charge_pair_gt {
  ...
  bool operator()(
      std::pair<charge, charge> const& i, std::pair<charge, charge> const& j
  ) const {
...
struct pair_cmp_inv {
  bool operator()(
      std::pair<std::size_t, std::size_t> const& i,
      std::pair<std::size_t, std::size_t> const& j
  ) const {
 ...
 public:
  ...
  charge_overlap(Index<SymmGroup> const& phys) {
    throw std::runtime_error(
        "Charge overlap only implemented for TwoU1 symmetry"
    );
  }

  std::size_t operator()(charge c1, charge c2) { return 0; }
  std::vector<charge> get_charge_deltas() const {
    return std::vector<charge>();
  }
};
...
class charge_overlap<Matrix, TwoU1> {
  ...
  typedef typename compressor_detail::charge_pair_gt<TwoU1>::charge_cnt_t
      charge_cnt_t;
 ...
 public:
  charge_overlap(Index<TwoU1> const& phys) : phys_i(phys) {
    ...
    Index<TwoU1> two_site_mutations =
        one_site_mutations * phys_i * adjoin(phys_i);
    // TwoU1 only
    index_iterator new_end = std::remove_if(
        two_site_mutations.begin(), two_site_mutations.end(),
      ...
      for (it2 = ts_mut_vec.begin(); it2 != ts_mut_vec.end(); ++it2) {
        ...
        for (size_t ls = 0; ls < phys_i.size(); ++ls)
          for (size_t rs = 0; rs < phys_i.size(); ++rs) {
            charge lc = TwoU1::fuse(
                *it1, TwoU1::fuse(phys_i[ls].first, -phys_i[rs].first)
            );
            if (lc == *it2) outr++;
          }
    ...
    for (it1 = ts_mut_vec.begin(); it1 != ts_mut_vec.end(); ++it1) {
      for (it2 = ts_mut_vec.begin(); it2 != ts_mut_vec.end(); ++it2) {
        if (op_chain_map_hash[std::make_pair(*it1, *it2)] !=
            op_chain_map_hash[std::make_pair(*it2, *it1)])
          exit(1);
 ...
 private:
  Index<TwoU1> phys_i;
  std::map<
      std::pair<charge, charge>, charge_cnt_t,
      compressor_detail::charge_pair_gt<TwoU1> >
      op_chain_map;
  boost::unordered_map<std::pair<charge, charge>, charge_cnt_t>
      op_chain_map_hash;
...
template <class Matrix, class SymmGroup>
class MPOIndexer
    : public std::vector<std::vector<typename SymmGroup::charge> > {
 public:
  typedef std::vector<typename SymmGroup::charge> element_type;
  typedef typename MPO<Matrix, SymmGroup>::elem_type::data_t::const_iterator
      map_it_t;
  ...
  MPOIndexer(MPO<Matrix, SymmGroup> const& mpo_in) {
      ...
      for (map_it_t it = mpo_in[p - 1].data_.lower_bound(std::make_pair(0, 0));
        ...
        do {
          ...
          if (r > 0) {
            ...
            map_it_t next_term =
                mpo_in[tp].data_.lower_bound(std::make_pair(r, 0));
            c = next_term->first.second;
            assert(next_term->first.first == r);
          } else
            c = it->first.second;

          typename SymmGroup::charge charge_diff = SymmGroup::fuse(
              (*this)[tp][r], SymmGroup::fuse(
                                  mpo_in[tp](r, c).left_basis()[0].first,
                                  -mpo_in[tp](r, c).right_basis()[0].first
                              )
          );

          (*this)[tp + 1][c] = charge_diff;
          // while the operator chain starting here(p-1) goes on, follow the
          // terms
        } while (c > 1);
      }
    }
    ...
    for (mpo_index_t p = 0; p <= L; ++p) {
      ...
      for (typename element_type::iterator it = (*this)[p].begin();
           it != (*this)[p].end(); ++it)
        if (index.has(*it))
          index[index.position(*it)] =
              std::make_pair(*it, index.size_of_block(*it) + 1);
        else
  ...
  void update_bond(Index<SymmGroup> const& rhs, mpo_index_t p) {
    ...
    mpo_index_t count = 0;
    (*this)[p].clear();
    for (typename Index<SymmGroup>::basis_iterator it = rhs.basis_begin();
         !it.end(); ++it)
...
class column_iterator {
  ...
  typedef std::map<
      std::pair<mpo_index_t, mpo_index_t>, block_matrix<Matrix, SymmGroup>,
      compressor_detail::pair_cmp_inv>
      map_t;
  typedef std::vector<std::pair<mpo_index_t, typename Matrix::value_type> >
      ret_t;

 public:
  column_iterator(
      MPOTensor<Matrix, SymmGroup> const& mpos, charge lc,
      Index<SymmGroup> const& ii, bond_charge_t const& bd,
      Index<SymmGroup> const& phys, charge_overlap<Matrix, SymmGroup> co
  )
      : cur_col(-1),
        c(-1),
        coldim(0),
    ...
    for (int k = 0; k < phys_i.size(); ++k) phys_set.insert(phys_i[k].first);
    ...
    rowstart = 2;
    for (typename Index<SymmGroup>::iterator it = in_index.begin();
         it != in_index.end(); ++it) {
    ...
    if (left_charge == SymmGroup::IdentityCharge) {
      ...
      for (map_it_t it =
               mpo_source.data_.lower_bound(std::make_pair(rowstart, 0));
           it != mpo_source.data_.lower_bound(std::make_pair(rowend + 1, 0));
           ++it) {
        typename map_t::key_type key_ =
            std::make_pair(it->first.first - rowstart + 2, it->first.second);
      ...
      for (map_it_t it =
           ...
           ++it) {
        typename map_t::key_type key_ =
            std::make_pair(it->first.first - rowstart, it->first.second);
        col_order[key_] = it->second;
      }
    }
  ...
  column_iterator& operator++() {
    ...
    for (mpo_index_t ls = 0; ls < phys_i.size(); ++ls) {
      ...
      if (phys_set.count(op_out) == 0) continue;
  ...
  ret_t operator*() const {
    ...
    while (op_it_loc != op_it_high) {
      ...
      // mpo_index_t r = ( (left_charge == SymmGroup::IdentityCharge) && (rloc <
      // 2) ? rloc : rloc+rowstart); maquis::cout << left_charge << "(" << rloc
      // << "," << cur_col << "): "
      //              << ret[rloc] << " <- (" << r << "," << c << ")" <<
      //              std::endl;
      ++op_it_loc;
    ...
    // for (typename MPOTensor<Matrix, SymmGroup>::data_t::const_iterator it =
    // mpo_source.data_.begin();
    //               it != mpo_source.data_.end(); ++it)
    //     if( it->first.second == c)
    //         maquis::cout << "Note: rrange: " << rowstart << "-" << rowend <<
    //         ", has (" << it->first.first << "," << c << "): " << it->second
    //         << std::endl;
    //    empty_count++;
    //}
  ...
  mpo_index_t key() const {
    return (mpo_index_t(c) << 4) + (local_ls << 2) +
           mpo_index_t(phys_i.position(op_out));
  }
...
class compressor {
  typedef typename SymmGroup::charge charge;
  typedef typename alps::numeric::associated_dense_matrix<Matrix>::type
      dense_matrix;
 ...
 public:
  ...
  void compress(
      MPO<Matrix, SymmGroup> const& mpo_in, MPO<Matrix, SymmGroup>& mpo_out,
      double cutoff
  );

  template <class Matrix2>
  static void convert_to_dense_matrix(
      MPO<Matrix, SymmGroup> const& mpo_in, MPO<Matrix2, SymmGroup>& mpo_out
  );

 private:
  block_matrix<dense_matrix, SymmGroup> make_left_matrix(
      MPO<Matrix, SymmGroup> const& mpo_in, std::size_t p
  );
  block_matrix<dense_matrix, SymmGroup> make_right_matrix(
      MPO<Matrix, SymmGroup> const& mpo_in, std::size_t p
  );
  block_matrix<dense_matrix, SymmGroup> make_M_matrix(
      MPO<Matrix, SymmGroup> const& mpo_in, MPO<Matrix, SymmGroup>& mpo_out,
      std::size_t p
  );

  void replace_pair(
      block_matrix<dense_matrix, SymmGroup> const& left,
...
template <class Matrix, class SymmGroup>
void compressor<Matrix, SymmGroup>::compress(
    MPO<Matrix, SymmGroup> const& mpo_in, MPO<Matrix, SymmGroup>& mpo_out,
    double cutoff
) {
  ...
  /*
  unsigned probe = 3;
  column_iterator<Matrix, SymmGroup> p1adaptor(mpo_in[probe],
  bond_descriptor.index(probe).charges()[6], bond_descriptor.index(probe),
  bond_descriptor[probe+1], phys_i, op_offset);
  ...
  maquis::cout << "Matrix block " << p1adaptor.row_dim() << "x" <<
  p1adaptor.col_dim() << ", empty cols: " << p1adaptor.empty_cols() <<
  std::endl;
  */
  ////////////////////////////////////////////////////////////////
  ...
  Timer cot("compression"), lm("left_matrix"), rm("right_matrix"),
      mm("m_matrix"), tsvd("SVD"), rep("replace");
  cot.begin();
  ...
  for (int p = 0; p < mpo_in.size() - 1; ++p) {
    ...
    mm.begin();
    block_matrix<dense_matrix, SymmGroup> MM =
        make_M_matrix(mpo_in, mpo_out, p);
    mm.end();
    ...
    block_matrix<
        typename alps::numeric::associated_real_diagonal_matrix<
            dense_matrix>::type,
        SymmGroup>
        S, Sqrt;

    // maquis::cout << "MM shape @ " << p << std::endl; mem_footprint(MM);
    // maquis::cout << std::endl;
    tsvd.begin();
    ...
    maquis::cout << "MPO bond truncation: "
  ...
  cot.end();
  maquis::cout << "Rest: "
               << cot.get_time() - tsvd.get_time() - rep.get_time() -
                      mm.get_time()
               << std::endl;
}

template <class Matrix, class SymmGroup>
block_matrix<typename compressor<Matrix, SymmGroup>::dense_matrix, SymmGroup>
compressor<Matrix, SymmGroup>::make_left_matrix(
    MPO<Matrix, SymmGroup> const& mpo_in, std::size_t p
) {
  ...
  typedef std::set<
      std::pair<mpo_index_t, mpo_index_t>, compressor_detail::pair_cmp_inv>
      rset_t;
  rset_t col_order;
  ...
  mpo_index_t cprev = 0;
  for (typename rset_t::iterator it = col_order.begin(); it != col_order.end();
       ++it) {
    ...
    for (mpo_index_t ls = 0; ls < phys_i.size(); ++ls) {
      ...
      if (!op.has_block(phys_i[ls].first, op_out)) continue;

      mpo_index_t cs = op.left_basis().position(phys_i[ls].first);
      ret(std::make_pair(rc, outr), std::make_pair(rc, visited_c_basis[rc])) =
          op[cs](0, 0);
    }
  }
...
template <class Matrix, class SymmGroup>
block_matrix<typename compressor<Matrix, SymmGroup>::dense_matrix, SymmGroup>
compressor<Matrix, SymmGroup>::make_right_matrix(MPO<Matrix, SymmGroup> const &
mpo_in, std::size_t p)
{
    Index<SymmGroup> left_i = bond_descriptor.index(p);
    Index<SymmGroup> right_i = adjoin(phys_i) * phys_i *
bond_descriptor.index(p+1); right_i = common_subset(right_i, left_i);
    block_matrix<dense_matrix, SymmGroup> ret(left_i, right_i);
...
inline void
axpy(const int*, const Value_Type*, const Value_Type*, const int*, Value_Type*, const int*) {
  throw std::runtime_error(
      "Blas axpy call not implemented for Matrix::value_type used\n"
  );
}

template <>
inline void axpy(
    const int* nr, const double* val, const double* x, const int* ix, double* y,
    const int* iy
) {
...
template <>
inline void axpy(
    const int* nr, const std::complex<double>* val,
    const std::complex<double>* x, const int* ix, std::complex<double>* y,
    const int* iy
) {
#if defined(BIND_FORTRAN_LOWERCASE) || defined(__xlC__)
...
template <class Matrix, class SymmGroup>
block_matrix<typename compressor<Matrix, SymmGroup>::dense_matrix, SymmGroup>
compressor<Matrix, SymmGroup>::make_M_matrix(
    MPO<Matrix, SymmGroup> const& mpo_in, MPO<Matrix, SymmGroup>& mpo_out,
    std::size_t p
) {
  Index<SymmGroup> left_i = phys_i * adjoin(phys_i) * bond_descriptor.index(p);
  ...
  Index<SymmGroup> right_i =
      adjoin(phys_i) * phys_i * bond_descriptor.index(p + 2);

  left_i = common_subset(left_i, mid_i1);
  right_i = common_subset(right_i, mid_i2);

  if (left_i.size() != right_i.size()) {
    maquis::cout << "M index left != right in size\n";
  }
  block_matrix<dense_matrix, SymmGroup> left =
      make_left_matrix((p > 0) ? mpo_out : mpo_in, p);
  // maquis::cout << "L shape @ " << p << std::endl; mem_footprint(left);
  // maquis::cout << std::endl;
  ...
  for (mpo_index_t csl = 0; csl < left_i.size(); ++csl) {
    ...
    column_iterator<Matrix, SymmGroup> rAdaptor(
        mpo_in[p + 1], left_i[csl].first, bond_descriptor.index(p + 1),
        bond_descriptor[p + 2], phys_i, op_offset
    );

    // dense_matrix block(left_i[csl].second, right_i[csl].second);
    mpo_index_t newpos = ret.insert_block(
        dense_matrix(left_i[csl].second, right_i[csl].second),
    ...
    for (; !rAdaptor.end(); ++rAdaptor) {
      ...
      std::vector<std::pair<mpo_index_t, typename Matrix::value_type> > mult =
          (*rAdaptor);
      for (typename std::vector<std::pair<
               mpo_index_t, typename Matrix::value_type> >::const_iterator
               col_it = mult.begin();
           col_it != mult.end(); ++col_it) {
        ...
        // axpy(&nr, &val, left[csl].col(s).first, &one,
        // ret[csl].col(colcnt).first, &one);
      }
    ...
    // maquis::cout << "resized " << p << "@" << left_i[csl].first << " block "
    // << right_i[csl].second << " -> " << colcnt << std::endl;
    ret.resize_block(
        left_i[csl].first, left_i[csl].first, left_i[csl].second, colcnt, false
    );
    ...
    */
    ...
    // maquis::cout << "block num_cols " << block.num_cols() << " is shrinkable
    // " << block.is_shrinkable() << std::endl;

    // M-block built

    // dense_matrix U, V, sU, sV;
    // typename
    // alps::numeric::associated_real_diagonal_matrix<dense_matrix>::type S;
...
template <class Matrix, class SymmGroup>
void compressor<Matrix, SymmGroup>::replace_pair(
    block_matrix<
        typename compressor<Matrix, SymmGroup>::dense_matrix, SymmGroup> const&
        left,
    block_matrix<
        typename compressor<Matrix, SymmGroup>::dense_matrix, SymmGroup> const&
        right,
  ...
  mpo_out[p] = MPOTensor<Matrix, SymmGroup>(
      (p > 0) ? mpo_out[p].row_dim() : mpo_in[p].row_dim(),
      bond_descriptor.index(p + 1).sum_of_sizes()
  );

  mpo_index_t term_cnt = 0;
  std::size_t bstart = 0, bend = 0;
  for (typename Index<SymmGroup>::const_iterator it =
           left.right_basis().begin();
      ...
      for (size_t r = 0; r < mpo_out[p].row_dim(); ++r) {
        ...
        for (size_t ls = 0; ls < phys_i.size(); ++ls) {
          ...
          if (std::abs(val) > 1e-40) {
            term_cnt++;
            mpo_out[p](r, c).insert_block(
                Matrix(1, 1, val), phys_i[ls].first, op_out
            );
          }
        }
      }
  ...
  // maquis::cout << "left replace @" << p << ": " << term_cnt << " terms in "
  // << mpo_out[p].data_.size() << " map blocks\n"; maquis::cout << "original
  // size " << mpo_in[p].data_.size() << " map blocks\n";

  mpo_out[p + 1] = MPOTensor<Matrix, SymmGroup>(
      bond_descriptor.index(p + 1).sum_of_sizes(), mpo_in[p + 1].col_dim()
  );
    ...
    for (size_t r = bstart; r < bend; ++r) {
      for (size_t k = 0; k < locKey.size(); ++k) {
        ...
        if (std::abs(val) > 1e-40) {
          ...
          mpo_index_t c = (key >> 4);
          mpo_out[p + 1](r, c).insert_block(
              Matrix(1, 1, val), phys_i[((key & 12u) >> 2)].first,
              phys_i[key & 3u].first
          );
          // ***** Debug *********
          // maquis::cout << "right checkback " << p+1 << "(" << r << "," << c
          // << ")[" << ls << "," << phys_i.position(op_out) << "]: " << val
          //             << " | key " << (key >> 4) << "," << ((key&12u)>>2) <<
          //             "," << (key&3u) << std::endl;
        }
      }
  ...
  // maquis::cout << "right replace @" << (p+1) << ": " << term_cnt << " terms
  // in " << mpo_out[p+1].data_.size() << " map blocks\n"; maquis::cout <<
  // "original size " << mpo_in[p+1].data_.size() << " map blocks\n";
}

template <class Matrix, class SymmGroup>
template <class Matrix2>
void compressor<Matrix, SymmGroup>::convert_to_dense_matrix(
    MPO<Matrix, SymmGroup> const& mpo_in, MPO<Matrix2, SymmGroup>& mpo_out
) {
  mpo_out.resize(mpo_in.size());
  for (int p = 0; p < mpo_in.size(); ++p) {
    mpo_out[p] =
        MPOTensor<Matrix2, SymmGroup>(mpo_in[p].row_dim(), mpo_in[p].col_dim());
    for (map_it_t it = mpo_in[p].data_.begin(); it != mpo_in[p].data_.end();
         ++it) {
      block_matrix<Matrix2, SymmGroup> new_op;
      block_matrix<Matrix, SymmGroup> const& old_op = it->second;
      for (int b = 0; b < old_op.n_blocks(); ++b)
        new_op.insert_block(
            Matrix2(1, 1, old_op[b](0, 0)), old_op.left_basis()[b].first,
            old_op.right_basis()[b].first
        );
...
// Relabel the MPO-indices such that the 13 possible bond charges are grouped
// together
template <class Tuple, class Matrix, class SymmGroup>
void charge_sort(
    std::vector<std::vector<
        // boost::tuple<std::size_t, std::size_t, block_matrix<Matrix,
        // SymmGroup> > > > & prempo,
        Tuple> >& prempo,
    MPO<Matrix, SymmGroup> const& mpo_in
) {
  // typedef boost::tuple<std::size_t, std::size_t, block_matrix<Matrix,
  // SymmGroup> > block;
  typedef typename SymmGroup::charge charge;

  Timer csort("MPO_charge_sort");
  csort.begin();
  ...
  for (std::size_t p = 0; p < prempo.size() - 1; ++p) {
    ...
    std::size_t offset = 2;
    for (typename std::map<charge, std::size_t>::reverse_iterator it =
             visited_delta_basis.rbegin();
    ...
    for (typename std::vector<Tuple>::iterator it = prempo[p + 1].begin();
         it != prempo[p + 1].end(); ++it)
