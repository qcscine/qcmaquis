...
struct measure_ {
  ...
  void operator()(
      MPS<Matrix, SymmGroup>& mps, adj::Adjacency& adj,
      mpos::Hamiltonian<Matrix, SymmGroup>& H, BaseParameters& model,
      Archive& ar
  ) {}
};

template <class Matrix, class SymmGroup, class Archive>
void measure_2pt_correlation(
    MPS<Matrix, SymmGroup>& mps, adj::Adjacency& adj,
    block_matrix<Matrix, SymmGroup> const& identity,
    block_matrix<Matrix, SymmGroup> const& fill, Archive& ar,
    std::vector<block_matrix<Matrix, SymmGroup> >& ops, std::string base_path
) {
  std::vector<double> dc;
  std::vector<std::string> labels;
  for (size_t p = 0; p < adj.size() - 1; ++p) {
    mpos::CorrMaker<Matrix, U1> dcorr(mps.length(), identity, fill, ops, p);
...
struct measure_<Matrix, U1> {
  template <class Archive>
  void measure_blbq(
      MPS<Matrix, U1>& mps, adj::Adjacency& adj,
      mpos::Hamiltonian<Matrix, U1>& H, BaseParameters& model, Archive& ar
  ) {
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      std::string n = std::string("/spectrum/results/") + names[i] +
                      std::string("/mean/value");
      ar[n] << magns;
    }
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      corr.push_back(ops[i]);
      //            mpos::CorrMaker<Matrix, U1> mcorr(mps.length(), ident,
      //            ident, corr); MPO<Matrix, U1> mpo = mcorr.create_mpo();
      //
      //            std::vector<double> mc_v = maquis::real(multi_expval(mps,
      //            mpo));
      std::string name = std::string("/spectrum/results/") + names[i] +
                         std::string("Correlation");

      measure_2pt_correlation(mps, adj, ident, ident, ar, corr, name);
    }
  }
  ...
  void measure_ff(
    ...
    density_corr.push_back(dens);
    measure_2pt_correlation(
        mps, adj, ident, ident, ar, density_corr,
    ...
    measure_2pt_correlation(
        mps, adj, ident, sign, ar, onebody, "/spectrum/results/OneBodyDM"
    );
  }
...
void measure(
    MPS<Matrix, SymmGroup>& mps, adj::Adjacency& adj,
    mpos::Hamiltonian<Matrix, SymmGroup>& H, BaseParameters& model, Archive& ar
) {
