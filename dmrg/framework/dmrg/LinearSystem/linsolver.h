...
class LinSolver {
  ...
  // using mat_type = typename Eigen::Matrix<ScalarType, Eigen::Dynamic,
  // Eigen::Dynamic>; using vec_type = Eigen::Matrix<ScalarType, Eigen::Dynamic,
  // 1>;
 ...
 public:
  /**
   * @brief Constructor
   * @param sp Pointer to the SiteProblem representing the lhs of the linear
   * system.
   ...
   * @param precond pointer to the preconditioner. If set, applies the
   * preconditioning.
   */
  LinSolver(
      std::shared_ptr<SiteProblem<Matrix, SymmGroup>> sp,
      const MPSTensorType& initialMPS, const MPSTensorType& rhsMPS,
      ScalarType shift, BaseParameters& parms,
      std::shared_ptr<block_matrix<Matrix, SymmGroup>> precond, bool verbose,
      RealType coreEnergy = 0
  )
      : sp_(sp),
        parms_(parms),
        rhsMPS_(rhsMPS),
  ...
  //, isFolded_(false)
  {
    ...
    if (parms_["linsystem_solver"] == "GMRES") {
      if (verbose_)
        maquis::cout << " - Solving the linear system with ALPS-based GMRES"
                     << std::endl;
      ...
      if (parms_["linsystem_solver"] == "GMRES_EIGEN") {
        ...
        if (verbose_)
          maquis::cout
              << " - Solving the linear system with Eigen-based BiCGSTAB"
              << std::endl;
      }
  ...
  std::tuple<energy_type, energy_type, MPSTensorType> res() {
    ...
    auto en = maquis::real(
        ietl::dot(currentSolution_, tmp3) /
        ietl::dot(currentSolution_, currentSolution_)
    );
    if (verbose_) {
 ...
 protected:
  ...
  void eigenLinearSolver() {
    ...
    using SolverType = typename EigenSolverTraitClass<
        MatrixWrapperType, EigenSolver, Preconditioner>::SolverType;
    ...
    Eigen::Matrix<ScalarType, Eigen::Dynamic, 1> initialEigenMatrix =
        currentSolution_.getEigenRepresentation();
    Eigen::Matrix<ScalarType, Eigen::Dynamic, 1> rhsMatrix =
        rhsMPS_.getEigenRepresentation();
    Eigen::Matrix<ScalarType, Eigen::Dynamic, 1> newEigenVector =
        solver.solveWithGuess(rhsMatrix, initialEigenMatrix);
    maquis::cout << std::endl;
    maquis::cout << " - Linear solver converged after " << solver.iterations()
                 << " iterations." << std::endl;
  ...
  void gmres() {
    ...
    if (precond_) {
      ...
      initialErrorNorm =
          std::sqrt(maquis::real(ietl::dot(initialError, preconditionedError)));
    } else {
    ...
    if (verbose_ && (residual[iter] > gmresTol_)) {
      ...
      maquis::cout << " ------------------------------------- " << std::endl;
      maquis::cout << std::setw(5) << iter << "          " << std::setw(15)
                   << std::scientific << residual[iter] << std::endl;
    }
    // == MAIN LOOP ==
    while (residual[iter] > gmresTol_ && iter < krylovDim_ - 1 && !exit) {
      ...
      for (int iPair = 0; iPair < iter; iPair++) {
        std::tie(R(iPair, iter), R(iPair + 1, iter)) =
            givensRotations[iPair].apply(R(iPair, iter), R(iPair + 1, iter));
      }
      givensRotations.emplace_back(GivensType(R(iter, iter), R(iter + 1, iter))
      );
      std::tie(R(iter, iter), R(iter + 1, iter)) =
          givensRotations[iter].apply(R(iter, iter), R(iter + 1, iter));
      std::tie(y[iter], y[iter + 1]) =
          givensRotations[iter].apply(y[iter], y[iter + 1]);
    ...
    if (iter != 0) {
      // vec_type result = R.block(0, 0, iter,
      // iter).colPivHouseholderQr().solve(y.head(iter)); vec_type result =
      // vec_type::Zero(iter);
      ...
      */
      auto info =
          boost::numeric::bindings::lapack::gels(smallerMatrix, smallerVector);
      if (info != 0) {
  ...
  /** @brief Solve the linear system with MINRES (based on the PyKry python
   * library). */
  void minres() {
    // Printing
    if (verbose_) {
 ...
 private:
  ...
  /* Private members */
  std::shared_ptr<SiteProblem<Matrix, SymmGroup>>
      sp_;  // Pointer to the site problem representing the linear system.
  std::shared_ptr<block_matrix<Matrix, SymmGroup>>
      precond_;                  // Pointer to the preconditioner.
  const MPSTensorType& rhsMPS_;  // Reference to the MPS representing the RHS of
                                 // the local linear system.
  BaseParameters& parms_;        // Parameter container.
  ScalarType shift_;             // Shift to apply to the Hamiltonian.
  MPSTensorType currentSolution_;  // Stores the current approximation to the
                                   // solution of the linear system.
  int numberOfMacroIterations_;    // Number of restarts for the solution of the
                                   // linear system.
  int krylovDim_;                  // Maximum dimension of the Krylov space.
  RealType gmresTol_;  // Convergence threshold for the iterative solution to
                       // the linear system.
  ...
  RealType coreEnergy_;  // Core energy to be added to local result (nonzero for
                         // electronic problems)
};
