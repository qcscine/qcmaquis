...
class Givens {
 public:
  ...
  std::pair<double, std::complex<double>> calculateRotation(
      std::complex<double> ca, std::complex<double> cb
  ) {
    if (std::abs(ca) < 1.0E-16) {
      ...
      auto norm =
          scale *
          std::sqrt(
              std::pow(std::abs(ca / std::complex<double>(scale, 0.0)), 2) +
