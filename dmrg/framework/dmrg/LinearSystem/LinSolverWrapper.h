...
enum class PreconditionerType { IdentityPreconditioner, DiagonalPreconditioner };

template<class MatrixWrapper, EigenSolverType EigenSolver, PreconditionerType Preconditioner>
class EigenSolverTraitClass;

template<class MatrixWrapper>
class EigenSolverTraitClass<MatrixWrapper, EigenSolverType::BiCGSTAB, PreconditionerType::IdentityPreconditioner> {
public:
  using SolverType = Eigen::BiCGSTAB<MatrixWrapper, Eigen::IdentityPreconditioner>;
};

template<class MatrixWrapper>
class EigenSolverTraitClass<MatrixWrapper, EigenSolverType::BiCGSTAB, PreconditionerType::DiagonalPreconditioner> {
  using ScalarType = typename MatrixWrapper::Scalar;
public:
  using SolverType = Eigen::BiCGSTAB<MatrixWrapper, Eigen::DiagonalPreconditioner<ScalarType> >;
};

template<class MatrixWrapper>
class EigenSolverTraitClass<MatrixWrapper, EigenSolverType::GMRES, PreconditionerType::IdentityPreconditioner> {
public:
  using SolverType = Eigen::GMRES<MatrixWrapper, Eigen::IdentityPreconditioner>;
};

template<class MatrixWrapper>
class EigenSolverTraitClass<MatrixWrapper, EigenSolverType::GMRES, PreconditionerType::DiagonalPreconditioner> {
  using ScalarType = typename MatrixWrapper::Scalar;
public:
  using SolverType = Eigen::GMRES<MatrixWrapper, Eigen::DiagonalPreconditioner<ScalarType> >;
};
...
struct traits<LinearSolverWrapper<Matrix, SymmGroup>>
  : public traits<Eigen::Matrix<typename Matrix::value_type, Eigen::Dynamic, Eigen::Dynamic>> {};
} // namespace internal
} // namespace Eigen
 ...
 */
template<class Matrix, class SymmGroup>
class LinearSolverWrapper : public Eigen::EigenBase<LinearSolverWrapper<Matrix, SymmGroup>> {
 public:
  ...
  /**
   * @brief Constructor taking the wave function gradients and their expectation value.
   * @param gradients A matrix containing the determinant's gradient as columns and parameters for rows.
   * @param gradientsExpectation The expectation value of the gradients over determinants.
   ...
   */
  LinearSolverWrapper(std::shared_ptr<SiteProblemType> siteProblem, const VectorType& mpsTensor,
                      std::shared_ptr<BlockMatrixType> preconditioner, Scalar z)
    : matrixFree(siteProblem), mpsReference(mpsTensor), shift(z)
  {
    if (preconditioner)
      populatePreconditioner(preconditioner);
  };

  /** @brief Getter for the rows of the wrapped matrix. */
  Eigen::Index rows() const {
    return mpsReference.num_elements();
  }
  ...
  auto getPrecondValue(int idx) const {
    ...
    return this->isPrecond() ? preconditionValues[idx] : static_cast<Scalar>(1.);
  }
  ...
  VectorType getCopyRefMPSTensor() const {
    return mpsReference;
  }

  /** @brief Gets a reference to the underlying SiteProblem object */
  auto getSiteProblem() const {
    return matrixFree;
  }
  ...
  Eigen::Product<LinearSolverWrapper<Matrix, SymmGroup>, Rhs, Eigen::AliasFreeProduct>
  operator*(const Eigen::EigenBase<Rhs>& rhs) const {
    return Eigen::Product<LinearSolverWrapper<Matrix, SymmGroup>, Rhs, Eigen::AliasFreeProduct>(*this, rhs.derived());
  }
...
struct generic_product_impl<LinearSolverWrapper<Matrix, SymmGroup>, Rhs, DenseShape, DenseShape, GemvProduct>
  : generic_product_impl_base<LinearSolverWrapper<Matrix, SymmGroup>, Rhs, generic_product_impl<LinearSolverWrapper<Matrix, SymmGroup>, Rhs>>
{
  ...
  /**
   * @brief Implement the effect of the matrix-vector multiplication. This is the formation of the simga vector.
  ...
  static void scaleAndAddTo(Destination& dest, const WrapperType& lhs, const Rhs& rhs, const Scalar& alpha) {
    // This method should implement "dst += alpha * lhs * rhs" inplace,
    // however, for iterative solvers, alpha is always equal to 1, so let's not bother about it.
    assert(alpha == static_cast<Scalar>(1) && "scaling is not implemented");
    EIGEN_ONLY_USED_FOR_DEBUG(alpha);
    assert(dest.size() == rhs.size() && "Destination vector has not the same size as input one in iterative SR.");
