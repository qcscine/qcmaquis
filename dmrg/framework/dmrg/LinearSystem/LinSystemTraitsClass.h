...
class LinSystemTraitClass {
 public:
  ...
  /** @brief Calculates the error associated with the solution of a linear
   * system */
  static double calculateError(
      const MPSType& mps, const MPSType& rhsMps, const MPOType& mpo,
      ValueType shift, const ModelType& model, const Lattice& lattice,
      ChargeType totalQN, int mMax
  ) {
    auto traitClass = MPOTimesMPSTraitClass<Matrix, SymmGroup>(
        mps, model, lattice, totalQN, mMax
    );
    auto outputMPS = traitClass.applyMPO(mpo);
    double res = 0.;
    // First term, (H - zI)^2. Note that we do not include the core contribution
    // in H since it is not included in the solution of the linear system as
    // well.
    res += norm(outputMPS);
