...
TagHandler<Matrix, SymmGroup>::size() const {
  return operator_table->size();
}

template <class Matrix, class SymmGroup>
std::shared_ptr<OPTable<Matrix, SymmGroup>>
TagHandler<Matrix, SymmGroup>::get_operator_table() const {
  return operator_table;
}

template <class Matrix, class SymmGroup>
bool TagHandler<Matrix, SymmGroup>::is_fermionic(
    typename OPTable<Matrix, SymmGroup>::tag_type query_tag
) const {
...
template <class Matrix, class SymmGroup>
typename OPTable<Matrix, SymmGroup>::tag_type
TagHandler<Matrix, SymmGroup>::herm_conj(
    typename OPTable<Matrix, SymmGroup>::tag_type query_tag
) const {
...
template <class Matrix, class SymmGroup>
typename OPTable<Matrix, SymmGroup>::tag_type
TagHandler<Matrix, SymmGroup>::register_op(
    const op_t& op_, tag_detail::operator_kind kind
) {
...
TagHandler<Matrix, SymmGroup>::checked_register(
    typename OPTable<Matrix, SymmGroup>::op_t const& sample,
    tag_detail::operator_kind kind
) {
  std::pair<tag_type, value_type> ret =
      operator_table->checked_register(sample);
...
template <class Matrix, class SymmGroup>
void TagHandler<Matrix, SymmGroup>::hermitian_pair(
    typename OPTable<Matrix, SymmGroup>::tag_type pair_tag1,
  ...
  if (hermitian[pair_tag1] == pair_tag2 && hermitian[pair_tag2] == pair_tag1) {
    return;
  }
  assert(
      hermitian[pair_tag1] == pair_tag1 && hermitian[pair_tag2] == pair_tag2
  );
...
// access operators
template <class Matrix, class SymmGroup>
typename OPTable<Matrix, SymmGroup>::value_type&
TagHandler<Matrix, SymmGroup>::get_op(tag_type i) {
  return (*operator_table)[i];
}

template <class Matrix, class SymmGroup>
typename OPTable<Matrix, SymmGroup>::value_type const&
TagHandler<Matrix, SymmGroup>::get_op(tag_type i) const {
  return (*operator_table)[i];
}

template <class Matrix, class SymmGroup>
std::vector<typename OPTable<Matrix, SymmGroup>::value_type>
TagHandler<Matrix, SymmGroup>::get_ops(std::vector<tag_type> const& tags
) const {
...
template <class Matrix, class SymmGroup>
bool TagHandler<Matrix, SymmGroup>::product_is_null(
...
std::pair<
    typename OPTable<Matrix, SymmGroup>::tag_type,
    typename TagHandler<Matrix, SymmGroup>::value_type>
TagHandler<Matrix, SymmGroup>::get_product_tag(
    const typename OPTable<Matrix, SymmGroup>::tag_type t1,
    const typename OPTable<Matrix, SymmGroup>::tag_type t2
) {
  ...
  // compute and register the product, then return the new tag
    ...
    std::pair<tag_type, value_type> ret =
        this->checked_register(product, prod_kind);
...
template <class Matrix, class SymmGroup>
std::pair<
    std::vector<typename OPTable<Matrix, SymmGroup>::tag_type>,
    std::vector<typename TagHandler<Matrix, SymmGroup>::value_type>>
TagHandler<Matrix, SymmGroup>::get_product_tags(
    std::vector<typename OPTable<Matrix, SymmGroup>::tag_type> const& ops1,
  ...
  for (typename SymmGroup::subcharge sc = 0; sc < ops1.size(); ++sc) {
    std::pair<tag_type, value_type> ptag =
        this->get_product_tag(ops1[sc], ops2[sc]);
...
template <class Map>
typename OPTable<Matrix, SymmGroup>::tag_type
TagHandler<Matrix, SymmGroup>::duplicates_(Map const& sample) {
  ...
  for (const auto& it_s : sample) {
    ...
    for (const auto& it_unique : unique_ops) {
      if (equal(
              (*operator_table)[it_s.second.first], (*operator_table)[it_unique]
          )
              .first) {
        unique = false;
        break;
...
template <class Matrix, class SymmGroup>
typename OPTable<Matrix, SymmGroup>::tag_type
TagHandler<Matrix, SymmGroup>::get_num_products() const {
