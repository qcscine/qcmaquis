 ...
 public:
  ...
  Orbitals(BaseParameters& parms) : L(parms["L"]), irreps(L, 0), order(L) {
    ...
    if (parms.is_set("integral_file")) {
      ...
      throw std::runtime_error(
          "\"integral_file\" in parms input file or site_types is not set\n"
      );
    }
  ...
  std::any get_prop_(std::string const& property, std::vector<pos_t> const& pos)
      const override {
    if (property == "label" &&
        pos.size() == 1) {  // return "( label )" as string
      return std::any(site_label(order[pos[0]]));
    }
    if (property == "label_int" &&
        pos.size() == 1) {  // just return label as integer
