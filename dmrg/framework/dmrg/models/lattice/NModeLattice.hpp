...
class NModeLattice : public lattice_impl {
 public:
   ...
   */
  NModeLattice(BaseParameters& parameters)
      : L(0), vectorWithStartingPositions(0), vector_types(0) {
    // Parameter initialization
    L = parameters["L"];
    order =
        LatticeHelperClass::getOrbitalOrder(parameters, "modals_order", false);
    inverseOrder.resize(L);
    for (int iOrder = 0; iOrder < order.size(); iOrder++) {
      auto pointerToFoundElement =
          std::find(order.begin(), order.end(), iOrder);
      assert(pointerToFoundElement != order.end());
      inverseOrder[iOrder] =
          std::distance(order.begin(), pointerToFoundElement);
    }
      ...
      if (idx == 0) {
        ...
        if (mod <= 0)
          ...
          vectorWithStartingPositions.push_back(
              vectorWithStartingPositions[idx - 1] + mod
          );
      }
    }
    int count = vectorWithStartingPositions[size_vec.size() - 1] +
                stoi(size_vec[size_vec.size() - 1]);
    if (count != L)
    ...
    for (int iSite = 0; iSite < L; iSite++) {
      if (iMode != num_modes - 1 &&
          iSite == vectorWithStartingPositions[iMode + 1])
        iMode++;
   ...
   * the mode and modal index, returns the corresponding absolute position in
   * the DMRG lattice.
   *
  ...
  std::any get_prop_(std::string const& property, std::vector<pos_t> const& pos)
    ...
    if (property == "label" && pos.size() == 1)
      ...
      int posInConventionalSorting =
          vectorWithStartingPositions[pos[0]] + pos[1];
