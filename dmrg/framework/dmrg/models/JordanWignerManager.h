...
enum class OperatorType { Normal, CreateAlpha, CreateBeta, DestroyAlpha, DestroyBeta, Filling };

namespace OperatorTypeUtils {

inline bool isAlpha(OperatorType tmp) { return tmp == OperatorType::CreateAlpha || tmp == OperatorType::DestroyAlpha; }
inline bool isBeta(OperatorType tmp) { return tmp == OperatorType::CreateBeta || tmp == OperatorType::DestroyBeta; }
inline bool isCreate(OperatorType tmp) { return tmp == OperatorType::CreateAlpha || tmp == OperatorType::CreateBeta; }
inline bool isDestroy(OperatorType tmp) { return tmp == OperatorType::DestroyAlpha || tmp == OperatorType::DestroyBeta; }
...
public:
   ...
   */
  JordanWignerHandler(const Lattice& lat, const std::vector<tag_type>& fill_op, const std::vector<tag_type>& create_up_op,
                      const std::vector<tag_type>& create_down_op, const std::vector<tag_type>& destroy_up_op,
                      const std::vector<tag_type>& destroy_down_op)
      : lat_(lat), fillOp_(fill_op), createUpOp_(create_up_op), createDownOp_(create_down_op),
        destroyUpOp_(destroy_up_op), destroyDownOp_(destroy_down_op) 
      {}

  /**
   * @brief Construct from a single set of tags
   */
  JordanWignerHandler(const Lattice& lat, tag_type fill_op, tag_type create_up_op, tag_type create_down_op,
                      tag_type destroy_up_op, tag_type destroy_down_op) : lat_(lat) {
    fillOp_ = {fill_op};
  ...
  term_descriptor getTerm(std::vector<pos_t> positions, std::vector<OperatorType> tags,
    ...
    for (auto i = localOperatorBuffer.begin(); i != localOperatorBuffer.end();) {
      auto n = std::next(i);
      if (n == localOperatorBuffer.end())
        break;
      if (i->posType_ == n->posType_ && i->opType_ == n->opType_ && i->opType_ == OperatorType::Filling) {
    ...
    for (int idx = 0; idx < localOperatorBuffer.size(); idx++) {
      ...
      tag_type localTag = (localOperatorBuffer[idx].opType_ == OperatorType::Filling) 
        ? fillOp_[lat_.get_prop<sc_t>("type", localOperatorBuffer[idx].posType_)] : localOperatorBuffer[idx].tagType_;
...
private:
   ...
   */
  double calculatePermutationSign(const std::vector< PairType >& positionVector) const {
