...
struct coded_model_factory<Matrix, TwoU1PG> {
    ...
    static std::shared_ptr<model_impl<Matrix, TwoU1PG> > parse(Lattice const & lattice, BaseParameters & parms)
    {
        ...
        if (parms["MODEL"] == std::string("quantum_chemistry")) {
            ...
            return impl_ptr( new qc_model<Matrix, TwoU1PG, chem::Hamiltonian::Electronic>(lattice, parms) );
        }
        else {
