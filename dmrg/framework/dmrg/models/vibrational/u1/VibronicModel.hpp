 ...
 public:
  ...
  VibronicModel(const Lattice& lattice_, BaseParameters& parameters_)
      : lat(lattice_),
        ...
        L_(parameters_["L"]),
        n_ele_states_(parameters_["vibronic_num_elestates"]),
        n_vib_states_(parameters_["vibronic_num_vibmodes"]) {
    // Variable definition
    int nMax = parameters_["Nmax"];
    int maxCoupling =
        chem::getIndexDim(
            chem::Hamiltonian::Vibronic,
            chem::HamiltonianTransformation::Conventional
        ) /
        2;  // Two indices are required as the electronic state is also included
    op_t ident_vib_op, ident_ele_op, create_ele_op, destroy_ele_op,
        count_ele_op;
    ...
    // Moreover, we explicitly write q^2 and p^2 - we don't need to be overly
    // generic, since we don't have so far anharmonic couplings.
  ...
  void create_terms() override {
    auto hamiltonianTerms =
        Vibrational::detail::parseIntegralVibronic<value_type>(parameters, lat);
    for (int iSize = 0; iSize < hamiltonianTerms.first.size(); iSize++) {
      ...
      auto uniqueCoefficients = std::set<int>(
          hamiltonianTerms.first[iSize].begin() + 2,
          hamiltonianTerms.first[iSize].end()
      );
      uniqueCoefficients.erase(0);
      // Manages separately the vertical energy
      bool isConstantTerm = uniqueCoefficients.size() == 1 &&
                            hamiltonianTerms.first[iSize][2] == 0;
      if (!isConstantTerm) {
        for (const auto& iSite : uniqueCoefficients) {
          // Note that here the index of the
          auto vibrationalIndex =
              lat.get_prop<int>("vibindex", 0, std::abs(iSite) - 1);
          positions.push_back(vibrationalIndex);
          auto numberOfOccurrences = std::count(
              hamiltonianTerms.first[iSize].begin() + 2,
  ...
  // Type physical basis depends on the type of the site (0 --> nuclear, 1 -->
  // electron)
  ...
  // Total quantum number which must be obtained at the end of the MPS. Should
  // be 1 in all cases.
