...
class HolsteinHubbardExcitonicHamiltonian : public model_impl<Matrix, U1> {
 public:
   ...
   * In the Holstein-Hubbard Hamiltonian, we have multiple monomers, each one
   * described by harmonic PESs, where the excited states are modelled with the
   * LVC model. Moreover, off-diagonal coordinate-independent electronic
   * coupling terms are present.
   */
  HolsteinHubbardExcitonicHamiltonian(
    ...
    maxCoupling = chem::getIndexDim(
        chem::Hamiltonian::Excitonic,
    ...
    if (model["vibronic_J_interaction_type"] == "nn") only_nn_ = true;
    // Variable definition
    ...
    // First we manage the dimensions for the vibrations, then the ones of the
    // nuclei.
    ...
    for (int iOrder = 1; iOrder <= maxCoupling; iOrder++) {
      positionPowers[iOrder] = tag_handler->register_op(
          powersOfPositions_op[iOrder], tag_detail::bosonic
      );
      momentumPowers[iOrder] = tag_handler->register_op(
  ...
  void create_terms() override {
    ...
    auto hamiltonianTerms =
        Vibrational::detail::parseIntegralExcitonic<value_type>(model, lat);
