  ...
  ExcitonicNmodeProducts(const Lattice& lat_, BaseParameters& model_)
    ...
    for (const auto& nModals_idx : nModalsUnique) {
      ...
      for (int n = 1; n < overallDimension; n++) {
        Matrix mcreate(overallDimension, overallDimension, 0.),
      ...
      for (const auto& iEl : mdestroyVec)
        std::cout << "destroy operator " << iEl << std::endl;
      for (const auto& iEl : mcreateVec)
        std::cout << "create operator " << iEl << std::endl;
      // local operators
    ...
    create = modelHelper<Matrix, U1>::register_all_types(
    ...
    destroy = modelHelper<Matrix, U1>::register_all_types(
    ...
    for (int i_body = 0; i_body < n_particles_;
      ...
      for (int idx = 0; idx < hamiltonianTerms.first.size();
        ...
        for (int i = 0; i < modes.size(); i += 2) {
          ...
          std::cout << "pushed back vib op : " << create[sum + modalToCreate]
                    << std::endl;
          operators.push_back(destroy[sum + modalToDestroy]);
          std::cout << "pushed back vib op : " << destroy[sum + modalToCreate]
                    << std::endl;
        }
        if (skip) continue;
