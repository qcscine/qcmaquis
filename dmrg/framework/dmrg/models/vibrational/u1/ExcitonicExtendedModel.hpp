...
class HolsteinbHubbardExcitonicExtendedHamiltonian
 ...
 public:
   ...
   * In contrast to the Regular Holstein Hubbard Hamiltonian, excited states can
   * be equipped with an energy offsest and non-local potentials can be present
   * Moreover, off-diagonal coordinate-independent electronic coupling terms are
   * present.
   */
  HolsteinbHubbardExcitonicExtendedHamiltonian(
      const Lattice& lat_, BaseParameters& model_
  )
      : lat(lat_),
        model(model_),
        /*tag_handler(new table_type()),*/ L_(model["L"]),
        n_ele_states_(model["vibronic_num_elestates"]),
        n_vib_states_(model["vibronic_num_vibmodes"]),
        n_particles_(model["vibronic_num_molecules"]),
        phys_indexes(0),
        J_(0.),
        epsilon_(1.),
        only_nn_(false) {
    ...
    check_maxVibMode = 0;
    //  Note that, since we have two different types of sites, we also have
    //  different sets of operators for electrons and for the vibrations
    op_t ident_ele_op;
    ...
    if (nMaxVec.size() == 1) {
      ...
      throw std::runtime_error(
          "Nmax needs to be either a single integer or a list with lenght "
          "n_modes*n_particles-n_connectingmodes"
      );
    }
    ...
    phys_indexes.resize(
        num_vibtypes + 1
    );  // TODO: Number of excitons hardcoded to 1 atm
    ...
    for (const auto& iEl : phys_indexes) std::cout << iEl << std::endl;
#endif
    // Registering the electronic operators
    ident_ele_op.insert_block(Matrix(1, 1, 1), 0, 0);
    ident_ele_op.insert_block(Matrix(1, 1, 1), 1, 1);
    create_ele_op.insert_block(
        Matrix(1, 1, 1), 0, 1
    );  // contrary to usual matrix index notation, the first index here
        // corresponds to the column and the second one to the row
    destroy_ele_op.insert_block(
        Matrix(1, 1, 1), 1, 0
    );  // contrary to usual matrix index notation, the first index here
        // corresponds to the column and the second one to the row
    count_ele_op.insert_block(Matrix(1, 1, 1), 1, 1);
    count_ele_op_gs.insert_block(
        Matrix(1, 1, 1), 0, 0
    );  // electronic ground state count operator
#ifndef NDEBUG
    ...
    std::cout << "electronic count, ground state" << count_ele_op_gs
              << std::endl;
#endif
    ...
    count_ele_gs =
        tag_handler->register_op(count_ele_op_gs, tag_detail::bosonic);
    ...
    for (const auto& nMax : nMaxUnique) {
      ...
      Matrix mpos(overallDimension, overallDimension, 0.),
          mmom(overallDimension, overallDimension, 0.),
          mident(overallDimension, overallDimension, 0.);  // dimension right?
      // loads matrices
      mident(0, 0) = 1.;
      ...
      auto powersOfPositions_op =
          VibrationalHelpers<Matrix, U1>::generatePowersOfPositionOperator(
              maxCoupling, nMax, ident_vib_op, position_vib_op
          );
      auto powersOfMomentum_op =
          VibrationalHelpers<Matrix, U1>::generatePowersOfMomentumOperator(
              maxCoupling, nMax, ident_vib_op, momentum_vib_op
          );
      ident_vib_op.resize_block(0, nMax, nMax);
      ident_vib[nMax] =
          tag_handler->checked_register(ident_vib_op, tag_detail::bosonic);
#ifndef NDEBUG
      std::cout << ident_vib_op << std::endl;
      std::cout << "Identity registered with tag " << ident_vib[nMax].first
                << " and coefficient " << ident_vib[nMax].second << std::endl;
#endif
      ...
      for (int iOrder = 1; iOrder <= maxCoupling; iOrder++) {
        positionPowers[nMax][iOrder] = tag_handler->checked_register(
            powersOfPositions_op[iOrder], tag_detail::bosonic
        );
        momentumPowers[nMax][iOrder] = tag_handler->checked_register(
            powersOfMomentum_op[iOrder], tag_detail::bosonic
        );
#ifndef NDEBUG
        std::cout << powersOfPositions_op[iOrder] << std::endl;
        std::cout << powersOfMomentum_op[iOrder] << std::endl;
        std::cout << "Position registered with tag "
                  << positionPowers[nMax][iOrder].first << " and coeff "
                  << positionPowers[nMax][iOrder].second << std::endl;
        std::cout << "Momentum registered with tag "
                  << momentumPowers[nMax][iOrder].first << " and coeff "
                  << momentumPowers[nMax][iOrder].second << std::endl;
#endif
      }
    }
  }
  ...
  void create_terms() override {
    ...
    auto hamiltonianTerms =
        Vibrational::detail::parseIntegralExcitonicExtended<value_type>(
            model, lat
        );
    // == Extracts whether we want to freeze any degrees of freedom
    // == Main loop over the monomers ==
    // We first loop over the number of molecules of the aggregate, and then
    // over the terms entering the vibronic Hamiltonian.
    for (int i_body = 0; i_body < n_particles_;
         i_body++) {  // loop over all monomers
      std::vector<int> vec_jnk(2);
      std::vector<int> vec_jnk_next(2);
      vec_jnk[0] = i_body;
      vec_jnk_next[0] = i_body + 1;
      int flag = 0;
      for (int idx = 0; idx < hamiltonianTerms.first.size();
           idx++) {  // loop over all rows of the integral file
        if (i_body == n_particles_ - 1 && abs(hamiltonianTerms.first[idx][2]) >
                                              n_vib_states_ - n_connectingmodes)
          break;
        // Prepares the vectors to be employed when building the Hamiltonian
        std::vector<tag_type> operators;
        std::vector<pos_t> positions;
        int ele_state =
            hamiltonianTerms.first[idx][0];  // store whether the parameters
                                             // read in correspond to an excited
                                             // or ground electronic state
        int connecting =
            hamiltonianTerms
                .first[idx]
                      [1];  // stores information wether the vibrational mode is
                            // monomer-internal or connecting two monomers
        ...
        std::set<int> uniqueIndices =
            std::set<int>(tmpVec.begin(), tmpVec.end());
        for (const int& index : uniqueIndices) {
          int countOccurrences =
              std::count(tmpVec.begin(), tmpVec.end(), index);
          if (index < 0) {  // if momentum operator
            operators.push_back(
                momentumPowers[nMaxVec[i_body * n_vib_states_ + abs(index) - 1]]
                              [countOccurrences]
                                  .first
            );
#ifndef NDEBUG
...
#ifndef NDEBUG
            std::cout << "momentum registered with power" << " "
                      << countOccurrences << std::endl;
#endif
          } else if (index > 0) {  // if position operator
            operators.push_back(
                positionPowers[nMaxVec[i_body * n_vib_states_ + abs(index) - 1]]
                              [countOccurrences]
                                  .first
            );
#ifndef NDEBUG
            ...
            positions.push_back(lat.get_prop<int>("vibindex", vec_jnk));
#ifndef NDEBUG
            std::cout << "position registered with power" << " "
                      << countOccurrences << std::endl;
#endif
          }
        }
        ...
        if (!(i_body == n_particles_ - 1 && connecting == 1) && flag == 0) {
          ...
          maquis::cout << "created term for monomer " << i_body
                       << " and integral file line " << idx << std::endl;
#endif
        }
        flag = 0;
      }
      if (check_maxVibMode + 1 < n_vib_states_)
        throw std::runtime_error(
            "more vibronic_num_vibmodes than modes in FCIDUMP file"
        );
    }
    ...
    for (int i1_body = 0; i1_body < n_particles_; i1_body++) {
      for (int i2_body = 0; i2_body < n_particles_; i2_body++) {
        if ((only_nn_ && (i1_body - i2_body == 1 || i2_body - i1_body == 1)) ||
            (!only_nn_ && i1_body != i2_body)) {
          std::vector<tag_type> operators;
          std::vector<pos_t> positions;
          vec_jnk[0] = i1_body;
          vec_jnk[1] = 0;
          ...
          modelHelper<Matrix, U1>::add_term(
              positions, operators, J_, tag_handler, this->terms_
          );
        }
      }
    }
    // On-site term (for now not used, because we have only one electronic
    // state, but this would be crucial when we have more than a single excited
    // state).
    for (int i1_body = 0; i1_body < n_particles_; i1_body++) {
      vec_jnk[0] = i1_body;
      std::vector<tag_type> operators;
      ...
      operators.push_back(count_ele);
      modelHelper<Matrix, U1>::add_term(
          positions, operators, epsilon_, tag_handler, this->terms_
      );
    }
  }
  ...
  measurements_type measurements() const {
      ...
      for (std::size_t idx = 0; idx < n_particles_; idx++) {
...
#ifndef NDEBUG
        maquis::cout << "pos of pushed back positions for meas. of population: "
                     << (n_vib_states_ + n_ele_states_) * idx << std::endl;
#endif
        // Generates vector for the fillings and identity operators
        op_vec identities_local, fillings_local;
        for (std::size_t idx1 = 0; idx1 < num_vibtypes + n_ele_states_;
             idx1++) {
        ...
        for (std::size_t idx1 = 0; idx1 < num_vibtypes; idx1++) {
          local_op_vec.push_back(
              tag_handler->get_op(ident_vib.at(nMaxVec[idx1]).first)
          );
        }
    ...
    if (model.is_set("MEASURE[Displacement]")) {
      ...
      for (std::size_t idx = 0; idx < n_particles_;
           idx++) {  // loop over monomers
        for (std::size_t idx1 = 0; idx1 < n_vib_states_;
             idx1++) {  // loop over vibrational modes
          // for(std::size_t idx1 = 0; idx_1 < (n_ele_states_+n_vib_states))
          // //DEBUG if non existing lattice site: break. IMPORTANT: assumes
          // underlying lattice sorting -> may be problematic Assumed lattice
          // sorting: intertwined with all connecting modes active. After an
          // electronic site first come the local modes followed by the
          // connecting modes
          if ((idx == n_particles_ - 1) &&
              (idx1 >= (n_vib_states_ - n_connectingmodes)))
            break;
          std::string name = "Displacement" + std::to_string(idx) + "Mode" +
                             std::to_string(idx1);
          std::vector<pos_t> pos_internal(0);
          std::vector<std::vector<pos_t>> pos_local(0);
          pos_internal.push_back(
              (idx1 + 1) + (idx * (n_vib_states_ + n_ele_states_))
          );  // pushes back positions of vibrational states
          pos_local.push_back(pos_internal);
          // Generates vector for the fillings and identity operators
          op_vec identities_local, fillings_local;
          ...
          for (std::size_t idx2 = 0; idx2 < num_vibtypes; idx2++) {
            if (typeCount == idx2) {
              auto localOperator = tag_handler->get_op(
                  positionPowers.at(nMaxVec[typeCount])[1].first
              );
