...
class ExcitonicNmode : public model_impl<Matrix, U1> {
 public:
  ...
  ExcitonicNmode(const Lattice& lat_, BaseParameters& model_)
      : lat(lat_),
        model(model_),
        L_(model["L"]),
        /*tag_handler(new table_type()),*/ n_ele_states_(
            model["vibronic_num_elestates"]
        ),
    ...
    op_t ident_ele_op, create_ele_op, destroy_ele_op, count_ele_op,
        count_ele_op_gs;  // Electronic operators:
    ...
    phys_indexes.resize(
        num_vibtypes + 1
    );  // currently only one electronic state possible
    ...
    count_ele_op.insert_block(Matrix(1, 1, 1), 1, 1);  // count for excited
                                                       // state
    count_ele_op_gs.insert_block(
        Matrix(1, 1, 1), 0, 0
    );  // count for ground state
    ...
    std::cout << "registered electronic identity with tag " << ident_ele
              << std::endl;
    create_ele = tag_handler->register_op(create_ele_op, tag_detail::bosonic);
    std::cout << "registered electronic creator with tag " << create_ele
              << std::endl;
    destroy_ele = tag_handler->register_op(destroy_ele_op, tag_detail::bosonic);
    std::cout << "registered electronic destroyer with tag " << destroy_ele
              << std::endl;
    count_ele = tag_handler->register_op(count_ele_op, tag_detail::bosonic);
    std::cout << "registered electronic count op with tag " << count_ele
              << std::endl;
    count_ele_gs =
        tag_handler->register_op(count_ele_op_gs, tag_detail::bosonic);
    std::cout << "registered electronic ground state count op with tag "
              << count_ele_gs << std::endl;
    ...
    for (const auto& nModals_idx : nModalsUnique) {
      ...
      Matrix mident(overallDimension, overallDimension, 0.),
          mcount(overallDimension, overallDimension, 0.);
      ...
      for (int n = 0; n < overallDimension; n++) {
        for (int m = 0; m < overallDimension; m++) {
          ...
          mpaired(n, m) = 1;
          std::cout
              << "created a paired operator with the entry 1 in row index " << n
              << " and column index " << m << std::endl;
          mpairedVec.push_back(mpaired);
        }
        mident(n, n) = 1.;
        if (n != 0)
          mcount(n, n) =
              value_type(n);  // same count operator as in Watson Model
      }
      // DEBUG
      std::cout << "printing operators" << std::endl;
      std::cout << "identity " << mident << std::endl;
      std::cout << "count " << mcount << std::endl;
      for (const auto& iEl : mpairedVec)
        std::cout << "paired operator " << iEl << std::endl;
      // local operators
  ...
  void create_terms() override {
    ...
    for (int i_body = 0; i_body < n_particles_;
      ...
      for (int idx = 0; idx < hamiltonianTerms.first.size();
           idx++) {  // loop over rows of integral file
        std::cout << "iteration of " << idx << "th row of integral file"
                  << std::endl;
        int connecting = -1;
        int ele_state = -1;
        if (model["vibronic_max_coupling_nmode"].as<int>() != 3) {
          auto it = std::find(
              hamiltonianTerms.first[idx].begin(),
              hamiltonianTerms.first[idx].end(), -1
          );
          if (it != hamiltonianTerms.first[idx].end()) {
            // Calculate the index by subtracting begin() iterator from the
            // found iterator
            int maxIndex =
                std::distance(hamiltonianTerms.first[idx].begin(), it);
            ...
            throw std::runtime_error(
                "Problem with returned operator array from integral parser"
            );
          }
        } else {
          ele_state =
              hamiltonianTerms.first[idx][hamiltonianTerms.first.size() - 1];
          std::cout << "ele_state is " << ele_state << std::endl;
          connecting =
              hamiltonianTerms.first[idx][hamiltonianTerms.first.size() - 2];
          std::cout << "connecting variable is " << connecting << std::endl;
        }
        if (connecting == -1 || ele_state == -1)
          throw std::runtime_error(
              "Error reading variable <<connecting>> and/or <<ele_state>>"
          );
        auto matrixElement =
            static_cast<value_type>(hamiltonianTerms.second[idx]);
        ...
        bool skip = false;
        for (int i = 0; i < hamiltonianTerms.first[idx].size();
             i++) {  // loop over coupled modes and modals in a single integral
                     // line
          if (hamiltonianTerms.first[idx][i] == -1 ||
              hamiltonianTerms.first[idx][i + 2] == -1)
            break;
          else if (i % 2 == 0)
            modes.push_back(
                hamiltonianTerms.first[idx][i] - 1
            );  //-1 so that mode index starts at zero
          else
            modals.push_back(hamiltonianTerms.first[idx][i]);
        }
        for (int i = 0; i < modes.size(); i += 2) {
          ...
          for (const auto& element : nModalsUnique) {
            ...
            if (element < localDimension) {
              ...
              sumSquaredSmaller +=
                  squaredElement;  // Add the squared element to the sum
            }
          }
          if (modalToCreate >= localDimension ||
              modalToDestroy >= localDimension)
            skip = true;
          int indexOp = (sumSquaredSmaller - 1) +
                        modalToCreate * (localDimension) + modalToDestroy;
          ...
          std::cout << "pushed back paired operator with tag " << operators[0]
                    << " at position " << positions[0] << std::endl;
        }
        if (skip) continue;
  ...
  tag_type identity_matrix_tag(size_t type) const {
    ...
    if (type != 0) {
      ...
      );  // extract index of entry dimension in set
  ...
  measurements_type measurements() const {
    ...
    if (model.is_set("MEASURE[ModeExcitationDegree]")) {
      int typeCount = 1;
      for (std::size_t iBody = 0; iBody < n_particles_;
           iBody++) {  // loop over monomers
        for (std::size_t iMode = 0; iMode < n_vib_states_;
             iMode++) {  // loop over vibrational modes
          if ((iBody == n_particles_ - 1) &&
              (iMode >= (n_vib_states_ - n_connectingmodes)))
            break;
          std::string name = "Monomer" + std::to_string(iBody) +
                             "ExcitationMode" + std::to_string(iMode);
          // Generates vectors for the positions
          std::vector<pos_t> pos_internal(0);
          std::vector<std::vector<pos_t>> pos_local(0);
          pos_internal.push_back(
              (iMode + 1) + (iBody * (n_vib_states_ + n_ele_states_))
          );
          pos_local.push_back(pos_internal);
          ...
          for (std::size_t idx = 1; idx <= num_vibtypes; idx++) {
            if (typeCount == idx) {
              auto localOperator = tag_handler->get_op(count_matrix_tag(idx)
              );  // TODO get correct operator
              local_op_vec.push_back(localOperator);
            } else
              local_op_vec.push_back(tag_handler->get_op(identity_matrix_tag(idx
              )));
          }
          typeCount++;
          ops.push_back(std::make_pair(local_op_vec, false));
 ...
 private:
  ...
  int num_vibtypes;
  // operators
  operators_type ident, count, create, destroy, paired;  // vibrational
                                                         // operators
  tag_type ident_ele, count_ele, count_ele_gs, create_ele,
      destroy_ele;  // electronic operators
};
