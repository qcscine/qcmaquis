 ...
 public:
        ...
        phys_indexes(0)  // check whether everything needed is here
  {
    ...
    if (nModalsUnique.size() == 1) {
      ...
      create = modelHelper<Matrix, TrivialGroup>::register_all_types(
          create_op, tag_detail::bosonic, tag_handler
      );
      destroy = modelHelper<Matrix, TrivialGroup>::register_all_types(
          destroy_op, tag_detail::bosonic, tag_handler
      );
      // Registers the hermitian pairs
      modelHelper<Matrix, TrivialGroup>::registerHermitianConjugates(
