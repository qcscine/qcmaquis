...
class WatsonHamiltonian : public model_impl<Matrix, TrivialGroup> {
 ...
 public:
   ...
   */
  WatsonHamiltonian(
      const Lattice& lattice, BaseParameters& parameters, bool verbose
  )
      : lattice_(lattice),
        parameters_(parameters),
        tag_handler_(new table_type()),
        physIndices_(0) {
    // Model parameters
    ...
    if (parameters_["watson_coordinate_type"] == "cartesian") {
      ...
      maquis::cout
          << " Coordinate type: Internal coordinates-based Normal Modes"
          << std::endl;
    } else {
      throw std::runtime_error("Coordinate type not recognized");
    }
    // Set the number of indices which are expected in the FCIDUMP
    // Determines also the maximum many-body coupling degree. Per default read
    // in all integrals that are given
    maxCoupling_ = chem::getIndexDim(
        chem::Hamiltonian::VibrationalCanonical,
        chem::HamiltonianTransformation::Conventional
    );
    maxManyBodyCoupling_ = (parameters.is_set("watson_max_coupling"))
                               ? parameters["watson_max_coupling"]
                               : maxCoupling_;
    maxInputManyBodyCoupling_ = (parameters.is_set("watson_max_coupling_input"))
                                    ? parameters["watson_max_coupling_input"]
                                    : maxCoupling_;
    maquis::cout << " - Maximum many-body coupling order supported: "
                 << maxCoupling_ << std::endl;
    maquis::cout << " - Many-body coupling order expected as input: "
                 << maxInputManyBodyCoupling_ << std::endl;
    maquis::cout
        << " - Maximum many-body coupling order included in the Hamiltonian "
        << maxManyBodyCoupling_ << std::endl;
    maquis::cout << std::endl;
    ...
    if (nMaxVec.size() == 1) {
      ...
      throw std::runtime_error(
          "Nmax needs to be either a single integer or a list with lenght L"
      );
    }
    ...
    for (const auto& nMax : nMaxUnique) {
      ...
      powersOfPositions_op = VibrationalHelpers<Matrix, TrivialGroup>::
          generatePowersOfPositionOperator(
              maxInputManyBodyCoupling_, nMax, ident_op, position_op
          );
      powersOfMomentum_op = VibrationalHelpers<Matrix, TrivialGroup>::
          generatePowersOfMomentumOperator(
              maxInputManyBodyCoupling_, nMax, ident_op, momentum_op
          );
      ...
      numberOperators_[nMax] =
          tag_handler_->register_op(count_op, tag_detail::bosonic);
      ...
      for (int iOrder = 1; iOrder <= maxInputManyBodyCoupling_; iOrder++) {
        positionPowers_[nMax][iOrder] = tag_handler_->register_op(
            powersOfPositions_op[iOrder], tag_detail::bosonic
        );
        momentumPowers_[nMax][iOrder] = tag_handler_->register_op(
  ...
  void create_terms() override {
    auto hamiltonianTerms =
        Vibrational::detail::WatsonIntegralParser<value_type>(
            parameters_, lattice_, coordinateType_, maxCoupling_,
    ...
    for (const auto& iTerms : hamiltonianTerms) {
      ...
      auto termVector =
          std::vector<int>(iTerms.first.begin(), iTerms.first.end());
      ...
      while (outerCounter < numberOfNonZeroElements) {
        ...
        int innerCounter = 0;
        while (termVector[outerCounter + innerCounter] == referenceValue &&
               innerCounter + outerCounter != numberOfNonZeroElements)
          innerCounter += 1;
  ...
  Index<TrivialGroup> const& phys_dim(size_t type) const override {
    return physIndices_[type];
  }
  ...
  tag_type filling_matrix_tag(size_t type) const override {
      ...
      for (int iSite = 0; iSite < lattice_.size(); iSite++) {
        ...
        for (int iType = 0; iType < lattice_.getMaxType(); iType++)
          local_op_vec.push_back(
              tag_handler_->get_op(numberOperators_.at(nMaxVec[iType]))
          );
        ops.push_back(std::make_pair(local_op_vec, false));
        meas.push_back(new measurements::local_at<Matrix, TrivialGroup>(
            name, lattice_, pos_local, identities_local, fillings_local, ops
        ));
      }
    }
    return meas;
 ...
 private:
  ...
  std::unordered_map<int, std::vector<tag_type>> positionPowers_,
      momentumPowers_;
