...
class VibrationalRDM : public measurement<Matrix, NU1_template<N>> {
 public:
   ...
   * @param op: vector of the tags associated with the terms that are to be
   * added up to the measurement.
   * @param coeffs: scalar coefficients for the operator.
   * @param tagger: tag_handler associated with the model.
   * @param excludeSameSite: if true, excludes elements of the RDM sitting on
   * the same site.
   */
  VibrationalRDM(
      Lattice const& lat_, std::string name_,
      const std::vector<std::vector<operators_type>>& op,
      const std::vector<float_t>& coeffs,
      const std::shared_ptr<TagHandler<Matrix, SymmGroup>>& tagger,
    ...
    for (const auto& iOp : op) assert(iOp.size() == op[0].size());
    ...
    for (int iTerm = 0; iTerm < overallCombinations; iTerm++) {
      ...
      auto newIntegerRepresentation =
          this->to_base(iTerm, L, numberOfSQOperators);
      assert(newIntegerRepresentation.size() <= numberOfSQOperators);
      auto setSize =
          std::set<int>(
              newIntegerRepresentation.begin(), newIntegerRepresentation.end()
          )
              .size();
      bool accept = (modality == VibrationalRDMModality::AddAll) ||
                    (modality == VibrationalRDMModality::ExcludeAllSame &&
                     setSize != 1) ||
                    (modality == VibrationalRDMModality::ExcludeSame &&
                     setSize == newIntegerRepresentation.size());
      if (accept) {
        ...
        for (int k = 0; k < coeffs.size(); ++k) {
          ...
          auto term = modelHelper<Matrix, SymmGroup>::arrange_operators(
              positions, operators, scaling, tagger
          );
          term.first.coeff = scaling;
          assert(!term.second);
          terms_.push_back(term.first);
        }
        auto mpoMaker = generate_mpo::TaggedMPOMaker<Matrix, SymmGroup>(
            lat_, ident, ident, ident, tagger, terms_
        );
  ...
  void evaluate(
    ...
    /*
    using contr = contraction::Engine<Matrix, typename
    storage::constrained<Matrix>::type, SymmGroup>;
    ...
    for (int k = 0; k < leftBoundary[0].n_blocks(); ++k)
        for (int iRow = 0; iRow < num_rows(leftBoundary[0][k]); iRow++)
            for (int iCol = 0; iCol < num_cols(leftBoundary[0][k]); iCol++)
                leftBoundary[0][k](iRow, iCol) = (iRow == iCol) ?
    static_cast<value_type>(1.) : static_cast<value_type>(0.);
    // Contraction
    leftBoundaryNextSite = contr::overlap_mpo_left_step(mps[site_], mps[site_],
    leftBoundary, mpo[site_]); assert(leftBoundaryNextSite.aux_dim() == 1);
  ...
  measurement<Matrix, SymmGroup>* do_clone() const {
    return new VibrationalRDM(*this);
  }

 private:
  /** @brief Small helper function to convert a decimal number n into an
   * arbitrary base b */
