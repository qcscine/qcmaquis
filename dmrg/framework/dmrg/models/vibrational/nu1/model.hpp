...
class NMode : public model_impl<Matrix, NU1> {
    // Types definition
...
public:
     ...
     */
    NMode(const Lattice& lattice_, const BaseParameters& parameters_, bool verbose)
        ...
        ident = modelHelper<Matrix, NU1>::register_all_types(ident_op, tag_detail::bosonic);
        create = modelHelper<Matrix, NU1>::register_all_types(create_op, tag_detail::bosonic);
        destroy = modelHelper<Matrix, NU1>::register_all_types(destroy_op, tag_detail::bosonic);
        count = modelHelper<Matrix, NU1>::register_all_types(count_op, tag_detail::bosonic);
        //
        ...
        while (getline(orb_file, line_string)) {
          ...
          operators.resize(0);
          // Extract the data 
          auto Hamiltonian_term = VibrationalIntegralParser::<value_type>(model, lat, line_string);
          if (Hamiltonian_term.first.size() == 0 || std::fabs(Hamiltonian_term.second) < model["integral_cutoff"])
            continue;
          convertLineToOperators(Hamiltonian_term, positions, operators;
          modelHelper<Matrix, NU1>::add_term(positions, operators, Hamiltonian_term.second, verbose);
        }
    }
    ...
    typename NU1::charge total_quantum_numbers(BaseParameters &parms) const {
        return NU1::charge(std::vector<int>(n_modes, 1));
    }
...
private:
    ...
    void convertLineToOperators(const H_terms_type& ham_term, positions_type& pos, operators_type& ops)
    {
        ...
        for (std::size_t jj = 0; jj < nmode_order; jj++) {
           ...
           std::size_t offset = lat.get_basis_previous(ham_term.first[2*jj]-1);
           std::size_t index  = ham_term.first[2*jj+1] + offset;
           assert ( index < lattice_size );
           int i_type = lat.get_prop<int>("type", index);
           //
           pos.push_back(index) ;
           if (jj % 2 == 0)
