...
class modelHelper {
 ...
 public:
   ...
   * @return std::pair< term_descriptor, bool > First element is the tag, second
   * element is true if the term to be added is zero.
   */
  static std::pair<term_descriptor, bool> arrange_operators(
      const positions_type& positions, const operators_type& operators,
      value_type& scaling,
  ...
  static void add_term(
      positions_type const& positions, operators_type const& operators,
      value_type const& coeff,
      const std::shared_ptr<TagHandler<Matrix, SymmGroup>> tag_handler,
      terms_type& terms, bool verbose = false
  ) {
    value_type scaling = 1.;
    std::pair<term_descriptor, bool> ret =
        modelHelper<Matrix, SymmGroup>::arrange_operators(
            positions, operators, scaling, tag_handler
        );
    if (!ret.second) {
      auto term = ret.first;
   ...
   */
  static std::vector<tag_type> register_all_types(
      const std::vector<op_t>& ops, tag_detail::operator_kind kind,
      std::shared_ptr<TagHandler<Matrix, SymmGroup>> tag_handler
  ) {
    std::vector<tag_type> ret;
    for (std::size_t idx = 0; idx < ops.size(); idx++) {
      std::pair<tag_type, value_type> newtag =
          tag_handler->checked_register(ops[idx], kind);
   ...
   *
   * Note that this call implicitly changes the internal structure of the
   * tag_handler.
   *
   ...
   * @return std::vector<tag_type> vector of the tags associated with ops
   */
  static void registerHermitianConjugates(
      const std::vector<tag_type>& ops, const std::vector<tag_type>& hermOps,
      std::shared_ptr<TagHandler<Matrix, SymmGroup>> tag_handler
  ) {
   ...
   *
   * Note that this method assumes that, if an operator is registered in the
   * tag_handler, also the hermitian conjugate is. The method raises an
   * assertion exception otherwise.
   *
   * @param tag Tag for which the hermitian conjugate should be registered
   * @param tag_handler std::shared_ptr<TagHandler> pointer carrying the
   * operator table.
   */
  static void registerNewHermitianConjugate(
      const tag_type& tag,
      std::shared_ptr<TagHandler<Matrix, SymmGroup>> tag_handler
  ) {
    auto operatorType = (tag_handler->is_fermionic(tag)) ? tag_detail::fermionic
                                                         : tag_detail::bosonic;
    ...
    // If already present, it may be either the same or they are already in the
    // table
    if (alreadyPresent) {
      auto newTag =
          tag_handler->checked_register(hermitianOperator, operatorType);
      assert(
          newTag.first == tag || tag_handler->herm_conj(tag) == newTag.first
      );
    } else {
