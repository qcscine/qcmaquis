...
struct deas_mps_init : public mps_initializer<Matrix, SymmGroup> {
  ...
  deas_mps_init(
      DmrgParameters parms_, const Matrix &s1_,
      std::vector<Index<SymmGroup>> const &phys_dims_,
      typename SymmGroup::charge right_end_, std::vector<int> const &site_type
  )
    ...
    if (!parms.is_set("orbital_order"))
      for (int p = 0; p < hf_unordered.size(); ++p) order[p] = p + 1;
    else
      order = parms["orbital_order"].template as<std::vector<int>>();

    std::transform(order.begin(), order.end(), order.begin(), [](const int e) {
      return e - 1;
    });
    std::vector<int> inv_order(L);
    for (int p = 0; p < order.size(); ++p)
      inv_order[p] = std::distance(
          order.begin(), std::find(order.begin(), order.end(), p)
      );

    std::copy(
        order.begin(), order.end(), std::ostream_iterator<int>(std::cout, " ")
    );
    maquis::cout << std::endl;

    for (int i : order) {
      hf_occ.push_back(hf_unordered[i]);
    }

    std::cout << "hf_occ: ";
    std::copy(
        hf_occ.begin(), hf_occ.end(), std::ostream_iterator<int>(std::cout, " ")
    );
    maquis::cout << std::endl;
    ...
    for (int i = 0; i < L; i++)
      maquis::cout << casv_sort[i].first << " " << casv_sort[i].second
                   << std::endl;
    ...
    std::cout << "CAS vector: ";
    for (int i = 0; i < L; i++) std::cout << cas_vector[i] << " ";
    std::cout << std::endl;
  }
  ...
  void operator()(MPS<Matrix, SymmGroup> &mps) {
    ...
    charge doubly_occ = phys_dims[0].begin()->first,
           empty = phys_dims[0].rbegin()->first;
    ...
    // another loop has to be inserted here where determinants are generated
    // until m is reached at some site; correct numbers have to be checked here
...
#pragma omp critical
      ...
      determinants = get_charge_determinants(
          new_det_list, det_list_new, phys_dims, site_types, right_end
      );
      ...
      for (int d = 0; d < determinants.size(); ++d) {
        ...
        for (int s = L - 1; s > 0; --s) {
          ...
          accumulated_charge =
              SymmGroup::fuse(accumulated_charge, -site_charge);
          if (ChargeDetailClass<SymmGroup>::physical(accumulated_charge)) {
            ...
            std::map<std::string, int> &str_map =
                str_to_col_map[s - 1][accumulated_charge];
    ...
    for (int i = 0; i < L; ++i) std::cout << sum_size[i] << " ";
    std::cout << std::endl;

    init_sect(mps, str_to_col_map, true, 0);

    // this here is absolutely necessary
    for (pos_t i = 0; i < L; ++i) mps[i].multiply_by_scalar(0.0);
    ...
    for (int d = 0; d < total_dets.size(); ++d) {
      ...
      for (int s = L - 1; s > 0; --s) {
        ...
        charge search_charge =
            SymmGroup::fuse(accumulated_charge, -site_charge);
        if (ChargeDetailClass<SymmGroup>::physical(search_charge) &&
            mps[s].row_dim().has(search_charge)) {
          int nrows_fill = mps[s].row_dim().size_of_block(search_charge);
          // get current matrix
          size_t max_pos =
              mps[s].data().left_basis().position(accumulated_charge);
          ...
          if (site_charge == phys_dims[site_types[s]][1].first) {
            if (mps[s].row_dim().has(
                    SymmGroup::fuse(accumulated_charge, -doubly_occ)
                ))
              off = mps[s].row_dim().size_of_block(
                  SymmGroup::fuse(accumulated_charge, -doubly_occ)
              );
          } else if (site_charge == phys_dims[site_types[s]][2].first) {
            if (mps[s].row_dim().has(accumulated_charge))
              off = nrows - nrows_fill -
                    mps[s].row_dim().size_of_block(accumulated_charge);
            else
              off = nrows - nrows_fill;
  ...
  void init_sect(
      ...
      const std::vector<std::map<charge, std::map<std::string, int>>>
          &str_to_col_map,
      bool fillrand = true, typename Matrix::value_type val = 0
  ) {
    parallel::scheduler_balanced scheduler(mps.length());
    std::size_t L = mps.length();

    std::vector<Index<SymmGroup>> allowed =
        allowed_sectors(site_types, phys_dims, right_end, 5);
    ...
    omp_for(size_t i, parallel::range<size_t>(0, L), {
      ...
      mps[i] = MPSTensor<Matrix, SymmGroup>(
          phys_dims[site_types[i]], allowed[i], allowed[i + 1], fillrand, val
      );
      mps[i].divide_by_scalar(mps[i].scalar_norm());
    });
  }

  std::vector<Index<SymmGroup>> adapt_allowed(
      std::vector<Index<SymmGroup>> allowed,
      const std::vector<std::map<charge, std::map<std::string, int>>>
          &str_to_col_map
  ) {
 ...
 private:
  ...
  Determinant<SymmGroup> str_from_det(
      std::vector<typename SymmGroup::charge> const &charge_vec,
      std::vector<Index<SymmGroup>> const &phys_dims,
      std::vector<typename SymmGroup::subcharge> const &site_types
  ) {
  ...
  std::vector<std::vector<typename SymmGroup::charge>> get_charge_determinants(
      std::vector<Determinant<SymmGroup>> const &det_list,
    ...
    for (int i = 0; i < dummy_dets.size(); ++i) {
      ...
      for (int j = 0; j < det_list[i].size(); ++j) {
        if (dummy_dets[i][j].size() != 1) {
          ...
          for (int k = 0; k < times; k++) single_det.push_back(single_det[k]);

          for (int k = 0; k < single_det.size(); ++k) {
            if (k < single_det.size() / 2)  // works only if there are two
                                            // options, like (1,1) and (1,-1)
      ...
      for (int m = 0; m < single_det.size(); ++m) {
        ...
        for (int l = 1; l < L; ++l) {
          accumulated_charge =
              SymmGroup::fuse(accumulated_charge, single_det[m][l]);
          if (!ChargeDetailClass<SymmGroup>::physical(accumulated_charge))
            valid = false;
        }
        ...
        if (valid && accumulated_charge == right_end &&
            std::find(
                determinants.begin(), determinants.end(), single_det[m]
            ) == determinants.end()) {
#pragma omp critical
          {
            determinants.push_back(single_det[m]);
            Determinant<SymmGroup> det_str =
                str_from_det(single_det[m], phys_dims, site_types);
