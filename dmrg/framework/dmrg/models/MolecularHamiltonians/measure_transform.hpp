...
struct measure_transform<
  ...
  void operator()(
      std::string rfile, std::string result_path, Lattice lat,
    ...
    //     prepare_measurements(lat, mps, parms_chementropy);
    // }
    // else
    {
      std::tie(mps_tmp, transformed_measurements) =
    ...
    std::for_each(
        transformed_measurements.begin(), transformed_measurements.end(),
  ...
  results_map_type meas_out(
      Lattice lat, MPS<Matrix, SymmGroup> const& mps,
      BaseParameters const& measurement_parms = BaseParameters(),
      const std::string& rfile = "", const std::string& result_path = ""
  ) {
 ...
 private:
  ...
  std::pair<
      MPS<Matrix, SymmOut>, typename Model<Matrix, SymmOut>::measurements_type>
  prepare_measurements(
    ...
    BaseParameters parms_tmp = chem::detail::set_2u1_parameters(
        mps.size(), Nup, Ndown, measurement_parms
    );
    ...
    return std::make_pair(
        transform_mps<Matrix, SymmGroup>()(mps, Nup, Ndown),
        model_tmp.measurements()
    );
  }
  ...
  // Not ready yet
  /*
  void operator()(std::vector<std::string> rfiles, std::string result_path,
  Lattice lat, const std::vector<MPS<Matrix,SymmGroup > > & mps_vec)
  {
      typedef typename boost::mpl::if_<symm_traits::HasPG<SymmGroup>, TwoU1PG,
  TwoU1>::type SymmOut;
      ...
      BaseParameters parms_tmp = chem::detail::set_2u1_parameters(mps.size(),
  Nup, Ndown); parms_tmp.set("MEASURE[ChemEntropy]", 1);

      Model<Matrix, SymmOut> model_tmp(lat, parms_tmp);

      std::vector<MPS<Matrix,SymmGroup> > mps_out;

      std::transform(mps_vec.begin(), mps_vec.end(), mps_out.begin(), [&](const
  MPS<Matrix,SymmGroup> & m) { return transform_mps<Matrix, SymmGroup>()(m, Nup,
  Ndown); });
