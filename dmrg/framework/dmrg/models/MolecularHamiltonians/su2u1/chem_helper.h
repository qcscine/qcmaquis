...
class ChemHelperSU2 {
 public:
  ...
  ChemHelperSU2(
    ...
    boost::tie(idx_, matrix_elements) =
        parse_integrals<InputType, SymmGroup, chem::Hamiltonian::Electronic>(
            parms, lat
        );
  ...
  void add_4term(std::vector<term_descriptor>& tagterms, term_descriptor term) {
    using EightTuple = IndexTuple<SymmGroup, 8>;
    IndexTuple<SymmGroup, 4> pos(std::array<int, 4>(
        {term.position(0), term.position(1), term.position(2), term.position(3)}
    ));
    IndexTuple<SymmGroup, 4> ops(std::array<int, 4>(
        {static_cast<int>(term.operator_tag(0)),
         static_cast<int>(term.operator_tag(1)),
         static_cast<int>(term.operator_tag(2)),
         static_cast<int>(term.operator_tag(3))}
    ));
