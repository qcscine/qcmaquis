...
struct TermMakerSU2 {
  ...
  struct Operators {
    tag_vec create_fill, create, destroy_fill, destroy, create_fill_couple_down,
        destroy_fill_couple_down, create_couple_up, destroy_couple_up,
        create_fill_count, create_count, destroy_fill_count, destroy_count,
        count, docc, e2d, d2e, flip_S0, flip_to_S2, flip_to_S0, ident,
        ident_full, fill, count_fill;
  };
  ...
  static std::vector<op_t> generate_site_specific_ops(
      op_t const& op, typename S::subcharge max_irrep
  ) {
  ...
  static tag_vec register_site_specific(
      std::vector<op_t> const& ops, tag_detail::operator_kind kind,
      typename S::subcharge max_irrep, const tag_handler_t& tag_handler
  ) {
    std::vector<tag_type> ret;
    for (typename S::subcharge sc = 0; sc < max_irrep + 1; ++sc) {
      std::pair<tag_type, value_type> newtag =
          tag_handler->checked_register(ops[sc], kind);
  ...
  static Operators construct_operators(
      typename S::subcharge max_irrep, const tag_handler_t& tag_handler
  ) {
    Operators ret;
...
#define GENERATE_SITE_SPECIFIC(opname) \
  std::vector<op_t> opname##s = generate_site_specific_ops(opname, max_irrep);
...
#define REGISTER(op, kind) \
  ret.op = register_site_specific(op##_ops, kind, max_irrep, tag_handler);
...
#define HERMITIAN(op1, op2)                   \
  for (int hh = 0; hh < ret.op1.size(); ++hh) \
    tag_handler->hermitian_pair(ret.op1[hh], ret.op2[hh]);
...
#undef HERMITIAN
  ...
  static OperatorCollection construct_operator_collection(
      Operators const& ops, typename S::subcharge max_irrep
  ) {
  ...
  static term_descriptor two_term(
      bool sign, tag_vec full_ident, value_type scale, pos_t i, pos_t j,
  ...
  static term_descriptor positional_two_term(
    ...
    term.push_back(
        std::make_pair(start, op1_use[lat.get_prop<sc>("type", start)])
    );
  ...
  static term_descriptor three_term(
      tag_vec full_ident, value_type scale, pos_t pb, pos_t p1, pos_t p2,
      tag_vec boson_op, tag_vec boson_op_fill, tag_vec op1, tag_vec op1_fill,
      tag_vec op2, tag_vec op2_fill, Lattice const& lat
  ) {
    term_descriptor term;
    ...
    term.push_back(
        std::make_pair(pb, boson_op_use[lat.get_prop<sc>("type", pb)])
    );
    term.push_back(
        std::make_pair(start, op1_use[lat.get_prop<sc>("type", start)])
    );
    ...
    // maquis::cout << "  term " << pb << p1 << p2 << "   " <<
    // boson_op_use[lat.get_prop<sc>("type", pb)] << "," <<
    // op1_use[lat.get_prop<sc>("type", start)] << "," <<
    // op2_use[lat.get_prop<sc>("type", end)] << std::endl;
  ...
  static term_descriptor four_term(
      tag_vec full_ident, int max_two_S, value_type scale, pos_t i, pos_t j,
      pos_t k, pos_t l, OperatorBundle op_i, OperatorBundle op_k,
      Lattice const& lat
  ) {
    using std::get;
    ...
    if (max_two_S == 2) {
      term.push_back(std::make_pair(
          get<0>(sterm[0]),
          get<1>(sterm[0])
              .fill_couple_up[lat.get_prop<sc>("type", get<0>(sterm[0]))]
      ));
      term.push_back(std::make_pair(
          get<0>(sterm[1]),
          get<1>(sterm[1]).couple_up[lat.get_prop<sc>("type", get<0>(sterm[1]))]
      ));
      term.push_back(std::make_pair(
          get<0>(sterm[2]),
          get<1>(sterm[2])
              .fill_couple_down[lat.get_prop<sc>("type", get<0>(sterm[2]))]
      ));
      term.push_back(std::make_pair(
          get<0>(sterm[3]),
          get<1>(sterm[3])
              .couple_down[lat.get_prop<sc>("type", get<0>(sterm[3]))]
      ));
    } else {
      term.push_back(std::make_pair(
          get<0>(sterm[0]),
      ...
      term.push_back(std::make_pair(
          get<0>(sterm[1]),
          get<1>(sterm[1])
              .couple_down[lat.get_prop<sc>("type", get<0>(sterm[1]))]
      ));
      term.push_back(std::make_pair(
          get<0>(sterm[2]),
          get<1>(sterm[2])
              .fill_couple_up[lat.get_prop<sc>("type", get<0>(sterm[2]))]
      ));
      term.push_back(std::make_pair(
          get<0>(sterm[3]),
          get<1>(sterm[3])
...
class SpinSumSU2 {
 public:
  ...
  static std::vector<term_descriptor> V_term(
      value_type matrix_element, pos_t i, pos_t k, pos_t l, pos_t j,
    ...
    switch (n_unique) {
      ...
      case 1:
        term_descriptor term;
        term.coeff = value_type(2.) *
                     matrix_element;  // 2 spin combinations are non-zero
        term.push_back(std::make_pair(
            i,
            ops.docc.no_couple[lat.get_prop<typename S::subcharge>("type", i)]
        ));
  ...
  static std::vector<term_descriptor> two_term(
      value_type matrix_element, pos_t i, pos_t k, pos_t l, pos_t j,
      OperatorCollection const& ops, Lattice const& lat
  ) {
    if (i == j && k == l && j != k) {
  ...
  static std::vector<term_descriptor> three_term(
      value_type matrix_element, pos_t i, pos_t k, pos_t l, pos_t j,
      OperatorCollection const& ops, Lattice const& lat
  ) {
    if (i == j || k == l) {
  ...
  static std::vector<term_descriptor> four_term(
      value_type matrix_element, pos_t i, pos_t k, pos_t l, pos_t j,
      OperatorCollection const& ops, Lattice const& lat
  ) {
    std::vector<term_descriptor> ret;

    // These 3 cases produce different S_z spin patterns, which differ along
    // with different index permutations As in standard notation of the
    // Hamiltonian, the first two positions get a creator, the last two a
    // destructor
    ...
    if (k_ > l_ && l_ > j_) {  // eg V_4132
      // generates up|up|up|up + up|down|down|up + down|up|up|down +
      // down|down|down|down
      ret.push_back(TM::four_term(
          ops.ident_full.no_couple, 2,
          value_type(-std::sqrt(3.)) * matrix_element, i, k, l, j, ops.create,
          ops.destroy, lat
      ));
      ret.push_back(TM::four_term(
          ops.ident.no_couple, 1, matrix_element, i, k, l, j, ops.create,
          ops.destroy, lat
      ));
    } else if (k_ > j_ && j_ > l_) {  // eg V_4231
      // generates up|up|up|up + up|down|up|down + down|up|down|up +
      // down|down|down|down
      ...
      ret.push_back(TM::four_term(
          ops.ident_full.no_couple, 2,
          value_type(std::sqrt(3.)) * local_element, i, k, l, j, ops.create,
          ops.destroy, lat
      ));
      ret.push_back(TM::four_term(
          ops.ident.no_couple, 1, local_element, i, k, l, j, ops.create,
          ops.destroy, lat
      ));
    } else if (j_ > k_ && k_ > l_) {  // eg V_4321
      // generates up|up|up|up + up|up|down|down + down|down|up|up +
      // down|down|down|down
      ret.push_back(TM::four_term(
          ops.ident.no_couple, 1, value_type(2.) * matrix_element, i, k, l, j,
          ops.create, ops.destroy, lat
      ));
    } else {
 ...
 private:
  static std::vector<term_descriptor> two_termA(
      value_type matrix_element, pos_t i, pos_t k, pos_t l, pos_t j,
      OperatorCollection const& ops, Lattice const& lat
  ) {
    std::vector<term_descriptor> ret;
    ret.push_back(TM::two_term(
        false, ops.ident.no_couple, matrix_element, i, k, ops.count.no_couple,
        ops.count.no_couple, lat
    ));
    return ret;
  }

  static std::vector<term_descriptor> two_termB1(
      value_type matrix_element, pos_t i, pos_t k, pos_t l, pos_t j,
      OperatorCollection const& ops, Lattice const& lat
  ) {
    std::vector<term_descriptor> ret;
    ret.push_back(TM::two_term(
        false, ops.ident.no_couple, value_type(2.0) * matrix_element, i, j,
        ops.e2d.no_couple, ops.d2e.no_couple, lat
    ));
    return ret;
  }

  static std::vector<term_descriptor> two_termB2(
      value_type matrix_element, pos_t i, pos_t k, pos_t l, pos_t j,
      OperatorCollection const& ops, Lattice const& lat
  ) {
    std::vector<term_descriptor> ret;
    ...
    // the sqrt(3.) counteracts the Clebsch coeff C^{110}_{mrm'} which applies
    // when the spin1 couples back to spin0
    ret.push_back(TM::positional_two_term(
        false, ops.ident_full.no_couple,
        value_type(std::sqrt(3.)) * matrix_element, i, j, ops.flip.couple_down,
        ops.flip.couple_up, ops.flip.couple_down, ops.flip.couple_up, lat
    ));

    ret.push_back(TM::two_term(
        false, ops.ident.no_couple, value_type(-0.5) * matrix_element, i, j,
        ops.count.no_couple, ops.count.no_couple, lat
    ));

    return ret;
  }

  static std::vector<term_descriptor> two_termC(
      value_type matrix_element, pos_t i, pos_t k, pos_t l, pos_t j,
      OperatorCollection const& ops, Lattice const& lat
  ) {
    int s;
    ...
    if (i == k && k == l) {
      ...
      throw std::runtime_error(
          "Term generation logic has failed for V_ijjj term\n"
      );
    }
    ...
    if (i == k) {  // one lonely destructor
      ret.push_back(TM::positional_two_term(
          true, ops.ident.no_couple, value_type(std::sqrt(2.)) * matrix_element,
          s, p, ops.create_count.couple_down, ops.create_count.fill_couple_up,
          ops.destroy.couple_down, ops.destroy.fill_couple_up, lat
      ));
    } else {  // one lonely constructor
      ret.push_back(TM::positional_two_term(
          true, ops.ident.no_couple,
          value_type(-std::sqrt(2.)) * matrix_element, s, p,
          ops.destroy_count.couple_down, ops.destroy_count.fill_couple_up,
  ...
  static std::vector<term_descriptor> three_termA(
      value_type matrix_element, pos_t i, pos_t k, pos_t l, pos_t j,
      OperatorCollection const& ops, Lattice const& lat
  ) {
    std::vector<term_descriptor> ret;
    ...
    ret.push_back(TM::three_term(
        ops.ident.no_couple, value_type(std::sqrt(2.)) * matrix_element,
        same_idx, k, l, ops.count.no_couple, ops.count.fill_no_couple,
  ...
  static std::vector<term_descriptor> three_termB(
      value_type matrix_element, pos_t i, pos_t k, pos_t l, pos_t j,
      OperatorCollection const& ops, Lattice const& lat
  ) {
    std::vector<term_descriptor> ret;
    ...
    if (i == k) {
      ...
      ret.push_back(TM::three_term(
          ops.ident.no_couple, value_type(-std::sqrt(2.)) * matrix_element,
          same_idx, pos1, pos2, ops.e2d.no_couple, ops.e2d.no_couple,
    ...
    if (j == l) {
      ...
      ret.push_back(TM::three_term(
          ops.ident.no_couple, value_type(-std::sqrt(2.)) * matrix_element,
          same_idx, pos1, pos2, ops.d2e.no_couple, ops.d2e.no_couple,
          ops.create.couple_down, ops.create.fill_couple_up,
          ops.create.couple_down, ops.create.fill_couple_up, lat
      ));
    }
    ...
    if (j == k || i == l) {
      ...
      if (same_idx < std::min(pos1, pos2)) {
        ret.push_back(TM::three_term(
            ops.ident_full.no_couple,
            phase * value_type(std::sqrt(3.)) * matrix_element, same_idx, pos1,
            pos2, ops.flip.couple_up, ops.flip.couple_up,
            ops.create.couple_down, ops.create.fill_couple_down,
            ops.destroy.couple_down, ops.destroy.fill_couple_down, lat
        ));
        ret.push_back(TM::three_term(
            ops.ident.no_couple,
            value_type(-0.5 * std::sqrt(2.)) * matrix_element, same_idx, pos1,
            pos2, ops.count.no_couple, ops.count.no_couple,
            ops.create.couple_down, ops.create.fill_couple_up,
            ops.destroy.couple_down, ops.destroy.fill_couple_up, lat
        ));
        ...
        ret.push_back(TM::three_term(
            ops.ident_full.no_couple,
            phase * value_type(std::sqrt(3.)) * matrix_element, same_idx, pos1,
            pos2, ops.flip.couple_down, ops.flip.couple_down,
            ops.create.couple_up, ops.create.fill_couple_up,
            ops.destroy.couple_up, ops.destroy.fill_couple_up, lat
        ));
        ret.push_back(TM::three_term(
            ops.ident.no_couple,
            value_type(-0.5 * std::sqrt(2.)) * matrix_element, same_idx, pos1,
        ...
        ret.push_back(TM::three_term(
            ops.ident.no_couple,
            phase * value_type(std::sqrt(3.)) * matrix_element, same_idx, pos1,
            pos2, ops.flip.no_couple, ops.flip.no_couple,
            ops.create.couple_down, ops.create.fill_couple_up,
            ops.destroy.couple_down, ops.destroy.fill_couple_up, lat
        ));
        ret.push_back(TM::three_term(
            ops.ident.no_couple,
            value_type(-0.5 * std::sqrt(2.)) * matrix_element, same_idx, pos1,
            pos2, ops.count.fill_no_couple, ops.count.fill_no_couple,
            ops.create.couple_down, ops.create.fill_couple_up,
            ops.destroy.couple_down, ops.destroy.fill_couple_up, lat
        ));
      }
