...
class RelChemHelper {
 public:
  ...
  RelChemHelper(
      BaseParameters& parms, const Lattice& lat_,
      const std::vector<tag_type>& ident_, const std::vector<tag_type>& fill_,
    ...
    boost::tie(idx_, matrix_elements) = parse_integrals<
        InputType, SymmGroup, chem::Hamiltonian::RelativisticElectronic>(
        parms, lat
    );
  ...
  void commit_terms(std::vector<term_descriptor>& tagterms) {
    for (const auto& it : two_terms) tagterms.push_back(it.second);
    for (const auto& it : three_terms) tagterms.push_back(it.second);
    for (const auto& it : four_terms) tagterms.push_back(it.second);
  }

  void add_term(
      std::vector<term_descriptor>& tagterms, value_type scale, int p1, int p2,
      std::vector<tag_type> const& op_1, std::vector<tag_type> const& op_2
  ) {
    auto term = TermMaker<Matrix, SymmGroup>::two_term(
        false, ident, scale, p1, p2, op_1, op_2, tag_handler, lat
    );
    IndexTuple<SymmGroup, 4> id(
        {p1, p2,
         static_cast<int>(
             op_1[lat.get_prop<typename SymmGroup::subcharge>("type", p1)]
         ),
         static_cast<int>(
             op_2[lat.get_prop<typename SymmGroup::subcharge>("type", p2)]
         )}
    );
  ...
  void add_term(
      std::vector<term_descriptor>& tagterms, value_type scale, int s, int p1,
      int p2, std::vector<tag_type> const& op_i,
      std::vector<tag_type> const& op_k, std::vector<tag_type> const& op_l,
      std::vector<tag_type> const& op_j
  ) {
    auto term = TermMaker<Matrix, SymmGroup>::three_term(
        ident, fill, scale, s, p1, p2, op_i, op_k, op_l, op_j, tag_handler, lat
    );
    IndexTuple<SymmGroup, 6> id(
        {static_cast<int>(term.position(0)), static_cast<int>(term.position(1)),
  ...
  void add_term(
      std::vector<term_descriptor>& tagterms, value_type scale, int i, int k,
      int l, int j, std::vector<tag_type> const& op_i,
      std::vector<tag_type> const& op_k, std::vector<tag_type> const& op_l,
      std::vector<tag_type> const& op_j
  ) {
    auto term = TermMaker<Matrix, SymmGroup>::four_term(
        ident, fill, scale, i, k, l, j, op_i, op_k, op_l, op_j, tag_handler, lat
    );
    if (i < k) std::swap(i, k);
    if (j < l) std::swap(j, l);
    IndexTuple<SymmGroup, 8> id(
        IndexTuple<SymmGroup, 4>(std::array<int, 4>{i, k, l, j}),
        IndexTuple<SymmGroup, 4>(std::array<int, 4>{
            static_cast<int>(
                op_i[lat.get_prop<typename SymmGroup::subcharge>("type", i)]
            ),
            static_cast<int>(
                op_k[lat.get_prop<typename SymmGroup::subcharge>("type", k)]
            ),
            static_cast<int>(
                op_l[lat.get_prop<typename SymmGroup::subcharge>("type", l)]
            ),
            static_cast<int>(
