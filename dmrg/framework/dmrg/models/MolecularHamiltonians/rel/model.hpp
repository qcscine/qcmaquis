...
rel_qc_model<SymmGroup>::rel_qc_model(
  ...
  op_t create_op, destroy_op, count_op, ident_op, fill_op;
...
void rel_qc_model<SymmGroup>::create_terms() {
  chem::detail::RelChemHelper<Matrix, SymmGroup> term_assistant(
...
#define onsite
    ...
    // On site energy t_ii
    else if (i == j && k == -1 && l == -1) {
      {
        ...
        term.coeff = matrixElement;
        term.push_back(std::make_pair(
            i, count[lat.get_prop<typename SymmGroup::subcharge>("type", i)]
        ));
        this->terms_.push_back(term);
      }
      used_elements[m] += 1;
...
#define hopping
    ...
    // Hopping term t_ij
      ...
      if (!parms["MAGNETIC"]) {  // with an external magnetic field applied -
                                 // Kramers symmetry is broken
        this->terms_.push_back(
            TermMaker<Matrix, SymmGroup>::positional_two_term(
                true, fill, chem::detail::cconj(matrix_elements[m]), j, i,
                create, destroy, tag_handler, lat
            )
        );
      }
      used_elements[m] += 1;
...
#define all_2_terms
    ...
    // V_iijj
      ...
      if (is_term_allowed(i, j, k, l)) {
        term_assistant.add_term(
            this->terms_, matrixElement, i, k, count, count
        );
        used_elements[m] += 1;
      }
    }
    ...
    // V_ijji
      ...
      if (is_term_allowed(i, j, k, l)) {
        term_assistant.add_term(
            this->terms_, -matrixElement, i, j, count, count
        );
        used_elements[m] += 1;
      }
    }
...
#define all_3_terms
    ...
    // V_iikl
      ...
      if (is_term_allowed(i, j, k, l)) {
        term_assistant.add_term(
            this->terms_, matrixElement, i, k, l, create, destroy, create,
            destroy
        );
        used_elements[m] += 1;
      }
    ...
    // V_ijkk
      ...
      if (is_term_allowed(i, j, k, l)) {
        term_assistant.add_term(
            this->terms_, matrixElement, k, i, j, create, destroy, create,
            destroy
        );
        used_elements[m] += 1;
      }
    ...
    // V_ijil & V_ijkj
    // This case doesn't exist for the relativistic model!
    else if ((i == k && i != j && i != l) or (j == l && j != i && j != k)) {
      used_elements[m] += 1;
    }
    ...
    // V_ijki
      ...
      if (is_term_allowed(i, j, k, l)) {
        term_assistant.add_term(
            this->terms_, -matrixElement, i, k, j, create, destroy, create,
            destroy
        );
        used_elements[m] += 1;
      }
    ...
    // V_ijjl
      ...
      if (is_term_allowed(i, j, k, l)) {
        term_assistant.add_term(
            this->terms_, -matrixElement, j, i, l, create, destroy, create,
            destroy
        );
        used_elements[m] += 1;
      }
...
#define all_4_terms
    ...
    // V_ijkl
      ...
      if (is_term_allowed(i, j, k, l)) {
        term_assistant.add_term(
            this->terms_, matrixElement, i, k, l, j, create, create, destroy,
            destroy
        );
        used_elements[m] += 1;
      }
