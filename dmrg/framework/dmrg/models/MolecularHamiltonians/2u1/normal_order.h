...
namespace {
    ...
    std::string getIdentifier(int i, int j, int k, int l, int m, int n) {
        ...
        std::stringstream is;
        is << indices[0].first << " " << indices[0].second << " "
           << indices[1].first << " " << indices[1].second << " "
...
struct intTupleHash {
    size_t operator()(std::tuple<int, int, int, int> t) const noexcept {
        ...
        hash ^= std::hash<int>()(std::get<1>(t)) + 0x9e3779b9 + (hash << 6) + (hash >> 2);
        hash ^= std::hash<int>()(std::get<2>(t)) + 0x9e3779b9 + (hash << 6) + (hash >> 2);
        hash ^= std::hash<int>()(std::get<3>(t)) + 0x9e3779b9 + (hash << 6) + (hash >> 2);
        return hash;
    }
};
...
class NormalOrderingHelper {
    ...
    */
    ...
    /**
       ...
       *        chain of operators.
     */
    std::unordered_map<std::tuple<int, int, int, int>, double, intTupleHash> getNoTwoBodyCoefficients(
            chem::detail::ChemHelper<Matrix, SymmGroup, HamiltonianType, Transcorrelated> &term_assistant,
            std::size_t nelems, std::size_t L, const std::unordered_set<std::size_t> &hole_states) {
        auto &matrix_elements = term_assistant.getMatrixElements();
