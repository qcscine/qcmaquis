...
template <
    ...
    HamiltonianTransformation HamiltonianTranscorrelated>
class qc_model : public model_impl<Matrix, SymmGroup> {
  ...
  using one_matrix =
      typename alps::numeric::associated_one_matrix<Matrix>::type;
 ...
 public:
  ...
  measurements_type measurements() const {
    ...
    std::vector<tag_type> create_up_count_down =
        (tag_handler->get_product_tags(count_down, create_up)).first;
    std::vector<tag_type> create_down_count_up =
        (tag_handler->get_product_tags(count_up, create_down_for_meas)).first;
        ...
        if (std::regex_match(lhs, what, expression)) {
          ...
          if (it.second == "Nup") {
            ...
            throw std::runtime_error(
                "Invalid observable\nLocal measurements supported so far are "
                "\"Nup\" and \"Ndown\"\n"
            );
          }

          meas.push_back(new measurements::local<Matrix, SymmGroup>(
              what.str(1), lat, ident_ops, fill_ops, meas_op
          ));
        }
      }
    }

    {
      ...
      std::regex expression_transition_twoptdm_uuuu(
          "^MEASURE\\[trans2rdm_aaaa\\]"
      );
      std::regex expression_transition_twoptdm_uddu(
          "^MEASURE\\[trans2rdm_abba\\]"
      );
      std::regex expression_transition_twoptdm_duud(
          "^MEASURE\\[trans2rdm_baab\\]"
      );
      std::regex expression_transition_twoptdm_dddd(
          "^MEASURE\\[trans2rdm_bbbb\\]"
      );
      ...
      for (auto&& it : parms.get_range()) {
        ...
        if (std::regex_match(lhs, what, expression_twoptdm) ||
          ...
          // the position vector reflects the loop ordering in the 3-RDM
          // emasurement
          std::vector<pos_t> positions;
          std::vector<std::string> value_split;
          ...
          if (value_split.size() > 1) {
            ...
            std::ostream_iterator<pos_t>(std::cout, " "), boost::lambda::    _1
            + 1); maquis::cout << " " << std::endl;
            */
          ...
          // stknecht - may 2015: collapsing operator combinations - check for
          // odd electron/open shell systems
          ...
          // the position vector reflects the loop ordering in the 4-RDM
          // emasurement
          std::vector<pos_t> positions;
          std::vector<std::string> value_split;
        ...
        // 1-RDM and transition-1RDM
          ...
          // if bra == ket the cross terms will be zero for sure, no need to
          // include them
...

          {
            ...
            // if spin density, we need the second term with a negative
            // prefactor (measure Nup-Ndown) instead of Nup + Ndown
          ...
          for (tokenizer::iterator it2 = corr_tokens.begin();
            ...
            if (*it2 == "c_up") {
              ...
              throw std::runtime_error(
                  "Unrecognized operator in correlation measurement: " +
          ...
          if (f_ops % 2 != 0) {
            throw std::runtime_error(
                "In " + name +
                ": Number of fermionic operators has to be even in correlation "
                "measurements."
            );
          }
 ...
 private:
  ...
  std::vector<tag_type> ident, fill, create_up, create_down, destroy_up,
      destroy_down, create_down_for_meas, destroy_down_for_meas, count_up,
      count_down, count_up_down, docc, e2d, d2e, d2u, u2d;
  ...
  std::vector<tag_type> register_site_specific(
    ...
    for (const auto& op : ops) {
      std::pair<tag_type, value_type> newtag =
          tag_handler->checked_register(op, kind);
