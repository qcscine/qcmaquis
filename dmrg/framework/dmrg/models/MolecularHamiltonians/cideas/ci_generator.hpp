...
std::vector<int> el_casv(
    const int &side, const int &left, std::vector<int> &casv
) {
  int len = casv.size();
...
std::vector<Determinant<SymmGroup> > copy_det(
    std::vector<Determinant<SymmGroup> > dets
) {
  int end = dets.size();
...
template <class SymmGroup>
std::vector<Determinant<SymmGroup> > deas(
    const int &pos, const int &act_orb,
    std::vector<Determinant<SymmGroup> > dets
) {
...
std::vector<int> get_half_filled(int nelec_left, int left) {
  ...
  if (nelec_left > left) {
    nelec_left = left - (nelec_left - left);
  }
  if (nelec_left > 8) {
    if (nelec_left % 2 == 0) {
      nelec_left = 8;
...
// function to reduce the symmetry vector - here actually no pair is required
// since later only first entry is used
std::vector<std::pair<int, int> > reduce_symvec(
    const std::vector<int> &symvec_left
) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (size > 0) {
      symvec_red.emplace_back(i, size);
    }
  }
  return symvec_red;
}
...
std::vector<std::pair<int, int> > get_orb(std::vector<int> hf_occ) {
  ...
  for (int i = 0; i < hf_occ.size(); i++)
    if (hf_occ[i] != 1) {
      occ_orb.emplace_back(i, hf_occ[i]);
    }
...
template <class SymmGroup>
std::vector<Determinant<SymmGroup> > generate_deas(
    Determinant<SymmGroup> const &hf_occ, std::vector<int> const &casv,
    int &run, std::vector<Determinant<SymmGroup> > &deas_dets
) {
  ...
  for (int i = 0; i < hf_occ.size(); ++i) maquis::cout << hf_occ[i] << " ";
