...
MPS<Matrix, SymmGroup> cideas(DmrgParameters parms, const Matrix& s1) {
  ...
  std::vector<typename SymmGroup::subcharge> site_types(lat.size()),
      site_types_distinct;
  for (int i = 0; i < lat.size(); i++) {
    site_types[i] = lat.get_prop<typename SymmGroup::subcharge>("type", i);
    if (std::find(
            site_types_distinct.begin(), site_types_distinct.end(),
            site_types[i]
        ) == site_types_distinct.end())
  ...
  typename SymmGroup::charge right_end =
      chem::detail::qn_helper<SymmGroup>().total_qn(parms);
  maquis::cout << "Right end: " << right_end << std::endl;

  // create MPS
  MPS<Matrix, SymmGroup> mps(lat.size());
  deas_mps_init<Matrix, SymmGroup> deas_creator(
      parms, s1, phys_dims, right_end, site_types
  );
  deas_creator(mps);
