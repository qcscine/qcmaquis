 ...
 *
 * The term descriptor class represents an entry of an operators expressed in
 * second-quantization. The underlying data type is a vector of pairs (int,
 * int). The first element of the pair is the site on which the operator acts,
 * and the second element is the tag of the corresponding elementary operator.
 *
...
class term_descriptor : public std::vector<std::pair<int, unsigned int> > {
 public:
  ...
  base getBase() const { return base(this->begin(), this->end()); }
