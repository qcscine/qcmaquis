...
struct prempo_key {
  ...
  prempo_key(std::vector<pos_op_type> const& po_, index_type o_ = 0)
      : kind(bulk), pos_op(po_), offset(o_) {}
  ...
  bool operator<(prempo_key const& lhs) const {
    if (kind != lhs.kind) {
      return kind < lhs.kind;
    }
    // if (pos_op.size() != lhs.pos_op.size()) return pos_op.size() <
    // lhs.pos_op.size();
...
std::ostream& operator<<(
    std::ostream& os, detail::prempo_key<pos_t, tag_type, index_type> key
) {
 ...
 *
 * Intermediate class that takes the terms from a model and generates an object
 * that can be then fed to the MPO constructor The class has the following
 * attributes:
 *
 ...
 * prempo: vector of prempo_map_type objects, which are map associating pairs of
 * prempo_key objects to values (i.e., coefficients of the Hamiltonian). The
 * pair of objects, which are the keyword for the dictionary, are the labels
 * which are associated to the MPO bond, i.e. the operators which have been
 *         applied before and/or after the current site
 * trivial_left/trivial_right: operators to be put on the left/right of each
 * strings of SQ operators site_terms: map storing operators acting ONLY on the
 * site (diagonal one-body operators) leftmost_right/rightmost_left: last and
 * first site where there are no trivial operators core_energy: coefficient of
 * the full identity operator
 */
...
class TaggedMPOMaker {
  ...
  using prempo_map_type = std::multimap<
      std::pair<prempo_key_type, prempo_key_type>, prempo_value_type,
 ...
 public:
   ...
   * This method should be used for constructing operators != from the
   * Hamiltonian.
   */
  TaggedMPOMaker(
      Lattice const& lat_, tag_vec const& i_, tag_vec const& i_f_,
      tag_vec const& f_, std::shared_ptr<TagHandler<Matrix, SymmGroup> > th_,
    ...
    //     prempo[p][make_pair(trivial_left,trivial_left)] =
   ...
   */
  TaggedMPOMaker(
      Lattice const& lat_, tag_vec const& i_, tag_vec const& f_,
      std::shared_ptr<TagHandler<Matrix, SymmGroup> > th_,
      typename Model<Matrix, SymmGroup>::terms_type const& terms
  )
      : lat(lat_),
        identities(i_),
  ...
  MPO<Matrix, SymmGroup> create_mpo() {
    ...
    for (pos_t p = 0; p < length; ++p) {
      ...
      for (const auto& it : prempo[p]) {
        // Remember that tag_block identifies a tuple with (size, size, tag_type
        // and scaling factor) The first two sizes should identify the operators
        // acting on the left and right, respectively, the tag_type is the
        // operator and the scaling factor is the coefficient appearing in the
        ...
        // Looks for the tag inside the left dictionary. Here an error is raised
        // if the operator is not found because, at each cycle, the left is
        // filled with the right at the previous iteration
        ...
        pre_tensor.push_back(
            tag_block(ll->second, rr_dim, val.first, val.second)
        );
      ...
      for (const auto& it : pre_tensor) {
        spin_desc_t out_spin = couple(
            left_spins[std::get<0>(it)],
            tag_handler->get_op(std::get<2>(it)).spin()
        );
        index_type out_index = std::get<1>(it);
        assert(
            right_spins[out_index].get() == 0 ||
      ...
      MPOTensor_detail::Hermitian h_(
          LeftHerm, RightHerm, LeftPhase, RightPhase
      );
      // Construction of the MPO tensor
      if (p == 0) {
        mpo.push_back(MPOTensor<Matrix, SymmGroup>(
            1, rcd.second, pre_tensor, tag_handler->get_operator_table(), h_,
            left_spins, right_spins
        ));
      } else if (p == length - 1) {
        mpo.push_back(MPOTensor<Matrix, SymmGroup>(
            rcd.first, 1, pre_tensor, tag_handler->get_operator_table(), h_,
            left_spins, right_spins
        ));
      } else {
        mpo.push_back(MPOTensor<Matrix, SymmGroup>(
            rcd.first, rcd.second, pre_tensor,
      ...
      // Final Print
      if (verbose) {
        maquis::cout << "MPO Bond " << p << ": " << rcd.second << "/" << cnt
                     << std::endl;
      }
    }
 ...
 private:
   ...
   *
   * Note that if the SQ operator is the identity, the code just updates the
   * core energy
   */
  void add_1term(term_descriptor const& term) {
    assert(term.size() == 1);
    /// Due to numerical instability: treat the core energy separately
    if (term.operator_tag(0) ==
        identities[lat.get_prop<int>("type", term.position(0))]) {
  ...
  void add_2term(term_descriptor const& term) {
    ...
    std::vector<bool> v_trivial_fill(
        Nt, true
    );  // This is used to keep track of the filling to be used per particle
        // type
    std::vector<pos_t> v_pos(2
    );  // Vector storing the position in which each operator acts
    std::vector<pos_t> v_part_type(2);  // Particle type for each site
    for (int i = 0; i < 2; ++i) {
      // Extract position and then type
      v_pos[i] = term.position(i);
      v_part_type[i] = lat.template get_prop<int>(
          "ParticleType", std::vector<pos_t>{v_pos[i]}
      );
    ...
    prempo_key_type k1 = trivial_left;
    {
      int i = 0;
      mpo_spin =
          couple(mpo_spin, (tag_handler->get_op(term.operator_tag(i))).spin());
      prempo_key_type k2;
      k2.pos_op.push_back(term[i + 1]);
      k1 = insert_operator(
          term.position(i), make_pair(k1, k2),
          prempo_value_type(term.operator_tag(i), term.coeff), detach
      );
      if (tag_handler->is_fermionic(term.operator_tag(i))) {
        v_nferm[v_part_type[i]] -= 1;
      }
    ...
    insert_filling(
        term.position(0) + 1, term.position(1), k1, v_trivial_fill,
        mpo_spin.get() > 1
    );
    // Second term
    {
      ...
      insert_operator(
          term.position(i), make_pair(k1, k2),
          prempo_value_type(term.operator_tag(i), 1.), attach
      );
    }
  ...
  void add_3term(term_descriptor const& term)
  {
      ...
      {
          int i = 0;
          mpo_spin = couple(mpo_spin,
  (tag_handler->get_op(term.operator_tag(i))).spin()); prempo_key_type k2;
          k2.pos_op.push_back(to_pair(term[i])); // k2: applied operator
          k1 = insert_operator(term.position(i), make_pair(k1, k2),
  prempo_value_type(term.operator_tag(i), 1.), attach); if
  (tag_handler->is_fermionic(term.operator_tag(i))) nferm -= 1; bool
  trivial_fill = (nferm % 2 == 0); insert_filling(term.position(i)+1,
  term.position(i+1), k1, trivial_fill, (mpo_spin.get() > 1) ?
  ...
  (tag_handler->get_op(term.operator_tag(i))).spin()); prempo_key_type k2;
          ...
          k1 = insert_operator(term.position(i), make_pair(k1, k2),
  prempo_value_type(term.operator_tag(i), maquis::real(term.coeff)), detach); if
  (tag_handler->is_fermionic(term.operator_tag(i))) nferm -= 1; bool
  trivial_fill = (nferm % 2 == 0); insert_filling(term.position(i)+1,
  term.position(i+1), k1, trivial_fill, (mpo_spin.get() > 1) ?
  term.full_identity : -1);
      ...
      /// op_2
      {
          int i = 2;
          mpo_spin = couple(mpo_spin,
  (tag_handler->get_op(term.operator_tag(i))).spin());
          insert_operator(term.position(i), make_pair(k1, trivial_right),
  prempo_value_type(term.operator_tag(i), 1.), attach);
      }
      assert(mpo_spin.get() == 0); // H is a spin 0 operator
  }
  ...
  */
   ...
   *
   * This method generalizes the [add_3body] and [add_4body] operator to
   * arbitrarily complex many-body coupling terms When the boolean prefer_fork
   * is true, if the number of operators is odd uses one additional fork label,
   * otherwise uses the merge one.
   */
  void add_generic_term(
      const term_descriptor& term, bool const& prefer_fork = true
  ) {
    ...
    SpinDescriptor<typename symm_traits::SymmType<SymmGroup>::type> mpo_spin;
    std::vector<pos_op_type> ops_left;
    std::vector<pos_op_type> ops_right;
    ...
    for (std::size_t i = 0; i < thresh; ++i) {
      ...
      // Checks how many fermionic operators are left - if the number is odd,
      // will insert the filling, otherwise will insert the identity
      if (tag_handler->is_fermionic(term.operator_tag(i))) {
        v_nferm[v_part_type[i]] -= 1;
      }
      v_trivial_fill[v_part_type[i]] = (v_nferm[v_part_type[i]] % 2 == 0);
      // -> if types at  term.position(i) and  term.position(i+1) are different
      // insert trivial fill
      insert_filling(
          term.position(i) + 1, term.position(i + 1), k1, v_trivial_fill,
          mpo_spin.get() > 1
      );
    }
    // == MIDDLE OPERATOR ==
    ...
    mpo_spin = couple(
        mpo_spin, (tag_handler->get_op(term.operator_tag(thresh))).spin()
    );
    k1 = insert_operator(
        term.position(thresh), make_pair(k1, k2),
        prempo_value_type(term.operator_tag(thresh), term.coeff), detach
    );
    // Extract position and then type
    if (tag_handler->is_fermionic(term.operator_tag(thresh))) {
      v_nferm[v_part_type[thresh]] -= 1;
    }
    v_trivial_fill[v_part_type[thresh]] =
        (v_nferm[v_part_type[thresh]] % 2 == 0);
    insert_filling(
        term.position(thresh) + 1, term.position(thresh + 1), k1,
        v_trivial_fill, mpo_spin.get() > 1
    );
    // == MERGE OPERATOR ==
   ...
   *
   * Note that in this case the filling operators must be included in the
   * terms_, since they are not added by the `tagged_mpo_maker`.
   */
  void add_nterm(term_descriptor const& term) {
    ...
    k2.pos_op.push_back(term[nops - 1]);
    {
      ...
      if (i < nops - 1 && term.position(i) + 1 != term.position(i + 1)) {
        throw std::runtime_error(
            "for n > 4 operators filling is assumed to be done manually. the "
            "list of operators contains empty sites."
        );
      }
    }
    //
    for (int i = 1; i < nops; ++i) {
      ...
      insert_operator(
          term.position(i), make_pair(k1, k2),
          prempo_value_type(term.operator_tag(i), 1.), detach
      );
      if (i < nops - 1 && term.position(i) + 1 != term.position(i + 1)) {
        throw std::runtime_error(
            "for n > 4 operators filling is assumed to be done manually. the "
   ...
   * @param trivial_fill vector of bool with size == number of particle types
   * indicating whether the identity or the filling operator should be used
   * @param isSpinLargerThanOne if true, uses the identity full operator instead
   * of the "simpler" identity
   */
  void insert_filling(
      pos_t i, pos_t j, prempo_key_type k, std::vector<bool> trivial_fill,
      bool isSpinLargerThanOne
  ) {
    for (; i < j; ++i) {
      auto typei = lat.get_prop<int>("type", i);
      auto particleTypei = lat.get_prop<int>("ParticleType", i);
      tag_type use_ident =
          (isSpinLargerThanOne) ? identities_full[typei] : identities[typei];
      tag_type op = (trivial_fill[particleTypei]) ? use_ident : fillings[typei];
      // std::pair<typename prempo_map_type::iterator,bool> ret =
      // prempo[i].insert( make_pair(make_pair(k,k), prempo_value_type(op, 1.))
      // ); if (!ret.second && ret.first->second.first != op)
      if (prempo[i].count(make_pair(k, k)) == 0) {
        auto ret = prempo[i].insert(
            make_pair(make_pair(k, k), prempo_value_type(op, 1.))
        );
      } else {
        if (prempo[i].find(make_pair(k, k))->second !=
            prempo_value_type(op, 1.)) {
          throw std::runtime_error(
              "Pre-existing term at site " + std::to_string(i) + ". Needed " +
              std::to_string(op) + ", found " +
              std::to_string(prempo[i].find(make_pair(k, k))->second.first)
          );
        }
      }
    }
   ...
   * @param merge_kind merge_behaviour: detach if a new branch should not be
   * created, attach otherwise
   */
  prempo_key_type insert_operator(
      pos_t p, std::pair<prempo_key_type, prempo_key_type> kk,
      prempo_value_type val, merge_kind merge_behavior = detach
  ) {
    /// merge_behavior == detach: a new branch will be created, in case op
    /// already exist, an offset is used merge_behavior == attach: if operator
    /// tags match, keep the same branch
   ...
   *
   * Manages the one-site operators and add identity at the two extreme of the
   * lattice (if it was not done before)
   */
  void finalize() {
    /// site terms
    std::pair<prempo_key_type, prempo_key_type> kk =
        make_pair(trivial_left, trivial_right);
    for (typename std::map<pos_t, op_t>::const_iterator it = site_terms.begin();
         it != site_terms.end(); ++it) {
      tag_type site_tag =
          tag_handler->register_op(it->second, tag_detail::bosonic);
      // std::pair<typename prempo_map_type::iterator,bool> ret;
      // ret = prempo[it->first].insert( make_pair( kk,
      // prempo_value_type(site_tag,1.) ) );
      typename prempo_map_type::iterator ret;
      ret = prempo[it->first].insert(
          make_pair(kk, prempo_value_type(site_tag, 1.))
      );
    ...
    for (size_t p = 0; p < rightmost_left; ++p) {
      prempo[p].insert(make_pair(
          make_pair(trivial_left, trivial_left),
          prempo_value_type(identities[lat.get_prop<int>("type", p)], 1.)
      ));
    }
    ...
    for (size_t p = leftmost_right + 1; p < length; ++p) {
      prempo[p].insert(make_pair(
          make_pair(trivial_right, trivial_right),
          prempo_value_type(identities[lat.get_prop<int>("type", p)], 1.)
      ));
    }
    finalized = true;
   ...
   *
   * Note that, in the model construction, we register pairs of Hermitian
   * conjugate *elementary* operators (i.e., operators that act on a single
   * site).
   */
  std::pair<prempo_key_type, std::pair<int, int> > conjugate_key(
      prempo_key_type k, pos_t p
  ) {
    // Defines a function to get the number of particles of a given symmetry
    // charge
    ...
    // Loop over all the elements of the prempo_key_type operator and finds the
    // corresponding Hermitian conjugate. Note that it is sufficient that one of
    // the key has no corresponding conjugate to not register the hermitian
    // conjugate.
    ...
    // Calculates the phase correction for the hermitian conjugate of SU(2)
    // operators.
    std::pair<int, int> phase(1, 1);
    if (k.pos_op.size() == 1) {
      // merge type operator ahead of current position
      if (p < k.pos_op[0].first) {
        SiteOperator<Matrix, SymmGroup> const& op1 =
            tag_handler->get_op(k.pos_op[0].second);
        typename SymmGroup::subcharge pdiff =
    ...
    if (k.pos_op.size() == 2) {
      SiteOperator<Matrix, SymmGroup> const& op1 =
          tag_handler->get_op(k.pos_op[0].second);
      SiteOperator<Matrix, SymmGroup> const& op2 =
          tag_handler->get_op(k.pos_op[1].second);
      // if k contains (c^dag c)_S=0 or (c c^dag)_S=0
      if (op1.spin().get() == 1 && op2.spin().get() == 1 &&
          op2.spin().action() == -1 &&
          np(op1.basis().left_charge(0)) - np(op1.basis().right_charge(0)) ==
