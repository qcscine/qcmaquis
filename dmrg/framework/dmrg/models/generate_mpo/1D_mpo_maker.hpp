...
std::vector<std::pair<
    Lattice::pos_t,
    std::pair<std::vector<typename OPTable<Matrix, SymmGroup>::op_t>, bool> > >
arrange_operators(
    std::vector<Lattice::pos_t> const& positions,
    std::vector<std::pair<
        std::vector<typename OPTable<Matrix, SymmGroup>::op_t>, bool> > const&
        operators
)

{
  // input: list of positions and operators
  // output: list of (position, operator)-pairs, sorted, unique positions with
  // operators multiplied
  ...
  std::
      transform(positions.begin(), positions.end() - 1, operators.begin(), std::back_inserter(pos_ops), std::make_pair<pos_t const&, site_ops_t const&>);
  // boost::bind(static_cast<pos_op_t(*)(pos_t const&, site_ops_t const&)>
  // (std::make_pair<pos_t, site_ops_t>), boost::lambda::_1,
  // boost::lambda::_2));
  ...
  for (size_t opnr = 0; opnr < pos_ops.size();) {
    ...
    while (range_end < pos_ops.size() &&
           pos_ops[range_end].first == pos_ops[opnr].first) {
    ...
    for (size_t r = opnr; r < range_end; ++r) pos_ops[r].second = product;
...
template <class Matrix, class SymmGroup>
term_descriptor<typename Matrix::value_type> arrange_operators(
    std::vector<Lattice::pos_t> const& positions,
    ...
    for (pos_t c1 = 0; c1 < n - 1; c1++)
      for (pos_t c2 = c1 + 1; c2 < n; c2++)
        if (positions[c1] > positions[c2] &&
            ...
            tag_handler->is_fermionic(operators[c2]))
          inv_count++;
  ...
  // arrange position / operators in pairs
  std::
      transform(positions.begin(), positions.end(), operators.begin(), std::back_inserter(pos_ops), std::make_pair<pos_t const&, tag_type const&>);
  std::
      stable_sort(pos_ops.begin(), pos_ops.end(), generate_mpo::compare<pos_op_t>);
  ...
  for (size_t opnr = 0; opnr < pos_ops.size();) {
    ...
    while (range_end < pos_ops.size() &&
           pos_ops[range_end].first == pos_ops[opnr].first) {
      value_type scale = 1.;
      std::tie(product, scale) =
          tag_handler->get_product_tag(pos_ops[range_end].second, product);
      term.coeff *= scale;
      range_end++;
    }
...
inline typename boost::tuples::access_traits<
    typename boost::tuples::element<N, Tuple>::type>::const_type
get(Tuple const& t) {
...
template <class Matrix, class SymmGroup>
MPO<Matrix, SymmGroup> sign_and_fill(
    term_descriptor<typename Matrix::value_type> term,
  ...
  for (pos_t p = 0; p < lat.size(); ++p) {
    ...
    if (it != term.end())  // if yes
    {
      ...
      if (tag_handler->is_fermionic(product) != carry_sign) {
        ...
        std::tie(product, scale) = tag_handler->get_product_tag(
            fill[lat.get_prop<sc>("type", p)], product
        );
        term.coeff *= scale;
      }
      ...
      ret[p] = MPOTensor<Matrix, SymmGroup>(
          1, 1, prempo, tag_handler->get_operator_table()
      );
    }
    ...
    {
      prempo.push_back(
          std::make_tuple(0, 0, fill[lat.get_prop<sc>("type", p)], 1.)
      );
      ret[p] = MPOTensor<Matrix, SymmGroup>(
          1, 1, prempo, tag_handler->get_operator_table()
      );
      ...
      prempo.push_back(
          std::make_tuple(0, 0, ident[lat.get_prop<sc>("type", p)], 1.)
      );
      ret[p] = MPOTensor<Matrix, SymmGroup>(
          1, 1, prempo, tag_handler->get_operator_table()
      );
...
template <class Matrix, class SymmGroup>
MPO<Matrix, SymmGroup> make_1D_mpo(
    std::vector<Lattice::pos_t> const& positions,
    std::vector<typename OPTable<Matrix, SymmGroup>::tag_type> const& operators,
    std::vector<typename OPTable<Matrix, SymmGroup>::tag_type> const& ident,
    std::vector<typename OPTable<Matrix, SymmGroup>::tag_type> const& fill,
  ...
  term_descriptor<typename Matrix::value_type> term =
      arrange_operators(positions, operators, tag_handler);
  term.coeff *= scale;
...
/**
 * @brief Routine that builds the MPO representation of the destruction
 * operator.
 * TODO: Check if it works also for the TwoU1 and SU2U1 symmetry.
 */
template <class Matrix, class SymmGroup>
MPO<Matrix, SymmGroup> make_destroy_mpo(
    const Lattice& lat, const Model<Matrix, SymmGroup>& model, int iOrb,
    IonizedOrbital alphaOrBeta = IonizedOrbital::Up
) {
  using tag_type = typename OPTable<Matrix, SymmGroup>::tag_type;
  std::vector<int> pos(1, iOrb);
  auto name = alphaOrBeta == IonizedOrbital::Up ? "destroy_up" : "destroy_down";
  tag_type opDestroy = model.get_operator_tag(
      name, lat.template get_prop<typename SymmGroup::subcharge>("type", pos[0])
  );
  ...
  return generate_mpo::make_1D_mpo(
      pos, ops, identities, fillings, model.operators_table(), lat
  );
}
