...
class MPOMaker {
 ...
 public:
  MPOMaker(
    ...
    for (size_t p = 0; p < length; ++p) {
      if (p + 1 < length)
        prempo[p].push_back(std::make_tuple(
  ...
  void add_term(OperatorTerm<Matrix, SymmGroup> const& term) {
    ...
    std::
        sort(ops.begin(), ops.end(), compare<std::pair<typename Lattice::pos_t, op_t>>);
    ...
    for (size_t p = minp; p <= maxp; ++p)
      if (!done[p]) {
        op_t const& current_filling =
            (term.with_sign) ? fillings[lat.get_prop<int>("type", p)]
                             : identities[lat.get_prop<int>("type", p)];
  ...
  MPO<Matrix, SymmGroup> create_mpo() {
    ...
    for (size_t p = 1; p < length - 1; ++p) r[p] = as_bulk(prempo[p]);
  ...
  std::vector<std::vector<block>>& get_prempo() { return prempo; }
 ...
 private:
  ...
  void finalize() {
    for (typename std::map<std::size_t, op_t>::const_iterator it =
             site_terms.begin();
    ...
    for (size_t p = leftmost_right + 1; p < length; ++p)
      prempo[p].push_back(
          std::make_tuple(1, 1, identities[lat.get_prop<int>("type", p)])
      );
