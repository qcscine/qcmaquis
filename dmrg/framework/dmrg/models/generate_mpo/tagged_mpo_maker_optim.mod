    ...
    private:
        ...
        void add_2term(term_descriptor const& term)
        {
            ...
            prempo_key_type k1 = trivial_left;
            {
                ...
                k1 = insert_operator(term.position(i), make_pair(k1, k2), prempo_value_type(term.operator_tag(i), term.coeff), detach, 1);
            }

            bool trivial_fill = !tag_handler->is_fermionic(term.operator_tag(1));

            //if (k1.offset > 0 && (term.position(1)-term.position(0)) > 1 && term.operator_tag(0)==7) {
            //    maquis::cout << "fuse " << term.operator_tag(0) << "@" << term.position(0) << " -> " << term.operator_tag(1) << "@" << term.position(1) << std::endl;
            //    prempo_key_type k_hop_on = k1;
            //    k_hop_on.offset = 0;
            //    tag_type fill_op = (trivial_fill) ? model.identity_matrix_tag(lat.get_prop<int>("type",term.position(0)+1))
            //                        : model.filling_matrix_tag(lat.get_prop<int>("type",term.position(0)+1));
            //    k1 = insert_operator(term.position(0)+1, make_pair(k1, k_hop_on), prempo_value_type(fill_op, 1.), detach, 42);

            //    insert_filling(term.position(0)+2, term.position(1), k_hop_on, trivial_fill); // todo: check with long-range n_i*n_j
            //}
            //else
...

            {
                ...
                insert_operator(term.position(i), make_pair(k1, k2), prempo_value_type(term.operator_tag(i), 1.), attach, 2);
            }
        }
        ...
        void add_3term(term_descriptor const& term)
        {
            ...
            /// op_1
            {
                ...
                k1 = insert_operator(term.position(i), make_pair(k1, k2), prempo_value_type(term.operator_tag(i), term.coeff), detach, 4);
                ...
                if (k1.offset > 0 && (term.position(2)-term.position(1)) > 1 && (term.operator_tag(2)==7 || term.operator_tag(2)==6)) {
                //if (k1.offset > 0 && (term.position(2)-term.position(1)) > 1) {
                    maquis::cout << "3insert " << term.operator_tag(1) << "@" << term.position(1) << " -> " << term.operator_tag(2) << "@" << term.position(2) << std::endl;
                    prempo_key_type k_hop_on = k1;
                    k_hop_on.offset = 0;
                    tag_type fill_op = (trivial_fill) ? model.identity_matrix_tag(lat.get_prop<int>("type",term.position(0)+1))
                                        : model.filling_matrix_tag(lat.get_prop<int>("type",term.position(0)+1));
                    k1 = insert_operator(term.position(1)+1, make_pair(k1, k_hop_on), prempo_value_type(fill_op, 1.), attach);

                    insert_filling(term.position(1)+2, term.position(2), k1, trivial_fill);
                }
                else
                    insert_filling(term.position(1)+1, term.position(2), k1, trivial_fill);
            }

            /// op_2
            {
                int i = 2;
                insert_operator(term.position(i), make_pair(k1, trivial_right), prempo_value_type(term.operator_tag(i), 1.), attach, 5);
            }
        }
        ...
        void add_4term(term_descriptor const& term)
        {
            ...
            /// op_2
            {
                ...
                k1 = insert_operator(term.position(i), make_pair(k1, k2), prempo_value_type(term.operator_tag(i), term.coeff), detach, 7);
            ...
            {
                int i = 3;
                insert_operator(term.position(i), make_pair(k1, trivial_right), prempo_value_type(term.operator_tag(i), 1.), attach, 8);
            }
        }
		...
		prempo_key_type insert_operator(pos_t p, std::pair<prempo_key_type, prempo_key_type> kk, prempo_value_type val,
                                        merge_kind merge_behavior=detach, int track = 0)
		{
            ...
            if (merge_behavior == detach) {
                if (!match.second) {
                    ...
                    //if (track)
                    //    maquis::cout << "source " << track << ", offset " << kk.second.offset
                    //                 << ", " << val.first << "@" << p
                    //                 << " -> " << kk.second.pos_op[0].second
                    //                 << "@" << kk.second.pos_op[0].first << std::endl;
                }
            }
            else {
