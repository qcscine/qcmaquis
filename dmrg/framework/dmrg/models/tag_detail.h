 ...
 */
template <class BlockMatrix>
std::pair<bool, typename BlockMatrix::matrix_type::value_type> is_scaled(
    BlockMatrix const& reference, BlockMatrix const& sample
) {
  ...
  try {
    ...
    maquis::cout << "invscale1 numcheck failed\n";
    exit(1);
  }
  try {
    num_check(invscale2);
  } catch (const std::exception& e) {
    maquis::cout << "invscale2 numcheck failed\n";
    exit(1);
  }
  try {
    num_check(scale);
  } catch (const std::exception& e) {
    maquis::cout << "scale numcheck failed\n";
    exit(1);
  }
#endif
