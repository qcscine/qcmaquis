...
struct call_hf_init {
  static typename Model<Matrix, SymmGroup>::initializer_ptr call(
    ...
    return typename Model<Matrix, SymmGroup>::initializer_ptr(
        new default_mps_init<Matrix, SymmGroup>(
            parms, phys_dims, right_end, site_type
        )
    );
  }
};
...
struct call_hf_init<
    Matrix, SymmGroup, symm_traits::enable_if_chemmodel_t<SymmGroup> > {
  static typename Model<Matrix, SymmGroup>::initializer_ptr call(
      BaseParameters parms, std::vector<Index<SymmGroup> > const& phys_dims,
      typename SymmGroup::charge right_end, std::vector<int> const& site_type
  ) {
    return typename Model<Matrix, SymmGroup>::initializer_ptr(
        new hf_mps_init<Matrix, SymmGroup>(
            parms, phys_dims, right_end, site_type
        )
    );
  }
...
model_impl<Matrix, SymmGroup>::initializer(
    Lattice const& lat, BaseParameters& parms
) const {
  ...
  if (parms["init_type"] == "default")
    return initializer_ptr(new default_mps_init<Matrix, SymmGroup>(
        parms, site_bases, initc, site_types
    ));
    ...
    return initializer_ptr(new basis_mps_init_generic_const<Matrix, SymmGroup>(
        parms, site_bases, initc, site_types
    ));
  else if (parms["init_type"] == "basis_state_generic_default")
    return initializer_ptr(
        new basis_mps_init_generic_default<Matrix, SymmGroup>(
            parms, site_bases, initc, site_types
        )
    );
  else if (parms["init_type"] == "coherent")
    return initializer_ptr(new coherent_mps_init<Matrix, SymmGroup>(
        parms, site_bases, initc, site_types
    ));
  else if (parms["init_type"] == "hf")
    return detail::call_hf_init<Matrix, SymmGroup>::call(
        parms, site_bases, initc, site_types
    );
  else {
