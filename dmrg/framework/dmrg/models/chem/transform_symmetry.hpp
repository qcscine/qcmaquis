...
std::vector<typename SymmOut::charge> transform_charge(
    typename SymmIn::charge cin
) {
...
void transform_site(
    MPSTensor<Matrix, SymmIn> const& mps_in, MPSTensor<Matrix, SymmOut>& mps_out
) {
  ...
  // each 2u1 sector (out_charge) contains a SU2 Index to describe the SU2
  // blocks within the larger 2u1 block
  ...
  for (int pass = 0; pass < 2; ++pass) {
    ...
    if (pass == 1) {
      ...
      for (typename subsector_map_t::iterator it = right_subblocks.begin();
           it != right_subblocks.end(); ++it)
        new_right_i.insert(
            std::make_pair(it->first, (it->second).sum_of_sizes())
        );

      mps_out = MPSTensor<Matrix, SymmOut>(
          physical_i_out, new_left_i, new_right_i, false, 0.
      );
    }
    ...
    for (size_t block = 0; block < m1.n_blocks(); ++block) {
      ...
      for (size_t s = 0; s < physical_i.size(); ++s) {
        size_t l = left_i.position(
            SymmIn::fuse(m1.basis().left_charge(block), -physical_i[s].first)
        );
        ...
        std::vector<out_charge> l_sectors =
            transform_detail::transform_charge<SymmIn, SymmOut>(in_l_charge);
        std::vector<out_charge> r_sectors =
            transform_detail::transform_charge<SymmIn, SymmOut>(in_r_charge);

        // form pairs from matching right and left sectors
        std::vector<std::pair<out_charge, out_charge>> sectors;
        for (typename std::vector<out_charge>::const_iterator it1 =
                 l_sectors.begin();
             it1 != l_sectors.end(); ++it1)
          for (typename std::vector<out_charge>::const_iterator it2 =
                   r_sectors.begin();
               it2 != r_sectors.end(); ++it2)
            for (typename Index<SymmOut>::const_iterator itp =
                     physical_i_out.begin();
                 itp != physical_i_out.end(); ++itp)
        ...
        if (pass == 0) {
          // insert the source sector into a non-paired target symmetry
          // block_matrix
          for (typename std::vector<
                   std::pair<out_charge, out_charge>>::const_iterator it =
                   sectors.begin();
               it != sectors.end(); ++it) {
            out_charge leftc = it->first, rightc = it->second,
                       physc = SymmOut::fuse(leftc, -rightc);

            if (!left_i_out.has(leftc) || !right_i_out.has(rightc)) continue;
            ...
            assert(
                left_subblocks[leftc].size_of_block(in_l_charge) ==
                left_i[l].second
            );
            assert(
                right_subblocks[rightc].size_of_block(in_r_charge) ==
                right_i[r].second
            );
          }
        }
        // transfer the blocks
          ...
          std::size_t in_left_offset =
              in_left_pb(physical_i[s].first, left_i[l].first);
          ...
          for (std::size_t ci = 0; ci < num_cols(iblock); ++ci)
            std::copy(
                iblock.col(ci).first + in_left_offset,
                iblock.col(ci).first + in_left_offset + ldim,
                source_block.col(ci).first
            );
          ...
          for (typename std::vector<
            ...
            if (!m2.has_block(rightc, rightc)) continue;
            ...
            int l1 = SymmIn::spin(in_l_charge),
                l2 = std::abs(SymmIn::spin(physical_i[s].first)),
                l3 = SymmIn::spin(in_r_charge);
            int m1 = leftc[0] - leftc[1], m2 = physc[0] - physc[1],
                m3 = rightc[0] - rightc[1];
            double clebsch_gordan =
                pow(-1.0, (l1 - l2 + m3) / 2) * sqrt(l3 + 1.0) *
                WignerWrapper::gsl_sf_coupling_3j(l1, l2, l3, m1, m2, -m3);

            for (std::size_t ci = 0; ci < num_cols(source_block); ++ci)
              std::transform(
                  source_block.col(ci).first, source_block.col(ci).second,
                  current_block.col(ci + out_right_offset_su2).first +
                      out_left_offset_2u1 + out_left_offset_su2,
...
struct transform_mps {
  using SymmOut = typename boost::mpl::if_<
      symm_traits::HasPG<SymmGroup>, TwoU1PG, TwoU1>::type;

  void operator()(
      MPS<Matrix, SymmGroup> const& mps_in, MPS<Matrix, SymmOut>& mps_out
  ) {}
};
...
struct transform_mps<
  ...
  MPS<Matrix, SymmOut> operator()(
      MPS<Matrix, SymmGroup> mps_in, int Nup, int Ndown
  ) {
    ...
    typename SymmOut::subcharge irrep =
        getPG<SymmGroup>()(mps_in[mps_in.size() - 1].col_dim()[0].first);

    const_mps_init<Matrix, SymmOut> mpsinit(
        parms,
        chem::detail::make_2u1_site_basis<Matrix, SymmOut>(
            L, Nup, Ndown, parms["site_types"]
        ),
        chem::detail::make_2u1_initc<SymmOut>(Nup, Ndown, irrep),
        parms["site_types"]
    );
