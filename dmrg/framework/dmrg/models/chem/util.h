...
// Create 2U1 parameter set from a given L, number of spin-up and spin-down
// electrons (Nup,Ndown) copy over parameters from existing parameters if
// provided
inline BaseParameters set_2u1_parameters(
    int L, int Nup, int Ndown,
...
inline std::string infer_site_types(MPS<Matrix, SymmGroup> const& mps) {
  ...
  for (Lattice::pos_t p = 0; p < mps.size(); ++p)
    for (std::size_t i = 0; i < mps[p].site_dim().size(); ++i) {
      ...
      if (i == mps[p].site_dim().size() - 1) site_types += "0,";
    }
...
template <class SymmGroup>
inline typename SymmGroup::charge make_2u1_initc(
    int Nup, int Ndown, int irrep
) {
...
template <class Matrix, class SymmGroup>
inline std::vector<Index<SymmGroup> > make_2u1_site_basis(
    int L, int Nup, int Ndown, const std::string& site_types
) {
