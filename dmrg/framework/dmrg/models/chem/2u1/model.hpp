...
qc_model<Matrix, SymmGroup>::qc_model(
    Lattice const& lat_, BaseParameters& parms_
)
    : lat(lat_), parms(parms_), tag_handler(new table_type()), max_irrep(0) {
...
#undef REGISTER
  ...
  std::pair<std::vector<tag_type>, std::vector<value_type> > cdtf =
      tag_handler->get_product_tags(create_down, fill);
  ...
  std::pair<std::vector<tag_type>, std::vector<value_type> > ftdd =
      tag_handler->get_product_tags(fill, destroy_down);
  ...
  std::pair<std::vector<tag_type>, std::vector<value_type> > cdnu =
      tag_handler->get_product_tags(create_down, count_up);
  std::pair<std::vector<tag_type>, std::vector<value_type> > ddnu =
      tag_handler->get_product_tags(destroy_down, count_up);
  ...
  std::pair<std::vector<tag_type>, std::vector<value_type> > ddcu =
      tag_handler->get_product_tags(destroy_down, create_up);
  std::pair<std::vector<tag_type>, std::vector<value_type> > ducd =
      tag_handler->get_product_tags(destroy_up, create_down);
  //**********************************************************************

  //#define PRINT(op) maquis::cout << #op << "\t"; std::copy(op.begin(),
  //op.end(), std::ostream_iterator<tag_type>(std::cout, " ")); maquis::cout <<
  //std::endl; PRINT(ident) #undef PRINT

#define HERMITIAN(op1, op2)               \
  for (int hh = 0; hh < op1.size(); ++hh) \
    tag_handler->hermitian_pair(op1[hh], op2[hh]);
...
void qc_model<Matrix, SymmGroup>::create_terms() {
  chem::detail::ChemHelper<Matrix, SymmGroup> term_assistant(
      parms, lat, ident, fill, tag_handler
  );
  ...
  for (std::size_t m = 0; m < matrix_elements.size(); ++m) {
    ...
    if (i == -1 && j == -1 && k == -1 && l == -1) {
      ...
      term.push_back(std::make_pair(
          0, ident[lat.get_prop<typename SymmGroup::subcharge>("type", 0)]
      ));
    ...
    // On site energy t_ii
    else if (i == j && k == -1 && l == -1) {
      {
        ...
        term.coeff = matrixElement;
        term.push_back(std::make_pair(
            i, count_up[lat.get_prop<typename SymmGroup::subcharge>("type", i)]
        ));
        this->terms_.push_back(term);
      }
      {
        ...
        term.push_back(std::make_pair(
            i,
            count_down[lat.get_prop<typename SymmGroup::subcharge>("type", i)]
        ));
        this->terms_.push_back(term);
      }
    ...
    // Hopping term t_ij
      ...
      this->terms_.push_back(TermMaker<Matrix, SymmGroup>::positional_two_term(
          true, fill, matrixElement, i, j, create_up, destroy_up, tag_handler,
          lat
      ));
      this->terms_.push_back(TermMaker<Matrix, SymmGroup>::positional_two_term(
          true, fill, matrixElement, i, j, create_down, destroy_down,
          tag_handler, lat
      ));
      this->terms_.push_back(TermMaker<Matrix, SymmGroup>::positional_two_term(
          true, fill, matrixElement, j, i, create_up, destroy_up, tag_handler,
          lat
      ));
      this->terms_.push_back(TermMaker<Matrix, SymmGroup>::positional_two_term(
          true, fill, matrixElement, j, i, create_down, destroy_down,
          tag_handler, lat
      ));
    ...
    // On site Coulomb repulsion V_iiii
      ...
      term.coeff = matrixElement;
      term.push_back(std::make_pair(
          i, docc[lat.get_prop<typename SymmGroup::subcharge>("type", 0)]
      ));
      this->terms_.push_back(term);
    ...
    // V_ijjj = V_jijj = V_jjij = V_jjji
      ...
      this->terms_.push_back(TermMaker<Matrix, SymmGroup>::positional_two_term(
          true, fill, matrixElement, same_idx, pos1, count_down, create_up,
          destroy_up, tag_handler, lat
      ));
      ...
      this->terms_.push_back(TermMaker<Matrix, SymmGroup>::positional_two_term(
          true, fill, -matrixElement, same_idx, pos1, destroy_up, count_down,
          create_up, tag_handler, lat
      ));
      ...
      this->terms_.push_back(TermMaker<Matrix, SymmGroup>::positional_two_term(
          true, fill, matrixElement, same_idx, pos1, count_up, create_down,
          destroy_down, tag_handler, lat
      ));
      ...
      this->terms_.push_back(TermMaker<Matrix, SymmGroup>::positional_two_term(
          true, fill, -matrixElement, same_idx, pos1, destroy_down, count_up,
          create_down, tag_handler, lat
      ));
    ...
    // V_iijj == V_jjii
      ...
      // term_assistant.add_term(this->terms_, matrixElement, i, k, count_up,
      // count_up); term_assistant.add_term(this->terms_, matrixElement, i, k,
      // count_up, count_down); term_assistant.add_term(this->terms_,
      // matrixElement, i, k, count_down, count_up);
      // term_assistant.add_term(this->terms_, matrixElement, i, k, count_down,
      // count_down);
      term_assistant.add_term(
          this->terms_, matrixElement, i, k, count_up_down, count_up_down
      );
    ...
    // V_ijij == V_jiji = V_ijji = V_jiij
      ...
      term_assistant.add_term(
          this->terms_, -matrixElement, i, j, count_up, count_up
      );
      term_assistant.add_term(
          this->terms_, -matrixElement, i, j, count_down, count_down
      );
      ...
      term_assistant.add_term(
          // this->terms_, -matrixElement * ptag1.second * ptag2.second, i, j,
          // ptag1.first, ptag2.first
          this->terms_, -matrixElement, i, j, destroy_down, create_up,
          destroy_up, create_down
      );
      ...
      term_assistant.add_term(
          // this->terms_, -matrixElement * ptag1.second * ptag2.second, i, j,
          // ptag1.first, ptag2.first
          this->terms_, -matrixElement, i, j, destroy_up, create_down,
          destroy_down, create_up
      );
    ...
    //                         contained above
      ...
      // n_up * cdag_up * c_up <--
      term_assistant.add_term(
          this->terms_, matrixElement, same_idx, k, l, create_up, destroy_up,
          create_up, destroy_up
      );
      // n_up * cdag_down * c_down <--
      term_assistant.add_term(
          this->terms_, matrixElement, same_idx, k, l, create_up, destroy_up,
          create_down, destroy_down
      );
      // n_down * cdag_up * c_up <--
      term_assistant.add_term(
          this->terms_, matrixElement, same_idx, k, l, create_down,
          destroy_down, create_up, destroy_up
      );
      // n_down * cdag_down * c_down <--
      term_assistant.add_term(
          this->terms_, matrixElement, same_idx, k, l, create_down,
          destroy_down, create_down, destroy_down
      );

      // --> n_up * c_up * cdag_up
      term_assistant.add_term(
          this->terms_, matrixElement, same_idx, l, k, create_up, destroy_up,
          create_up, destroy_up
      );
      ...
      // --> n_down * c_up * cdag_up
      term_assistant.add_term(
          this->terms_, matrixElement, same_idx, l, k, create_down,
          destroy_down, create_up, destroy_up
      );
    ...
    //                           V_ilij = V_ilji = V_liij = V_liji
      ...
      typename SymmGroup::subcharge irr =
          lat.get_prop<typename SymmGroup::subcharge>("type", same_idx);

      term_assistant.add_term(
          this->terms_, matrixElement, same_idx, pos1, pos2, create_up,
          create_down, destroy_down, destroy_up
      );
      term_assistant.add_term(
          this->terms_, matrixElement, same_idx, pos1, pos2, create_down,
          create_up, destroy_up, destroy_down
      );
      term_assistant.add_term(
          this->terms_, matrixElement, same_idx, pos1, pos2, destroy_down,
          destroy_up, create_up, create_down
      );
      term_assistant.add_term(
          this->terms_, matrixElement, same_idx, pos1, pos2, destroy_up,
          destroy_down, create_down, create_up
      );

      term_assistant.add_term(
          this->terms_, -matrixElement, same_idx, pos1, pos2, create_up,
          destroy_up, create_up, destroy_up
      );
      term_assistant.add_term(
          this->terms_, -matrixElement, same_idx, pos1, pos2, create_up,
          destroy_down, create_down, destroy_up
      );
      term_assistant.add_term(
          this->terms_, -matrixElement, same_idx, pos1, pos2, create_down,
          destroy_up, create_up, destroy_down
      );
      term_assistant.add_term(
          this->terms_, -matrixElement, same_idx, pos1, pos2, create_down,
          destroy_down, create_down, destroy_down
      ...
      term_assistant.add_term(
          this->terms_, -matrixElement, same_idx, pos2, pos1, create_up,
          destroy_up, create_up, destroy_up
      );
      term_assistant.add_term(
          this->terms_, -matrixElement, same_idx, pos2, pos1, create_up,
      ...
      term_assistant.add_term(
          this->terms_, -matrixElement, same_idx, pos2, pos1, create_down,
          destroy_up, create_up, destroy_down
      );
      term_assistant.add_term(
          this->terms_, -matrixElement, same_idx, pos2, pos1, create_down,
    ...
    // 32 (8x4)-fold degenerate V_ijkl = V_jikl = V_ijlk = V_jilk = V_klij =
    // V_lkij = V_klji = V_lkji * spin V_ijkl -> 24 permutations which fall into
    // 3 equivalence classes of 8 elements (with identical V_ matrix element)
    // coded: 4 index permutations x 4 spin combinations
    else if (i != j && j != k && k != l && i != k && j != l) {
      // 1
      term_assistant.add_term(
          this->terms_, i, k, l, j, create_up, create_up, destroy_up, destroy_up
      );
      term_assistant.add_term(
          this->terms_, i, k, l, j, create_up, create_down, destroy_down,
          destroy_up
      );
      term_assistant.add_term(
          this->terms_, i, k, l, j, create_down, create_up, destroy_up,
          destroy_down
      );
      term_assistant.add_term(
          this->terms_, i, k, l, j, create_down, create_down, destroy_down,
          destroy_down
      );

      // 2
      term_assistant.add_term(
          this->terms_, i, l, k, j, create_up, create_up, destroy_up, destroy_up
      );
      term_assistant.add_term(
          this->terms_, i, l, k, j, create_up, create_down, destroy_down,
          destroy_up
      );
      term_assistant.add_term(
          this->terms_, i, l, k, j, create_down, create_up, destroy_up,
          destroy_down
      );
      term_assistant.add_term(
          this->terms_, i, l, k, j, create_down, create_down, destroy_down,
          destroy_down
      );

      // 3
      term_assistant.add_term(
          this->terms_, j, k, l, i, create_up, create_up, destroy_up, destroy_up
      );
      term_assistant.add_term(
          this->terms_, j, k, l, i, create_up, create_down, destroy_down,
          destroy_up
      );
      term_assistant.add_term(
          this->terms_, j, k, l, i, create_down, create_up, destroy_up,
          destroy_down
      );
      term_assistant.add_term(
          this->terms_, j, k, l, i, create_down, create_down, destroy_down,
          destroy_down
      );

      // 4
      term_assistant.add_term(
          this->terms_, j, l, k, i, create_up, create_up, destroy_up, destroy_up
      );
      term_assistant.add_term(
          this->terms_, j, l, k, i, create_up, create_down, destroy_down,
          destroy_up
      );
      term_assistant.add_term(
          this->terms_, j, l, k, i, create_down, create_up, destroy_up,
          destroy_down
      );
      term_assistant.add_term(
          this->terms_, j, l, k, i, create_down, create_down, destroy_down,
          destroy_down
      );
  ...
  maquis::cout << "\nThe hamiltonian will contain " << this->terms_.size()
               << " terms\n";
}
