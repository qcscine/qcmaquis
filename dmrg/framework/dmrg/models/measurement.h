...
inline std::vector<std::string> label_strings (const Lattice& lat,
                                 const std::vector<std::vector<Lattice::pos_t> >& labels,
                                 const std::vector<Lattice::pos_t> & ext_labels = std::vector<Lattice::pos_t>()
                                 )
{
         ...
         it != labels.end(); ++it)
    {
        ...
        if (!ext_labels.empty())
          std::for_each(ext_labels.begin(), ext_labels.end(), [&](decltype(*ext_labels.begin()) i)->void { oss << "( " << i << " ) -- ";} );
