...
// now the integral parser can both real and complex integrals without
// specialization
template <class T>
inline std::pair<
    std::vector<chem::index_type<chem::Hamiltonian::PreBO>>, std::vector<T>>
parse_integrals(BaseParameters& parms, Lattice const& lat) {
  ...
  if (parms.is_set("integral_file")) {
    ...
    //                                               the rule: a(i)+ a(m)+ a(m)-
    //                                               a(i)- The matrix element
