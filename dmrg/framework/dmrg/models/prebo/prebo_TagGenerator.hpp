 ...
 * @brief This class handels the correct generation of fermionic and bosonic
 * tags.
...
class TagGenerator {
 ...
 public:
  ...
  TagGenerator(
      ...
      std::shared_ptr<TagHandler<Matrix, NU1>>& tag_handler_
  )
    ...
    construct_excited_states(
        fer_excited_states_array, bos_excited_states_array
    );
    // All operators are collected in vectors
    // identity, creation operator (up, down), annihilation operator (up,down),
    // occupation number operator. Fermionic operators
    ...
    // Filling the tags...
    create_tags(
        fer_excited_states_array, bos_excited_states_array, num_fer_types,
        num_bos_types
    );
  ...
  void construct_excited_states(
    ...
    charge_type empty_state(0);
    // Dimension of the excited_states array is the number of different particle
    // types. Now the excited states are populated; here it is important to
    // distinguish between fermionic and bosonic particles. Fermions are
    // associated with the basis <0,0>, <1,0>, <0,1>, and <1,1>, whereas bosons
    // are of type <0>, <1>, ..., <n>,<n+1>, where n is the number of bosons of
    // the given type. The unoccupied state is always implicit.
    for (std::size_t it = 0; it < num_particle_types; it++) {
      if (isFermion[it] == 1) {
        ...
        throw std::runtime_error(
            "Particle is not specified to be a boson or fermion."
        );
      }
    }
    // Prepare the excited states.
    // The fermion and boson counters are required for keeping track of the
    // indices.
    int fcounter = 0;
    int bcounter = 0;
    for (std::size_t it = 0; it < num_particle_types; it++) {
      ...
      // Bosons: depends on the number of bosons of type "it"
        ...
        for (std::size_t part_it = 0; part_it < vec_particles[it] + 1;
             part_it++) {
          bos_excited_states_array[bcounter][part_it][it + fcounter] =
              part_it + 1;
        }
        bcounter++;
      }
   ...
   * The tags are needed to label the operators, which are later used for the
  ...
  void create_tags(
    ...
    std::vector<op_t> fer_ident, fer_filling, fer_create_up, fer_create_down,
        fer_dest_up, fer_dest_down;  //, fer_count;
    ...
    for (std::size_t it = 0; it < num_particle_types; it++) {
      if (isFermion[it] == 1) {
        op_t ident, filling, create_up, create_down, destroy_up,
            destroy_down;  //, count;
        // identities
        ...
        filling.insert_block(
            Matrix(1, 1, -1), fer_excited_states_array[fcounter][0],
            fer_excited_states_array[fcounter][0]
        );
        filling.insert_block(
            Matrix(1, 1, -1), fer_excited_states_array[fcounter][1],
            fer_excited_states_array[fcounter][1]
        );
        filling.insert_block(
            Matrix(1, 1, 1), fer_excited_states_array[fcounter][2],
            fer_excited_states_array[fcounter][2]
        );
        // creators
        create_up.insert_block(
            Matrix(1, 1, 1), empty_state, fer_excited_states_array[fcounter][0]
        );
        create_up.insert_block(
            Matrix(1, 1, 1), fer_excited_states_array[fcounter][1],
            fer_excited_states_array[fcounter][2]
        );
        create_down.insert_block(
            Matrix(1, 1, 1), empty_state, fer_excited_states_array[fcounter][1]
        );
        create_down.insert_block(
            Matrix(1, 1, 1), fer_excited_states_array[fcounter][0],
            fer_excited_states_array[fcounter][2]
        ...
        // destroyers
        destroy_up.insert_block(
            Matrix(1, 1, 1), fer_excited_states_array[fcounter][0], empty_state
        );
        destroy_up.insert_block(
            Matrix(1, 1, 1), fer_excited_states_array[fcounter][2],
            fer_excited_states_array[fcounter][1]
        );
        destroy_down.insert_block(
            Matrix(1, 1, 1), fer_excited_states_array[fcounter][1], empty_state
        );
        destroy_down.insert_block(
            Matrix(1, 1, 1), fer_excited_states_array[fcounter][2],
            fer_excited_states_array[fcounter][0]
        ...
        // creators & destroyers
        create.insert_block(
            Matrix(1, 1, 1), empty_state, bos_excited_states_array[bcounter][0]
        );
        destroy.insert_block(
            Matrix(1, 1, 1), bos_excited_states_array[bcounter][0], empty_state
        );
        for (std::size_t jt = 1; jt < vec_particles[it] + 1; jt++) {
          // create
          create.insert_block(
              Matrix(1, 1, std::sqrt(jt + 1)),
    ...
    // Fermions
    if (num_fer_types > 0) {
      this->fer_ident_tag = modelHelper<Matrix, NU1>::register_all_types(
          fer_ident, tag_detail::bosonic, tag_handler
      );
      this->fer_filling_tag = modelHelper<Matrix, NU1>::register_all_types(
          fer_filling, tag_detail::bosonic, tag_handler
      );
      this->fer_create_up_tag = modelHelper<Matrix, NU1>::register_all_types(
          fer_create_up, tag_detail::fermionic, tag_handler
      );
      this->fer_create_down_tag = modelHelper<Matrix, NU1>::register_all_types(
          fer_create_down, tag_detail::fermionic, tag_handler
      );
      this->fer_dest_up_tag = modelHelper<Matrix, NU1>::register_all_types(
          fer_dest_up, tag_detail::fermionic, tag_handler
      );
      this->fer_dest_down_tag = modelHelper<Matrix, NU1>::register_all_types(
          fer_dest_down, tag_detail::fermionic, tag_handler
      );
      modelHelper<Matrix, NU1>::registerHermitianConjugates(
          fer_create_up_tag, fer_dest_up_tag, tag_handler
      );
      modelHelper<Matrix, NU1>::registerHermitianConjugates(
          fer_create_down_tag, fer_dest_down_tag, tag_handler
      );
    }
    // Bosons
    if (num_bos_types > 0) {
      this->bos_ident_tag = modelHelper<Matrix, NU1>::register_all_types(
          bos_ident, tag_detail::bosonic, tag_handler
      );
      this->bos_create_tag = modelHelper<Matrix, NU1>::register_all_types(
          bos_create, tag_detail::bosonic, tag_handler
      );
      this->bos_dest_tag = modelHelper<Matrix, NU1>::register_all_types(
          bos_dest, tag_detail::bosonic, tag_handler
      );
      this->bos_count_tag = modelHelper<Matrix, NU1>::register_all_types(
          bos_count, tag_detail::bosonic, tag_handler
      );
      modelHelper<Matrix, NU1>::registerHermitianConjugates(
          bos_create_tag, bos_dest_tag, tag_handler
      );
    }

  }  // create_tags

  auto get_tag_vector(const Type& type, const OpType& op, const Spin& spin)
      -> std::vector<tag_type>& {
    switch (type) {
      case Type::Fermion:
        switch (op) {
  ...
  auto getPhysIndexes() -> std::vector<Index<NU1>>& { return phys_indexes; }
};
