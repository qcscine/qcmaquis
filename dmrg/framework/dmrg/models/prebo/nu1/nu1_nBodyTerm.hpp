 ...
 * @brief Takes an n-body 2nd quantization string that can contain operators of
 * different particle types and creates a string of `SymbolicOperator`s.
 * Moreover, it constructs all different spin-combinations. Note that creation
 * operators are always on the left, and -obviously- there must be even numbers
 * of operators for all types.
 */
class NBodyTerm {
  // Types definition
 ...
 private:
  ...
  std::vector<size_t>
      m_spin;  // physical spin is spin/2. So electrons have spin=1
 ...
 public:
  ...
  // +-------------+
  NBodyTerm(
      const std::vector<std::pair<part_type, pos_t>> &nbody_term_,
      const std::vector<bool> &isFermion_,
      const std::vector<int> &vec_orbitals_, const std::vector<pos_t> &order_
  ) {
    ...
    for (auto const &sigma : isFermion_) m_spin.push_back(sigma);
    ...
    // std::pairs for each type. This requires a normal ordered input!!! N[ a+ b
    // b+ a] = a+ b+ b a
 ...
 private:
   ...
   */
  std::vector<std::vector<SymbolicOperator>> nBodyTerm2OperatorString(
      std::vector<std::pair<part_type, pos_t>> temp_nbody
  ) {
    ...
    if (m_spin[nt] == 0) {
      ...
      for (size_t i = 0; i < temp_nbody.size(); i++) {
        ...
        if (i < length / 2) {
          SymbolicOperator tempOp(abs_index, OpType::Create, nt, Spin::Zero);
          tmp.push_back(tempOp);
        }
        if (i >= length / 2) {
          SymbolicOperator tempOp(
              abs_index, OpType::Annihilate, nt, Spin::Zero
          );
          tmp.push_back(tempOp);
        }
      }
    ...
    // Fermion
      ...
      for (auto &spin_config : m_spin_configs) {
        ...
        for (size_t i = 0; i < temp_nbody.size(); i++) {
          ...
          if (i < length / 2) {
            SymbolicOperator tempOp(
                abs_index, OpType::Create, nt, spin_config.at(i)
            );
            tmp.push_back(tempOp);
          }
          if (i >= length / 2) {
            SymbolicOperator tempOp(
                abs_index, OpType::Annihilate, nt, spin_config.at(i)
            );
            tmp.push_back(tempOp);
          }
        }
 ...
 public:
   ...
   * @return
   */
  static pos_t retrieve_abs_index(
      const pos_t &rel_orb_index, const part_type &type,
      const std::vector<int> &vec_orbitals, const std::vector<pos_t> &inv_order
  ) {
    pos_t abs_index = 0;
    for (size_t i = 0; i < type; i++) {
