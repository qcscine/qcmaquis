 ...
 *
 * This class enables DMRG calculations for Pre-Born-Oppenheimer Hamiltonians
 * associated with multiple indistinguishable fermionic particles. Bosonic
 * particles will be implemented soon.
 *
...
class PreBO : public model_impl<Matrix, NU1_template<N>> {
 ...
 private:
  ...
  /** Vector of size [num_particle_types] with true for fermions and false for
   * bosons */
  ...
  /** Pointer to the term generator (helper class to generate the SQ
   * Hamiltonian) */
 ...
 public:
  ...
  PreBO(const Lattice& lat_, BaseParameters& parms_, bool verbose_ = true)
    ...
    ptr_term_generator = std::make_shared<prebo::TermGenerator<Matrix, N>>(
        ptr_lat, ptr_tag_handler, verbose
    );
   ...
   *
   * Note that, unlike the other classes, the term vector is *not* populated at
   * construction level, and [create_terms] must be called externally to make
   * terms_ get pupulated.
   */
  void create_terms() {
    ...
    if (verbose)
      std::cout << "Construction of the Hamiltonian took " << duration.count()
                << " seconds" << std::endl
                << std::endl;
  }
   ...
   * Note that this method is called in the constructor of the MPS Tensor
   * Network
   */
   ...
   *
   * Returns the total quantum number associated to the Hamiltonian, i.e., the
   * number of particles per type and Sz value
   */
  ...
  tag_type identity_matrix_tag(size_t type) const {
    ...
    if (isFermion[type]) {
      auto tag_vec = ptr_term_generator->getTagContainer()->get_tag_vector(
  ...
  table_ptr operators_table() const { return ptr_tag_handler; }
  ...
  measurements_type measurements() const {
    ...
    for (auto&& it : parms.get_range()) {
      ...
      if (std::regex_match(lhs, what, expression_1ParticleRDM)) {
        meas.push_back(new measurements::PreBOParticleRDM<Matrix, N>(
            parms, lat, identities, fillings, ptr_term_generator
        ));
      }
      if (std::regex_match(lhs, what, expression_MutualInformation)) {
        meas.push_back(new measurements::PreBOMutualInformation<Matrix, N>(
            parms, lat, identities, fillings, ptr_term_generator
        ));
      }
    }
