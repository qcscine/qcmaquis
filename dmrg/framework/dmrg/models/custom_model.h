...
class custom_model_impl : public model_impl<Matrix, SymmGroup> {
 ...
 public:
  ...
  custom_model_impl(Index<SymmGroup> const& phys_)
      : tag_handler(new table_type()), phys(phys_) {
  ...
  void add_bondterm(
      op_t const& op_left, size_t i, op_t const& op_right, size_t j,
      value_type const& coeff = 1.
  ) {
    tag_type tag_left = tag_handler->register_op(op_left, tag_detail::bosonic);
    tag_type tag_right =
        tag_handler->register_op(op_right, tag_detail::bosonic);
  ...
  void add_nterm(
      std::vector<std::tuple<size_t, op_t> > const& ops,
      value_type const& coeff = 1.
  ) {
    term_descriptor term;
    term.coeff = coeff;
    for (typename std::vector<std::tuple<size_t, op_t> >::const_iterator it =
             ops.begin();
         it != ops.end(); ++it) {
      tag_type tag =
          tag_handler->register_op(std::get<1>(*it), tag_detail::bosonic);
  ...
  Index<SymmGroup> const& phys_dim(size_t type) const { return phys; }
  ...
  typename SymmGroup::charge total_quantum_numbers(BaseParameters& parms
    ...
    throw std::runtime_error(
        "Model cannot parse total_quantum_numbers from parameters."
    );
    return SymmGroup::IdentityCharge;
  }
  measurements_type measurements() const { return measurements_type(); }
...
class CustomModel {
 ...
 public:
  ...
  CustomModel(Index<SymmGroup> const& phys) : impl_(new impl_type(phys)) {}

  Model<Matrix, SymmGroup> make_model() const {
    return Model<Matrix, SymmGroup>(
        boost::static_pointer_cast<model_impl<Matrix, SymmGroup> >(impl_)
    );
  }
