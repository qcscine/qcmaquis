...
inline std::vector<std::vector<pos_t>> resort_labels(
    const std::vector<std::vector<pos_t>>& labels,
    std::vector<pos_t> const& order, bool is_nn = false
) {
...
class CorrPermutator {
 public:
  ...
  using op_t = typename measurement<Matrix, SymmGroup>::op_t;
  using inner_t = std::pair<std::vector<op_t>, bool>;
  using value_t = std::vector<inner_t>;
...
class correlations : public measurement<Matrix, SymmGroup> {
 ...
 public:
  correlations(
      std::string const& name_, Lattice lat, op_vec const& identities_,
      op_vec const& fillings_, std::vector<std::pair<op_vec, bool>> const& ops_,
      bool half_only_, bool nearest_neighbors_only,
      positions_type positions_ = positions_type()
  )
      : base(name_),
        lattice(std::move(lat)),
        positions_first(std::move(positions_)),
    ...
    if (positions_first.size() == 0)
      std::copy(
          ...
          boost::counting_iterator<int>(lattice.size() - (ops.size() - 1)),
 ...
 protected:
  ...
  void measure_correlation(
    ...
    using maker_ptr =
        std::shared_ptr<generate_mpo::CorrMakerBase<Matrix, SymmGroup>>;

    for (int it : positions_first) {
      if (it >= lattice.size() - (ops.size() - 1))
        throw std::runtime_error(
            "cannot measure correlation with first operator at p=" +
      ...
      maker_ptr dcorr;
      if (is_nn)
        dcorr.reset(new generate_mpo::CorrMakerNN<Matrix, SymmGroup>(
            lattice, identities, fillings, ops, it
        ));
      else
        dcorr.reset(new generate_mpo::CorrMaker<Matrix, SymmGroup>(
            lattice, identities, fillings, ops, it
        ));
      ...
      std::vector<std::vector<pos_t>> num_labels =
          (order.size() > 0)
              ? detail::resort_labels(dcorr->numeric_labels(), order, is_nn)
              : dcorr->numeric_labels();
