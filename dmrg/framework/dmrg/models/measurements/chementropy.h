...
template <class Matrix>
Matrix load_vector(
    const maquis::results_map_type<typename Matrix::value_type>& map,
    std::string path
) {
  const maquis::meas_with_results_type<typename Matrix::value_type>& meas =
      map.at(path);
...
template <class Matrix>
Matrix assy_hc(
    const std::vector<std::string>& label_str, Matrix const& values, const int L
) {
  std::vector<std::pair<int, int> > labels;
...
template <class Matrix>
Matrix merge_transform(
    const std::vector<std::string>& lstr1, Matrix const& values1,
...
Matrix load_matrix_pair(
    const maquis::results_map_type<typename Matrix::value_type>& map,
    std::string path1, std::string path2, int L
) {
  const std::vector<std::vector<int> >& labels1 = map.at(path1).first;
  const std::vector<typename Matrix::value_type>& values1 =
      map.at(path1).second;
  const std::vector<std::vector<int> >& labels2 = map.at(path2).first;
  const std::vector<typename Matrix::value_type>& values2 =
      map.at(path2).second;
...
Matrix two_orb_rdm(int p, int q, EntropyData<Matrix>& data) {
  ...
  pq_dm_matrix(1, 1) = -data.Nupdown(p, 0) - data.doccdocc(p, q) +
                       ...
                       data.doccndown(p, q) - data.ndownndown(p, q) +
                       data.doccnup(p, q) - data.ndownnup(p, q);
  // O(11)O(1)
  pq_dm_matrix(2, 2) = -data.Nupdown(p, 0) - data.doccdocc(p, q) +
                       data.doccndown(p, q) + data.Nup(p, 0) +
                       data.nupdocc(p, q) - data.nupndown(p, q) +
                       data.doccnup(p, q) - data.nupnup(p, q);
  // O(16)O(1)
  pq_dm_matrix(3, 3) = data.Nupdown(p, 0) - data.doccndown(p, q) -
                       data.doccnup(p, q) + data.doccdocc(p, q);
  // O(1)O(6)
  pq_dm_matrix(4, 4) = -data.Nupdown(q, 0) - data.doccdocc(p, q) +
                       data.ndowndocc(p, q) + data.Ndown(q, 0) +
                       data.doccndown(p, q) - data.ndownndown(p, q) +
                       data.nupdocc(p, q) - data.nupndown(p, q);
  // O(6)O(6)
  pq_dm_matrix(5, 5) = data.ndownndown(p, q) - data.ndowndocc(p, q) -
                       data.doccndown(p, q) + data.doccdocc(p, q);
  // O(11)O(6)
  pq_dm_matrix(6, 6) = data.nupndown(p, q) - data.doccndown(p, q) -
                       data.nupdocc(p, q) + data.doccdocc(p, q);
  // O(16)O(11)
  pq_dm_matrix(7, 7) = data.doccndown(p, q) - data.doccdocc(p, q);
  // O(1)O(11)
  pq_dm_matrix(8, 8) = -data.Nupdown(q, 0) - data.doccdocc(p, q) +
                       data.ndowndocc(p, q) + data.nupdocc(p, q) +
                       data.Nup(q, 0) + data.doccnup(p, q) -
                       data.ndownnup(p, q) - data.nupnup(p, q);
  // O(6)O(11)
  pq_dm_matrix(9, 9) = data.ndownnup(p, q) - data.ndowndocc(p, q) -
                       data.doccnup(p, q) + data.doccdocc(p, q);
  // O(11)O(11)
  pq_dm_matrix(10, 10) = data.nupnup(p, q) - data.nupdocc(p, q) -
                         data.doccnup(p, q) + data.doccdocc(p, q);
  // O(16)O(11)
  pq_dm_matrix(11, 11) = data.doccnup(p, q) - data.doccdocc(p, q);
  // O(1)O(16)
  pq_dm_matrix(12, 12) = data.Nupdown(q, 0) - data.nupdocc(p, q) -
                         data.ndowndocc(p, q) + data.doccdocc(p, q);
  // O(1)O(16)
  ...
  // O(9)O(2)
  pq_dm_matrix(1, 4) = data.dm_down(p, q) -
                       data.transfer_down_while_up_at_1(p, q) -
                       data.transfer_down_while_up_at_2(p, q) +
                       data.transfer_down_while_up(p, q);
  pq_dm_matrix(4, 1) = pq_dm_matrix(1, 4);
  // O(9)O(2)
  pq_dm_matrix(2, 8) = data.dm_up(p, q) -
                       data.transfer_up_while_down_at_1(p, q) -
                       data.transfer_up_while_down_at_2(p, q) +
                       data.transfer_up_while_down(p, q);
  pq_dm_matrix(8, 2) = pq_dm_matrix(2, 8);
  // O(15)O(2)
  pq_dm_matrix(3, 6) = data.transfer_down_while_up_at_1(p, q) -
                       data.transfer_down_while_up(p, q);
  pq_dm_matrix(6, 3) = pq_dm_matrix(3, 6);
  // O(14)O(3)
  pq_dm_matrix(3, 9) = -data.transfer_up_while_down_at_1(p, q) +
                       data.transfer_up_while_down(p, q);
  ...
  // O(9)O(8)
  pq_dm_matrix(6, 12) = -data.transfer_up_while_down_at_2(p, q) +
                        data.transfer_up_while_down(p, q);
  pq_dm_matrix(12, 6) = pq_dm_matrix(6, 12);
  // O(8)O(9)
  pq_dm_matrix(9, 12) = data.transfer_down_while_up_at_2(p, q) -
                        data.transfer_down_while_up(p, q);
...
class EntanglementData {
 ...
 public:
  EntanglementData(std::string rfile) {
    ...
    entanglement_detail::EntropyData<Matrix> data =
        entanglement_detail::loadData<Matrix>(ar);
  ...
  EntanglementData(const maquis::results_map_type<value_type>& meas_map) {
    entanglement_detail::EntropyData<Matrix> data =
        entanglement_detail::loadData<Matrix>(meas_map);
    calculateData(data);
  }

  Matrix s1() { return s1_; }

  Matrix s2() { return s2_; }
 ...
 private:
  ...
  void calculateData(entanglement_detail::EntropyData<Matrix>& data) {
    ...
    for (int i = 0; i < L; ++i) {
      ...
      s1_(0, i) =
          -(m11(i, 0) * logm11 + m22(i, 0) * logm22 + m33(i, 0) * logm33 +
            m44(i, 0) * logm44);
    }
    ...
    for (int p = 0; p < L; ++p) {
      for (int q = (p + 1); q < L; ++q) {
        ...
        for (int j = 0; j < 16; ++j) {
          ...
          if (eval[j] > 0.0) {
            s2_(p, q) = s2_(p, q) - eval[j] * log(eval[j]);
          }
        }
        s2_(q, p) = s2_(p, q);
      }
    ...
    // }
    // std::sort(casv_sort.begin(), casv_sort.end(),
    // entanglement_detail::comp<std::pair<real_type, int> >); for(int i = 0;
    // i<L; i++){
