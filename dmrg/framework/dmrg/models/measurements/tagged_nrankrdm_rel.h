...
class TaggedNRankRDM<
    Matrix, SymmGroup, symm_traits::enable_if_u1dg_t<SymmGroup> >
 ...
 public:
  TaggedNRankRDM(
      std::string const& name_, const Lattice& lat,
      std::shared_ptr<TagHandler<Matrix, SymmGroup> > tag_handler_,
      tag_vec const& identities_, tag_vec const& fillings_,
      std::vector<scaled_bond_term> const& ops_, bool half_only_,
  ...
  void evaluate(
    ...
    //     measure_4rdm(bra_mps, ket_mps);
      ...
      throw std::runtime_error(
          "relativistic correlation measurements at the moment supported with "
          "2 and 4 operators, size is " +
 ...
 protected:
  ...
  void measure_correlation(
      MPS<Matrix, SymmGroup> const& dummy_bra_mps,
      MPS<Matrix, SymmGroup> const& ket_mps
  ) {
    // Test if a separate bra state has been specified
    bool bra_neq_ket = (dummy_bra_mps.length() > 0);
    // MPS<Matrix, SymmGroup> const & bra_mps = (bra_neq_ket) ? dummy_bra_mps :
    // ket_mps;
...
#pragma omp parallel for schedule(dynamic) firstprivate(ket_mps_local, bra_mps)
    ...
    for (std::size_t i = 0; i < positions_first.size(); ++i) {
      ...
      std::shared_ptr<TagHandler<Matrix, SymmGroup> > tag_handler_local(
          new TagHandler<Matrix, SymmGroup>(*tag_handler)
      );
      ...
      for (pos_t p2 = p1; p2 < lattice.size(); ++p2) {
        ...
        tag_vec operators(2);
        operators[0] = operator_terms[0].first[0][lattice.get_prop<
            typename SymmGroup::subcharge>("type", p1)];
        operators[1] = operator_terms[0].first[1][lattice.get_prop<
            typename SymmGroup::subcharge>("type", p2)];
        ...
        // if(measurements_details::checkpg<SymmGroup>()(term,
        // tag_handler_local, lattice))
        {
          ...
          typename MPS<Matrix, SymmGroup>::scalar_type value =
              operator_terms[0].second * expval(bra_mps, ket_mps_local, mpo);
          dct.push_back(value);
...
#pragma omp critical
#endif
      {
        ...
        std::copy(
            dct.rbegin(), dct.rend(), std::back_inserter(this->vector_results)
        );
        ...
        std::copy(
            num_labels.rbegin(), num_labels.rend(),
...
#pragma omp parallel for collapse(1) schedule(dynamic) \
    firstprivate(ket_mps_local, bra_mps)
#endif
    for (pos_t p1 = 0; p1 < lattice.size(); ++p1)
      for (pos_t p2 = 0; p2 < lattice.size(); ++p2) {
        ...
        for (pos_t p3 = ((bra_neq_ket) ? 0 : std::min(p1, p2));
             p3 < lattice.size(); ++p3) {
          ...
          for (pos_t p4 = 0; p4 < lattice.size(); ++p4) {
            if (std::max(p1, p2) > std::max(p3, p4) && std::max(p3, p4) > p2)
              continue;
            if (std::max(p1, p2) == std::max(p3, p4) && p1 < p4) continue;
            if (p3 > p1 && p2 > p4) continue;
            if (p1 == p4 && p3 == p2) continue;

            std::vector<pos_t> positions = {p1, p3, p4, p2};
            std::vector<pos_t> positionsORD = {p1, p2, p3, p4};

            // Loop over operator terms that are measured synchronously and
            // added together
            typename MPS<Matrix, SymmGroup>::scalar_type value = 0;
            bool measured = false;
            for (std::size_t synop = 0; synop < operator_terms.size();
                 ++synop) {
              tag_vec operators(4);
              operators[0] = operator_terms[synop].first[0][lattice.get_prop<
                  typename SymmGroup::subcharge>("type", p1)];
              operators[1] = operator_terms[synop].first[1][lattice.get_prop<
                  typename SymmGroup::subcharge>("type", p2)];
              operators[2] = operator_terms[synop].first[2][lattice.get_prop<
                  typename SymmGroup::subcharge>("type", p3)];
              operators[3] = operator_terms[synop].first[3][lattice.get_prop<
                  typename SymmGroup::subcharge>("type", p4)];
              term_descriptor term = generate_mpo::arrange_operators(
                  positions, operators, tag_handler_local
              );
              // check if term is allowed by symmetry
              // if(not measurements_details::checkpg<SymmGroup>()(term,
              // tag_handler_local, lattice))
              //    continue;
              measured = true;
              MPO<Matrix, SymmGroup> mpo = generate_mpo::sign_and_fill(
              ...
              // value += operator_terms[synop].second * expval(bra_mps,
              // ket_mps, mpo);
              value += (this->cast_to_real)
                           ? maquis::real(
                                 operator_terms[synop].second *
                                 expval(bra_mps, ket_mps_local, mpo)
                             )
                           : operator_terms[synop].second *
                                 expval(bra_mps, ket_mps_local, mpo);
            }
...
#endif
          {
            this->vector_results.reserve(
                this->vector_results.size() + dct.size()
            );
            std::copy(
                dct.rbegin(), dct.rend(),
                std::back_inserter(this->vector_results)
            );
            this->labels.reserve(this->labels.size() + dct.size());
            std::copy(
                lbt.rbegin(), lbt.rend(), std::back_inserter(this->labels)
            );
            this->labels_num.reserve(this->labels_num.size() + dct.size());
            std::copy(
                num_labels.rbegin(), num_labels.rend(),
