...
namespace measurements {
    ...
    protected:
                          ...
                          std::vector<pos_t> const & order = std::vector<pos_t>())
        {
            ...
            for (pos_t p2 = 0; p2 < ((bra_neq_ket) ? lattice.size() : p1+1) ; ++p2)
            {
                // if bra != ket, no transpose symmetry
                pos_t subref = 0;
                if (bra_neq_ket)
...
//              maquis::cout << "bra==ket? " << bra_neq_ket << " subref --> " << subref << std::endl;
                    ...
                    for (pos_t p4 = 0; p4 < lattice.size(); ++p4)
                    {
                        ...
                        { 
                            ...
                            if (bra_neq_ket)
                                ...
                                subsubref = std::min(p4, p5);

//                          maquis::cout << "bra==ket? " << bra_neq_ket << " subsubref --> " << subsubref << std::endl;
