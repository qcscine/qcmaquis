...
int to_integer(alps::half_integer<T> const& qn_value) {
  return qn_value.get_twice(
  );  // always works with double QN, so that spin-1/2 and spin-1 are compatible
}
}  // namespace detail

template <class SymmGroup>
typename SymmGroup::charge state_to_charge(
...
template <class SymmGroup>
typename SymmGroup::charge init_charge(
...
class symmetric_basis_descriptor {
 public:
  ...
  symmetric_basis_descriptor(
      alps::SiteBasisDescriptor<I> const& b, qn_map_type const& all_conserved_qn
  ) {
    ...
    for (size_t i = 0; i < states.size(); ++i) {
      state_charge[i] =
          state_to_charge<SymmGroup>(states[i], b, all_conserved_qn);

      size_t ci = phys_.position(state_charge[i]);
      if (ci < phys_.size())
        phys_[ci].second += 1;
      else
        ci = phys_.insert(std::make_pair(state_charge[i], 1));
  ...
  coord_t coords(size_t i) const {
    return coord_t(phys_[state_index_[i]].first, state_offset_[i]);
  }
