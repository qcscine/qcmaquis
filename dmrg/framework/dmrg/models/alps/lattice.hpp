...
class alps_lattice : public lattice_impl {
 public:
  ...
  alps_lattice(const alps::Parameters& p) : parms(p), graph(parms) {
    // storing lattice informations
    forward_.resize(size());
    backward_.resize(size());

    for (graph_type::bond_iterator it = graph.bonds().first;
         it != graph.bonds().second; ++it) {
  ...
  std::vector<pos_t> forward(pos_t p) const { return forward_[p]; }
  std::vector<pos_t> all(pos_t p) const {
    std::vector<pos_t> ret = forward_[p];
    std::copy(
        backward_[p].begin(), backward_[p].end(), std::back_inserter(ret)
    );
    return ret;
  }

  pos_t size() const { return graph.num_sites(); }
  ...
  std::any get_prop_(std::string const& property, std::vector<pos_t> const& pos)
    ...
    if (property == "label" && pos.size() == 1)
      ...
      return std::any(alps::bond_label(
          graph.graph(), graph.bond(bond_index_map[pos[0]][pos[1]])
      ));
      ...
      return std::any(static_cast<int>(
          graph.bond_type(graph.bond(bond_index_map[pos[0]][pos[1]]))
      ));
    else if (property == "wraps_pbc" && pos.size() == 2)
      return std::any(static_cast<bool>(std::get(
          alps::boundary_crossing_t(), graph.graph(),
  ...
  const graph_type& alps_graph() const { return graph; }
