 ...
 * The object is basically a wrapper around a string --> vector<std::any>
 * object. The string identifies the specific result of the simulation. The
 * vector<std::any> object is represented, in practice, by a pointer to a
...
class results_collector {
 private:
  /** @brief Virtual class representing an object that can hold generic values
   * (see cpp for implementation) */
  class collector_impl_base;

  /**
   * @brief Actual implementation based on a std::any vector
   * Note that the template parameter T represents the type used for the casting
   * of std::any.
   */
  template <class T>
