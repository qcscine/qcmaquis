...
namespace parallel {
    ...
    struct traits {
        ...
        static resource_iterator permute(int k, const std::vector<int>& s, size_t gran){ return ambient::scope::permute(k, s, gran); }
        static size_t size(){ return ambient::scope::size(); }
        template<typename T>
        static resource_iterator to_iterator(T offset){ return (ambient::scope::begin() + offset); }
        template<typename T>
