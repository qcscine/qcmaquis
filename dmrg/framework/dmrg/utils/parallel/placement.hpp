...
inline void make_consistent(
    std::vector<int>& p_new, int b2, std::vector<int>& e_new,
...
std::vector<int> get_left_placement(
    const MPOTensor<Matrix, SymmGroup>& mpo,
    const std::vector<int>& placement_l_old, const std::vector<int>& placement_r
) {
  ...
  for (int b1 = 0; b1 < placement_l_old.size(); b1++)
    for (int b2 = 0; b2 < placement_r.size(); b2++) {
      if (mpo.has(b1, b2)) {
        make_consistent(
            placement_l, b1, ts_exceptions_l, placement_r, b2, ts_exceptions_r
        );
      }
    }
...
template <class Matrix, class SymmGroup>
std::vector<int> get_right_placement(
    const MPOTensor<Matrix, SymmGroup>& mpo,
    const std::vector<int>& placement_l, const std::vector<int>& placement_r_old
) {
  std::vector<int> ts_exceptions_l, ts_exceptions_r;
  ...
  for (int b1 = 0; b1 < placement_l.size(); b1++)
    for (int b2 = 0; b2 < placement_r_old.size(); b2++) {
      if (mpo.has(b1, b2)) {
        make_consistent(
            placement_r, b2, ts_exceptions_r, placement_l, b1, ts_exceptions_l
        );
      }
    }
...
template <class Matrix, class SymmGroup>
std::vector<std::vector<int> > construct_placements(
    const MPO<Matrix, SymmGroup>& mpo
) {
  std::vector<std::vector<int> > placements(mpo.length() + 1);
  std::vector<std::pair<std::vector<int>, std::vector<int> > > exceptions(
      mpo.length() + 1
  );
  ...
  for (int s = 0; s < mpo.length(); s++) {
    ...
    for (int b1 = 0; b1 < placements[s].size(); b1++)
      for (int b2 = 0; b2 < placements[s + 1].size(); b2++) {
        if (mpo[s].has(b1, b2)) {
          make_consistent(
              placements[s + 1], b2, exceptions[s + 1].second, placements[s],
              b1, exceptions[s].first
          );
        }
      }
