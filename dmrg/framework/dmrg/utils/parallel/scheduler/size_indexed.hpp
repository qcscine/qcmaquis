...
class scheduler_size_indexed {
 public:
  ...
  struct index {
    ...
    void set(const Matrix& m) {
      ...
      for (size_t k = 0; k < loop_max; ++k)
        tmp[k] = std::make_pair(
            m.left_basis()[k].second * m.right_basis()[k].second, k
        );
      std::sort(tmp.begin(), tmp.end());

      data.resize(loop_max);
      for (size_t k = 0; k < loop_max; ++k) data[tmp[k].second] = k;
    }
    ...
    friend void swap(index& x, index& y) { std::swap(x.data, y.data); }
  ...
  bool propagate() const { return !i.empty(); }
  index& i;
};
}  // namespace parallel
