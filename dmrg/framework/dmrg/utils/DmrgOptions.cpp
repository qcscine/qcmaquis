...
DmrgOptions::DmrgOptions(int argc, char** argv) {
  ...
  if (argc) {
    ...
    )("time-limit,T", po::value<double>(&time_limit)->default_value(0),
      "time limit for the simulation")(
        "parms-file", po::value<std::string>(&parms_fname),
        "input dmrg parameters"
    )("model-file", po::value<std::string>(&model_fname),
