...
struct get_ {
  T operator()(std::string const& val) {
    try {
      ...
      maquis::cerr << "Exception raised casting " << val << " to type "
...
class proxy {
 public:
  ...
  bool operator==(proxy const& rhs) const { return (val_ == rhs.val_); }

  bool operator==(const char* rhs) const { return (val_ == std::string(rhs)); }
  ...
  std::string const& str() const { return val_; }

  const char* c_str() const { return val_.c_str(); }

  bool empty() const { return val_.empty(); }

#define PROXY_BINARY_OP_DECL(U, OP, T)                                         \
  friend U operator OP(T lhs, proxy const& rhs) { return lhs OP rhs.as<T>(); } \
  friend U operator OP(proxy const& rhs, T lhs) { return rhs.as<T>() OP lhs; }
