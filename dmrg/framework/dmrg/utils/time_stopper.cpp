...
time_stopper::time_stopper(double timelimit)
    : limit(timelimit), start(std::chrono::high_resolution_clock::now()) {}

bool time_stopper::valid() const { return limit.count(); }

bool time_stopper::operator()() const {
#ifdef USE_AMBIENT
  ambient::future<double> flag;
  ambient::async(
      [&](ambient::future<double>& r) {
        r.set(
            limit.count() > 0 &&
            std::chrono::high_resolution_clock::now() > start + limit
        );
      },
      flag
  );
