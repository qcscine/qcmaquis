...
class ts_optimize : public optimizer_base<Matrix, SymmGroup, Storage> {
 public:
  ...
  ts_optimize(
      MPS<Matrix, SymmGroup>& mps_, MPO<Matrix, SymmGroup> const& mpo_,
      BaseParameters& parms_, std::function<bool()> stop_callback_,
      const Lattice& lat, int initial_site_ = 0
  )
      : base(
            mps_, mpo_, parms_, stop_callback_,
  ...
  void sweep(int sweep, OptimizeDirection d = Both) {
    ...
    for (; _site < 2 * L - 2; ++_site) {
      ...
      maquis::cout << "Sweep " << sweep << ", optimizing sites " << site1
                   << " and " << site2 << std::endl;
      ...
      SiteProblem<Matrix, SymmGroup> sp(
          left_[site1], right_[site2 + 1], ts_cache_mpo[site1]
      );

      /// Compute orthogonal vectors
      std::vector<MPSTensor<Matrix, SymmGroup> > ortho_vecs(base::northo);
      for (int n = 0; n < base::northo; ++n) {
        TwoSiteTensor<Matrix, SymmGroup> ts_ortho(
            base::ortho_mps[n][site1], base::ortho_mps[n][site2]
        );
        ortho_vecs[n] = contraction::site_ortho_boundaries(
            twin_mps, ts_ortho.make_mps(), base::ortho_left_[n][site1],
...
#ifndef NDEBUG
      // Caution: this is an O(L) operation, so it really should be done only in
      // debug mode
      for (int n = 0; n < base::northo; ++n)
        maquis::cout << "MPS overlap: " << overlap(mps, base::ortho_mps[n])
                     << std::endl;
#endif

      {
        ...
        maquis::cout << "Energy " << lr << " "
                     << res.first + mpo.getCoreEnergy() << std::endl;
      ...
      if (!parms.is_set("PreBO_MaxBondDimVector"))
        ...
        auto m1 = lat_.template get_prop<size_t>(
            "Mmax", lat_.template get_prop<int>("type", site1)
        );
        auto m2 = lat_.template get_prop<size_t>(
            "Mmax", lat_.template get_prop<int>("type", site2)
        );
        Mmax = (m1 > m2) ? m1 : m2;
      ...
      if (lr == +1) {
        ...
        if (parms["twosite_truncation"] == "svd")
          std::tie(mps[site1], mps[site2], trunc) =
              tst.split_mps_l2r(Mmax, cutoff);
        else
          std::tie(mps[site1], mps[site2], trunc) = tst.predict_split_l2r(
              Mmax, cutoff, alpha, left_[site1], mpo[site1], true
          );
        END_TIMING("TRUNC")
        tst.clear();
        ...
        if (site1 != L - 2) Storage::drop(right_[site2 + 1]);
        ...
        {
          parallel::guard proc(scheduler_mps(site1));
          storage::migrate(mps[site1]);
        }
        {
          parallel::guard proc(scheduler_mps(site2));
          storage::migrate(mps[site2]);
        }
      }
      if (lr == -1) {
        // Write back result from optimization
        ...
        if (parms["twosite_truncation"] == "svd")
          std::tie(mps[site1], mps[site2], trunc) =
              tst.split_mps_r2l(Mmax, cutoff);
        else
          std::tie(mps[site1], mps[site2], trunc) = tst.predict_split_r2l(
              Mmax, cutoff, alpha, right_[site2 + 1], mpo[site2], true
          );
        END_TIMING("TRUNC")
        tst.clear();
        ...
        if (site1 != 0) Storage::drop(left_[site1]);
      ...
      std::chrono::high_resolution_clock::time_point sweep_then =
          std::chrono::high_resolution_clock::now();
      double elapsed =
          std::chrono::duration<double>(sweep_then - sweep_now).count();
      maquis::cout << "Sweep has been running for " << elapsed << " seconds."
                   << std::endl;

      if (stop_callback()) throw dmrg::time_limit(sweep, _site + 1);
