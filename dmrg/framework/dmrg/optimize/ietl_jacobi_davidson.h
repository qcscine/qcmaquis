...
solve_ietl_jcd(
  ...
  for (int n = 0; n < ortho_vecs.size(); ++n) {
    ...
    if (ortho_vecs[n].scalar_norm() > thresholdForCompleteness) {
      ...
      maquis::cout
          << "State " << n
          << " neglected because the corresponding boundary is too small"
          << std::endl;
    }
  }
  // Checks if the number of constraints is > than the actual size of the vector
  // space
  auto tmp = initial;
  ...
  if (!skipOptimization) {
    ...
    if (verbose) {
      ...
      for (int n = 0; n < ortho_vecs_local.size(); ++n) {
        //            maquis::cout << "Ortho norm " << n << ": " <<
        //            ietl::two_norm(ortho_vecs[n]) << std::endl;
        maquis::cout << "Input <MPS|O[" << n
                     << "]> : " << ietl::dot(initial, ortho_vecs_local[n])
                     << std::endl;
        maquis::cout << "Output <MPS|O[" << n
                     << "]> : " << ietl::dot(r0.second, ortho_vecs_local[n])
                     << std::endl;
      }
    }
  } else {
    if (verbose) {
      maquis::cout << "Vector space too small, diagonalization skipped"
                   << std::endl;
    }
