...
class ref_diag {
 public:
  void operator()(
      SiteProblem<Matrix, SymmGroup> const& H, MPSTensor<Matrix, SymmGroup> x
  ) {}
};
 ...
 public:
  void operator()(
    ...
    block_matrix<Matrix, SymmGroup> ret2 =
        contraction::SU2::diagonal_hamiltonian(H.left, H.right, H.mpo, x);

    for (size_t b = 0; b < bm.n_blocks(); ++b) {
      maquis::cout << bm.basis().left_charge(b) << bm.basis().right_charge(b)
                   << std::endl;
      for (size_t i = 0; i < num_rows(bm[b]); ++i)
        for (size_t j = 0; j < num_cols(bm[b]); ++j) {
          ...
          ietl::mult(H, x, prod);
          maquis::cout << "  " << i << "," << j << "  " << prod.data()[b](i, j)
                       << " " << ret2[b](i, j) << std::endl;
          if (std::abs(prod.data()[b](i, j) - ret2[b](i, j)) > 1e-12)
            throw std::runtime_error("diagonal wrong\n");
          bm[b](i, j) = 0;
        }
    }
  }
 ...
 public:
  MultDiagonal(SiteProblem<Matrix, SymmGroup> const& H, vector_type const& x) {
    throw std::runtime_error(
        "Davidson only implemented for spin-adapted Hamiltonians\n"
    );
  }
...
template <class Matrix, class SymmGroup>
std::pair<double, MPSTensor<Matrix, SymmGroup>> solve_ietl_davidson(
  ...
  for (int n = 1; n < ortho_vecs.size(); ++n)
    for (int n0 = 0; n0 < n; ++n0)
      ortho_vecs[n] -= ietl::dot(ortho_vecs[n0], ortho_vecs[n]) /
                       ietl::dot(ortho_vecs[n0], ortho_vecs[n0]) *
                       ortho_vecs[n0];
  ...
  contraction::ContractionGrid<Matrix, SymmGroup>::iterate_reduction_layout(
      0, params["ietl_jcd_maxiter"]
  );

  for (int n = 0; n < ortho_vecs.size(); ++n) {
    maquis::cout << "Input <MPS|O[" << n
                 << "]> : " << ietl::dot(initial, ortho_vecs[n]) << std::endl;
  }

  std::pair<double, Vector> r0 =
      jd.calculate_eigenvalue(initial, jcd_gmres, mdiag, iter);

  for (int n = 0; n < ortho_vecs.size(); ++n)
    maquis::cout << "Output <MPS|O[" << n
                 << "]> : " << ietl::dot(r0.second, ortho_vecs[n]) << std::endl;

  maquis::cout << "Davidson used " << iter.iterations() << " iterations."
               << std::endl;
