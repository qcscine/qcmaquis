...
class MultiVecTraits<double, IETLMultMv<Vector> > {
 ...
 public:
  ...
  static Teuchos::RCP<MV> CloneCopy(
      const MV& mv, const std::vector<int>& index
  ) {
  ...
  static Teuchos::RCP<MV> CloneViewNonConst(
      MV& mv, const std::vector<int>& index
  ) {
    MV* ret = new MV(index.size());
    for (int k = 0; k < index.size(); ++k) ret->data[k] = mv->data[k];
    return Teuchos::RCP<MV>(ret);
  }

  static Teuchos::RCP<const MV> CloneView(
      const MV& mv, const std::vector<int>& index
  ) {
    MV* ret = new MV(index.size());
    for (int k = 0; k < index.size(); ++k) ret->data[k] = mv->data[k];
  ...
  static int GetVecLength(const MV& mv) { return 0; }

  static int GetNumberVecs(const MV& mv) { return mv.data.size(); }

  static void MvTimesMatAddMv(
      const ScalarType alpha, const MV& A,
  ...
  static void MvAddMv(
      const ScalarType alpha, const MV& A, const ScalarType beta, const MV& B,
      MV& mv
  ) {
    UndefinedMultiVecTraits<ScalarType, MV>::notDefined();
  }
  ...
  static void MvTransMv(
      const ScalarType alpha, const MV& A, const MV& mv,
  ...
  static void MvInit(
      MV& mv, const ScalarType alpha = Teuchos::ScalarTraits<ScalarType>::zero()
  ) {
    UndefinedMultiVecTraits<ScalarType, MV>::notDefined();
  }
