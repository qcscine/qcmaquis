...
namespace ietl
{
    ...
    class jacobi_davidson_modified_mo : public jacobi_davidson_modified<MATRIX, VS, SymmGroup, ITER>
    {
    public:
        typedef jacobi_davidson_modified<MATRIX, VS, SymmGroup, ITER> base;
        ...
        jacobi_davidson_modified_mo(MATRIX& matrix, VS& vec, CorrectionEquation* corrector,
                                    ...
                                    const std::vector<magnitude_type>& omega_vec, const pov_vec_type& pov,
                ...
                : base::jacobi_davidson_modified(matrix, vec, corrector, micro_iterator, finalizer, ortho, omega_vec, nmin,
                                                 nmax, n_block, block_thresh, site1, site2, order, sa_alg, n_lanczos,
                                                 do_chebychev, chebyshev_shift, do_H_squared, reshuffle_variance, track_variance,
                                                 is_folded, energy_thresh)
                , pov_(pov) , n_maxov_(n), root_homing_type_(root_homing_type),
                  side_tofollow_(side_tofollow), homing_ratio_(homing_ratio)
        {
            // Set the bound object
            if (side_tofollow == 0){
                bounds.do_lower = true ;
                bounds.do_upper = true ;
            } else if (side_tofollow == 1 ) {
                bounds.do_lower = true ;
            } else if (side_tofollow == -1 ) {
                bounds.do_upper = true ;
            } else {
             ...
             jacobi_davidson_modified_mo<MATRIX, VS, ITER, OtherMatrix, SymmGroup>::generate_property()
    {
        ...
        size_t dim = candidates_collector_.size() ;
    ...
    template<class MATRIX, class VS, class ITER, class OtherMatrix, class SymmGroup>
    double jacobi_davidson_modified_mo<MATRIX, VS, ITER, OtherMatrix, SymmGroup>::compute_overlap
            (const vector_type& u,
             const vector_type& uA)
    {
        double ret ;
        vector_type tmp_u  = u/ietl::two_norm(u) ;
        vector_type tmp_uA = uA/ietl::two_norm(uA) ;
        // Calculates the two overlaps
        if (root_homing_type_ == 4) {
            // Sign artificially changes to make the vector with the smaller variance have the largest property
            ret = -this->compute_variance(tmp_u, false);
        } else {
    ...
    //
    template<class MATRIX, class VS, class ITER, class OtherMatrix, class SymmGroup>
    void jacobi_davidson_modified_mo<MATRIX, VS, ITER, OtherMatrix, SymmGroup>::set_interval(const std::size_t& dim)
    {
        // Both sides
    ...
    //
    template<class MATRIX, class VS, class ITER, class OtherMatrix, class SymmGroup>
    void jacobi_davidson_modified_mo<MATRIX, VS, ITER, OtherMatrix, SymmGroup>::print_header_table() {
        print_endline() ;
    ...
    //
    template<class MATRIX, class VS, class ITER, class OtherMatrix, class SymmGroup>
    void jacobi_davidson_modified_mo<MATRIX, VS, ITER, OtherMatrix, SymmGroup>::print_endline() {
        std::cout << "-----------+-----------+-------------+---------------+---------------+-------------" << std::endl ;
    } ;
    //
    template<class MATRIX, class VS, class ITER, class OtherMatrix, class SymmGroup>
    void jacobi_davidson_modified_mo<MATRIX, VS, ITER, OtherMatrix, SymmGroup>::print_newline_table
            (const size_t& i, const double& error, const magnitude_type& en, const size_t& idx, const bool& converged)
    {
        ...
        n = sprintf(buf, "%5d      |%5d      | %1.4E  |  %6.5f  |    %4.4f     | %5d", a, this->get_dim(V_), error, en,
                    fabs(eigen_collector_[idx].property_), static_cast<int>(i_homing_selected_));
        std::cout << buf ;
        if (converged)
            std::cout << " CONVERGED" ;
        std::cout << std::endl;
