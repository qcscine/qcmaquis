...
*
*****************************************************************************/
#ifndef MODIFIED_FINALIZER_HPP
#define MODIFIED_FINALIZER_HPP

template<class Matrix, class VecSpace>
class ModifiedFinalizer : public Finalizer<Matrix, VecSpace> {
public:
    ...
    ModifiedFinalizer() { is_si_ = true; };

    magnitude_type compute_energy(size_t i_state, size_t idx)
    {
        return this->get_omega() - 1./this->get_eigen(idx) ;
    }
    ...
    {
        return (this->get_u(idx)) - (this->get_uA(idx)) * this->get_eigen(idx)  ;
    }
};
