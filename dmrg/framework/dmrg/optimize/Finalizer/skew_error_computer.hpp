...
#ifndef MAQUIS_DMRG_SKEW_ERROR_HPP
#define MAQUIS_DMRG_SKEW_ERROR_HPP
...
class SkewError : public ErrorComputer<MATRIX, VecSpace> {
public:
    // Types definition
    ...
    vector_type compute_error(Finalizer<MATRIX, VecSpace>* finalizer,
                              ...
                              size_t const& idx)
    {
        vector_type jnk = ((finalizer->get_uA(idx))) - (finalizer->get_eigen(idx)) * ((finalizer->get_u(idx))) ;
        jnk /= ietl::two_norm(((finalizer->get_u(idx)))) ;
        return jnk ;
    }
};
