...
class optimizer_base {
 ...
 public:
  optimizer_base(
      ...
      BaseParameters& parms_, std::function<bool()> stop_callback_, int site = 0
  )
      : mps(mps_),
        mpo(mpo_),
        parms(parms_),
    ...
    maquis::cout << "Expecting " << northo << " states to orthogonalize to."
                 << std::endl;
    ...
    if (parms_.is_set("ortho_states")) {
      ...
      for (int n = 0; n < northo; ++n) {
        maquis::cout << "Loading ortho state " << n << " from " << files[n]
                     << std::endl;
        ...
        maquis::checks::right_end_check(
            files[n], ortho_mps[n], mps[mps.length() - 1].col_dim()[0].first
        );

        maquis::cout << "Right end: "
                     << ortho_mps[n][mps.length() - 1].col_dim() << std::endl;
      }
    }
 ...
 protected:
  inline void boundary_left_step(MPO<Matrix, SymmGroup> const& mpo, int site) {
    left_[site + 1] = contr::overlap_mpo_left_step(
        mps[site], mps[site], left_[site], mpo[site]
    );
    for (int n = 0; n < northo; ++n) {
      ortho_left_[n][site + 1] = contr::overlap_left_step(
          mps[site], ortho_mps[n][site], ortho_left_[n][site]
      );
    }
  }
  ...
  inline void boundary_right_step(MPO<Matrix, SymmGroup> const& mpo, int site) {
    right_[site] = contr::overlap_mpo_right_step(
        mps[site], mps[site], right_[site + 1], mpo[site]
    );
    for (int n = 0; n < northo; ++n) {
      ortho_right_[n][site] = contr::overlap_right_step(
          mps[site], ortho_mps[n][site], ortho_right_[n][site + 1]
      );
    }
  }
  ...
  double get_cutoff(int sweep) const {
    ...
    if (sweep >= parms.template get<int>("ngrowsweeps")) {
      ...
      cutoff = log_interpolate(
          parms.template get<double>("truncation_initial"),
          parms.template get<double>("truncation_main"),
          parms.template get<int>("ngrowsweeps"), sweep
      );
    }
    return cutoff;
  }
  ...
  std::size_t get_Mmax(int sweep) const {
    ...
    if (parms.is_set("sweep_bond_dimensions")) {
      std::vector<std::size_t> ssizes =
          parms.template get<std::vector<std::size_t> >("sweep_bond_dimensions"
          );
