 ...
 public:
  ...
  void sweep(int sweep, OptimizeDirection d = Both) {
    ...
    for (; _site < 2 * L; ++_site) {
      ...
      maquis::cout << "Sweep " << sweep << ", optimizing site " << site
                   << std::endl;
      ...
      // SiteProblem<Matrix, SymmGroup> sp(mps[site], left_[site],
      // right_[site+1], mpo[site]);

      std::chrono::high_resolution_clock::time_point now, then;

      std::pair<
          typename MPSTensor<Matrix, SymmGroup>::value_type,
          MPSTensor<Matrix, SymmGroup> >
          res;
      SiteProblem<Matrix, SymmGroup> sp(
          left_[site], right_[site + 1], mpo[site]
      );
      ...
      for (int n = 0; n < base::northo; ++n)
        ortho_vecs[n] = contraction::site_ortho_boundaries(
            mps[site], base::ortho_mps[n][site], base::ortho_left_[n][site],
      ...
      double alpha;
      int ngs = parms.template get<int>("ngrowsweeps"),
      ...
      if (!parms.is_set("PreBO_MaxBondDimVector"))
        ...
        Mmax = lat_.template get_prop<size_t>(
            "Mmax", lat_.template get_prop<int>("type", site)
        );
      ...
      if (lr == +1) {
        if (site < L - 1) {
          ...
          trunc = mps.grow_l2r_sweep(
              mpo[site], left_[site], right_[site + 1], site, alpha, cutoff,
              Mmax
          );
        } else {
          block_matrix<Matrix, SymmGroup> t =
              mps[site].normalize_left(DefaultSolver());
          if (site < L - 1) mps[site + 1].multiply_from_left(t);
        }
        ...
        if (site > 0) {
          ...
          trunc = mps.grow_r2l_sweep(
              mpo[site], left_[site], right_[site + 1], site, alpha, cutoff,
              Mmax
          );
        } else {
          block_matrix<Matrix, SymmGroup> t =
              mps[site].normalize_right(DefaultSolver());
