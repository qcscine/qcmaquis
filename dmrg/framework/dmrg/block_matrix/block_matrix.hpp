...
block_matrix<Matrix, SymmGroup>::block_matrix(
    Index<SymmGroup> const& rows, Index<SymmGroup> const& cols
) {
  assert(rows.size() == cols.size());

  basis_.resize(rows.size());
  for (size_type k = 0; k < rows.size(); ++k) {
    basis_[k] = typename DualIndex<SymmGroup>::value_type(
        rows[k].first, cols[k].first, rows[k].second, cols[k].second
    );
  }
...
block_matrix<Matrix, SymmGroup>::block_matrix(
    block_matrix<OtherMatrix, SymmGroup> const& rhs
)
    : iter_index(rhs.iter_index),
      size_index(rhs.size_index),
      basis_(rhs.basis()) {
...
template <class Matrix, class SymmGroup>
block_matrix<Matrix, SymmGroup>& block_matrix<Matrix, SymmGroup>::operator=(
    block_matrix rhs
) {
  swap(*this, rhs);
  return *this;
...
template <class OtherMatrix>
block_matrix<Matrix, SymmGroup>& block_matrix<Matrix, SymmGroup>::operator=(
    const block_matrix<OtherMatrix, SymmGroup>& rhs
) {
  basis_ = rhs.basis_;
  size_index = rhs.size_index;
  iter_index = rhs.iter_index;
...
template <class Matrix, class SymmGroup>
typename block_matrix<Matrix, SymmGroup>::size_type block_matrix<
    Matrix, SymmGroup>::insert_block(Matrix const& mtx, charge c1, charge c2) {
  assert(!has_block(c1, c2));
  size_type i1 = basis_.insert(typename DualIndex<SymmGroup>::value_type(
      c1, c2, num_rows(mtx), num_cols(mtx)
  ));
...
template <class Matrix, class SymmGroup>
typename block_matrix<Matrix, SymmGroup>::size_type block_matrix<
    Matrix, SymmGroup>::insert_block(Matrix* mtx, charge c1, charge c2) {
  assert(!has_block(c1, c2));
  size_type i1 = basis_.insert(typename DualIndex<SymmGroup>::value_type(
      c1, c2, num_rows(*mtx), num_cols(*mtx)
  ));
...
Index<SymmGroup> block_matrix<Matrix, SymmGroup>::left_basis() const {
  ...
  std::transform(basis_.begin(), basis_.end(), ret.begin(), [](const auto& e) {
    return std::make_pair(e.lc, e.ls);
  });
  return ret;
}
...
Index<SymmGroup> block_matrix<Matrix, SymmGroup>::right_basis() const {
  Index<SymmGroup> ret(basis_.size());
  std::transform(basis_.begin(), basis_.end(), ret.begin(), [](const auto& e) {
    return std::make_pair(e.rc, e.rs);
  });
...
template <class Matrix, class SymmGroup>
DualIndex<SymmGroup> const& block_matrix<Matrix, SymmGroup>::basis() const {
  return basis_;
}

template <class Matrix, class SymmGroup>
typename block_matrix<Matrix, SymmGroup>::charge
block_matrix<Matrix, SymmGroup>::get_left_charge(std::size_t idx) const {
...
template <class Matrix, class SymmGroup>
typename block_matrix<Matrix, SymmGroup>::charge
block_matrix<Matrix, SymmGroup>::get_right_charge(std::size_t idx) const {
  assert(idx < this->n_blocks());
  return basis_[idx].rc;
}

template <class Matrix, class SymmGroup>
typename block_matrix<Matrix, SymmGroup>::size_type
block_matrix<Matrix, SymmGroup>::get_left_dim(std::size_t idx) const {
  assert(idx < this->n_blocks());
...
template <class Matrix, class SymmGroup>
typename block_matrix<Matrix, SymmGroup>::size_type
block_matrix<Matrix, SymmGroup>::get_right_dim(std::size_t idx) const {
  assert(idx < this->n_blocks());
  return basis_[idx].rs;
}

template <class Matrix, class SymmGroup>
typename Matrix::size_type block_matrix<Matrix, SymmGroup>::n_blocks() const {
  return data_.size();
}
...
template <class Matrix, class SymmGroup>
void block_matrix<Matrix, SymmGroup>::shift_basis(
    typename block_matrix<Matrix, SymmGroup>::charge diff
) {
  basis_.shift(diff);
}
...
template <class Matrix, class SymmGroup>
typename block_matrix<Matrix, SymmGroup>::size_type
block_matrix<Matrix, SymmGroup>::find_block(charge r, charge c) const {
...
template <class Matrix, class SymmGroup>
bool block_matrix<Matrix, SymmGroup>::has_block(
    std::pair<charge, size_type> const& r, std::pair<charge, size_type> const& c
) const {
  return has_block(r.first, c.first);
}

template <class Matrix, class SymmGroup>
typename Matrix::value_type& block_matrix<Matrix, SymmGroup>::operator()(
    std::pair<charge, size_type> const& r, std::pair<charge, size_type> const& c
) {
...
template <class Matrix, class SymmGroup>
typename Matrix::value_type const& block_matrix<Matrix, SymmGroup>::operator()(
    std::pair<charge, size_type> const& r, std::pair<charge, size_type> const& c
) const {
  return data_[basis_.position(r.first, c.first)](r.second, c.second);
}
...
// template<class Matrix, class SymmGroup>
// void block_matrix<Matrix, SymmGroup>::remove_rows_from_block(size_type block,
// size_type r, size_type k) { // we should add an assert block < data_.size()
...
// Remove by Tim 06/08/2012, presently not used in any DMRG/TE code
// template<class Matrix, class SymmGroup>
// void block_matrix<Matrix, SymmGroup>::remove_cols_from_block(size_type block,
// size_type r, size_type k) { // we should add an assert block < data_.size()
...
block_matrix<Matrix, SymmGroup>::operator*=(const scalar_type& v) {
...
block_matrix<Matrix, SymmGroup>::trace() const {
  std::vector<scalar_type> vt;
  vt.reserve(data_.size());
  std::transform(
      data_.begin(), data_.end(), back_inserter(vt), utils::functor_trace()
  );
...
block_matrix<Matrix, SymmGroup>::norm() const {
  parallel::scheduler_balanced_iterative scheduler(*this);
...
std::ostream& operator<<(
    std::ostream& os, block_matrix<Matrix, SymmGroup> const& m
) {
...
template <class Matrix, class SymmGroup>
void block_matrix<Matrix, SymmGroup>::match_and_add_block(
    Matrix const& mtx, charge c1, charge c2
) {
  ...
  if (block_exists) {
    if (num_rows(mtx) == num_rows((*this)[match]) &&
      ...
      std::size_t maxrows = std::max(num_rows(mtx), num_rows((*this)[match]));
      std::size_t maxcols = std::max(num_cols(mtx), num_cols((*this)[match]));

      Matrix cpy(mtx
      );  // only in this case do we need to copy the argument matrix
...
template <class Matrix, class SymmGroup>
void block_matrix<Matrix, SymmGroup>::resize_block(
    charge r, charge c, size_type new_r, size_type new_c, bool pretend
) {
...
template <class Matrix, class SymmGroup>
void block_matrix<Matrix, SymmGroup>::resize_block(
    size_type pos, size_type new_r, size_type new_c, bool pretend
) {
  if (!pretend) {
...
void block_matrix<Matrix, SymmGroup>::load(Archive& ar) {
  ...
  for (std::size_t s = 0; s < r_.size(); ++s) {
    basis_[s] = typename DualIndex<SymmGroup>::value_type(
        r_[s].first, c_[s].first, r_[s].second, c_[s].second
    );
  }

  data_.clear();
  if (alps::is_complex<typename Matrix::value_type>() &&
      !ar.is_complex("data_")) {
#ifdef USE_AMBIENT
    printf("ERROR: LOAD COMPLEX DATA NOT TESTED!\n\n");
#endif
    using LoadMatrix = typename alps::numeric::matrix<
        typename alps::numeric::real_type<typename Matrix::value_type>::type>;
    std::vector<LoadMatrix> tmp;
    ar["data_"] >> tmp;
    for (typename std::vector<LoadMatrix>::const_iterator it = tmp.begin();
         it != tmp.end(); ++it) {
    ...
    for (typename std::vector<Matrix>::const_iterator it = tmp.begin();
         it != tmp.end(); ++it) {
      data_.push_back(new Matrix(*it));
    }
  }
...
template <class Matrix, class SymmGroup>
void block_matrix<Matrix, SymmGroup>::reserve(
    charge c1, charge c2, std::size_t r, std::size_t c
) {
  if (this->has_block(c1, c2)) {
    ...
    size_type i1 =
        basis_.insert(typename DualIndex<SymmGroup>::value_type(c1, c2, r, c));
...
bool block_matrix<Matrix, SymmGroup>::reasonable() const {
  for (size_t k = 0; k < n_blocks(); ++k) {
    if (num_rows((*this)[k]) != basis_[k].ls ||
        num_cols((*this)[k]) != basis_[k].rs) {
      return false;
    }
  }
  return true;
...
template <class Matrix, class SymmGroup>
typename block_matrix<Matrix, SymmGroup>::scalar_type
block_matrix<Matrix, SymmGroup>::scalar_overlap(
    block_matrix<Matrix, SymmGroup> const& rhs
) const {
...
//  ADD_BLOCK_TO_ROW
// +----------------+
// "Merges" a given symmetry block of a block_matrix given in input with the
// corresponding block of the object

template <class Matrix, class SymmGroup>
void block_matrix<Matrix, SymmGroup>::add_block_to_row(
    block_matrix& rhs, charge r, charge c
) {
  // Check coherence in the dimensions of the rows (the columns might be, in
  // principle, different
  ...
  if (match < this->n_blocks()) {
    ...
    //
    this->resize_block(
        r, c, num_row_original + num_row_toadd, num_cols_common, false
    );
...
// +-------------------+
// "Merges" a given symmetry block of a block_matrix given in input with the
// corresponding block of the object. If the requested symmetry block is not
// present,

template <class Matrix, class SymmGroup>
void block_matrix<Matrix, SymmGroup>::add_block_to_column(
    block_matrix& rhs, charge r, charge c
) {
  // Check coherence in the dimensions of the rows (the columns might be, in
  // principle, different
  ...
  if (match < this->n_blocks()) {
    ...
    //
    this->resize_block(
        r, c, num_rows((*this)(r, c)), num_col_toadd + num_col_original, false
    );
