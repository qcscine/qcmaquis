...
public:
    ...
    std::size_t position(charge c) const
    {
        ...
        if (sorted_)
            match = std::lower_bound(this->begin(), this->end(), std::make_pair(c,0), index_detail::gt<SymmGroup>());
        else
            match = std::find_if(this->begin(), this->end(), index_detail::is_first_equal<SymmGroup>(c));
