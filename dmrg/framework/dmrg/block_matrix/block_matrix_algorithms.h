...
// +----------------------+
//
// List of routines used to act on a block matrix object
//
// Truncation results
// ------------------
// Structure collecting the relevant data to be passed once the SVD truncation of
// an MPS has been performed (the new bond dimension, the fraction of stuff that
// has been neglected and the smallest eigenvalue that has been kept)
...
struct truncation_results {
    ...
    std::vector<size_t> keeps;      // number of eigenvalues to keep for the truncation for each block.

    truncation_results() { }

    truncation_results(std::size_t m, double tw, double tf, double se, std::vector<size_t> k)
    : bond_dimension(m)
    , truncated_weight(tw)
...
// GEMM
// ----
// Subroutine that mimics the behaviour of LAPACK GEMM for block matrices
...
// SVD
// ---
// Performs an SVD of a block matrix
         ...
         block_matrix<DiagMatrix, SymmGroup> & S)
{
    ...
    // Initialization of the index of the intermediate matrix.
    // The dimension of the intermediate, diagonal matrix depends on the shape
    // of M (different, for example, for left and right sweeps)
    ...
    // Initialize the output matrices. Remember that, when a block_matrix object
    // is initialized by giving two Index objects, the number of blocks is NOT
    // the product of the two dimensions, but just the size of a single Index
...
// HEEV
// ----
// Diagonalization of a block matrix. The overall structure is similar to the previous
// case
...
// ESTIMATE_TRUNCATION
// -------------------
// Estimate the error associated with the truncation in the DMRG algorithm
//
                         ...
                         double & smallest_ev)
{
    ...
    // Compute the overall number of eigenvalues and extract them inside allevals
    size_t length = 0;
...
//    ////// DEBUG END
    ...
    // Keeps is a vector of the eigenvalues retained for each block
    for(std::size_t k = 0; k < evals.n_blocks(); ++k){
...
//
// ESTIMATE_TRUNCATION_KEEPS
// -------------------
// Estimate the error associated with the truncation in the DMRG algorithm,
// but with a given number of kept eigenvalues per block (e.g. obtained from a previous truncation)
//
template<class DiagMatrix, class SymmGroup>
void estimate_truncation_keeps(block_matrix<DiagMatrix, SymmGroup> const & evals,
                         ...
                         double & smallest_ev)
{
    ...
    // Vector that contains smallest eigenvalues per block
    real_vector_t smallest_evs(evals.n_blocks());

    // Loop through all blocks of evals and accumulate all eigenvalues

    truncated_fraction = 0.0; truncated_weight = 0.0;

    for(std::size_t k = 0; k < evals.n_blocks(); ++k)
    {
        // Calculate truncated_fraction and truncated_weight
        for (std::size_t l = 0; l < keeps[k]; ++l)
        {
            real_type eval = *(evals[k].diagonal().first + l);
            truncated_fraction += eval;
            truncated_weight += eval * eval;
        }

        // Get smallest eigenvalue per block
    ...
    smallest_ev = *std::max_element(smallest_evs.begin(),smallest_evs.end());
...
// SVD_TRUNCATE
// ------------
// Function that diagonalizes a block_matrix and removes the columns based on a given threshold.
// Same as HEEV_TRUNCATE (see below), but employs SVD instead of standard diagonalization
                                ...
                                const std::vector<size_t> &ext_keeps = std::vector<size_t>())
{
    ...
    // otherwise call estimate_truncation()
    std::vector<size_t> keeps(S.n_blocks(), 0);
    if (!ext_keeps.empty())
        keeps = ext_keeps;
...
)
{
    // Simple checks
    assert( M.basis().sum_of_left_sizes() > 0 && M.right_basis().sum_of_sizes() > 0 );
    // Diagonalization of the input matrix
    #ifdef USE_AMBIENT
    ...
    // If we provide the number of eigenvalues per block (in ext_keeps) use it for the truncation,
    // otherwise call estimate_truncation()
    std::vector<size_t> keeps(evals.n_blocks(), 0);
    if (!ext_keeps.empty())
        keeps = ext_keeps;
    double truncated_fraction, truncated_weight, smallest_ev;
    if (!ext_keeps.empty())
    ...
    for ( int k = evals.n_blocks() - 1; k >= 0; --k)
    {
        size_t keep = keeps[k];
        if (keep == 0) {
...
// C - Tim : I reversed the loop because the new version was incompatible with the keeps array, and created a bug when keeps[k]=0.
    ...
    return truncation_results(bond_dimension, truncated_fraction, truncated_fraction, smallest_ev, keeps);
}
...
template<class Matrix, class SymmGroup>
block_matrix<Matrix, SymmGroup> inverse(block_matrix<Matrix, SymmGroup>  m)
{
    // todo: make it parallel when it works
    typedef typename Matrix::value_type vt;
    for (std::size_t k = 0; k < m.n_blocks(); ++k)
        std::transform(m[k].diagonal().first, m[k].diagonal().second, m[k].diagonal().first,
            [](vt i){ if (std::abs(i) > 1e-16) { return 1./i; } else { throw std::runtime_error("Zero singular value in pseudoinverse");}; });
    return m;
}

// Set small values (below machine precision) to zero
template<class Matrix, class SymmGroup>
void zero_small_values_inplace(block_matrix<Matrix, SymmGroup>&  m)
{
    // todo: make it parallel when it works
    typedef typename Matrix::value_type vt;
    for (std::size_t k = 0; k < m.n_blocks(); ++k)
        std::transform(m[k].diagonal().first, m[k].diagonal().second, m[k].diagonal().first,
            [](vt i){ return (std::abs(i) < 1e-16) ? 0 : i; });
}
