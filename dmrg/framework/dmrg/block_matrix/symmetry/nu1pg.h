...
class hash<std::pair<NU1ChargePG<N, S>, NU1ChargePG<N, S> > > {
 public:
  size_t operator()(
      std::pair<NU1ChargePG<N, S>, NU1ChargePG<N, S> > const &Pair_of_charge
  ) const {
    std::size_t seed = 0;
    for (int i(0); i < N + 1; i++) {
...
template <int N, class S>
NU1ChargePG<N, S> operator+(
    NU1ChargePG<N, S> const &a, NU1ChargePG<N, S> const &b
) {
  NU1ChargePG<N, S> ret;
  tpl_arith_<NU1PG<N, S>, N, 0>().operator_plus(
...
class NU1PG {
 public:
  ...
  static subcharge adjoin(subcharge I) { return I; }
...
template <int N, class S>
const typename NU1PG<N, S>::charge NU1PG<N, S>::IdentityCharge =
    typename NU1PG<N, S>::charge();
template <int N, class S>
const alps::numeric::matrix<S> NU1PG<N, S>::mult_table =
    generate_mult_table<S>();
