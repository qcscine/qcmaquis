...
template <int N, class S>
NU1ChargeDG<N, S> operator+(
    NU1ChargeDG<N, S> const &a, NU1ChargeDG<N, S> const &b
) {
  NU1ChargeDG<N, S> ret;
  tpl_arith_<NU1DG<N, S>, N, 0>().operator_plus(
      a.begin(), b.begin(), ret.begin()
  );
  return ret;
}
...
class NU1DG {
  template <class G, int A, int B>
  friend struct tpl_arith_;
 ...
 public:
  ...
  static subcharge adjoin(subcharge I) { return adjoin_table[I]; }

  static subcharge particleNumber(charge rhs) {
    return std::accumulate(rhs.begin(), &rhs[N], 0);
  }
  ...
  template <int R>
  static charge fuse(std::array<charge, R> const &v) {
    charge ret = v[0];
    for (int i = 1; i < R; ++i) {
      ret = fuse(ret, v[i]);
    }
  ...
  static void set_dg_id(std::size_t group) { group_id = group; }

  static void set_dg_table() {
    // set up multiplication table according to group_id (same numbering as in
    // Dirac)
    switch (group_id) {
      // C2h, D2h
      case 4:
      ...
      default:
        throw std::runtime_error(
            "Double group not known!\nAvailable double groups are C1, Ci, C2h, "
            "D2h, C2, D2, C2v, Cs, Cinfv, Dinfh.\n"
        );
    }
  }

  static subcharge const get_max_irrep() { return max_irrep; }

  template <S>
  friend std::vector<S> default_dg_adjoin_table();
  template <S>
  friend alps::numeric::matrix<S> default_dg_mult_table();
};
...
template <int N, class S>
const typename NU1DG<N, S>::charge NU1DG<N, S>::IdentityCharge =
    typename NU1DG<N, S>::charge();
template <int N, class S>
alps::numeric::matrix<S> NU1DG<N, S>::mult_table = default_dg_mult_table<S>();
template <int N, class S>
std::vector<S> NU1DG<N, S>::adjoin_table = default_dg_adjoin_table<S>();
template <int N, class S>
std::size_t NU1DG<N, S>::group_id = 0;
template <int N, class S>
typename NU1DG<N, S>::subcharge NU1DG<N, S>::max_irrep = 0;
