...
class Ztwo {
 public:
  ...
  static charge fuse(std::array<charge, R> v) {
    ...
    for (int i = 1; i < R; i++) v[0] = fuse(v[0], v[i]);
    return v[0];
  }
};

inline void save(
    alps::hdf5::archive& ar, std::string const& p, Ztwo::charge const& v,
...
inline void load(
    alps::hdf5::archive& ar, std::string const& p, Ztwo::charge& v,
    const std::vector<std::size_t>& size = std::vector<std::size_t>(),
    const std::vector<std::size_t>& chunk = std::vector<std::size_t>(),
    const std::vector<std::size_t>& offset = std::vector<std::size_t>()
) {
...
inline std::ostream& operator<<(
    std::ostream& ost, const std::vector<Ztwo::charge>& c
) {
  ost << "[ ";
  for (std::vector<Ztwo::charge>::const_iterator it = c.begin(); it != c.end();
       it++)
    ost << ", " << *it;
  ost << " ]";
  return ost;
