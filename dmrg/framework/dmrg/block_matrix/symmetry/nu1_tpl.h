...
class hash<std::pair<NU1Charge<N, S>, NU1Charge<N, S> > > {
 public:
  size_t operator()(
      std::pair<NU1Charge<N, S>, NU1Charge<N, S> > const &Pair_of_charge
  ) const {
    std::size_t seed = 0;
    for (int i(0); i < N; i++) {
...
class NU1_template {
 public:
  ...
  static subcharge particleNumber(charge rhs) {
    return std::accumulate(rhs.begin(), rhs.end(), 0);
  }
...
template <int N, class S>
const typename NU1_template<N, S>::charge NU1_template<N, S>::IdentityCharge =
    typename NU1_template<N, S>::charge();
