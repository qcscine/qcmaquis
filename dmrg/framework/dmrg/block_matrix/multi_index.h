...
class index_product_iterator
    : public boost::forward_iterator_helper<
          ...
          std::vector<
              std::pair<typename Index<SymmGroup>::charge, std::size_t> >,
          std::ptrdiff_t,
          std::vector<
              std::pair<typename Index<SymmGroup>::charge, std::size_t> >*,
          std::vector<
...
class MultiIndex {
 public:
  ...
  index_t const& index(index_id i) const { return idx_[i]; }

  size_t size() const { return idx_.size(); }

  set_id create_set(
      std::vector<std::pair<index_id, bool> > const& vec_left,
  ...
  std::pair<coord_t, coord_t> get_coords(set_id s, key_t const& key) const {
    ...
    for (const auto& i : set_left[s]) left_k.push_back(key[i.first]);
    for (const auto& i : set_right[s]) right_k.push_back(key[i.first]);

    return std::make_pair(
        get_left_coord(s, left_k), get_right_coord(s, right_k)
    );
  }
  ...
  // coord --> coord
  std::pair<coord_t, coord_t> convert_coords(
      set_id set1, coord_t const& coord1_left, coord_t const& coord1_right,
      set_id set2
  ) const {
    ...
    for (int i = 0; i < set_left[set1].size(); ++i) {
      //            maquis::cout << "key index = " << set_left[set1][i].first <<
      //            std::endl;
  ...
  inline std::pair<coord_t, coord_t> convert_coords(
      set_id set1, std::pair<coord_t, coord_t> const& coords1, set_id set2
  ) const {
    return convert_coords(set1, coords1.first, coords1.second, set2);
  }

  const_iterator begin() const { return const_iterator(idx_); }

  const_iterator end() const { return const_iterator(); }

 private:
  coord_t const& key_to_val(
      key_t const& key, std::vector<key_t> const& keys,
      std::vector<coord_t> const& vals
  ) const {
  ...
  key_t const& val_to_key(
      coord_t const& ci, std::vector<key_t> const& keys,
      std::vector<coord_t> const& vals
  ) const {
...
template <class SymmGroup>
typename MultiIndex<SymmGroup>::set_id MultiIndex<SymmGroup>::create_set(
    std::vector<std::pair<index_id, bool> > const& vec_left,
    std::vector<std::pair<index_id, bool> > const& vec_right
) {
  {
    std::vector<index_t> b;
    for (const auto& i : vec_left) b.push_back(idx_[i.first]);

    std::map<charge, size_t> block_begins;
    std::vector<key_t> keys_;
    std::vector<coord_t> vals_;

    for (const_iterator it = const_iterator(b); it != const_iterator(); it++) {
      charge c = SymmGroup::IdentityCharge;
      for (int i = 0; i < b.size(); ++i)
        c = (vec_left[i].second) ? SymmGroup::fuse(c, (*it)[i].first)
                                 : SymmGroup::fuse(c, -(*it)[i].first);
      keys_.push_back(*it);
  ...
  {
    std::vector<index_t> b;
    for (const auto& i : vec_right) b.push_back(idx_[i.first]);
    ...
    for (const_iterator it = const_iterator(b); it != const_iterator(); it++) {
      charge c = SymmGroup::IdentityCharge;
      for (int i = 0; i < b.size(); ++i)
        c = (vec_right[i].second) ? SymmGroup::fuse(c, (*it)[i].first)
                                  : SymmGroup::fuse(c, -(*it)[i].first);
      keys_.push_back(*it);
      vals_.push_back(std::make_pair(c, block_begins[c]));
      block_begins[c]++;
...
// ostreams
// template <class SymmGroup>
// std::ostream& operator<< (std::ostream& os, std::pair<typename
// SymmGroup::charge, std::size_t> const& p)
//{
//    os << "(" << p.first << " : " << p.second << ")";
//    return os;
//}
//
// template <class SymmGroup>
// std::ostream& operator<< (std::ostream& os, typename
// index_product_iterator<SymmGroup>::value_type const& v)
//{
//    //std::copy(v.begin(), v.end(),
//    std::ostream_iterator<std::pair<symm::charge, std::size_t> >(os, " "));
...
// template <class SymmGroup>
// std::ostream& operator<< (std::ostream& os, std::pair<typename
// MultiIndex<SymmGroup>::coord_t, typename MultiIndex<SymmGroup>::coord_t>
// const& p)
//{
//    os << p.first << ", " << p.second;
//    return os;
