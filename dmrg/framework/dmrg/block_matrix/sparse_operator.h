...
class SparseOperator {
 ...
 public:
  ...
  SparseOperator(SiteOperator<Matrix, SymmGroup> const& bm) { update(bm); }

  SparseOperator(
      block_matrix<Matrix, SymmGroup> const& bm, spin_basis_type const& sb
  ) {
    update(bm);
  }
  ...
  std::pair<const_iterator, const_iterator> block(std::size_t b) const {
    return std::make_pair(
        data_.begin() + blocks_[b], data_.begin() + blocks_[b + 1]
    );
  }
  ...
  void update(
    ...
    // similar to doing a prefix sum on the number of non-zero entries over the
    // blocks
    int entry_counter = 0;
      ...
      for (std::size_t ss1 = 0; ss1 < num_rows(bm[b]); ++ss1) {
        for (std::size_t ss2 = 0; ss2 < num_cols(bm[b]); ++ss2) {
          if (bm[b](ss1, ss2) != float_type()) {  // only add non-zero values
            ...
            // data_.push_back(value_type(ss1, ss2, left_spins.at(ss1),
            // right_spins.at(ss2), bm[b](ss1,ss2)));
            ++entry_counter;
          }
        }
      }
...
class SparseOperator<
 ...
 public:
  ...
  void update(
      ...
      spin_basis_type const& spin_basis
  ) {
    ...
    for (std::size_t b = 0; b < bm.n_blocks(); ++b) {
      blocks_[b] = entry_counter;
      std::vector<subcharge> const& left_spins =
          spin_basis
              .at(std::make_pair(
                  bm.basis().left_charge(b), bm.basis().right_charge(b)
              ))
              .first;
      std::vector<subcharge> const& right_spins =
          spin_basis
              .at(std::make_pair(
                  bm.basis().left_charge(b), bm.basis().right_charge(b)
              ))
        ...
        for (std::size_t ss2 = 0; ss2 < num_cols(bm[b]); ++ss2) {
          if (bm[b](ss1, ss2) != float_type()) {
            data_.push_back(value_type(
                ss1, ss2, left_spins[ss1], right_spins[ss2], bm[b](ss1, ss2)
            ));
            // data_.push_back(value_type(ss1, ss2, left_spins.at(ss1),
            // right_spins.at(ss2), bm[b](ss1,ss2)));
            ++entry_counter;
