...
template <class Matrix, class SymmGroup>
SiteOperator<Matrix, SymmGroup> adjoin(SiteOperator<Matrix, SymmGroup> const& m
  ...
  for (size_t k = 0; ret && k < m.n_blocks(); ++k) {
    if (m.basis().left_size(k) != m.basis().right_size(k)) {
      ...
      ret = is_hermitian(m[k]);
                   ...
                   m.basis().right_charge(k), m.basis().left_charge(k)
               )) {
      return false;
    } else {
      ret =
          (m[k] ==
           transpose(conj(m(m.basis().right_charge(k), m.basis().left_charge(k))
           )));
    }
  }
  return ret;
...
SiteOperator<Matrix, SymmGroup> op_exp_hermitian(
    Index<SymmGroup> const& phys, SiteOperator<Matrix, SymmGroup> M,
    A const& alpha = 1.
) {
  for (typename Index<SymmGroup>::const_iterator it_c = phys.begin();
       it_c != phys.end(); it_c++) {
    if (M.has_block(it_c->first, it_c->first)) {
      M(it_c->first, it_c->first) =
          exp_hermitian(M(it_c->first, it_c->first), alpha);
    } else {
      M.insert_block(
          Matrix::identity_matrix(phys.size_of_block(it_c->first)), it_c->first,
          it_c->first
      );
    }
  }
...
OutOp op_exp_hermitian(
    Index<SymmGroup> const& phys, SiteOperator<Matrix, SymmGroup> const& M,
    A const& alpha = 1.
) {
  OutOp ret(M.basis());
  for (typename Index<SymmGroup>::const_iterator it_c = phys.begin();
...
typename std::enable_if<
    boost::is_complex<typename Matrix::value_type>::value, Matrix>::type
...
template <class Matrix, class SymmGroup, class A>
SiteOperator<Matrix, SymmGroup> op_exp(
    Index<SymmGroup> const& phys, SiteOperator<Matrix, SymmGroup> M,
    A const& alpha = 1.
) {
  for (typename Index<SymmGroup>::const_iterator it_c = phys.begin();
    ...
    if (M.has_block(it_c->first, it_c->first)) {
      M(it_c->first, it_c->first) =
          detail::exp_dispatcher(M(it_c->first, it_c->first), alpha);
    } else {
      M.insert_block(
          Matrix::identity_matrix(phys.size_of_block(it_c->first)), it_c->first,
          it_c->first
...
template <
    class Matrix1, class Matrix2, class SymmGroup,
    class = symm_traits::disable_if_su2_t<SymmGroup>>
void op_kron(
    Index<SymmGroup> const& phys_A, Index<SymmGroup> const& phys_B,
    SiteOperator<Matrix1, SymmGroup> const& A,
  ...
  for (int i = 0; i < A.n_blocks(); ++i) {
    for (int j = 0; j < B.n_blocks(); ++j) {
      typename SymmGroup::charge new_right =
          SymmGroup::fuse(A.basis().right_charge(i), B.basis().right_charge(j));
      typename SymmGroup::charge new_left =
          SymmGroup::fuse(A.basis().left_charge(i), B.basis().left_charge(j));

      Matrix2 tmp(
          pb_left.size(A.basis().left_charge(i), B.basis().left_charge(j)),
          pb_right.size(A.basis().right_charge(i), B.basis().right_charge(j)), 0
      );

      maquis::dmrg::detail::op_kron(
          tmp, B[j], A[i],
...
std::vector<Integer> allowed_spins(
    Integer left, Integer right, Integer k1, Integer k2
);
}
...
template <
    class Matrix1, class Matrix2, class SymmGroup,
    class =
        typename std::enable_if<symm_traits::HasSU2<SymmGroup>::value>::type>
void op_kron(
    Index<SymmGroup> const& phys_A, Index<SymmGroup> const& phys_B,
  ...
  std::map<
      charge_pair, std::pair<std::vector<subcharge>, std::vector<subcharge>>,
      compare_pair<charge_pair>>
      basis_spins;
  ...
  for (std::size_t i = 0; i < A.n_blocks(); ++i) {
    for (std::size_t j = 0; j < B.n_blocks(); ++j) {
      ...
      maquis::dmrg::detail::op_kron(
          tmp, B[j], A[i], in_offset, out_offset, A.basis().left_size(i),
          B.basis().left_size(j), A.basis().right_size(i),
          B.basis().right_size(j)
      );
      ...
      typename Matrix2::value_type coupling =
          SU2::mod_coupling(j1, j2, J, k1, k2, k, j1p, j2p, Jp);
      tmp *= coupling;
      ...
      basis_spins[std::make_pair(new_left, new_right)].second.resize(
          num_cols(tmp)
      );
  ...
  typename Matrix2::value_type coupling =
      std::sqrt((jpp + 1) * (k + 1)) *
      WignerWrapper::gsl_sf_coupling_6j(j, jp, k, k2, k1, jpp);
...
//             has_block = has_block && B.has_block((*it)[2*dist].first,
//             (*it)[2*dist+1].first); for (size_t i=1; has_block && i<dist;
//             ++i)
//                 has_block = F.has_block((*it)[2*i].first,
//                 (*it)[2*i+1].first);
//
//             if (!has_block)
//                 continue;
//
//             typename Matrix::value_type val = A((*it)[0], (*it)[1]) *
//             B((*it)[2*dist], (*it)[2*dist+1]); for (size_t i=1; i<dist; ++i)
...
//                               midx.left_size(s, coord_l.first),
//                               midx.right_size(s, coord_r.first));
//                 else
