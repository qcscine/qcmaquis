...
bool lt(
    std::pair<typename SymmGroup::charge, std::size_t> const& a,
...
template <class SymmGroup>
typename SymmGroup::charge get_first(
    std::pair<typename SymmGroup::charge, std::size_t> const& x
) {
  return x.first;
}

template <class SymmGroup>
std::size_t get_second(
    std::pair<typename SymmGroup::charge, std::size_t> const& x
) {
  return x.second;
}
...
class is_first_equal {
 public:
  ...
  bool operator()(std::pair<typename SymmGroup::charge, std::size_t> const& x
  ) const {
    return x.first == c_;
  }
...
template <class SymmGroup>
class Index : protected std::vector<
                  std::pair<typename SymmGroup::charge, std::size_t> > {
  typedef std::vector<std::pair<typename SymmGroup::charge, std::size_t> >
      base_t;
  typedef boost::container::flat_map<typename SymmGroup::charge, std::size_t>
      pos_t;
 ...
 public:
  ...
  // This seems to be necessary because the flat_map has weird copy/assignment
  // semantics
  ...
  std::size_t size_of_block(charge c, bool position_check) const {
    ...
    if (position_check && it == positions.end()) return 0;
  ...
  std::size_t position(std::pair<charge, std::size_t> x) const {
    ...
    return x.second +
           std::accumulate(
               ...
               boost::lambda::_1 +
                   boost::lambda::bind(
                       index_detail::get_second<SymmGroup>, boost::lambda::_2
                   )
           );
  }

  bool has(charge c) const { return positions.count(c) > 0; }
  ...
  void insert(std::size_t position, std::pair<charge, std::size_t> const& x) {
    std::vector<std::pair<charge, std::size_t> >::insert(
  ...
  value_type& operator[](std::size_t p) {
    return static_cast<base_t&>(*this)[p];
  }
  value_type const& operator[](std::size_t p) const {
    return static_cast<base_t const&>(*this)[p];
  }
  ...
  iterator erase(iterator p) {
    iterator r = base_t::erase(p);
    calc_positions();
    return r;
  }
  iterator erase(iterator a, iterator b) {
    iterator r = base_t::erase(a, b);
    calc_positions();
    return r;
  }
...
class ProductBasis {
 ...
 private:
  ...
  void init(Index<SymmGroup> const& a, Index<SymmGroup> const& b, Fusion f) {
    ...
    for (typename Index<SymmGroup>::const_iterator it1 = a.begin();
         it1 != a.end(); ++it1)
      for (typename Index<SymmGroup>::const_iterator it2 = b.begin();
           it2 != b.end(); ++it2) {
 ...
 public:
  size_t operator()(charge a, charge b) const {
    typedef typename boost::unordered_map<
        std::pair<charge, charge>, size_t>::const_iterator match_type;
...
Index<SymmGroup> common_subset(Index<SymmGroup>& a, Index<SymmGroup>& b) {
  a.erase(
      std::remove_if(
          a.begin(), a.end(),
          !boost::lambda::bind(
              &Index<SymmGroup>::has, b,
              boost::lambda::bind(
                  index_detail::get_first<SymmGroup>, boost::lambda::_1
              )
          )
      ),
      a.end()
