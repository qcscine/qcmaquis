...
class QnBlock {
 ...
 public:
  ...
  bool operator<(const QnBlock& other) const { return other > *this; }
...
const auto is_left_charge_greater = [](const auto& QnBlockA,
                                       const auto& QnBlockB) -> bool {
  return QnBlockA.lc > QnBlockB.lc;
};

const auto are_charges_equal = [](const auto& QnBlock, const auto& left_charge,
                                  const auto& right_charge) -> bool {
 ...
 */
const auto is_left_charge_equal = [](const auto& QnBlock, const auto& charge
                                  ) -> bool { return QnBlock.lc == charge; };
}  // namespace dual_index_detail
...
void serialize(
    Archive& ar, dual_index_detail::QnBlock<SymmGroup>& t,
 ...
 *        where each element corresponds to a specific block of the block
 * matrix.
...
class DualIndex {
 ...
 public:
  ...
  std::size_t position(charge row, charge col) const {
    ...
    if (sorted_) {
      ...
      match = std::lower_bound(
          data_.begin(), data_.end(), value_type(row, col, 0, 0), std::greater{}
      );
    } else {
      match = std::find_if(
    ...
    bool not_found =
        match != data_.end() && ((*match).lc != row || (*match).rc != col);
    if (not_found) {
      match = data_.end();
    }
  ...
  bool has(charge row, charge col) const {
    if (sorted_) {
      ...
      return std::find_if(
                 data_.begin(), data_.end(),
                 [&row, &col](const auto& QnBlock) {
                   return dual_index_detail::are_charges_equal(
                       QnBlock, row, col
                   );
                 }
             ) != data_.end();
  ...
  bool operator!=(DualIndex const& o) const { return !(*this == o); }
  ...
  std::size_t sum_of_left_sizes() const {
    return std::accumulate(
        ...
        [&](const auto& acc, const auto& x) { return acc + x.ls; }
    );
  }
  ...
  std::size_t memory_size() const {
    return std::accumulate(
        data_.begin(), data_.end(), 0,
        [](const auto& acc, const auto& x) { return acc + x.ls * x.rs; }
    );
  }
 ...
 private:
  ...
  std::size_t destination(value_type const& x) const {
    return std::distance(
        data_.begin(),
        std::find_if(
            data_.begin(), data_.end(), [x](const auto& e) { return e < x; }
        )
    );
  }
...
std::ostream& operator<<(
    std::ostream& os, const dual_index_detail::QnBlock<SymmGroup>& block
) {
  os << "( " << block.lc << "," << block.rc << ": " << block.ls << "x"
     << block.rs << " )";
  return os;
}
