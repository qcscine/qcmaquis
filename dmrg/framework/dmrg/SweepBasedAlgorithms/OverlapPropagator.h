...
class OverlapPropagator {
 public:
  ...
  using BlockMatrixType =
      block_matrix<typename storage::constrained<Matrix>::type, SymmGroup>;
  using Contraction = contraction::Engine<
      Matrix, typename storage::constrained<Matrix>::type, SymmGroup>;

  /** @brief Class constructor */
  OverlapPropagator(
      const MPSType& refMPS, const std::vector<std::string>& mpsFiles,
      BaseParameters& parms_, int initSite = 0
  )
      : refMPS_(refMPS), L_(refMPS_.length()) {
    nOrthogonalMPSs_ = mpsFiles.size();
    for (int n = 0; n < nOrthogonalMPSs_; n++) {
      MPSType tmp;
      maquis::cout << "Orthogonal state " << n << " loaded from " << mpsFiles[n]
                   << std::endl;
      ...
      maquis::checks::right_end_check(
          mpsFiles[n], orthoMPS_[n], refMPS_[L_ - 1].col_dim()[0].first
      );
      maquis::cout << "Right end: " << orthoMPS_[n][L_ - 1].col_dim()
                   << std::endl;
    }
  ...
  OverlapPropagator(
      const MPSType& refMPS, const MPSType& otherMPS, int initSite = 0
  )
  ...
  OverlapPropagator(
      const MPSType& refMPS, const std::vector<MPSType>& otherMPSs,
      int initSite = 0
  )
      : refMPS_(refMPS),
        orthoMPS_(otherMPSs),
        nOrthogonalMPSs_(otherMPSs.size()),
        L_(refMPS_.length()) {
    initializeData(initSite);
  }
  ...
  auto getOrthogonalVector(int iVector, int siteLeft, int siteRight) const {
    using SweepMPSContainerType =
        SweepMPSContainer<Matrix, SymmGroup, SweepType>;
    auto mpsContainer = SweepMPSContainerType(refMPS_);
    auto orthoMPSContainer = SweepMPSContainerType(orthoMPS_[iVector]);
    return contraction::site_ortho_boundaries(
        mpsContainer.getMPSTensor(siteLeft),
        orthoMPSContainer.getMPSTensor(siteLeft),
        partialContractionLeft_[iVector][siteLeft],
        partialContractionRight_[iVector][siteRight]
    );
  }
  ...
  auto getOrthogonalVector(int siteLeft, int siteRight) const {
    return this->template getOrthogonalVector<SweepType>(
        0, siteLeft, siteRight
    );
  }

  /** @brief Getter for the number of orthogonal states */
  int getNumberOfOverlapMPSs() const { return nOrthogonalMPSs_; }

  /** @brief Propagation algorithm for the left overlap boundaries (see
   * [BoundaryPropagator]) */
  inline void updateLeftOverlapBoundaries(int iSite) {
    if (iSite > 0 && iSite <= L_) {
      for (int n = 0; n < nOrthogonalMPSs_; n++)
        partialContractionLeft_[n][iSite] = Contraction::overlap_left_step(
            refMPS_[iSite - 1], orthoMPS_[n][iSite - 1],
            partialContractionLeft_[n][iSite - 1]
        );
    }
  }

  /** @brief Propagation algorithm for the right boundaries (see
   * [BoundaryPropagator]) */
  inline void updateRightOverlapBoundaries(int iSite) {
    if (iSite >= 0 && iSite < L_) {
      for (int n = 0; n < nOrthogonalMPSs_; n++)
        partialContractionRight_[n][iSite] = Contraction::overlap_right_step(
            refMPS_[iSite], orthoMPS_[n][iSite],
            partialContractionRight_[n][iSite + 1]
        );
    }
  }
 ...
 private:
  ...
  /** @brief Prepares the data structure that accomondates the partial
   * contraction */
  ...
  std::vector<std::vector<BlockMatrixType> > partialContractionLeft_,
      partialContractionRight_;
};
