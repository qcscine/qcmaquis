...
class SweepMPSUpdater<
    Matrix, SymmGroup, Storage, SweepOptimizationType::SingleSite> {
 public:
  // Type declaration
  ...
  SweepMPSUpdater(
      const MPOType& mpo, MPSType& mps,
      std::shared_ptr<BoundaryPropagatorType> boundaryPropagator,
      BaseParameters& parms, bool verbose
  )
      : boundaryPropagator_(boundaryPropagator),
        mpo_(mpo),
        mps_(mps),
        parms_(parms),
        L_(mps_.size()),
  ...
  auto generateUnitaryFactor(
      int siteLeft, int siteRight, GrowBoundaryModality boundaryModality,
      const MPSTensorType& inputMPS, double alpha, double cutoff, double mMax,
      bool normalizeEnd, bool perturbDM
  ) {
    ...
    if (boundaryModality == GrowBoundaryModality::LeftToRight) {
      if (siteLeft < L_ - 1) {
        /*
         truncationOutput = mps_.grow_l2r_sweep(mpo_[siteLeft],
         boundaryPropagator_->getLeftBoundary(siteLeft),
                                                boundaryPropagator_->getRightBoundary(siteRight),
         siteLeft, alpha, cutoff, mMax, true, verbose_);
        */
        std::tie(unitaryFactor, truncationOutput) =
            Contractor::predict_new_state_l2r_sweep(
                mps_[siteLeft], mpo_[siteLeft],
                boundaryPropagator_->getLeftBoundary(siteLeft),
                boundaryPropagator_->getRightBoundary(siteRight), alpha, cutoff,
                mMax, perturbDM, verbose_
            );
        zeroSiteTensor_ = Contractor::getZeroSiteTensorL2R(
            mps_[siteLeft + 1], mps_[siteLeft], unitaryFactor
        );
    ...
    // Backward case
      ...
      if (siteLeft > 0) {
        /*
          truncationOutput = mps_.grow_r2l_sweep(mpo_[siteLeft],
          boundaryPropagator_->getLeftBoundary(siteLeft),
                                                 boundaryPropagator_->getRightBoundary(siteRight),
          siteLeft, alpha, cutoff, mMax, true, verbose_);
        */
        std::tie(unitaryFactor, truncationOutput) =
            Contractor::predict_new_state_r2l_sweep(
                mps_[siteLeft], mpo_[siteLeft],
                boundaryPropagator_->getLeftBoundary(siteLeft),
                boundaryPropagator_->getRightBoundary(siteRight), alpha, cutoff,
                mMax, perturbDM, verbose_
            );
        zeroSiteTensor_ = Contractor::getZeroSiteTensorR2L(
            mps_[siteLeft - 1], mps_[siteLeft], unitaryFactor
        );
        mps_[siteLeft] = unitaryFactor;
      } else if (normalizeEnd) {
  ...
  void performBackPropagation(
      GrowBoundaryModality boundaryModality, int siteLeft, int siteRight,
      std::shared_ptr<TimeEvolverType> timeEvolver
  ) {
    int site = siteLeft;
    if (loadedUnitaryFactor_) {
      if (boundaryModality == GrowBoundaryModality::LeftToRight) {
        if (site < L_ - 1) {
          auto zsp = ZeroSiteProblemType(
              mpo_[site], mpo_[site + 1],
              boundaryPropagator_->getLeftBoundary(site + 1),
        ...
        if (site > 0) {
          auto zsp = ZeroSiteProblemType(
              mpo_[site - 1], mpo_[site],
              boundaryPropagator_->getLeftBoundary(site),
      ...
      throw std::runtime_error(
          "ERROR: trying to back-propagate before performing the SVD"
      );
    }
  }
#endif  // DMRG_TD

  /** @brief Moves the normalization to the previous/next site (depending
   * whether it's l2r or r2l sweep) */
  void mergeUnitaryFactor(
      GrowBoundaryModality boundaryModality, int siteLeft, int siteRight,
      bool normalizeEnd
  ) {
    int site = siteLeft;
    if (boundaryModality == GrowBoundaryModality::LeftToRight) {
      if (site < L_ - 1) {
        // mps_[site+1] = Contractor::predict_lanczos_l2r_sweep(mps_[site+1],
        // mps_[site], unitaryFactor_); mps_[site] = unitaryFactor_;
        mps_[site + 1].multiply_from_left(zeroSiteTensor_);
      }
    } else if (boundaryModality == GrowBoundaryModality::RightToLeft) {
      if (site > 0) {
        // mps_[site-1] = Contractor::predict_lanczos_r2l_sweep(mps_[site-1],
        // mps_[site], unitaryFactor_); mps_[site] = unitaryFactor_;
...
template <class Matrix, class SymmGroup, class Storage>
class SweepMPSUpdater<
    Matrix, SymmGroup, Storage, SweepOptimizationType::TwoSite> {
 public:
  // Type declaration
  using BoundaryPropagatorType = BoundaryPropagator<Matrix, SymmGroup, Storage>;
  using Contractor = typename contraction::Engine<Matrix, Matrix, SymmGroup>;
  ...
  auto generateUnitaryFactor(
    ...
    // Converts back the MPS into the two-site tensor. Note that here the tst is
    // *not* the contraction of mps_[siteLeft] and mps_[siteLeft+1], since tst
    // << inputMPS overwrites this contraction. mps_[siteLeft] and
    // mps_[siteLeft+1] are used only to have the correct indices
    ...
    if (boundaryModality == GrowBoundaryModality::LeftToRight) {
      ...
      if (parms_["twosite_truncation"] == "svd") {
        std::tie(mps_[siteLeft], mps_[siteLeft + 1], truncationOutput) =
            tst.split_mps_l2r(mMax, cutoff, verbose_);
      } else {
        std::tie(mps_[siteLeft], mps_[siteLeft + 1], truncationOutput) =
            tst.predict_split_l2r(
                mMax, cutoff, alpha,
                boundaryPropagator_->getLeftBoundary(siteLeft), mpo_[siteLeft],
                perturbDM
            );
      }
    } else if (boundaryModality == GrowBoundaryModality::RightToLeft) {
      if (parms_["twosite_truncation"] == "svd") {
        std::tie(mps_[siteLeft], mps_[siteLeft + 1], truncationOutput) =
            tst.split_mps_r2l(mMax, cutoff, verbose_);
      } else {
        std::tie(mps_[siteLeft], mps_[siteLeft + 1], truncationOutput) =
            tst.predict_split_r2l(
                mMax, cutoff, alpha,
                boundaryPropagator_->getRightBoundary(siteRight),
  ...
  void performBackPropagation(
    ...
    if (loadedUnitaryFactor_) {
      if (boundaryModality == GrowBoundaryModality::LeftToRight) {
        if (siteRight != L_ - 1) {
          SiteProblemType sp2(
              boundaryPropagator_->getLeftBoundary(siteRight - 1),
              boundaryPropagator_->getRightBoundary(siteRight),
              mpo_[siteRight - 1]
          );
        ...
        if (siteLeft != 0) {
          SiteProblemType sp2(
              boundaryPropagator_->getLeftBoundary(siteLeft),
              boundaryPropagator_->getRightBoundary(siteLeft + 1),
              mpo_[siteLeft]
          );
      ...
      throw std::runtime_error(
          "ERROR: trying to back-propagate before performing SVD"
      );
    }
  }
#endif  // DMRG_TD
