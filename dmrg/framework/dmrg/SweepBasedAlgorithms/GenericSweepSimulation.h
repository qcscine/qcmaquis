...
class GenericSweepSimulation {
 public:
  ...
  GenericSweepSimulation(
      ...
      std::string simulationName = "Optimization"
  )
      : mps_(mps),
        mpoContainer_(mpo, mps),
        mpsContainer_(mps),
        L_(mps_.length()),
    ...
    boundaryPropagator_ =
        std::make_shared<BoundaryPropagatorType>(mps_, mpoContainer_.getMPO());

    mpsUpdater_ = std::make_unique<SweepMPSUpdaterType>(
        mpoContainer_.getMPO(), mps_, boundaryPropagator_, parms_, verbose_
    );
  }
   ...
   *
   * Note that we delegate every action to the derived class, with the exception
   * of the memory management, which is done here to ensure that the
   * implementation is consistent for all methods.
   */
  ...
  ValueType runSingleSweep(int iSweep) {
    ...
    auto numberOfMicroIterations =
        SweepTraitClass::getNumberOfMicroiterations(L_);
    ...
    while (indexOfMicroIteration_ < numberOfMicroIterations) {
      ...
      auto changeDirection = SweepTraitClass::changeDirectionNextMicroiteration(
          L_, indexOfMicroIteration_
      );
      // Syncs the storage
      // if (sweepType == SweepDirectionType::Backward && indexOfMicroIteration_
      // == lastSite_) {
      ...
      // Gets the boundary that are needed. Note that, in a forward sweep, the
      // left boundary is assumed to have been generated during the previous
      // boundary update and, therefore, is not fetched.
      if (sweepType == SweepDirectionType::Backward ||
          indexOfMicroIteration_ == 0) {
      ...
      // Note that, for instance, we don't prefetch the left boundary for the
      // l2r sweep because this will be taken care in the boundary propagation
      // (in other words, there is no need to prefetch the left boundary since
      // it will be anyways modified by the boundary propagation)
      if (sweepType == SweepDirectionType::Forward) {
        auto sweepTypeNext =
            SweepTraitClass::getSweepDirection(L_, indexOfMicroIteration_ + 1);
        if (sweepTypeNext == sweepType) {
          Storage::prefetch(boundaryPropagator_->getRightBoundary(
              SweepTraitClass::getIndexOfRightBoundary(
                  L_, indexOfMicroIteration_ + 1
              )
          ));
        } else if (sweepTypeNext != SweepDirectionType::EndOfLattice) {
          Storage::prefetch(boundaryPropagator_->getLeftBoundary(
              SweepTraitClass::getIndexOfLeftBoundary(
                  L_, indexOfMicroIteration_ + 1
              )
          ));
        }
      ...
      auto boundaryGrowthModality =
          (sweepType == SweepDirectionType::Forward && !changeDirection)
              ? GrowBoundaryModality::LeftToRight
              : GrowBoundaryModality::RightToLeft;
      auto truncationResults = mpsUpdater_->generateUnitaryFactor(
          siteLeft_, siteRight_, boundaryGrowthModality, outputTensor,
      ...
      mpsUpdater_->mergeUnitaryFactor(
          boundaryGrowthModality, siteLeft_, siteRight_, this->normalizeAtEnd()
      );
      // Now, can drop the memory of the right boundary (in the case of a l2r
      // sweep). The memory will anyways be overwritten by the r2l sweep that
      // will follow. Note also that, if we are at a point at which we reverse
      // the direction of the boundary propagation, we don't drop the right
      // boundary because the next step will be a r2l sweep and, therefore, that
      // element of the right boundary won't be overwritten.
      ...
      if (verbose_) {
        maquis::cout << "            Energy = " << std::setprecision(16)
                     << energy << "  ";
        maquis::cout << " [Time: " << std::fixed << std::setprecision(2)
                     << duration.count() << std::defaultfloat << " s]\n";
      }
    }
    // At the end, just stores to file the final right boundary (if needed, one
    // can use it for the next sweep)
  ...
  void printSweepEnergy() {
    ...
    if (energy_vec.back().type() == typeid(double)) {
      ...
      maquis::cout << " SWEEP ENERGY (Complex Valued) = "
                   << std::setprecision(16) << std::real(energy) << " + i "
                   << std::imag(energy) << " ";
      if (energy_vec.size() > 1) {
        auto energy_prev =
            std::any_cast<std::complex<double>>(energy_vec.front());
 ...
 protected:
  ...
  int get_Mmax(int sweep) const {
    ...
    if (parms_.is_set("sweep_bond_dimensions")) {
      auto ssizes =
          parms_.template get<std::vector<std::size_t>>("sweep_bond_dimensions"
          );
  ...
  void updateSites() {
    siteLeft_ =
        SweepTraitClass::getIndexOfLeftBoundary(L_, indexOfMicroIteration_);
    siteRight_ =
        SweepTraitClass::getIndexOfRightBoundary(L_, indexOfMicroIteration_);
  }
  ...
  void printSweepSpecificInfo(int iSweep) const {
    maquis::cout << std::defaultfloat;
    maquis::cout << "SWEEP " << iSweep + 1
                 << "; Noise: " << this->getAlpha(iSweep)
  ...
  void printMicroiterInfo(SweepDirectionType sweepType) const {
    ...
    maquis::cout << (sweepType == SweepDirectionType::Backward ? "  <" : "   ")
                 << "-" << siteLeft_ << "-";
  ...
  /** @brief Checks whether the current microiteration is associated with a
   * terminal site */
  inline bool isTerminal() const {
    return indexOfMicroIteration_ == 0 ||
           (SweepTraitClass::changeDirectionNextMicroiteration(
                L_, indexOfMicroIteration_
            ) &&
            SweepTraitClass::getSweepDirection(L_, indexOfMicroIteration_) ==
                SweepDirectionType::Forward);
  }
