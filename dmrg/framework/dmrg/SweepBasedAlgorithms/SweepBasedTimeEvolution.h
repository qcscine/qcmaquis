...
class SweepBasedTimeEvolution
 ...
 public:
  ...
  SweepBasedTimeEvolution(
    ...
    // perturber_ = std::make_shared< PerturberType >(left_, right_, mpo_,
    // parms_);
  }
  ...
  /** @brief General method for performing the back-propagation step
   * (specialized later) */
  void performBackPropagation(GrowBoundaryModality boundaryGrowthModality
  ) final {
    if (doBackpropagation_) {
      mpsUpdater_->performBackPropagation(
          boundaryGrowthModality, siteLeft_, siteRight_, timeEvolver_
      );
    }
  }
  ...
  std::pair<ValueType, MPSTensorType> solveLocalProblem() final {
    ...
    // Note that the energy is calculated only once per sweep -- it will (or
    // should) anyways be conserved, so it's not useful to print the value of
    // the energy for each microiteration.
    if (siteLeft_ == 0 || isImaginaryTime_) {
      auto energy = ietl::get_energy(*(siteProblem_.get()), mpsToPropagate) +
                    maquis::real(mpoContainer_.getMPO().getCoreEnergy());
      resultOfLocalSiteProblem_.first = energy;
      if (verbose) {
        maquis::cout << " Energy = " << std::setprecision(16)
                     << resultOfLocalSiteProblem_.first << std::endl;
      }
      // normalize the mps
      mpsToPropagate.divide_by_scalar(mpsToPropagate.scalar_norm());
#ifndef NDEBUG
      maquis::cout << "Wave function norm " << mpsToPropagate.scalar_norm()
                   << std::endl;
#endif
    }
    ...
    // std::setprecision(10) << " Energy = " << std::setprecision(16) << energy
    // << std::endl; iterationResults_["Energy"] << energy; return
    // resultOfLocalSiteProblem_.second;
  }
  ...
  void finalizeMicroIteration(const truncation_results& trunc) final {
    ...
    if (SweepTraitClass::changeDirectionNextMicroiteration(
  ...
  bool normalizeAtEnd() final { return true; }

  /** @brief Whether to appy the noise-based perturbation */
  bool activatePerturbation() final { return perturbMPS_; }
 ...
 private:
  ...
  double timeStep_;
  // std::shared_ptr< SiteShifter< Matrix, SymmGroup, TimeEvolver<Matrix,
  // SymmGroup, BaseParameters>, PerturberType > > site_shifter_;
