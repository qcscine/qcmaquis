...
class SweepSimulationFactory {
  // Types declaration
  using GenericSSSimulationType = GenericSweepSimulation<
      Matrix, SymmGroup, Storage, SweepOptimizationType::SingleSite>;
  using GenericTSSimulationType = GenericSweepSimulation<
      Matrix, SymmGroup, Storage, SweepOptimizationType::TwoSite>;
  using OptimizationSSSimulationType = SweepBasedEnergyMinimization<
      Matrix, SymmGroup, Storage, SweepOptimizationType::SingleSite>;
  using OptimizationTSSimulationType = SweepBasedEnergyMinimization<
      Matrix, SymmGroup, Storage, SweepOptimizationType::TwoSite>;
  ...
  using LinearSystemTSSimulationType = SweepBasedLinearSystem<
      Matrix, SymmGroup, Storage, SweepOptimizationType::TwoSite>;
#ifdef DMRG_TD
  using EvolverSSSimulationType = SweepBasedTimeEvolution<
      Matrix, SymmGroup, Storage, SweepOptimizationType::SingleSite>;
  using EvolverTSSimulationType = SweepBasedTimeEvolution<
      Matrix, SymmGroup, Storage, SweepOptimizationType::TwoSite>;
#endif  // DMRG_TD
 ...
 public:
  SweepSimulationFactory(
      std::string simulationName, SweepOptimizationType sweepType, MPSType& mps,
      const MPOType& mpo, BaseParameters& parms, const ModelType& model,
    ...
    if (simulationName == "optimize") {
      if (sweepType_ == SweepOptimizationType::SingleSite) {
        ssSimulator_ = std::make_unique<OptimizationSSSimulationType>(
            mps, mpo, parms, model, lattice, verbose
        );
      } else if (sweepType_ == SweepOptimizationType::TwoSite) {
        tsSimulator_ = std::make_unique<OptimizationTSSimulationType>(
            mps, mpo, parms, model, lattice, verbose
        );
      }
    }
    // Solution of a linear system
      ...
      if (sweepType_ == SweepOptimizationType::SingleSite) {
        ssSimulator_ = std::make_unique<LinearSystemSSSimulationType>(
            mps, mpo, parms, model, lattice, linsystem_verbose
        );
      } else if (sweepType_ == SweepOptimizationType::TwoSite) {
        tsSimulator_ = std::make_unique<LinearSystemTSSimulationType>(
            mps, mpo, parms, model, lattice, linsystem_verbose
        );
      }
    }
      ...
      if (sweepType_ == SweepOptimizationType::SingleSite) {
        ssSimulator_ = std::make_unique<EvolverSSSimulationType>(
            mps, mpo, parms, model, lattice, verbose
        );
      } else if (sweepType_ == SweepOptimizationType::TwoSite) {
        tsSimulator_ = std::make_unique<EvolverTSSimulationType>(
            mps, mpo, parms, model, lattice, verbose
        );
      }
    }
    ...
    if (!ssSimulator_ && !tsSimulator_) {
      throw std::runtime_error(
          "Error in parameters for [SweepSimulationFactory] object"
      );
    }
  };
