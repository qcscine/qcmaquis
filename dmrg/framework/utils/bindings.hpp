...
struct binding<
    std::vector<std::vector<T> >, C<alps::numeric::diagonal_matrix<T>, S> > {
  static std::vector<std::vector<T> > convert(
...
struct binding<am_matrix<T, A>, al_matrix<T> >
    : public adaptor<am_matrix<T, A>, al_matrix<T> > {};
template <typename T, class A>
struct binding<al_matrix<T>, am_matrix<T, A> >
    : public adaptor<al_matrix<T>, am_matrix<T, A> > {};
template <typename T>
struct binding<am_diagonal<T>, al_diagonal<T> >
    : public adaptor<am_diagonal<T>, al_diagonal<T> > {};
template <typename T>
struct binding<al_diagonal<T>, am_diagonal<T> >
    : public adaptor<al_diagonal<T>, am_diagonal<T> > {};
template <typename T, class A>
struct binding<am_tiles<am_matrix<T, A> >, al_matrix<T> >
    : public adaptor<am_tiles<am_matrix<T, A> >, al_matrix<T> > {};
template <typename T2, typename T1, class A>
struct binding<am_tiles<am_matrix<T2, A> >, al_matrix<T1> >
    : public adaptor<am_tiles<am_matrix<T2, A> >, al_matrix<T1> > {};
template <typename T, class A>
struct binding<al_matrix<T>, am_tiles<am_matrix<T, A> > >
    : public adaptor<al_matrix<T>, am_tiles<am_matrix<T, A> > > {};
template <typename T>
struct binding<am_tiles<am_diagonal<T> >, al_diagonal<T> >
    : public adaptor<am_tiles<am_diagonal<T> >, al_diagonal<T> > {};
template <typename T>
struct binding<al_diagonal<T>, am_tiles<am_diagonal<T> > >
    : public adaptor<al_diagonal<T>, am_tiles<am_diagonal<T> > > {};
template <typename T>
struct binding<std::vector<T>, am_diagonal<T> >
    : public adaptor<std::vector<T>, am_diagonal<T> > {};
template <typename T, typename D>
struct binding<am_tiles<am_diagonal<T> >, am_tiles<am_diagonal<D> > >
    : public adaptor<am_tiles<am_diagonal<T> >, am_tiles<am_diagonal<D> > > {};
template <typename T, typename S, template <class M, class SS> class C>
struct binding<std::vector<std::vector<T> >, C<am_diagonal<T>, S> >
    : public adaptor<std::vector<std::vector<T> >, C<am_diagonal<T>, S> > {};
template <typename T, typename S, template <class M, class SS> class C>
struct binding<std::vector<std::vector<T> >, C<am_tiles<am_diagonal<T> >, S> >
    : public adaptor<
          std::vector<std::vector<T> >, C<am_tiles<am_diagonal<T> >, S> > {};
#undef am_tiles
#undef am_diagonal
#undef am_matrix
#undef al_matrix
...
inline double real(const std::complex<double>& f) { return f.real(); }

inline std::vector<double> real(const std::vector<std::complex<double> >& f) {
  return alps::numeric::real(f);
}

inline std::vector<double> real(const std::vector<double>& f) { return f; }

template <typename T>
inline const typename real_type<ambient::future<T> >::type& real(
    const ambient::future<T>& f
) {
