...
namespace maquis { namespace traits {
    ...
    template<class T> struct real_type { using type = typename real_type<typename T::value_type>::type; };
    ...
    template <class Matrix> struct transpose_view { using type = Matrix; };
    template <class Matrix> struct adjoint_view { using type = Matrix; };
