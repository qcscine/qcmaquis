...
/* $Id: fixed_capacity_deque.h 954 2004-06-29 09:29:05Z troyer $ */
...
struct deque_iterator {
  ...
  bool operator<(const deque_iterator& x) const { return pos_() < x.pos_(); }
...
class fixed_capacity_deque {
 ...
 public:
  ...
  fixed_capacity_deque(const fixed_capacity_deque& x) {
    ...
    if (x.is_continuous()) {
      insert_dispatch(
          end(), (const T*)x.first_, (const T*)x.last_,
          std::random_access_iterator_tag()
      );
    } else {
      const T* x_base = x.base();
      insert_dispatch(
          end(), (const T*)x.first_, x_base + M,
          std::random_access_iterator_tag()
      );
      insert_dispatch(
          end(), x_base, (const T*)x.last_, std::random_access_iterator_tag()
      );
    }
  }
  ...
  const_iterator end() const { return const_iterator(last_, base(), first_); }
  ...
  // element access:
  reference operator[](size_type i) { return *(base() + position(i)); }
...
inline bool operator==(
    const fixed_capacity_deque<T, N>& x, const fixed_capacity_deque<T, N>& y
) {
...
template <class T, std::size_t N>
inline void swap(fixed_capacity_deque<T, N>& x, fixed_capacity_deque<T, N>& y) {
  x.swap(y);
}
