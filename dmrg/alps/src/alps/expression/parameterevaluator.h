...
/* $Id: parameterevaluator.h 2832 2008-06-02 23:04:23Z troyer $ */
...
template <class T>
bool ParameterEvaluator<T>::can_evaluate(const std::string& name, bool isarg)
    const {
  ...
  bool can =
      Expression<T>(parms_[name])
          .can_evaluate(
              ParameterEvaluator<T>(parms, this->evaluate_random()), isarg
          );
  return can;
}

template <class T>
Expression<T> ParameterEvaluator<T>::partial_evaluate(
  ...
  if (ParameterEvaluator<T>::can_evaluate(name, isarg))
    ...
    e.partial_evaluate(
        ParameterEvaluator<T>(p, this->evaluate_random()), isarg
    );
  }
  return e;
}
...
template <class T>
typename ParameterEvaluator<T>::value_type ParameterEvaluator<T>::evaluate(
    const std::string& name, bool isarg
) const {
  ...
  if (parms_[name].template get<std::string>() == "Infinite recursion check")
    boost::throw_exception(
        std::runtime_error("Infinite recursion when evaluating " + name)
    );
  Parameters parms(parms_);
  parms[name] = "Infinite recursion check";
  typename ParameterEvaluator<T>::value_type res = alps::evaluate<value_type>(
      parms_[name], ParameterEvaluator<T>(parms, this->evaluate_random()), isarg
  );
  return res;
}
