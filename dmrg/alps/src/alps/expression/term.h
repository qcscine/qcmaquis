...
/* $Id: term.h 7602 2015-08-17 14:03:12Z dolfim $ */
...
bool Term<T>::depends_only_on(const std::string& s) const {
  if (!depends_on(s)) return false;

  for (factor_iterator it = factors().first; it != factors().second; ++it) {
    if (it->depends_on(s)) continue;
    try {
...
template <class T>
typename Term<T>::value_type Term<T>::value(const Evaluator<T>& p, bool isarg)
    const {
  value_type val(1.);
  if (p.direction() == Evaluator<T>::left_to_right) {
    ...
    for (int i = int(terms_.size()) - 1; i >= 0 && numeric::is_nonzero(val);
         --i) {
      value_type tmp = terms_[i].value(p, isarg);
      val *= tmp;
    }
  }
  if (is_negative() && numeric::is_nonzero(val)) val = val * (-1.);
  return val;
}
...
void Term<T>::partial_evaluate(const Evaluator<T>& p, bool isarg) {
  if (can_evaluate(p, isarg)) {
    ...
    if (numeric::is_zero(val))
      ...
      if (val != value_type(1.)) terms_.insert(terms_.begin(), Factor<T>(val));
    }
  }
...
std::pair<T, Term<T> > Term<T>::split() const {
  ...
  if (t.terms_.empty())
    val = 0.;
  else if (t.terms_[0].can_evaluate()) {
    val = t.terms_[0].value();
    t.terms_.erase(t.terms_.begin());
  }
  if (t.is_negative_) val = -val;
...
inline bool operator==(
    const alps::expression::Term<T>& ex1, const alps::expression::Term<T>& ex2
) {
  return (
...
template <class T>
inline bool operator==(
    const alps::expression::Term<T>& ex, const std::string& s
) {
...
template <class T>
inline bool operator==(
    const std::string& s, const alps::expression::Term<T>& ex
) {
  return ex == s;
}
