...
/* $Id: evaluate_helper.h 5796 2011-10-16 15:45:58Z troyer $ */
...
struct numeric_cast_helper {
  static U value(typename boost::call_traits<T>::param_type x) { return x; }
};
  ...
  static U value(const std::complex<T>& x) {
    if (numeric::is_nonzero(x.imag()))
      boost::throw_exception(
          std::runtime_error("can not convert complex number into real one")
      );
    return x.real();
  }
};
...
template <typename U, typename T>
U numeric_cast(
    T x, typename boost::enable_if<boost::is_arithmetic<T> >::type* = 0
) {
  return numeric_cast_helper<U, T>::value(x);
}
template <typename U, typename T>
U numeric_cast(
    const T& x, typename boost::disable_if<boost::is_arithmetic<T> >::type* = 0
) {
  return numeric_cast_helper<U, T>::value(x);
}
#else
...
struct evaluate_helper {
  ...
  template <class R>
  static U value(
      const Term<R>& ex, const Evaluator<R>& = Evaluator<R>(), bool = false
  ) {
    return ex;
  }
  template <class R>
  static U value(
      const Expression<R>& ex, const Evaluator<R>& = Evaluator<R>(),
      bool = false
  ) {
    return ex;
  }
...
struct evaluate_helper<Expression<U> > {
  typedef U value_type;
  static Expression<U> value(
      const Term<U>& ex, const Evaluator<U>& ev = Evaluator<U>(),
      bool isarg = false
  ) {
    Term<U> t(ex);
  ...
  static Expression<U> value(
      const Expression<U>& ex, const Evaluator<U>& ev = Evaluator<U>(),
      bool isarg = false
  ) {
    Expression<U> e(ex);
...
struct evaluate_helper<double> {
  ...
  template <class R>
  static double value(
      const Term<R>& ex, const Evaluator<R>& ev = Evaluator<R>(),
      bool isarg = false
  ) {
  ...
  template <class R>
  static double value(
      const Expression<R>& ex, const Evaluator<R>& ev = Evaluator<R>(),
      bool isarg = false
  ) {
    return alps::expression::numeric_cast<double>(ex.value(ev, isarg));
  }
...
struct evaluate_helper<float> {
  ...
  static float evaluate_symbol(const std::string& name, bool = false) {
    if (name == "Pi" || name == "PI" || name == "pi")
      return static_cast<float>(std::acos(-1.));
    boost::throw_exception(std::runtime_error("can not evaluate " + name));
    return 0.;
  }
};
...
struct evaluate_helper<std::complex<U> > {
  ...
  static std::complex<U> evaluate_symbol(
      const std::string& name, bool = false
  ) {
    if (name == "Pi" || name == "PI" || name == "pi") return std::acos(-1.);
