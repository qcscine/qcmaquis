...
/* $Id: symbol.h 1646 2005-05-13 17:46:35Z troyer $ */
...
class Symbol : public Evaluatable<T> {
 public:
  ...
  bool can_evaluate(const Evaluator<T>& ev = Evaluator<T>(), bool isarg = false)
      const {
...
template <class T>
typename Symbol<T>::value_type Symbol<T>::value(
    const Evaluator<T>& eval, bool isarg
) const {
...
template <class T>
Evaluatable<T>* Symbol<T>::partial_evaluate_replace(
