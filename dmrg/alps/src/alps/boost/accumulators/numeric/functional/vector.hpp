...
typename lazy_enable_if<
    is_scalar<Right>,
    detail::make_vector<functional::divides<Left, Right> > >::type
...
template <typename Left, typename Right>
std::vector<typename functional::divides<Left, Right>::result_type> operator/(
    std::vector<Left> const &left, std::vector<Right> const &right
) {
  typedef typename functional::divides<Left, Right>::result_type value_type;
  std::vector<value_type> result(left.size());
  for (std::size_t i = 0, size = result.size(); i != size; ++i) {
...
typename lazy_enable_if<
    is_scalar<Right>,
    detail::make_vector<functional::multiplies<Left, Right> > >::type
operator*(std::vector<Left> const &left, Right const &right) {
...
// Handle vector<Left> + vector<Right>
template <typename Left, typename Right>
std::vector<typename functional::plus<Left, Right>::result_type> operator+(
    std::vector<Left> const &left, std::vector<Right> const &right
) {
...
// Handle vector<Left> - vector<Right>
template <typename Left, typename Right>
std::vector<typename functional::minus<Left, Right>::result_type> operator-(
    std::vector<Left> const &left, std::vector<Right> const &right
) {
...
std::vector<Left> &operator+=(
    std::vector<Left> &left, std::vector<Left> const &right
) {
...
std::vector<typename functional::unary_minus<Arg>::result_type> operator-(
...
struct average<Left, Right, std_vector_tag, void>
    : mpl::if_<
          ...
          divides<Left, double const>, divides<Left, Right> >::type {};
...
struct promote<ToFrom, ToFrom, std_vector_tag, std_vector_tag>
  ...
  ToFrom &operator()(ToFrom &tofrom) const { return tofrom; }
};
...
struct as_min<T, std_vector_tag>
  ...
  typename remove_const<T>::type operator()(T &arr) const {
    return 0 == arr.size() ? T() : T(arr.size(), numeric::as_min(arr[0]));
  }
};
...
struct as_max<T, std_vector_tag>
    : std::unary_function<T, typename remove_const<T>::type> {
  typename remove_const<T>::type operator()(T &arr) const {
    return 0 == arr.size() ? T() : T(arr.size(), numeric::as_max(arr[0]));
  }
};
...
struct as_zero<T, std_vector_tag>
    : std::unary_function<T, typename remove_const<T>::type> {
  typename remove_const<T>::type operator()(T &arr) const {
    return 0 == arr.size() ? T() : T(arr.size(), numeric::as_zero(arr[0]));
  }
};
...
struct as_one<T, std_vector_tag>
    : std::unary_function<T, typename remove_const<T>::type> {
  typename remove_const<T>::type operator()(T &arr) const {
    return 0 == arr.size() ? T() : T(arr.size(), numeric::as_one(arr[0]));
  }
};
