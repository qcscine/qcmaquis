...
inline U cast_generic(T arg, X) {
  throw bad_cast(
      std::string("cannot cast from ") + typeid(T).name() + " to " +
...
template <typename U, typename T>
inline U cast_generic(T arg, boost::mpl::int_<1> const&) {
  return arg;
}
...
#define ALPS_NGS_CAST_STRING(T, p, c)                                       \
  ...
  template <>                                                               \
  struct cast_hook<T, std::string> {                                        \
    static inline T apply(std::string arg) {                                \
      T value = 0;                                                          \
      if (arg.size() && sscanf(arg.c_str(), "%" c, &value) < 0)             \
        throw std::runtime_error(                                           \
            "error casting from string to " #T ": " + arg + ALPS_STACKTRACE \
        );                                                                  \
      return value;                                                         \
    }                                                                       \
  };
...
struct cast_hook<std::complex<U>, T> {
  static inline std::complex<U> apply(T const& arg) { return cast<U>(arg); }
};
...
struct cast_hook<std::string, std::complex<T> > {
  static inline std::string apply(std::complex<T> const& arg) {
    return cast<std::string>(arg.real()) + "+" + cast<std::string>(arg.imag()) +
           "i";
  }
};
...
template <typename T>
struct cast_hook<boost::filesystem::path, T> {
...
template <typename T>
struct cast_hook<boost::filesystem::path, std::complex<T> > {
...
template <typename U, typename T>
inline U cast(T const& arg) {
  return cast_hook<U, T>::apply(arg);
}

template <typename U, typename T>
inline void cast(U const* src, U const* end, T* dest) {
  for (U const* it = src; it != end; ++it) dest[it - src] = cast<T>(*it);
}
}  // namespace alps
