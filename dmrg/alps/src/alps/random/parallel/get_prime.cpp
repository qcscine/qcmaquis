...
struct primes_storage {
  ...
  int operator[](int i) const { return primes_[i]; }

  int size() const { return obtained; }
...
boost::uint64_t get_prime_64(unsigned int offset) {
  ...
  if (offset < PRIMELISTSIZE1) return primelist_64[offset];

  unsigned int largest = MAXPRIME;

  int index =
      (unsigned int)((offset - PRIMELISTSIZE1 + 1) / STEP) + PRIMELISTSIZE1 - 1;
  ...
  boost::throw_exception(
      std::runtime_error(std::string("Insufficient number of primes"))
  );
