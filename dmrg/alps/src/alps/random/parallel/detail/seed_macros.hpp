...
#define ALPS_RANDOM_PARALLEL_ITERATOR_SEED_DEFAULT_IMPL(z, n, unused)       \
  ...
  void seed(                                                                \
    ...
    if (f == l) throw std::invalid_argument("invalid seeding argument");    \
    seed(                                                                   \
        alps::random::parallel::global_seed =                               \
            *f++ BOOST_PP_COMMA_IF(n) BOOST_PP_ENUM_PARAMS(n, x)            \
    );                                                                      \
  }
...
#define ALPS_RANDOM_PARALLEL_ITERATOR_SEED_IMPL(z, n, unused)                 \
  ...
  void seed(                                                                  \
    ...
    iterator_seed_named(                                                      \
        alps::random::parallel::first = f, alps::random::parallel::last =     \
                                               l BOOST_PP_COMMA_IF(n)         \
...
#define ALPS_RANDOM_PARALLEL_SEED_PARAMS(RNG, PARAMS, INIT)      \
  BOOST_PP_REPEAT_FROM_TO(                                       \
      0, ALPS_RANDOM_MAXARITY, ALPS_RANDOM_PARALLEL_CONSTRUCTOR, \
      (2, (RNG, INIT))                                           \
  )                                                              \
  BOOST_PARAMETER_MEMFUN(void, seed_named, 0, ALPS_RANDOM_MAXARITY, PARAMS)

#define ALPS_RANDOM_PARALLEL_SEED(RNG)                           \
  ALPS_RANDOM_PARALLEL_SEED_PARAMS(                              \
      RNG, alps::random::parallel::seed_params, BOOST_PP_EMPTY() \
  )
...
#define ALPS_RANDOM_PARALLEL_ITERATOR_SEED_PARAMS(PARAMS)                                                       \
  ...
  BOOST_PARAMETER_MEMFUN(                                                                                       \
      void, iterator_seed_named, 2, ALPS_RANDOM_MAXARITY, PARAMS                                                \
  )

#define ALPS_RANDOM_PARALLEL_ITERATOR_SEED()       \
  ALPS_RANDOM_PARALLEL_ITERATOR_SEED_PARAMS(       \
      alps::random::parallel::iterator_seed_params \
  )
