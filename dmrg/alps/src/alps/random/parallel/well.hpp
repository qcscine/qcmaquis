...
///
/// This class template implements the WELL generator of F. Panneton, P.
/// L'Ecuyer and M. Matsumoto
template <
    class UIntType, int statesize, UIntType val, class F1, class F2, class F3,
    class F4, class F5, class F6, class F7, class F8, int p1, int p2, int p3,
    UIntType mask = statesize - 1, class RNG = alps::lcg64a>
class well {
 public:
  ...
  BOOST_STATIC_CONSTANT(
      typename RNG::result_type, max_streams = RNG::max_streams
  );
  ...
  ///
  /// All standard and named parameter constructors of random number generator
  /// and parallel random number generators are provided
  well(...);

  /// @brief the seed fuctions
  ///
  /// All standard and named parameter seed functions of random number generator
  /// and parallel random number generators are provided
  void seed(...);
  ...
  ALPS_RANDOM_PARALLEL_SEED(well) {
    ...
    for (unsigned int i = 0; i < 1000; i++) rng();

    // seed the generator
    for (int i = 0; i < statesize; i++) state[i] = rng();
    state_i = 0;
  }
  ...
  friend bool operator==(const well& x, const well& y) {
    for (int j = 0; j < statesize; ++j)
      if (x.state[j] != y.state[j]) return false;
    return true;
  }

  // !=

  friend bool operator!=(const well& x, const well& y) { return !(x == y); }

  // ostream

  template <class CharT, class Traits>
  friend std::basic_ostream<CharT, Traits>& operator<<(
      std::basic_ostream<CharT, Traits>& os, const well& w
  ) {
    for (int j = 0; j < statesize; j++) os << w.state[j] << " ";
    os << w.state_i;
    return os;
  }
  ...
  friend std::basic_istream<CharT, Traits>& operator>>(
      std::basic_istream<CharT, Traits>& is, well& w
  ) {
    for (int j = 0; j < statesize; j++) is >> w.state[j] >> std::ws;
    is >> w.state_i;
    return is;
  }
  ...
  ///
  /// The validation function checks whether the passed value is the 10'000-th
  /// integer generated from a default-seeded generator. The 10'000-th integer
  /// is determined with the original RNG of F. Panneton, P. L'Ecuyer and M.
  /// Matsumoto, see http://www.iro.umontreal.ca/~panneton/WELLRNG.html
  static bool validation(result_type value) { return (val == value); }

  /// @returns the next random number
  result_type operator()() {
    result_type z0 = state[(state_i + (statesize - 1)) & mask];
    result_type z1 =
        F1::f(state[state_i]) ^ F2::f(state[(state_i + p1) & mask]);
    result_type z2 = F3::f(state[(state_i + p2) & mask]) ^
                     F4::f(state[(state_i + p3) & mask]);
...
template <
    class UIntType, int statesize, UIntType val, class F1, class F2, class F3,
    class F4, class F5, class F6, class F7, class F8, int p1, int p2, int p3,
    UIntType mask, class RNG>
const bool well<
    UIntType, statesize, val, F1, F2, F3, F4, F5, F6, F7, F8, p1, p2, p3, mask,
    RNG>::has_fixed_range;
...
typedef well<
    uint32_t, 16, 1584514050u, mat0neg<-16>, mat0neg<-15>, mat0pos<11>, zero,
    mat0neg<-2>, mat0neg<-18>, mat3neg<-28>, mat4neg<-5>, 13, 9, 1>
    well512a;

/// A 1024-bit version of the WELL generator
typedef well<
    uint32_t, 32, 2159746073u, identity, mat0pos<8>, mat0neg<-19>, mat0neg<-14>,
    mat0neg<-11>, mat0neg<-7>, mat0neg<-13>, zero, 3, 24, 10>
    well1024a;
