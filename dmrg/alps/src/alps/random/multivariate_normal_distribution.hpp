...
/// @file This header provides a multi-variate normal distribution of correlated
/// normally distributed random numbers.

namespace alps {

/// @brief a distribution creating correlated multi-variate normally distributed
/// numbers
///
/// the multivariate normal distribution creates correlated random numbers with
/// a specified mean vector and covariance matrix. Instead of using a vector as
/// the @c result_type, the
/// @c result_type is a scalar real number, and the @c operator() should be
/// called once for each element of the vector.
...
class multivariate_normal_distribution {
 public:
  ...
  /// the type of the matrix storing the Cholesky decomposition of the
  /// covariance matrix
  ...
  /// decomposition any other square root of the covariance matrix could be
  /// passed.
  ///
  /// Requires:  mean.size() == cholesky.size1() and cholesky.size1() ==
  /// cholesky.size2()

  multivariate_normal_distribution(
      const matrix_type& cholesky, const vector_type& mean
  )
      : mean_(mean),
        cholesky_(cholesky),
        buffer_(mean_.size()),
        ptr_(buffer_.end()) {
    BOOST_ASSERT(
        mean_.size() == cholesky_.size1() && mean_.size() == cholesky_.size2()
    );
  }

  /// @brief the constructor of the multi-variate normal distribution with zero
  /// mean
  /// @param cholesky the Cholesky decomposition of the covariance matrix
  ///
  ...
  /// INTERNAL ONLY
  multivariate_normal_distribution& operator=(
      const multivariate_normal_distribution& other
  ) {
    mean_ = other.mean_;
    cholesky_ = other.cholesky_;
  ...
  /// distributed random numbers, caches it, and returns the first value. The
  /// next n-1 calls return the next elements of the vector. The n+1-st call
  /// again creates a new vector of multivariate normally distributed random
  /// numbers.
  ...
  friend std::basic_ostream<CharT, Traits>& operator<<(
      ...
      const multivariate_normal_distribution& mnd
  ) {
  ...
  friend std::basic_istream<CharT, Traits>& operator>>(
      ...
      multivariate_normal_distribution& mnd
  ) {
    std::size_t pos;
    is >> std::ws >> mnd.mean_ >> std::ws >> mnd.cholesky_ >> std::ws >>
        mnd.buffer_ >> std::ws >> pos;
