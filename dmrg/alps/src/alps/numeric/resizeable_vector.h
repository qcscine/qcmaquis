...
class rsvector {
 public:
  ...
  rsvector &operator=(const rsvector &V) {
    ...
    if (size_ > 0) memcpy(values_, V.values_, size_ * sizeof(double));
    return *this;
  }
  rsvector(const rsvector &V) {
    ...
    size_ = V.size_;
    memory_size_ = V.size_;
    if (size_ > 0) memcpy(values_, V.values_, sizeof(double) * V.size_);
  }
  ...
  void clear() { memset(values_, 0, sizeof(double) * size_); }
  void exp(double c) {
