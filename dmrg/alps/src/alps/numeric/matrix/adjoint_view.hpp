...
struct transpose_helper<matrix<T,MemoryBlock> > {
    static matrix<T,MemoryBlock> apply( adjoint_view<matrix<T,MemoryBlock> > const& v) {
        typedef typename adjoint_view<matrix<T,MemoryBlock> >::const_col_element_iterator const_col_element_iterator;
...
//        const_element_iterator;                       // Const version of element_iterator (REALLY SLOW! USE row_-/column_iterators INSTEAD!)
    ...
    explicit adjoint_view(Matrix const& m)
    : m_(m){
    };
...
struct multiply_return_type<Matrix1,adjoint_view<Matrix2>,tag::matrix,tag::matrix>
: multiply_return_type<Matrix1,Matrix2,tag::matrix,tag::matrix>
{
};

template <typename Matrix1, typename Matrix2>
struct multiply_return_type<adjoint_view<Matrix1>,Matrix2,tag::matrix,tag::matrix>
: multiply_return_type<Matrix1,Matrix2,tag::matrix,tag::matrix>
{
};

template <typename Matrix1, typename Matrix2>
struct multiply_return_type<adjoint_view<Matrix1>,adjoint_view<Matrix2>,tag::matrix,tag::matrix>
: multiply_return_type<Matrix1,Matrix2,tag::matrix,tag::matrix>
{
};

template <typename Matrix1, typename Matrix2>
struct plus_minus_return_type<adjoint_view<Matrix1>, Matrix2, tag::matrix, tag::matrix>
: plus_minus_return_type<Matrix1,Matrix2,tag::matrix,tag::matrix>
{
};

template <typename Matrix1, typename Matrix2>
struct plus_minus_return_type<Matrix1, adjoint_view<Matrix2>, tag::matrix, tag::matrix>
: plus_minus_return_type<Matrix1,Matrix2,tag::matrix,tag::matrix>
{
};

template <typename Matrix1, typename Matrix2>
struct plus_minus_return_type<adjoint_view<Matrix1>, adjoint_view<Matrix2>, tag::matrix, tag::matrix>
: plus_minus_return_type<Matrix1,Matrix2,tag::matrix,tag::matrix>
{
};
