...
void resize(
    matrix<T, MemoryBlock>& m, typename matrix<T, MemoryBlock>::size_type i,
...
template <typename T, typename MemoryBlock>
void remove_cols(
    matrix<T, MemoryBlock>& m, typename matrix<T, MemoryBlock>::size_type j,
    typename matrix<T, MemoryBlock>::difference_type k = 1
) {
  BOOST_CONCEPT_ASSERT((alps::numeric::ResizableMatrix<matrix<T, MemoryBlock> >)
  );
...
void append_rows(
    matrix<T, MemoryBlock>& m, INPUT_ITERATOR_PAIR range,
    typename matrix<T, MemoryBlock>::difference_type k = 1
) {
  BOOST_CONCEPT_ASSERT((alps::numeric::ResizableMatrix<matrix<T, MemoryBlock> >)
  );
