...
struct transpose_helper<matrix<T, MemoryBlock> > {
  static matrix<T, MemoryBlock> apply(
      transpose_view<matrix<T, MemoryBlock> > const& v
  ) {
    typedef typename transpose_view<matrix<T, MemoryBlock> >::
...
class transpose_view {
 public:
  ...
  explicit transpose_view(Matrix const& m) : m_(m){};
...
struct multiply_return_type<
    Matrix1, transpose_view<Matrix2>, tag::matrix, tag::matrix>
    : multiply_return_type<Matrix1, Matrix2, tag::matrix, tag::matrix> {};

template <typename Matrix1, typename Matrix2>
struct multiply_return_type<
    transpose_view<Matrix1>, Matrix2, tag::matrix, tag::matrix>
    : multiply_return_type<Matrix1, Matrix2, tag::matrix, tag::matrix> {};

template <typename Matrix1, typename Matrix2>
struct multiply_return_type<
    transpose_view<Matrix1>, transpose_view<Matrix2>, tag::matrix, tag::matrix>
    : multiply_return_type<Matrix1, Matrix2, tag::matrix, tag::matrix> {};
