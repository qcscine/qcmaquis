...
#define ALPS_UNUSED(x) static_cast<void>(x)
 ...
 public:
  ...
  BOOST_CONCEPT_USAGE(DiagonalIteratableMatrix) {
    ...
    std::pair<diagonal_iterator, diagonal_iterator> diagonal_range =
        diagonal(x);
    std::pair<const_diagonal_iterator, const_diagonal_iterator>
        const_diagonal_range = diagonal(y);
...
struct IteratableMatrix : DiagonalIteratableMatrix<X> {
 public:
  ...
  BOOST_CONCEPT_USAGE(IteratableMatrix) {
    ...
    std::pair<const_row_element_iterator, const_row_element_iterator>
        const_row_range = row(y, 0);
    std::pair<const_col_element_iterator, const_col_element_iterator>
        const_col_range = col(y, 0);
  }
};
