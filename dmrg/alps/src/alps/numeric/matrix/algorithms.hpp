 ...
 *                              Andreas Hehn <hehn@phys.ethz.ch> * Michele Dolfi
 * <dolfim@phys.ethz.ch>                * Tim Ewart <timothee.ewart@unige.ch> *
 *                                                                                 *
...
typename Matrix::value_type trace(Matrix const& m) {
  ...
  typename Matrix::value_type tr = accumulate(
      diagonal(m).first, diagonal(m).second, typename Matrix::value_type(0)
  );
  return tr;
}
...
Matrix inverse(Matrix M) {
  ...
  if (info != 0) throw std::runtime_error("Error in GETRF !");

  info = boost::numeric::bindings::lapack::getri(M, ipiv);
  if (info != 0) throw std::runtime_error("Error in GETRI !");
...
template <typename Matrix>
void copy_block(
    Matrix const& A, typename Matrix::size_type ai,
    typename Matrix::size_type aj, Matrix& B, typename Matrix::size_type bi,
    typename Matrix::size_type bj, typename Matrix::difference_type m,
    typename Matrix::difference_type n
) {
  assert(num_cols(B) >= bj + n);
  assert(num_rows(B) >= bi + m);
  for (typename Matrix::difference_type j = 0; j < n; ++j)
    std::copy(
        A.col(aj + j).first + ai, A.col(aj + j).first + ai + m,
        B.col(bj + j).first + bi
    );
}
...
Matrix direct_sum(Matrix const& a, Matrix const& b) {
  ...
  for (st r = 0; r < num_rows(a); ++r)
    for (st c = 0; c < num_cols(a); ++c) ret(r, c) = a(r, c);
    ...
    for (size_t j1 = 0; j1 < num_cols(M1); ++j1)
      for (size_t i2 = 0; i2 < num_rows(M2); ++i2)
        for (size_t j2 = 0; j2 < num_cols(M2); ++j2)
          MM(i1 * num_rows(M1) + i2, j1 * num_cols(M1) + j2) =
              M1(i1, j1) * M2(i2, j2);
  return MM;
}
...
bool is_hermitian(Matrix const& M) {
  ...
  if (num_rows(M) != num_cols(M)) return false;
  for (size_t i = 0; i < num_rows(M); ++i)
    for (size_t j = 0; j < num_cols(M); ++j)
      if (M(i, j) != conj(M(j, i))) return false;
  return true;
}
...
template <typename T, class memoryblock>
typename matrix<T, memoryblock>::value_type overlap(
    const matrix<T, memoryblock>& M1, const matrix<T, memoryblock>& M2
) {
...
struct qrhelper {
  int static getq(
      ...
      typename associated_vector<
          matrix<typename detail::sv_type<T>::type, memoryblock> >::type& tau
  ) {
...
struct qrhelper<std::complex<T>, memoryblock> {
  int static getq(
      matrix<std::complex<T>, memoryblock>& q,
      typename associated_vector<matrix<std::complex<T>, memoryblock> >::type&
          tau
  ) {
...
template <typename T, class MemoryBlock>
void svd(
    matrix<T, MemoryBlock> M, matrix<T, MemoryBlock>& U,
  ...
  typename matrix<T, MemoryBlock>::size_type k =
      (std::min)(num_rows(M), num_cols(M));
  resize(U, num_rows(M), k);
  resize(V, k, num_cols(M));
  resize(S, k, k);

  int info = boost::numeric::bindings::lapack::gesvd(
      'S', 'S', M, S.get_values(), U, V
  );
...
template <typename T, class MemoryBlock>
void qr(
    matrix<T, MemoryBlock> M, matrix<T, MemoryBlock>& Q,
    matrix<T, MemoryBlock>& R
) {
  typename matrix<T, MemoryBlock>::size_type k =
      (std::min)(num_rows(M), num_cols(M));
  typename associated_vector<
      matrix<typename detail::sv_type<T>::type, MemoryBlock> >::type tau(k);

  int info = boost::numeric::bindings::lapack::geqrf(M, tau);
  if (info != 0) throw std::runtime_error("Error in GEQRF !");
  ...
  for (std::size_t c = 0; c < num_cols(R); ++c)
    for (std::size_t r = 0; r <= c && r < num_rows(R); ++r) R(r, c) = M(r, c);
  // case M(m,n) with m < n, it will be useless if direct access to the lapack
  // wrapper not pb because M is passed by copy !
  if (num_rows(M) < num_cols(M)) {
    resize(M, k, k);
  }
  // get Q
  info = detail::qrhelper<T, MemoryBlock>::getq(M, tau);
  if (info != 0) throw std::runtime_error("Error in GRGQR !");
  std::copy(elements(M).first, elements(M).second, elements(Q).first);
}

template <typename T, class MemoryBlock>
void lq(
    matrix<T, MemoryBlock> M, matrix<T, MemoryBlock>& L,
    matrix<T, MemoryBlock>& Q
) {
  typename matrix<T, MemoryBlock>::size_type k =
      (std::min)(num_rows(M), num_cols(M));
  ...
  if (info != 0) throw std::runtime_error("Error in GELQF !");
  ...
  for (std::size_t c = 0; c < num_cols(L); ++c)
    for (std::size_t r = c; r < num_rows(L); ++r) L(r, c) = M(r, c);

  // case M(m,n) with m > n, it will be useless if direct access to the lapack
  // wrapper not pb because M is passed by copy !
  if (num_rows(M) > num_cols(M)) resize(M, k, k);
  // get Q
  info = detail::lqhelper<T, MemoryBlock>::getq(M, tau);
  if (info != 0) throw std::runtime_error("Error in GRGLQ !");
  std::copy(elements(M).first, elements(M).second, elements(Q).first);
}

template <typename T, class MemoryBlock>
matrix<T, MemoryBlock> exp(
    matrix<T, MemoryBlock> M, T const& alpha = 1, bool doShift = false
) {
  BOOST_STATIC_ASSERT(boost::is_complex<T>::value);
  typedef
      typename boost::numeric::bindings::remove_imaginary<T>::type real_type;
  assert(num_rows(M) == num_cols(M));
  ...
  if (info != 0) throw std::runtime_error("Error in GEEV !");
...
/**
 * @brief Calculates the exponential of an Hermitian matrix via the
 * eigendecomposition.
 *
 * Note that this method calculates exp(alpha*M), where alpha is a generic
 * scalar factor. The method offers the possibility of shifting the exponential
 * by the maximum eigenvalue, to avoid that the norm explodes. This happens for
 * real-valued exponentials, when alpha*norm(L) >> 1.
 *
 * @tparam T type associated with the memory entries
 * @tparam MemoryBlock data type underlying the memory storage (by default,
 * std::vector)
 ...
 */
template <typename T, class MemoryBlock>
matrix<T, MemoryBlock> exp_hermitian(
    matrix<T, MemoryBlock> M, const T& alpha = 1, bool doShift = false
) {
...
template <typename T, class MemoryBlock>
void heev(
    matrix<T, MemoryBlock> M, matrix<T, MemoryBlock>& evecs,
  ...
  for (std::size_t c = 0; c < num_cols(M); ++c)
    std::copy(
        col(M, c).first, col(M, c).second, col(evecs, num_cols(M) - 1 - c).first
    );
}
...
void heev(
    matrix<T, MemoryBlock> M, matrix<T, MemoryBlock>& evecs,
    typename associated_real_diagonal_matrix<matrix<T, MemoryBlock> >::type&
        evals
) {
  assert(num_rows(M) == num_cols(M));
  typename associated_real_vector<matrix<T, MemoryBlock> >::type evals_(
      num_rows(M)
  );
  heev(M, evecs, evals_);
  evals =
 ...
 *
 * Note that, unlike the Hermitean case, here we don't sort the eigenvectors
 * based on the corresponding eigenvalue because the eigenvalues are not ensure
 * to be real, so a proper comparison operator is not defined.
 *
 * @tparam T Scalar type associated with the matrix entries.
 * @tparam MemoryBlock Class used to store the entries of the matrices.
 * @param M input matrix.
...
template <typename T, class MemoryBlock>
void geev(
    matrix<T, MemoryBlock> M, matrix<T, MemoryBlock>& eVecsLeft,
    matrix<T, MemoryBlock>& eVecsRight,
    typename associated_vector<matrix<T, MemoryBlock> >::type& eVals
) {
  // Standard checks
  assert(num_rows(M) == num_cols(M));
  assert(eVals.size() == num_rows(M));
  int info = boost::numeric::bindings::lapack::geev(
      'V', 'V', M, eVals, eVecsLeft, eVecsRight
  );
}
 ...
 */
template <typename T, class MemoryBlock>
typename boost::enable_if<boost::is_complex<T>, void>::type ggev(
    matrix<T, MemoryBlock> M, matrix<T, MemoryBlock> overlapMatrix,
  ...
  using RealVectorType =
      typename associated_vector<matrix<RealType, MemoryBlock> >::type;
  RealVectorType leftPermutations(num_rows(M), 0.),
      rightPermutations(num_rows(M), 0.), eValCond(num_rows(M), 0.),
      eVecCond(num_rows(M), 0.);
  // Actual calculation
  fortran_int_t minSize = 1, maxSize = num_rows(M);
  boost::numeric::bindings::lapack::ggevx(
      'B', 'V', 'V', 'B', M, overlapMatrix, alphaVec, betaVec, leftEVecs,
      rightEVecs, minSize, maxSize, leftPermutations, rightPermutations, aNorm,
      bNorm, eValCond, eVecCond
  );
}
...
typename boost::enable_if<boost::is_complex<T>, void>::type ggev(
  ...
  for (int iVec = 0; iVec < num_rows(M); iVec++)
    evals[iVec] = (std::abs(betaVec[iVec]) > threshold)
                      ? alphaVec[iVec] / betaVec[iVec]
                      : 0.;
}
...
ggev(
  ...
  fortran_int_t minSize = 1, maxSize = num_rows(M);
  boost::numeric::bindings::lapack::ggevx(
      'B', 'V', 'V', 'B', M, overlapMatrix, alphaVecReal, alphaVecImag, betaVec,
      leftEVecs, rightEVecs, minSize, maxSize, leftPermutations,
      rightPermutations, aNorm, bNorm, eValCond, eVecCond
  );
}
...
ggev(
  ...
  assert(evals.size() == num_rows(M));
  using VectorType = typename associated_vector<matrix<T, MemoryBlock> >::type;
  VectorType alphaVecReal(num_rows(M), 0.), alphaVecImag(num_rows(M), 0.),
      betaVec(num_rows(M), 0.);
  ggev(
      M, overlapMatrix, alphaVecReal, alphaVecImag, betaVec, leftEVecs,
      rightEVecs, threshold
  );
  // Loads the final results
  for (int iVec = 0; iVec < num_rows(M); iVec++)
    evals[iVec] =
        (std::abs(betaVec[iVec]) > threshold)
            ? std::complex<T>(alphaVecReal[iVec], alphaVecImag[iVec]) /
                  betaVec[iVec]
            : 0.;
}

/*
 * Some block_matrix algorithms necessitate nested specialization due to ambient
 * scheduler the algos are full rewritten or partly with subset specialization
 * an alternative implementation is presented inside
 * p_dense_matrix/algorithms.hpp
 */
}  // namespace numeric
}  // end namespace alps
