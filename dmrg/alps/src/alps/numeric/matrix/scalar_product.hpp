...
struct scalar_product_return_type
    : detail::auto_deduce_multiply_return_type<
          typename element_type<T1>::type, typename element_type<T2>::type> {};
...
template <typename T1, typename T2>
typename scalar_product_return_type<T1, T2>::type scalar_product_impl(
    T1 const& t1, T2 const& t2, boost::mpl::false_
) {
  using alps::numeric::conj;
  typename scalar_product_return_type<T1, T2>::type r(0);
  for (std::size_t i = 0; i < t1.size(); ++i) r += conj(t1[i]) * t2[i];
  return r;
}
...
//
// template <typename T1, typename T2>
// typename scalar_product_return_type<T1,T2>::type scalar_product_impl(T1
// const& t1, T2 const& t2, boost::mpl::true_)
//{
//    ALPS_NUMERIC_MATRIX_DEBUG_OUTPUT( "using blas dot for " <<
//    typeid(t1).name() << " " << typeid(t2).name() ); return
//    boost::numeric::bindings::blas::dot(t1,t2);
//}

template <typename T1, typename T2>
typename scalar_product_return_type<T1, T2>::type scalar_product(
