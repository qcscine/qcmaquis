...
struct fourier_real {
  ...
  static T evaluate(
    ...
    std::transform(
        boost::counting_iterator<T>(0), boost::counting_iterator<T>(A.size()),
        std::back_inserter(_2PInx_over_N), boost::lambda::_1 * _2PIx_over_N
    );
    ...
    return (
        std::accumulate(COS.begin(), COS.end(), 0.) +
        std::accumulate(SIN.begin() + 1, SIN.end(), 0.)
    );
  }  // objective:  F(x) = a0 + a1 * cos(2\pix/N) + a2 * cos(4\pix/N) + ... + b1
     // * sin(2\pix/N) + b2 * sin(4\pix/N) + ...
};
