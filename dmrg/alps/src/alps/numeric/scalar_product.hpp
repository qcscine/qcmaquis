...
// The generic implementation of the scalar_product moved to
// alps/numeric/matrix/scalar_product.hpp

/// \overload
template <class T>
inline T scalar_product(
    const std::valarray<T>& c1, const std::valarray<T>& c2
) {
  return std::inner_product(
      data(c1), data(c1) + c1.size(), data(c2), T(), std::plus<T>(),
      conj_mult<T, T>()
  );
}
