...
/* $Id: real.hpp 6379 2012-08-30 12:24:49Z dolfim $ */
...
template <class T>
inline typename real_type<T>::type real(T);
...
struct real_hook<std::vector<std::complex<T> > > {
  static inline std::vector<T> apply(std::vector<std::complex<T> > const& x) {
    ...
    re.reserve(x.size());
    std::transform(
        x.begin(), x.end(), std::back_inserter(re),
        static_cast<T (*)(std::complex<T> const&)>(
...
struct real_hook<std::vector<std::vector<T> > > {
  static inline std::vector<std::vector<T> > const& apply(
...
struct real_hook<std::vector<std::vector<std::complex<T> > > > {
  static inline std::vector<std::vector<T> > apply(
      std::vector<std::vector<std::complex<T> > > const& x
  ) {
    std::vector<std::vector<T> > re;
    re.reserve(x.size());
    std::transform(
        x.begin(), x.end(), std::back_inserter(re),
        static_cast<std::vector<T> (*)(std::vector<std::complex<T> > const&)>(
            &real_hook<std::vector<std::complex<T> > >::apply
        )
    );
    return re;
  }
