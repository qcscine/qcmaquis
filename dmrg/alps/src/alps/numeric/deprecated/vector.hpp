...
void plus_assign(
  ...
  std::transform(
      first1, last1, first2, first1,
      std::plus<typename std::iterator_traits<InputIterator2>::value_type>()
  );
}
...
void minus_assign(
  ...
  std::transform(
      first1, last1, first2, first1,
      std::minus<typename std::iterator_traits<InputIterator2>::value_type>()
  );
}
template <typename T>
class vector : public std::vector<T> {
 public:
  ...
  friend void swap(vector<T>& x, vector<T>& y) { std::swap(x, y); }
...
void multiplies_assign(ForwardIterator start1, ForwardIterator end1, T lambda) {
  std::transform(start1, end1, start1, [&lambda](T t) { return t * lambda; });
}
...
inline vector<T> exp(T c, vector<T> v) {
  ...
  v *= c;
  std::transform(
      v.begin(), v.end(), result.begin(), static_cast<T (*)(T)>(&exp)
  );
  return result;
}
...
inline vector<double> exp(double c, vector<double> v) {
  ...
  using std::exp;
  std::transform(
      v.begin(), v.end(), result.begin(), static_cast<double (*)(double)>(&exp)
  );
#endif
#endif
#endif
...
#define PLUS_ASSIGN(T)                                                 \
  void plus_assign(                                                    \
      std::vector<T>::iterator first1, std::vector<T>::iterator last1, \
      std::vector<T>::const_iterator first2                            \
  ) {                                                                  \
    boost::numeric::bindings::blas::detail::axpy(                      \
        last1 - first1, 1., &*first2, 1, &*first1, 1                   \
    );                                                                 \
  }
...
#define MULTIPLIES_ASSIGN(T)                                                   \
  void multiplies_assign(                                                      \
      std::vector<T>::iterator start1, std::vector<T>::iterator end1, T lambda \
  ) {                                                                          \
    boost::numeric::bindings::blas::detail::scal(                              \
        end1 - start1, lambda, &*start1, 1                                     \
    );                                                                         \
  }
...
#define SCALAR_PRODUCT(T)                                \
  inline T scalar_product(                               \
      const std::vector<T>& v1, const std::vector<T>& v2 \
  ) {                                                    \
    return boost::numeric::bindings::blas::detail::dot(  \
        v1.size(), &v1[0], 1, &v2[0], 1                  \
    );                                                   \
  }
