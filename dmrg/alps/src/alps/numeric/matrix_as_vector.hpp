...
/* $Id: matrix_as_vector.hpp 3958 2010-03-05 09:24:06Z troyer $ */
...
class element_proxy {
 public:
  ...
  ///
  /// the scale factor is initialized to 1 and the proxy refers to the \a i -th
  /// element of the vector \param m the matrix \param i the index of the vector
  /// element
  element_proxy(matrix_type& m, size_type i)
      : matrix_(m), index_(i), val_(1.) {}
  ...
  const element_proxy<M>& operator*=(value_type x) {
    val_ *= x;
    return *this;
  }
  ...
  template <class MM>
  void operator=(const element_proxy<MM>& x) {
    matrix_(index(), x.index()) = x.value();
  }
...
///
/// This class allows to use a matrix instead of a vector in some generic
/// algorithms. For example, vector operations such as a[j] = b * c[i] result in
/// the value b being stored at location (i,j) in the matrix
/// @param M the matrix type. It needs to support element access in the form
/// m(i,j), for objects m of type M.
template <class M>
class matrix_as_vector {
 public:
  ...
  detail::element_proxy<M> operator[](size_type i) {
    return detail::element_proxy<M>(matrix(), i);
  }
  /// \param i the index into the vector
  /// \returns an element_proxy referring to the \a i -th vector element
  detail::element_proxy<const M> operator[](size_type i) const {
    return detail::element_proxy<const M>(matrix(), i);
  }
...
alps::numeric::detail::element_proxy<M> operator*(
    alps::numeric::detail::element_proxy<M> x, T y
) {
  return x *= y;
}
...
template <class M, class T>
alps::numeric::detail::element_proxy<M> operator*(
    T x, alps::numeric::detail::element_proxy<M> y
) {
  return y *= x;
}
