...
template <
    class InputIterator, class BinaryOperation, class BinaryPredicate, class T>
T accumulate_if(
    InputIterator first, InputIterator last, T init, BinaryOperation binary_op,
    BinaryPredicate pred
) {
  while (first != last) {
    if (pred(*first)) init = binary_op(init, *first);
    ++first;
  }
  return init;
}
...
T accumulate_if(
    InputIterator first, InputIterator last, T init, BinaryPredicate pred
) {
