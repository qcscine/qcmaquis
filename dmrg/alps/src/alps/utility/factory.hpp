...
/* $Id: factory.hpp 5358 2011-01-08 12:43:43Z troyer $ */
...
/// The factory can create instances of default-constructible types derived from
/// the type \a BASE, where the concrete derived type is specified by a key of
/// type \c KEY. Each concrete type needs to be associated with a key bey
/// calling the \c register function.

template <class KEY, class BASE>
class factory {
 public:
  /// the type of the base class from which all objects created by the factory
  /// are derived
  ...
  ///
  /// attempts to create an object of the type previously associated with the
  /// key. \param k the key referring to the object type \returns a pointer to a
  /// new object of the type registered with the key \throws \c
  /// std::runtime_error if no type was associated with the key
  base_type* operator()(key_type k) const {
    typename map_type::const_iterator it = creators_.find(k);
    if (it == creators_.end())
      boost::throw_exception(
          std::runtime_error("Type not registered in alps::factory::create")
      );
    return it->second();
  }
