...
/* $Id: vectorio.hpp 3958 2010-03-05 09:24:06Z troyer $ */
...
/// \brief reads a vector from a std::istream, until the end of the stream is
/// reached. \param in the stream \param v the vector to be read
...
inline void read_vector_resize(std::istream& in, CONTAINER& v) {
  ...
  std::copy(
      std::istream_iterator<value_type>(in),
      std::istream_iterator<value_type>(), std::back_inserter(tmp)
  );
  v.resize(tmp.size());
  for (std::size_t i = 0; i < size(v); ++i) v[i] = tmp[i];
}
...
/// \brief reads a vector from a std::string, until the end of the string is
/// reached. \param s the string \param v the vector to be read
template <class CONTAINER>
...
/// \brief reads a vector from a std::string, until the end of the string is
/// reached. \param s the string \param v the vector to be read the number of
/// elements to be read is taken from the size of the vector.
template <class CONTAINER>
...
/// \brief reads a vector from a std::string, until the end of the string is
/// reached. \param s the string \return the vector read from the string
template <class CONTAINER>
...
/// \brief reads a vector from a std::istream, until the end of the stream is
/// reached. \param in the stream \return the vector read from the stream
template <class CONTAINER>
...
template <class CONTAINER>
inline std::string write_vector(
    const CONTAINER& v, const std::string& delim = " ", int prec = 20
) {
  std::ostringstream str;
  str << std::setprecision(prec);
  for (std::size_t i = 0; i < alps::size(v); ++i) {
    str << v[i];
    if (i != alps::size(v) - 1) str << delim;
  }
  return str.str();
}
