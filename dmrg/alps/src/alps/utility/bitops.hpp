...
/* $Id: bitops.hpp 6826 2013-04-16 12:00:43Z hehn $ */
...
inline long gcc_popcount(unsigned long long x) {
...
#if __GNUC__ > 3 || (__GNUC__ == 3 && __GNUC_MINOR__ >= 4)
inline long popcnt(uint32_t x) { return detail::gcc_popcount(x); }
#else
...
/// @return the number of 1-bits in the word
#if __GNUC__ > 3 || (__GNUC__ == 3 && __GNUC_MINOR__ >= 4)
inline long popcnt(uint64_t x) { return detail::gcc_popcount(x); }
#else
inline long popcnt(uint64_t x) {
  return popcnt(static_cast<uint32_t>(x)) +
         popcnt(static_cast<uint32_t>(x >> 32));
}
#endif
...
/// \return the integer i with the bit at position p set to to the specified
/// value
template <class T, class U, class V>
inline T ibset(T i, U p, V b) {
  return (i & (~(1 << p))) | ((b & 1) << p);
}
