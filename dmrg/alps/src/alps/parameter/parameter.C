...
/* $Id: parameter.C 2288 2006-10-06 09:44:24Z wistaria $ */
...
void Parameter::parse(std::string const& str, bool replace_env) {
  if (!bs::parse(
           str.c_str(), ParameterParser(*this) >> !bs::ch_p(';') >> !bs::end_p,
           bs::blank_p
      )
           .full)
...
void Parameter::replace_envvar() {
  ...
  if (!bs::parse(
           ...
           *(bs::anychar_p - '$')[insert] %
                   (bs::ch_p('$') >>
                    bs::confix_p('{', (*bs::anychar_p)[replace], '}')) >>
               bs::end_p
      )
           .full)
...
void ParameterXMLHandler::start_element(
    const std::string& name, const XMLAttributes& attributes, xml::tag_type type
) {
  if (type == xml::element) {
    if (name != "PARAMETER")
      boost::throw_exception(
          std::runtime_error("ParameterXMLHandler: unknown tag name : " + name)
      );
