...
/* $Id: parser.h 3616 2009-12-29 08:06:46Z troyer $ */
...
struct XMLTag {
  ...
  /*/// \brief the type of tag
    ///
    /// The meaning of the values, and what is stored in the \c name member can
    ...
    /// <TR><TD><TT>COMMENT</TT></TD><TD><TT>!</TT></TD><TD><TT>&lt;!-- comment
    --!/&gt;</TT></TD></TR>
    /// <TR><TD><TT>PROCESSING</TT></TD><TD><TT>?</TT></TD><TD><TT>&lt;?
    processing instruction ?/&gt;</TT></TD></TR>
    /// </TABLE>
    /// \endhtmlonly
    /// \latexonly
...
/// and additionally any arbitrary sequence of characters (including whitespace)
/// surrounded by \c [ ... \ ] characters, such as in \c MEASURE[Staggered \c
/// Magnetization^2] .
ALPS_DECL std::string parse_parameter_name(std::istream& in);

/// \brief reads until the next occurence of the character \a end or until the
/// end of the stream is reached.
///
/// \param in the stream to be read
/// \param end the character until which should be read
/// \return  string containing the characters read, excluding leading and
/// trailing whitespace and excluding the terminating character \a end.
...
///  reads the next character (slipping white space) and checks if it is the
///  same as the character passed as argument \a c and throws a \c
///  std::runtime_error otherwise.
ALPS_DECL void check_character(
    std::istream& in, char c, const std::string& err
);

/// \brief parses an XML tag
/// \param in the stream to be read
/// \param skip_comments if true, the function skips any comments or processing
/// instructions while parsing \return an \c XMLTag structure containing
...
/// the function reads until it finds the closing tag correesponding to the \a
...
/// \throw \c std::runtime_error if the next tag read from the stream \a in does
/// not have the name given by the argument \a name.
