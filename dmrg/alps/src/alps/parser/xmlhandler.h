...
/* $Id: xmlhandler.h 3544 2009-12-17 09:59:36Z wistaria $ */
...
class ALPS_DECL DummyXMLHandler : public XMLHandlerBase {
 public:
  DummyXMLHandler(const std::string& basename) : XMLHandlerBase(basename) {}
  void start_element(
      const std::string& /* name */, const XMLAttributes& /* attributes */,
      xml::tag_type /* type */
  ) {}
 ...
 public:
  ...
  virtual void end_element(const std::string& name, xml::tag_type type) {
    if (type == xml::element) {
      ...
      if (!started_)
        boost::throw_exception(std::runtime_error(
            "SimpleXMLHandler::end_element: unbalanced end tag </" +
            basename() + ">"
        ));
      if (attr_.empty()) {
...
class ALPS_DECL CompositeXMLHandler : public XMLHandlerBase {
 ...
 protected:
  virtual void start_top(
      const std::string& /* name */, const XMLAttributes& /* attributes */,
      xml::tag_type /* type */
  ) {}
  virtual void
  end_top(const std::string& /* name */, xml::tag_type /* type */) {}
  virtual void start_child(
      const std::string& /* name */, const XMLAttributes& /* attributes */,
      xml::tag_type /* type */
  ) {}
  virtual void
  end_child(const std::string& /* name */, xml::tag_type /* type */) {}

  virtual bool start_element_impl(
      const std::string& /* name */, const XMLAttributes& /* attributes */,
      xml::tag_type /* type */
  ) {
    return false;
  ...
  virtual bool
  end_element_impl(const std::string& /* name */, xml::tag_type /* type */) {
    return false;
  }
...
class ALPS_DECL PrintXMLHandler : public XMLHandlerBase {
 public:
  ...
  void start_element(
    ...
    switch (type) {
      case xml::element:
        oxs_ << start_tag(name) << attributes;
        break;
      case xml::processing_instruction:
        oxs_ << processing_instruction(name) << attributes;
      case xml::stylesheet:
...
class ALPS_DECL StylesheetXMLHandler : public XMLHandlerBase {
 public:
  ...
  void start_element(
      const std::string&, const XMLAttributes& attributes, xml::tag_type type
  ) {
