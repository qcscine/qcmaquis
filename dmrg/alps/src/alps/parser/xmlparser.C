...
/* $Id: xmlparser.C 5688 2011-08-08 04:40:39Z troyer $ */
...
class XMLHandlerAdaptor : public XERCES_CPP_NAMESPACE_QUALIFIER HandlerBase {
 public:
  ...
  void characters(const XMLCh* const chars, const unsigned int /* length */) {
    std::string t = XERCES_CPP_NAMESPACE_QUALIFIER XMLString::transcode(chars);
    std::size_t pi = 0;
  ...
  void processingInstruction(
    ...
    XMLAttributes attr(XERCES_CPP_NAMESPACE_QUALIFIER XMLString::transcode(data)
    );
...
void XMLParser::parse(const std::string& file) { parser_->parse(file.c_str()); }
  ...
  do {
    ...
    if (!XML_Parse(p, buf, fin.gcount(), done)) {
      boost::throw_exception(std::runtime_error(
          "XMLParser: " + std::string(XML_ErrorString(XML_GetErrorCode(p)))
      ));
    }
  } while (!done);
  XML_ParserFree(p);
  ...
  while (in) {
    ...
    if (c == '<') {
      ...
      if (tag.type == XMLTag::CLOSING || tag.type == XMLTag::SINGLE) {
        // end tag
        if (tag.type == XMLTag::CLOSING) tag.name.erase(0, 1);
      ...
      if (tag.type == XMLTag::PROCESSING) {
        ...
        if (tag.name == "xml-stylesheet") {
          ...
          handler_.start_element(
              tag.name, tag.attributes, xml::processing_instruction
          );
