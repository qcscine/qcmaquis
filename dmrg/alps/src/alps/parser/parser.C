...
/* $Id: parser.C 6127 2012-05-03 06:38:38Z troyer $ */
...
std::string parse_parameter_name(std::istream& in) {
  ...
  std::string name;
  while (in && !in.eof() &&
         (detail::is_identifier_char(c) || c == '\'' || c == '[')) {
    name += c;
    if (c == '[') do {
        c = in.get();
        name += c;
...
std::string read_until(std::istream& in, char end) {
  ...
  if (c != end)
    boost::throw_exception(std::runtime_error(
        std::string("read past end of stream while scanning for ") + end
    ));
...
ALPS_DECL void check_character(
    std::istream& in, char test, const std::string& error
) {
  char c;
  in >> c;
  if (c != test) boost::throw_exception(std::runtime_error(error));
}
...
void skip_element(std::istream& in, const XMLTag& start) {
  if (start.type != XMLTag::OPENING) return;
  while (true) {
    parse_content(in);
...
void check_tag(std::istream& in, const std::string& name) {
  ...
  if (tag.name != name)
    boost::throw_exception(std::runtime_error(
        "Encountered tag <" + tag.name + "> instead of <" + name + ">"
    ));
}
