...
/* $Id: archivedump.h 5976 2012-01-28 00:59:57Z wistaria $ */
...
#define ALPS_DUMP_DO_TYPE(T) \
  void write_simple(T x) { archive_ << x; }
...
#undef ALPS_DUMP_DO_TYPE
  ...
  void write_string(std::size_t, const char* x) {
    write_string(std::string(x));
  }
...
/// INTERNAL ONLY
#define ALPS_DUMP_DO_TYPE(T) \
  void read_simple(T& x) { archive_ >> x; }
  ALPS_DUMP_DO_TYPE(bool)
  ALPS_DUMP_DO_TYPE(char)
  ALPS_DUMP_DO_TYPE(signed char)
  ALPS_DUMP_DO_TYPE(unsigned char)
...
#undef ALPS_DUMP_DO_TYPE
  ...
  void read_string(std::size_t, char* s) {
    std::string y;
    archive_ >> y;
    std::strcpy(s, y.c_str());
  }
