...
bool_t xdr_string(XDR *xdrs, char **cpp, u_int maxsize) {
  ...
  switch (xdrs->x_op) {
    case XDR_DECODE:
      if (sp == NULL) *cpp = sp = (char *)malloc(nodesize);
      if (sp == NULL) {
...
static bool_t xdrstdio_getlong(XDR *xdrs, long *lp) {
  ...
  if (fread((caddr_t)&mycopy, 4, 1, (FILE *)xdrs->x_private) != 1) return FALSE;
...
static bool_t xdrstdio_putint32(XDR *xdrs, const int32_t *ip) {
  ...
  ip = &mycopy;
  if (fwrite((caddr_t)ip, 4, 1, (FILE *)xdrs->x_private) != 1) return FALSE;
  return TRUE;
}
