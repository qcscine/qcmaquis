...
/* $Id: mpdump.C 5655 2011-07-22 21:54:17Z troyer $ */
...
OMPDump::OMPDump() : valid_(false) { init(); }
...
void OMPDump::send(const Process& where, int32_t t) {
  ...
  if (!valid_)
    boost::throw_exception(
        std::logic_error("message not initialized in OMPDump::send")
    );
#endif
  int info;
  if ((info = MPI_Send(buf_, buf_.size(), MPI_BYTE, where, t, MPI_COMM_WORLD)
      ) != 0)
    boost::throw_exception(std::runtime_error(
        ("Error " + boost::lexical_cast<std::string, int>(info) +
...
void OMPDump::send(const Process&, int32_t) {
  boost::throw_exception(
      std::logic_error("message passing useless in single CPU programs")
  );
}
#endif
...
void OMPDump::send(const ProcessList& where, int32_t t) {
  ...
  if (!valid_)
    boost::throw_exception(
        std::runtime_error("message not initialized in osiris::OMPDump::send")
    );
#endif
  ...
  for (std::size_t i = 0; i < where.size(); ++i)
    if (!where[i].local()) send(where[i], t);
}
...
#define ALPS_DUMP_DO_TYPE(T)                                                 \
  void OMPDump::write_simple(T) {                                            \
    boost::throw_exception(std::logic_error(                                 \
        "message passing useless for single process programs"                \
    ));                                                                      \
  }                                                                          \
  void IMPDump::read_array(std::size_t n, T* p) { IDump::read_array(n, p); } \
...
#ifdef ALPS_HAVE_MPI
void OMPDump::write_string(std::size_t n, const char* x) { buf_.write(x, n); }
#else
...
IMPDump::IMPDump() : valid_(false) { init(); }
...
const Process& IMPDump::sender() const { return theSender_; }
...
void IMPDump::receive(const Process* where, int32_t t) {
  ...
  if ((info = MPI_Recv(
           buf_, buf_.size(), MPI_BYTE, status.MPI_SOURCE, status.MPI_TAG,
...
#ifdef ALPS_TRACE
  std::cerr << "Received message " << status.MPI_TAG << " from process "
            << status.MPI_SOURCE << ".\n";
#endif
}
...
void IMPDump::receive(const Process*, int32_t) {
  boost::throw_exception(
      std::logic_error("message passing useless for single process programs")
  );
}

#endif

void IMPDump::receive(const Process& where, int32_t t) { receive(&where, t); }

void IMPDump::receive(int32_t t) { receive(0, t); }
...
void IMPDump::broadcast(const alps::Process& sender) {
  ...
  buf_.resize(cnt);
  // receive the message, aborts if an error occurs
  if ((info = MPI_Bcast(buf_, buf_.size(), MPI_BYTE, sender, MPI_COMM_WORLD)) !=
      0)
    boost::throw_exception(std::runtime_error(
        ("Error " + boost::lexical_cast<std::string, int>(info) +
         " when receiving message")
...
void IMPDump::read_string(std::size_t n, char* x) { buf_.read(x, n); }

#else

void IMPDump::read_string(std::size_t n, char* x) { IDump::read_string(n, x); }
...
// check for a message
int32_t IMPDump::probe(const Process& w, int32_t t) { return probe(&w, t); }

int32_t IMPDump::probe(int32_t t) { return probe(0, t); }
...
int32_t IMPDump::probe(const Process* w, int32_t t) {
  ...
  if (flag) {
#ifdef ALPS_TRACE
    std::cerr << "Found message " << status.MPI_TAG << " from "
              << status.MPI_SOURCE << ".\n";
#endif
