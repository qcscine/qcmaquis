...
/* $Id: xdrdump.C 7391 2014-03-17 14:12:01Z gukel $ */
...
#define ALPS_DUMP_DO_TYPE(T)                                                 \
  void OXDRDump::write_simple(T x) {                                         \
    if (!detail::xdr_helper<T, int(sizeof(T))>::xdr_do_type(                 \
            &xdr_, const_cast<T *>(&x)                                       \
        ))                                                                   \
      boost::throw_exception(std::runtime_error("failed to write type " #T   \
                                                " to an OXDRDump"));         \
  }                                                                          \
  void OXDRDump::write_array(size_t n, const T *p) {                         \
    int l = n;                                                               \
    if (!xdr_vector(                                                         \
            &xdr_, reinterpret_cast<char *>(const_cast<T *>(p)), l,          \
            int(sizeof(T)),                                                  \
            (xdrproc_t) & detail::xdr_helper<T, int(sizeof(T))>::xdr_do_type \
        ))                                                                   \
      boost::throw_exception(std::runtime_error(                             \
          "failed to write array of type " #T " to an OXDRDump"              \
      ));                                                                    \
  }                                                                          \
  void IXDRDump::read_simple(T &x) {                                         \
    if (!detail::xdr_helper<T, int(sizeof(T))>::xdr_do_type(&xdr_, &x))      \
      boost::throw_exception(std::runtime_error("failed to read type " #T    \
                                                " from an IXDRDump"));       \
  }                                                                          \
  void IXDRDump::read_array(size_t n, T *p) {                                \
    int l = n;                                                               \
    if (!xdr_vector(                                                         \
            &xdr_, reinterpret_cast<char *>(p), l, int(sizeof(T)),           \
            (xdrproc_t) & detail::xdr_helper<T, int(sizeof(T))>::xdr_do_type \
        ))                                                                   \
      boost::throw_exception(std::runtime_error(                             \
          "failed to read array of type " #T " from an IXDRDump"             \
      ));                                                                    \
  }
...
void OXDRDump::write_string(size_t n, const char *p) {
  ...
  if (!xdr_string(&xdr_, &ncp, l))
    boost::throw_exception(
        std::runtime_error("failed to write a string to an OXDRDump")
    );
}
...
void IXDRDump::read_string(size_t n, char *p) {
  ...
  if (!xdr_string(&xdr_, &p, l))
    boost::throw_exception(
        std::runtime_error("failed to read a string from an IXDRDump")
    );
}
...
uint32_t IXDRDump::getPosition() const { return xdr_getpos((XDR *)&xdr_); }
...
void OXDRFileDump::flush() { std::fflush(file_); }
