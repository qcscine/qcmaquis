...
struct is_datatype_caller {
  static bool apply(A const &ar, std::string path) {
    throw std::logic_error(
        "only native datatypes can be probed: " + path + ALPS_STACKTRACE
    );
    return false;
  }
};
...
#define ALPS_NGS_HDF5_IS_DATATYPE_CALLER(T)              \
  template <typename A>                                  \
  struct is_datatype_caller<A, T> {                      \
    static bool apply(                                   \
        A const &ar, std::string path,                   \
        T unused = alps::detail::type_wrapper<T>::type() \
    ) {                                                  \
...
class ALPS_DECL archive {
 public:
  ...
  /* TODO: implement
                  void move_data(std::string current_path, std::string new_path)
     const; void move_attribute(std::string current_path, std::string new_path)
     const;
  */
  ...
  template <typename T>
  void read(
      std::string path, T *, std::vector<std::size_t>,
...
#endif
  static std::map<
      std::string, std::pair<detail::archivecontext *, std::size_t> >
      ref_cnt_;
};

template <typename T>
struct is_continuous : public boost::false_type {};

template <typename T>
struct is_content_continuous : public is_continuous<T> {};

template <typename T>
struct has_complex_elements : public boost::false_type {};
...
struct is_vectorizable {
  static bool apply(T const &value) { return false; }
};

template <typename T>
struct get_pointer {
  static typename alps::hdf5::scalar_type<T>::type *apply(T &) { return NULL; }
};

template <typename T>
struct get_pointer<T const> {
...
template <typename T>
typename scalar_type<T>::type *get_pointer(T &value) {
  return detail::get_pointer<T>::apply(value);
}

template <typename T>
typename scalar_type<T>::type const *get_pointer(T const &value) {
...
void save(
  ...
  if (chunk.size())
    throw std::logic_error(
        "user defined objects needs to be written continously" + ALPS_STACKTRACE
    );
...
#define ALPS_NGS_HDF5_DEFINE_FREE_FUNCTIONS(T)                            \
  template <>                                                             \
  struct is_continuous<T> : public boost::true_type {};                   \
  template <>                                                             \
  struct is_continuous<T const> : public boost::true_type {};             \
                                                                          \
  namespace detail {                                                      \
  template <>                                                             \
  struct ALPS_DECL is_vectorizable<T> {                                   \
    static bool apply(T const &value);                                    \
  };                                                                      \
  template <>                                                             \
  struct ALPS_DECL is_vectorizable<T const> {                             \
    static bool apply(T &value);                                          \
  };                                                                      \
                                                                          \
  template <>                                                             \
  struct ALPS_DECL get_pointer<T> {                                       \
    static alps::hdf5::scalar_type<T>::type *apply(T &value);             \
  };                                                                      \
                                                                          \
  template <>                                                             \
  struct ALPS_DECL get_pointer<T const> {                                 \
...
typename boost::enable_if<
    ...
    archive &>::type
operator<<(archive &ar, detail::make_pvp_proxy<T> const &proxy) {
...
typename boost::disable_if<
    typename boost::mpl::and_<
        typename boost::is_same<
            ...
            char>::type,
        typename boost::is_array<T>::type>::type,
    hdf5::detail::make_pvp_proxy<T &> >::type
...
typename boost::disable_if<
    typename boost::mpl::and_<
        typename boost::is_same<
            ...
            char>::type,
        typename boost::is_array<T>::type>::type,
    hdf5::detail::make_pvp_proxy<T const &> >::type
make_pvp(std::string const &path, T const &value) {
...
typename boost::enable_if<
    typename boost::mpl::and_<
        typename boost::is_same<
            ...
            char>::type,
        typename boost::is_array<T>::type>::type,
    hdf5::detail::make_pvp_proxy<std::string const> >::type
make_pvp(std::string const &path, T const &value) {
...
template <typename A>
template <typename T>
archive_proxy<A> &archive_proxy<A>::operator=(T const &value) {
...
template <typename A>
template <typename T>
archive_proxy<A> &archive_proxy<A>::operator>>(T &value) {
  ar_ >> make_pvp(path_, value);
  return *this;
}
