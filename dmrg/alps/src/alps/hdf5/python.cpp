...
bool is_vectorizable_generic(T const &value) {
  static char const *scalar_types[] = {
      "int",
      "long",
      "float",
      "complex",
  ...
  if (size == 0)
    ...
    std::string first_dtype =
        boost::python::object(value[0]).ptr()->ob_type->tp_name;
    bool next_homogenious;
    std::vector<std::size_t> first_extent;
    if (first_dtype == "list") {
      if (!is_vectorizable<boost::python::list>::apply(
              boost::python::extract<boost::python::list>(value[0])()
          ))
        return false;
      first_extent =
          get_extent(boost::python::extract<boost::python::list>(value[0])());
    } else if (first_dtype == "tuple") {
      if (!is_vectorizable<boost::python::tuple>::apply(
              boost::python::extract<boost::python::tuple>(value[0])()
          ))
        return false;
      first_extent =
    ...
    for (boost::python::ssize_t i = 0; i < size; ++i) {
      ...
      if (dtype == "list") {
        if (!is_vectorizable<boost::python::list>::apply(
                boost::python::extract<boost::python::list>(value[i])()
            ))
          return false;
        std::vector<std::size_t> extent =
            get_extent(boost::python::extract<boost::python::list>(value[i])());
        if (first_extent.size() != extent.size() ||
            !std::equal(
        ...
        if (!is_vectorizable<boost::python::tuple>::apply(
                boost::python::extract<boost::python::tuple>(value[i])()
            ))
          return false;
        std::vector<std::size_t> extent =
            get_extent(boost::python::extract<boost::python::tuple>(value[i])()
            );
        if (first_extent.size() != extent.size() ||
            !std::equal(
                first_extent.begin(), first_extent.end(), extent.begin()
        ...
        std::vector<std::size_t> extent = get_extent(
            boost::python::extract<alps::python::numpy::array>(value[i])()
        );
        if (first_extent.size() != extent.size() ||
            !std::equal(
                first_extent.begin(), first_extent.end(), extent.begin()
...
bool is_vectorizable<boost::python::list>::apply(
    boost::python::list const &value
) {
  return is_vectorizable_generic<boost::python::list>(value);
}
bool is_vectorizable<boost::python::tuple>::apply(
...
std::vector<std::size_t> get_extent<boost::python::list>::apply(
    boost::python::list const &value
) {
  return get_extent_generic<boost::python::list>(value);
}
std::vector<std::size_t> get_extent<boost::python::tuple>::apply(
    boost::python::tuple const &value
) {
  return get_extent_generic<boost::python::tuple>(value);
}

void set_extent<boost::python::list>::apply(
    boost::python::list &value, std::vector<std::size_t> const &extent
) {}
}  // namespace detail
...
void save_generic(
  ...
  if (len(value) == 0)
    ...
    for (boost::python::ssize_t i = 0; i < len(value); ++i)
      save(
          ar, path + "/" + cast<std::string>(i), boost::python::object(value[i])
      );
  }
}

void save(
    archive &ar, std::string const &path, boost::python::list const &value,
    std::vector<std::size_t> size, std::vector<std::size_t> chunk,
    std::vector<std::size_t> offset
) {
...
void save(
    archive &ar, std::string const &path, boost::python::tuple const &value,
    std::vector<std::size_t> size, std::vector<std::size_t> chunk,
    std::vector<std::size_t> offset
) {
...
void load(
    archive &ar, std::string const &path, boost::python::list &value,
  ...
  if (ar.is_group(path)) {
    ...
    if (list.size()) {
      ...
      for (std::vector<boost::python::object>::const_iterator it = data.begin();
           it != data.end(); ++it)
        value.append(*it);
    }
    ...
    if (ar.dimensions(path) != 1)
      throw archive_error(
          "More than 1 Dimension is not supported." + ALPS_STACKTRACE
      );
...
bool is_vectorizable<alps::python::numpy::array>::apply(
    alps::python::numpy::array const &value
) {
  return true;
}

std::vector<std::size_t> get_extent<alps::python::numpy::array>::apply(
    alps::python::numpy::array const &value
) {
  ...
  return std::vector<std::size_t>(
      PyArray_DIMS(ptr), PyArray_DIMS(ptr) + PyArray_NDIM(ptr)
  );
}

// To set the extent of a numpy array, we need the type, extent is set in load
void set_extent<alps::python::numpy::array>::apply(
    alps::python::numpy::array &value, std::vector<std::size_t> const &extent
) {}

template <typename T>
void load_python_numeric(
    archive &ar, std::string const &path, alps::python::numpy::array &value,
    std::vector<std::size_t> chunk, std::vector<std::size_t> offset, int type
) {
  std::vector<std::size_t> extent(ar.extent(path));
  if (ar.is_complex(path)) extent.pop_back();
  std::vector<npy_intp> npextent(extent.begin(), extent.end());
  std::size_t len = std::accumulate(
      extent.begin(), extent.end(), std::size_t(1),
      std::multiplies<std::size_t>()
  );
  value = alps::python::numpy::from_pyobject(
      boost::python::object(boost::python::handle<>(
          PyArray_SimpleNew(npextent.size(), &npextent.front(), type)
      ))
  );
  if (len) {
    ...
    PyArrayObject *ptr = (PyArrayObject *)value.ptr();
    memcpy(
        PyArray_DATA(ptr), raw.get(), PyArray_ITEMSIZE(ptr) * PyArray_SIZE(ptr)
    );
  }
}
}  // namespace detail
...
void save(
    archive &ar, std::string const &path,
    alps::python::numpy::array const &value, std::vector<std::size_t> size,
    std::vector<std::size_t> chunk, std::vector<std::size_t> offset
) {
  import_numpy();
  if (ar.is_group(path)) ar.delete_group(path);
  ...
  if (!PyArray_Check(ptr))
    ...
    throw std::runtime_error(
        "numpy array cannot be converted to continous array" + ALPS_STACKTRACE
    );
  std::vector<std::size_t> extent(
      PyArray_DIMS(ptr), PyArray_DIMS(ptr) + PyArray_NDIM(ptr)
  );
...
#undef NGS_PYTHON_HDF5_CHECK_NUMPY
  else throw std::runtime_error("unknown numpy element type" + ALPS_STACKTRACE);
...
#undef NGS_PYTHON_HDF5_LOAD_NUMPY
  else throw std::runtime_error("Unsupported type." + ALPS_STACKTRACE);
}

void save(
    archive &ar, std::string const &path, boost::python::dict const &value,
    std::vector<std::size_t> size, std::vector<std::size_t> chunk,
    std::vector<std::size_t> offset
) {
  if (ar.is_group(path)) ar.delete_group(path);
  ...
  for (boost::python::ssize_t i = 0; i < len(keys); ++i) {
    ...
    std::string k =
        boost::python::call_method<std::string>(pyk.ptr(), "__str__");
    save(
        ar, ar.complete_path(path) + "/" + ar.encode_segment(k), value.get(pyk)
    );
  }
}

void load(
    archive &ar, std::string const &path, boost::python::dict &value,
    std::vector<std::size_t> chunk, std::vector<std::size_t> offset
) {
...
bool is_vectorizable<boost::python::object>::apply(
  ...
  if (dtype == "list")
    return is_vectorizable<boost::python::list>::apply(
        boost::python::extract<boost::python::list>(value)()
    );
  else if (dtype == "numpy.ndarray")
    return is_vectorizable<alps::python::numpy::array>::apply(
        boost::python::extract<alps::python::numpy::array>(value)()
    );
  return find(scalar_types, scalar_types + 19, dtype) < scalar_types + 19;
}

std::vector<std::size_t> get_extent<boost::python::object>::apply(
    boost::python::object const &value
) {
  using alps::hdf5::get_extent;
  std::string dtype = value.ptr()->ob_type->tp_name;
...
void set_extent<boost::python::object>::apply(
    boost::python::object &value, std::vector<std::size_t> const &extent
) {}

struct save_python_object_visitor {
  save_python_object_visitor(
      archive &ar, std::string const &path, std::vector<std::size_t> size,
      std::vector<std::size_t> chunk, std::vector<std::size_t> offset
  )
      : _ar(ar), _path(path), _size(size), _chunk(chunk), _offset(offset) {}
  template <typename T>
  void operator()(T const &value) {
    save(_ar, _path, value, _size, _chunk, _offset);
    if (has_complex_elements<T>::value) _ar.set_complex(_path);
  }
  template <typename T>
  void operator()(T const *, std::vector<std::size_t>) {
...
template <typename T>
void load_python_object(
    archive &ar, std::string const &path, boost::python::object &value,
    std::vector<std::size_t> chunk, std::vector<std::size_t> offset, int type
) {
  T data;
  load(ar, path, data, chunk, offset);
...
void save(
    archive &ar, std::string const &path, boost::python::object const &value,
    std::vector<std::size_t> size, std::vector<std::size_t> chunk,
    std::vector<std::size_t> offset
) {
  std::string dtype = value.ptr()->ob_type->tp_name;
  if (dtype == "numpy.ndarray")
    ...
    boost::python::call_method<void>(
        value.ptr(), "save", boost::python::object(ar)
    );
...
void load(
  ...
  if (PyObject_HasAttrString(value.ptr(), "load") &&
      std::string(PyObject_GetAttrString(value.ptr(), "load")->ob_type->tp_name
      ) == "MethodType") {
    std::string context = ar.get_context();
    ar.set_context(ar.complete_path(path));
    boost::python::call_method<void>(
        value.ptr(), "load", boost::python::object(ar), path
    );
    ar.set_context(context);
    ...
    for (std::vector<std::string>::const_iterator it = list.begin();
         is_list && it != list.end(); ++it) {
      for (std::string::const_iterator jt = it->begin();
           is_list && jt != it->end(); ++jt)
...
#undef NGS_PYTHON_HDF5_LOAD_SCALAR_NUMPY
    ...
    alps::python::numpy::array array =
        alps::python::numpy::from_pyobject(boost::python::object());
