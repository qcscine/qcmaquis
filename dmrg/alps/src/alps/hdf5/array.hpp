...
struct scalar_type<std::array<T, N> > {
  typedef
      typename scalar_type<typename std::array<T, N>::value_type>::type type;
};

template <typename T, std::size_t N>
struct is_continuous<std::array<T, N> > : public is_continuous<T> {};
template <typename T, std::size_t N>
struct is_continuous<std::array<T, N> const> : public is_continuous<T> {};

template <typename T, std::size_t N>
struct has_complex_elements<std::array<T, N> >
    : public has_complex_elements<typename alps::detail::remove_cvr<
          typename std::array<T, N>::value_type>::type> {};

namespace detail {

template <typename T, std::size_t N>
struct get_extent<std::array<T, N> > {
...
template <typename T, std::size_t N>
struct set_extent<std::array<T, N> > {
  static void apply(
      std::array<T, N> &value, std::vector<std::size_t> const &extent
  ) {
    using alps::hdf5::set_extent;
    if (extent.size() > 1)
      for (typename std::array<T, N>::iterator it = value.begin();
           it != value.end(); ++it)
        set_extent(
...
template <typename T, std::size_t N>
struct is_vectorizable<std::array<T, N> > {
  static bool apply(std::array<T, N> const &value) {
    ...
    if (!is_continuous<std::array<T, N> >::value) {
      ...
      std::vector<std::size_t> first(get_extent(value[0]));
      for (typename std::array<T, N>::const_iterator it = value.begin();
           it != value.end(); ++it)
...
template <typename T, std::size_t N>
struct get_pointer<std::array<T, N> > {
  static typename alps::hdf5::scalar_type<std::array<T, N> >::type *apply(
      std::array<T, N> &value
  ) {
...
template <typename T, std::size_t N>
struct get_pointer<std::array<T, N> const> {
  static typename alps::hdf5::scalar_type<std::array<T, N> >::type const *apply(
...
template <typename T, std::size_t N>
void save(
    archive &ar, std::string const &path, std::array<T, N> const &value,
...
template <typename T, std::size_t N>
void load(
    archive &ar, std::string const &path, std::array<T, N> &value,
    std::vector<std::size_t> chunk = std::vector<std::size_t>(),
    std::vector<std::size_t> offset = std::vector<std::size_t>()
) {
  using alps::cast;
  if (ar.is_group(path)) {
    ...
    if (size.size() > 0 && N != *(size.begin() + chunk.size()) &&
        (is_continuous<T>::value || *(size.begin() + chunk.size()) > 0))
      throw archive_error("dimensions do not match" + ALPS_STACKTRACE);
