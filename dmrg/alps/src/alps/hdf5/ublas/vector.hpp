...
struct scalar_type<boost::numeric::ublas::vector<T, A> > {
  typedef typename scalar_type<
      typename boost::numeric::ublas::vector<T, A>::value_type>::type type;
};

template <typename T, typename A>
struct has_complex_elements<boost::numeric::ublas::vector<T, A> >
    : public has_complex_elements<typename alps::detail::remove_cvr<
          typename boost::numeric::ublas::vector<T, A>::value_type>::type> {};

namespace detail {

template <typename T, typename A>
struct get_extent<boost::numeric::ublas::vector<T, A> > {
  static std::vector<std::size_t> apply(
    ...
    if (value.size()) {
      ...
      if (!boost::is_scalar<
              typename boost::numeric::ublas::vector<T, A>::value_type>::value)
        for (typename boost::numeric::ublas::vector<T, A>::const_iterator it =
                 value.begin() + 1;
             it != value.end(); ++it) {
...
template <typename T, typename A>
struct set_extent<boost::numeric::ublas::vector<T, A> > {
  static void apply(
    ...
    if (extent.size() > 1)
      for (typename boost::numeric::ublas::vector<T, A>::iterator it =
               value.begin();
           it != value.end(); ++it)
        set_extent(
...
template <typename T, typename A>
struct is_vectorizable<boost::numeric::ublas::vector<T, A> > {
...
template <typename T, typename A>
struct get_pointer<boost::numeric::ublas::vector<T, A> > {
  static typename alps::hdf5::scalar_type<
      boost::numeric::ublas::vector<T, A> >::type *
  apply(boost::numeric::ublas::vector<T, A> &value) {
...
template <typename T, typename A>
struct get_pointer<boost::numeric::ublas::vector<T, A> const> {
  static typename alps::hdf5::scalar_type<
      boost::numeric::ublas::vector<T, A> >::type const *
  apply(boost::numeric::ublas::vector<T, A> const &value) {
    using alps::hdf5::get_pointer;
    return get_pointer(value[0]);
  }
};
  ...
  if (is_continuous<T>::value && value.size() == 0)
    ar.write(
        ...
        static_cast<typename scalar_type<
            boost::numeric::ublas::vector<T, A> >::type const *>(NULL),
...
void load(
  ...
  if (ar.is_group(path)) {
    ...
    if (is_continuous<T>::value) {
      ...
      set_extent(
          value, std::vector<std::size_t>(1, *(size.begin() + chunk.size()))
      );
      chunk.push_back(1);
      offset.push_back(0);
      for (typename boost::numeric::ublas::vector<T, A>::iterator it =
