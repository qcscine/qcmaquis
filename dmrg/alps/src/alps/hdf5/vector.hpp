...
struct scalar_type<std::vector<T, A> > {
  typedef
      typename scalar_type<typename std::vector<T, A>::value_type>::type type;
};

template <typename T, typename A>
struct is_content_continuous<std::vector<T, A> > : public is_continuous<T> {};
template <typename A>
struct is_content_continuous<std::vector<bool, A> > : public boost::false_type {
};

template <typename T, typename A>
struct has_complex_elements<std::vector<T, A> >
    : public has_complex_elements<typename alps::detail::remove_cvr<
          typename std::vector<T, A>::value_type>::type> {};
  ...
  static std::vector<std::size_t> apply(std::vector<T, A> const &value) {
    ...
    if (value.size()) {
      ...
      if (!boost::is_scalar<typename std::vector<T, A>::value_type>::value)
        for (typename std::vector<T, A>::const_iterator it = value.begin() + 1;
             it != value.end(); ++it) {
          std::vector<std::size_t> size(get_extent(*it));
          if (first.size() != size.size() ||
...
struct set_extent<std::vector<T, A> > {
  static void apply(
      std::vector<T, A> &value, std::vector<std::size_t> const &extent
  ) {
    using alps::hdf5::set_extent;
    value.resize(extent[0]);
    if (extent.size() > 1)
      for (typename std::vector<T, A>::iterator it = value.begin();
           it != value.end(); ++it)
        set_extent(
...
struct set_extent<std::vector<bool, A> > {
  static void apply(
      std::vector<bool, A> &value, std::vector<std::size_t> const &extent
  ) {
    if (extent.size() != 1)
      throw archive_error("dimensions do not match" + ALPS_STACKTRACE);
    value.resize(extent[0]);
...
template <typename A>
struct is_vectorizable<std::vector<bool, A> > {
  static bool apply(std::vector<bool, A> const &value) { return true; }
};

template <typename T, typename A>
struct get_pointer<std::vector<T, A> > {
  static typename alps::hdf5::scalar_type<std::vector<T, A> >::type *apply(
...
struct get_pointer<std::vector<T, A> const> {
  static typename alps::hdf5::scalar_type<std::vector<T, A> >::type const *
  apply(std::vector<T, A> const &value) {
    using alps::hdf5::get_pointer;
    return get_pointer(value[0]);
  }
};
...
struct get_pointer<std::vector<bool, A> > {
  static typename alps::hdf5::scalar_type<std::vector<bool, A> >::type *apply(
      std::vector<bool, A> &value
  ) {
    throw archive_error(
        "std::vector<bool, A>[0] cannot be dereferenced" + ALPS_STACKTRACE
    );
    return NULL;
  }
};
...
struct get_pointer<std::vector<bool, A> const> {
  static typename alps::hdf5::scalar_type<std::vector<bool, A> >::type const *
  apply(std::vector<bool, A> const &value) {
    throw archive_error(
        "std::vector<bool>[0] cannot be dereferenced" + ALPS_STACKTRACE
    );
    return NULL;
  }
};
...
template <typename T, typename A>
void save(
    archive &ar, std::string const &path, std::vector<T, A> const &value,
  ...
  if (is_continuous<T>::value && value.size() == 0)
    ...
    for (typename std::vector<T, A>::const_iterator it = value.begin();
      ...
      save(
          ar,
          ar.complete_path(path) + "/" + cast<std::string>(it - value.begin()),
          *it
      );
  }
}
...
template <typename A>
void save(
    archive &ar, std::string const &path, std::vector<bool, A> const &value,
    std::vector<std::size_t> size = std::vector<std::size_t>(),
    std::vector<std::size_t> chunk = std::vector<std::size_t>(),
    std::vector<std::size_t> offset = std::vector<std::size_t>()
) {
  ...
  if (value.size() == 0)
    ...
    chunk.push_back(1);
    offset.push_back(0);
    for (typename std::vector<bool, A>::const_iterator it = value.begin();
         it != value.end(); ++it) {
      offset.back() = it - value.begin();
      bool const elem = *it;
...
template <typename T, typename A>
void load(
    archive &ar, std::string const &path, std::vector<T, A> &value,
    std::vector<std::size_t> chunk = std::vector<std::size_t>(),
    std::vector<std::size_t> offset = std::vector<std::size_t>()
) {
  using alps::cast;
  if (ar.is_group(path)) {
    ...
    for (typename std::vector<std::string>::const_iterator it =
         ...
         it != children.end(); ++it)
      load(
          ar, ar.complete_path(path) + "/" + *it, value[cast<std::size_t>(*it)]
      );
  } else {
    ...
    if (size.size() == 0)
      ...
      set_extent(
          value,
          std::vector<std::size_t>(size.begin() + chunk.size(), size.end())
      );
      if (value.size()) {
        std::copy(
            size.begin() + chunk.size(), size.end(), std::back_inserter(chunk)
        );
...
template <typename A>
void load(
    archive &ar, std::string const &path, std::vector<bool, A> &value,
    std::vector<std::size_t> chunk = std::vector<std::size_t>(),
    std::vector<std::size_t> offset = std::vector<std::size_t>()
) {
  if (ar.is_group(path))
    ...
    if (size.size() == 0)
      ...
      chunk.push_back(1);
      offset.push_back(0);
      for (typename std::vector<bool, A>::iterator it = value.begin();
           it != value.end(); ++it) {
        offset.back() = it - value.begin();
        bool elem;
