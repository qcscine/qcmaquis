...
struct scalar_type<alps::numeric::matrix<T, MemoryBlock> > {
  typedef typename scalar_type<
      typename alps::numeric::matrix<T, MemoryBlock>::value_type>::type type;
};

template <typename T, typename MemoryBlock>
struct has_complex_elements<alps::numeric::matrix<T, MemoryBlock> >
    : public has_complex_elements<typename alps::detail::remove_cvr<
          typename alps::numeric::matrix<T, MemoryBlock>::value_type>::type> {};
...
struct get_extent<alps::numeric::matrix<T, MemoryBlock> > {
  static std::vector<std::size_t> apply(
      alps::numeric::matrix<T, MemoryBlock> const& m
  ) {
    using alps::hdf5::get_extent;
    using std::copy;
    using std::equal;
    typedef typename alps::numeric::matrix<
        T, MemoryBlock>::const_col_element_iterator col_iterator;
    ...
    if (!is_continuous<T>::value && (size.size() != 2))
      for (std::size_t j = 0; j < num_cols(m); ++j)
        for (std::pair<col_iterator, col_iterator> r = col(m, j);
             r.first != r.second; ++r.first)
          set_extent(
              *r.first, std::vector<std::size_t>(size.begin() + 2, size.end())
          );
...
struct is_vectorizable<alps::numeric::matrix<T, MemoryBlock> > {
  static bool apply(alps::numeric::matrix<T, MemoryBlock> const& m) {
    ...
    using std::equal;
    if (boost::is_scalar<typename alps::numeric::matrix<
            T, MemoryBlock>::value_type>::value ||
        m.empty())
      return true;
    else {
      ...
      for (std::size_t j = 0; j < num_cols(m); ++j) {
        for (std::pair<col_iterator, col_iterator> r = col(m, j);
          ...
          if (!is_vectorizable(*r.first)) {
            ...
            if (first_element_extent.size() != element_extent.size() ||
                !equal(
                    first_element_extent.begin(), first_element_extent.end(),
                    element_extent.begin()
                ))
              return false;
          }
        }
...
struct get_pointer<alps::numeric::matrix<T, MemoryBlock> > {
  ...
  apply(alps::numeric::matrix<T, MemoryBlock>& m) {
...
struct get_pointer<alps::numeric::matrix<T, MemoryBlock> const> {
  static typename alps::hdf5::scalar_type<
      alps::numeric::matrix<T, MemoryBlock> >::type const*
  apply(alps::numeric::matrix<T, MemoryBlock> const& m) {
    using alps::hdf5::get_pointer;
    return get_pointer(m(0, 0));
  }
};
...
void save(
  ...
  if (is_continuous<T>::value && m.empty())
    ar.write(
        path,
        static_cast<typename scalar_type<
            alps::numeric::matrix<T, MemoryBlock> >::type const*>(NULL),
    ...
    for (std::size_t j = 0; j < num_cols(m); ++j)
      for (std::size_t i = 0; i < num_rows(m); ++i)
        save(
            ar,
            ar.complete_path(path) + "/" + cast<std::string>(j) + "/" +
                cast<std::string>(i),
            m(i, j)
        );
  }
...
void load(
  ...
  if (ar.is_group(path)) {
    ...
    std::vector<std::string> const rows =
        ar.list_children(path + "/" + columns[0]);
    ...
    for (std::vector<std::string>::const_iterator it = columns.begin();
      ...
      std::vector<std::string> const elements =
          ar.list_children(path + "/" + *it);
      if (elements.size() != rows.size())
        throw invalid_path("invalid path" + ALPS_STACKTRACE);
      for (std::vector<std::string>::const_iterator eit = elements.begin();
           eit != elements.end(); ++eit) {
        load(
            ar, ar.complete_path(path) + "/" + *it + "/" + *eit,
            m2(cast<std::size_t>(*eit), cast<std::size_t>(*it))
        );
      }
    }
  } else {
    ...
    if (size.size() == 1 && size[0] == 0)
      ...
      // We need to make sure that reserve() will reserve exactly the num_rows()
      // we asked for. The only way to ensure that is by creating a new matrix
      // which has no reserved space.
      set_extent(
          m2, std::vector<std::size_t>(size.begin() + chunk.size(), size.end())
      );
