...
class basic_lanczos_iteration {
 public:
  basic_lanczos_iteration(unsigned int max_iter, T r = 0., T a = 0.)
      : error(0), i(0), max_iter_(max_iter), rtol_(r), atol_(a) {}
...
class lanczos_iteration_nlowest
 ...
 public:
  lanczos_iteration_nlowest(
      unsigned int max_iter, unsigned int n = 1,
...
class lanczos_nlowest_better
 ...
 public:
  lanczos_nlowest_better(
      unsigned int max_iter, unsigned int n,
      T r = 100. * std::numeric_limits<T>::epsilon(),
      T a = 100. * std::numeric_limits<T>::epsilon(),
...
class fixed_lanczos_iteration
 ...
 public:
  ...
  template <class Tmatrix>
  bool converged(const Tmatrix&) const {
    return false;
  }
};
...
class bandlanczos_iteration_nhighest {
 public:
  bandlanczos_iteration_nhighest(
      unsigned int max_iter, T def_tol, T dep_tol, T ghost_tol,
      bool ghost_discarding, unsigned int evs
  )
