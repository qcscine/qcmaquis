...
class Info {
 public:
  ...
  Info() {}
  Info(
      std::vector<int> M1, std::vector<int> M2, std::vector<int> Ma,
...
class lanczos : public Tmatrix<VS> {
 ...
 public:
  ...
  void eigenvectors(
      IN in_eigvals_start, IN in_eigvals_end, OUT eig_vectors,
...
template <class MATRIX, class VS>
template <class IN, class OUT, class GEN>
void lanczos<MATRIX, VS>::eigenvectors(
    IN in_eigvals_start, IN in_eigvals_end, OUT eig_vectors,
    Info<magnitude_type>& inf, GEN gen_, int maxiter, int maxcount
) {
    ...
    if (ma != 0) {
      ...
      do {
        ...
        if (nth == -1) {
          ...
          error = fabs(
              super_type::beta[ma - 1] * z(ma - 1, nth)
          );  // beta[ma - 1] = betaMplusOne.
  ...
  while (m2counter < (in_eigvals_end - in_eigvals_start) &&
    ...
    while (in_eigvals != in_eigvals_end) {
      if (*M1_itr == 0 || *M2_itr == 0) {
        ...
        ub = std::lower_bound(
            eval.begin(), eval.end(), *in_eigvals + super_type::thold
        );
        lb = std::upper_bound(
            eval.begin(), eval.end(), *in_eigvals - super_type::thold
        );
