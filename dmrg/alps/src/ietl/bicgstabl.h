...
// "Bicgstab(l) For Linear Equations Involving Unsymmetric Matrices With Complex
// Spectrum (1993)" p. 21
...
class ietl_bicgstabl {
 ...
 public:
  ietl_bicgstabl(unsigned int b, size_t maxiter = 100, bool verbose = false)
      : BICGSTAB_L(b), maxiter(maxiter), verbose(verbose) {}

  template <class VECTOR, class MATRIX>
  VECTOR operator()(
      const MATRIX& A, const VECTOR& b, const VECTOR& x0,
    ...
    std::vector<scalar_t> gamma0(BICGSTAB_L), gamma1(BICGSTAB_L),
        gamma2(BICGSTAB_L), sigma(BICGSTAB_L);
    ...
    for (size_t iter = 1; iter <= maxiter && normr > abs_tol;
         ++iter, k += BICGSTAB_L) {
      rho0 *= -omega;
      ...
      for (size_t j = 0; j < BICGSTAB_L; ++j) {
        ...
        for (size_t i = 0; i < j + 1; ++i) r[i] -= alpha * u[i + 1];
      ...
      if (verbose)
        std::cout << "BiCGstab(" << BICGSTAB_L << ") Iteration " << iter
                  << ",\t Residual = " << normr << "\n";
