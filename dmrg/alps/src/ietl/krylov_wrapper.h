...
namespace ietl {
template <
    class Matrix, class VectorX, class VectorB, class Preconditioner,
    class Iteration>
...
class cg_wrapper {
 ...
 public:
  ...
  void operator()(
      const Matrix& A, scalar_type s, VectorX& x, const VectorB& b
...
class bicg_wrapper {
 ...
 public:
  ...
  void operator()(
    ...
    if (cg_error_code != 0) {
      throw std::runtime_error(
          "ERROR in BICG_WRAPPER:    bicg can't solve system"
      );
    }
  }
};
...
namespace ietl {
template < class Matrix, class VectorX, class VectorB, class Preconditioner,
class Iteration, class Basis > class gmres_wrapper
{
 private:
  Preconditioner M_;
  Iteration iter_;
...
throw (std::runtime_error)
  {
    ...
    if (error_code != 0)
    {
      throw std::runtime_error("ERROR in GMRES_WRAPPER:    bicg can't solve
system");
    }
  }
};
...
class bicgstab_wrapper {
 ...
 public:
  ...
  void operator()(
    ...
    if (error_code != 0) {
      throw std::runtime_error(
          "ERROR in BICGSTAB_WRAPPER:    bicgstab can't solve system"
      );
    }
  }
};
...
namespace ietl {
template <
    class Matrix, class VectorX, class VectorB, class Precond1, class Precond2,
    class Iteration>
  ...
  void operator()(
    ...
    if (qmr_error_code != 0) {
      switch (qmr_error_code) {
        case 1:
          throw std::runtime_error(
              "ERROR in QMR_WRAPPER: no convergence after maximum iterations"
          );
        case 2:
  ...
  void operator()(
    ...
    if (error_code != 0) {
      switch (error_code) {
        case 1:
          throw std::runtime_error(
              "ERROR in TFQMR_WRAPPER: no convergence after maximum iterations"
          );
        case 2:
...
class cheby_wrapper {
 ...
 public:
  cheby_wrapper(
      ...
      typename VectorB::value_type eigmin, typename VectorB::value_type eigmax
  )
      : M_(M), iter_(iter), N_(0) {}
  ...
  void operator()(
    ...
    if (error_code != 0) {
      throw std::runtime_error(
          "ERROR in CHEBY_WRAPPER:    cheby can't solve system"
      );
    }
  }
};
...
class richardson_wrapper {
 ...
 public:
  ...
  void operator()(
    ...
    if (error_code != 0) {
      throw std::runtime_error(
          "ERROR in RICHARDSON_WRAPPER:    richardson can't solve system"
      );
    }
  }
};
