...
inline std::complex<double> make_complex(double re, double im) {
  return std::complex<double>(re, im);
}

template <class MatrixT>
void arnoldi_geev(
    MatrixT& mtx, std::vector<std::complex<double> >& evals, MatrixT& evecs,
    double
) {
  std::vector<double> ev1(mtx.size1()), ev2(mtx.size1());
  MatrixT null_matrix(mtx.size1(), mtx.size2());
  boost::numeric::bindings::lapack::geev(
      'N', 'V', mtx, ev1, ev2, null_matrix, evecs
  );
  std::transform(
      ev1.begin(), ev1.end(), ev2.begin(), evals.begin(), &make_complex
  );
}
...
void arnoldi_geev(MatrixT& mtx, std::vector<std::complex<double> >& evals, MatrixT& evecs, std::complex<double>) {
  MatrixT null_matrix(mtx.size1(), mtx.size2());
  boost::numeric::bindings::lapack::geev(
      'N', 'V', mtx, evals, null_matrix, evecs
  );
}
}  // namespace detail
...
class arnoldi_iteration : public basic_iteration<T> {
 public:
  arnoldi_iteration(
      unsigned int max_iter_, unsigned int desired_eigenvalues__, T reltol_,
      T abstol_
  )
...
class simple_arnoldi {
 public:
  ...
  void calculate_eigenvalues(Iter& iter, bool verbose = false) {
    ...
    do {
      ...
      if (j > iter.desired_eigenvalues()) {
        ...
        h_matrix_type evecs(H.size1(), H.size2()),
            H2 = H;  // keep a backup because geev destroys the matrix
  ...
  std::complex<double> get_eigenvalue(int i) { return evals[i]; }

 protected:
  typedef boost::numeric::ublas::matrix<
      scalar_type, boost::numeric::ublas::column_major>
      h_matrix_type;
