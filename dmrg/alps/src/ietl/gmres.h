...
template <class T>
std::vector<T> Update(
    boost::numeric::ublas::matrix<T> const& H, std::vector<T> const& S,
    std::size_t k
) {
...
class ietl_gmres {
 ...
 public:
  ...
  Vector operator()(
      Matrix const& A, Vector const& b, Vector const& x0, double abs_tol = 1e-6
  ) {
    std::vector<typename Vector::value_type> s(max_iter + 1), cs(max_iter + 1),
        sn(max_iter + 1);
    ...
    if (std::abs(s[0]) < abs_tol) {
      if (verbose) std::cout << "Already done with x0." << std::endl;
      return x0;
    }

    v[0] = r / s[0];

    boost::numeric::ublas::matrix<typename Vector::value_type> H(
    ...
    for (; i < max_iter - 1; ++i) {
      ...
      if (verbose)
        std::cout << "GMRES iteration " << i
                  << ", resid = " << std::abs(s[i + 1]) << std::endl;

      if (std::abs(s[i + 1]) < abs_tol) break;
    }

    std::vector<typename Vector::value_type> y = detail::Update(H, s, i);
    r = x0;
    for (std::size_t k = 0; k < i; ++k) r += y[k] * v[k];
    return r;
  }
};
}  // namespace ietl
