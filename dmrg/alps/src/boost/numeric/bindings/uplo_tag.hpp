...
struct uplo_tag {
  typedef typename detail::uplo_tag_impl<
      typename detail::property_at<T, tag::data_side>::type, Trans>::type type;
};

}  // namespace result_of

//
// uplo_tag will output tags that are compatible with BLAS and LAPACK, either
// one of tag::transpose, tag::no_transpose, or tag::conjugate. It needs an
// library-orientation (Trans) before it can make a decision about whether a
// tranpose is in order. Consult the various cases above to see what kind of tag
// is being generated in what kind of situation.
//
