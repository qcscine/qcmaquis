...
struct adaptor<ublas::banded_matrix<T, F, A>, Id, Enable> {
  ...
  typedef typename if_row_major<
      typename convert_to<tag::data_order, F>::type, tag::column_major,
      tag::row_major>::type data_order;
  typedef mpl::map<
  ...
  static std::ptrdiff_t bandwidth1(const Id& id) { return id.lower(); }

  // A.k.a. right half-bandwidth
  static std::ptrdiff_t bandwidth2(const Id& id) { return id.upper(); }
...
struct adaptor<ublas::banded_adaptor<T>, Id, Enable>
    : basic_ublas_adaptor<
          T, Id, mpl::pair<tag::matrix_type, tag::band>,
          mpl::pair<tag::bandwidth_type<1>, std::ptrdiff_t>,
          mpl::pair<tag::bandwidth_type<2>, std::ptrdiff_t> > {
  // A.k.a. left half-bandwidth
  static std::ptrdiff_t bandwidth1(const Id& id) { return id.lower(); }
