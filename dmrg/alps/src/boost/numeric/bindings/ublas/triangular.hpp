...
template <
    typename T, typename F1, typename F2, typename A, typename Id,
    typename Enable>
struct adaptor<ublas::triangular_matrix<T, F1, F2, A>, Id, Enable> {
  ...
  typedef mpl::map<
      ...
      mpl::pair<
          tag::matrix_type, typename convert_to<tag::matrix_type, F1>::type>,
      mpl::pair<tag::data_structure, tag::triangular_array>,
      mpl::pair<tag::data_side, typename convert_to<tag::data_side, F1>::type>,
      mpl::pair<
          tag::data_order, typename convert_to<tag::data_order, F2>::type> >
      property_map;

  static std::ptrdiff_t size1(const Id& t) { return t.size1(); }
  ...
  static value_type* end_value(Id& t) { return bindings::end_value(t.data()); }
};
...
struct adaptor<ublas::triangular_adaptor<T, F>, Id, Enable>
    : basic_ublas_adaptor<
          T, Id,
          mpl::pair<
              tag::matrix_type, typename convert_to<tag::matrix_type, F>::type>,
          mpl::pair<
              tag::data_side, typename convert_to<tag::data_side, F>::type> > {
};
