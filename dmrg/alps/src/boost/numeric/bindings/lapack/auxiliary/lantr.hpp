...
inline std::ptrdiff_t lantr(
    const char norm, const UpLo, const Diag, const fortran_int_t m,
    const fortran_int_t n, const float* a, const fortran_int_t lda, float* work
) {
  fortran_int_t info(0);
  LAPACK_SLANTR(
      &norm, &lapack_option<UpLo>::value, &lapack_option<Diag>::value, &m, &n,
      a, &lda, work
  );
  return info;
}
...
struct lantr_impl {
  ...
  static std::ptrdiff_t invoke(
    ...
    BOOST_ASSERT(
        ...
        std::max<std::ptrdiff_t>(bindings::size_row(a), 1)
    );
    return detail::lantr(
        norm, uplo(), diag(), bindings::size_row(a), bindings::size_column(a),
  ...
  //
  static std::ptrdiff_t min_size_work(const char norm, const std::ptrdiff_t m) {
    if (norm == 'I')
...
lantr(const char norm, const MatrixA& a, Workspace work) {
  return lantr_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      norm, a, work
  );
}
