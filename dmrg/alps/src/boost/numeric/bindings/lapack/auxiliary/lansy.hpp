...
inline std::ptrdiff_t lansy(
    const char norm, const UpLo, const fortran_int_t n, const float* a,
...
template <typename UpLo>
inline std::ptrdiff_t lansy(
    const char norm, const UpLo, const fortran_int_t n, const double* a,
...
inline std::ptrdiff_t lansy(
    const char norm, const UpLo, const fortran_int_t n,
    const std::complex<double>* a, const fortran_int_t lda, double* work
) {
  fortran_int_t info(0);
...
struct lansy_impl {
  ...
  static std::ptrdiff_t invoke(
    ...
    return detail::lansy(
        norm, uplo(), bindings::size_column(a), bindings::begin_value(a),
        bindings::stride_major(a),
...
lansy(const char norm, const MatrixA& a, Workspace work) {
  return lansy_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      norm, a, work
  );
}
