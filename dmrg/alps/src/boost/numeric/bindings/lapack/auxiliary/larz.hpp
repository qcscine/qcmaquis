...
inline std::ptrdiff_t larz(
  ...
  LAPACK_SLARZ(
      &lapack_option<Side>::value, &m, &n, &l, v, &incv, &tau, c, &ldc, work
  );
  return info;
}
...
inline std::ptrdiff_t larz(
    const Side, const fortran_int_t m, const fortran_int_t n,
    const fortran_int_t l, const std::complex<float>* v,
...
inline std::ptrdiff_t larz(
    const Side, const fortran_int_t m, const fortran_int_t n,
    const fortran_int_t l, const std::complex<double>* v,
...
struct larz_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
      const Side side, const fortran_int_t l, const VectorV& v,
      const real_type tau, MatrixC& c, detail::workspace1<WORK> work
  ) {
    ...
    return detail::larz(
        side, bindings::size_row(c), bindings::size_column(c), l,
        bindings::begin_value(v), bindings::stride(v), tau,
...
struct larz_impl<Value, typename boost::enable_if<is_complex<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
      const Side side, const fortran_int_t l, const VectorV& v,
      const value_type tau, MatrixC& c, detail::workspace1<WORK> work
  ) {
    namespace bindings = ::boost::numeric::bindings;
    BOOST_STATIC_ASSERT((bindings::is_column_major<MatrixC>::value));
    BOOST_STATIC_ASSERT(
...
larz(
  ...
  return larz_impl<typename bindings::value_type<VectorV>::type>::invoke(
      side, l, v, tau, c, work
  );
}
...
larz(
  ...
  return larz_impl<typename bindings::value_type<VectorV>::type>::invoke(
      side, l, v, tau, c, optimal_workspace()
  );
}
//
// Overloaded function for larz. Its overload differs for
