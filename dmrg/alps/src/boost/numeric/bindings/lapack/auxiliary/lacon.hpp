...
//
inline std::ptrdiff_t lacon(
    const fortran_int_t n, std::complex<float>* v, std::complex<float>* x,
...
//
inline std::ptrdiff_t lacon(
    const fortran_int_t n, std::complex<double>* v, std::complex<double>* x,
    double& est, fortran_int_t& kase
) {
  fortran_int_t info(0);
...
struct lacon_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
      const fortran_int_t n, VectorX& x, real_type& est, fortran_int_t& kase,
      detail::workspace2<V, ISGN> work
  ) {
    namespace bindings = ::boost::numeric::bindings;
    BOOST_STATIC_ASSERT((bindings::is_mutable<VectorX>::value));
    BOOST_ASSERT(
        bindings::size(work.select(fortran_int_t())) >= min_size_isgn(n)
    );
    BOOST_ASSERT(bindings::size(work.select(real_type())) >= min_size_v(n));
    BOOST_ASSERT(n >= 1);
    return detail::lacon(
        n, bindings::begin_value(work.select(real_type())),
        bindings::begin_value(x),
        bindings::begin_value(work.select(fortran_int_t())), est, kase
    );
  }
  ...
  static std::ptrdiff_t invoke(
    ...
    bindings::detail::array<fortran_int_t> tmp_isgn(min_size_isgn(n));
  ...
  // workspace-array v.
  //
  static std::ptrdiff_t min_size_v(const std::ptrdiff_t n) { return n; }

  //
  // Static member function that returns the minimum size of
  // workspace-array isgn.
  //
  static std::ptrdiff_t min_size_isgn(const std::ptrdiff_t n) { return n; }
};
...
struct lacon_impl<Value, typename boost::enable_if<is_complex<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
      const fortran_int_t n, VectorX& x, real_type& est, fortran_int_t& kase,
      detail::workspace1<V> work
  ) {
    namespace bindings = ::boost::numeric::bindings;
    BOOST_STATIC_ASSERT((bindings::is_mutable<VectorX>::value));
    BOOST_ASSERT(bindings::size(work.select(value_type())) >= min_size_v(n));
    BOOST_ASSERT(n >= 1);
    return detail::lacon(
        n, bindings::begin_value(work.select(value_type())),
...
lacon(
  ...
  return lacon_impl<typename bindings::value_type<VectorX>::type>::invoke(
      n, x, est, kase, optimal_workspace()
  );
}
