...
inline std::ptrdiff_t hetri(
  ...
  fortran_int_t info(0);
  LAPACK_CHETRI(&lapack_option<UpLo>::value, &n, a, &lda, ipiv, work, &info);
  return info;
}
...
inline std::ptrdiff_t hetri(
  ...
  fortran_int_t info(0);
  LAPACK_ZHETRI(&lapack_option<UpLo>::value, &n, a, &lda, ipiv, work, &info);
  return info;
}
...
hetri(MatrixA& a, const VectorIPIV& ipiv, Workspace work) {
  return hetri_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      a, ipiv, work
  );
}
