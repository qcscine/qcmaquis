...
inline std::ptrdiff_t unmrz(
    const Side, const Trans, const fortran_int_t m, const fortran_int_t n,
  ...
  LAPACK_SORMRZ(
      &lapack_option<Side>::value, &lapack_option<Trans>::value, &m, &n, &k, &l,
...
inline std::ptrdiff_t unmrz(
    const Side, const Trans, const fortran_int_t m, const fortran_int_t n,
    const fortran_int_t k, const fortran_int_t l, const std::complex<float>* a,
    const fortran_int_t lda, const std::complex<float>* tau,
    std::complex<float>* c, const fortran_int_t ldc, std::complex<float>* work,
...
inline std::ptrdiff_t unmrz(
    const Side, const Trans, const fortran_int_t m, const fortran_int_t n,
    const fortran_int_t k, const fortran_int_t l, const std::complex<double>* a,
...
struct unmrz_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  //
  template <
      typename Side, typename MatrixA, typename VectorTAU, typename MatrixC,
      typename WORK>
  static std::ptrdiff_t invoke(
      const Side side, const MatrixA& a, const VectorTAU& tau, MatrixC& c,
    ...
    BOOST_ASSERT(
        ...
        min_size_work(side, bindings::size_row(c), bindings::size_column(c))
    );
    ...
    BOOST_ASSERT(
        ...
        std::max<std::ptrdiff_t>(1, bindings::size(tau))
    );
    BOOST_ASSERT(
        bindings::stride_major(c) >=
        std::max<std::ptrdiff_t>(1, bindings::size_row(c))
    );
    return detail::unmrz(
        side, trans(), bindings::size_row(c), bindings::size_column(c),
        bindings::size(tau), bindings::size_column_op(a, trans()),
  ...
  static std::ptrdiff_t min_size_work(
      const Side side, const std::ptrdiff_t m, const std::ptrdiff_t n
  ) {
    return std::max<std::ptrdiff_t>(1, bindings::detail::if_left(side, n, m));
  }
};
...
struct unmrz_impl<Value, typename boost::enable_if<is_complex<Value> >::type> {
  ...
  template <
      ...
      typename WORK>
  static std::ptrdiff_t invoke(
      const Side side, const fortran_int_t k, const MatrixA& a,
      const VectorTAU& tau, MatrixC& c, detail::workspace1<WORK> work
  ) {
    namespace bindings = ::boost::numeric::bindings;
    typedef typename result_of::data_order<MatrixC>::type order;
    typedef typename result_of::trans_tag<MatrixA, order>::type trans;
    ...
    BOOST_ASSERT(bindings::size_row(c) >= 0);
    BOOST_ASSERT(bindings::stride_major(a) >= std::max<std::ptrdiff_t>(1, k));
    BOOST_ASSERT(
        bindings::stride_major(c) >=
        std::max<std::ptrdiff_t>(1, bindings::size_row(c))
    );
    return detail::unmrz(
        side, trans(), bindings::size_row(c), bindings::size_column(c), k,
        bindings::size_column_op(a, trans()), bindings::begin_value(a),
  ...
  static std::ptrdiff_t invoke(
    ...
    detail::unmrz(
        ...
        bindings::size_column_op(a, trans()), bindings::begin_value(a),
        bindings::stride_major(a), bindings::begin_value(tau),
        bindings::begin_value(c), bindings::stride_major(c), &opt_size_work, -1
    );
    bindings::detail::array<value_type> tmp_work(
        traits::detail::to_int(opt_size_work)
    );
...
unmrz(
  ...
  return unmrz_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      side, a, tau, c, work
  );
}
...
unmrz(const Side side, const MatrixA& a, const VectorTAU& tau, MatrixC& c) {
  return unmrz_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      side, a, tau, c, optimal_workspace()
  );
}
//
// Overloaded function for unmrz. Its overload differs for
