...
inline std::ptrdiff_t gbbrd(
  ...
  LAPACK_SGBBRD(
      &vect, &m, &n, &ncc, &kl, &ku, ab, &ldab, d, e, q, &ldq, pt, &ldpt, c,
      &ldc, work, &info
  );
  return info;
}
...
inline std::ptrdiff_t gbbrd(
    ...
    std::complex<float>* q, const fortran_int_t ldq, std::complex<float>* pt,
    const fortran_int_t ldpt, std::complex<float>* c, const fortran_int_t ldc,
...
inline std::ptrdiff_t gbbrd(
    const char vect, const fortran_int_t m, const fortran_int_t n,
    const fortran_int_t ncc, const fortran_int_t kl, const fortran_int_t ku,
    std::complex<double>* ab, const fortran_int_t ldab, double* d, double* e,
    std::complex<double>* q, const fortran_int_t ldq, std::complex<double>* pt,
    const fortran_int_t ldpt, std::complex<double>* c, const fortran_int_t ldc,
...
struct gbbrd_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  //
  template <
      typename MatrixAB, typename VectorD, typename VectorE, typename MatrixQ,
      typename MatrixPT, typename MatrixC, typename WORK>
  static std::ptrdiff_t invoke(
      const char vect, MatrixAB& ab, VectorD& d, VectorE& e, MatrixQ& q,
      MatrixPT& pt, MatrixC& c, detail::workspace1<WORK> work
  ) {
    ...
    BOOST_STATIC_ASSERT(
        (boost::is_same<
            typename remove_const<
                typename bindings::value_type<MatrixAB>::type>::type,
            typename remove_const<
                typename bindings::value_type<MatrixPT>::type>::type>::value)
    );
    BOOST_STATIC_ASSERT(
        (boost::is_same<
            typename remove_const<
    ...
    BOOST_ASSERT(
        ...
        min_size_work(bindings::size_row(ab), bindings::size_column(ab))
    );
    ...
    BOOST_ASSERT(
        bindings::size_minor(pt) == 1 || bindings::stride_minor(pt) == 1
    );
    BOOST_ASSERT(
    ...
    BOOST_ASSERT(
        ...
        bindings::bandwidth_lower(ab) + bindings::bandwidth_upper(ab) + 1
    );
    BOOST_ASSERT(vect == 'N' || vect == 'Q' || vect == 'P' || vect == 'B');
    return detail::gbbrd(
        vect, bindings::size_row(ab), bindings::size_column(ab),
        bindings::size_column(c), bindings::bandwidth_lower(ab),
        ...
        bindings::stride_major(q), bindings::begin_value(pt),
        bindings::stride_major(pt), bindings::begin_value(c),
  ...
  template <
      typename MatrixAB, typename VectorD, typename VectorE, typename MatrixQ,
      typename MatrixPT, typename MatrixC>
  static std::ptrdiff_t invoke(
      const char vect, MatrixAB& ab, VectorD& d, VectorE& e, MatrixQ& q,
      MatrixPT& pt, MatrixC& c, minimal_workspace
  ) {
    namespace bindings = ::boost::numeric::bindings;
  ...
  static std::ptrdiff_t invoke(
      const char vect, MatrixAB& ab, VectorD& d, VectorE& e, MatrixQ& q,
      MatrixPT& pt, MatrixC& c, optimal_workspace
  ) {
    namespace bindings = ::boost::numeric::bindings;
...
struct gbbrd_impl<Value, typename boost::enable_if<is_complex<Value> >::type> {
  ...
  template <
      typename MatrixAB, typename VectorD, typename VectorE, typename MatrixQ,
      typename MatrixPT, typename MatrixC, typename WORK, typename RWORK>
  static std::ptrdiff_t invoke(
      const char vect, MatrixAB& ab, VectorD& d, VectorE& e, MatrixQ& q,
      MatrixPT& pt, MatrixC& c, detail::workspace2<WORK, RWORK> work
  ) {
    namespace bindings = ::boost::numeric::bindings;
    BOOST_STATIC_ASSERT((bindings::is_column_major<MatrixAB>::value));
    BOOST_STATIC_ASSERT((bindings::is_column_major<MatrixQ>::value));
    ...
    BOOST_ASSERT(
        bindings::size(work.select(real_type())) >=
        min_size_rwork(bindings::size_row(ab), bindings::size_column(ab))
    );
    BOOST_ASSERT(
  ...
  static std::ptrdiff_t invoke(
    ...
    return invoke(vect, ab, d, e, q, pt, c, workspace(tmp_work, tmp_rwork));
  }
...
template <
    typename MatrixAB, typename VectorD, typename VectorE, typename MatrixQ,
    typename MatrixPT, typename MatrixC, typename Workspace>
...
gbbrd(
  ...
  return gbbrd_impl<typename bindings::value_type<MatrixAB>::type>::invoke(
      vect, ab, d, e, q, pt, c, work
  );
}
...
inline typename boost::disable_if<
    detail::is_workspace<MatrixC>, std::ptrdiff_t>::type
gbbrd(
    const char vect, MatrixAB& ab, VectorD& d, VectorE& e, MatrixQ& q,
    MatrixPT& pt, MatrixC& c
) {
  return gbbrd_impl<typename bindings::value_type<MatrixAB>::type>::invoke(
      vect, ab, d, e, q, pt, c, optimal_workspace()
  );
}
