...
inline std::ptrdiff_t spcon(
    const UpLo, const fortran_int_t n, const float* ap,
    const fortran_int_t* ipiv, const float anorm, float& rcond, float* work,
    fortran_int_t* iwork
) {
  fortran_int_t info(0);
  LAPACK_SSPCON(
      &lapack_option<UpLo>::value, &n, ap, ipiv, &anorm, &rcond, work, iwork,
      &info
  );
  return info;
}
...
template <typename UpLo>
inline std::ptrdiff_t spcon(
    const UpLo, const fortran_int_t n, const double* ap,
...
inline std::ptrdiff_t spcon(
  ...
  fortran_int_t info(0);
  LAPACK_CSPCON(
      &lapack_option<UpLo>::value, &n, ap, ipiv, &anorm, &rcond, work, &info
  );
  return info;
}
...
struct spcon_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  //
  template <
      typename MatrixAP, typename VectorIPIV, typename WORK, typename IWORK>
  static std::ptrdiff_t invoke(
      const MatrixAP& ap, const VectorIPIV& ipiv, const real_type anorm,
  ...
  static std::ptrdiff_t invoke(
    ...
    return invoke(ap, ipiv, anorm, rcond, workspace(tmp_work, tmp_iwork));
  }
...
spcon(
  ...
  return spcon_impl<typename bindings::value_type<MatrixAP>::type>::invoke(
