...
inline std::ptrdiff_t tbcon(
  ...
  LAPACK_STBCON(
      &norm, &lapack_option<UpLo>::value, &lapack_option<Diag>::value, &n, &kd,
      ab, &ldab, &rcond, work, iwork, &info
  );
  return info;
}
...
inline std::ptrdiff_t tbcon(
    ...
    const fortran_int_t ldab, float& rcond, std::complex<float>* work,
    float* rwork
) {
  fortran_int_t info(0);
  LAPACK_CTBCON(
      &norm, &lapack_option<UpLo>::value, &lapack_option<Diag>::value, &n, &kd,
      ab, &ldab, &rcond, work, rwork, &info
  );
  return info;
}
...
inline std::ptrdiff_t tbcon(
    ...
    const fortran_int_t ldab, double& rcond, std::complex<double>* work,
    double* rwork
) {
  fortran_int_t info(0);
  LAPACK_ZTBCON(
...
struct tbcon_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
      const char norm, const fortran_int_t kd, const MatrixAB& ab,
    ...
    return detail::tbcon(
        norm, uplo(), diag(), bindings::size_column(ab), kd,
        bindings::begin_value(ab), bindings::stride_major(ab), rcond,
...
tbcon(
  ...
  return tbcon_impl<typename bindings::value_type<MatrixAB>::type>::invoke(
      norm, kd, ab, rcond, work
  );
}
...
tbcon(
  ...
  return tbcon_impl<typename bindings::value_type<MatrixAB>::type>::invoke(
      norm, kd, ab, rcond, optimal_workspace()
  );
}
