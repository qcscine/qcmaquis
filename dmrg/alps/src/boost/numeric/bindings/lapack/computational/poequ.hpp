...
//
inline std::ptrdiff_t poequ(
    const fortran_int_t n, const double* a, const fortran_int_t lda, double* s,
...
struct poequ_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
      const MatrixA& a, VectorS& s, real_type& scond, real_type& amax
  ) {
    ...
    return detail::poequ(
        ...
        bindings::stride_major(a), bindings::begin_value(s), scond, amax
    );
  }
};
...
template <typename MatrixA, typename VectorS>
inline std::ptrdiff_t poequ(
    const MatrixA& a, VectorS& s,
    typename remove_imaginary<
        typename bindings::value_type<MatrixA>::type>::type& scond,
    typename remove_imaginary<
        typename bindings::value_type<MatrixA>::type>::type& amax
) {
  return poequ_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      a, s, scond, amax
  );
}
