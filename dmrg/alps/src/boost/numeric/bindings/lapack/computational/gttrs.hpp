...
inline std::ptrdiff_t gttrs(
  ...
  LAPACK_SGTTRS(
      &lapack_option<Trans>::value, &n, &nrhs, dl, d, du, du2, ipiv, b, &ldb,
      &info
  );
  return info;
}
...
inline std::ptrdiff_t gttrs(
    ...
    const fortran_int_t* ipiv, std::complex<float>* b, const fortran_int_t ldb
) {
  fortran_int_t info(0);
  LAPACK_CGTTRS(
      &lapack_option<Trans>::value, &n, &nrhs, dl, d, du, du2, ipiv, b, &ldb,
...
inline std::ptrdiff_t gttrs(
    ...
    const fortran_int_t* ipiv, std::complex<double>* b, const fortran_int_t ldb
) {
  fortran_int_t info(0);
  LAPACK_ZGTTRS(
      &lapack_option<Trans>::value, &n, &nrhs, dl, d, du, du2, ipiv, b, &ldb,
...
//
template <
    typename VectorDL, typename VectorD, typename VectorDU, typename VectorDU2,
    typename VectorIPIV, typename MatrixB>
inline std::ptrdiff_t gttrs(
    const VectorDL& dl, const VectorD& d, const VectorDU& du,
    const VectorDU2& du2, const VectorIPIV& ipiv, MatrixB& b
) {
  return gttrs_impl<typename bindings::value_type<VectorDL>::type>::invoke(
      dl, d, du, du2, ipiv, b
  );
}
