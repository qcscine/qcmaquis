...
struct steqr_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  static std::ptrdiff_t min_size_work(
      const std::ptrdiff_t n, const char compz
  ) {
    if (compz == 'N') {
      return 1;
    } else {
...
steqr(const char compz, VectorD& d, VectorE& e, MatrixZ& z, Workspace work) {
  return steqr_impl<typename bindings::value_type<MatrixZ>::type>::invoke(
      compz, d, e, z, work
  );
}
