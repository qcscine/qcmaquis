...
inline std::ptrdiff_t opmtr(
    const Side, const char uplo, const Trans, const fortran_int_t m,
    const fortran_int_t n, const float* ap, const float* tau, float* c,
    const fortran_int_t ldc, float* work
) {
  fortran_int_t info(0);
  LAPACK_SOPMTR(
      &lapack_option<Side>::value, &uplo, &lapack_option<Trans>::value, &m, &n,
      ap, tau, c, &ldc, work, &info
  );
  return info;
}
...
inline std::ptrdiff_t opmtr(
    const Side, const char uplo, const Trans, const fortran_int_t m,
    const fortran_int_t n, const double* ap, const double* tau, double* c,
...
struct opmtr_impl {
  ...
  //
  template <
      typename Side, typename VectorAP, typename VectorTAU, typename MatrixC,
      typename WORK>
  static std::ptrdiff_t invoke(
      const Side side, const char uplo, const VectorAP& ap,
    ...
    return detail::opmtr(
        side, uplo, trans(), bindings::size_row(c), bindings::size_column(c),
...
opmtr(
    const Side side, const char uplo, const VectorAP& ap, const VectorTAU& tau,
    MatrixC& c, Workspace work
) {
  return opmtr_impl<typename bindings::value_type<VectorAP>::type>::invoke(
      side, uplo, ap, tau, c, work
  );
}
...
opmtr(
  ...
  return opmtr_impl<typename bindings::value_type<VectorAP>::type>::invoke(
      side, uplo, ap, tau, c, optimal_workspace()
  );
}
