...
inline std::ptrdiff_t pbtrs(
    ...
    const fortran_int_t ldab, std::complex<float>* b, const fortran_int_t ldb
) {
  fortran_int_t info(0);
  LAPACK_CPBTRS(
      &lapack_option<UpLo>::value, &n, &kd, &nrhs, ab, &ldab, b, &ldb, &info
...
inline std::ptrdiff_t pbtrs(const MatrixAB& ab, MatrixB& b) {
  return pbtrs_impl<typename bindings::value_type<MatrixAB>::type>::invoke(
      ab, b
  );
}
