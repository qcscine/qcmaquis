...
inline std::ptrdiff_t tptri(
    const UpLo, const Diag, const fortran_int_t n, float* ap
) {
  fortran_int_t info(0);
  LAPACK_STPTRI(
      &lapack_option<UpLo>::value, &lapack_option<Diag>::value, &n, ap, &info
  );
  return info;
}
...
template <typename UpLo, typename Diag>
inline std::ptrdiff_t tptri(
    const UpLo, const Diag, const fortran_int_t n, double* ap
) {
  fortran_int_t info(0);
  LAPACK_DTPTRI(
      &lapack_option<UpLo>::value, &lapack_option<Diag>::value, &n, ap, &info
...
template <typename UpLo, typename Diag>
inline std::ptrdiff_t tptri(
    const UpLo, const Diag, const fortran_int_t n, std::complex<float>* ap
) {
  fortran_int_t info(0);
  LAPACK_CTPTRI(
      &lapack_option<UpLo>::value, &lapack_option<Diag>::value, &n, ap, &info
...
template <typename UpLo, typename Diag>
inline std::ptrdiff_t tptri(
    const UpLo, const Diag, const fortran_int_t n, std::complex<double>* ap
) {
  fortran_int_t info(0);
  LAPACK_ZTPTRI(
      &lapack_option<UpLo>::value, &lapack_option<Diag>::value, &n, ap, &info
...
struct tptri_impl {
  ...
  static std::ptrdiff_t invoke(MatrixAP& ap) {
    ...
    return detail::tptri(
        uplo(), diag(), bindings::size_column(ap), bindings::begin_value(ap)
    );
  }
};
...
inline std::ptrdiff_t tptri(MatrixAP& ap) {
  return tptri_impl<typename bindings::value_type<MatrixAP>::type>::invoke(ap);
}
