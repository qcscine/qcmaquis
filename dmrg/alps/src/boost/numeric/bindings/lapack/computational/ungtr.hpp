...
struct ungtr_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
      const fortran_int_t n, MatrixA& a, const VectorTAU& tau,
    ...
    BOOST_ASSERT(n >= 0);
    return detail::ungtr(
        uplo(), n, bindings::begin_value(a), bindings::stride_major(a),
  ...
  static std::ptrdiff_t invoke(
      const fortran_int_t n, MatrixA& a, const VectorTAU& tau, minimal_workspace
  ) {
    namespace bindings = ::boost::numeric::bindings;
    typedef typename result_of::uplo_tag<MatrixA>::type uplo;
  ...
  template <typename MatrixA, typename VectorTAU>
  static std::ptrdiff_t invoke(
      const fortran_int_t n, MatrixA& a, const VectorTAU& tau, optimal_workspace
  ) {
    namespace bindings = ::boost::numeric::bindings;
    typedef typename result_of::uplo_tag<MatrixA>::type uplo;
...
struct ungtr_impl<Value, typename boost::enable_if<is_complex<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
    ...
    BOOST_STATIC_ASSERT((bindings::is_mutable<MatrixA>::value));
    BOOST_ASSERT(bindings::size(tau) >= n - 1);
    BOOST_ASSERT(bindings::size(work.select(value_type())) >= min_size_work(n));
    BOOST_ASSERT(
        bindings::size_minor(a) == 1 || bindings::stride_minor(a) == 1
    );
...
ungtr(const fortran_int_t n, MatrixA& a, const VectorTAU& tau, Workspace work) {
  return ungtr_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      n, a, tau, work
  );
}
