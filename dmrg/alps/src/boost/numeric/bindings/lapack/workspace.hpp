 ...
 * - minimal_workspace() : the function allocates the minimum workspace required
 * for the function
 * - optimal_workspace() : the function allocates the amount of workspace that
 * allows optimal execution.
 * - workspace( work )   : the function uses the workspace array in work.
 * - workspace( rwork, work ) : the function uses a real array rwork and a
 * compolex array work as workspace. (There are Lapack functions for complex
 * matrices that require two workarrays)
 * */

// Four classes are introduced to distinguish between the different type of
// memory allocations
...
class workspace2 {
 public:
  workspace2(W& w, WRI& wri) : w_(w), wri_(wri) {}
...
class workspace3 {
 public:
  workspace3(W& w, WR& wr, WI& wi) : w_(w), wr_(wr), wi_(wi) {}
...
class workspace4 {
 public:
  workspace4(W& w, WR& wr, WI& wi, WB& wb) : w_(w), wr_(wr), wi_(wi), wb_(wb) {}
...
//
//   Complex valued: workspace( complex array, real array, integer array, bool
//   array )
//
template <typename W, typename WR, typename WI, typename WB>
