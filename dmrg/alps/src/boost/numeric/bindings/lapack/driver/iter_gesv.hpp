...
// * double value-type.
//
inline std::ptrdiff_t iter_gesv(
...
//
inline std::ptrdiff_t iter_gesv(
    const fortran_int_t n, const fortran_int_t nrhs, std::complex<double>* a,
    const fortran_int_t lda, fortran_int_t* ipiv, const std::complex<double>* b,
  ...
  fortran_int_t info(0);
  LAPACK_ZCGESV(
      &n, &nrhs, a, &lda, ipiv, b, &ldb, x, &ldx, work, swork, rwork, &iter,
      &info
  );
  return info;
}
...
struct iter_gesv_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  //
  template <
      typename MatrixA, typename VectorIPIV, typename MatrixB, typename MatrixX,
      typename WORK, typename SWORK>
  static std::ptrdiff_t invoke(
      MatrixA& a, VectorIPIV& ipiv, const MatrixB& b, MatrixX& x,
    ...
    BOOST_ASSERT(
        ...
        min_size_swork(bindings::size_column(a), bindings::size_column(b))
    );
    BOOST_ASSERT(
        bindings::size(work.select(real_type())) >=
        min_size_work(bindings::size_column(a), bindings::size_column(b))
    );
  ...
  static std::ptrdiff_t invoke(
    ...
    bindings::detail::array<real_type> tmp_swork(
        min_size_swork(bindings::size_column(a), bindings::size_column(b))
    );
  ...
  //
  static std::ptrdiff_t min_size_swork(
      const std::ptrdiff_t n, const std::ptrdiff_t nrhs
  ) {
    return n * (n + nrhs);
  }
...
template <typename Value>
struct iter_gesv_impl<
    Value, typename boost::enable_if<is_complex<Value> >::type> {
  typedef Value value_type;
  typedef typename remove_imaginary<Value>::type real_type;
  ...
  static std::ptrdiff_t invoke(
      MatrixA& a, VectorIPIV& ipiv, const MatrixB& b, MatrixX& x,
      fortran_int_t& iter, detail::workspace3<WORK, SWORK, RWORK> work
  ) {
    namespace bindings = ::boost::numeric::bindings;
    BOOST_STATIC_ASSERT((bindings::is_column_major<MatrixA>::value));
    BOOST_STATIC_ASSERT((bindings::is_column_major<MatrixB>::value));
  ...
  static std::ptrdiff_t invoke(
    ...
    bindings::detail::array<value_type> tmp_swork(
        min_size_swork(bindings::size_column(a), bindings::size_column(b))
    );
    ...
    return invoke(
        a, ipiv, b, x, iter, workspace(tmp_work, tmp_swork, tmp_rwork)
    );
  }
...
iter_gesv(
  ...
  return iter_gesv_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      a, ipiv, b, x, iter, work
  );
}
...
inline typename boost::disable_if<
    detail::is_workspace<MatrixX>, std::ptrdiff_t>::type
iter_gesv(
    MatrixA& a, VectorIPIV& ipiv, const MatrixB& b, MatrixX& x,
    fortran_int_t& iter
) {
  return iter_gesv_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      a, ipiv, b, x, iter, optimal_workspace()
  );
}
