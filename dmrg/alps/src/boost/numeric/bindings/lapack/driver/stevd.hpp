...
inline std::ptrdiff_t stevd(
  ...
  LAPACK_SSTEVD(&jobz, &n, d, e, z, &ldz, work, &lwork, iwork, &liwork, &info);
  return info;
}
...
inline std::ptrdiff_t stevd(
  ...
  fortran_int_t info(0);
  LAPACK_DSTEVD(&jobz, &n, d, e, z, &ldz, work, &lwork, iwork, &liwork, &info);
  return info;
}
...
struct stevd_impl {
  ...
  //
  template <
      typename VectorD, typename VectorE, typename MatrixZ, typename WORK,
      typename IWORK>
  static std::ptrdiff_t invoke(
      const char jobz, const fortran_int_t n, VectorD& d, VectorE& e,
    ...
    BOOST_ASSERT(
        bindings::size(work.select(fortran_int_t())) >= min_size_iwork(jobz, n)
    );
    BOOST_ASSERT(
        bindings::size(work.select(real_type())) >= min_size_work(jobz, n)
    );
    BOOST_ASSERT(
    ...
    BOOST_ASSERT(n >= 0);
    return detail::stevd(
        jobz, n, bindings::begin_value(d), bindings::begin_value(e),
  ...
  static std::ptrdiff_t invoke(
    ...
    namespace bindings = ::boost::numeric::bindings;
    bindings::detail::array<real_type> tmp_work(min_size_work(jobz, n));
    bindings::detail::array<fortran_int_t> tmp_iwork(min_size_iwork(jobz, n));
...
stevd(
    const char jobz, const fortran_int_t n, VectorD& d, VectorE& e, MatrixZ& z,
    Workspace work
) {
  return stevd_impl<typename bindings::value_type<VectorD>::type>::invoke(
      jobz, n, d, e, z, work
  );
}
...
stevd(
  ...
  return stevd_impl<typename bindings::value_type<VectorD>::type>::invoke(
      jobz, n, d, e, z, optimal_workspace()
  );
}
