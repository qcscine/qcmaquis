...
inline std::ptrdiff_t gelsd(
    ...
    const fortran_int_t ldb, float* s, const float rcond, fortran_int_t& rank,
    std::complex<float>* work, const fortran_int_t lwork, float* rwork,
    fortran_int_t* iwork
) {
  fortran_int_t info(0);
  LAPACK_CGELSD(
      &m, &n, &nrhs, a, &lda, b, &ldb, s, &rcond, &rank, work, &lwork, rwork,
      iwork, &info
  );
  return info;
}
...
inline std::ptrdiff_t gelsd(
    ...
    std::complex<double>* b, const fortran_int_t ldb, double* s,
    const double rcond, fortran_int_t& rank, std::complex<double>* work,
...
struct gelsd_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  //
  template <
      typename MatrixA, typename MatrixB, typename VectorS, typename WORK,
      typename IWORK>
  static std::ptrdiff_t invoke(
      MatrixA& a, MatrixB& b, VectorS& s, const real_type rcond,
    ...
    return detail::gelsd(
        ...
        bindings::stride_major(b), bindings::begin_value(s), rcond, rank,
  ...
  static std::ptrdiff_t min_size_iwork(
      const std::ptrdiff_t minmn, const std::ptrdiff_t nlvl
  ) {
...
struct gelsd_impl<Value, typename boost::enable_if<is_complex<Value> >::type> {
  ...
  static std::ptrdiff_t invoke(
    ...
    BOOST_ASSERT(
        ...
        min_size_work(bindings::size_column(a), minmn, bindings::size_column(b))
    );
    BOOST_ASSERT(bindings::size_column(a) >= 0);
    BOOST_ASSERT(bindings::size_column(b) >= 0);
    BOOST_ASSERT(
  ...
  static std::ptrdiff_t min_size_work(
    ...
    return std::max<std::ptrdiff_t>(
        1, 2 * minmn + std::max<std::ptrdiff_t>(n, minmn * nrhs)
    );
  }
  ...
  static std::ptrdiff_t min_size_rwork(
    ...
    std::ptrdiff_t smlsiz_plus_one = smlsiz + 1;
    return std::max<std::ptrdiff_t>(
        1, 10 * minmn + 2 * minmn * smlsiz + 8 * minmn * nlvl +
...
//
template <
    typename MatrixA, typename MatrixB, typename VectorS, typename Workspace>
inline typename boost::enable_if<
    detail::is_workspace<Workspace>, std::ptrdiff_t>::type
gelsd(
    MatrixA& a, MatrixB& b, VectorS& s,
    const typename remove_imaginary<
        typename bindings::value_type<MatrixA>::type>::type rcond,
    fortran_int_t& rank, Workspace work
) {
  return gelsd_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      a, b, s, rcond, rank, work
  );
}
...
inline typename boost::disable_if<
    detail::is_workspace<VectorS>, std::ptrdiff_t>::type
gelsd(
    MatrixA& a, MatrixB& b, VectorS& s,
    const typename remove_imaginary<
        typename bindings::value_type<MatrixA>::type>::type rcond,
  ...
  return gelsd_impl<typename bindings::value_type<MatrixA>::type>::invoke(
      a, b, s, rcond, rank, optimal_workspace()
  );
}
