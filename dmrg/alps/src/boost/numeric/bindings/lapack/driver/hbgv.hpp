...
struct hbgv_impl<Value, typename boost::enable_if<is_complex<Value> >::type> {
  ...
  //
  static std::ptrdiff_t min_size_rwork(const std::ptrdiff_t n) { return 3 * n; }
};
...
hbgv(
  ...
  return hbgv_impl<typename bindings::value_type<MatrixAB>::type>::invoke(
      jobz, ab, bb, w, z, work
  );
}
...
hbgv(const char jobz, MatrixAB& ab, MatrixBB& bb, VectorW& w, MatrixZ& z) {
  return hbgv_impl<typename bindings::value_type<MatrixAB>::type>::invoke(
