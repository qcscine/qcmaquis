...
inline std::ptrdiff_t ptsvx(
  ...
  LAPACK_SPTSVX(
      &fact, &n, &nrhs, d, e, df, ef, b, &ldb, x, &ldx, &rcond, ferr, berr,
      work, &info
  );
  return info;
}
...
struct ptsvx_impl<Value, typename boost::enable_if<is_real<Value> >::type> {
  ...
  //
  template <
      typename VectorD, typename VectorE, typename VectorDF, typename VectorEF,
      typename MatrixB, typename MatrixX, typename VectorFERR,
      typename VectorBERR, typename WORK>
  static std::ptrdiff_t invoke(
      const char fact, const VectorD& d, const VectorE& e, VectorDF& df,
      VectorEF& ef, const MatrixB& b, MatrixX& x, real_type& rcond,
      VectorFERR& ferr, VectorBERR& berr, detail::workspace1<WORK> work
  ) {
    ...
    BOOST_ASSERT(
        ...
        std::max<std::ptrdiff_t>(1, bindings::size(d))
    );
    BOOST_ASSERT(
    ...
    return detail::ptsvx(
        fact, bindings::size(d), bindings::size_column(b),
  ...
  static std::ptrdiff_t invoke(
    ...
    namespace bindings = ::boost::numeric::bindings;
    return invoke(
        fact, d, e, df, ef, b, x, rcond, ferr, berr, minimal_workspace()
    );
  }
...
ptsvx(
    const char fact, const VectorD& d, const VectorE& e, VectorDF& df,
    VectorEF& ef, const MatrixB& b, MatrixX& x,
  ...
  return ptsvx_impl<typename bindings::value_type<VectorE>::type>::invoke(
      fact, d, e, df, ef, b, x, rcond, ferr, berr, work
  );
}
...
ptsvx(
  ...
  return ptsvx_impl<typename bindings::value_type<VectorE>::type>::invoke(
      fact, d, e, df, ef, b, x, rcond, ferr, berr, optimal_workspace()
  );
}
