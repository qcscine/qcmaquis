...
inline std::ptrdiff_t spsv(
    const UpLo, const fortran_int_t n, const fortran_int_t nrhs,
    std::complex<double>* ap, fortran_int_t* ipiv, std::complex<double>* b,
    const fortran_int_t ldb
) {
  fortran_int_t info(0);
  LAPACK_ZSPSV(
...
inline std::ptrdiff_t spsv(MatrixAP& ap, VectorIPIV& ipiv, MatrixB& b) {
  return spsv_impl<typename bindings::value_type<MatrixAP>::type>::invoke(
      ap, ipiv, b
  );
}
