...
template <typename T>
struct column_wrapper : adaptable_type<column_wrapper<T> >,
                        reference_wrapper<T> {
  column_wrapper(T& t, std::size_t index)
      : reference_wrapper<T>(t), m_index(index) {}
...
struct adaptor<column_wrapper<T>, Id, Enable> {
  ...
  static typename result_of::begin_value<T>::type begin_value(Id& id) {
    return bindings::begin_value(id.get()) + offset(id.get(), 0, id.m_index);
  }
...
template <typename T>
detail::column_wrapper<const T> const column(
    const T& underlying, std::size_t index
) {
