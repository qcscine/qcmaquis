...
template <typename T>
struct vector_view_wrapper : adaptable_type<vector_view_wrapper<T> > {
  typedef T value_type;

  vector_view_wrapper(T* t, std::size_t size) : m_t(t), m_size(size) {}
  T* m_t;
  std::size_t m_size;
};
...
struct adaptor<vector_view_wrapper<T>, Id, Enable> {
  ...
  static std::ptrdiff_t size1(const Id& id) { return id.m_size; }

  static value_type* begin_value(Id& id) { return id.m_t; }

  static value_type* end_value(Id& id) { return id.m_t + id.m_size; }
};
...
template <typename T>
detail::vector_view_wrapper<const T> const vector_view(
