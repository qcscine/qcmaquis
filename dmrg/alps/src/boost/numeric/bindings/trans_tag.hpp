...
struct trans_tag {
  typedef typename detail::trans_tag_impl<
      Order, typename result_of::data_order<T>::type,
...
//
// trans_tag will output tags that are compatible with BLAS and LAPACK, either
// one of tag::transpose, tag::no_transpose, or tag::conjugate. It needs an
// library-orientation (Order) before it can make a decision about whether a
