...
struct end_impl<T, tag::value> {
  ...
  static result_type invoke(T& t) { return adaptor_access<T>::end_value(t); }
};
...
//
template <typename T, typename Tag>
inline typename result_of::end<T, Tag>::type end(T& t, Tag) {
  return detail::end_impl<T, Tag>::invoke(t);
}

template <typename T, typename Tag>
inline typename result_of::end<const T, Tag>::type end(const T& t, Tag) {
