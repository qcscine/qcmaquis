...
struct begin_impl<T, tag::value> {
  ...
  static result_type invoke(T& t) { return adaptor_access<T>::begin_value(t); }
};
...
//
template <typename T, typename Tag>
inline typename result_of::begin<T, Tag>::type begin(T& t, Tag) {
  return detail::begin_impl<T, Tag>::invoke(t);
}

template <typename T, typename Tag>
inline typename result_of::begin<const T, Tag>::type begin(const T& t, Tag) {
...
typename boost::enable_if<
    mpl::less<rank<T>, mpl::int_<2> >, typename result_of::begin<T>::type>::type
begin(T& t) {
