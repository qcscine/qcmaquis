...
BOOST_AUTO_TEST_CASE_TEMPLATE(constructors_test, T, test_types) {
  ...
  for (unsigned int i = 0; i < 10; ++i)
    for (unsigned int j = 0; j < 10; ++j) BOOST_CHECK_EQUAL(b(i, j), T());

  matrix<T> c(15, 5, 5);
  BOOST_CHECK_EQUAL(num_rows(c), 15u);
  BOOST_CHECK_EQUAL(num_cols(c), 5u);
  for (unsigned int i = 0; i < 15; ++i)
    for (unsigned int j = 0; j < 5; ++j) BOOST_CHECK_EQUAL(c(i, j), T(5));
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE(column_constructors_test, T, test_types) {
  ...
  std::vector<std::vector<T> > original(
      num_of_cols, std::vector<T>(num_of_rows)
  );
  T iota = 1;
  for (std::size_t i = 0; i < original.size(); ++i)
    iota =
        fill_range_with_numbers(original[i].begin(), original[i].end(), iota);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(row_iterator_test, T, test_types) {
  ...
  for (unsigned int i = 0; i < num_rows(a); ++i) {
    std::pair<
        typename matrix<T>::row_element_iterator,
        typename matrix<T>::row_element_iterator>
        range(row(a, i));
    unsigned int j = 0;
    for (typename matrix<T>::const_row_element_iterator it(range.first);
         it != range.second; ++it) {
...
BOOST_AUTO_TEST_CASE_TEMPLATE(col_iterator_test, T, test_types) {
  ...
  for (unsigned int j = 0; j < num_cols(a); ++j) {
    std::pair<
        typename matrix<T>::col_element_iterator,
        typename matrix<T>::col_element_iterator>
        range(col(a, j));
    unsigned int i = 0;
    for (typename matrix<T>::const_col_element_iterator it(range.first);
         it != range.second; ++it) {
      BOOST_CHECK_EQUAL(a(i, j), *it);
      ++i;
    }
...
BOOST_AUTO_TEST_CASE_TEMPLATE(diagonal_iterator_test, T, test_types) {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    std::pair<const_diagonal_iterator, const_diagonal_iterator> r_b =
        diagonal(b);
    BOOST_CHECK_EQUAL(
        distance(r_a.first, r_a.second),
        static_cast<std::ptrdiff_t>((std::min)(num_rows(a), num_cols(a)))
    );
    BOOST_CHECK_EQUAL(
        distance(r_b.first, r_b.second),
        static_cast<std::ptrdiff_t>((std::min)(num_rows(a), num_cols(a)))
    );
...
BOOST_AUTO_TEST_CASE_TEMPLATE(element_iterator_test, T, test_types) {
  ...
  matrix<T> b(10, 20);
  std::pair<
      typename matrix<T>::element_iterator,
      typename matrix<T>::element_iterator>
      range(elements(a));
...
BOOST_AUTO_TEST_CASE_TEMPLATE(resize_exception_test, T, test_types) {
  ...
  // Remains the matrix unchanged if an exception is thrown during the resize
  // process? Case 1: size1 > reserved_size1_
  matrix<T> ref(a);
  matrix<T> c(a);
  matrix<T> d(a);
  std::vector<T> test;
...
BOOST_AUTO_TEST_CASE_TEMPLATE(reserve_test, T, test_types) {
  ...
  BOOST_CHECK_EQUAL(a, ref);
  BOOST_CHECK_EQUAL(
      a.capacity().first >= 30 && a.capacity().second >= 30, true
  );
  ...
  BOOST_CHECK_EQUAL(a, ref);
  BOOST_CHECK_EQUAL(
      a.capacity().first >= 30 && a.capacity().second >= 40, true
  );
...
BOOST_AUTO_TEST_CASE_TEMPLATE(append_rows_test, T, test_types) {
  ...
  iota =
      fill_range_with_numbers(data_multiple.begin(), data_multiple.end(), iota);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(insert_rows_test, T, test_types) {
  ...
  insert_rows(
      a, initsize, std::make_pair(data_single.begin(), data_single.end())
  );
  ...
  insert_rows(
      a, 8, std::make_pair(data_multiple.begin(), data_multiple.end()), 3
  );
  for (unsigned int i = 0; i < num_rows(a); ++i)
    for (unsigned int j = 0; j < num_cols(a); ++j) {
      switch (i) {
...
BOOST_AUTO_TEST_CASE_TEMPLATE(matrix_vector_multiply_test, T, test_types) {
  ...
  for (unsigned int i = 0; i < num_rows(a); ++i) {
    T row_result(0);
    for (unsigned int j = 0; j < num_cols(a); ++j) row_result += a(i, j) * v(j);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(
    matrix_vector_multiply_mixed_types_test, TPair, test_type_pairs
) {
