...
int main(int argc, char** argv) {
  std::vector<double> A;
  std::transform(
      boost::counting_iterator<int>(0), boost::counting_iterator<int>(30),
      std::back_inserter(A), 0.1 * boost::lambda::_1
  );
  ...
  double conditional_sum_sq = alps::numeric::accumulate_if(
      A.begin(), A.end(), 0.0,
      boost::bind(
          std::plus<double>(), boost::lambda::_1,
          boost::bind(
  ...
  std::cout << "\nSum of the square of all elements in A if > 1. :\t"
            << conditional_sum_sq << "\n";
