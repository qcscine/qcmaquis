...
typedef boost::mpl::list<
    float, double, int, unsigned int, long unsigned int, std::complex<float>,
    std::complex<double> >
    test_types;
...
//
// List of type pairs <T,U> for which the mixed type matrix std::vector
// multiplication is tested.
//
typedef boost::mpl::list<
    type_pairs::IntDouble, type_pairs::DoubleInt, type_pairs::DoubleDComplex,
    type_pairs::DComplexDouble>
    test_type_pairs;
...
T fill_range_with_numbers(OutputIterator begin, OutputIterator end, T iota) {
  ...
  std::transform(
      begin, end, begin,
      boost::lambda::_1 = (boost::lambda::var(iota) += T(1)) - T(1)
  );
  return iota;
}
...
T fill_matrix_with_numbers(alps::numeric::matrix<T>& a, T iota = T(0)) {
  for (unsigned int i = 0; i < num_rows(a); ++i) {
    std::pair<
        typename alps::numeric::matrix<T>::row_element_iterator,
        typename alps::numeric::matrix<T>::row_element_iterator>
        range(row(a, i));
