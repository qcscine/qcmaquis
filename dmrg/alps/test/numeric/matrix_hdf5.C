...
BOOST_AUTO_TEST_CASE_TEMPLATE(hdf5, T, test_types) {
...

  {
    ...
    BOOST_CHECK_EQUAL(
        a.capacity() > b.capacity(), true
    );  // maybe this should be an assert instead

    {
      alps::hdf5::archive ar(
          filename, alps::hdf5::archive::WRITE | alps::hdf5::archive::REPLACE
      );
      ar["/matrix"] << a;
    }
...
BOOST_AUTO_TEST_CASE_TEMPLATE(hdf5_matrix_matrix, T, test_types) {
...

  {
    ...
    for (std::size_t j = 0; j < num_cols(a); ++j) {
      for (std::pair<col_iterator, col_iterator> r = col(a, j);
