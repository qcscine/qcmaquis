...
#define ValueWG 33
#define tuple1(z, n, unused) \
  BOOST_PP_COMMA_IF(n) size<(BOOST_PP_ADD(n, 1)), 11, double>
#define tuple2(z, n, unused) \
  BOOST_PP_COMMA_IF(n) size<(BOOST_PP_ADD(n, 1)), -11, double>
#define tuple3(z, n, unused) \
  BOOST_PP_COMMA_IF(n) size<(BOOST_PP_ADD(n, 1)), 11, std::complex<double> >
#define tuple4(z, n, unused) \
  BOOST_PP_COMMA_IF(n) size<(BOOST_PP_ADD(n, 1)), -11, std::complex<double> >

template <int n, int m, typename T>  // n # of workgroup, T double or
                                     // std::complex<double>
struct size {
  ...
  enum {
    valuex = n * ValueWG + m
  };  // n is the number or work group, m how we resize
  enum {
    valuey = n * ValueWG - m
  };  // n is the number or work group, m how we resize
};
...
typedef boost::mpl::list<
    size<4, 0, double>, size<4, 0, std::complex<double> >,
    BOOST_PP_REPEAT(4, tuple1, ~), BOOST_PP_REPEAT(4, tuple2, ~),
    BOOST_PP_REPEAT(4, tuple3, ~), BOOST_PP_REPEAT(4, tuple4, ~)>
    test_types;
// typedef boost::mpl::list< size<1,1,double>, size<1,-1,std::complex<double> >
// > test_types; //Dev line typedef boost::mpl::list< size<1,-2,double> >
// test_types; //Dev line
...
boost::variate_generator<
    boost::random::mt19937&, boost::random::uniform_real_distribution<> >
    uniDblGen(rng, dist);
...
struct ValidateHelper<std::complex<T> > {
  void static validate(
      matrix<std::complex<T> > const& M1, matrix<std::complex<T> > const& M2
  ) {
    BOOST_CHECK_EQUAL(num_rows(M1), num_rows(M2));
    BOOST_CHECK_EQUAL(num_cols(M1), num_cols(M2));
    for (std::size_t j(0); j < num_cols(M1); ++j)
...
BOOST_AUTO_TEST_CASE_TEMPLATE(trace_test, T, test_types) {
  ...
  for (std::size_t i = 0; i < num_rows(m); ++i) check += m(i, i);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(SVD_test, T, test_types) {
  typedef matrix<typename T::value_type> Matrix;
  typename associated_real_diagonal_matrix<
      matrix<typename T::value_type> >::type S;
...
// second test we check Q is an Id matrix, cautions we implemented the thin LQ
// so only Q*Qt is equal to one
...
// second test we check Q is an Id matrix, cautions we implemented the thin QR
// so only Qt*Q is equal to one
