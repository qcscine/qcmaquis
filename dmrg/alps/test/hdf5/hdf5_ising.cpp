 ...
 * ALPS Libraries *
 *                                                                                 *
 * Copyright (C) 2003 by Brigitte Surer and Jan Gukelberger * Copyright (C) 2010
...
class Simulation {
 public:
  Simulation(double beta, size_t L, std::string output_file)
    ...
    for (int E = -4; E <= 4; E += 2) exp_table_[E] = exp(2 * beta * E);

    // Init random spin configuration
    for (size_t i = 0; i < L; ++i) {
      for (size_t j = 0; j < L; ++j) spins_[i][j] = 2 * randint(2) - 1;
    }
  }
  ...
  void run(size_t ntherm, size_t n) {
    ...
    std::cout << energy_.name() << ":\t" << energy_.mean() << " +- "
              ...
              << ";\tconverged: "
              << alps::convergence_to_text(energy_.converged_errors())
              << std::endl;
    std::cout << magnetization_.name() << ":\t" << magnetization_.mean()
              << " +- " << magnetization_.error()
              << ";\ttau = " << magnetization_.tau() << ";\tconverged: "
              << alps::convergence_to_text(magnetization_.converged_errors())
              << std::endl;
  }
  void step() {
    for (size_t s = 0; s < L_ * L_; ++s) {
      ...
      if (e > 1 || rng_() < exp_table_[e]) spins_[i][j] = -spins_[i][j];
    }
  }
  void measure() {
    int E = 0;  // energy
  ...
  void save(std::string const& filename) {
    ...
    ar << alps::make_pvp(
        "/simulation/results/" + energy_.representation(), energy_
    );
    ar << alps::make_pvp(
        "/simulation/results/" + magnetization_.representation(), magnetization_
    );
    ar << alps::make_pvp(
        "/simulation/results/" + abs_magnetization_.representation(),
        abs_magnetization_
    );
 ...
 protected:
  ...
  int randint(int max) const { return static_cast<int>(max * rng_()); }
