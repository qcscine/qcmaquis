...
namespace detail {
void copy_data(
    alps::hdf5::archive &tar, alps::hdf5::archive &sar,
    std::string const &segment
) {
  if (false)
    ;
#define CHECK_TYPE(T)                                                          \
  else if (sar.is_datatype<T>(segment) && sar.is_null(segment)) tar[segment] = \
      std::vector<T>();                                                        \
...
#undef CHECK_TYPE
      ...
      "Unknown type in path: " + sar.complete_path(segment) + ALPS_STACKTRACE
  );
}
}  // namespace detail

void copy(
    alps::hdf5::archive &tar, std::string const &tpath,
    alps::hdf5::archive &sar, std::string const &spath
) {
  ...
  for (std::vector<std::string>::const_iterator it = children.begin();
    ...
    if (sar.is_group(*it))
      ...
      for (std::vector<std::string>::const_iterator jt = attributes.begin();
