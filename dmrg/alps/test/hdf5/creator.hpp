...
class custom_allocator : public std::allocator<T> {
 ...
 public:
  ...
  pointer allocate(size_type n) { return alloc_t::allocate(n); }

  void deallocate(pointer p, size_type n) { alloc_t::deallocate(p, n); }
  ...
  void construct(pointer p, const_reference val) { alloc_t::construct(p, val); }

  void destroy(pointer p) { alloc_t::destroy(p); }
};
...
template <typename T>
void initialize(std::complex<T> &v) {
  v = std::complex<T>(rng(), rng());
}
void initialize(std::string &v) { v = boost::lexical_cast<std::string>(rng()); }
template <typename T, typename A>
void initialize(std::vector<T, A> &v) {
  v = creator<std::vector<T, A> >::random();
}
template <typename A>
void initialize(std::vector<bool, A> &v) {
  v = creator<std::vector<bool, A> >::random();
}
template <typename T, std::size_t N>
void initialize(std::array<T, N> &v) {
  for (typename std::array<T, N>::iterator it = v.begin(); it != v.end(); ++it)
    initialize(*it);
}
template <
    int N, typename T0, typename T1, typename T2, typename T3, typename T4,
    typename T5, typename T6, typename T7, typename T8, typename T9>
void initialize_tuple_value(
    std::tuple<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9> &v, boost::false_type
) {
  using std::get;
  initialize(get<N>(v));
}
template <
    ...
    typename T5, typename T6, typename T7, typename T8, typename T9>
void initialize_tuple_value(
    std::tuple<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9> &v, boost::true_type
) {}
template <
...
template <typename T>
void initialize(userdefined_class<T> &v);

template <typename T, typename U>
void initialize(cast_type<T, U> &v);

template <typename T>
bool equal(T const &a, T const &b);

template <typename T>
bool equal(T *const &a, T *const &b, std::size_t size);
...
class userdefined_class {
 public:
  userdefined_class() : b(VECTOR_SIZE) {
    initialize(a);
    for (std::size_t i = 0; i < VECTOR_SIZE; ++i) initialize(b[i]);
    initialize(c);
  }
  void save(alps::hdf5::archive &ar) const {
    ar << alps::make_pvp("a", a) << alps::make_pvp("b", b)
       << alps::make_pvp("c", c);
  }
  ...
  bool operator==(userdefined_class<T> const &v) const {
    return a == v.a && b.size() == v.b.size() &&
           (b.size() == 0 || std::equal(b.begin(), b.end(), v.b.begin())) &&
           c == v.c;
  }
...
template <typename T, typename U>
class cast_type_base {
 public:
  cast_type_base(T const &v = T()) : has_u(false), t(v) {}
  void save(alps::hdf5::archive &ar) const { ar << alps::make_pvp("t", t); }
  void load(alps::hdf5::archive &ar) {
    has_u = true;
    ar >> alps::make_pvp("t", u);
  }
  bool has_u;
  T t;
  U u;
...
template <typename T, typename U>
class cast_type : public cast_type_base<T, U> {
 public:
  typedef cast_type_base<T, U> base_type;
  cast_type() : base_type() { initialize(base_type::t); }
 ...
 private:
  ...
  bool compare(cast_type<T, U> const &v, boost::mpl::false_) const {
    return (base_type::has_u ? base_type::u : alps::cast<U>(base_type::t)) ==
           (v.has_u ? v.u : alps::cast<U>(v.t));
  }
};
...
#define HDF5_DEFINE_VECTOR_VECTOR_CAST_TYPE(C, D)                           \
  template <typename T, typename U>                                         \
  class cast_type<C<T>, D<U> > : public cast_type_base<C<T>, D<U> > {       \
   public:                                                                  \
      ...
      if (base_type::has_u && !v.has_u)                                     \
        return base_type::u.size() == v.t.size() &&                         \
               (v.t.size() == 0 ||                                          \
                std::equal(                                                 \
                    &nonconstbase.u[0],                                     \
                    &nonconstbase.u[0] + nonconstbase.u.size(), &v.t[0]     \
                ));                                                         \
        ...
        return base_type::t.size() == v.u.size() &&                         \
               (v.u.size() == 0 ||                                          \
                std::equal(                                                 \
                    &nonconstbase.t[0],                                     \
                    &nonconstbase.t[0] + nonconstbase.t.size(), &v.u[0]     \
                ));                                                         \
      else                                                                  \
        return false;                                                       \
    }                                                                       \
...
HDF5_DEFINE_VECTOR_VECTOR_CAST_TYPE(
    std::valarray, boost::numeric::ublas::vector
)
HDF5_DEFINE_VECTOR_VECTOR_CAST_TYPE(
    boost::numeric::ublas::vector, std::valarray
)
...
class cast_type<
    ...
    : public cast_type_base<
          std::pair<T *, std::vector<std::size_t> >,
          std::vector<std::vector<std::vector<U> > > > {
 public:
  typedef cast_type_base<
      std::pair<T *, std::vector<std::size_t> >,
      std::vector<std::vector<std::vector<U> > > >
      base_type;
  cast_type()
  ...
  bool operator==(cast_type<
                  std::pair<T *, std::vector<std::size_t> >,
                  std::vector<std::vector<std::vector<U> > > > const &v) const {
      ...
      for (std::size_t i = 0; i < v.u.size(); ++i)
        for (std::size_t j = 0; j < v.u[i].size(); ++j)
          for (std::size_t k = 0; k < v.u[i][j].size(); ++k)
            if (base_type::t
                    .first[(i * v.u[i].size() + j) * v.u[i][j].size() + k] !=
                v.u[i][j][k])
              return false;
      return true;
    } else
...
void save(
    alps::hdf5::archive &ar, std::string const &path, enum_type const &value,
  ...
  switch (value) {
    case PLUS:
      ar << alps::make_pvp(path, std::string("plus"));
      break;
    case MINUS:
      ar << alps::make_pvp(path, std::string("minus"));
      break;
  }
}
void load(
    alps::hdf5::archive &ar, std::string const &path, enum_type &value,
    std::vector<std::size_t> chunk = std::vector<std::size_t>(),
    std::vector<std::size_t> offset = std::vector<std::size_t>()
) {
  std::string s;
...
template <>
struct scalar_type<enum_vec_type> {
  typedef boost::int_t<sizeof(enum_vec_type) * 8>::exact type;
};

template <>
struct is_continuous<enum_vec_type> : public boost::true_type {};

template <>
struct has_complex_elements<enum_vec_type> : public boost::false_type {};

namespace detail {

template <>
struct get_extent<enum_vec_type> {
...
template <>
struct set_extent<enum_vec_type> {
  static void apply(enum_vec_type &, std::vector<std::size_t> const &) {}
};

template <>
struct is_vectorizable<enum_vec_type> {
  static bool apply(enum_vec_type const &value) { return true; }
};

template <>
struct get_pointer<enum_vec_type> {
...
template <>
struct get_pointer<enum_vec_type const> {
  static scalar_type<enum_vec_type>::type const *apply(
...
void save(
    alps::hdf5::archive &ar, std::string const &path,
    enum_vec_type const &value,
    std::vector<std::size_t> size = std::vector<std::size_t>(),
    std::vector<std::size_t> chunk = std::vector<std::size_t>(),
    std::vector<std::size_t> offset = std::vector<std::size_t>()
  ...
  ar.write(
      path, (scalar_type<enum_vec_type>::type const *)get_pointer(value), size,
      chunk, offset
  );
}

void load(
    alps::hdf5::archive &ar, std::string const &path, enum_vec_type &value,
    std::vector<std::size_t> chunk = std::vector<std::size_t>(),
    std::vector<std::size_t> offset = std::vector<std::size_t>()
) {
  if (chunk.size() == 0) {
  ...
  ar.read(
      path, (scalar_type<enum_vec_type>::type *)get_pointer(value), chunk,
      offset
  );
}
...
struct creator {
  ...
  template <typename X>
  static base_type random(X const &) {
    return base_type();
  }
  template <typename X>
  static base_type empty(X const &) {
    return base_type();
  }
  template <typename X>
  static base_type special(X const &) {
    return base_type();
  }
};
template <typename T>
...
#define HDF5_DEFINE_MULTI_ARRAY_TYPE(P, C)                                   \
  template <typename T, typename A>                                          \
  struct creator<C<P ::multi_array<T, 1, A> > > {                            \
  ...
  template <typename T, typename A>                                          \
  struct creator<C<P ::multi_array<T, 2, A> > > {                            \
    typedef C<P ::multi_array<T, 2, A> > base_type;                          \
    static base_type random() {                                              \
      base_type value(                                                       \
          VECTOR_SIZE,                                                       \
          P ::multi_array<T, 2, A>(boost::extents[MATRIX_SIZE][MATRIX_SIZE]) \
      );                                                                     \
      for (std::size_t i = 0; i < VECTOR_SIZE; ++i)                          \
        for (std::size_t j = 0; j < MATRIX_SIZE; ++j)                        \
  ...
  template <typename T, typename A>                                          \
  struct creator<C<P ::multi_array<T, 3, A> > > {                            \
  ...
  template <typename T, typename A>                                          \
  struct creator<C<P ::multi_array<T, 4, A> > > {                            \
    typedef C<P ::multi_array<T, 4, A> > base_type;                          \
    static base_type random() {                                              \
      base_type value(                                                       \
          VECTOR_SIZE,                                                       \
          P ::multi_array<T, 4, A>(boost::extents[MATRIX_SIZE][MATRIX_SIZE]  \
...
#define HDF5_DEFINE_VECTOR_TYPE(C)                                         \
  ...
  struct creator<C<T> > {                                                  \
    ...
    static base_type random() {                                            \
      ...
      for (std::size_t i = 0; i < VECTOR_SIZE; ++i) initialize(value[i]);  \
      return value;                                                        \
    }                                                                      \
    static base_type empty() { return base_type(); }                       \
    static base_type special() { return base_type(); }                     \
  ...
  template <typename T>                                                    \
  bool equal(C<T> const &a, C<T> const &b) {                               \
    return a.size() == b.size() &&                                         \
           (a.size() == 0 || std::equal(                                   \
                                 &const_cast<C<T> &>(a)[0],                \
                                 &const_cast<C<T> &>(a)[0] + a.size(),     \
                                 &const_cast<C<T> &>(b)[0]                 \
                             ));                                           \
  }                                                                        \
  template <typename T, typename U>                                        \
  struct creator<C<std::pair<T, U> > > {                                   \
  ...
  template <typename T, typename U>                                        \
  struct destructor<C<std::pair<T, U> > > {                                \
  ...
  template <typename T, typename U>                                        \
  bool equal(C<std::pair<T, U> > const &a, C<std::pair<T, U> > const &b) { \
  ...
  template <typename T>                                                    \
  struct creator<C<alps::numeric::matrix<T> > > {                          \
...
template <>
struct creator<std::vector<bool> > {
...
template <>
bool equal(std::vector<bool> const &a, std::vector<bool> const &b) {
  return a.size() == b.size() &&
         (a.size() == 0 || std::equal(a.begin(), a.end(), b.begin()));
}

template <typename T, typename U>
struct creator<std::pair<T, U> > {
  ...
  template <typename X>
  static base_type random(X const &x) {
    return std::make_pair(creator<T>::random(x), creator<U>::random(x));
  }
  template <typename X>
  static base_type empty(X const &x) {
    return std::make_pair(creator<T>::empty(x), creator<U>::empty(x));
  }
  template <typename X>
  static base_type special(X const &x) {
    return std::make_pair(creator<T>::special(x), creator<U>::special(x));
  }
};
template <typename T, typename U>
bool equal(std::pair<T, U> const &a, std::pair<T, U> const &b) {
  return equal(a.first, b.first) && equal(a.second, b.second);
}

template <typename T, typename U>
struct creator<std::pair<T *, std::vector<U> > > {
  typedef std::pair<T *, std::vector<U> > base_type;
  static base_type random() {
    base_type value = std::make_pair(
        new typename boost::remove_const<T>::type[VECTOR_SIZE],
        std::vector<U>(1, VECTOR_SIZE)
    );
    for (std::size_t i = 0; i < VECTOR_SIZE; ++i)
      initialize(
          const_cast<typename boost::remove_const<T>::type &>(value.first[i])
      );
    return value;
  }
  static base_type empty() {
    return std::make_pair(
        static_cast<typename boost::remove_const<T>::type *>(NULL),
        std::vector<U>()
    );
  }
  static base_type special() {
    base_type value = std::make_pair(
        new typename boost::remove_const<
            T>::type[MATRIX_SIZE * MATRIX_SIZE * MATRIX_SIZE],
        std::vector<U>(3, MATRIX_SIZE)
    );
...
template <typename T, typename U>
struct destructor<std::pair<T *, std::vector<U> > > {
  static void apply(std::pair<T *, std::vector<U> > &value) {
    if (value.second.size()) delete[] value.first;
  }
};
template <typename T, typename U>
bool equal(
    std::pair<T *, std::vector<U> > const &a,
    std::pair<T *, std::vector<U> > const &b
) {
  if (a.second.size() == b.second.size() &&
      std::equal(a.second.begin(), a.second.end(), b.second.begin())) {
    for (std::size_t i = 0;
         a.second.size() &&
         i < std::accumulate(
                 a.second.begin(), a.second.end(), std::size_t(1),
                 std::multiplies<std::size_t>()
             );
         ++i)
      if (!equal(a.first[i], b.first[i])) return false;
    return true;
  } else
    return false;
}
...
#define HDF5_DEFINE_MULTI_ARRAY_TYPE(P)                                        \
  template <typename T, typename A>                                            \
  struct creator<P ::multi_array<T, 1, A> > {                                  \
    ...
    static base_type special() {                                               \
      return base_type(boost::extents[VECTOR_SIZE]);                           \
    }                                                                          \
    template <typename X>                                                      \
    static base_type random(X const &) {                                       \
      return base_type(boost::extents[VECTOR_SIZE]);                           \
    ...
    template <typename X>                                                      \
    static base_type empty(X const &) {                                        \
      return base_type(boost::extents[0]);                                     \
    }                                                                          \
    template <typename X>                                                      \
    static base_type special(X const &) {                                      \
      return base_type(boost::extents[VECTOR_SIZE]);                           \
  ...
  template <typename T, typename A>                                            \
  bool equal(                                                                  \
      P ::multi_array<T, 1, A> const &a, P ::multi_array<T, 1, A> const &b     \
  ) {                                                                          \
    if (!std::equal(                                                           \
            a.shape(), a.shape() + P ::multi_array<T, 1, A>::dimensionality,   \
            b.shape()                                                          \
        ))                                                                     \
      return false;                                                            \
    ...
    return true;                                                               \
                                                                               ...
                                                                               \
  template <typename T, typename A>                                            \
  struct creator<P ::multi_array<T, 2, A> > {                                  \
    ...
    static base_type special() {                                               \
      return base_type(boost::extents[MATRIX_SIZE][MATRIX_SIZE]);              \
    }                                                                          \
    template <typename X>                                                      \
    static base_type random(X const &) {                                       \
      return base_type(boost::extents[5][5]);                                  \
    }                                                                          \
    template <typename X>                                                      \
    static base_type empty(X const &) {                                        \
      return base_type(boost::extents[0][0]);                                  \
    }                                                                          \
    template <typename X>                                                      \
    static base_type special(X const &) {                                      \
      return base_type(boost::extents[MATRIX_SIZE][MATRIX_SIZE]);              \
  ...
  template <typename T, typename A>                                            \
  bool equal(                                                                  \
      P ::multi_array<T, 2, A> const &a, P ::multi_array<T, 2, A> const &b     \
  ) {                                                                          \
    if (!std::equal(                                                           \
            a.shape(), a.shape() + P ::multi_array<T, 2, A>::dimensionality,   \
            b.shape()                                                          \
        ))                                                                     \
      return false;                                                            \
    for (std::size_t i = 0; i < a.shape()[0]; ++i)                             \
    ...
    return true;                                                               \
                                                                               ...
                                                                               \
  template <typename T, typename A>                                            \
  struct creator<P ::multi_array<T, 3, A> > {                                  \
    ...
    static base_type special() {                                               \
      return base_type(boost::extents[MATRIX_SIZE][MATRIX_SIZE][MATRIX_SIZE]); \
    }                                                                          \
    template <typename X>                                                      \
    static base_type random(X const &) {                                       \
      return base_type(boost::extents[MATRIX_SIZE][MATRIX_SIZE][MATRIX_SIZE]); \
    ...
    template <typename X>                                                      \
    static base_type empty(X const &) {                                        \
      return base_type(boost::extents[0][0][0]);                               \
    }                                                                          \
    template <typename X>                                                      \
    static base_type special(X const &) {                                      \
      return base_type(boost::extents[MATRIX_SIZE][MATRIX_SIZE][MATRIX_SIZE]); \
  ...
  template <typename T, typename A>                                            \
  bool equal(                                                                  \
      P ::multi_array<T, 3, A> const &a, P ::multi_array<T, 3, A> const &b     \
  ) {                                                                          \
    if (!std::equal(                                                           \
            a.shape(), a.shape() + P ::multi_array<T, 3, A>::dimensionality,   \
            b.shape()                                                          \
        ))                                                                     \
      return false;                                                            \
    for (std::size_t i = 0; i < a.shape()[0]; ++i)                             \
    ...
    return true;                                                               \
                                                                               ...
                                                                               \
  template <typename T, typename A>                                            \
  struct creator<P ::multi_array<T, 4, A> > {                                  \
  ...
  template <typename T, typename A>                                            \
  bool equal(                                                                  \
      P ::multi_array<T, 4, A> const &a, P ::multi_array<T, 4, A> const &b     \
  ) {                                                                          \
    if (!std::equal(                                                           \
            a.shape(), a.shape() + P ::multi_array<T, 4, A>::dimensionality,   \
            b.shape()                                                          \
        ))                                                                     \
      return false;                                                            \
    for (std::size_t i = 0; i < a.shape()[0]; ++i)                             \
      for (std::size_t j = 0; j < a.shape()[1]; ++j)                           \
        for (std::size_t k = 0; k < a.shape()[2]; ++k)                         \
          for (std::size_t l = 0; l < a.shape()[2]; ++l)                       \
            if (!equal(a[i][j][k][l], b[i][j][k][l])) return false;            \
    return true;                                                               \
  }
...
struct creator<
  ...
  template <typename X>
  static base_type random(X const &) {
    return base_type(MATRIX_SIZE, MATRIX_SIZE);
  }
  template <typename X>
  static base_type empty(X const &) {
    return base_type(MATRIX_SIZE, MATRIX_SIZE);
...
template <typename T>
bool equal(
    boost::numeric::ublas::matrix<T, boost::numeric::ublas::column_major> const
        &a,
    boost::numeric::ublas::matrix<T, boost::numeric::ublas::column_major> const
        &b
) {
...
template <typename T>
struct creator<alps::numeric::matrix<T> > {
...
template <typename T>
bool equal(
    alps::numeric::matrix<T> const &a, alps::numeric::matrix<T> const &b
) {
  return a == b;
}
...
#define HDF5_DEFINE_VECTOR_VECTOR_TYPE(C, D)                        \
  ...
  struct creator<C<D<T> > > {                                       \
    ...
    static base_type special() {                                    \
      ...
      for (std::size_t i = 0; i < value.size(); ++i) {              \
        value[i].resize(                                            \
            1 + static_cast<std::size_t>(rng()) % (2 * MATRIX_SIZE) \
        );                                                          \
        for (std::size_t j = 0; j < value[i].size(); ++j)           \
          initialize(value[i][j]);                                  \
      }                                                             \
  ...
  template <typename T>                                             \
  bool equal(C<D<T> > const &a, C<D<T> > const &b) {                \
    for (std::size_t i = 0; i < a.size(); ++i)                      \
      if (a[i].size() != b[i].size() ||                             \
          (a[i].size() > 0 &&                                       \
           !equal(                                                  \
               &const_cast<C<D<T> > &>(a)[i][0],                    \
               &const_cast<C<D<T> > &>(b)[i][0], a[i].size()        \
           )))                                                      \
        return false;                                               \
    return true;                                                    \
  }
...
HDF5_DEFINE_VECTOR_VECTOR_TYPE(
    boost::numeric::ublas::vector, boost::numeric::ublas::vector
)
...
#define HDF5_DEFINE_VECTOR_VECTOR_VECTOR_TYPE(C, D, E)                         \
  ...
  struct creator<C<D<E<T> > > > {                                              \
    ...
    static base_type special() {                                               \
      ...
      for (std::size_t i = 0; i < value.size(); ++i) {                         \
        value[i] =                                                             \
            D<E<T> >(1 + static_cast<std::size_t>(rng()) % (2 * MATRIX_SIZE)); \
        for (std::size_t j = 0; j < value[i].size(); ++j) {                    \
          value[i][j] =                                                        \
              E<T>(1 + static_cast<std::size_t>(rng()) % (2 * MATRIX_SIZE));   \
          for (std::size_t k = 0; k < value[i][j].size(); ++k)                 \
            initialize(value[i][j][k]);                                        \
        }                                                                      \
      }                                                                        \
  ...
  template <typename T>                                                        \
  bool equal(C<D<E<T> > > const &a, C<D<E<T> > > const &b) {                   \
    for (std::size_t i = 0; i < a.size(); ++i)                                 \
      for (std::size_t j = 0; j < a[i].size(); ++j)                            \
        if (a[i][j].size() != b[i][j].size() ||                                \
            (a[i][j].size() > 0 &&                                             \
             !std::equal(                                                      \
                 &a[i][j][0], &a[i][j][0] + a[i][j].size(), &b[i][j][0]        \
             )))                                                               \
          return false;                                                        \
    return true;                                                               \
  }
