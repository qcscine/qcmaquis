...
int main() {
  if (boost::filesystem::exists("real_complex_matrix.h5") &&
      boost::filesystem::is_regular_file("real_complex_matrix.h5"))
  ...
  try {
    ...
    for (int i = 0; passed && i < size; ++i)
      for (int j = 0; passed && j < size; ++j) passed = (A(i, j) == B(i, j));
