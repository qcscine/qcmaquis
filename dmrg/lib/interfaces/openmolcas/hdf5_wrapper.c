...
#define id_tostring(type, c) if (type == c) return #c
...
herr_t h5dread_c(hid_t dset_id, hid_t h5type, void *buf)
{
  ...
  switch(h5type)
  {
    ...
    default:
      // not supported! TODO: handle this
      native_h5type = 0;
      fprintf(stderr, "Error in h5dread_c wrapper: type %s not supported yet!\n", check_h5type(h5type));
      break;
  }
...
herr_t hdf5_get_data_c(hid_t fileid, hid_t h5type, const char *dname, int rank, const hsize_t* dims, void *buf)
{
  ...
  switch(h5type)
  {
    ...
    case MYH5T_DOUBLE:
      ...
      break;
    case MYH5T_INT32:
      native_h5type = H5T_STD_I32LE;// warning -- maybe a mismatch since we used H5T_IEEE_F64LE to write
      break;
    case MYH5T_STRING: // Variable length strings
      native_h5type = H5Tcopy(H5T_C_S1);
      error = H5Tset_size(native_h5type, H5T_VARIABLE);
      if (error < 0)
        fprintf(stderr, "Read error %i in reading variable-length string.\n", error);
        // error handling?
      break;
    default:
      // not supported! TODO: handle this
  ...
  if (h5type == MYH5T_STRING)
  {
      ...
      free(strbuf);
      if (error < 0)
          fprintf(stderr, "Error in H5Dvlen_reclaim in reading variable-length string.\n");
  }
  else
  {
