 ...
 * This interface replaces the previous Fortran module-based HDF5 F2003
 * interface and does not require the compilation of the HDF5 library with the
 * same compiler version as the QCMaquis driver/NEVPT2.
 *
 * Fortran-C interoperability inspired by
 * http://fortranwiki.org/fortran/show/Generating+C+Interfaces and Steven
 * Vancoillie's hdf5_util in OpenMOLCAS (C) 2017 Leon Freitag and Stefan Knecht
...
#define MYH5T_STRING 3
#define MYH5T_INT32 4
...
#define id_tostring(type, c) \
  if (type == c) return #c
const char *check_h5type(hid_t h5type) {
  id_tostring(h5type, H5T_IEEE_F64LE);
  id_tostring(h5type, H5T_NATIVE_DOUBLE);
  id_tostring(h5type, H5T_NATIVE_LONG);
  id_tostring(h5type, H5T_STD_I64LE);
  id_tostring(h5type, H5T_C_S1);
  id_tostring(h5type, H5T_FORTRAN_S1);
  id_tostring(h5type, H5T_STRING);
  id_tostring(h5type, H5T_STD_I32LE);
...
herr_t hdf5_close_c(hid_t file_id) { return H5Fclose(file_id); }
...
hid_t h5dcreate_c(
    hid_t myfile_id, const char *dname, hid_t h5type, hid_t space_id
) {
  hid_t native_h5type;
  switch (h5type) {
    case MYH5T_INT:
    ...
    default:
      ...
      native_h5type = 0;
      fprintf(
          stderr, "Error in h5dcreate_c wrapper: type not supported yet!\n"
      );
      break;
  }
  return H5Dcreate(
      myfile_id, dname, native_h5type, space_id, H5P_DEFAULT, H5P_DEFAULT,
      H5P_DEFAULT
  );
}
...
herr_t h5dread_c(hid_t dset_id, hid_t h5type, void *buf) {
  ...
  switch (h5type) {
    ...
    default:
      ...
      native_h5type = 0;
      fprintf(
          stderr, "Error in h5dread_c wrapper: type %s not supported yet!\n",
          check_h5type(h5type)
      );
      break;
  }
  error = H5Dread(dset_id, native_h5type, H5S_ALL, H5S_ALL, H5P_DEFAULT, buf);
  return error;
}

herr_t hdf5_write_cholesky_c(
    hid_t dset_id, hid_t space_id, const hsize_t *start, const hsize_t *count,
    void *buf
) {
  ...
  hsize_t c_start[rank],
      c_count[rank];  // Does this work? one should probably use malloc
  hid_t memspace_id;
  herr_t error;
  transpose_f2c(rank, start, c_start);
  transpose_f2c(rank, count, c_count);
  error = H5Sselect_hyperslab(
      space_id, H5S_SELECT_SET, c_start, NULL, c_count, NULL
  );
  if (error < 0) {
    fprintf(
        stderr, "hdf5_write_cholesky_c: Error %i in H5Sselect_hyperslab\n",
        error
    );
    // error handling
  }
  ...
  error = H5Dwrite(
      dset_id, H5T_IEEE_F64LE, memspace_id, space_id, H5P_DEFAULT, buf
  );
  if (error < 0) {
...
herr_t hdf5_put_data_c(
    hid_t fileid, hid_t h5type, const char *dname, int rank,
    const hsize_t *dims, void *buf
) {
  herr_t error;
  hid_t dspace, dset;
  ...
  dset = H5Dcreate(
      fileid, dname, native_h5type, dspace, H5P_DEFAULT, H5P_DEFAULT,
      H5P_DEFAULT
  );
...
herr_t hdf5_get_data_c(
  ...
  switch (h5type) {
    case MYH5T_INT:
      native_h5type = H5T_READ_INT;  // warning -- maybe a mismatch since we
                                     // used H5T_STD_I64LE to write
      break;
    case MYH5T_DOUBLE:
      native_h5type = H5T_READ_DOUBLE;  // warning -- maybe a mismatch since we
                                        // used H5T_IEEE_F64LE to write
      break;
    case MYH5T_INT32:
      native_h5type = H5T_STD_I32LE;  // warning -- maybe a mismatch since we
                                      // used H5T_IEEE_F64LE to write
      break;
    case MYH5T_STRING:  // Variable length strings
      native_h5type = H5Tcopy(H5T_C_S1);
      error = H5Tset_size(native_h5type, H5T_VARIABLE);
      if (error < 0)
        fprintf(
            stderr, "Read error %i in reading variable-length string.\n", error
        );
      // error handling?
      break;
    default:
  ...
  if (!H5Tequal(read_h5type, native_h5type))
    fprintf(
        stderr,
        "Warning in hdf5_get_data_c: type mismatch when reading dataset %s: %s "
        "instead of %s\n",
        dname, check_h5type(read_h5type), check_h5type(native_h5type)
    );

  // Variable-length string must be read as a string array with size of 1
  // so we need to allocate an additional buffer
  if (h5type == MYH5T_STRING) {
    dspace = H5Dget_space(dset);
    char **strbuf = (char **)malloc(sizeof(char *));

    error = H5Dread(dset, read_h5type, H5S_ALL, H5S_ALL, H5P_DEFAULT, strbuf);
    if (error < 0)
      fprintf(
          stderr,
          "hdf5_put_data_c: Error %i in H5Dread when reading dataset %s\n",
          error, dname
      );
    ...
    if (error < 0)
      fprintf(
          stderr,
          "Error in H5Dvlen_reclaim in reading variable-length string.\n"
      );
  } else {
