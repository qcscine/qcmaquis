...
! internal functions, but exported because nevpt2 uses them to read rdms from HDF5
  public h5dopen
  public h5dget_space
  public h5sget_simple_extent_dims
  public h5sclose
  public h5dclose
! for some reason, hdf5_get_data generic does not work for strings,
! so we need to call the string overload explicitly
  public hdf5_get_data_str
...
! export the data types used by hdf5 since we're no longer including the HDF5 module
  ...
  interface hdf5_get_data
   module procedure  hdf5_get_data_dp,  hdf5_get_data_ix, hdf5_get_data_ixx, &
      hdf5_get_data_int32xx, hdf5_get_data_dpscalar, hdf5_get_data_iscalar, hdf5_get_data_str
...
! The code below should map the correct Fortran types to C types
  ...
  INTEGER, PARAMETER :: H5S_seloper_t = c_int
  ...
  INTEGER(HID_T), PARAMETER :: MYH5T_STRING = 3;
  INTEGER(HID_T), PARAMETER :: MYH5T_INT32 = 4;
...
!       ! unused because we don't use chunking right now
!       ! call C function h5sselect_hyperslab with stride and block set to NULL
!       subroutine h5sselect_hyperslab(space_id, start, count, err)
!         interface
!           function h5sselect_hyperslab_c(space_id, op, start, stride, count, blk) result(err) bind(C,name='H5Sselect_hyperslab')
!             import herr_t, hid_t, H5S_seloper_t
!             INTEGER(HID_T), value :: space_id
!             INTEGER(H5S_seloper_t), value :: op
!             INTEGER(HSIZE_T) :: start(*), stride(*), count(*), blk(*)
!             INTEGER(HERR_T) :: err
!           end function
!         end interface
!         import H5S_SELECT_SET,herr_t, hid_t, H5S_seloper_t, c_null_ptr
!         INTEGER(HID_T), value :: space_id
!         INTEGER(H5S_seloper_t), value :: op
!         INTEGER(HSIZE_T) :: start(*), stride(*), count(*), blk(*)
!         INTEGER(HERR_T) :: err
!         err = h5sselect_hyperslab_c(space_id, H5S_SELECT_SET, start, c_null_ptr, count, c_null_ptr)
!       end subroutine
...
!------------------------------------------------------------------------
       integer(c_int)                                  :: data_rank
       character(len=:), allocatable                     :: mydata
!------------------------------------------------------------------------
...
!------------------------------------------------------------------------
      ...
      ! for 2D integer arrays
      subroutine hdf5_get_data_ixx(myfile_id, mydata, dims, data_out)
...
!------------------------------------------------------------------------
      ...
      end subroutine hdf5_get_data_ixx

      ! for 2D 32-bit integer arrays
      subroutine hdf5_get_data_int32xx(myfile_id, mydata, dims, data_out)

       integer(HID_T)  , intent(in)                       :: myfile_id
       character(len=*), intent(in)                       :: mydata
       INTEGER(HSIZE_T), DIMENSION(*), intent(in)         :: dims
       integer*4     , DIMENSION(:,:), intent(out),target :: data_out
!------------------------------------------------------------------------
       integer(c_int)                                     :: data_rank
!------------------------------------------------------------------------
       ...
       f_ptr = C_LOC(data_out)
       error = hdf5_get_data_c(myfile_id, MYH5T_INT32, trim(mydata)//C_NULL_CHAR, data_rank, dims, f_ptr)

      end subroutine hdf5_get_data_int32xx

      ! for variable-length strings
      ! NB: here dims and data_rank are irrelevant
      subroutine hdf5_get_data_str(myfile_id, mydata, dims, data_out)
       integer(HID_T)  , intent(in)                       :: myfile_id
       character(len=*), intent(in)                       :: mydata
       INTEGER(HSIZE_T), DIMENSION(*), intent(in)         :: dims
...
!------------------------------------------------------------------------
       ...
       f_ptr = C_LOC(data_out)
       error = hdf5_get_data_c(myfile_id, MYH5T_STRING, trim(mydata)//C_NULL_CHAR, data_rank, dims, f_ptr)

!      handling C null char is seemingly not needed?! If there are bugs maybe this is where to look for them

      end subroutine hdf5_get_data_str
!------------------------------------------------------------------------
