...
bool is_square(I x) {
  ...
  // if we use floating point, then the return of the sqrt should be of the same
  // type
  typedef typename std::conditional<
      std::is_floating_point<I>::value, I, double>::type T;
...
struct MPSSIInterface<V>::Impl {
  ...
  // Overlap calculation
  V overlap_2u1(
      const std::string& bra_checkpoint, const std::string& ket_checkpoint
  ) {
  ...
  std::string twou1_name(std::string pname, int state, int Ms, bool rotated) {
    ...
    return maquis::interface_detail::twou1_name(
        pname, state, nel_, multiplicities_[idx], Ms
    );
  }

  // MPS rotation
  void rotate(
      const std::string& pname, int state, const std::vector<V>& t,
      V scale_inactive, int Ms
  ) {
#if defined(HAVE_SU2U1PG)
    typedef TwoU1PG grp;
#elif defined(HAVE_SU2U1)
    ...
    if ((twou1_pos == std::string::npos) || (twou1_pos > 0)) {
      throw std::runtime_error(
          "checkpoint for MPS rotation does not have 2U1 symmetry"
      );
    }
    ...
    std::filesystem::copy(
        checkpoint_name + "/props.h5", checkpoint_name_rotated + "/props.h5"
    );
  }
  ...
  Impl(
    ...
    for (int i = 0; i < project_names.size(); i++) {
      // Load the first state of each project group and get its number of
      // electrons and spin. We will not check if the subsequent states have the
      // same spin for now, but if you want to implement if for better error
      // safety, feel free.
      const std::string& pname = project_names[i];
      assert(
          states[i].size() > 0
      );  // make sure we do not have empty state containers
      int state = states[i][0];

      std::string su2u1_checkpoint_name =
          maquis::interface_detail::su2u1_name(pname, state);
      BaseParameters parms;
      if (!std::filesystem::exists(su2u1_checkpoint_name)) {
        throw std::runtime_error(
            "SU2U1 MPS checkpoint " + su2u1_checkpoint_name +
            " is required but does not exist\n"
            "You might be getting this error because qcm_checkpoint_rename.py "
      ...
      ar_in["/parameters"] >>
          parms;  // TODO: check if /parameters/nelec and /parameters/spin can
                  // be read directly and if it saves time

      int nel = parms["nelec"];
      if ((nel_ != -1) && (nel_ != nel)) {
        throw std::runtime_error(
            "Different electron numbers in different project groups: This is "
            "not supported in MPSSI yet."
        );
      }
      nel_ = nel;
      ...
      for (auto&& st : states[i]) {
        ...
        // For each spin multiplicity, we need to have the Ms=S and Ms=min(S,
        // otherS) for all other S so we create a list with these Ms values
  ...
  int get_multiplicity(const std::string& pname) {
    ...
    auto index_itr = std::find_if(
        project_names_.cbegin(), project_names_.cend(),
        [&pname](const std::string& key) -> bool { return pname == key; }
    );
    if (index_itr == project_names_.cend()) {
      throw std::runtime_error(
          "cannot find multiplicity for project name " + pname
      );
    }
  ...
  // (i.e. the project prefix and the state is present in the vectors with which
  // the class has been initialised) If project name or state is not found,
  // returns -1 otherwise returns the index of the project name (we don't care
  // about the state index for now)
  ...
  int allowed_names_states(const std::string& pname, int state) {
    ...
    auto index_st_itr = std::find_if(
        states_[idx].cbegin(), states_[idx].cend(),
        [&state](const int key) -> bool { return state == key; }
    );
 ...
 private:
  // State indexes for each project. E.g. if states_[0] is {0,3}, we are
  // interested in states 0 and 3 of the first project Each project has a
  // different name, a typical use would be to use one project name for each
  // multiplicity or more general, results of a single DMRGSCF calculation
  ...
  // Number of electrons, for now only one number of electrons supported for all
  // projects. (i.e. no Dyson orbitals)
  int nel_;
};

template <class V>
MPSSIInterface<V>::MPSSIInterface(
    const std::vector<std::string>& project_names,
    const std::vector<std::vector<int> >& states
)
...
template <class V>
std::string MPSSIInterface<V>::twou1_name(
    const std::string& pname, int state, int Ms, bool rotated
) {
...
/*
template <class V>
meas_with_results_type<V> MPSSIInterface<V>::onetdm(const std::string&
bra_pname, int bra_state, const std::string& ket_pname, int ket_state)
{
...
template <class V>
std::vector<meas_with_results_type<V> > MPSSIInterface<V>::onetdm_spin(
    const std::string& bra_pname, int bra_state, const std::string& ket_pname,
    int ket_state
) {
  typedef alps::numeric::matrix<V> Matrix;
  DmrgParameters parms;
...
template <class V>
V MPSSIInterface<V>::overlap(
    const std::string& bra_pname, int bra_state, const std::string& ket_pname,
    int ket_state, bool su2u1
) {
  if ((bra_pname == ket_pname) && su2u1) {
    ...
    std::string ket_name =
        maquis::interface_detail::su2u1_name(ket_pname, ket_state);
    std::string bra_name =
        maquis::interface_detail::su2u1_name(bra_pname, bra_state);
    return impl_->overlap_su2u1(ket_name, bra_name);
  } else {
    if ((bra_state == ket_state) && (bra_pname == ket_pname)) {
      return (V)1.0;
    }
    bool rotated = (bra_pname != ket_pname);
