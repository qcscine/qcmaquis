...
public:
  ...
  void set(const index_type<HamiltonianType, Transcorrelation> &key, const V value) {
  ...
  const V &operator[](const index_type<HamiltonianType, Transcorrelation> &key) const {
    return map_[maquis::detail::AlignTraitTypeClass<V>::align(key)];
  }
...
template <class V, Hamiltonian HamiltonianType = Hamiltonian::Electronic, HamiltonianTransformation Transcorrelated = HamiltonianTransformation::Conventional>
// template <class V, Hamiltonian HamiltonianType = Hamiltonian::Electronic>
std::string serialize(const integral_map<V, HamiltonianType, Transcorrelated> &ints) {
    std::stringstream ss;
