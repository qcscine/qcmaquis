...
void qcmaquis_interface_preinit(
    int nel_, int L_, int irrep_, int* site_types_, double conv_thresh_, int m_,
    bool meas_2rdm_, bool entropy_, bool magnetism_,
    const std::string& init_type_, const std::string& optimization_,
    int ietl_jcd_maxiter_, double ietl_jcd_tol_, double truncation_initial_,
...

{
  ...
  if (entropy_) cpp_parms.set("MEASURE[ChemEntropy]", 1);
...
void qcmaquis_interface_update_integrals(
    std::vector<std::vector<int>> integral_indices,
    std::vector<V> integral_values, int integral_size
) {
  ...
  for (auto&& t: integrals){
      std::cout << "element " << t.first[0] << t.first[1] << t.first[2] <<
  t.first[3] << " is " << t.second << std::endl;
  }
  */
...
void qcmaquis_interface_set_nsweeps(
...
void qcmaquis_interface_optimize() { cpp_interface_ptr->optimize(); }
...
{
  const typename maquis::DMRGInterface<V>::meas_with_results_type& meas =
      cpp_interface_ptr->onerdm();
...
void qcmaquis_interface_get_1rdm(V* rdm1)  // CQ
{
  ...
  // initialize spinor reordering index vector (first unbarred than the barred
  // partner)
  ...
  for (int i = 0; i < meas.first.size(); i++) {
    // std::cout << " 1-rdm from meas ... element " <<  meas.first[i][0]<<" " <<
    // meas.first[i][1] << " is --> " << meas.second[i] << std::endl;
    rdm1
        [revSpinorOrder[meas.first[i][0]] +
         revSpinorOrder[meas.first[i][1]] * L_] = meas.second[i];
    if (meas.first[i][0] < meas.first[i][1])
      rdm1
          [revSpinorOrder[meas.first[i][1]] +
           revSpinorOrder[meas.first[i][0]] * L_] = std::conj(meas.second[i]);
  }
}
...
void qcmaquis_interface_get_2rdm(V* rdm2) {
  ...
  for (int i = 0; i < meas.first.size(); i++) {
    ...
    // permutation           m                         n o p
    rdm2
        [revSpinorOrder[pos_[0]] + revSpinorOrder[pos_[1]] * L_ +
         revSpinorOrder[pos_[2]] * L2_ + revSpinorOrder[pos_[3]] * L3_] =
            meas.second[i];
    // permutation           o                         p m n
    rdm2
        [revSpinorOrder[pos_[2]] + revSpinorOrder[pos_[3]] * L_ +
         revSpinorOrder[pos_[0]] * L2_ + revSpinorOrder[pos_[1]] * L3_] =
            meas.second[i];
    // permutation           m                         p o n
    rdm2
        [revSpinorOrder[pos_[0]] + revSpinorOrder[pos_[3]] * L_ +
         revSpinorOrder[pos_[2]] * L2_ + revSpinorOrder[pos_[1]] * L3_] =
            -meas.second[i];
    // permutation           o                         n m p
    rdm2
        [revSpinorOrder[pos_[2]] + revSpinorOrder[pos_[1]] * L_ +
         revSpinorOrder[pos_[0]] * L2_ + revSpinorOrder[pos_[3]] * L3_] =
            -meas.second[i];
  }
}
