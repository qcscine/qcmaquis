...
std::string su2u1_name(const std::string& pname, int state) {
  // TODO: should we check if pname and state are allowed here too with
  // allowed_names_states()? if (allowed_names_states(pname, state) == -1)
  //     throw std::runtime_error("Do not know the project name" + pname );

  std::string ret =
      pname + ".checkpoint_state." + std::to_string(state) + ".h5";
  return ret;
}

// Generate names for 2U1 checkpoint files (helper)
std::tuple<std::string, int, int> twou1_name_Nup_Ndown(
    const std::string& pname, int state, int nel, int multiplicity, int Ms
) {
  int Nup = (nel + Ms) / 2;
  int Ndown = (nel - Ms) / 2;
...
// Mostly copy-paste from mps_transform.cpp, but creates only one 2U1 checkpoint
// per state corresponding to the state with the highest Sz
...
void transform(const std::string& pname, int state, int Ms) {
  ...
  if (!std::filesystem::exists(checkpoint_name)) {
    throw std::runtime_error(
        "input MPS " + checkpoint_name + " does not exist\n"
    );
  }
  ...
  // get number of up/down electrons and the checkpoint name for the 2U1
  // checkpoint
  std::tie(twou1_checkpoint_name, Nup, Ndown) =
      maquis::interface_detail::twou1_name_Nup_Ndown(
          pname, state, nel, multiplicity, Ms
      );
  ...
  // transform MPS
  MPS<matrix, TwoU1grp> mps_out =
      transform_mps<matrix, SU2U1grp>()(mps, Nup, Ndown);
  ...
  std::filesystem::copy(
      checkpoint_name + "/props.h5", twou1_checkpoint_name + "/props.h5"
  );
