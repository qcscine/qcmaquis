...
def plotMutualInformation(mutualInformation, oneModalEntropy, numModals, L,
    ...
    #         Iij = mat_I[i, j]
    #         if Iij >= 0.1:
    #             line = lines.Line2D(x, y, linewidth=8*Iij, color='black', linestyle='-', alpha=1, label='0.1')
    #             legendlines['0.1'] = line
    #             ax.add_line(line)
    #         elif Iij >=0.01:
    #             line = lines.Line2D(x, y, linewidth=5*Iij, color='gray', linestyle='-', alpha=1, label='0.01')
...
def plotEntanglementDiagram(inputFileName: str):
  ...
  for i in range(9):
    lst = []
    for j in range(19):
      lst.append(oneModalEntropy[jcont])
      jcont = jcont+1
    print(lst)
...
if __name__ == "__main__":
  ...
  parser.add_argument("-d", "--diagram", action="store_true", help="If present, plots the entanglement diagram")
