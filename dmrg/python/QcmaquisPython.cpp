...
PYBIND11_MODULE(_dmrg, module) {
  ...
  py::class_<
      chem::integral_map<std::complex<double>, chem::Hamiltonian::Electronic,
                         chem::HamiltonianTransformation::Transcorrelated>>(
      module, "ComplexTCIntegralMap")
      .def(py::init<>())
      .def("set", &chem::integral_map<
                      std::complex<double>, Hamiltonian::Electronic,
                      chem::HamiltonianTransformation::Transcorrelated>::set)
      .def("at", [](chem::integral_map<
                        std::complex<double>, Hamiltonian::Electronic,
                        chem::HamiltonianTransformation::Transcorrelated> &blub,
                    const std::array<int, 6> &blah,
                    double &hihi) { blub.at(blah) = hihi; })
      .def("at", [](const chem::integral_map<
                        std::complex<double>, Hamiltonian::Electronic,
                        chem::HamiltonianTransformation::Transcorrelated> &blub,
      ...
      .def("__getitem__",
           [](chem::integral_map<
                  std::complex<double>, Hamiltonian::Electronic,
                  chem::HamiltonianTransformation::Transcorrelated> &blub,
              const std::array<int, 6> &blah) { return &blub[blah]; })
      .def("__setitem__",
           [](chem::integral_map<
                  std::complex<double>, Hamiltonian::Electronic,
                  chem::HamiltonianTransformation::Transcorrelated> &blub,
  ...
  py::class_<
      chem::integral_map<double, chem::Hamiltonian::Electronic,
                         chem::HamiltonianTransformation::Transcorrelated>>(
      module, "TCIntegralMap")
      .def(py::init<>())
      .def("set", &chem::integral_map<
                      double, Hamiltonian::Electronic,
                      chem::HamiltonianTransformation::Transcorrelated>::set)
      .def("at", [](chem::integral_map<
                        double, Hamiltonian::Electronic,
                        chem::HamiltonianTransformation::Transcorrelated> &blub,
  ...
  py::class_<chem::integral_map<double>>(module, "IntegralMap")
      ...
      .def("at",
           [](chem::integral_map<double> &blub, const std::array<int, 4> &blah,
              double &hihi) { blub.at(blah) = hihi; })
      .def("at", [](const chem::integral_map<double> &blub,
                    const std::array<int, 4> &blah) { return &blub.at(blah); })
      .def("__getitem__",
           [](chem::integral_map<double> &blub,
              const std::array<int, 4> &blah) { return &blub[blah]; })
      .def("__setitem__",
           [](chem::integral_map<double> &blub,
              const std::array<int, 4> &blah) { return &blub[blah]; });
  ...
  py::class_<SRCAS<double>>(module, "SRCASReal")
      // .def(py::init<DmrgParameters &, maquis::DMRGInterface<double> &>())
      .def(py::init<DmrgParameters &,
                    std::shared_ptr<maquis::DMRGInterface<double>>>())
      .def("run", &SRCAS<double>::run)
      .def("printSRCASSettings", &SRCAS<double>::printSRCASSettings)
      .def("printResults", &SRCAS<double>::printResults)
      .def("getCurrentQueen", &SRCAS<double>::getCurrentQueen)
      .def("getDetTable", &SRCAS<double>::getDetTable)
      .def("getCompleteness", &SRCAS<double>::getCompleteness);

  py::class_<SRCAS<std::complex<double>>>(module, "SRCASComplex")
      .def(py::init<
           DmrgParameters &,
           std::shared_ptr<maquis::DMRGInterface<std::complex<double>>> &>())
      .def("run", &SRCAS<std::complex<double>>::run)
      .def("printSRCASSettings",
           &SRCAS<std::complex<double>>::printSRCASSettings)
      .def("printResults", &SRCAS<std::complex<double>>::printResults)
      .def("getCurrentQueen", &SRCAS<std::complex<double>>::getCurrentQueen)
      .def("getDetTable", &SRCAS<std::complex<double>>::getDetTable)
      .def("getCompleteness", &SRCAS<std::complex<double>>::getCompleteness);

  // DmrgParameters
  py::class_<DmrgParameters, BaseParameters>(module, "DmrgParameters")
      .def(py::init<>());

  // TODO: make sure the smart ptr is not always copying everything (checkout
  // std::enable_shared_from_this<interface>) Dmrg real
  py::class_<maquis::DMRGInterface<double>,
      ...
      .def("measure_and_save_trans3rdm",
  ...
  // Dmrg complex
  py::class_<maquis::DMRGInterface<std::complex<double>>,
             std::shared_ptr<maquis::DMRGInterface<std::complex<double>>>>(
      module, "DmrgComplex")
      .def(py::init<DmrgParameters &>())
      ...
      .def("runInversePowerIteration",
           &maquis::DMRGInterface<
               std::complex<double>>::runInversePowerIteration)
