...
struct SiteProblem {
  SiteProblem(
      ...
      : left(left_), right(right_), mpo(mpo_) {}
...
void split_ts(
    TwoSiteTensor<Matrix, SymmGroup> const& tst,
    MPSTensor<Matrix, SymmGroup>& mps1, MPSTensor<Matrix, SymmGroup>& mps2,
    block_matrix<DiagMatrix, SymmGroup>& inv, double cutoff, std::size_t Mmax
) {
  tst.make_both_paired();

  typedef
      typename alps::numeric::associated_real_diagonal_matrix<Matrix>::type dmt;
  block_matrix<Matrix, SymmGroup> u, v, m1, m2;
  block_matrix<dmt, SymmGroup> s;

  truncation_results trunc =
      svd_truncate(tst.data(), u, v, s, cutoff, Mmax, true);

  gemm(u, s, m1);
  gemm(s, v, m2);

  mps1 = MPSTensor<Matrix, SymmGroup>(
      tst.local_site_dim(0), tst.row_dim(), m1.right_basis(), m1, LeftPaired
  );
  mps2 = MPSTensor<Matrix, SymmGroup>(
      tst.local_site_dim(1), m2.left_basis(), tst.col_dim(), m2, RightPaired
  );

  inv = s;
  typename dmt::diagonal_iterator it, end;
  for (size_t k = 0; k < inv.n_blocks(); ++k)
    for (std::tie(it, end) = inv[k].diagonal(); it != end; ++it) *it = 1. / *it;
}

template <class Matrix, class DiagMatrix, class SymmGroup>
void central_optim(
    unsigned site, block_matrix<DiagMatrix, SymmGroup>& inv,
  ...
  MPOTensor<Matrix, SymmGroup> ts_mpo = make_twosite_mpo<Matrix, Matrix>(
      mpo[site], mpo[site + 1], mps[site].site_dim(), mps[site + 1].site_dim()
  );
  ...
  /// Move boundaries
  {
    ...
    newleft = contraction::Engine<matrix, matrix, grp>::overlap_mpo_left_step(
        t, t, left, mpo[site]
    );
  }
  {
    ...
    newright = contraction::Engine<matrix, matrix, grp>::overlap_mpo_right_step(
        t, t, right, mpo[site + 1]
    );
  }
}
...
template <class Matrix, class SymmGroup>
void dmrg_optim(
    unsigned site, unsigned local_site, int lr, int L,
    MPS<Matrix, SymmGroup>& mps, MPO<Matrix, SymmGroup> const& mpo,
    std::vector<Boundary<Matrix, SymmGroup> >& left,
    std::vector<Boundary<Matrix, SymmGroup> >& right, double cutoff,
    std::size_t Mmax, BaseParameters& parms, optim_normalize_t normalize
) {
  /// Create TwoSite objects
  TwoSiteTensor<Matrix, SymmGroup> tst(mps[site], mps[site + 1]);
  MPSTensor<Matrix, SymmGroup> ts_mps = tst.make_mps();
  ...
  SiteProblem<Matrix, SymmGroup> sp(
      left[local_site], right[local_site + 1], ts_mpo
  );
  ...
  if (lr == +1) {
    if (site < L + 1)
      left[local_site + 1] =
          contraction::Engine<matrix, matrix, grp>::overlap_mpo_left_step(
              mps[site], mps[site], left[local_site], mpo[site]
          );
  }
  if (lr == -1) {
    if (site > 0)
      right[local_site] =
          contraction::Engine<matrix, matrix, grp>::overlap_mpo_right_step(
              mps[site + 1], mps[site + 1], right[local_site + 1], mpo[site + 1]
          );
  }
}
...
int main(int argc, char** argv) {
  try {
    ...
    maquis::cout << "[" << rank << "] Task is " << start << " --> " << end
                 << std::endl;

    if (nprocs % 2 != 0)
      throw std::runtime_error("number of processes must be a multiple of 2");

    if (comm.rank() == 0)
      mps = MPS<matrix, grp>(
          lattice.size(), *(model.initializer(lattice, parms))
      );
    mpi::broadcast(comm, mps, 0);
    maquis::cout << "[" << rank << "] "
                 << "BCast MPS done!" << std::endl;
    ;

    mps.canonize((lr == +1) ? start : end - 1);

    std::vector<Boundary<matrix, grp> > left(local_size + 1),
        right(local_size + 1);
    {
      ...
      if (rank == 0) {
        ...
        for (int i = L - 2; i >= 0; --i)
          tmp_right[i] =
              contraction::Engine<matrix, matrix, grp>::overlap_mpo_right_step(
                  mps[i + 1], mps[i + 1], tmp_right[i + 1], mpo[i + 1]
              );
      ...
      if (remainder > 0 && rank == nprocs - 1) {
        ...
        std::copy(
            tmp_left.begin() + chunk_size * nprocs, tmp_left.end(),
            left.begin() + chunk_size
        );
      }
    }
    ...
    block_matrix<dmt, grp> inv =
        identity_matrix<block_matrix<dmt, grp> >(mps[end - 1].col_dim());
    for (int sweep = 0; sweep < nsweeps; ++sweep) {
      if (rank > 0 && rank < nprocs - 1) {
        if (rank % 2 != 0) {
          ...
          maquis::cout << "[" << rank << "] "
                       << "doing : " << end - 1 << " dir M" << std::endl;
          central_optim(
              end - 1, inv, mps, mpo, left[local_size - 1], right[local_size],
              left[local_size], right[local_size - 1], cutoff, Mmax, parms
          );
      ...
      if (rank % 2 != 0) {
        for (site = end - 2; site >= start; --site) {
          maquis::cout << "[" << rank << "] "
                       << "doing : " << site << " dir " << -1 << std::endl;
          optim_normalize_t normalize =
              (site != start) ? with_normalization : no_normalization;
          dmrg_optim(
              site, site - start, -1, L, mps, mpo, left, right, cutoff, Mmax,
              parms, normalize
          );
        }
      } else {
        for (site = start; site < end - 1; ++site) {
          maquis::cout << "[" << rank << "] "
                       << "doing : " << site << " dir " << +1 << std::endl;
          optim_normalize_t normalize =
              (site != end - 2) ? with_normalization : no_normalization;
          dmrg_optim(
              site, site - start, +1, L, mps, mpo, left, right, cutoff, Mmax,
              parms, normalize
          );
      ...
      if (rank % 2 != 0) {
        ...
        for (site = end - 2; site >= int(start); --site) {
          ...
          if (rank == 0)
            ...
            normalize = (site != start) ? with_normalization : no_normalization;
          dmrg_optim(
              site, site - start, -1, L, mps, mpo, left, right, cutoff, Mmax,
              parms, normalize
          );
    ...
    if (rank > 0 && rank < nprocs-1) {
        if (rank % 2 != 0) {
            ...
            maquis::cout << "[" << rank << "] " << "doing : " << end-1 << " dir
    M" << std::endl; central_optim(end-1, inv, mps, mpo, left[local_size-1],
    right[local_size], left[local_size], right[local_size-1], cutoff, Mmax,
    parms);
    ...
    */
    ...
    if (remainder > 0 && rank == 0)
      comm.recv(nprocs - 1, 100, &full_mps[chunk_size * nprocs], remainder);
    if (remainder > 0 && rank == nprocs - 1)
      comm.send(0, 100, &mps[chunk_size], remainder);
