...
inline BaseParameters compute_initial_parms(BaseParameters parms) {
  ...
  std::string chkpfile = boost::trim_right_copy_if(
...
class multigrid_sim : public sim<Matrix, SymmGroup> {
 ...
 public:
  multigrid_sim(DmrgParameters& parms_)
      : base(compute_initial_parms(parms_)), initial_graining(0) {
    if (this->restore) {
  ...
  void model_init() {
    ...
    this->all_measurements << overlap_measurements<Matrix, SymmGroup>(
        this->parms
    );
  }

  std::shared_ptr<mps_initializer<Matrix, SymmGroup> > empty_mps_initiatializer(
  ) const {
    int max_site_type = 0;
    ...
    std::shared_ptr<mps_initializer<Matrix, SymmGroup> > initializer(
        new empty_mps_init<Matrix, SymmGroup>(site_bases, site_types)
    );
    return initializer;
  }
  ...
  void run() {
    ...
    for (int graining = initial_graining; graining < parms["ngrainings"];
         ++graining) {
      /// usual optimization
      if (init_sweep < parms["nsweeps"]) dmrg_run(graining);
      ...
      if (graining < parms["ngrainings"] - 1) {
        ...
        BaseParameters iteration_params =
            parms.iteration_params("graining", graining + 1);
        parms << iteration_params;
        this->model_init();
        measurements_type always_measurements =
            this->iteration_measurements(init_sweep);

        MPS<Matrix, SymmGroup> new_mps =
            MPS<Matrix, SymmGroup>(lat.size(), *empty_mps_initiatializer());
        ...
        if (curL < new_mps.length())
          graining_results =
              multigrid::extension_optim(base::parms, mps, new_mps, mpo_mix);
        else if (this->mps.length() > new_mps.length())
          throw std::runtime_error(
              "Restriction operation not really implemented."
          );
        ...
        /// write iteration results
        {
          ...
          ar[results_archive_path(0, graining, mg_measure) + "/parameters"]
              << iteration_params;
          ar[results_archive_path(0, graining, mg_measure) + "/results"]
              << graining_results;
        }
        ...
        if (always_measurements.size() > 0)
          this->measure(
              results_archive_path(0, graining, mg_measure) + "/results/",
              always_measurements
          );
  ...
  ~multigrid_sim() { storage::disk::sync(); }
 ...
 private:
  ...
  void checkpoint_simulation(
      MPS<Matrix, SymmGroup> const& state, int sweep, int site, int graining
  ) {
    status_type status;
    status["sweep"] = sweep;
    status["site"] = site;
  ...
  std::string results_archive_path(int sweep, int graining, measure_t m_type)
      const {
    std::ostringstream oss;
    oss.str("");
    switch (m_type) {
      case sweep_measure:
        oss << "/simulation/iteration/graining/" << graining << "/sweep"
            << sweep;
        break;
      case mg_measure:
    ...
    try {
      for (int sweep = init_sweep; sweep < parms["nsweeps"]; ++sweep) {
        ...
        if ((sweep + 1) % meas_each == 0 || (sweep + 1) == parms["nsweeps"]) {
          ...
          {
            storage::archive ar(rfile, "w");
            ar[results_archive_path(sweep, graining, sweep_measure) +
               "/parameters"]
                << parms;
            ar[results_archive_path(sweep, graining, sweep_measure) +
               "/results"]
                << optimizer->iteration_results();
            // ar[results_archive_path(sweep, graining, sweep_measure) +
            // "/results/Runtime/mean/value"] << std::vector<double>(1,
            // elapsed_sweep + elapsed_measure);
          }

          /// measure observables specified in 'always_measure'
          if (always_measurements.size() > 0)
            this->measure(
                results_archive_path(sweep, graining, sweep_measure) +
                    "/results/",
                always_measurements
            );
        }
        ...
        if (stopped || (sweep + 1) % chkp_each == 0 ||
      ...
      {
        storage::archive ar(rfile, "w");
        ar[results_archive_path(e.sweep(), graining, sweep_measure) +
           "/parameters"]
            << parms;
        ar[results_archive_path(e.sweep(), graining, sweep_measure) +
           "/results"]
  ...
  MPO<Matrix, SymmGroup> mixed_mpo(
      BaseParameters& parms1, int L1, BaseParameters& parms2, int L2
  ) {
...
//        maquis::cout << "MIXED LATTICE ( " << L1 << ", " <<  L2 << " )" <<
//        std::endl; for (int p=0; p<lat->size(); ++p) {
//            maquis::cout << lat->get_prop<std::string>("label", p) << ": " <<
//            lat->get_prop<double>("dx", p) << std::endl; maquis::cout <<
//            lat->get_prop<std::string>("label", p, p+1) << ": " <<
//            lat->get_prop<double>("dx", p, p+1) << std::endl; maquis::cout <<
//            lat->get_prop<std::string>("label", p, p-1) << ": " <<
//            lat->get_prop<double>("dx", p, p-1) << std::endl;
//        }
#endif

    Model<Matrix, SymmGroup> tmpmodel =
        model_factory<Matrix, SymmGroup>(lat, parms1);
    ;
