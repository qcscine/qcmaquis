 ...
 * defined in that input file, loads the corresponding MPS, and calculates the
 * overlap with the Slater determinants that are listed in a given input file,
 * which is specified in the DMRG input file.
 */
  ...
  if (opt.valid) {
    ...
    while (getline(is, str)) {
      ...
      if (std::abs(overlap) > threshold)
        std::cout << "CI coefficient of " << str << " : " << overlap
                  << std::endl;
    }
  } else {
