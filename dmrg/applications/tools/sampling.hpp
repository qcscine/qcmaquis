...
std::vector<typename SymmGroup::charge> parse_det(
    std::vector<typename SymmGroup::charge> const& det,
    std::vector<Index<SymmGroup> > const& site_dims
) {
...
void quick_sort(double data[], int left, int right) {
  ...
  int i = left, j = right;
  for (; i < j;) {
    while (!(i >= p || std::abs(pivot) < std::abs(data[i]))) ++i;
    if (i < p) {
      data[p] = data[i];
      p = i;
    }
    while (!(j <= p || std::abs(data[j]) < std::abs(pivot))) --j;
    if (j > p) {
      data[p] = data[j];
      p = j;
    }
...
struct Sampling {
  ...
  template <
      typename Determinants, typename MPS, typename Hash_value,
      typename Hash_index, typename SiteSymm, typename Determinant,
      typename SymmGroup>
  void generate_dets(
      Determinants dets, Determinants dets_mclr, MPS mps, Hash_value hash,
      Hash_index hash_index, SiteSymm site_dims, int norb, int nsample,
      int nitermax, double CI_threshold, double COM_threshold
  ) {
    ...
    // This part will be deleted, since the elements in NU1ChargePG could
    // already be used.
    ...
    for (std::size_t c = 0; c < dets_mclr.size(); ++c) {
      ...
      hash[det] = ci0;
      //            maquis::cout << "follow determinant " << c << " with
      //            coefficient " << ci0 << std::endl; for(int p = det_length-1;
      //            p >= 0; --p)
      //                maquis::cout << det[p][0] << det[p][1] << det[p][2] <<
      //                std::endl;
    }
    ...
    do {
      ...
      maquis::cout << " nele_excited " << nele_excited << " in " << norb
                   << " orbitals" << std::endl;
      for (int isample = 0; isample < nsample; isample++) {
        ...
        do {
          ...
          if (nhole_alpha != 0 && nele_alpha != 0 && dist(gen) > 0.5) {
            ...
            //                maquis::cout << " alpha - electron from " << ifr
            //                << " goto " << ito << std::endl;
            det_tmp[ifr][0] = 0;
            det_tmp[ito][0] = 1;
            ...
            //                maquis::cout << " beta  - electron from " << ifr
            //                << " goto " << ito << std::endl;
            det_tmp[ifr][1] = 0;
            det_tmp[ito][1] = 1;
          }
          ...
          charge total = std::accumulate(
              det_bee.begin(), det_bee.end(), SymmGroup::IdentityCharge
          );

          if (iele_excited >= nele_excited && total == target) break;

        } while (true);

        //            maquis::cout << " This det_bee is : " << std::endl;
        //            for(int p = det_length-1; p >= 0; --p)
        //                maquis::cout << det_bee[p][0] << det_bee[p][1] <<
        //                det_bee[p][2]  << std::endl;

        //        for(typename Determinants::iterator it = dets.begin();it !=
        //        dets.end(); ++it)
        ci = extract_coefficient(mps, det_bee);
        sum_ci2 = sum_ci2 + pow(ci, 2.0);
        //            maquis::cout << isample << "-th " << " CI coefficient: "
        //            << ci << std::endl;
      ...
      for (iter = hash.begin(); iter != hash.end(); iter++) {
        ...
        //             maquis::cout << "Determinant-" << "  coeff= " << ci_tmp
        //             << std::endl;
      }

      //        number_of_dets=dets.size();
      //        maquis::cout << " Total number of determinants : " <<
      //        number_of_dets << std::endl;
      ...
      maquis::cout << " Current completeness (1-\\sum(ci^2)) : " << completeness
                   << std::endl;
    ...
    for (iter = hash.begin(); iter != hash.end(); iter++) {
      ...
      for (int p = 0; p < det_length; p++) {
        if (iter->first[p][0] == 1 && iter->first[p][1] == 1) ctmp = ctmp + "2";
        if (iter->first[p][0] == 1 && iter->first[p][1] == 0) ctmp = ctmp + "u";
        if (iter->first[p][0] == 0 && iter->first[p][1] == 1) ctmp = ctmp + "d";
        if (iter->first[p][0] == 0 && iter->first[p][1] == 0) ctmp = ctmp + "0";
      }
    ...
    for (uint32_t i = 0; i < hash.size(); i++) {
      if (CIs_index[hash.size() - i - 1] != 0) {
        fout << CIs_index[hash.size() - i - 1] << "  "
             << CIs_show[hash.size() - i - 1] << std::endl;
      }
    }
    ...
    for (int i = 0; i < hash.size(); i++)
      maquis::cout << " The sorted determinants: "
                   << dets_show[hash.size() - i - 1] << " "
                   << CIs_show[hash.size() - i - 1] << std::endl;
  }
