  ...
  try {
    if (argc != 2) {
      if (argc != 3) {
        ...
        std::cout << "or optional:  " << argv[0] << " <mps.h5>"
                  << " <target Sz value>" << std::endl;
        return 1;
      }
    }
    ...
    if (!std::filesystem::exists(mps_in_file))
      throw std::runtime_error(
          "input MPS " + mps_in_file + " does not exist\n"
      );
    ...
    int target_sz = 0;
    if (argc == 3) target_sz = atoi(argv[2]);
    ...
    for (int Sz = -TwoS; Sz <= TwoS; Sz += 2) {
      ...
      if ((argc == 3) && (Sz != target_sz)) continue;
      ...
      // the output MPS
      MPS<matrix, mapgrp> mps_out =
          transform_mps<matrix, grp>()(mps, Nup, Ndown);

      std::string mps_out_file = mps_in_file;
      std::size_t pos = mps_out_file.find(".h5");
      if (pos != mps_out_file.size()) mps_out_file.erase(pos, 3);
      mps_out_file += "." + boost::lexical_cast<std::string>(TwoS) + "." +
                      boost::lexical_cast<std::string>(Nup - Ndown) + ".h5";
      ...
      std::filesystem::copy(
          mps_in_file + "/props.h5", mps_out_file + "/props.h5"
      );
