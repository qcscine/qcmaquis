...
int main(int argc, char** argv) {
  try {
    ...
    for (index_type b2 = 0; b2 < mpo[0].col_dim(); ++b2) {
      ...
      for (col_proxy::const_iterator col_it = col_b2.begin();
           col_it != col_b2.end(); ++col_it) {
        index_type b1 = col_it.index();
        MPOTensor_detail::term_descriptor<matrix, symm> access =
            mpo[0].at(b1, b2);
    ...
    for (size_t p = 1; p < length; ++p) {
      for (index_type b2 = 0; b2 < mpo[p].col_dim(); ++b2) {
        ...
        for (col_proxy::const_iterator col_it = col_b2.begin();
          ...
          for (col_proxy::const_iterator prev_it = col_prev.begin();
               prev_it != col_prev.end(); ++prev_it) {
