...
void measure_correlation_parallel(
    MPS<Matrix, SymmGroup> const& mps, const Lattice& lat,
...
int main(int argc, char** argv) {
  try {
    ...
    model_parser<matrix, grp>(
        parms["lattice_library"], parms["model_library"], lattice, model
    );
    ...
    Measurements<matrix, grp>::mterm_t obdm_meas =
        measurements.get("Onebody density matrix");
...
#pragma omp single
      ...
      measure_correlation_parallel(
          mps, *lattice, measurements.get_identity(),
          measurements.get_identity(), meas_obdm.operators, vals, labels
      );
    }
