...
int main(int argc, char** argv) {
  try {
    ...
    default_mps_init<matrix, grp> mpsinit(
        parms,
        chem::detail::make_2u1_site_basis<matrix, grp>(
            L, Nup, Ndown, parms["site_types"]
        ),
        chem::detail::make_2u1_initc<grp>(Nup, Ndown, irrep),
        parms["site_types"]
    );
    ...
    MPO<matrix, grp> mpo = generate_mpo::make_1D_mpo(
        positions, operators, ident, fill, model.operators_table(), lat
    );
