...
int main(int argc, char** argv) {
  try {
    ...
    if (argc != 3) throw std::runtime_error("Usage: <parms> <model_parms>");
    ...
    /// Initialize MPS
    maquis::dmrg::mps<matrix, symm> mps =
        model.intialize_mps(parms["initial_bond_dimension"], parms);
    // model.physical_dimension(), model.total_charge(),

    /// Initialize optimizer /* , ... restart(sweep, -starting site) */)
    maquis::dmrg::optimizer<
        matrix, symm, maquis::single_site, maquis::storage::disk>
        optimizer(mps, mpo, parms);
    // could be implemented as template <class M, class S, class P, class D>
    // class optimizer : P::apply<M,S,D>::type;
    ...
    for (int sweep = 0; sweep < sweeps; ++sweep) {
      ...
      std::string iteration_path = std::string("/simulation/sweeps/") +
                                   sweep;  // maybe boost::lexical_cast
    ...
    maquis::measurements results = maquis::measure(
        mps, lattice, model.measurements()
    );  // or mps.measure(...);
...
class mps<Matrix, DefaultSymm> {
 public:
  mps(parameters p) {
    switch (....) {
      case '0':
        impl = std::shared_ptr<mps_model<Matrix, NoSymm> >(
            mps_model<Matrix, NoSymm>(p)
        );
    }
  }
...
class mps<Matrix, DefaultSymm> {
 public:
  mps(parameters p) {
    switch (....) {
      case '0':
        impl = std::shared_ptr<mps<Matrix, NoSymm> >(mps<Matrix, NoSymm>(p));
    }
  }
  results_type measure() { return impl->measure(); }
