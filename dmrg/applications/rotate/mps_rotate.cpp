...
template <class Matrix, class SymmGroup>
std::pair<Matrix, typename Matrix::value_type> get_t_from_files(
    std::size_t L, std::string scale_fac, std::string fcidump
) {
  ...
  parms_temp.set("L", L);
  parms_temp.set(
      "integral_file", fcidump + ".1"
  );  // needed to construct the lattice below. Make sure this file exists...
  Lattice lat(parms_temp);  // this is needed for parse_integrals, although
                            // parse_integrals reads only L... such an overkill

  for (int i = 0; i < L; i++) {
    // get diagonal elements
    ret(i, i) =
        get_scaling<Matrix, SymmGroup>(scale_fac + "." + std::to_string(i + 1));
    ...
    std::tie(index_temp, values_temp) =
        chem::detail::parse_integrals<vt, SymmGroup>(parms_temp, lat, false);

    // copy the parsed integrals at their according positions in the ret matrix
    for (std::size_t m = 0; m < values_temp.size(); m++) {
      int i = index_temp(m, 0), j = index_temp(m, 1), k = index_temp(m, 2),
          l = index_temp(m, 3);
      assert(k == -1 && l == -1);

      // the matrix elements in MOLCAS are already divided by the diagonal
      // element so we need to scale them back
...
int main(int argc, char** argv) {
  try {
    ...
    std::tie(t, scale_inactive) =
        get_t_from_files<matrix, grp>(L, scale_fac_file, fcidump_file);
