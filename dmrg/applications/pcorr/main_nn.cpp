...
class triag_range {
 public:
  ...
  triag_range(size_type length, size_type nprocs = 1, size_type rank = 0)
    ...
    if (rank == 0 && length % 2 != 0) indexes_.push_back(length - 1);
  }
...
void measure_correlation(
    Range const& range, MPS<Matrix, SymmGroup> const& bra,
    MPS<Matrix, SymmGroup> const& ket, Lattice const& lattice,
  ...
  std::vector<std::vector<MPOTensor<Matrix, SymmGroup> > > mpo_ops(nops),
      mpo_signed_ops(nops);
  ...
  for (size_t i = 0; i < range.size(); ++i) {
    ...
    Timer tim("measure p=" + boost::lexical_cast<std::string>(p));
    tim.begin();
    ...
    Boundary<Matrix, SymmGroup> current =
        contraction::Engine<matrix, matrix, grp>::overlap_mpo_left_step(
            bra[p], ket[p], left[p], mpo_signed_ops[0][type_p]
        );

    {
      current = contraction::Engine<matrix, matrix, grp>::overlap_mpo_left_step(
          bra[p + d], ket[p + d], current,
          mpo_ops[1][lattice.get_prop<int>("type", p + d)]
      );

      for (int q = p + d + 1; q < L - d; ++q) {
        int type_q = lattice.get_prop<int>("type", q);

        Boundary<Matrix, SymmGroup> current2 =
            contraction::Engine<matrix, matrix, grp>::overlap_mpo_left_step(
                bra[q], ket[q], current, mpo_signed_ops[2][type_q]
            );
        for (int r = q + 1; r < q + d; ++r) {
          ...
          current2 =
              contraction::Engine<matrix, matrix, grp>::overlap_mpo_left_step(
                  bra[r], ket[r], current2, mpo_fill
              );
        }
        current2 =
            contraction::Engine<matrix, matrix, grp>::overlap_mpo_left_step(
                bra[q + d], ket[q + d], current2,
                mpo_ops[3][lattice.get_prop<int>("type", q + d)]
            );
        block_matrix<Matrix, SymmGroup> const& vec = current2[0];

        double obs = 0.;
        for (int k = 0; k < vec.n_blocks(); ++k) {
          std::size_t matched_block = right[q + d + 1][0].find_block(
              vec.right_basis()[k].first, vec.left_basis()[k].first
          );
...
int main(int argc, char** argv) {
  try {
    ...
    {
      std::regex expression("^MEASURE_HALF_NN_CORRELATIONS\\[(.*)]$");
      std::smatch what;
      for (alps::Parameters::const_iterator it = parms.begin();
           it != parms.end(); ++it) {
        ...
        if (std::regex_match(lhs, what, expression)) {
          ...
          if (corr_tokens.size() != 4)
            throw std::runtime_error(
                "`pcorr_nn` can compute only two-body correlation functions."
            );
          ...
          for (it2 = corr_tokens.begin(), ii = 0; it2 != corr_tokens.end();
               it2++, ++ii) {
    ...
    maquis::cout << "[" << rank << "] will do " << myrange.size() << " steps."
                 << std::endl;

    /// compute left / right boundaries
    /// note: this is the current bottleneck. this part has to run sequentially!
    std::vector<Boundary<matrix, grp> > right(
        L + 1, Boundary<matrix, grp>(Index<grp>(), Index<grp>(), 1)
    );
    std::vector<Boundary<matrix, grp> > left(
        L + 1, Boundary<matrix, grp>(Index<grp>(), Index<grp>(), 1)
    );
    ...
    if (rank == 0) {
      ...
      for (int p = L - 1; p >= static_cast<int>(0); --p)
        right[p][0] =
            contraction::Engine<matrix, matrix, grp>::overlap_right_step(
                mps[p], MPSTensor<matrix, grp>(mps[p]), right[p + 1][0]
            );
    }
    ...
    for (std::vector<corr_measurement>::const_iterator it =
             measurements.begin();
      ...
      std::vector<std::string> labels;
      measure_correlation(
          myrange, mps, mps, lattice, identities,
          (it->fermionic) ? fillings : identities, *it, left, right, values,
          labels
      );
      ...
      if (rank != 0) {
        ...
        ar["/spectrum/results/" + it->name + "/mean/value"]
            << std::vector<std::vector<value_type> >(1, values);
      }
    }
