...
int main(int argc, char ** argv)
{
    try {
        ...
        {
            boost::regex expression("^MEASURE_HALF_CORRELATIONS\\[(.*)]$");
            boost::smatch what;
        ...
        for (std::vector<corr_measurement>::const_iterator it = measurements.begin();
            ...
            std::vector<std::string> labels;
            measure_correlation(myrange, mps, mps, lattice, (it->fermionic) ? fillings : identities, *it, left, right, values, labels, contr);
