...
void measure_correlation(
  ...
  for (size_t i = 0; i < range.size(); ++i) {
    ...
    for (int q = p + 1; q < L; ++q) {
      ...
      block_matrix<Matrix, SymmGroup> vec =
          contraction::Engine<matrix, matrix, grp>::overlap_mpo_left_step(
              bra[q], ket[q], current, mpo_op2
          )[0];

      double obs = 0.;
      for (int k = 0; k < vec.n_blocks(); ++k) {
        std::size_t matched_block = right[q + 1][0].find_block(
...
int main(int argc, char** argv) {
  try {
        ...
        if (std::regex_match(lhs, what, expression)) {
          ...
          if (corr_tokens.size() != 2)
            throw std::runtime_error(
                "`pcorr` can compute only two-body correlation functions."
            );
          ...
          for (std::vector<std::string>::const_iterator it2 =
            ...
            for (int type = 0; type < ntypes; ++type) {
              ...
              if (it2 == corr_tokens.begin())
                meas.op1[type] = tag_handler->get_op(tag);
              else
                meas.op2[type] = tag_handler->get_op(tag);
    ...
    for (std::vector<corr_measurement>::const_iterator it =
      ...
      std::vector<std::string> labels;
      measure_correlation(
          myrange, mps, mps, lattice, (it->fermionic) ? fillings : identities,
          *it, left, right, values, labels
      );
