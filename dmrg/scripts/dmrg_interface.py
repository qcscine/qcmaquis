...
'''
pattern_harmonic       = r'\sMode\(Quanta\)\s+E\(harm\)\s+E\(anharm\)' 
pattern_quadratic      = r':\s+QUADRATIC\sFORCE\sCONSTANTS\sIN\sNORMAL\sMODES'
...
for i in log_anharm:
    ...
    match_Qdr = re.search(pattern_quadratic, i)
    ...
    LJunk0  = re.search(pattern_quadratic_const,i,re.DOTALL|re.VERBOSE)
    ...
    if (FndHrm and LJunk0):
        Indx1   = int(LJunk0.group('index1'))
        Indx2   = int(LJunk0.group('index2'))
        LstJnk  = [Indx1, Indx2]
        LstJnk2 = [LstJnk, float(LJunk1.group('const1'))]
        Lst_Hrm.append(LstJnk2)
...
for i in range(len(Lst_Hrm)):
    if(not DoFact):
        fact = multinomial(Lst_Hrm[i][0])*2.
    else:
        fact = 2.
    out_file.write(str_qdr.format(value=Lst_Hrm[i][1]/fact, index1=Lst_Hrm[i][0], index2=Lst_Hrm[i][0]))
    out_file.write(str_qdr.format(value=-Lst_Hrm[i][1]/fact, index1=-Lst_Hrm[i][0], index2=-Lst_Hrm[i][0]))
