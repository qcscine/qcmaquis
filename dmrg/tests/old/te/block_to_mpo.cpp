...
std::ostream& operator<<(
    std::ostream& os, std::pair<grp::charge, std::size_t> const& p
) {
  os << "(" << p.first << " : " << p.second << ")";
  return os;
}
...
std::ostream& operator<<(
    std::ostream& os, index_product_iterator<grp>::value_type const& v
) {
  // std::copy(v.begin(), v.end(), std::ostream_iterator<std::pair<grp::charge,
  // std::size_t> >(os, " "));
...
std::ostream& operator<<(
    std::ostream& os,
    std::pair<MultiIndex<grp>::coord_t, MultiIndex<grp>::coord_t> const& p
) {
  os << p.first << ", " << p.second;
  return os;
}
...
MPO<Matrix, grp> block_to_mpo(
    Index<grp> const& phys_i, block_matrix<Matrix, grp> ident,
    block_matrix<Matrix, grp> bond_op, std::size_t dist, std::size_t length
) {
  ...
  block_matrix<alps::numeric::associated_diagonal_matrix<Matrix>::type, grp> S,
      Ssqrt;
  svd(bond_op, U, V, S);
  Ssqrt = sqrt(S);
  gemm(U, Ssqrt, left);
  ...
  std::vector<block_matrix<Matrix, grp> > U_list =
      reshape_right_to_list(phys_i, left);
  std::vector<block_matrix<Matrix, grp> > V_list =
      reshape_left_to_list(phys_i, right);
...
int main(int argc, char** argv) {
  ...
  MPO<Matrix, grp> block_mpo_easy =
      block_to_mpo(phys, ident, bond_op2, dist, dist + 1);
