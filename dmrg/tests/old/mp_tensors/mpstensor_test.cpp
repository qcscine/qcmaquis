...
void ng() {
  ...
  maquis::cout << "Norm after normalization: " << mps.scalar_norm()
               << std::endl;
...
void u1() {
  ...
  MPSTensor<Matrix, grp> mps2 = mps;
  //    block_matrix<Matrix, grp> left = identity_matrix<Matrix,
  //    grp>(mps.row_dim()); left = contraction::overlap_left_step(mps, mps2,
  //    left); maquis::cout << left << std::endl; mps.normalize_right(SVD); mps2
  //    = mps; left = contraction::overlap_right_step(mps, mps2, left);
  //    maquis::cout << left << std::endl;
