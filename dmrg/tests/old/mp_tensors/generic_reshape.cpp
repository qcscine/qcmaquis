...
void test_reshape() {
  ...
  std::vector<std::pair<MultiIndex<symm>::index_id, bool> > left_paired_rows(2),
      left_paired_cols(1);
  left_paired_rows[0] = std::make_pair(sigma, true);
  left_paired_rows[1] = std::make_pair(alpha, true);
  left_paired_cols[0] = std::make_pair(beta, true);
  MultiIndex<symm>::set_id left_paired =
      midx.create_set(left_paired_rows, left_paired_cols);

  std::vector<std::pair<MultiIndex<symm>::index_id, bool> > right_paired_rows(1
  ),
      right_paired_cols(2);
  right_paired_rows[0] = std::make_pair(alpha, true);
  right_paired_cols[0] = std::make_pair(sigma, false);
  right_paired_cols[1] = std::make_pair(beta, true);
  MultiIndex<symm>::set_id right_paired =
      midx.create_set(right_paired_rows, right_paired_cols);
  ...
  for (size_t i = 0; i < max_i; ++i)
    m1.insert_block(
        Matrix(left_i[i].second, beta_i[i].second), left_i[i].first,
        beta_i[i].first
    );
  ...
  for (size_t n = 0; n < res1.n_blocks(); ++n)
    assert(equal(
        res1[n].elements().first, res1[n].elements().first,
  ...
  for (size_t n = 0; n < res2.n_blocks(); ++n)
    assert(equal(
        res2[n].elements().first, res2[n].elements().first,
