...
int main(int argc, char** argv) {
  try {
    ...
    /// Timers
    Timer time_model("Parsing model"), time_load("Load MPS"),
    ...
    maquis::cout << "Optimization at site " << site << " in " << lr
                 << " direction." << std::endl;
    ...
    tst.clear();
    MPOTensor<matrix, grp> ts_mpo = make_twosite_mpo<matrix, matrix>(
        mpo[site], mpo[site + 1], mps[site].site_dim(), mps[site + 1].site_dim()
    );
    if (lr == +1) {
      ts_mpo.placement_l = mpo[site].placement_l;
      ts_mpo.placement_r = parallel::get_right_placement(
          ts_mpo, mpo[site].placement_l, mpo[site + 1].placement_r
      );
    } else {
      ts_mpo.placement_l = parallel::get_left_placement(
          ts_mpo, mpo[site].placement_l, mpo[site + 1].placement_r
      );
...
#endif
    {
      ...
      for (size_t i = 0; i < site; ++i) {
        left = contraction::Engine<matrix, matrix, grp>::overlap_mpo_left_step(
    ...
    boundary_name =
        "right" + boost::lexical_cast<std::string>(site + 2) + ".h5";
#ifndef USE_AMBIENT
    if (exists(chkpfile / boundary_name)) {
    ...
    if (lr == +1) {
      if (parms["twosite_truncation"] == "svd")
        std::tie(mps[site], mps[site + 1], trunc) =
            tst.split_mps_l2r(Mmax, cutoff);
      else
        std::tie(mps[site], mps[site + 1], trunc) =
            tst.predict_split_l2r(Mmax, cutoff, alpha, left, mpo[site]);
      tst.clear();
    ...
    if (lr == -1) {
      if (parms["twosite_truncation"] == "svd")
        ...
        std::tie(mps[site], mps[site + 1], trunc) =
            tst.predict_split_r2l(Mmax, cutoff, alpha, right, mpo[site + 1]);
      tst.clear();
