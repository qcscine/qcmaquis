...
struct U1System {
  ...
  static grp::charge total_quantum_numbers() { return 5; }
...
BOOST_AUTO_TEST_CASE_TEMPLATE(custom_model_energy, ML, test_systems) {
  ...
  Model<matrix, grp> mdl = model_builder.make_model();
  MPO<matrix, grp> mpo = make_mpo(lattice, mdl);

  charge initc = ML::total_quantum_numbers();
  default_mps_init<matrix, grp> initializer(
      parms, std::vector<Index<grp> >(1, phys), initc, std::vector<int>(L, 0)
  );
  ...
  BOOST_CHECK_CLOSE(
      energy, -5.445283602, 1e-7
  );  // if the random state does not change, it should be the correct energy.
}
