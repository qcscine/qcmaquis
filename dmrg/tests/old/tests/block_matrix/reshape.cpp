...
std::vector<double> density(
    ...
    block_matrix<Matrix, SymmGroup> const& dens_op,
...
MPS<Matrix, SymmGroup> state_mps(
    std::vector<short> const& state, Index<SymmGroup> const& phys
) {
...
std::vector<std::vector<short> > create_basis(int L, int N) {
  ...
  for (int n2 = 0; n2 * 2 <= N; ++n2) {
    ...
    for (int i = n0; i < n0 + n1; ++i) state[i] = 1;
    for (int i = n0 + n1; i < n0 + n1 + n2; ++i) state[i] = 2;
    do {
...
BOOST_AUTO_TEST_CASE(free_test_function) {
  ...
  /*
      ...
      //    std::vector<std::vector<double> > over(Nrep,
     std::vector<double>(basis.size())); for (int i=0; i<Nrep; i++) {
      ...
      //        {
      //            printf("#%03d: | ", j+1);
      //            std::copy(basis[j].begin(), basis[j].end(),
