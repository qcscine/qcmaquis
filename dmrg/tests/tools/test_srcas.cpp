...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(
  ...
  BOOST_CHECK_CLOSE(
      srcas.getCompleteness(), 1.0, 1.0E-10
  ...
  if (parametersLiH["symmetry"] == "su2u1pg" ||
    ...
    BOOST_CHECK_CLOSE(
        srcas2.getCompleteness(), 1.0, 1.0E-10
    );  // Can be this thight, because we should sample all dets
  }
}
...
BOOST_FIXTURE_TEST_CASE(
    Test_Vibrational_SRCAS_Ethylene_Harmonic, WatsonFixture
) {
  using InterfaceType = maquis::DMRGInterface<double>;
  ...
  BOOST_CHECK_CLOSE(
      srcas.getCompleteness(), 1.0, 1.0E-10
  );  // Can be this thight, because we start in GS and have no coupling
}
 ...
 */
BOOST_FIXTURE_TEST_CASE(
    Test_Vibrational_SRCAS_Ethylene_Sextic_SingleSite, WatsonFixture
) {
  using InterfaceType = maquis::DMRGInterface<double>;
  // Adds the final input parameters
...
BOOST_FIXTURE_TEST_CASE(
  ...
  std::vector<int> currQueen = srcas.getCurrentQueen();
  BOOST_CHECK_EQUAL(
      currQueen.size(), parametersFADOneBodyBinary["nmode_num_modes"]
  );
  int sumOfQueen = std::accumulate(currQueen.begin(), currQueen.end(), 0);
  BOOST_CHECK_EQUAL(sumOfQueen, 0);
  srcas.run();
  ...
  BOOST_CHECK_EQUAL(detTable.size(), 1);
  BOOST_CHECK_CLOSE(
      srcas.getCompleteness(), 1.00, 1.0E-10
  );  // Can be this thight, because we start in GS and have no coupling
  ...
  auto maxAbsCICoeff = std::max_element(
      detTableES.begin(), detTableES.end(),
  ...
  BOOST_CHECK_EQUAL(
      std::accumulate(
          maxAbsCICoeff->first.begin(), maxAbsCICoeff->first.end(), 0
      ),
      1
  );
