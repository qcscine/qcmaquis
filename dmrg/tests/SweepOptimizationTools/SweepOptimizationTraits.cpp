...
BOOST_AUTO_TEST_CASE(Test_SweepOptimizationTrait_SS) {
  using SweepTraitClass =
      SweepOptimizationTypeTrait<SweepOptimizationType::SingleSite>;
  int latticeSize = 10;
  auto numberOfMicroiterations =
      SweepTraitClass::getNumberOfMicroiterations(latticeSize);
  ...
  for (int iSite = 0; iSite < numberOfMicroiterations; iSite++) {
    visitedSites.push_back(
        SweepTraitClass::convertMicroIterationToSite(latticeSize, iSite)
    );
  ...
  BOOST_CHECK_EQUAL(
      std::count(visitedSites.begin(), visitedSites.end(), 10), 0
  );
  // Does the same for the directions
  BOOST_CHECK_EQUAL(
      std::count(
          directions.begin(), directions.end(), SweepDirectionType::Forward
      ),
      10
  );
  BOOST_CHECK_EQUAL(
      std::count(
          directions.begin(), directions.end(), SweepDirectionType::Backward
      ),
      8
  );
}
...
BOOST_AUTO_TEST_CASE(Test_SweepOptimizationTrait_TS) {
  using SweepTraitClass =
      SweepOptimizationTypeTrait<SweepOptimizationType::TwoSite>;
  int latticeSize = 10;
  auto numberOfMicroiterations =
      SweepTraitClass::getNumberOfMicroiterations(latticeSize);
