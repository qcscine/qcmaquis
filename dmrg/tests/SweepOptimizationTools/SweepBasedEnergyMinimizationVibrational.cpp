...
/**
 * @brief Checks that the constructor for the sweep-based energy minimization
 * works.
 */
BOOST_FIXTURE_TEST_CASE(
    Test_SweepBasedEnergyMinimizerSS_Vibrational_Watson, WatsonFixture
) {
#ifdef HAVE_TrivialGroup
  // Data Generation
  using SweepBasedMinimizerSS = SweepBasedEnergyMinimization<
      matrix, TrivialGroup, storage::disk, SweepOptimizationType::SingleSite>;
  ...
  auto watsonModel =
      Model<matrix, TrivialGroup>(lattice, parametersEthyleneWatsonHarmonic);
  ...
  auto mpsDefault = MPS<matrix, TrivialGroup>(
      lattice.size(),
      *(watsonModel.initializer(lattice, parametersEthyleneWatsonHarmonic))
  );
  mpsDefault.normalize_right();
  SweepBasedMinimizerSS energyMinimizer(
      mpsDefault, watsonHarmonicMPO, parametersEthyleneWatsonHarmonic,
      watsonModel, lattice, false
  );
  energyMinimizer.runSweepSimulation();
  double optimalEnergyFromSweeper =
      energyMinimizer.getSpecificResult<double>("Energy");
  ...
  BOOST_CHECK_CLOSE(
      optimalEnergyFromInterface, optimalEnergyFromSweeper, 1.0e-7
  );
...
BOOST_FIXTURE_TEST_CASE(
    Test_SweepBasedEnergyMinimizerTS_Vibrational_Bilinearly, WatsonFixture
) {
#ifdef HAVE_TrivialGroup
  // Data Generation
  using SweepBasedMinimizerTS = SweepBasedEnergyMinimization<
      matrix, TrivialGroup, storage::disk, SweepOptimizationType::TwoSite>;
  using MPSType = MPS<matrix, TrivialGroup>;
  ...
  auto bilinearlyModel =
      Model<matrix, TrivialGroup>(lattice, parametersBilinearly);
  ...
  auto mpsConst = MPS<matrix, TrivialGroup>(
      lattice.size(),
      *(bilinearlyModel.initializer(lattice, parametersBilinearly))
  );
  mpsConst.normalize_right();
  SweepBasedMinimizerTS energyMinimizer(
      mpsConst, bilinearlyMPO, parametersBilinearly, bilinearlyModel, lattice,
      false
  );
  energyMinimizer.runSweepSimulation();
  double optimalEnergyFromSweeper =
