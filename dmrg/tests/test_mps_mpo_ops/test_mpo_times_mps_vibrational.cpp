 ...
 * expval. Note that here we use 1) the harmonic MPS and 2) the harmonic
 * Hamiltonian.
 */
BOOST_FIXTURE_TEST_CASE(Test_MPO_Times_MPS_None, WatsonFixture) {
  ...
  auto mps = MPS<matrix, TrivialGroup>(
      lattice.size(),
      *(watsonModel.initializer(lattice, parametersEthyleneWatsonHarmonic))
  );
  // Calculates the MPS-MPO contraction
  auto traitClass = MPOTimesMPSTraitClass<matrix, TrivialGroup>(
      mps, watsonModel, lattice,
      watsonModel.total_quantum_numbers(parametersEthyleneWatsonHarmonic),
  ...
  auto energyFromMPSTimesMPO =
      overlap(mps, outputMPS) / norm(mps) + watsonHarmonicMPO.getCoreEnergy();
...
BOOST_FIXTURE_TEST_CASE(Test_MPO_Times_MPS_Hermiticity_None, WatsonFixture) {
  ...
  auto watsonHarmonicMPO = make_mpo(lattice, watsonModel);
  auto mps1 = MPS<matrix, TrivialGroup>(
      lattice.size(), *(watsonModel.initializer(lattice, parametersH2COWatson))
  );
  // Calculates the default guess
  parametersH2COWatson.set("init_type", "default");
  auto mps2 = MPS<matrix, TrivialGroup>(
      lattice.size(), *(watsonModel.initializer(lattice, parametersH2COWatson))
  );
...
/**
 * @brief Checks that the H^2 expectation value, caluclated via [mpo_times_mps]
 * and expva, gives coherent results.
 */
BOOST_FIXTURE_TEST_CASE(Test_MPO_Times_MPS_Variance_None, WatsonFixture) {
  ...
  auto outputMPS = traitClass.applyMPO(watsonHarmonicMPO);
  // Calculates the energy in two ways and check that the results are coherent
  auto squaredEnergy = overlap(outputMPS, outputMPS) / norm(mps) +
                       watsonHarmonicMPO.getCoreEnergy();
  auto energyFromExpVal = expval(mps, watsonHarmonicMPO) / norm(mps);
...
/**
 * @brief Checks that [mpo_times_mps] gives results that are coherent with
 * expval. Note that here we use the NU1 symmetry group and the default guess.
 */
BOOST_FIXTURE_TEST_CASE(Test_MPO_Times_MPS_NU1, NModeFixture) {
  ...
  auto mps = MPS<matrix, NU1_template<2>>(
      lattice.size(), *(nModeModel.initializer(lattice, parametersFADTwoBody))
  );
  // Calculates the MPS-MPO contraction
  auto traitClass = MPOTimesMPSTraitClass<matrix, NU1_template<2>>(
      mps, nModeModel, lattice,
      nModeModel.total_quantum_numbers(parametersFADTwoBody),
      parametersFADTwoBody["max_bond_dimension"]
  );
  auto outputMPS = traitClass.applyMPO(nModeMPO);
  // Calculates the energy in two ways and check that the results are coherent
  auto energyFromMPSTimesMPO =
      overlap(mps, outputMPS) / norm(mps) + nModeMPO.getCoreEnergy();
