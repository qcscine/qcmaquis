...
/** @brief Checks that overlap and norm give coherent results for the electronic
 * case */
BOOST_FIXTURE_TEST_CASE_TEMPLATE(
    Test_MPS_Overlap_Electronic, S, symmetries, BenzeneFixture
) {
...
/** @brief Checks Hermitianity of the overlap calculation for the real-valued
 * electronic case */
BOOST_FIXTURE_TEST_CASE_TEMPLATE(
    Test_MPS_Overlap_Hermitian_Electronic, S, symmetries, BenzeneFixture
) {
  // Global variables
  auto lattice = Lattice(parametersBenzene);
  auto model = Model<matrix, S>(lattice, parametersBenzene);
...
/** @brief Checks Hermitianity of the overlap calculation for the relativistic
 * (complex-valued) electronic case */
BOOST_FIXTURE_TEST_CASE(
    Test_MPS_Overlap_Hermitian_Electronic_Complex, RelativisticFixture
) {
  // Global variables
  ...
  auto mpsConst = MPS<cmatrix, U1DG>(
      lattice.size(), *(model.initializer(lattice, parametersComplex))
  );
  parametersComplex.set("init_type", "default");
  auto mpsDefault = MPS<cmatrix, U1DG>(
      lattice.size(), *(model.initializer(lattice, parametersComplex))
  );
  // Calculates the overlap
  ...
  BOOST_CHECK_CLOSE(
      std::imag(overlapOriginal), -std::imag(overlapHerm), 1.E-10
  );
}
