...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(
    Test_MPS_MPO_Hermitian_Left_Electronic, S, symmetries, BenzeneFixture
) {
  ...
  auto mpsConst = MPS<matrix, S>(
      lattice.size(), *(modelConst.initializer(lattice, parametersBenzene))
  );
  ...
  auto mpsDefault = MPS<matrix, S>(
      lattice.size(), *(modelDefault.initializer(lattice, parametersBenzene))
  );
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(
    Test_MPS_MPO_Hermitian_Right_Electronic, S, symmetries, BenzeneFixture
) {
  // Generates the HF MPS
  auto lattice = Lattice(parametersBenzene);
  auto modelHF = Model<matrix, S>(lattice, parametersBenzene);
