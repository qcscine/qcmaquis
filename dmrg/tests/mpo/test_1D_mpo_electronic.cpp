...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(
    Test_MPO_Times_MPS_ExpVal, S, symmetries, H2Fixture
) {
  ...
  auto mpsHF = MPS<matrix, S>(
      lattice.size(), *(model.initializer(lattice, parametersH2))
  );
  // Creates the destructor operator
  auto traitClass = MPOTimesMPSTraitClass<matrix, S>(
      mpsHF, model, lattice, model.total_quantum_numbers(parametersH2),
  ...
  auto mpsHFCation = MPS<matrix, S>(
      lattice.size(), *(modelCation.initializer(lattice, parametersH2))
  );
