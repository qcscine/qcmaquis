...
// Note that here below at least one test is defined also if TrivialGroup is not
// available (otherwise boost will complain)

/** Checks consistency for the physical dimensions for the ethylene Watson
 * Hamiltonian */
BOOST_FIXTURE_TEST_CASE(Test_Model_PhysDim_Ethylene, WatsonFixture) {
  ...
  auto nModeModel = WatsonHamiltonian<matrix>(
      lattice, parametersEthyleneWatsonHarmonic, false
  );
  int siteType = 0;
...
/** Checks consistency for the physical dimensions for the ethylene Watson
 * Hamiltonian with NMax vector initialization */
...
BOOST_FIXTURE_TEST_CASE(Test_Model_Tag_SimpleCheck_Ethylene, WatsonFixture) {
  auto lattice = Lattice(parametersEthyleneWatsonHarmonic);
  auto watsonModel = WatsonHamiltonian<matrix>(
      lattice, parametersEthyleneWatsonHarmonic, false
  );
...
BOOST_FIXTURE_TEST_CASE(Test_Model_Symbolic_Operator_Ethylene, WatsonFixture) {
  ...
  BOOST_CHECK(
      watsonModel.filling_matrix_tag(0) ==
      watsonModel.get_operator_tag("fill", 0)
  );
}

/** Check consistency in the dimension of the integral container */
BOOST_FIXTURE_TEST_CASE(
    Test_Model_Watson_Ethylene_IntegralContainer, WatsonFixture
) {
  auto lattice = lattice_factory(parametersEthyleneWatsonHarmonic);
  auto integrals = Vibrational::detail::WatsonIntegralParser<double>(
      parametersEthyleneWatsonHarmonic, lattice,
      WatsonCoordinateType::CartesianNormalModes, 6, 6, 6
  );
