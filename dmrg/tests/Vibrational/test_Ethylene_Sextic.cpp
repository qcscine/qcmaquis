...
BOOST_FIXTURE_TEST_CASE(
    Test_vDMRG_Calculation_Ethylene_Harmonic, WatsonFixture
) {
...
/**
 * @brief Watson-based calculation on the harmonic ethylene PES, but with input
 * given only on two rows.
 */
BOOST_FIXTURE_TEST_CASE(
    Test_vDMRG_Calculation_Ethylene_Harmonic_TwoRows, WatsonFixture
) {
  using InterfaceType = maquis::DMRGInterface<double>;
  // Adds the final input parameters
  ...
  parametersEthyleneWatsonHarmonicTwoRows.set(
      "MEASURE[ModeExcitationDegree]", 1
  );
  // Creates the interface
 ...
 */
BOOST_FIXTURE_TEST_CASE(
    Test_vDMRG_Calculation_Ethylene_Sextic_SingleSite, WatsonFixture
) {
  using InterfaceType = maquis::DMRGInterface<double>;
  // Adds the final input parameters
 ...
 */
BOOST_FIXTURE_TEST_CASE(
    Test_vDMRG_Calculation_Ethylene_Sextic_TwoSite, WatsonFixture
) {
  using InterfaceType = maquis::DMRGInterface<double>;
  // Adds the final input parameters
  parametersEthyleneWatson.set("init_type", "basis_state_generic");
