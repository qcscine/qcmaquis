...
/** @brief Checks that the constructor for the Watson-based Hamiltonian works
 * properly */
...
BOOST_FIXTURE_TEST_CASE(
    Test_Lattice_MaxVertexType_Watson_Ethylene, WatsonFixture
) {
  auto lattice = WatsonLattice(parametersEthyleneWatson);
  auto size = lattice.size();
...
BOOST_FIXTURE_TEST_CASE(
    Test_Lattice_PropertyGetter_Watson_Ethylene, WatsonFixture
) {
  auto lattice = WatsonLattice(parametersEthyleneWatson);
...
/** @brief Validates the implementation of a generic sorting for an n-mode
 * Lattice */
BOOST_FIXTURE_TEST_CASE(Test_Lattice_NMode_Arbitrary_Sorting, NModeFixture) {
  DmrgParameters parametersArbitrarySorting;
  std::string order =
      "22,27,24,49,48,52,50,51,56,55,86,84,85,57,54,53,47,28,23,46,26,25,20,19,"
      "17,18,16,15,14,13,12,10,11,9,8,7,6,5,4,3,2,1,0,79,77,76,74,72,61,60,62,"
      "63,70,64,69,";
  order +=
      "65,67,68,66,59,71,58,73,29,30,33,34,32,31,35,36,37,75,38,81,39,40,41,78,"
      "82,42,80,43,83,44,45,21";
  ...
  for (int iMode = 0; iMode < 3; iMode++) {
    for (int iModal = 0; iModal < 29; iModal++) {
      auto tmpPos =
          lattice.get_prop<int>("absolutePositionInLattice", iMode, iModal);
