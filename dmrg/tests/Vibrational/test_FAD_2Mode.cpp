...
BOOST_FIXTURE_TEST_CASE(
    Test_Lattice_Size_1ModeHamiltonian_ExcitedState, NModeFixture
) {
  // Adds the final input parameters
...
/** @brief Check that the ground-state energy is the same irrespectively on the
 * modal order */
BOOST_FIXTURE_TEST_CASE(
    Test_Lattice_Size_FingerprintHamiltonian_Sorting, NModeFixture
) {
  // Adds the final input parameters
  ...
  // Creates the interface and checks the resulting energy
  maquis::DMRGInterface<double> interfaceConventionalSorting(
  ...
  for (int iSite = 0; iSite < latticeSize; iSite++) newOrder[iSite] = iSite;
  ...
  for (int iElement = 0; iElement < newOrder.size(); iElement++) {
    ...
    if (iElement != newOrder.size() - 1) inputOrder += ",";
  }
  parametersFADTwoBodyFingerPrint.set("modals_order", inputOrder);
  maquis::DMRGInterface<double> interfaceRandomSorting(
      parametersFADTwoBodyFingerPrint
  );
...
BOOST_FIXTURE_TEST_CASE(
    Test_Lattice_Size_2ModeSystem_MeasOneParticle, NModeFixture
) {
  // Adds the final input parameters
  parametersFADTwoBody.set("init_type", "const");
  ...
  for (int iSite = 0; iSite < 22; iSite++)
    BOOST_CHECK_SMALL(
        std::fabs(firstMeas[iSite] + secondMeas[iSite] - 1.), 1.0E-15
    );
}

/**
 * @brief Measure the two-modal RDM and checks sanity of the modal entropies
 * The sanity check is done by verifying that the subadditivity property is
 * verified (i.e., that S_{pq} < S_p + S_q, see Chem. Phys. 323, 519 (2006))
 */
BOOST_FIXTURE_TEST_CASE(
    Test_Lattice_Size_2ModeSystem_Subadditivity, NModeFixture
) {
  // Adds the final input parameters
  ...
  interface.measure();
  // == TWO-MODAL ENTROPY ==
  tmatrix<double> twoMatrixEntanglement(22, 22, 0.), twoMode00(22, 22, 0.),
      twoMode11(22, 22, 0.), twoMode22(22, 22, 0.), twoMode33(22, 22, 0.),
      twoMode12(22, 22, 0.), twoMode21(22, 22, 0.);
  BOOST_CHECK_EQUAL(
      interface.getMeasurement("twomodeRDM_00").first.size(),
  ...
  BOOST_CHECK_EQUAL(
      interface.getMeasurement("twomodeRDM_21").first.size(),
      interface.getMeasurement("twomodeRDM_12").first.size()
  );
  BOOST_CHECK_EQUAL(
      interface.getMeasurement("twomodeRDM_21").first.size(),
  ...
  BOOST_CHECK_EQUAL(
      interface.getMeasurement("twomodeRDM_33").first.size(),
      interface.getMeasurement("twomodeRDM_22").first.size()
  );
  ...
  for (int iRow = 0; iRow < 22; iRow++) {
    for (int iCol = 0; iCol < 22; iCol++) {
      BOOST_TEST(
          twoMatrixEntanglement(iRow, iCol) <
          vectorEntanglement[iRow] + vectorEntanglement[iCol]
      );
    }
  }
}
 ...
 * Note that the trace of the one-mode RDM should be 1 for each sub-block of
 * modals belonging to the same mode. The overall trace should then be equal to
 * the overall number of modes.
 */
BOOST_FIXTURE_TEST_CASE(Test_Lattice_Size_2ModeSystem_ModeRDM, NModeFixture) {
  ...
  for (int iElement = 0; iElement < overallSize; iElement++) {
    ...
    oneModeRDM(iRow, iCol) =
        interface.getMeasurement("onemodeRDM").second[iElement];
  }
  // Calculates the overall trace
  double trace = 0.;
  for (int idx = 0; idx < 22; idx++) trace += oneModeRDM(idx, idx);
