...
BOOST_FIXTURE_TEST_CASE(Test_Integral_Parser_Vibronic, VibronicFixture) {
  ...
  auto integrals = Vibrational::detail::parseIntegralVibronic<double>(
      parametersFakeVibronic, lattice
  );
  // Checks sizes
...
BOOST_FIXTURE_TEST_CASE(Test_VibronicModel_Create_Terms, VibronicFixture) {
  auto lattice = Lattice(parametersFakeVibronic);
  auto fakeVibronicModel =
      VibronicModel<tmatrix<double>>(lattice, parametersFakeVibronic);
...
/** Checks consistency for the physical dimensions for the "trivial" vibronic
 * model */
...
BOOST_FIXTURE_TEST_CASE(Test_Model_TotalQN_FakeVibronic, VibronicFixture) {
  ...
  auto fakeVibronicModel =
      VibronicModel<tmatrix<double>>(lattice, parametersFakeVibronic);
  auto totalQN =
      fakeVibronicModel.total_quantum_numbers(parametersFakeVibronic);
  BOOST_CHECK_EQUAL(totalQN, 1);
}

/** Simple check on tags */
BOOST_FIXTURE_TEST_CASE(
    Test_Model_Tag_SimpleCheck_FakeVibronic, VibronicFixture
) {
  auto lattice = Lattice(parametersFakeVibronic);
  auto fakeVibronicModel =
      VibronicModel<tmatrix<double>>(lattice, parametersFakeVibronic);
...
BOOST_FIXTURE_TEST_CASE(
    Test_Model_Symbolic_Operator_FakeVibronic, VibronicFixture
) {
  auto lattice = Lattice(parametersFakeVibronic);
  auto fakeVibronicModel =
      VibronicModel<tmatrix<double>>(lattice, parametersFakeVibronic);
  BOOST_CHECK(
      fakeVibronicModel.filling_matrix_tag(0) ==
      fakeVibronicModel.get_operator_tag("fill", 0)
  );
  BOOST_CHECK(
      fakeVibronicModel.filling_matrix_tag(1) ==
      fakeVibronicModel.get_operator_tag("fill", 1)
  );
}
