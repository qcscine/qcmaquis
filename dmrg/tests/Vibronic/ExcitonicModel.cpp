...
BOOST_FIXTURE_TEST_CASE(Test_Integral_Parser_Excitonic, VibronicFixture) {
  ...
  auto integrals = Vibrational::detail::parseIntegralExcitonic<double>(
      parametersExcitonicAggregate, lattice
  );
  // Checks sizes
...
BOOST_FIXTURE_TEST_CASE(Test_Thiophene_Dimer_Excitonic, VibronicFixture) {
  ...
  auto integrals = Vibrational::detail::parseIntegralExcitonic<double>(
      parametersVibronicThiopheneDimer, lattice
  );
  // Checks sizes
  ...
  maquis::DMRGInterface<std::complex<double>> interface_single(
      parametersVibronicThiopheneDimer
  );
...
// Checks energy before and after time evolution. Value should be twice the ZPE
// of the system (given by the second line in the integral file)
#ifdef DMRG_TD
  maquis::DMRGInterface<std::complex<double>> interface_single_TD(
      parametersVibronicThiopheneDimer
  );
  double interface_single_TD_initialEnergy =
      interface_single_TD.energy().real();
  ...
  maquis::DMRGInterface<std::complex<double>> interface_double(
      parametersVibronicThiopheneDimer
  );
...
// Checks energy before and after time evolution. Value should be twice the ZPE
// of the system, since the ground and excited state PES is identical.
#ifdef DMRG_TD
  maquis::DMRGInterface<std::complex<double>> interface_double_TD(
      parametersVibronicThiopheneDimer
  );
  double interface_double_TD_initialEnergy =
      interface_double_TD.energy().real();
...
BOOST_FIXTURE_TEST_CASE(Test_Energy_Various_Nmax, VibronicFixture) {
  ...
  auto integrals = Vibrational::detail::parseIntegralExcitonicExtended<double>(
      parametersTestNmax, lattice
  );
  // Checks sizes
...
BOOST_FIXTURE_TEST_CASE(
    Test_Integral_Parser_ExtendedExcitonic, VibronicFixture
) {
#ifdef HAVE_U1
  auto lattice = Lattice(parametersExcitonicExtendedAggregate
  );  // add this to vibronic fixture
  auto integrals = Vibrational::detail::parseIntegralExcitonicExtended<double>(
      parametersExcitonicExtendedAggregate, lattice
  );
  // Checks sizes
  ...
  BOOST_CHECK_EQUAL(
      integrals.first[0].size(), 4
  );  // check if two operators tags, an electronic state and site specific tag
      // have been read in
#endif  // HAVE_U1
}
