...
BOOST_AUTO_TEST_CASE_TEMPLATE(TimeEvolversAddTime, S, symmetries) {
  ...
  // Note that here we have an additional 0.5 factor because we propagate by
  // t/2. in the l->r sweep, and then by other t/2. in the r->l sweep.
...
/** @brief Checks that the energy of an MPS is conserved after the propagation
 * of a SiteProblem */
BOOST_FIXTURE_TEST_CASE_TEMPLATE(
    TestEnergyConservationSiteproblem, S, symmetries, TestTimeEvolverFixture
) {
  // Types declaration
  using BoundaryType =
      Boundary<typename storage::constrained<cmatrix>::type, S>;
  using contr = contraction::Engine<
      cmatrix, typename storage::constrained<cmatrix>::type, S>;
  // The parameters are not really used
  DmrgParameters p;
  ...
  for (int iSite = 0; iSite < latticeSize; iSite++)
    left[iSite + 1] = contr::overlap_mpo_left_step(
        mps[iSite], mps[iSite], left[iSite], mpo[iSite]
    );
  right[latticeSize] = mps.right_boundary();
  for (int iSite = latticeSize - 1; iSite >= 0; iSite--)
    right[iSite] = contr::overlap_mpo_right_step(
        mps[iSite], mps[iSite], right[iSite + 1], mpo[iSite]
    );
  ...
  auto normAfterPropagation =
      std::sqrt(maquis::real(ietl::dot(mps[0], mps[0])));
  ...
  auto intermediateEnergy =
      maquis::real(ietl::dot(mps[0], sigmaVector)) + mpo.getCoreEnergy();
...
/** Checks that the energy of an MPS is conserved after the propagation of a
 * ZeroSiteProblem */
BOOST_FIXTURE_TEST_CASE_TEMPLATE(
    TestEnergyConservationZeroSiteproblem, S, symmetries, TestTimeEvolverFixture
) {
  // Types declaration
  using BoundaryType =
      Boundary<typename storage::constrained<cmatrix>::type, S>;
  ...
  block_matrix<
      typename alps::numeric::associated_real_diagonal_matrix<cmatrix>::type, S>
      SMat;
  ...
  auto finalEnergy =
      maquis::real(sigmaVectorBM.scalar_overlap(RMat)) + mpo.getCoreEnergy();
  BOOST_CHECK_CLOSE(initialEnergy, finalEnergy, 1e-10);
}

/** Checks that the energy decreases if the imaginary-time propagation is
 * performed */
BOOST_FIXTURE_TEST_CASE_TEMPLATE(
    TestEnergyDecreaseSiteproblem, S, symmetries, TestTimeEvolverFixture
) {
  // Types declaration
  using BoundaryType =
      Boundary<typename storage::constrained<cmatrix>::type, S>;
