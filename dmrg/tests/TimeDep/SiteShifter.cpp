...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(
  ...
  auto pointerToPerturber =
      std::make_shared<PerturberType>(left, right, mpo, p);
  auto timeEvolver = std::make_shared<TimeEvolverType>(p);
  auto siteShifter = SiteShifter<matrix, S, TimeEvolverType, PerturberType>(
      mps, timeEvolver, pointerToPerturber, 0, true
  );
