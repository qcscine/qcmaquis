...
// typedef alps::numeric::associated_real_diagonal_matrix<matrix>::type
// DiagMatrix;
...
BOOST_AUTO_TEST_CASE(BlockMatrixConstructorFromDualIndex) {
  ...
  auto block1 = typename dual_index_detail::QnBlock<TwoU1>(
      firstCharge, firstCharge, 1, 1
  );
  auto block2 = typename dual_index_detail::QnBlock<TwoU1>(
      secondCharge, secondCharge, 1, 1
  );
...
BOOST_AUTO_TEST_CASE(BlockMatrixHasBlock) {
  ...
  BOOST_CHECK_EQUAL(
      true, ba.has_block(std::make_pair(charge2, 1), std::make_pair(charge2, 3))
  );
  BOOST_CHECK_EQUAL(
      true, ba.has_block(std::make_pair(charge2, 3), std::make_pair(charge2, 1))
  );
...
BOOST_AUTO_TEST_CASE(BlockMatrixFindBlock) {
  ...
  // std::cout << "find c3 c3 is " << charge3 << " <-> " << charge3 << " == " <<
  // ba.find_block(charge3,charge3) << std::endl; std::cout << "find c3 c4 is "
  // << charge0 << " <-> " << charge0 << " == " <<
  // ba.find_block(charge0,charge0) << std::endl; std::cout << "find c3 c4 is "
  // << charge3 << " <-> " << charge4 << " == " <<
  // ba.find_block(charge3,charge4) << std::endl; std::cout << "find c3 c4 is "
  // << charge0 << " <-> " << charge4 << " == " <<
  // ba.find_block(charge0,charge4) << std::endl;
  /* FIND_BLOCK returns the data.size() AKA a "pointer" to the last elemenet for
  pairs of charges (charge3,charge4) that are not in the block matrix...*/
...
BOOST_AUTO_TEST_CASE(BlockMatrixResizeBlock) {
  ...
  for (int iRow = 0; iRow < 10; iRow++)
    for (int iCol = 0; iCol < 30; iCol++) ba[0](iRow, iCol) = 1.;
