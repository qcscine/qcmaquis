...
BOOST_AUTO_TEST_CASE(Test_Integral_Map) {
  ...
  // Two insertions of the symmetry-permuted element must yield only one element
  // in the map
  ...
  // Now try the relativistic permutation (in theory, the second value should
  // not override the first one because we don't have the full integral
  // symmetry)
