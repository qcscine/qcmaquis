...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(
    Test_MPS_Initializers_Electronic_H2, S, symmetries, H2Fixture
) {
  ...
  auto mpsTHF =
      MPS<matrix, S>(latticeSize, *(model.initializer(lattice, parametersH2)));
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(
    Test_MPS_Initializers_Electronic_Benzene, S, symmetries, BenzeneFixture
) {
  parametersBenzene.set(
      "symmetry", symm_traits::SymmetryNameTrait<S>::symmName()
  );
  ...
  auto mpsTHF = MPS<matrix, S>(
      latticeSize, *(model.initializer(lattice, parametersBenzene))
  );
  auto energyTHF = expval(mpsTHF, mpo) / norm(mpsTHF);
  parametersBenzene.set("hf_occ", "3,3,3,2,2,2");
  auto mpsSHF = MPS<matrix, S>(
      latticeSize, *(model.initializer(lattice, parametersBenzene))
  );
  auto energySHF = expval(mpsSHF, mpo) / norm(mpsSHF);
  parametersBenzene.set("init_type", "basis_state_generic");
  parametersBenzene.set("init_basis_state", "4,4,4,1,1,1");
  auto mpsTHF_g = MPS<matrix, S>(
      latticeSize, *(model.initializer(lattice, parametersBenzene))
  );
  auto energyTHF_g = expval(mpsTHF_g, mpo) / norm(mpsTHF_g);
  BOOST_CHECK_CLOSE(energyTHF, energyTHF_g, 1.0E-10);
  parametersBenzene.set("init_basis_state", "3,3,3,2,2,2");
  auto mpsSHF_g = MPS<matrix, S>(
      latticeSize, *(model.initializer(lattice, parametersBenzene))
  );
  auto energySHF_g = expval(mpsSHF_g, mpo) / norm(mpsSHF_g);
  if (symm_traits::SymmetryNameTrait<S>::symmName() == "2u1PG" ||
      symm_traits::SymmetryNameTrait<S>::symmName() ==
          "2u1")  // as this is not true for SU2
    BOOST_CHECK_CLOSE(energySHF, energySHF_g, 1.0E-10);
  parametersBenzene.set(
      "init_bond_dimension", 100
  );  // so that we don't truncate anything --> otherwise reshuffling the order
      // has an effect on the energy!
  parametersBenzene.set("init_type", "const");
  auto mpsC = MPS<matrix, S>(
      latticeSize, *(model.initializer(lattice, parametersBenzene))
  );
  auto energyC = expval(mpsC, mpo) / norm(mpsC);
  parametersBenzene.set("init_type", "basis_state_generic_const");
  parametersBenzene.set("init_space", "5,5,5,5,5,5");
  auto mps_gc = MPS<matrix, S>(
      latticeSize, *(model.initializer(lattice, parametersBenzene))
  );
  auto energy_gc = expval(mps_gc, mpo) / norm(mps_gc);
  BOOST_CHECK_CLOSE(energyC, energy_gc, 1.0E-10);
  parametersBenzene.set("init_space", "3,3,3,2,2,5");
  auto mpsSHF_gc = MPS<matrix, S>(
      latticeSize, *(model.initializer(lattice, parametersBenzene))
  );
  auto energySHF_gc = expval(mpsSHF_gc, mpo) / norm(mpsSHF_gc);
  if (symm_traits::SymmetryNameTrait<S>::symmName() == "2u1PG" ||
  ...
  parametersBenzene.set(
      "orbital_order", "6,5,4,3,2,1"
  );  // reshuffling the orbitals
  ...
  auto mpsSHFr_gc = MPS<matrix, S>(
      latticeSize, *(model_r.initializer(lattice_r, parametersBenzene))
  );
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(
    Test_MPS_Coherent_Initializer_Electronic_Benzene, S, symmetries,
    BenzeneFixture
) {
  parametersBenzene.set(
      "symmetry", symm_traits::SymmetryNameTrait<S>::symmName()
  ...
  auto mpsCoherentM = MPS<matrix, S>(
      latticeSize, *(model.initializer(lattice, parametersBenzene))
  );
 ...
 *
 * Note that the test coefficients for the coherent initializers have been taken
 * from a CISD calculation done with PySCF. We verify that we get the same
 * energy as PySCF does, and also that TwoU1 and SU2U1 give the very same
 * energy.
 */
BOOST_FIXTURE_TEST_CASE(Test_MPS_Initializers_Electronic_LiH_CISD, LiHFixture) {
  ...
  auto mpsHF_TwoU1 = MPS<matrix, TwoU1PG>(
      latticeLiH.size(),
      *(modelLiH_TwoU1.initializer(latticeLiH, parametersLiH_STO3G))
  );
  auto hfEnergy_TwoU1 = expval(mpsHF_TwoU1, mpoLiH_TwoU1);
  auto mpsHF_SU2U1 = MPS<matrix, SU2U1PG>(
      latticeLiH.size(),
      *(modelLiH_SU2U1.initializer(latticeLiH, parametersLiH_STO3G))
  );
  ...
  auto mpsCISD_TwoU1 = MPS<matrix, TwoU1PG>(
      latticeLiH.size(),
      *(modelLiH_TwoU1.initializer(latticeLiH, parametersLiH_STO3G))
  );
  // SU2U1
  ...
  auto mpsCISD_SU2U1 = MPS<matrix, SU2U1PG>(
      latticeLiH.size(),
      *(modelLiH_SU2U1.initializer(latticeLiH, parametersLiH_STO3G))
  );
  // Checks
...
BOOST_FIXTURE_TEST_CASE(Test_MPS_Initializers_SU2vs2U1, LiHFixture) {
  ...
  double scaling = 1.;
  for (const auto& excType :
       {singleExcitationsAlphaBeta, singleExcitationsBetaAlpha}) {
    for (const auto& iExc : excType) {
      parametersLiH_STO3G.set("init_type", "basis_state_generic");
      parametersLiH_STO3G.set("init_basis_state", iExc);
      auto modelLiH_SU2 =
          Model<matrix, SU2U1PG>(latticeLiH, parametersLiH_STO3G);
      auto mps_SU2 = MPS<matrix, SU2U1PG>(
          latticeLiH.size(),
          *(modelLiH_SU2.initializer(latticeLiH, parametersLiH_STO3G))
      );
      // Generates the MPS for the TwoU1 symmetry
      auto modelLiH_TwoU1 =
          Model<matrix, TwoU1PG>(latticeLiH, parametersLiH_STO3G);
      auto mps_TwoU1 = MPS<matrix, TwoU1PG>(
          latticeLiH.size(),
          *(modelLiH_TwoU1.initializer(latticeLiH, parametersLiH_STO3G))
      );
      // Transforms the SU2 MPS into the TwoU1 one
      MPS<matrix, TwoU1PG> mps_transformed =
          transform_mps<matrix, SU2U1PG>()(mps_SU2, 2, 2);
      auto overlapMPS = overlap(mps_transformed, mps_TwoU1);
      BOOST_CHECK_CLOSE(
          maquis::real(overlapMPS), scaling / std::sqrt(2.), 1.0E-10
      );
    }
    scaling = -1.;
  }
  ...
  auto mps_SU2 = MPS<matrix, SU2U1PG>(latticeLiH.size(),
  *(modelLiH_SU2.initializer(latticeLiH, parametersLiH_STO3G)));
  // Generates the MPSs for the TwoU1 symmetry
  auto modelLiH_TwoU1 = Model<matrix, TwoU1PG>(latticeLiH, parametersLiH_STO3G);
  auto mps_TwoU1 = MPS<matrix, TwoU1PG>(latticeLiH.size(),
  *(modelLiH_TwoU1.initializer(latticeLiH, parametersLiH_STO3G))); auto
  mps_transformed = transform_mps<matrix, SU2U1PG>()(mps_SU2, 2, 2); auto
  overlapMPS = overlap(mps_transformed, mps_TwoU1); std::cout <<
  std::setprecision(16); std::cout << "ALB " << overlapMPS << std::endl;
  //
  parametersLiH_STO3G.set("init_type", "basis_state_generic");
  parametersLiH_STO3G.set("init_basis_state", "2,3,2,1,1,3");
  mps_TwoU1 = MPS<matrix, TwoU1PG>(latticeLiH.size(),
  *(modelLiH_TwoU1.initializer(latticeLiH, parametersLiH_STO3G))); overlapMPS =
  overlap(mps_transformed, mps_TwoU1); std::cout << "ALB " << overlapMPS <<
  std::endl;
  //
  parametersLiH_STO3G.set("init_type", "basis_state_generic");
