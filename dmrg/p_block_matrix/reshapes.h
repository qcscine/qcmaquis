...
void reshape_left_to_right(Index<SymmGroup> physical_i,
                           ...
                           p_block_matrix<Matrix, SymmGroup> const & m1,
                           p_block_matrix<Matrix, SymmGroup> & m2)
{
//    assert(m1.right_basis() == right_i);
    ...
    m2 = p_block_matrix<Matrix, SymmGroup>();
            ...
            for (size_t l = 0; l < left_i.size(); ++l)
                for (size_t r = 0; r < right_i.size(); ++r)
                {
                    ...
                    if (!pretend) {
                        ...
                        out_block = Matrix(left_i[l].second, out_right_offset + physical_i[s].second * right_i[r].second);
        ...
        for (size_t s = 0; s < physical_i.size(); ++s)
            for (size_t l = 0; l < left_i.size(); ++l)
                for (size_t r = 0; r < right_i.size(); ++r)
                {
                    ...
                    if (!pretend) {
                        Matrix const & in_block = m1(in_l_charge, in_r_charge);
                        out_block = Matrix(out_left_offset + physical_i[s].second * left_i[l].second, right_i[r].second);
