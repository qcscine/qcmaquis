...
#include "types/dense_matrix/detail/vector_adaptor.hpp"
...
namespace alps {
	namespace hdf5 {
        ...
        void save(
                  ...
                  , maquis::types::vector<T, MemoryBlock> const & value
        ...
        void load(
                  alps::hdf5::archive & ar
                  , std::string const & path
                  , maquis::types::vector<T, MemoryBlock> & value
                  , std::vector<std::size_t> chunk = std::vector<std::size_t>()
                  , std::vector<std::size_t> offset = std::vector<std::size_t>()
                  ) {
			MemoryBlock tmp;
			ar >> make_pvp(path, tmp);
			value = maquis::types::vector<T, MemoryBlock>(tmp.begin(), tmp.end());
        }
