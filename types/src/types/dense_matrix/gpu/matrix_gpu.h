...
/**	
* Constructor m(rows) X n(columns) matrix using a maquis::types::dense_matrix, m(# of rows), n(# of columns) and ld parameters. 
* These parameters can be different of the maquis::types::dense_matrix, in case of divising the matrix B (from C = AXB)for
...
*/
	template <typename MemoryBlock>
	operator maquis::types::dense_matrix<T,MemoryBlock>()
	{
		maquis::types::dense_matrix<T,MemoryBlock> m(size1_,size2_);
...
void matrix_matrix_multiply(maquis::types::dense_matrix<T,MemoryBlock>const & Matrix_left,  maquis::types::dense_matrix<T,MemoryBlock>const & Matrix_right, maquis::types::dense_matrix<T,MemoryBlock>& Matrix_result);
